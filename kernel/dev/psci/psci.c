...
static void arm_psci_init(mdi_node_ref_t* node, uint level) {
    ...
    mdi_each_child(node, &child) {
        switch (mdi_id(&child)) {
        case MDI_KERNEL_DRIVERS_ARM_PSCI_USE_SMC:
            mdi_node_boolean(&child, &use_smc);
            break;
        case MDI_KERNEL_DRIVERS_ARM_PSCI_USE_HVC:
...
LK_PDEV_INIT(arm_psci_init, MDI_KERNEL_DRIVERS_ARM_PSCI, arm_psci_init, LK_INIT_LEVEL_PLATFORM_EARLY);
#endif // WITH_DEV_PDEV