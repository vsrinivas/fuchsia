...
static status_t pcnet_init(struct device *dev)
{
    ...
    uint32_t irq = apic_io_isa_to_global(15);
    register_int_handler(irq, pcnet_irq_handler, dev);
...
static status_t pcnet_read_pci_config(struct device *dev, pci_location_t *loc)
{
    ...
    if (config.interrupt_line != 0xff) {
        ...
        state->irq = apic_io_isa_to_global(config.interrupt_line);
    } else {
        res = ERR_NOT_CONFIGURED;
        goto error;
    }
...
#if QEMU_IRQ_BUG_WORKAROUND
    mask_interrupt(apic_io_isa_to_global(15));
#endif
...
#if QEMU_IRQ_BUG_WORKAROUND
        unmask_interrupt(apic_io_isa_to_global(15));
#endif
    }
