...
zx_status_t X86PageTableBase::SplitLargePage(PageTableLevel level, vaddr_t vaddr,
    ...
    for (int i = 0; i < NO_OF_PT_ENTRIES; i++) {
        ...
        UpdateEntry(lower_level(level), new_vaddr, e, new_paddr, flags, false /* was_terminal */);
        clf.FlushPtEntry(e);
        new_vaddr += ps;
        new_paddr += ps;
    }
    ...
    UpdateEntry(level, vaddr, pte, X86_VIRT_TO_PHYS(m), flags, true /* was_terminal */);
    ...
    for (; index != NO_OF_PT_ENTRIES && new_cursor->size != 0; ++index) {
        ...
        if (IS_LARGE_PAGE(pt_val)) {
            ...
            if (vaddr_level_aligned && new_cursor->size >= ps) {
                UnmapEntry(level, new_cursor->vaddr, e, true /* was_terminal */);
                clf.FlushPtEntry(e);
                unmapped = true;
        ...
        if (unmap_page_table) {
            ...
            UnmapEntry(level, new_cursor->vaddr, e, false /* was_terminal */);
            clf.FlushPtEntry(e);
...
bool X86PageTableBase::RemoveMappingL0(volatile pt_entry_t* table,
    ...
    *new_cursor = start_cursor;
    ...
    for (; index != NO_OF_PT_ENTRIES && new_cursor->size != 0; ++index) {
        volatile pt_entry_t* e = table + index;
        if (IS_PAGE_PRESENT(*e)) {
            UnmapEntry(PT_L, new_cursor->vaddr, e, true /* was_terminal */);
            clf.FlushPtEntry(e);
            unmapped = true;
        }
