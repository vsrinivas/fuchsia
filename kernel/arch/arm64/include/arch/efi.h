...
typedef struct {
    ...
    efi_status_t (*get_memory_map)(unsigned long* size, void* map, unsigned long* key,
                                   unsigned long* desc_size, uint32_t* desc_version);
...
struct efi_boot_ret {
    efi_zircon_hdr_t* efi_hdr;
    efi_physical_addr_t new_kernel_addr;
};

efi_boot_ret efi_boot(void* handle, efi_system_table_t* systable, uint64_t image_addr) __EXTERNALLY_VISIBLE;
