...
static void klog_print_callback(print_callback_t *cb, const char *str, size_t len)
{
    klog_puts_len(str, len);
}

static print_callback_t klog_cb = {
    .entry = { 0 },
    .print = klog_print_callback,
    .context = NULL
};

void klog_init(void)
{
    register_print_callback(&klog_cb);
}
