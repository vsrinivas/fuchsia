    ...
    bool Swap() {
        ...
        EXPECT_EQ(0u, ObjType::live_obj_count(), "");
        {
            ...
            for (size_t i = 0; i < OTHER_COUNT; ++i) {
                ...
                ContainerUtils<ContainerType>::MoveInto(other_container, utils::move(ptr));
            }
        ...
        if (PtrTraits::IsManaged) {
            while (extras_added < EXTRA_COUNT)
                ContainerUtils<ContainerType>::MoveInto(
                        container(),
                        utils::move(TestEnvTraits::CreateObject(extras_added++)));
        }
...
#else
        container() = utils::move(other_container);
#endif
