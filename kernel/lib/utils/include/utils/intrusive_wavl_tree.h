...
public:
    ...
    using CheckerType   = ::utils::tests::intrusive_containers::WAVLTreeChecker;
    ...
    typename PtrTraits::RefType front() {
        DEBUG_ASSERT(PtrTraits::IsValid(left_most_));
        return *left_most_;
    }
    ...
    typename PtrTraits::RefType back() {
        DEBUG_ASSERT(PtrTraits::IsValid(right_most_));
        return *right_most_;
    }
    ...
    void insert(PtrType&& ptr)      { internal_insert(utils::move(ptr)); }
    ...
    bool insert_or_find(PtrType&& ptr, iterator* iter = nullptr) {
        ...
        internal_insert(utils::move(ptr), &collision);
    ...
    void clear() {
        ...
        DEBUG_ASSERT(PtrTraits::IsValid(root_));
        DEBUG_ASSERT(PtrTraits::IsValid(left_most_));
        DEBUG_ASSERT(PtrTraits::IsValid(right_most_));
        ...
        while (true) {
            ...
            if ((ns.left_ == nullptr) && (ns.right_ == nullptr)) {
                ...
                *link_ptr  = nullptr;
                ...
                DEBUG_ASSERT(!PtrTraits::IsSentinel(ns.left_));
                DEBUG_ASSERT(!PtrTraits::IsSentinel(ns.right_));
...
private:
    ...
    private:
        ...
        void advance() {
            DEBUG_ASSERT(PtrTraits::IsValid(node_));
            ...
            if (rl_child != nullptr) {
                ...
                while (lr_child != nullptr) {
                    DEBUG_ASSERT(!PtrTraits::IsSentinel(lr_child));
                    node_    = lr_child;
                ...
                auto ns = &NodeTraits::node_state(*node_);
                do {
                    DEBUG_ASSERT(PtrTraits::IsValid(ns->parent_));

                    auto parent_ns = &NodeTraits::node_state(*ns->parent_);
                    done = (LRTraits::LRRawChild(*parent_ns) == node_);

                    DEBUG_ASSERT(done || (LRTraits::RLRawChild(*parent_ns) == node_));
    ...
    void internal_insert(PtrType&& ptr, RawPtrType* collision = nullptr) {
        ...
        DEBUG_ASSERT(ns.IsValid() && !ns.InContainer());
        ...
        if (root_ == nullptr) {
            ...
            DEBUG_ASSERT(PtrTraits::IsSentinel(left_most_) && PtrTraits::IsSentinel(right_most_));
            left_most_  = PtrTraits::GetRaw(ptr);
            right_most_ = PtrTraits::GetRaw(ptr);

            root_ = utils::move(ptr);
        ...
        while (true) {
            ...
            if (KeyTraits::EqualTo(key, parent_key)) {
                DEBUG_ASSERT(collision);

                if (collision) {
                    DEBUG_ASSERT(*collision == nullptr);
                    *collision = parent;
                }
        ...
        if (is_right_most) {
            DEBUG_ASSERT(PtrTraits::IsSentinel(*owner));
    ...
    PtrType internal_erase(RawPtrType ptr) {
        ...
        DEBUG_ASSERT(ns.parent_ != nullptr);
        ...
        DEBUG_ASSERT(PtrTraits::GetRaw(*owner) == ptr);
        ...
        if (PtrTraits::IsValid(ns.left_) && PtrTraits::IsValid(ns.right_)) {
            ...
            while (new_ns->left_ != nullptr) {
                DEBUG_ASSERT(!PtrTraits::IsSentinel(new_ns->left_));
        ...
        if      (PtrTraits::IsValid(ns.left_))  PromoteLRChild<ForwardTraits>(*owner, target);
            ...
            DEBUG_ASSERT(PtrTraits::IsSentinel(ns.left_)  == (left_most_  == target));
            DEBUG_ASSERT(PtrTraits::IsSentinel(ns.right_) == (right_most_ == target));

            if (PtrTraits::IsSentinel(ns.left_)) {
                if (PtrTraits::IsSentinel(ns.right_)) {
                    // Target is both left and right most.
                    DEBUG_ASSERT(count_ == 1);
                    DEBUG_ASSERT(PtrTraits::IsSentinel(ns.parent_));
                    left_most_  = sentinel();
                    right_most_ = sentinel();
    ...
    PtrType* SwapWithRightDescendant(PtrType& ptr_ref1, PtrType& ptr_ref2) {
        ...
        DEBUG_ASSERT(PtrTraits::IsValid(ns1.right_));
        ...
        if (&ptr_ref2 != &ns1.right_) {
            ...
            *ns1_rp = node2;
            ...
            DEBUG_ASSERT(ns1.parent_ == node1);
            DEBUG_ASSERT(PtrTraits::GetRaw(ns2.right_) == node2);
    ...
    void PromoteLRChild(PtrType& owner, RawPtrType node) {
        ...
        DEBUG_ASSERT(PtrTraits::IsValid(lr_child) && !PtrTraits::IsValid(rl_child));
        ...
        DEBUG_ASSERT((rl_most == node) == (PtrTraits::IsSentinel(rl_child)));
        if (PtrTraits::IsSentinel(rl_child)) {
            ...
            while (true) {
                ...
                DEBUG_ASSERT(!PtrTraits::IsSentinel(*next_rl_child));
        ...
        // nullptr by now.
        ns.parent_ = nullptr;
        DEBUG_ASSERT(ns.left_  == nullptr);
        DEBUG_ASSERT(ns.right_ == nullptr);
    }
    ...
    void FixSentinelsAfterSwap() {
        if (root_) {
            DEBUG_ASSERT(!PtrTraits::IsSentinel(root_));
            DEBUG_ASSERT(left_most_ &&  !PtrTraits::IsSentinel(left_most_));
            DEBUG_ASSERT(right_most_ && !PtrTraits::IsSentinel(right_most_));
            ...
            DEBUG_ASSERT(PtrTraits::IsSentinel(left_most_ns.left_));
            DEBUG_ASSERT(PtrTraits::IsSentinel(right_most_ns.right_));
            ...
            DEBUG_ASSERT(PtrTraits::IsSentinel(left_most_));
            DEBUG_ASSERT(PtrTraits::IsSentinel(right_most_));
            left_most_  = sentinel();
            right_most_ = sentinel();
        }
    }
    ...
    PtrType& GetLinkPtrToNode(RawPtrType node) {
        DEBUG_ASSERT(PtrTraits::IsValid(node));

        auto& ns = NodeTraits::node_state(*node);
        if (PtrTraits::IsSentinel(ns.parent_)) {
            DEBUG_ASSERT(ns.parent_ == sentinel());
            DEBUG_ASSERT(PtrTraits::GetRaw(root_) == node);
            return root_;
        }
        ...
        DEBUG_ASSERT(PtrTraits::GetRaw(parent_ns.right_) == node);
    ...
    void RotateLR(RawPtrType node, RawPtrType parent) {
        DEBUG_ASSERT(PtrTraits::IsValid(node));     // Node must be valid
        DEBUG_ASSERT(PtrTraits::IsValid(parent));   // Node must have a parent
        ...
        DEBUG_ASSERT(LRTraits::RLRawChild(Z_ns) == X);
        ...
        // X was LR-most.
        DEBUG_ASSERT(!PtrTraits::IsSentinel(Y));
    ...
    void PostInsertFixupLR(RawPtrType node, RawPtrType parent) {
        ...
        DEBUG_ASSERT(PtrTraits::IsValid(node));
        DEBUG_ASSERT(PtrTraits::IsValid(parent));

        auto& node_ns   = NodeTraits::node_state(*node);
        auto& parent_ns = NodeTraits::node_state(*parent);

        DEBUG_ASSERT(LRTraits::LRRawChild(parent_ns) == node);
    ...
    void BalancePostInsert(RawPtrType node) {
        ...
        auto node_ns = &NodeTraits::node_state(*node);
        DEBUG_ASSERT(PtrTraits::IsValid(node_ns->parent_));
    ...
    void BalancePostErase_Fix22Leaf(RawPtrType node) {
        ...
        DEBUG_ASSERT(is_left_child || (PtrTraits::GetRaw(parent_ns.right_) == node));
    ...
    void BalancePostErase_FixLR3Child(RawPtrType node) {
        ...
        while (true) {
            ...
            DEBUG_ASSERT(PtrTraits::IsValid(Y));
    ...
    void BalancePostErase_DoRotations(RawPtrType Y, RawPtrType Z) {
        using RLTraits = typename LRTraits::Inverse;
        DEBUG_ASSERT(PtrTraits::IsValid(Y));
        DEBUG_ASSERT(PtrTraits::IsValid(Z));
        ...
        if (Y_ns.rank_parity() != W_rank_parity) {
            ...
            DEBUG_ASSERT(PtrTraits::IsValid(V));                     // V must exist
            auto& V_ns = NodeTraits::node_state(*V);
            DEBUG_ASSERT(V_ns.rank_parity() != Y_ns.rank_parity());  // V must be a 1-child of Y
