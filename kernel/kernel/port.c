...
port_packet_t* buf_peek(port_buf_t* buf) {
    return buf_is_empty(buf) ? NULL : &buf->packet[buf->head];
}
...
status_t port_write(port_t port, const port_packet_t *pk, size_t count)
{
    ...
    if (wp->buf) {
        ...
        // there might be read ports. for each, write and attempt to wake a thread
...
status_t port_read(port_t port, lk_time_t timeout, port_result_t *result)
{
    ...
    if (rp->magic == READPORT_MAGIC) {
        ...
        if ((rc == ERR_TIMED_OUT) && (timeout == 0) && (rp->wport == NULL))
...
status_t port_peek(port_t port, port_result_t *result)
{
    ...
    if (rp->magic != READPORT_MAGIC) {
        ...
        port_packet_t* pp = buf_peek(rp->buf);
        if (!pp) {
            rc = rp->wport != NULL ? ERR_NOT_READY : ERR_CHANNEL_CLOSED;
        } else {
            result->ctx = rp->ctx;
            result->packet = *pp;
            rc = NO_ERROR;
...
status_t port_wait(port_t port, lk_time_t timeout) {
    ...
    if (rp->magic == READPORT_MAGIC) {
        ...
        if (!buf_is_empty(rp->buf)) {
            ...
            rc = wait_queue_block(&rp->wait, timeout);
        }
    } else if (rp->magic == PORTGROUP_MAGIC) {
        // dealing with a port group.
        port_group_t *pg = (port_group_t *)port;
...
status_t port_destroy(port_t port)
{
    ...
    if (wp->buf) {
        ...
        list_for_every_entry(&wp->rp_list, rp, read_port_t, w_node) {
            ...
            wait_queue_wake_all(&rp->wait, false, ERR_CHANNEL_CLOSED);
            if (rp->gport) {
                wait_queue_wake_all(&rp->gport->wait, false, ERR_CHANNEL_CLOSED);
            }
...
status_t port_close(port_t port)
{
    ...
    if (rp->magic == READPORT_MAGIC) {
        ...
        if (rp->wport) {
            // remove self from write port list.
