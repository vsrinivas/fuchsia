...
def parse_package_manifest(path, dst_root):
...
def parse_package_json(path, out_dir, dst_root):
  with open(path) as package_file:
    json_data = json.load(package_file)
    data = []
    for binary in json_data['binaries']:
        dst = os.path.join(dst_root, binary['bootfs_path'].lstrip('/'))
        src = os.path.join(out_dir, binary['binary'])
        data.append([dst, src])
  return data
...
def main():
  ...
  parser.add_argument(
      'package_file',
      help='JSON file containing package data. If the -m flag is used, then this is a package ' \
           'manifest generated by the build system for packages using the GN package template')
  parser.add_argument(
  ...
  parser.add_argument(
      ...
      '--manifest-file',
      action='store_true',
      help='Treat [package_file] as a package manifest generated by GN ' \
           '(e.g. out/debug-x86-64/package/foo/system_manifest)')
  parser.add_argument(
      '-v', '--verbose', action='store_true', help='Display copy filenames')

  args = parser.parse_args()

  manifest_file = args.manifest_file or False
  ...
  if manifest_file:
    package_data = parse_package_manifest(args.package_file, dst_root)
  else:
    package_data = parse_package_json(args.package_file, out_dir, dst_root)
