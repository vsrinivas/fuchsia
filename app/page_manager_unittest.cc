...
TEST_F(PageManagerTest, OnEmptyCallback) {
  ...
  PagePtr page1 = page_manager.NewPageImpl();
  PagePtr page2 = page_manager.NewPageImpl();
  page1.reset();
  page2.reset();
  message_loop_.Run();
  ...
  PagePtr page3 = page_manager.NewPageImpl();
  page3.reset();
  message_loop_.Run();
  EXPECT_TRUE(on_empty_called);
}
...
TEST_F(PageManagerTest, DeletingPageManagerClosesConnections) {
  ...
  PagePtr page = page_manager->NewPageImpl();
