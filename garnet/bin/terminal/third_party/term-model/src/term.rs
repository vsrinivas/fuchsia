...
use ansi::{self, Attr, CharsetIndex, Color, CursorStyle, Handler, NamedColor, StandardCharset};
use cell::{self, Cell, LineLength};
use color;
use config::{Config, VisualBellAnimation};
use font::{self, Size};
use grid::{BidirectionalIterator, ClearRegion, Grid, Indexed, ToRange};
use index::{self, Column, Contains, IndexRange, Line, Linear, Point, RangeInclusive};
use rgb::Rgb;
use selection::{self, Selection, Span};
...
pub struct RenderableCellsIter<'a> {
    ...
    cursor_cells: VecDeque<Indexed<Cell>>,
}
...
impl<'a> RenderableCellsIter<'a> {
    ...
    /// cursor.
    fn new<'b>(
        grid: &'b Grid<Cell>, cursor: &'b Point, colors: &'b color::List, mode: TermMode,
        config: &'b Config, selection: Option<RangeInclusive<index::Linear>>,
        ...
        RenderableCellsIter {
            ...
            config,
            colors,
            cursor_cells: VecDeque::with_capacity(3),
        }.initialize(cursor_style)
    }

    fn push_cursor_cells(&mut self, original_cell: Cell, cursor_cell: Cell, wide_cell: Cell) {
        ...
        match fg {
            ...
            Color::Named(ansi) => {
                match (
                    ...
                    (true, cell::Flags::DIM_BOLD) | (true, cell::Flags::BOLD) => {
                        self.colors[ansi.to_bright()]
                    }
                    // Cell is marked as dim and not bold
                    (_, cell::Flags::DIM) => self.colors[ansi.to_dim()],
...
impl Term {
    ...
    pub fn renderable_cells<'b>(
        &'b self, config: &'b Config, selection: Option<&'b Selection>, window_focused: bool,
    ) -> RenderableCellsIter {
    ...
    pub fn resize(&mut self, size: &SizeInfo) {
        ...
        if num_lines > old_lines {
            ...
            self.grid
                .clear_region((self.cursor.point.line + 1).., |c| c.reset(&template));
            self.alt_grid
                .clear_region((self.cursor_save_alt.point.line + 1).., |c| {
                    c.reset(&template)
                });
        }
    }
    ...
    fn scroll_down_relative(&mut self, origin: Line, lines: Line) {
        ...
        // Clear `lines` lines at bottom of area
        {
            ...
            self.grid
                .clear_region(start..self.scroll_region.end, |c| c.reset(&template));
        }

        // Scroll between origin and bottom
        self.grid
            .scroll_down(&(origin..self.scroll_region.end), lines);
    }
    ...
    fn scroll_up_relative(&mut self, origin: Line, lines: Line) {
        ...
        self.grid
            .scroll_up(&(origin..self.scroll_region.end), lines);
    }
...
#[cfg(test)]
mod tests {
    use super::{SizeInfo, Term, FONT_SIZE_STEP};

    use ansi::{CharsetIndex, Handler, StandardCharset};
    use cell::{self, Cell};
    use config::Config;
    use font::Size;
    use grid::Grid;
