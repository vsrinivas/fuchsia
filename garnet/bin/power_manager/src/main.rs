...
use fidl_fuchsia_power::{
    BatteryStatus, PowerManagerRequest, PowerManagerRequestStream, PowerManagerWatcherProxy,
...
impl BatteryStatusHelper {
    ...
    fn update_battery_status(
        &mut self,
        power_info: power::ioctl_power_get_info_t,
...
fn main_pm() -> Result<(), Error> {
    ...
    fs.dir("public").add_fidl_service(move |stream| {
        let pm = PowerManagerServer { battery_status_helper: bsh.clone() };
...
mod tests {
    ...
    fn update_battery_status() {
        ...
        let mut power_info =
            power::ioctl_power_get_info_t { power_type: power::POWER_TYPE_AC, state: 0 };
