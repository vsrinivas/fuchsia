...
pub trait Responder<R: From<Response> + Send + Sync + 'static> {
...
impl<R: From<Response> + Send + Sync + 'static, T: Responder<R> + Send + Sync + 'static>
    Work<R, T>
{
    pub fn new(setting_type: SettingType, request: Request, responder: T) -> Self {
...
mod tests {
    ...
    async fn test_request_basic_functionality() {
        ...
        fasync::Task::spawn(async move {
            work.execute(work_messenger).await;
        })
        .detach();
