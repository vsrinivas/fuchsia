...
async fn test_keyboard() {
    let initial_value = KeyboardInfo { keymap: Some(KeymapId::UsQwerty) };
    let changed_value = KeyboardInfo { keymap: Some(KeymapId::UsDvorak) };
    ...
    let watch_result = settings.keymap.map(|id| KeymapId::try_from(id).unwrap());
    assert_eq!(watch_result, initial_value.keymap);
    ...
    let settings = keyboard_service.watch().await.expect("watch completed");
    let watch_result = settings.keymap.map(|id| KeymapId::try_from(id).unwrap());
    assert_eq!(watch_result, changed_value.keymap);
}
