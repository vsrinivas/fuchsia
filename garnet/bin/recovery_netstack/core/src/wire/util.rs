...
mod checksum {
    ...
    impl Checksum {
        // Minimum number of bytes in a buffer to run the SIMD algorithm. Running the
        // algorithm with less than `MIN_BYTES_FOR_SIMD` bytes will cause the
        // benefits of SIMD to be dwarfed by the overhead (performing worse than the
        // normal/non-SIMD algorithm). This value was chosen after several benchmarks
        // which showed that the algorithm performed worse than the normal/non-simd
        // algorithm when the number of bytes was less than 64.
        ...
        /// architecture-specific SIMD instructions. It may not process all
        /// bytes, and whatever bytes are not processed will be returned. If
        /// no implementation exists for the target architecture and run-time
        /// CPU features, `add_bytes_simd` does nothing and simply returns
        /// `bytes` directly.
        #[inline(always)]
        ...
        unsafe fn add_bytes_x86_64<'a>(sum: &mut u32, mut bytes: &'a [u8]) -> &'a [u8] {
            ...
            while bytes.len() >= 32 {
                ...
                // accumulator because we are using 32bit integers which can hold
                // a max value of U32_MAX (2^32 - 1), and we are adding 16bit values
                // with a max value of U16_MAX (2^16 - 1).
                // U32_MAX = (U16_MAX << 16 + U16_MAX) = U16_MAX * (2 ^ 16 + 1)
                while bytes.len() >= 32 && add_count <= u32::from(std::u16::MAX) {
                    // Load 32 bytes from memory (16 16bit values to add to `sum`)
                    //
                    // `_mm256_lddqu_si256` does not require the memory address to be
                    // aligned so remove the linter check for casting from a less
                    ...
                    // Create 32bit words with most significant 16 bits = 0,
                    // least significant 16 bits set to a new 16 bit word
                    // to add to checksum from bytes. Setting the most significant
                    // 16 bits to 0 allows us to do 2^16 simd additions (2^20 16bit
                    // word additions) without worrying about overflows.
                    ...
                    // We did 2 additions to the accumulator in this iteration
                    // of the loop.
                    add_count += 2;
                ...
                // Store the results of our accumlator of 8x 32bit words to our temporary
                // buffer `data` so that we can iterate over data 16 bits at a time and
                // add the values to `sum`. Since `acc` is a 256bit value, it requires
                // 32 bytes, provided by `data`.
                //
        ...
        unsafe fn update_x86_64<'a, 'b>(
            ...
            // temporary variable `old_sum`. We can then add it as a normal
            // 16bit word to the current sum (`sum`) after normalizng it and
            // getting the 1s complement. This will 'remove' `old_bytes` from
            // `sum`.
            let mut old_sum = 0;
            ...
            // We should have the exact same number of bytes left over
            // for both `new_bytes` and `old_bytes`.
            assert_eq!(new_bytes.len(), old_bytes.len());
    ...
    mod benchmarks {
        // Benchmark results for comparing checksum calculation with and
        // without SIMD implementation, running on Google's Pixelbook.
        // Average values were calculated over 3 trials.
        //
        // Number of | Average time  | Average time | % Diff
        //   bytes   | (ns) w/o SIMD | (ns) w/ SIMD |
        // --------------------------------------------------
        //        31 |          3657 |         3692 |   1.01
        //        32 |          3735 |         3767 |   1.01
        //        64 |          7092 |         6580 |   0.93
        ...
        /// Benchmark time to calculate checksum with
        /// a single call to `add_bytes` with 31 bytes.
        #[bench]
        ...
        /// Benchmark time to calculate checksum with
        /// a single call to `add_bytes` with 32 bytes.
        #[bench]
        ...
        /// Benchmark time to calculate checksum with
        /// a single call to `add_bytes` with 64 bytes.
        #[bench]
        ...
        /// Benchmark time to calculate checksum with
        /// a single call to `add_bytes` with 128 bytes.
        #[bench]
        ...
        /// Benchmark time to calculate checksum with
        /// a single call to `add_bytes` with 256 bytes.
        #[bench]
        ...
        /// Benchmark time to calculate checksum with
        /// a single call to `add_bytes` with 1024 bytes.
        #[bench]
    ...
    mod tests {
        ...
        fn test_checksum() {
            ...
            // enough to test implementation with simd instructions
            // and cause overflow within the implementation.
            let mut c = Checksum::new();
        ...
        fn test_checksum_simd_rand() {
            ...
            // Test simd implementation with random values and buffer big
            // enough to cause an overflow within the implementation..
            // 2 bytes/word * 8 words/additions * 2^16 additions/overflow
            ...
            let single_bytes = {
                // Add 1 byte at a time to make sure we do not enter
            ...
            let all_bytes = {
                // Calculate checksum with same buffer, but this time
                // test the implementation with simd instructions
                let mut c = Checksum::new();
                c.add_bytes(&buf);
                c.checksum()
            };
        ...
        fn test_update() {
            ...
            // Test update with bytes big enough
            // to test simd implementation with overflow.
            const BUF_LEN: usize = (1 << 20) + 79;
        ...
        fn test_update_simd_rand() {
            ...
            // Test updating with random values and update size
            // big enough to test simd implementation with overflow
            ...
            let from_update =
                Checksum::update(orig_checksum, &buf[begin..end], &new_buf[begin..end]);
            let from_scratch = {
                let mut c = Checksum::new();
                c.add_bytes(&new_buf);
                c.checksum()
