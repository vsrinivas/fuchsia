...
bool SendAllBytes(zx::unowned_socket socket, const uint8_t* buf,
                  uint32_t buf_size) {
  uint32_t msg_size = htole32(buf_size);
  size_t actual = 0;
  zx_status_t status;

  status = socket->write(0, reinterpret_cast<char*>(&msg_size),
                         sizeof(msg_size), &actual);
...
bool SendMessage(zx::unowned_socket socket, const MessageLite& message) {
  ...
  return SendAllBytes(std::move(socket), buf, msg_size);
}

ssize_t RecvSockBlocking(zx::unowned_socket socket, uint8_t* buf,
                         uint32_t buf_size) {
  size_t bytes_left;
  size_t actual;
  zx_status_t status;
  ...
  while (bytes_left > 0) {
    status =
        socket->wait_one(ZX_SOCKET_READABLE, zx::time::infinite(), nullptr);
    if (status != ZX_OK) {
...
ssize_t RecvAllBytes(zx::unowned_socket socket, uint8_t* buf,
                     uint32_t buf_size) {
  uint32_t msg_size;

  // Receive the message's size
  if (RecvSockBlocking(zx::unowned_socket(socket),
  ...
  return RecvSockBlocking(std::move(socket), buf, msg_size);
}

bool RecvMessage(zx::unowned_socket socket, MessageLite* message) {
  ssize_t count;
  uint8_t buf[kMaxMessageSize];

  count = RecvAllBytes(std::move(socket), buf, sizeof(buf));
  if (count < 0) {
    return false;
  }
