...
RawAudio RawAudio::FromAUFile(const std::string& filename) {
  ...
  FXL_CHECK(input_file.is_open()) << "Could not open " << filename << " for reading.";

  const size_t file_size = input_file.tellg();
  input_file.seekg(0);

  FXL_CHECK(file_size > sizeof(uint32_t) * 6) << "File is not big enough for AU header.";
  ...
  FXL_CHECK(au_header[0] == kAUHeaderMagicConstant) << filename << " is not an AU file.";
  FXL_CHECK(au_header[3] == kLinear16BitSampleCode) << "Only 16 bit linear samples are supported.";

  const uint32_t data_offset = au_header[1];
  const uint32_t data_size = au_header[2];
  FXL_CHECK(data_size != kDataSizeUnknown && data_size <= file_size - data_offset);
  ...
  return RawAudio({.frequency = frequency, .channels = channels}, std::move(data));
}

RawAudio::CodecInput RawAudio::BuildCodecInput(size_t max_frames_per_packet) const {
  ...
  fuchsia::media::PcmFormat pcm_format{.pcm_mode = fuchsia::media::AudioPcmMode::LINEAR,
                                       .bits_per_sample = 16,
  ...
  return {
      .data = data_, .payload_offsets = std::move(payload_offsets), .format = std::move(format)};
}
