...
class DockyardProxyGrpc : public DockyardProxy {
 public:
  ...
  // |DockyardProxy|.
  virtual DockyardProxyStatus SendSample(const std::string& dockyard_path, uint64_t value) override;
 ...
 private:
  ...
  grpc::Status SendInspectJsonById(uint64_t time, dockyard::DockyardId dockyard_id,
                                   const std::string& json);

  // Actually send a single sample to the Dockyard.
  // |time| is in nanoseconds.
  // See also: SendSample().
  grpc::Status SendSampleById(uint64_t time, dockyard::DockyardId dockyard_id, uint64_t value);
  ...
  grpc::Status GetDockyardIdsForPaths(std::vector<dockyard::DockyardId>* dockyard_id,
