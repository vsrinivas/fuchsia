...
DockyardProxyStatus DockyardProxyGrpc::SendInspectJson(
  ...
  if (status.ok()) {
    return ToDockyardProxyStatus(
        SendInspectJsonById(nanoseconds, stream_id, json));
  }
...
DockyardProxyStatus DockyardProxyGrpc::SendSample(
  ...
  grpc::Status status = GetStreamIdForName(&stream_id, stream_name);
  if (status.ok()) {
    return ToDockyardProxyStatus(SendSampleById(nanoseconds, stream_id, value));
  }
  return ToDockyardProxyStatus(status);
}
...
grpc::Status DockyardProxyGrpc::SendSampleById(
    uint64_t time, dockyard::SampleStreamId stream_id, uint64_t value) {
  // Data we are sending to the server.
...
grpc::Status DockyardProxyGrpc::GetStreamIdForName(
