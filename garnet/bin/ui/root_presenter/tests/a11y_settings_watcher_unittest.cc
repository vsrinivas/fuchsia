...
TEST_F(A11ySettingsWatcherTest, HasColorConversionChanged_NoChange) {
  // Create Scenic and Session.
  scenic::ResourceId id = 1;
  fuchsia::ui::scenic::SessionPtr session_ptr;
  fidl::InterfaceHandle<fuchsia::ui::scenic::SessionListener> listener_handle;
...
TEST_F(A11ySettingsWatcherTest, HasColorConversionChanged_ColorCorrectionChanged) {
  // Create Scenic and Session.
  scenic::ResourceId id = 1;
  fuchsia::ui::scenic::SessionPtr session_ptr;
  fidl::InterfaceHandle<fuchsia::ui::scenic::SessionListener> listener_handle;
  ...
  Settings provided_settings_clone;
  provided_settings.Clone(&provided_settings_clone);
  settings_watcher->OnSettingsChange(std::move(provided_settings_clone));

  // Calling HasColorConversionChanged with same settings should result in
  // false.
  ASSERT_FALSE(settings_watcher->HasColorConversionChanged(provided_settings));
}

TEST_F(A11ySettingsWatcherTest, HasColorConversionChanged_ColorInversionChanged) {
  // Create Scenic and Session.
  scenic::ResourceId id = 1;
  fuchsia::ui::scenic::SessionPtr session_ptr;
  fidl::InterfaceHandle<fuchsia::ui::scenic::SessionListener> listener_handle;
