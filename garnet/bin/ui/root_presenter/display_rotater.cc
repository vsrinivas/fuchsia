...
void DisplayRotater::SetDisplayRotation(Presentation* p, float display_rotation_degrees,
                                        bool animate) {
  if (animate) {
...
bool DisplayRotater::UpdateAnimation(Presentation* p, uint64_t presentation_time) {
  if (spring_.is_done()) {
    return false;
  }
...
glm::vec2 DisplayRotater::RotatePointerCoordinates(Presentation* p, float x, float y) {
  ...
  const float anchor_w = p->display_model_actual_.display_info().width_in_px / 2;
  const float anchor_h = p->display_model_actual_.display_info().height_in_px / 2;
  ...
  FXL_VLOG(2) << "Pointer coordinates rotated [" << rotation << "=" << current_rotation << "-"
