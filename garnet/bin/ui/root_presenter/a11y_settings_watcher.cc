...
void A11ySettingsWatcher::OnSettingsChange(fuchsia::accessibility::Settings settings) {
  if (HasColorConversionChanged(settings)) {
...
void A11ySettingsWatcher::InitColorConversionCmd(
  ...
  if (settings_.has_color_adjustment_matrix()) {
...
bool A11ySettingsWatcher::HasColorConversionChanged(
    const fuchsia::accessibility::Settings& provided_settings) {
  // Check if Color Correction Setting has changed.
  if ((settings_.has_color_correction() &&
       (settings_.color_correction() != fuchsia::accessibility::ColorCorrection::DISABLED)) !=
      (provided_settings.has_color_correction() &&
  ...
  if ((settings_.has_color_inversion_enabled() && settings_.color_inversion_enabled()) !=
      (provided_settings.has_color_inversion_enabled() &&
       provided_settings.color_inversion_enabled())) {
    return true;
  }
