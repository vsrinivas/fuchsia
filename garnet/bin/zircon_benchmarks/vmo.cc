  ...
  while (state->KeepRunning()) {
    ...
    if (do_map) {
      ZX_ASSERT(zx::vmar::root_self()->map(0, vmo, 0, copy_size,
                                           ZX_VM_MAP_RANGE | ZX_VM_PERM_READ,
                                           &addr) == ZX_OK);
      state->NextStep();
    }

    zx::vmo clone;
    ZX_ASSERT(vmo.create_child(ZX_VMO_CHILD_COPY_ON_WRITE, 0, copy_size,
                               &clone) == ZX_OK);
    state->NextStep();
...
bool VmoCloneReadOrWriteTest(perftest::RepeatState* state, uint32_t copy_size,
                             bool do_write, bool do_target_clone,
                             bool do_full_op) {
  state->DeclareStep("clone");
...
void RegisterVmoTest(const char* name, Func fn, Args... args) {
  for (unsigned size_in_kbytes : {128, 512, 2048}) {
    ...
    perftest::RegisterTest(full_name.c_str(), fn, size_in_kbytes * 1024,
                           args...);
  }
}
  ...
  for (bool do_write : {false, true}) {
    for (bool do_target_clone : {false, true}) {
      for (bool do_full_op : {false, true}) {
        ...
        auto clone_rw_name =
            fbl::StringPrintf("Vmo/Clone/%s%s%s", rw, target, density);
        RegisterVmoTest(clone_rw_name.c_str(), VmoCloneReadOrWriteTest,
                        do_write, do_target_clone, do_full_op);
      }
    }
  }
}
