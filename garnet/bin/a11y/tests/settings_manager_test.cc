...
class SettingsManagerUnitTest : public sys::testing::TestWithContext {
 public:
  ...
  void SetUp() override {
    ...
    controller().AddService<SettingsManager>(
  ...
  void VerifySettings(const ExpectedSettings& expected_settings) {
    ...
    Settings settings;
    settings_manager_.GetSettings([&status, &settings]
      (SettingsManagerStatus ret_status, Settings ret_settings) {
        status = ret_status;
...
TEST_F(SettingsManagerUnitTest, SetMagnificationEnabledFalseCurrentlyFalse) {
  ...
  settings_manager_.SetMagnificationEnabled(false,
...
TEST_F(SettingsManagerUnitTest, SetMagnificationEnabledTrueCurrentlyFalse) {
    ...
    settings_manager_.SetMagnificationEnabled(true,
        [&status](SettingsManagerStatus retval) {status = retval; });
...
TEST_F(SettingsManagerUnitTest, SetMagnificationEnabledTrueCurrentlyTrue) {
    ...
    settings_manager_.SetMagnificationZoomFactor(2.0,
        [&status](SettingsManagerStatus retval) { status = retval; });
    RunLoopUntilIdle();
    ASSERT_EQ(status, SettingsManagerStatus::OK);
...
TEST_F(SettingsManagerUnitTest, SetMagnificationZoomFactorInvalidValue) {
  ...
  settings_manager_.SetMagnificationZoomFactor(0,
      [&status](SettingsManagerStatus retval) { status = retval; });
  RunLoopUntilIdle();
  ASSERT_EQ(status, SettingsManagerStatus::ERROR);
...
TEST_F(SettingsManagerUnitTest, SetScreenReaderEnabled) {
  ...
  settings_manager_.SetScreenReaderEnabled(true,
      [&status](SettingsManagerStatus retval) { status = retval; });
  RunLoopUntilIdle();
  ASSERT_EQ(status, SettingsManagerStatus::OK);
...
TEST_F(SettingsManagerUnitTest, SetColorInversionEnabled) {
  ...
  settings_manager_.SetColorInversionEnabled(true,
      [&status](SettingsManagerStatus retval) { status = retval; });
  RunLoopUntilIdle();
  ASSERT_EQ(status, SettingsManagerStatus::OK);
...
TEST_F(SettingsManagerUnitTest, SetColorCorrection) {
  ...
  settings_manager_.SetColorCorrection(ColorCorrection::CORRECT_PROTANOMALY,
      [&status](SettingsManagerStatus retval) { status = retval; });
  RunLoopUntilIdle();
  ASSERT_EQ(status, SettingsManagerStatus::OK);
