...
#include "garnet/lib/magma/tests_linux/unit_tests/basic_compute.h"
...
TEST_F(VirtMagmaTest, BasicCompute)
{
    ...
    for (auto& physical_device : physical_devices_) {
        ...
        vkGetPhysicalDeviceMemoryProperties(physical_device.device,
        ...
        compute_pipeline_create_info.stage.sType =
        ...
        VkPipeline pipeline{};
        result = vkCreateComputePipelines(device, nullptr, 1, &compute_pipeline_create_info,
                                          nullptr, &pipeline);
