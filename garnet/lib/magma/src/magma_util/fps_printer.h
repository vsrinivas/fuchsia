 ...
 public:
  void OnNewFrame() {
    ...
    if (elapsed.count() > kSlowFrameMs) {
      magma::log(magma::LOG_INFO, "Extra slow frame detected (> %u ms), restarting fps measurement",
                 kSlowFrameMs);
      t0_ = t1;
      return;
    }
    ...
    if (elapsed_frames_ && (elapsed_frames_ % num_frames_) == 0) {
      ...
      magma::log(magma::LOG_INFO, "Framerate average for last %u frames: %.2f frames per second",
                 num_frames_, fps);
      total_ms_ = 0;
