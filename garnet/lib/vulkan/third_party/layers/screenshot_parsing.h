...
typedef struct {
  ...
  int count;  // if the value is SCREEN_SHOT_FRAMES_UNLIMITED, it means unlimited screenshots until
              // capture/playback to end.
  int interval;
} FrameRange;
...
// return:
// return 0 if parsing rangeString successfully, other value is a status value indicating a
// specified error was encountered, currently support the following values:
...
// detect if the input command line option _vk_screenshot is definition of frame range or a frame
// list.
