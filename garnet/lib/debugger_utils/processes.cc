...
static zx_status_t TryGetProcessThreadKoids(zx_handle_t process, size_t try_count,
  ...
  for (size_t i = 0; i < try_count; ++i) {
    ...
    size_t num_threads = num_available_threads + max_num_new_threads;
    out_threads->resize(num_threads);

    size_t records_read;
    zx_status_t status = zx_object_get_info(process, ZX_INFO_PROCESS_THREADS, out_threads->data(),
...
zx_status_t TryGetProcessThreadKoidsForTesting(const zx::process& process, size_t try_count,
  ...
  return TryGetProcessThreadKoids(process.get(), try_count, initial_num_threads,
                                  max_num_new_threads, out_threads, out_num_available_threads);
}
...
zx_status_t GetProcessThreadKoids(const zx_handle_t process, size_t try_count,
  ...
  status = TryGetProcessThreadKoids(process, try_count, num_available_threads, max_num_new_threads,
...
zx_status_t GetProcessThreadKoids(const zx::process& process, size_t try_count,
                                  size_t max_num_new_threads, std::vector<zx_koid_t>* out_threads,
                                  size_t* out_num_available_threads) {
  return GetProcessThreadKoids(process.get(), try_count, max_num_new_threads, out_threads,
...
zx_status_t CreateProcessBuilder(const zx::job& job, const std::string& path,
  ...
  auto builder =
      std::make_unique<process::ProcessBuilder>(std::move(builder_job), std::move(services));
