...
/// A perfectly valid package signing key.
pub const TEST_SIGNING_KEY: &PackageSigningKey = include_bytes!("test_key.bin");
...
pub struct PackageBuilder {
    ...
    signing_key: &'static PackageSigningKey,
}

impl PackageBuilder {
    /// Creates a new `PackageBuilder`.
    pub fn new(name: impl Into<String>) -> Self {
        Self { name: name.into(), contents: BTreeMap::new(), signing_key: TEST_SIGNING_KEY }
    }
    ...
    /// Sets the signing key to use when signing the package.
    ///
    /// By default, it uses a built-in test key.
    pub fn signing_key(mut self, key: &'static PackageSigningKey) -> Self {
    ...
    pub async fn build(self) -> Result<Package, Error> {
        ...
        fs::write(indir.path().join("signing.key"), &self.signing_key[..])?;
...
mod tests {
    ...
    async fn test_basic() -> Result<(), Error> {
        ...
        assert_eq!(
            pkg.meta_far_merkle,
            "f9a58f043f07e4ad6347db7e043c7e27f83e1b06199922502e26e72675c9916a".parse()?
        );
    ...
    async fn test_identity() -> Result<(), Error> {
        let mut pkg =
            PackageBuilder::new("fuchsia_pkg_testing_tests").signing_key(&DEVELOPMENT_SIGNING_KEY);
