...
pub struct Predicate<T> {
    inner: Arc<dyn Fn(&T) -> bool>,
    /// A descriptive piece of text used for debug printing via `{:?}`
    description: String
}

impl<T> Clone for Predicate<T> {
    fn clone(&self) -> Predicate<T> { Predicate { inner: self.inner.clone(), description: self.description.clone() } }
}
...
impl<T: 'static> Predicate<T> {
    ...
    pub fn not(self) -> Predicate<T> {
        ...
        Predicate {
            inner: Arc::new(move |t: &T| -> bool { !(self.inner)(t) }),
            description,
        }
    }

    pub fn new<F: Fn(&T) -> bool + 'static>(f: F, label: Option<&str>) -> Predicate<T> {
        Predicate {
            inner: Arc::new(f),
...
pub mod peer {
    ...
    pub fn name(expected_name: &str) -> Predicate<RemoteDevice> {
        ...
        Predicate::<RemoteDevice>::new(move |peer| peer.name == name, Some(&format!("name == {}", expected_name)))
    }
    pub fn address(expected_address: &str) -> Predicate<RemoteDevice> {
        let address = expected_address.to_string();
        Predicate::<RemoteDevice>::new(move |peer| peer.address == address, Some(&format!("address == {}", expected_address)))
    }
    pub fn technology(tech: TechnologyType) -> Predicate<RemoteDevice> {
        Predicate::<RemoteDevice>::new(move |peer| peer.technology == tech, Some(&format!("technology == {:?}", tech)))
    }
    pub fn connected(connected: bool) -> Predicate<RemoteDevice> {
        Predicate::<RemoteDevice>::new(move |peer| peer.connected == connected, Some(&format!("connected == {}", connected)))
    }
    pub fn bonded(bonded: bool) -> Predicate<RemoteDevice> {
        Predicate::<RemoteDevice>::new(move |peer| peer.bonded == bonded, Some(&format!("bonded == {}", bonded)))
    }
}
...
pub mod host_driver {
    ...
    pub fn name(expected_name: &str) -> Predicate<AdapterState> {
        let name = Some(expected_name.to_string());
        Predicate::<AdapterState>::new(move |host_driver| host_driver.local_name == name, Some(&format!("name == {}", expected_name)))
    }
    pub fn discovering(discovering: bool) -> Predicate<AdapterState> {
        Predicate::<AdapterState>::new(move |host_driver| host_driver.discovering == Some(Box::new(Bool { value: discovering })), Some(&format!("discovering == {}", discovering)))
    }
    pub fn discoverable(discoverable: bool) -> Predicate<AdapterState> {
        Predicate::<AdapterState>::new(move |host_driver| host_driver.discoverable == Some(Box::new(Bool { value: discoverable })), Some(&format!("discoverable == {}", discoverable)))
    }
}
...
mod test {
    ...
    use fidl_fuchsia_bluetooth_control::{RemoteDevice, TechnologyType, Appearance};
    ...
    fn correct_name() -> Predicate<RemoteDevice> { peer::name(TEST_PEER_NAME) }
    fn incorrect_name() -> Predicate<RemoteDevice> { peer::name(INCORRECT_PEER_NAME) }
    fn correct_address() -> Predicate<RemoteDevice> { peer::address(TEST_PEER_ADDRESS) }
    fn incorrect_address() -> Predicate<RemoteDevice> { peer::address(INCORRECT_PEER_ADDRESS) }
    ...
    fn simple_predicate_succeeds() {
        let predicate = Predicate::<RemoteDevice>::new(move |peer| peer.name == Some(TEST_PEER_NAME.into()), None);
        assert!(predicate.satisfied(&test_peer()));
    }
    #[test]
    fn simple_incorrect_predicate_fail() {
        let predicate = Predicate::<RemoteDevice>::new(move |peer| peer.name == Some("INCORRECT_NAME".into()), None);
        assert!(!predicate.satisfied(&test_peer()));
    }
