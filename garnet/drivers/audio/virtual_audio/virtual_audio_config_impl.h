...
#ifndef GARNET_DRIVERS_AUDIO_VIRTUAL_AUDIO_VIRTUAL_AUDIO_CONFIG_IMPL_H_
#define GARNET_DRIVERS_AUDIO_VIRTUAL_AUDIO_VIRTUAL_AUDIO_CONFIG_IMPL_H_
...
class VirtualAudioConfigImpl {
 public:
  ...
  VirtualAudioConfigImpl();

  void Init();

  // virtualaudio.Config interface
  void SetDeviceName(const std::string& device_name);
  void SetManufacturer(const std::string& manufacturer_name);
  void SetProduct(const std::string& product_name);
  void SetUniqueId(const fidl::Array<uint8_t, 16>& unique_id);
  ...
  void SetFifoDepth(uint32_t fifo_depth_bytes);
  void SetExternalDelay(zx_duration_t external_delay);
...
#endif  // GARNET_DRIVERS_AUDIO_VIRTUAL_AUDIO_VIRTUAL_AUDIO_CONFIG_IMPL_H_
