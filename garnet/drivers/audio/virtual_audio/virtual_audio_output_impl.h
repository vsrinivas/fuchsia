...
#ifndef GARNET_DRIVERS_AUDIO_VIRTUAL_AUDIO_VIRTUAL_AUDIO_OUTPUT_IMPL_H_
#define GARNET_DRIVERS_AUDIO_VIRTUAL_AUDIO_VIRTUAL_AUDIO_OUTPUT_IMPL_H_
...
class VirtualAudioOutputImpl : public VirtualAudioDeviceImpl,
                               public fuchsia::virtualaudio::Output {
 public:
  static fbl::unique_ptr<VirtualAudioOutputImpl> Create(
      VirtualAudioControlImpl* owner) {
    return fbl::unique_ptr<VirtualAudioOutputImpl>(
        new VirtualAudioOutputImpl(owner));
  }

  bool CreateStream(zx_device_t* devnode) override {
    stream_ = ::audio::SimpleAudioStream::Create<VirtualAudioStreamOut>(
        this, devnode);
  ...
  void AddFormatRange(uint32_t sample_format_flags, uint32_t min_frame_ate,
                      uint32_t max_frame_rate, uint8_t min_channels,
                      uint8_t max_channels,
                      uint16_t rate_family_flags) override {
    VirtualAudioConfigImpl::AddFormatRange(sample_format_flags, min_frame_ate,
  ...
  //
  // virtualaudio.Output interface
  //
  // ... empty, for now

 private:
  friend class fbl::unique_ptr<VirtualAudioOutputImpl>;

  explicit VirtualAudioOutputImpl(VirtualAudioControlImpl* owner)
      : VirtualAudioDeviceImpl(owner) {}
};

}  // namespace virtual_audio

#endif  // GARNET_DRIVERS_AUDIO_VIRTUAL_AUDIO_VIRTUAL_AUDIO_OUTPUT_IMPL_H_
