...
std::unique_ptr<GpuMapping> AddressSpace::MapBufferGpu(std::shared_ptr<AddressSpace> address_space,
  ...
  if (offset + length > buffer->platform_buffer()->size())
    return DRETP(nullptr, "offset (0x%lx) + length (0x%lx) > buffer size (0x%lx)", offset, length,
  ...
  DLOG("MapBufferGpu offset 0x%lx length 0x%lx allocated gpu_addr 0x%lx", offset, length, gpu_addr);
  ...
  if (address_space->type() == ADDRESS_SPACE_PPGTT) {
    bus_mapping = address_space->owner_->GetBusMapper()->MapPageRangeBus(buffer->platform_buffer(),
...
magma::Status AddressSpace::GrowMapping(GpuMapping* mapping, uint64_t page_increment) {
  ...
  if (mapping->gpu_addr() + length > Size())
    return DRET_MSG(MAGMA_STATUS_INVALID_ARGS,
                    "gpu_addr 0x%lx + length %lu > address space size (0x%lx)", mapping->gpu_addr(),
                    length, Size());
  ...
  if (mapping->offset() + length > platform_buffer->size())
    return DRET_MSG(MAGMA_STATUS_INVALID_ARGS, "offset (%lu) + length (%lu) > buffer size (0x%lx)",
                    mapping->offset(), length, platform_buffer->size());
