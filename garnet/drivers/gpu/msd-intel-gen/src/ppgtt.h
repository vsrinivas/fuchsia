   ...
   public:
    ...
    PageTable* page_table(uint32_t index, bool alloc) {
      ...
      if (!page_tables_[index]) {
        ...
        page_directory_table_gpu()->entry[index] = gen_pde_encode(page_tables_[index]->bus_addr());
      }
   ...
   public:
    ...
    PageDirectory* page_directory(uint32_t index, bool alloc) {
      ...
      if (!page_directories_[index]) {
        ...
        page_directories_[index] = PageDirectory::Create(owner(), scratch_dir_->scratch_table());
        if (!page_directories_[index])
    ...
    gen_pte_t* page_table_entry(uint32_t page_directory_ptr_index, uint32_t page_directory_index,
    ...
    static std::unique_ptr<PageDirectoryPtrTable> Create(
        Owner* owner, std::shared_ptr<PageDirectory> scratch_dir);
  ...
  class Pml4Table : public Page {
   public:
    ...
    PageDirectoryPtrTable* page_directory_ptr(uint32_t index, bool alloc) {
      ...
      if (!directory_ptrs_[index]) {
        ...
        pml4_table_gpu()->entry[index] = gen_pml4_encode(directory_ptrs_[index]->bus_addr());
      }
   ...
   private:
    Pml4Table(uint64_t scratch_page_bus_addr,
        ...
        : directory_ptrs_(kPml4Entries + 1),
