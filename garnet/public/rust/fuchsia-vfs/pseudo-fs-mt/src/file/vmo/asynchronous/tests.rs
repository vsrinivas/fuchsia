...
#[test]
fn read_only_read() {
    run_server_client(OPEN_RIGHT_READABLE, simple_read_only(b"Read only test"), |proxy| {
        async move {
...
#[test]
fn write_only_write() {
    run_server_client(OPEN_RIGHT_WRITABLE, simple_write_only(b"Write only test"), |proxy| {
        async move {
...
#[test]
fn read_at_0() {
    run_server_client(OPEN_RIGHT_READABLE, simple_read_only(b"Whole file content"), |proxy| {
        async move {
...
#[test]
fn write_at_0() {
    run_server_client(OPEN_RIGHT_WRITABLE, simple_write_only(b"File content"), |proxy| {
        async move {
...
fn write_then_truncate() {
    run_server_client(OPEN_RIGHT_WRITABLE, simple_write_only(b"Replaced"), |proxy| {
        async move {
...
fn truncate_beyond_capacity() {
    run_server_client(OPEN_RIGHT_WRITABLE, simple_write_only_with_capacity(10, b""), |proxy| {
        async move {
...
fn truncate_read_only_file() {
    run_server_client(OPEN_RIGHT_READABLE, simple_read_only(b"Read-only content"), |proxy| {
        async move {
...
fn get_attr_read_only() {
    run_server_client(OPEN_RIGHT_READABLE, simple_read_only(b"Content"), |proxy| {
        async move {
            assert_get_attr!(
                proxy,
                NodeAttributes {
...
fn get_attr_write_only() {
    run_server_client(OPEN_RIGHT_WRITABLE, simple_write_only(b""), |proxy| {
        async move {
            assert_get_attr!(
                proxy,
                NodeAttributes {
...
fn clone_cannot_increase_access() {
    run_server_client(OPEN_RIGHT_READABLE, simple_read_only(b"Initial content"), |first_proxy| {
        async move {
            assert_read!(first_proxy, "Initial content");
...
fn node_reference_can_not_seek() {
    run_server_client(OPEN_FLAG_NODE_REFERENCE, read_only(simple_init_vmo(b"Content")), |proxy| {
        async move {
