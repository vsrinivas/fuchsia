...
/// Utility function to check if byte slices `left` and `right` are joinable.
///
/// `try_join` returns:
/// - `Some((base: B, len: usize))` if `left` and `right` are joinable by the
///    pointer in `base` with length `len`.
/// - `None` otherwise.
fn try_join<B: ByteSlice>(left: B, right: B) -> Option<(B, usize)> {
    if left.len() == 0 {
        let len = right.len();
        Some((right, len))
    } else if right.len() == 0 {
        ...
        // Guaranteed not to overflow because left is a valid &[u8].
        let left_end_ptr = left.as_ptr() as usize + left.len();
        if left_end_ptr == right.as_ptr() as usize {
            // Guaranteed not to overflow because, if it did, that would
            // imply that the left and right slices overlap in memory,
            // which we've just verified they don't.
...
/// Attempts to join slices `left` and `right` into a single slice reference.
///
/// If `left` and `right` are contiguous in memory, `join` returns a reference
/// to a slice that starts at `left` and has total length equal to the sum of
/// the lengths of `left` and `right`.
///
/// If the slices are not contiguous in memory, `join` returns `None`.
///
/// Empty slices (`&[]`) are always joinable with any other slice, including
/// another empty slice. If either `left` or `right` are empty slices, then
/// `join` always returns successfully, but there are no guarantees that the
/// returned slice contains in its address range the base address of any of the
/// given empty slices.
pub fn join<'a>(left: &'a [u8], right: &'a [u8]) -> Option<&'a [u8]> {
    let (base, len) = try_join(left, right)?;
    unsafe { Some(core::slice::from_raw_parts(base.as_ptr(), len)) }
}

/// Attempts to join slices `left` and `right` into a single mutable slice
/// reference.
///
/// Same as [`zerocopy::join`], but with mutable slices.
pub fn join_mut<'a>(left: &'a mut [u8], right: &'a mut [u8]) -> Option<&'a mut [u8]> {
    let (base, len) = try_join(left, right)?;
    unsafe { Some(core::slice::from_raw_parts_mut(base.as_mut_ptr(), len)) }
}

/// Attempts to join all slices in `parts` into a single slice reference.
///
/// Same as [`zerocopy::join`] but only suceeds if all the slices in `parts`
/// are contiguous (in iteration order).
///
/// Like [`zerocopy::join`], `join_iter` considers all empty slices joinable
/// with any other slices.
pub fn join_iter<'a, I: Iterator<Item = &'a [u8]>>(parts: I) -> Option<&'a [u8]> {
    parts.fold(Some(&[]), |l, r| join(l?, r))
}

/// Attempts to join all slices in `parts` into a single mutable slice
/// reference.
///
/// Same as [`zerocopy::join_iter`], but with mutable slices.
pub fn join_iter_mut<'a, I: Iterator<Item = &'a mut [u8]>>(parts: I) -> Option<&'a mut [u8]> {
    parts.fold(Some(&mut []), |l, r| join_mut(l?, r))
}
...
mod tests {
    ...
    #[test]
    fn test_join() {
        let buff = [1, 2, 3, 4, 5, 6];
        for i in 0..=6 {
            let (l, r) = buff.split_at(i);
            let j = join(l, r).unwrap();
            assert_eq!(j, &buff[..]);
            assert_eq!(j.as_ptr(), buff.as_ptr());
            if l.len() != 0 && r.len() != 0 {
                assert!(join(r, l).is_none());
            }
        }
    }
    ...
    fn test_join_mut() {
        for i in 0..=6 {
            let mut buff = [1, 2, 3, 4, 5, 6];
            let (l, r) = buff.split_at_mut(i);
            let j = join_mut(l, r).unwrap();
            j[0] = 10;
            j[5] = 20;
            assert_eq!(&buff[..], &[10, 2, 3, 4, 5, 20][..]);
        }
    }

    #[test]
    fn test_join_iter() {
        let buff = [1_u8, 2, 3, 4, 5, 6];
        for i in 0..=6 {
            for j in i..=6 {
                let (l, r) = buff.split_at(i);
                let (m, r) = r.split_at(j - i);
                let parts = [l, m, r];
                let j = join_iter(parts.iter().map(|x| *x)).unwrap();
                assert_eq!(j, &buff[..]);
                let parts = [parts[2], parts[1], parts[0]];
                if !parts.iter().any(|t| t.len() == 6) {
                    assert!(join_iter(parts.iter().map(|x| *x)).is_none());
                }
            }
        }
    }
    ...
    #[test]
    fn test_join_iter_mut() {
        for i in 0..=6 {
            for j in i..=6 {
                let mut buff = [1_u8, 2, 3, 4, 5, 6];
                let (l, r) = buff.split_at_mut(i);
                let (m, r) = r.split_at_mut(j - i);
                let mut parts = [l, m, r];
                let j = join_iter_mut(parts.iter_mut().map(|x| -> &mut [u8] { *x })).unwrap();
                j[0] = 10;
                j[5] = 20;
                assert_eq!(&buff[..], &[10, 2, 3, 4, 5, 20][..]);
            }
