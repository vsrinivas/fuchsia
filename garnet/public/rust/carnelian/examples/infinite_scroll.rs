...
impl AppAssistant for InfiniteScrollAppAssistant {
    ...
    fn create_view_assistant_image_pipe(
        ...
        // scrolling. 17 ms should be enough on most hardware but consider
        // using a lower value as input latency is increased by this offset.
        let mut touch_sampling_offset = zx::Duration::from_millis(17);
...
impl<T: Context> InfiniteScrollViewAssistant<T> {
    pub fn new(
        ...
        Self {
            ...
            last_update_time: Time::get(ClockId::Monotonic),
            touch_device,
...
impl<T: Context> ViewAssistant for InfiniteScrollViewAssistant<T> {
    ...
    fn update(&mut self, context: &ViewAssistantContext<'_>) -> Result<(), Error> {
        ...
        const SECONDS_PER_NANOSECOND: f32 = 1e-9;
        const NANOSECONDS_PER_SECOND: f32 = 1e+9;
        const REFRESH_RATE: f32 = 60.0;
        const NS_STEP: i64 = (NANOSECONDS_PER_SECOND / REFRESH_RATE) as i64;

        let current_time = Time::get(ClockId::Monotonic);
        let elapsed_ns = current_time.into_nanos() - self.last_update_time.into_nanos();

        // Round elapsed time to multiple of refresh rate and use it to
        // determine the new update time.
        // TODO: Support higher refresh rates.
        let elapsed_ns = ((elapsed_ns + NS_STEP / 2) / NS_STEP) * NS_STEP;
        let update_time = Time::from_nanos(self.last_update_time.into_nanos() + elapsed_ns);
        let elapsed = update_time - self.last_update_time;
        ...
        //
        // Smooth motion is achieved by using a fixed interval between frames
        // and approximating the touch location at the exact sampling time.
        if let Some(device) = self.touch_device.as_mut() {
            // Initialize current touch state.
            let mut previous_origin_scale = 0;
            let mut previous_origin = Vector2D::zero();
            let mut previous_time = Time::from_nanos(0);
            let mut origin_scale = self.touch_points.len();
            let mut origin = Vector2D::zero();
            for point in &self.touch_points {
                origin += point.to_vector();
            }

            // Determine sample time by removing sampling offset from update time.
            let sample_time = update_time - self.touch_sampling_offset;
            ...
            while self.touch_time <= sample_time {
                match device.get_report() {
                    Some(report) => {
                        ...
                        previous_origin_scale = self.touch_points.len();
                        ...
                        origin_scale = self.touch_points.len();
                        counter!("input", "touch_y", 0, "y" => (origin / origin_scale as f32).y.round() as u32);
                        ...
                        if previous_origin_scale != self.touch_points.len() {
                            self.scroll_origin = origin / origin_scale as f32;
                            previous_origin_scale = 0;
                        }
                    }
            ...
            if origin_scale > 0 && previous_origin_scale > 0 && self.touch_time > sample_time {
                ...
                if interval > 0.0 {
                    ...
                    origin /= origin_scale as f32;
                    previous_origin /= previous_origin_scale as f32;
                    origin = previous_origin + (origin - previous_origin) * scalar;
                    origin_scale = 1;
                }
            }
            ...
            if origin_scale > 0 {
                ...
                self.fake_scroll_start =
                    Time::from_nanos(current_time.into_nanos().saturating_add(
                        zx::Duration::from_seconds(FAKE_SCROLL_DELAY_SECONDS).into_nanos(),
                    ));
        ...
        if current_time.into_nanos() > self.fake_scroll_start.into_nanos() {
            ...
            let elapsed_seconds = (update_time.into_nanos() - self.fake_scroll_start.into_nanos())
                as f32
        ...
        if let Some(distance) = scroll_distance {
            ...
            if distance != 0.0 && elapsed.into_nanos() > 0 {
                ...
                self.fling_curve = Some(FlingCurve::new(Vector2D::new(0.0, velocity), update_time));
            }
        } else if let Some(fling) = &mut self.fling_curve {
            let (fling_active, delta) = fling.compute_scroll_delta_at_time(update_time);
