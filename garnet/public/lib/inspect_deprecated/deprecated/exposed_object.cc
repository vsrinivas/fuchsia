...
ExposedObject::ExposedObject(const std::string& name) : object_dir_(Object::Make(name.c_str())) {}

ExposedObject::ExposedObject(ObjectDir object_dir) : object_dir_(std::move(object_dir)) {}
...
void ExposedObject::set_parent(const ObjectDir& parent) { move_parents(parent); }

void ExposedObject::add_child(ExposedObject* child) { child->set_parent(object_dir_); }
