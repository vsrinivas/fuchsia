...
Node::Node(std::string name, std::vector<Property> properties, std::vector<Metric> metrics)
    : name_(std::move(name)), properties_(std::move(properties)), metrics_(std::move(metrics)) {}
...
ObjectHierarchy::ObjectHierarchy(hierarchy::Node node, std::vector<ObjectHierarchy> children)
    : node_(std::move(node)), children_(std::move(children)) {}

const ObjectHierarchy* ObjectHierarchy::GetByPath(std::vector<std::string> path) const {
