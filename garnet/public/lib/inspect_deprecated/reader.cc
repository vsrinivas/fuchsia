...
fit::result<ObjectHierarchy> Reader::Read() {
  ...
  while (it != parsed_objects_.end()) {
    ...
    if (it->second.is_complete()) {
      ...
      complete_objects.push(std::make_pair(std::move(it->second.hierarchy), it->second.parent));
      it = parsed_objects_.erase(it);
      continue;
    }
  ...
  while (complete_objects.size() > 0) {
    ...
    if (parent->is_complete()) {
      ...
      complete_objects.push(std::make_pair(std::move(parent->hierarchy), parent->parent));
...
void Reader::InnerParseMetric(ParsedObject* parent, const Block* block) {
  ...
  switch (type) {
    case BlockType::kIntValue:
      parent_metrics.emplace_back(
          hierarchy::Metric(std::move(name), hierarchy::IntMetric(block->payload.i64)));
      return;
    case BlockType::kUintValue:
      parent_metrics.emplace_back(
          hierarchy::Metric(std::move(name), hierarchy::UIntMetric(block->payload.u64)));
      return;
    case BlockType::kDoubleValue:
      parent_metrics.emplace_back(
          hierarchy::Metric(std::move(name), hierarchy::DoubleMetric(block->payload.f64)));
      return;
    case BlockType::kArrayValue: {
      BlockType entry_type = ArrayBlockPayload::EntryType::Get<BlockType>(block->payload.u64);
      uint8_t count = ArrayBlockPayload::Count::Get<uint8_t>(block->payload.u64);
      ...
      auto array_format =
          ArrayFormatToDisplay(ArrayBlockPayload::Flags::Get<ArrayFormat>(block->payload.u64));
      ...
      if (entry_type == BlockType::kIntValue) {
        ...
        parent_metrics.emplace_back(hierarchy::Metric(
            std::move(name), hierarchy::IntArray(std::move(values), array_format)));
        ...
        parent_metrics.emplace_back(hierarchy::Metric(
            std::move(name), hierarchy::UIntArray(std::move(values), array_format)));
        ...
        std::copy(GetArraySlot<const double>(block, 0), GetArraySlot<const double>(block, count),
                  std::back_inserter(values));
        parent_metrics.emplace_back(hierarchy::Metric(
            std::move(name), hierarchy::DoubleArray(std::move(values), array_format)));
      }
      return;
    }
    default:
