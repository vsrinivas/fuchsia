...
#ifndef LIB_UI_BASE_VIEW_CPP_VIEW_PROVIDER_COMPONENT_TRANSITIONAL_H_
#define LIB_UI_BASE_VIEW_CPP_VIEW_PROVIDER_COMPONENT_TRANSITIONAL_H_
...
class ViewProviderComponentTransitional {
 public:
  // Constructor for use with Views v2.
  ViewProviderComponentTransitional(ViewFactoryTransitional factory, async::Loop* loop,
                                    sys::ComponentContext* component_context = nullptr);
  ViewProviderComponentTransitional(const ViewProviderComponentTransitional&) = delete;
  ViewProviderComponentTransitional& operator=(const ViewProviderComponentTransitional&) = delete;
  ~ViewProviderComponentTransitional() = default;

 private:
  // Implementation of the |fuchsia::ui::views::View| interface that allows it
  // to be used with |ViewProviderComponentTransitional|.
  //
  ...
  class ViewImpl final : public fuchsia::ui::views::View {
   public:
    ...
    ViewImpl(ViewFactoryTransitional factory, fidl::InterfaceRequest<View> view_request,
   ...
   private:
    ...
    ViewFactoryTransitional factory_;
    ...
    // |scenic::View|.
    std::unique_ptr<BaseViewTransitional> view_;
  ...
  ViewProviderServiceTransitional service_;
  std::unique_ptr<ViewImpl> view_impl_;
};

}  // namespace scenic

#endif  // LIB_UI_BASE_VIEW_CPP_VIEW_PROVIDER_COMPONENT_TRANSITIONAL_H_
