...
std::vector<std::string> RandomlyReorderAndRepeat(
    std::vector<std::string> items, rng::Random* random) {
...
Table TableFromTableDescription(
  ...
  for (const std::vector<std::string>& leaf_full_name : table_description) {
    ...
    for (const std::string& short_name : leaf_full_name) {
      auto emplacement =
          current->children.emplace(short_name, std::make_unique<Table>());
...
bool PresentInTable(const Table* table,
                    const std::vector<std::string>& full_name) {
...
std::set<std::vector<std::string>> CompleteTableDescription(int depth) {
  if (depth == 1) {
    ...
    std::set<std::vector<std::string>> depth_minus_one_table =
        CompleteTableDescription(depth - 1);
...
::testing::Matcher<const inspect::ObjectHierarchy&> CompleteMatcher(int depth) {
  if (depth == 0) {
    ...
    return ChildrenMatch(ElementsAre(
        AllOf(NodeMatches(NameMatches("a")), CompleteMatcher(depth - 1)),
...
class Element final : public inspect::ChildrenManager {
 public:
  ...
  void GetNames(
    ...
    for (const auto& [child_name, unused_child_unique_pointer] :
         table_->children) {
  ...
  void Attach(std::string name,
    ...
    if (NextBool(random_)) {
      async::PostTask(test_loop_->dispatcher(),
                      [callback = std::move(callback),
                       retainer = std::move(retainer)]() mutable {
  ...
  std::pair<Element*, fit::closure> GetChild(
      const std::string& child_short_name) {
  ...
  void ActivateDescendant(std::vector<std::string> relative_descendant_name,
    ...
    auto implementation = [this, weak_this = weak_factory_.GetWeakPtr(),
                           relative_descendant_name =
                               std::move(relative_descendant_name),
      ...
      if (relative_descendant_name.size() == 1) {
        ...
        child->ActivateDescendant({relative_descendant_name.begin() + 1,
                                   relative_descendant_name.end()},
                                  std::move(callback));
      }
    };
    if (NextBool(random_)) {
  ...
  void DeleteDescendant(std::vector<std::string> relative_descendant_name) {
    ...
    if (it != children_.end()) {
      if (relative_descendant_name.size() == 1) {
        ...
        it->second.DeleteDescendant({relative_descendant_name.begin() + 1,
                                     relative_descendant_name.end()});
      }
    }
  }

  Element* DebugGetDescendant(
    ...
    if (it == children_.end()) {
      ...
      return it->second.DebugGetDescendant(
          {relative_descendant_name.begin() + 1,
           relative_descendant_name.end()});
    }
  }
 ...
 private:
  Element* ActivateChild(const std::string& child_short_name) {
    ...
    if (it == children_.end()) {
      inspect::Node child_inspect_node =
          inspect_node_.CreateChild(child_short_name);
  ...
  void CheckEmpty() {
    ...
    if (user_serving_retention_ == 0 && inspect_retention_ == 0 &&
        children_.empty() && on_empty_callback_) {
      on_empty_callback_();
    }
  }
...
class Application final {
 public:
  Application(async::TestLoop* loop, rng::Random* random,
              inspect::Node* application_inspect_node,
              const std::set<std::vector<std::string>>& table_description)
      : loop_(loop),
        random_(random),
  ...
  void Activate(std::vector<std::string> full_name,
    ...
    auto implementation = [this, full_name = std::move(full_name),
      ...
      if (full_name.size() == 1) {
        ...
        element->ActivateDescendant({full_name.begin() + 1, full_name.end()},
                                    std::move(callback));
      }
    };
  ...
  Activity DebugGetActivity(const std::vector<std::string>& full_name) {
    if (!PresentInTable(&table_, full_name)) {
      ...
      if (it == elements_.end()) {
        ...
        auto descendant = it->second.DebugGetDescendant(
            {full_name.begin() + 1, full_name.end()});
...
::testing::AssertionResult ChildrenManagerTest::ReadData(
  ...
  if (!callback_called) {
    return ::testing::AssertionFailure()
           << "Callback passed to ReadData not called!";
  } else {
    return ::testing::AssertionSuccess();
  }
}
...
::testing::AssertionResult ChildrenManagerTest::ListChildren(
    ...
    std::vector<std::string>* child_names) {
  bool callback_called;
  (*node)->ListChildren(callback::Capture(
      callback::SetWhenCalled(&callback_called), child_names));
  RunLoopUntilIdle();
  if (!callback_called) {
    return ::testing::AssertionFailure()
           << "Callback passed to ListChildren not called!";
  } else {
    return ::testing::AssertionSuccess();
  }
}
...
::testing::AssertionResult ChildrenManagerTest::Activate(
    Application* application, std::vector<std::string> full_name,
    fit::closure* retainer) {
  bool callback_called;
  bool success;
  ...
  if (!callback_called) {
    return ::testing::AssertionFailure()
           << "Callback passed to Activate not called!";
  } else if (!success) {
...
TEST_F(ChildrenManagerTest, SingleDynamicElement) {
  ...
  auto application = Application(&test_loop(), &random_, &elements_node_,
                                 {dynamic_child_full_name});
  fit::closure a_retainer;
  ASSERT_TRUE(
      Activate(&application, {dynamic_child_full_name[0]}, &a_retainer));
  ...
  ASSERT_EQ(Activity::INACTIVE,
            application.DebugGetActivity(dynamic_child_full_name));
...
TEST_F(ChildrenManagerTest, ElementsDeletedDuringInspection) {
  ...
  auto application = Application(&test_loop(), &random_, &elements_node_,
                                 {deepest_child_full_name});

  fit::closure a_retainer;
  ASSERT_TRUE(
      Activate(&application, {deepest_child_full_name[0]}, &a_retainer));
  ...
  fidl::InterfacePtr<fuchsia::inspect::Inspect> a_ptr;
  a_ptr.set_error_handler(callback::Capture(
      callback::SetWhenCalled(&a_error_callback_called), &a_error_status));
  ...
  fidl::InterfacePtr<fuchsia::inspect::Inspect> a_b_ptr;
  a_b_ptr.set_error_handler(callback::Capture(
      callback::SetWhenCalled(&a_b_error_callback_called), &a_b_error_status));
  ...
  a_b_c_ptr.set_error_handler(
      callback::Capture(callback::SetWhenCalled(&a_b_c_error_callback_called),
                        &a_b_c_error_status));
...
TEST_F(ChildrenManagerTest, FiveLevelsOfDynamicism) {
  ...
  std::vector<std::string> deeper_child_full_name(
      {"a", "b", "c", "1", "2", "3"});
  auto application =
      Application(&test_loop(), &random_, &elements_node_,
  ...
  ASSERT_EQ(Activity::ACTIVE,
            application.DebugGetActivity(deep_child_full_name));
  ASSERT_EQ(Activity::ACTIVE,
            application.DebugGetActivity(deeper_child_full_name));
  ...
  ASSERT_EQ(Activity::ACTIVE,
            application.DebugGetActivity({"a", "b", "c", "d"}));
  ASSERT_EQ(Activity::ACTIVE,
            application.DebugGetActivity({"a", "b", "c", "1"}));
  ASSERT_EQ(Activity::ACTIVE,
            application.DebugGetActivity({"a", "b", "c", "1", "2"}));
...
TEST_F(ChildrenManagerTest, ReaderAPIMinimalActiveElements) {
  auto depth = 3;
  auto application = Application(&test_loop(), &random_, &elements_node_,
                                 CompleteTableDescription(depth));
  fit::closure a_retainer;
  ASSERT_TRUE(Activate(&application, {"a"}, &a_retainer));
...
TEST_F(ChildrenManagerTest, ReaderAPINoInactiveElements) {
  ...
  auto application =
      Application(&test_loop(), &random_, &elements_node_, leaf_full_names);
...
TEST_F(ChildrenManagerTest, AbsentChildDoesNotDeadlock) {
  ...
  class ChildrenManager final : public component::ChildrenManager {
   public:
    ChildrenManager(fit::closure on_detachment)
        : on_detachment_(std::move(on_detachment)) {}
  ...
  auto children_manager_retainer =
      elements_node_.SetChildrenManager(&children_manager);
