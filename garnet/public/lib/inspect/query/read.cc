...
namespace inspect {
namespace {
fit::result<fidl::InterfaceHandle<fuchsia::inspect::Inspect>, std::string>
OpenInspectAtPath(const std::string& path) {
  ...
  if (status != ZX_OK) {
    return fit::error(fxl::StringPrintf("Failed to connect at %s with %d\n",
  ...
  if (location.type == Location::Type::INSPECT_FIDL) {
    ...
    if (!handle.is_ok()) {
      return fit::make_promise([path = location.AbsoluteFilePath()]()
        ...
        return fit::error(
            fxl::StringPrintf("Failed to open %s\n", path.c_str()));
      });
    }
    ...
    return fit::make_promise(
        [type = location.type]() -> fit::result<Source, std::string> {
          return fit::error(
              fxl::StringPrintf("Unknown location type %d\n", type));
        });
  }
}
