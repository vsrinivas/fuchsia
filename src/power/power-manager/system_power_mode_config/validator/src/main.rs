...
/// A simple program to test the validity of a system power mode configuration file.
///
/// This program will attempt to open the system power mode configuration file (supplied via the
/// "--input <file_path>" argument), then use the `system_power_mode_config` parser library to parse
/// it to into a valid `SystemPowerModeConfig`. If a config file can be read by the library, then it
...
#[derive(FromArgs)]
#[argh(description = "Input flags for the system power mode config validator")]
struct Flags {
    #[argh(option, description = "input path for the system power mode config JSON file")]
    input: PathBuf,
...
fn main_impl(flags: Flags) -> Result<(), Error> {
    SystemPowerModeConfig::read(&flags.input).map(|_| ())?;
...
mod tests {
    ...
    fn get_valid_configs() -> Vec<PathBuf> {
        get_configs("valid_system_power_mode_test_configs")
    }

    fn get_invalid_configs() -> Vec<PathBuf> {
        get_configs("invalid_system_power_mode_test_configs")
    }
