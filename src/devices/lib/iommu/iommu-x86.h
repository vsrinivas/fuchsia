...
class IommuManager : public iommu::IommuManagerBase {
 public:
  ...
  zx::unowned_iommu IommuForBdf(uint32_t bdf) override;
  // TODO(fxbug.dev/92140): parse ANDD for this information.
  zx::unowned_iommu IommuForAcpiDevice(const char* path) override { return DummyIommu(); }
