...
TEST_F(AcpiPwrsrcTest, TestGetInfo) {
  ...
  ASSERT_EQ(info.value().info.type, fuchsia_hardware_power::wire::PowerType::kAc);
  ASSERT_EQ(info.value().info.state, 0);
}

TEST_F(AcpiPwrsrcTest, TestNotify) {
  auto event = source_client_->GetStateChangeEvent();
  ASSERT_OK(event.status());
  ASSERT_OK(event.value().status);

  ASSERT_STATUS(event.value().handle.wait_one(ZX_USER_SIGNAL_0, zx::time::infinite_past(), nullptr),
                ZX_ERR_TIMED_OUT);
  ...
  // TODO(fxbug.dev/97955) Consider handling the error instead of ignoring it.
  (void)notify_client_->Handle(kPowerSourceStateChanged);
  ASSERT_OK(event.value().handle.wait_one(ZX_USER_SIGNAL_0, zx::time::infinite(), nullptr));
