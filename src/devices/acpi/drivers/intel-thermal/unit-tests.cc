...
TEST_F(IntelThermalTest, NotifysOnTrip) {
  ...
  zx::event event = std::move(result.value().handle);
...
TEST_F(IntelThermalTest, GetCurrentTemp) {
  ...
  ASSERT_OK(temp.value().status);

  ASSERT_FLOAT_EQ(temp.value().temp, 12.05f);
}
...
TEST_F(IntelThermalTest, GetInfo) {
  {
    ...
    ASSERT_FLOAT_EQ(info.value().info->critical_temp_celsius, 80.05f);
    ASSERT_FLOAT_EQ(info.value().info->passive_temp_celsius, 20.05f);
    ASSERT_EQ(info.value().info->max_trip_count, kTripPointCount);
    fidl::Array<float, fuchsia_hardware_thermal::wire::kMaxTripPoints> trip_points{0};
    ASSERT_EQ(memcmp(trip_points.data(), info.value().info->active_trip.data(),
                     trip_points.size() * sizeof(float)),
              0);
    ASSERT_EQ(info.value().info->state, fuchsia_hardware_thermal::wire::kThermalStateNormal);
  }
...

  {
    ...
    ASSERT_FLOAT_EQ(info.value().info->passive_temp_celsius, 20.05f);
    ASSERT_EQ(info.value().info->max_trip_count, kTripPointCount);
    ASSERT_FLOAT_EQ(info.value().info->active_trip[0], 22.05f);
    ASSERT_EQ(info.value().info->state, fuchsia_hardware_thermal::wire::kThermalStateTripViolation);
  }
}
}  // namespace intel_thermal
