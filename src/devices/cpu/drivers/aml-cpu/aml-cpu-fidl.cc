...
zx_status_t AmlCpu::Create(void* context, zx_device_t* parent) {
  ...
  st = device_get_metadata_size(parent, DEVICE_METADATA_AML_CPU, &metadata_size);
  if (st != ZX_OK) {
    zxlogf(ERROR, "%s: Failed to get metadata size, st = %d", __func__, st);
    return st;
  }
  ...
  st = device_get_metadata(parent, DEVICE_METADATA_AML_CPU, buffer, metadata_size, &actual);
  if (st != ZX_OK) {
    zxlogf(ERROR, "%s: Failed to get metadata, st = %d", __func__, st);
    return st;
  }

  if (actual != metadata_size) {
    zxlogf(ERROR, "%s: Couldn't get correct metadata size.", __func__);
    return ZX_ERR_INTERNAL;
  }

  fidl::DecodedMessage<fuchsia_amlogic_cpu_metadata::wire::AmlogicCpuMetadata> decoded(buffer, metadata_size);
  if (!decoded.ok()) {
    zxlogf(ERROR, "%s: Failed to deserialize metadata.", __func__);
    return ZX_ERR_INTERNAL;
  }

  fuchsia_amlogic_cpu_metadata::wire::AmlogicCpuMetadata* metadata = decoded.PrimaryObject();
  const size_t num_perf_domains = metadata->domains().count();
  ...
  if ((num_perf_domains * kFragmentsPerPfDomain) + 1 != fragment_count) {
    zxlogf(ERROR,
           ...
           __func__, kFragmentsPerPfDomain, num_perf_domains,
           num_perf_domains * kFragmentsPerPfDomain, fragment_count);
    return ZX_ERR_INTERNAL;
  }
  ...
  for (size_t i = 0; i < num_perf_domains; i++) {
    // const auto& perf_domain = perf_domains[i];
    const auto& perf_domain = metadata->domains()[i];
    ...
    for (const auto& src : perf_domain.operating_points()) {
      operating_point_t op = {
        .freq_hz = src.frequency(),
        .volt_uv = src.voltage(),
        .pd_id = perf_domain.id(),
      };
      pd_op_points.push_back(op);
    }
