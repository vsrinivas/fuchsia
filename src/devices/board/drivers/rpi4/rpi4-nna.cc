...
static pbus_mmio_t rpi4_nna_mmios[] = {
    {
        .base = A311D_NNA_BASE,
...
static pbus_dev_t nna_dev = []() {
  ...
  dev.mmio_list = rpi4_nna_mmios;
  dev.mmio_count = countof(rpi4_nna_mmios);
...
zx_status_t Rpi4::NnaInit() {
  zx_status_t status = pbus_.DeviceAdd(&nna_dev);
  if (status != ZX_OK) {
    zxlogf(ERROR, "Rpi4::NnaInit: pbus_device_add() failed for nna: %d", status);
    return status;
  }
  return ZX_OK;
}
