...
static const std::vector<fpbus::Mmio> buckeye_dmc_mmios{
    {{
        .base = A5_DMC_BASE,
...
static const std::vector<fpbus::Irq> buckeye_dmc_irqs{
    {{
        .irq = A5_DDR_BW_IRQ,
...
static const std::vector<fpbus::Smc> buckeye_dmc_smcs{
    {{
...
static const fpbus::Node dmc_dev = []() {
  ...
  dev.mmio() = buckeye_dmc_mmios;
  dev.irq() = buckeye_dmc_irqs;
  dev.smc() = buckeye_dmc_smcs;
  return dev;
}();

zx_status_t Buckeye::DmcInit() {
