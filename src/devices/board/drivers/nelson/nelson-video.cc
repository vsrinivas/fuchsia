...
constexpr pbus_dev_t video_dev = []() {
  ...
  dev.mmio_count = std::size(nelson_video_mmios);
  dev.bti_list = nelson_video_btis;
  dev.bti_count = std::size(nelson_video_btis);
  dev.irq_list = nelson_video_irqs;
  dev.irq_count = std::size(nelson_video_irqs);
  dev.smc_list = nelson_video_smcs;
  dev.smc_count = std::size(nelson_video_smcs);
  return dev;
}();
