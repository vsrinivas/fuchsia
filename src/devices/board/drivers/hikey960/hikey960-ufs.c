...
#define set_bits(v, a) MmioWrite32(MmioRead32(a) | (v), (a))
#define clr_bits(v, a) MmioWrite32(MmioRead32(a) & (uint32_t) ~(v), (a))

zx_status_t hikey960_ufs_clock_init(hikey960_t* hikey) {
  MMIO_PTR volatile void* ufs_sctrl = hikey->ufs_sctrl.vaddr;
  MMIO_PTR volatile void* peri_crg = hikey->peri_crg.vaddr;
  MMIO_PTR volatile void* pctrl = hikey->pctrl.vaddr;

  MmioWrite32(PERI_CRG_UFS_IO, peri_crg + PERI_CRG_UFS_ISODIS);
  ...
  MmioWrite32(BIT(0) | BIT(16), pctrl + PCTRL_CTRL3);
  zx_nanosleep(zx_deadline_after(ZX_MSEC(1)));

  /* Open device Ref clock */
  MmioWrite32(BIT(14), peri_crg + PERI_CRG_UFS_ISOEN);
...
zx_status_t hikey960_ufs_soc_init(hikey960_t* hikey) {
  ...
  /* HC reset_n enable */
  MmioWrite32(PERI_CRG_UFS_RST, peri_crg + PERI_CRG_UFS_RSTEN3);
  ...
  /* Enable device reset */
  MmioWrite32(UFS_SCTRL_MASK_DEV_RST | 0, ufs_sctrl + UFS_SCTRL_DEV_RST_CTRL);

  /* set HC hclk div */
  MmioWrite32(BIT(14 + 16) | 0, peri_crg + PERI_CRG_UFS_CLKDIV17);

  /* set HC hclk div */
  MmioWrite32((0x3 << 9) | (0x3 << (9 + 16)), peri_crg + PERI_CRG_UFS_CLKDIV16);

  /* set cfg clk freq */
  val = MmioRead32(ufs_sctrl + UFS_SCTRL_PHY_CLK_CTRL);
  val = val & (~UFS_SCTRL_CLK_FREQ_MASK);
  val = val | UFS_SCTRL_CLK_FREQ_CFG;
  MmioWrite32(val, ufs_sctrl + UFS_SCTRL_PHY_CLK_CTRL);
  ...
  /* disable areset_n */
  MmioWrite32(PERI_CRG_UFS_ARST, peri_crg + PERI_CRG_UFS_RSTDIS3);
  ...
  /* Reset device */
  MmioWrite32(UFS_SCTRL_MASK_UFS_RST | UFS_SCTRL_MASK_DEV_RST, ufs_sctrl + UFS_SCTRL_DEV_RST_CTRL);
   ...
   */
  MmioWrite32(0x03300330, ufs_sctrl + UFS_SCTRL_DEV_RST_CTRL);

  MmioWrite32(PERI_CRG_UFS_RST, peri_crg + PERI_CRG_UFS_RSTDIS3);
