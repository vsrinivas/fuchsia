...
zx_status_t Sherlock::VideoEncInit() {
  ...
  zx_status_t status =
      pbus_.CompositeDeviceAddNew(&video_enc_dev, fragments, countof(fragments), UINT32_MAX);
  if (status != ZX_OK) {
    zxlogf(ERROR, "Sherlock::VideoEncInit: CompositeDeviceAddNew() failed for video: %d", status);
    return status;
  }
  return ZX_OK;
}
