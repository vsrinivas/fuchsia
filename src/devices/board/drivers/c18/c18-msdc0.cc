...
zx_status_t C18::Msdc0Init() {
  ...
  status = pbus_.CompositeDeviceAddNew(&msdc0_dev, fragments, std::size(fragments), UINT32_MAX);
  if (status != ZX_OK) {
    zxlogf(ERROR, "%s: CompositeDeviceAddNew MSDC0 failed: %d", __FUNCTION__, status);
  }
