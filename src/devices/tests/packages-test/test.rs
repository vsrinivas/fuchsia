...
async fn serve_fake_filesystem(handles: MockHandles) -> Result<(), anyhow::Error> {
    let system = vfs::pseudo_directory! {
    ...
    let base_manifest = vfs::file::vmo::asynchronous::read_only_static(
        r#"[{"driver_url": "fuchsia-pkg://fuchsia.com/serial-ppp#driver/ddk-firmware-test.so"}]"#,
    );
    ...
    let pkgfs = vfs::pseudo_directory! {
        "packages" => vfs::pseudo_directory! {
            ...
            "serial-ppp" => vfs::pseudo_directory! {
                "0" => vfs::pseudo_directory! {
                    "driver" => driver_dir,
...
#[fuchsia::test]
async fn ddk_packages_test() -> Result<(), Error> {
    let mut builder = RealmBuilder::new().await?;

    builder
        .add_component(
            "fake_filesystem",
            ComponentSource::mock(move |h: MockHandles| serve_fake_filesystem(h).boxed()),
        )
        .await
