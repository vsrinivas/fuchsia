...
const DRIVERS_TO_WAIT_ON_DFV1: [&str; 1] = ["sys/test/sample_driver"];
const DRIVERS_TO_WAIT_ON_DFV2: [&str; 1] = ["compat/sample_driver"];
...
async fn test_get_device_info_no_filter_dfv1() -> Result<()> {
    ...
    wait_for_drivers(&instance, &DRIVERS_TO_WAIT_ON_DFV1).await?;
...
async fn test_get_device_info_no_filter_dfv2() -> Result<()> {
    ...
    wait_for_drivers(&instance, &DRIVERS_TO_WAIT_ON_DFV2).await?;
    let device_infos = get_device_info(&driver_dev, &[]).await?;
