...
zx_status_t Device::GetContiguousGuardParameters(uint64_t* guard_bytes_out,
  ...
  *crash_on_fail_out = false;
  ...
  if (getenv("driver.sysmem.contiguous_guard_pages_unused_disabled")) {
  ...
  const char* unused_page_check_cycle_period_seconds_string = getenv(kUnusedPageCheckCyclePeriodName);
  if (unused_page_check_cycle_period_seconds_string && strlen(unused_page_check_cycle_period_seconds_string)) {
    char* end = nullptr;
  ...
  const char* guard_count = getenv(kGuardBytesName);
  if (guard_count && strlen(guard_count)) {
    char* end = nullptr;
...
void Device::SetAuxServiceDirectory(SetAuxServiceDirectoryRequestView request,
  ...
  async::PostTask(loop_.dispatcher(), [this, aux_service_directory =
                                                 std::make_shared<sys::ServiceDirectory>(
                                                     std::move(request->service_directory))] {
