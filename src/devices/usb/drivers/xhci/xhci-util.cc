...
uint32_t* xhci_get_next_ext_cap(void* mmio, uint32_t* prev_cap, uint32_t* match_cap_id) {
  ...
  if (!cap_ptr) {
    ...
    uint32_t offset =
        XHCI_GET_BITS32(hccparams1, HCCPARAMS1_EXT_CAP_PTR_START, HCCPARAMS1_EXT_CAP_PTR_BITS);
    if (!offset) {
      return nullptr;
    }
  ...
  while (cap_ptr) {
    ...
    uint32_t offset = XHCI_GET_BITS32(cap_ptr, EXT_CAP_NEXT_PTR_START, EXT_CAP_NEXT_PTR_BITS);
