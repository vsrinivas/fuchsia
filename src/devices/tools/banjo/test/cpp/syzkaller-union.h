...
#include <banjo/examples/syzkaller/union.h>
...
#include "union-internal.h"

// DDK union-protocol support
//
// :: Proxies ::
//
...
public:
    ApiProtocol() {
        ...
        api_protocol_ops_.packet = ApiPacket;
...
private:
    static zx_status_t ApiPacket(void* ctx, zx_handle_t h, const packet_t* pkt) {
        auto ret = static_cast<D*>(ctx)->ApiPacket(zx::handle(h), pkt);
        return ret;
    }
};
...
public:
    ...
    zx_status_t Packet(zx::handle h, const packet_t* pkt) const {
        return ops_->packet(ctx_, h.release(), pkt);
    }
