...
#include <banjo/examples/syzkaller/flag.h>
...
#include "flag-internal.h"

// DDK flag-protocol support
//
// :: Proxies ::
//
...
public:
    ApiProtocol() {
        ...
        api_protocol_ops_.topic = ApiTopic;
...
private:
    static zx_status_t ApiTopic(void* ctx, zx_handle_t h, int32_t t) {
        auto ret = static_cast<D*>(ctx)->ApiTopic(zx::handle(h), t);
        return ret;
    }
};
...
public:
    ...
    zx_status_t Topic(zx::handle h, int32_t t) const {
        return ops_->topic(ctx_, h.release(), t);
    }
