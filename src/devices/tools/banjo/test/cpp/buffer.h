...
#include <banjo/examples/buffer.h>
...
public:
    ...
    SomeMethodsProtocolClient(CompositeProtocolClient& composite, const char* fragment_name) {
        zx_device_t* fragment;
        bool found = composite.GetFragment(fragment_name, &fragment);
        some_methods_protocol_t proto;
        if (found && device_get_protocol(fragment, ZX_PROTOCOL_SOME_METHODS, &proto) == ZX_OK) {
            ops_ = proto.ops;
            ctx_ = proto.ctx;
        } else {
            ops_ = nullptr;
    ...
    // Create a SomeMethodsProtocolClient from the given composite protocol.
    //
    // If ZX_OK is returned, the created object will be initialized in |result|.
...
private:
    some_methods_protocol_ops_t* ops_;
    void* ctx_;
};
