...
#ifndef SRC_LIB_FASYNC_INCLUDE_LIB_FASYNC_SCHEDULER_H_
#define SRC_LIB_FASYNC_INCLUDE_LIB_FASYNC_SCHEDULER_H_
...
// For a concrete implementation, see |fasync::single_threaded_executor|.
//
...
class scheduler final {
 public:
  ...
  void schedule(pending_task&& task) { runnable_tasks_.push(std::move(task)); }
  ...
  // Preconditions:
  // - the ticket's ref-count must be non-zero (positive)
  cpp17::optional<pending_task> release_ticket(suspended_task::ticket ticket);
 ...
 private:
  struct ticket_record {
    ...
    cpp17::optional<pending_task> task;
  };
...
#endif  // SRC_LIB_FASYNC_INCLUDE_LIB_FASYNC_SCHEDULER_H_
