...
fitx::result<Error, std::unique_ptr<DumpFile>> DumpFile::Open(fbl::unique_fd fd, bool try_mmap) {
  struct stat st;
  if (fstat(fd.get(), &st) < 0) {
    return fitx::error(Error{"fstat", ZX_ERR_IO});
  }
