...
BackgroundExecutor::BackgroundExecutor() {
  // Create a unit of work for the runner to munch on if no tasks are pending. This will ensure the
  // invocation of |executor::run()| doesn't terminate until the termination task is resumed.
  //
  ...
  auto work = fit::make_promise([this](fit::context& context) mutable -> fit::result<> {
    ...
    // Suspend the task, never to actually return. When the BackgroundExecutor destructor runs, this
    // suspended task will be destroyed.
