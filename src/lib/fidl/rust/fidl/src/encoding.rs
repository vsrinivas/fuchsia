...
/// representing a FIDL strict bits type. The struct must have been generated by
/// the bitflags crate.
///
/// For ease of transition to/from flexible bits, it also generates the
...
/// Implements the FIDL `Encodable` and `Decodable` traits for a struct
/// representing a FIDL flexible bits type. The struct must have been generated
...
/// ```
#[macro_export]
macro_rules! fidl_flexible_bits {
    (
        name: $name:ident,
        prim_ty: $prim_ty:ty,
    ) => {
...
pub unsafe fn encode_unknown_data(
    ...
    fidl_unsafe_encode!(&mut (val.bytes.len() as u32), encoder, offset, recursion_depth)?;
    fidl_unsafe_encode!(&mut (val.handles.len() as u32), encoder, offset + 4, recursion_depth)?;
    fidl_unsafe_encode!(&mut ALLOC_PRESENT_U64.clone(), encoder, offset + 8, recursion_depth)?;
    Encoder::check_recursion_depth(recursion_depth + 1)?;
...
pub unsafe fn encode_unknown_bytes(
    ...
    fidl_unsafe_encode!(&mut (val.len() as u64), encoder, offset, recursion_depth)?;
    fidl_unsafe_encode!(&mut ALLOC_PRESENT_U64.clone(), encoder, offset + 8, recursion_depth)?;
    Encoder::check_recursion_depth(recursion_depth + 1)?;
                                ...
                                $crate::encoding::ALLOC_PRESENT_U64 => {
                                    decoder.read_out_of_line(
                                        ...
                                        |decoder, offset| {
                                            ...
                                            $crate::fidl_decode!(val_ref, decoder, offset)?;
                                            Ok(())
                                        },
                                    )?;
                                }
...
mod test {
    ...
    fn encode_decode_strict_bits() {
        ...
        assert_eq!(Buttons::from_bits_truncate(2 | 8), Buttons::PAUSE);
        assert_eq!(Buttons::from_bits_truncate(u32::MAX), Buttons::all());
        ...
        // deprecation warning.
        #[allow(deprecated)]
        let has_unknown_bits = Buttons::PLAY.has_unknown_bits();
        assert_eq!(has_unknown_bits, false);
        #[allow(deprecated)]
        let get_unknown_bits = Buttons::PLAY.get_unknown_bits();
        ...
        identities![
            ...
            Buttons::from_bits_truncate(u32::MAX),
        ];
    }

    #[test]
    fn encode_decode_flexible_bits() {
        bitflags! {
            pub struct Buttons: u32 {
                const PLAY = 1;
                const PAUSE = 2;
