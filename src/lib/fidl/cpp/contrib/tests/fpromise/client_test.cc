// Copyright 2022 The Fuchsia Authors. All rights reserved.
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.

#include "src/lib/fidl/cpp/contrib/fpromise/client.h"

#include <fidl/fidl.test.coding.fuchsia/cpp/fidl.h>
#include <fidl/llcpptest.protocol.test/cpp/fidl.h>
#include <lib/async-loop/cpp/loop.h>
#include <lib/async/cpp/executor.h>
#include <lib/fpromise/promise.h>

#include <zxtest/zxtest.h>

namespace {

constexpr int32_t kExpectedReply = 7;

struct EchoServer : fidl::Server<fidl_test_coding_fuchsia::Simple> {
  void Echo(EchoRequest& request, EchoCompleter::Sync& completer) override {
    completer.Reply(request.request());
  }
  void Close(CloseRequest& request, CloseCompleter::Sync& completer) override {
    ADD_FAILURE("Unused");
  }
};

TEST(Client, Promisify) {
  auto server = std::make_unique<EchoServer>();
  async::Loop loop(&kAsyncLoopConfigNeverAttachToThread);
  async::Executor executor(loop.dispatcher());
  zx::status endpoints = fidl::CreateEndpoints<fidl_test_coding_fuchsia::Simple>();
  ASSERT_OK(endpoints.status_value());
  fidl::BindServer(loop.dispatcher(), std::move(endpoints->server), server.get());
  fidl::Client client(std::move(endpoints->client), loop.dispatcher());

  fpromise::promise<fidl_test_coding_fuchsia::SimpleEchoTopResponse, fidl::Error> p =
      fidl_fpromise::as_promise(client->Echo({kExpectedReply}));

  auto task = p.then(
      [&](fpromise::result<fidl_test_coding_fuchsia::SimpleEchoTopResponse, fidl::Error>& result) {
        ASSERT_TRUE(result.is_ok());
        ASSERT_EQ(kExpectedReply, result.value());
        loop.Quit();
      });

  executor.schedule_task(std::move(task));
  ASSERT_STATUS(ZX_ERR_CANCELED, loop.Run());
}

TEST(Client, PromisifyChaining) {
  auto server = std::make_unique<EchoServer>();
  async::Loop loop(&kAsyncLoopConfigNeverAttachToThread);
  async::Executor executor(loop.dispatcher());
  zx::status endpoints = fidl::CreateEndpoints<fidl_test_coding_fuchsia::Simple>();
  ASSERT_OK(endpoints.status_value());
  fidl::BindServer(loop.dispatcher(), std::move(endpoints->server), server.get());
  fidl::Client client(std::move(endpoints->client), loop.dispatcher());

  // Chain another continuation which operates on the FIDL result.
  auto p = fidl_fpromise::as_promise(client->Echo({kExpectedReply}))
               .and_then([&](const fidl_test_coding_fuchsia::SimpleEchoTopResponse& payload) {
                 // Convert response number to string
                 return fpromise::ok(std::to_string(payload.reply()));
               });

  // |p| is now transformed to a promise that resolves to a string.
  auto task = p.then([&](fpromise::result<std::string, fidl::Error>& result) {
    ASSERT_TRUE(result.is_ok());
    ASSERT_EQ("7", result.value());
    loop.Quit();
  });

  executor.schedule_task(std::move(task));
  ASSERT_STATUS(ZX_ERR_CANCELED, loop.Run());
}

TEST(Client, PromisifyTransportError) {
  async::Loop loop(&kAsyncLoopConfigNeverAttachToThread);
  async::Executor executor(loop.dispatcher());
  zx::status endpoints = fidl::CreateEndpoints<fidl_test_coding_fuchsia::Simple>();
  ASSERT_OK(endpoints.status_value());
  fidl::Client client(std::move(endpoints->client), loop.dispatcher());

  fpromise::promise<fidl_test_coding_fuchsia::SimpleEchoTopResponse, fidl::Error> p =
      fidl_fpromise::as_promise(client->Echo({kExpectedReply}));
  endpoints->server.reset();

  auto task = p.then(
      [&](fpromise::result<fidl_test_coding_fuchsia::SimpleEchoTopResponse, fidl::Error>& result) {
        ASSERT_TRUE(result.is_error());
        ASSERT_TRUE(result.error().is_peer_closed());
        loop.Quit();
      });

  executor.schedule_task(std::move(task));
  ASSERT_STATUS(ZX_ERR_CANCELED, loop.Run());
}

class ErrorServer : public fidl::Server<llcpptest_protocol_test::ErrorMethods> {
  void NoArgsPrimitiveError(NoArgsPrimitiveErrorRequest& request,
                            NoArgsPrimitiveErrorCompleter::Sync& completer) final {
    completer.Reply(fitx::error(42));
  }
  void ManyArgsCustomError(ManyArgsCustomErrorRequest& request,
                           ManyArgsCustomErrorCompleter::Sync& completer) final {
    completer.Reply(fitx::error(llcpptest_protocol_test::MyError::kBadError));
  }
};

TEST(Client, PromisifyApplicationErrorMethodCasePrimitiveError) {
  auto server = std::make_unique<ErrorServer>();
  async::Loop loop(&kAsyncLoopConfigNeverAttachToThread);
  async::Executor executor(loop.dispatcher());
  zx::status endpoints = fidl::CreateEndpoints<llcpptest_protocol_test::ErrorMethods>();
  ASSERT_OK(endpoints.status_value());
  fidl::BindServer(loop.dispatcher(), std::move(endpoints->server), server.get());
  fidl::Client client(std::move(endpoints->client), loop.dispatcher());

  fpromise::promise<void,
                    fidl::AnyErrorIn<llcpptest_protocol_test::ErrorMethods::NoArgsPrimitiveError>>
      p = fidl_fpromise::as_promise(client->NoArgsPrimitiveError({{.should_error = true}}));
  endpoints->server.reset();

  auto task = p.then(
      [&](fpromise::result<
          void, fidl::AnyErrorIn<llcpptest_protocol_test::ErrorMethods::NoArgsPrimitiveError>>&
              result) {
        ASSERT_TRUE(result.is_error());
        ASSERT_TRUE(result.error().is_application_error());
        ASSERT_EQ(42, result.error().application_error());
        loop.Quit();
      });

  executor.schedule_task(std::move(task));
  ASSERT_STATUS(ZX_ERR_CANCELED, loop.Run());
}

TEST(Client, PromisifyApplicationErrorMethodCaseCustomError) {
  auto server = std::make_unique<ErrorServer>();
  async::Loop loop(&kAsyncLoopConfigNeverAttachToThread);
  async::Executor executor(loop.dispatcher());
  zx::status endpoints = fidl::CreateEndpoints<llcpptest_protocol_test::ErrorMethods>();
  ASSERT_OK(endpoints.status_value());
  fidl::BindServer(loop.dispatcher(), std::move(endpoints->server), server.get());
  fidl::Client client(std::move(endpoints->client), loop.dispatcher());

  fpromise::promise<llcpptest_protocol_test::ErrorMethodsManyArgsCustomErrorResponse,
                    fidl::AnyErrorIn<llcpptest_protocol_test::ErrorMethods::ManyArgsCustomError>>
      p = fidl_fpromise::as_promise(client->ManyArgsCustomError({{.should_error = true}}));

  auto task = p.then(
      [&](fpromise::result<
          llcpptest_protocol_test::ErrorMethodsManyArgsCustomErrorResponse,
          fidl::AnyErrorIn<llcpptest_protocol_test::ErrorMethods::ManyArgsCustomError>>& result) {
        ASSERT_TRUE(result.is_error());
        ASSERT_TRUE(result.error().is_application_error());
        ASSERT_EQ(llcpptest_protocol_test::MyError::kBadError, result.error().application_error());
        loop.Quit();
      });

  executor.schedule_task(std::move(task));
  ASSERT_STATUS(ZX_ERR_CANCELED, loop.Run());
}

class SuccessServer : public fidl::Server<llcpptest_protocol_test::ErrorMethods> {
  void NoArgsPrimitiveError(NoArgsPrimitiveErrorRequest& request,
                            NoArgsPrimitiveErrorCompleter::Sync& completer) final {
    completer.Reply(fitx::ok());
  }
  void ManyArgsCustomError(ManyArgsCustomErrorRequest& request,
                           ManyArgsCustomErrorCompleter::Sync& completer) final {
    completer.Reply(fitx::ok(llcpptest_protocol_test::ErrorMethodsManyArgsCustomErrorResponse{
        {.a = 1, .b = 2, .c = 3}}));
  }
};

TEST(Client, PromisifyApplicationErrorMethodCaseNoArgsSuccess) {
  auto server = std::make_unique<SuccessServer>();
  async::Loop loop(&kAsyncLoopConfigNeverAttachToThread);
  async::Executor executor(loop.dispatcher());
  zx::status endpoints = fidl::CreateEndpoints<llcpptest_protocol_test::ErrorMethods>();
  ASSERT_OK(endpoints.status_value());
  fidl::BindServer(loop.dispatcher(), std::move(endpoints->server), server.get());
  fidl::Client client(std::move(endpoints->client), loop.dispatcher());

  fpromise::promise<void,
                    fidl::AnyErrorIn<llcpptest_protocol_test::ErrorMethods::NoArgsPrimitiveError>>
      p = fidl_fpromise::as_promise(client->NoArgsPrimitiveError({{.should_error = false}}));

  auto task = p.then(
      [&](fpromise::result<
          void, fidl::AnyErrorIn<llcpptest_protocol_test::ErrorMethods::NoArgsPrimitiveError>>&
              result) {
        ASSERT_TRUE(result.is_ok());
        loop.Quit();
      });

  executor.schedule_task(std::move(task));
  ASSERT_STATUS(ZX_ERR_CANCELED, loop.Run());
}

TEST(Client, PromisifyApplicationErrorMethodCaseManyArgsSuccess) {
  auto server = std::make_unique<SuccessServer>();
  async::Loop loop(&kAsyncLoopConfigNeverAttachToThread);
  async::Executor executor(loop.dispatcher());
  zx::status endpoints = fidl::CreateEndpoints<llcpptest_protocol_test::ErrorMethods>();
  ASSERT_OK(endpoints.status_value());
  fidl::BindServer(loop.dispatcher(), std::move(endpoints->server), server.get());
  fidl::Client client(std::move(endpoints->client), loop.dispatcher());

  fpromise::promise<llcpptest_protocol_test::ErrorMethodsManyArgsCustomErrorResponse,
                    fidl::AnyErrorIn<llcpptest_protocol_test::ErrorMethods::ManyArgsCustomError>>
      p = fidl_fpromise::as_promise(client->ManyArgsCustomError({{.should_error = false}}));

  auto task = p.then(
      [&](fpromise::result<
          llcpptest_protocol_test::ErrorMethodsManyArgsCustomErrorResponse,
          fidl::AnyErrorIn<llcpptest_protocol_test::ErrorMethods::ManyArgsCustomError>>& result) {
        ASSERT_TRUE(result.is_ok());
        ASSERT_EQ(llcpptest_protocol_test::ErrorMethodsManyArgsCustomErrorResponse(1, 2, 3),
                  result.value());
        loop.Quit();
      });

  executor.schedule_task(std::move(task));
  ASSERT_STATUS(ZX_ERR_CANCELED, loop.Run());
}

}  // namespace
