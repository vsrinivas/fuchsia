...
#include "intrusive_container/intrusive_wavl_tree_checker.h"
#include "intrusive_container/ordered_associative_container_test_environment.h"
#include "intrusive_container/test_thunks.h"
...
struct OtherTreeTraits {
  ...
  using PtrTraits = ::fidl::internal_wavl::internal::ContainerPtrTraits<PtrType>;
 ...
 public:
  // clang-format off
    ...
    using TaggedContainableBaseClasses =
        fidl::internal_wavl::ContainableBaseClasses<TaggedWAVLTreeContainable<PtrType, Tag1>,
...
using CU_WAVLTraits = WAVLTraits<PtrType, fidl::internal_wavl::NodeOptions::AllowClearUnsafe>;
...
TEST(WavlTreeTest, MismatchedPointerType) {
  struct Obj {
    fidl::internal_wavl::WAVLTreeNodeState<Obj*> wavl_node_state_;
    uintptr_t GetKey() const { return reinterpret_cast<uintptr_t>(this); }
  };
#if TEST_WILL_NOT_COMPILE || 0
  [[maybe_unused]] fidl::internal_wavl::WAVLTree<uintptr_t, std::unique_ptr<Obj>> tree;
#endif
}
...
#if TEST_WILL_NOT_COMPILE || 0
  [[maybe_unused]] fidl::internal_wavl::WAVLTree<uintptr_t, Obj*> tree;
#endif
}
