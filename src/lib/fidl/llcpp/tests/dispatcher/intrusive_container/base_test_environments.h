...
#ifndef SRC_LIB_FIDL_LLCPP_TESTS_DISPATCHER_INTRUSIVE_CONTAINER_BASE_TEST_ENVIRONMENTS_H_
#define SRC_LIB_FIDL_LLCPP_TESTS_DISPATCHER_INTRUSIVE_CONTAINER_BASE_TEST_ENVIRONMENTS_H_
...
#include "objects.h"
#include "test_environment_utils.h"
...
class TestEnvironment : public TestEnvironmentSpecialized<TestEnvTraits> {
 public:
  ...
  enum class EraseMethod {
    ...
    GlobalRemoveFromContainer,  // Erase of the form
                                // "fidl::internal_wavl::RemoveFromContainer<Tag>(object)"
  };
  ...
  void DoErase(TargetType&& target, size_t ndx, size_t remaining, bool check_ndx = true) {
...

    {
      ...
      if constexpr (Method == EraseMethod::ContainerErase) {
        ...
        tmp = RemoveFromContainer<typename ContainerType::TagType>(target);
      } else {
  ...
  void GlobalRemoveFromContainer() {
    ...
    // erase them using the global fidl::internal_wavl::RemoveFromContainer using a non-default
    // tag.
    if constexpr (PtrTraits::CanCopy) {
      ...
      for (size_t i = 0; i < OBJ_COUNT; ++i) {
        ...
        // scope immediately.
        {
          PtrType tmp = RemoveFromContainer<Tag>(*objects()[ndx]);
          ASSERT_NOT_NULL(tmp);
        }
...
#endif  // SRC_LIB_FIDL_LLCPP_TESTS_DISPATCHER_INTRUSIVE_CONTAINER_BASE_TEST_ENVIRONMENTS_H_
