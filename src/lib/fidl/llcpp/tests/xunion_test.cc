...
TEST(XUnionPayload, Primitive) {
  {
    ...
    int32_t primitive = 5;
    test_union.set_primitive(fidl::unowned(&primitive));
    EXPECT_EQ(llcpp_test::TestUnion::Tag::kPrimitive, test_union.which());
    EXPECT_EQ(5, test_union.primitive());
  }
  {
    int32_t primitive = 5;
    auto test_union = llcpp_test::TestUnion::WithPrimitive(fidl::unowned(&primitive));
    EXPECT_EQ(llcpp_test::TestUnion::Tag::kPrimitive, test_union.which());
    EXPECT_EQ(5, test_union.primitive());
  }
}
...
TEST(XUnionPayload, Struct) {
  ...
  auto test_xunion = llcpp_test::TestXUnion::WithCopyable(fidl::unowned(&copyable));
...
TEST(XUnionPayload, CopyableStruct) {
  {
    ...
    test_union.set_copyable(fidl::unowned(&copyable_struct));
    EXPECT_EQ(llcpp_test::TestUnion::Tag::kCopyable, test_union.which());
  }
  {
    llcpp_test::CopyableStruct copyable_struct{.x = 5};
    auto test_union = llcpp_test::TestUnion::WithCopyable(fidl::unowned(&copyable_struct));
    EXPECT_EQ(llcpp_test::TestUnion::Tag::kCopyable, test_union.which());
  }
}
...
TEST(XUnionPayload, MoveOnlyStruct) {
  {
    ...
    test_union.set_move_only(fidl::unowned(&move_only_struct));
  ...
  {
    llcpp_test::MoveOnlyStruct move_only_struct{.h = zx::handle()};
    auto test_union = llcpp_test::TestUnion::WithMoveOnly(fidl::unowned(&move_only_struct));
    EXPECT_EQ(llcpp_test::TestUnion::Tag::kMoveOnly, test_union.which());
  }
}
...
TEST(MoveUnion, NoDoubleDestructPayload) {
  ...
  memcpy(&dangerous_buffer[4], &h, sizeof(h));
  {
    ...
    llcpp_test::MoveOnlyStruct move_only_struct{.h = zx::handle()};
    union_with_absent_handle.set_move_only(fidl::unowned(&move_only_struct));
...
TEST(XUnion, UnknownTagFlexible) {
  ...
  auto flexible_xunion = llcpp_test::TestXUnion::WithPrimitive(fidl::unowned(&xunion_data));
...
TEST(XUnion, UnknownTagStrict) {
  fidl_xunion_tag_t unknown_tag = 0x01020304;
  int32_t xunion_data = 0x0A0B0C0D;
  auto strict_xunion = llcpp_test::TestStrictXUnion::WithPrimitive(fidl::unowned(&xunion_data));
