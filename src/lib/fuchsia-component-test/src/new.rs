...
impl RealmBuilder {
    ...
    pub async fn add_route(&self, route: impl Into<Route>) -> Result<(), Error> {
...
impl SubRealmBuilder {
    ...
    /// Adds a route between components within the realm
    pub async fn add_route(&self, route: impl Into<Route>) -> Result<(), Error> {
        let mut route: Route = route.into();
        ...
        if !route.capabilities.is_empty() {
            let mut route_targets = route.to.into_iter().map(Into::into).collect::<Vec<_>>();
            self.realm_proxy
                .add_route(
...
mod tests {
    ...
    async fn handle_realm_stream(
        ...
        while let Some(req) = stream.try_next().await.unwrap() {
            match req {
                ...
                ftest::RealmRequest::AddLegacyChild { responder, name, legacy_url, options } => {
                    report_requests
                ...
                ftest::RealmRequest::AddChildRealm { responder, child_realm, name, options } => {
                    ...
                    child_realm_streams.push(fasync::Task::local(async move {
                        handle_realm_stream(child_realm_stream, child_realm_report_requests).await
                    }));
                    responder.send(&mut Ok(())).unwrap();
                }
                ftest::RealmRequest::GetComponentDecl { responder, name } => {
                    report_requests.send(ServerRequest::GetComponentDecl { name }).await.unwrap();
                    responder.send(&mut Ok(fdecl::Component::EMPTY)).unwrap();
                }
                ftest::RealmRequest::ReplaceComponentDecl { responder, name, component_decl } => {
                    report_requests
    ...
    fn new_realm_builder_and_server_task(
        ...
        let builder_task = fasync::Task::local(async move {
