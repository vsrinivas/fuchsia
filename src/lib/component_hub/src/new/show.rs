...
use {
    crate::io::Directory,
    crate::new::list::{get_all_instances, Instance as ListInstance},
...
// Find all CMX/CML components whose component name, ID or URL contains |query| as a substring.
pub async fn find_instances(
    ...
    if list_instances.is_empty() {
        bail!("No matching components found for query \"{}\"", query);
    }
