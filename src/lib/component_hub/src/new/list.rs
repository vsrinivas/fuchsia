...
pub struct Instance {
    ...
    // Hub Directory (only present for CMX components)
    // TODO: Remove this when CMX is deprecated
...
pub async fn get_all_instances(
    ...
    let instances = if let Some(ListFilter::CML) = filter {
        get_all_cml_instances(explorer).await?
    } else if let Some(ListFilter::CMX) = filter {
        get_all_cmx_instances(query).await?
    } else {
...
pub async fn get_all_cmx_instances(query: &fsys::RealmQueryProxy) -> Result<Vec<Instance>> {
    ...
    let (_, resolved) = query
        .get_instance_info("./core/appmgr")
        .await?
        .map_err(|e| format_err!("could not get appmgr instance info from realm query: {:?}", e))?;

    let resolved = resolved.ok_or_else(|| format_err!("appmgr is not resolved"))?;
    let started = resolved.started.ok_or_else(|| format_err!("appmgr is not started"))?;
    let out_dir = started
        .out_dir
        .ok_or_else(|| format_err!("Outgoing directory is not available from appmgr"))?;
...
mod tests {
    ...
    #[fuchsia::test]
    async fn basic_cmx() {
        let (_temp_dir, out_dir) = create_appmgr_out();
