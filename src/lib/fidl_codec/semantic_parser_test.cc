...
TEST_F(SemanticParserTest, GlobalPrintExample) {
  ...
  ASSERT_EQ(out.str(), "MODE_TYPE_DIRECTORY");
}
...
TEST_F(SemanticParserTest, BadFieldName) {
  std::string text =
      "library fuchsia.io {\n"
      "  Directory::Open {\n"
      "    print xx as directory_open_mode;\n"
      "  }\n"
      "}\n";
  std::stringstream error_stream;
  ParserErrors parser_errors(error_stream);
...
TEST_F(SemanticParserTest, TypeNotDefined) {
  ...
  ASSERT_EQ(result,
            "    print mode as directory_open_mode;\n"
            "          ^\n"
            "3:11: Type not defined for field <mode>.\n"
            "    print mode as directory_open_mode;\n"
            "               ^\n"
            "3:16: Symbol ';' expected.\n");
}
...
TEST_F(SemanticParserTest, MissingAs) {
  std::string text =
      "library fuchsia.io {\n"
      "  Directory::Open {\n"
      "    print mode directory_open_mode;\n"
      "  }\n"
      "}\n";
  std::stringstream error_stream;
  ParserErrors parser_errors(error_stream);
...
TEST_F(SemanticParserTest, ResponseField) {
  std::string text =
      "library fuchsia.io {\n"
      "  Node::OnOpen {\n"
      "    print info as directory_open_mode\n"
      "  }\n"
      "}\n";
  std::stringstream error_stream;
  ParserErrors parser_errors(error_stream);
