...
//        1, parsing error or input parameters include negative value or parameter number is not two
//        or three. 2, start frame number < 0. 3, frameCount < 0. 4, interval <= 0
//        .......
    ...
    if ((!parameter.empty())) {
      if (parameter.compare("all") == 0) {
        ...
        if (!checkScreenShotFrameRangeString(rangeString)) {
          ...
          int frameCount = 0;
          int itemCount = sscanf(parameter.c_str(), "%d-%d-%d", &pFrameRange->startFrame,
                                 &frameCount, &pFrameRange->interval);
          if (itemCount >= 2) {
            if (itemCount == 2) {
              pFrameRange->interval = SCREEN_SHOT_FRAMES_INTERVAL_DEFAULT;
            }
