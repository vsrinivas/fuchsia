...
#ifndef SRC_LIB_INSPECT_DEPRECATED_DEPRECATED_OBJECT_DIR_H_
#define SRC_LIB_INSPECT_DEPRECATED_DEPRECATED_OBJECT_DIR_H_
...
class ObjectDir {
 public:
  ...
  static ObjectDir Make(std::string name) { return ObjectDir(Object::Make(std::move(name))); }
  ...
  template <typename T>
  typename std::enable_if_t<std::is_lvalue_reference<T>::value, bool>::type set_prop(
      ObjectPath path, std::string name, T value) const {
    return inner_set_prop(std::move(path), std::move(name), Property(std::move(value)));
  }
  ...
  void set_child(std::shared_ptr<Object> obj) const { set_child({}, std::move(obj)); }
  ...
  // All objects along the path that do not exist will be initialized.
  void set_children_callback(ObjectPath path, Object::ChildrenCallback callback) const;

 private:
  // Inner implementation of setting properties by path.
  bool inner_set_prop(ObjectPath path, std::string name, Property property) const;
...
#endif  // SRC_LIB_INSPECT_DEPRECATED_DEPRECATED_OBJECT_DIR_H_
