...
bool ObjectDir::inner_set_prop(ObjectPath path, std::string name, Property property) const {
  auto dir = find(path);
  return dir ? dir.object()->SetProperty(name, std::move(property)) : false;
}

bool ObjectDir::set_metric(ObjectPath path, std::string name, Metric metric) const {
...
void ObjectDir::set_children_callback(ObjectPath path, Object::ChildrenCallback callback) const {
  if (object_) {
