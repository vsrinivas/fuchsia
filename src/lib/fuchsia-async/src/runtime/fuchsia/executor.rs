...
mod tests {
    ...
    // This task spawns another tasks, which completes, and waits for two timers
    // and send & receives one IO packet. Min polls is 6
    async fn simple_task() {
        ...
        let t = crate::Task::spawn(async move {
            ...
            Timer::new(Time::after(1000.nanos())).await; // Note this is after the first timer
        });
        Timer::new(Time::after(1000.nanos())).await;
    ...
    fn snapshot_sanity_check(snapshot: &Snapshot, extra_tasks: usize) {
        assert!(snapshot.polls >= 6);
        assert_eq!(snapshot.tasks_created - extra_tasks, 2);
        assert_eq!(snapshot.tasks_completed - extra_tasks, 1);
        assert_eq!(snapshot.wakeups_io, 1);
        assert_eq!(snapshot.wakeups_deadline, 2);
        assert!(snapshot.wakeups_notification > 0);
        assert!(snapshot.ticks_awake > 0);
        assert!(snapshot.ticks_asleep > 0);
    }
    ...
    fn instrumentation_stepwise_sanity_check() {
        let mut executor = Executor::new_with_fake_time().unwrap();
        executor.set_fake_time(Time::from_zx(zx::Time::from_nanos(0)));
        let fut = simple_task();
        pin_mut!(fut);
        assert!(executor.run_until_stalled(&mut fut).is_pending());
        executor.set_fake_time(Time::from_zx(zx::Time::from_nanos(1000)));
        executor.wake_expired_timers();
        assert!(executor.run_until_stalled(&mut fut).is_pending());
        executor.set_fake_time(Time::from_zx(zx::Time::from_nanos(2000)));
        executor.wake_expired_timers();
