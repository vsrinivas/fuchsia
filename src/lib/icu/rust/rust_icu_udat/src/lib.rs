...
impl UDateFormat {
    ...
    pub fn parse_from_position(
        ...
        common::Error::ok_or_warning(status)?;
        Ok(Parsed { date, end_position: end_position as usize })
    }
...
mod tests {
    ...
    fn test_format_pattern() -> Result<(), common::Error> {
        ...
        let tests = vec![
            ...
            Test { date: 100000.0, pattern: "yyyy-MM-dd'T'HH", expected: "1969-12-31T19" },
            Test { date: 100000.0, pattern: "V", expected: "usnyc" },
        ];
        ...
        for t in tests {
            ...
            let actual = fmt.format(t.date)?;
            assert_eq!(actual, t.expected, "want: {:?}, got: {:?}", t.expected, actual);
        }
        Ok(())
    }
    ...
    fn parse_utf8() -> Result<(), common::Error> {
        ...
        for test in tests {
            ...
            assert_eq!(actual, test.expected, "want: {:?}, got: {:?}", test.expected, actual)
        }
        Ok(())
    }
}
