...
zx::status<std::unique_ptr<Reader>> DeviceReader::Create(fxl::WeakPtr<Controller> controller,
  ...
  auto status = zx::make_status(
      zx::vmar::root_self()->allocate(ZX_VM_CAN_MAP_READ, 0u, buffer_size, &vmar, &addr));
  if (status.is_error()) {
    FX_LOGS(ERROR) << "Unable to obtain vmar for reading trace data: " << status.status_string();
    return status.take_error();
  }

  std::unique_ptr<Reader> out_reader;
  out_reader.reset(new DeviceReader(std::move(controller), buffer_size, std::move(vmar)));
  return zx::ok(std::move(out_reader));
}
...
bool DeviceReader::MapBuffer(const std::string& name, uint32_t trace_num) {
  ...
  auto vmo = controller_->GetBufferHandle(name, trace_num);
  if (vmo.is_error()) {
    return false;
  }
