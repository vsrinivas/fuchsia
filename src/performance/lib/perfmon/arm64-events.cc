...
void RegisterArmv8Events(internal::EventRegistry* registry) {
  ...
  registry->RegisterEvents("armv8", "fixed", g_fixed_event_details, g_num_fixed_event_details);
  registry->RegisterEvents("armv8", "arch", g_arch_event_details, g_num_arch_event_details);
}
...
void RegisterAllArm64ModelEvents(EventRegistry* registry) { RegisterArmv8Events(registry); }
