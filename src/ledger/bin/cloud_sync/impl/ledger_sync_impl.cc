...
LedgerSyncImpl::LedgerSyncImpl(ledger::Environment* environment, const UserConfig* user_config,
  ...
  if (!user_config_->cloud_provider) {
    LEDGER_LOG(ERROR) << "Instantiated a LedgerSyncImpl with an invalid cloud provider.";
  }
  aggregator_.SetBaseWatcher(user_watcher_.get());
}

LedgerSyncImpl::~LedgerSyncImpl() {
  LEDGER_DCHECK(active_page_syncs_.empty());
...
void LedgerSyncImpl::CreatePageSync(
  ...
  if (!user_config_->cloud_provider) {
    ...
    LEDGER_LOG(WARNING) << "Skipped initializing the cloud sync. "
  ...
  encryption_service_->GetPageId(
      page_storage->GetId(), [this, page_storage, page_sync_client, callback = std::move(callback)](
                                 encryption::Status status, std::string page_id) {
        if (status != encryption::Status::OK) {
          LEDGER_LOG(ERROR)
              << "Failed to get the encoded version of page_id from the encryption service.";
          callback(storage::Status::INTERNAL_ERROR, nullptr);
          return;
        }
        cloud_provider::PageCloudPtr page_cloud;
        user_config_->cloud_provider->GetPageCloud(
            convert::ToArray(app_id_), convert::ToArray(page_id), page_cloud.NewRequest(),
            [](auto status) {
              if (status != cloud_provider::Status::OK) {
                LEDGER_LOG(ERROR) << "Failed to retrieve page cloud, status: "
        ...
        auto page_sync = std::make_unique<PageSyncImpl>(
            ...
            page_sync_client, encryption_service_, std::move(page_cloud),
            environment_->MakeBackoff(), environment_->MakeBackoff(),
