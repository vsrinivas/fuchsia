...
P2PProviderImpl::P2PProviderImpl(std::string host_name,
...
bool P2PProviderImpl::SendMessage(fxl::StringView destination, fxl::StringView data) {
  ...
  network_service_provider_.AddServiceForName(
      [this](zx::channel channel) {
        ...
        connection.set_on_message([this, &connection](std::vector<uint8_t> data) {
          ProcessHandshake(&connection, std::move(data), true, fxl::StringView());
        });
        connection.Start(std::move(channel));
      },
      kRespondingServiceName + user_id_);
  net_connector_->RegisterServiceProvider(kRespondingServiceName + user_id_, std::move(handle));
...
void P2PProviderImpl::ProcessHandshake(RemoteConnection* connection, std::vector<uint8_t> data,
  ...
  std::string remote_name(message->host_name()->begin(), message->host_name()->end());
  if (!network_remote_name.empty() && network_remote_name != remote_name) {
    ...
    FXL_LOG(ERROR) << "Network name " << network_remote_name << " different from declared name "
                   << remote_name << ", aborting.";
    connection->Disconnect();
    return;
  }
  ...
  connection->set_on_message(
      [this, remote_name](std::vector<uint8_t> data) { Dispatch(remote_name, std::move(data)); });
  ...
  if (should_send_handshake) {
    ...
    flatbuffers::Offset<Handshake> request =
        CreateHandshake(buffer, kCurrentVersion, convert::ToFlatBufferVector(&buffer, host_name_));
    flatbuffers::Offset<Envelope> envelope =
...
void P2PProviderImpl::ListenForNewDevices(uint64_t version) {
  net_connector_->GetKnownDeviceNames(version, [this](uint64_t new_version,
    ...
    for (auto& remote_name : devices) {
      ...
      net_connector_->GetDeviceServiceProvider(remote_name, device_service_provider.NewRequest());

      device_service_provider->ConnectToService(kRespondingServiceName + user_id_,
                                                std::move(remote));

      flatbuffers::FlatBufferBuilder buffer;
      flatbuffers::Offset<Handshake> request = CreateHandshake(
          buffer, kCurrentVersion, convert::ToFlatBufferVector(&buffer, host_name_));
      flatbuffers::Offset<Envelope> envelope =
          CreateEnvelope(buffer, EnvelopeMessage_Handshake, request.Union());
      buffer.Finish(envelope);

      RemoteConnection& connection = connections_.emplace();
      connection.set_on_message([this, &connection, remote_name_str](std::vector<uint8_t> data) {
        ProcessHandshake(&connection, std::move(data), false, remote_name_str);
      });
...
void P2PProviderImpl::Dispatch(fxl::StringView source, std::vector<uint8_t> data) {
  FXL_DCHECK(client_);
  flatbuffers::Verifier verifier(reinterpret_cast<const unsigned char*>(data.data()), data.size());
  if (!VerifyEnvelopeBuffer(verifier)) {
    // Wrong serialization, abort.
...
void P2PProviderImpl::OnDeviceChange(fxl::StringView remote_device, DeviceChangeType change_type) {
  FXL_DCHECK(client_);
