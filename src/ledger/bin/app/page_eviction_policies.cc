...
std::unique_ptr<PageEvictionPolicy> NewAgeBasedPolicy(
    coroutine::CoroutineService* coroutine_service,
    PageEvictionDelegate* delegate, timekeeper::Clock* clock) {
  return NewAgeBasedPolicy(coroutine_service, delegate, clock,
                           kUnusedTimeLimit);
}
...
std::unique_ptr<PageEvictionPolicy> NewAgeBasedPolicy(
    ...
    PageEvictionDelegate* delegate, timekeeper::Clock* clock,
    zx::duration unused_time_limit) {
  return std::make_unique<AgeBasedPageEvictionPolicy>(
      coroutine_service, delegate, clock, unused_time_limit);
}
