...
class LedgerMemoryEstimator {
 public:
  ...
  // Initializes the |LedgerMemoryEstimator|. This must be called before any call to
...
// Updates |memory| to store the memory usage, in bytes, of the current process. This only includes
// the private bytes, not the shared memory. Returns true on success; false otherwise.
