...
TEST_F(DbSerialization, SerializationVersionControl) {
  ...
  // Merge row.
  EXPECT_EQ("merges/parent1/parent2/merge", MergeRow::GetKeyFor("parent1", "parent2", "merge"));
  ...
  EXPECT_EQ(
      ...
      ReferenceRow::GetKeyForObject(ObjectDigest("source"), ObjectDigest("destination"),
                                    KeyPriority::LAZY));
  EXPECT_EQ(fxl::Concatenate({"refcounts/", destination_size_str, "destination/commit/source"}),
  ...
  EXPECT_EQ(fxl::Concatenate({"transient/object_digests/", identifier_serialization}),
            ObjectStatusRow::GetKeyFor(PageDbObjectStatus::TRANSIENT, identifier));

  // Object Status: Local row.
  EXPECT_EQ(fxl::Concatenate({"local/object_digests/", identifier_serialization}),
            ObjectStatusRow::GetKeyFor(PageDbObjectStatus::LOCAL, identifier));

  // Object Status: Synced row.
  EXPECT_EQ(fxl::Concatenate({"synced/object_digests/", identifier_serialization}),
...
TEST_F(DbSerialization, ReferenceRowNoCollision) {
  ...
  EXPECT_EQ(fxl::Concatenate({object1.Serialize(), ReferenceRow::kObjectPrefix,
