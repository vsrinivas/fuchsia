...
#ifndef SRC_LEDGER_LIB_MEMORY_REF_PTR_H_
#define SRC_LEDGER_LIB_MEMORY_REF_PTR_H_
...
#include "src/ledger/lib/memory/ref_ptr_internal.h"
...
class RefPtr final {
 ...
 private:
  ...
  enum AdoptTag { ADOPT };
  RefPtr(T* ptr, AdoptTag) : ptr_(ptr) { LEDGER_DCHECK(ptr_); }
...
namespace std {
template <typename T>
struct hash<ledger::RefPtr<T>> {
  using argument_type = ledger::RefPtr<T>;
...
#endif  // SRC_LEDGER_LIB_MEMORY_REF_PTR_H_
