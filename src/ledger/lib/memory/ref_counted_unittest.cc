...
class MyClass : public RefCountedThreadSafe<MyClass> {
 ...
 private:
  LEDGER_FRIEND_REF_COUNTED_THREAD_SAFE(MyClass);
  LEDGER_FRIEND_MAKE_REF_COUNTED(MyClass);
...
class MySubclass final : public MyClass {
 ...
 private:
  LEDGER_FRIEND_REF_COUNTED_THREAD_SAFE(MySubclass);
  LEDGER_FRIEND_MAKE_REF_COUNTED(MySubclass);
...
class MyPublicClass : public RefCountedThreadSafe<MyPublicClass> {
 public:
  ...
  MyPublicClass(const MyPublicClass&) = delete;
  MyPublicClass& operator=(const MyPublicClass&) = delete;
