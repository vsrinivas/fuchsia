...
#ifndef SRC_LEDGER_LIB_MEMORY_WEAK_PTR_H_
#define SRC_LEDGER_LIB_MEMORY_WEAK_PTR_H_
...
class WeakPtrFactory {
 public:
  explicit WeakPtrFactory(T* ptr) : ptr_(ptr) { LEDGER_DCHECK(ptr_); }
  ~WeakPtrFactory() {
    InvalidateWeakPtrs();
    LEDGER_DCHECK(*reinterpret_cast<uintptr_t volatile*>(const_cast<T**>(&ptr_)) =
                      kPoisonedPointer);
  }

  WeakPtrFactory(const WeakPtrFactory<T>&) = delete;
  WeakPtrFactory& operator=(const WeakPtrFactory<T>&) = delete;

  // Gets a new weak pointer, which will be valid until either
  // |InvalidateWeakPtrs()| is called or this object is destroyed.
  WeakPtr<T> GetWeakPtr() {
    LEDGER_DCHECK(reinterpret_cast<uintptr_t>(ptr_) != kPoisonedPointer);
    if (!flag_)
...
#endif  // SRC_LEDGER_LIB_MEMORY_WEAK_PTR_H_
