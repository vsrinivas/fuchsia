...
#ifndef SRC_LEDGER_LIB_CALLBACK_TRACE_CALLBACK_H_
#define SRC_LEDGER_LIB_CALLBACK_TRACE_CALLBACK_H_
...
class TracingLambda {
 public:
  ...
  TracingLambda(TracingLambda&& other) noexcept
    ...
    LEDGER_DCHECK(!other.did_run_or_moved_out_);
    other.did_run_or_moved_out_ = true;
  }

  TracingLambda(const TracingLambda&) = delete;
  TracingLambda& operator=(const TracingLambda&) = delete;
  ...
  auto operator()(ArgType&&... args) const {
    LEDGER_DCHECK(!did_run_or_moved_out_);
...
#define TRACE_CALLBACK(cb, category, name, args...)                                               \
  ([&]() mutable {                                                                                \
    ...
    return (                                                                                      \
        TRACE_ENABLED()                                                                           \
            ? ::ledger::internal::TraceCallback(                                                  \
                  std::move(trace_local_cb__),                                                    \
                  ::ledger::internal::CheckConstantCString<__builtin_strlen(category)>(category), \
                  ::ledger::internal::CheckConstantCString<__builtin_strlen(name)>(name),         \
                  name "_callback", ##args)                                                       \
            : ::ledger::internal::TraceCallback(std::move(trace_local_cb__)));                    \
  }())

#endif  // SRC_LEDGER_LIB_CALLBACK_TRACE_CALLBACK_H_
