...
void AmlMipiDevice::DumpCsiPhyRegs() {
  zxlogf(TRACE, "%s: MIPI CSI PHY REGS VALUES\n", __FUNCTION__);
  for (int i = 0; i < 27; i++) {
    zxlogf(TRACE, "0x%x  ---> 0x%x \n", (0xFF650000 + i * 4), csi_phy0_mmio_->Read32(i * 4));
  }
}

void AmlMipiDevice::DumpAPhyRegs() {
  zxlogf(TRACE, "%s: MIPI APHY REGS VALUES\n", __FUNCTION__);
...
void AmlMipiDevice::DumpHostRegs() {
  zxlogf(TRACE, "%s: MIPI HOST REGS VALUES\n", __FUNCTION__);
  for (int i = 0; i < 8; i++) {
...
void AmlMipiDevice::DumpFrontEndRegs() {
  zxlogf(TRACE, "%s: MIPI ADAP FRONTEND REGS VALUES\n", __FUNCTION__);
...
void AmlMipiDevice::DumpReaderRegs() {
  zxlogf(TRACE, "%s: MIPI ADAP READER REGS VALUES\n", __FUNCTION__);
...
void AmlMipiDevice::DumpAlignRegs() {
  zxlogf(TRACE, "%s: MIPI ADAP ALIGN REGS VALUES\n", __FUNCTION__);
...
void AmlMipiDevice::DumpPixelRegs() {
  zxlogf(TRACE, "%s: MIPI ADAP PIXEL REGS VALUES\n", __FUNCTION__);
...
void AmlMipiDevice::DumpMiscRegs() {
  zxlogf(TRACE, "%s: MIPI ADAP MISC REGS VALUES\n", __FUNCTION__);
