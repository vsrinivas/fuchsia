...
bool NodeIsInPlace(const InternalConfigNode& node) { return !node.output_constraints.has_value(); }
...
TEST(ConfigTest, InternalMonitorConfiguration) {
  ...
  EXPECT_EQ(NodeIsInPlace(ge2d_ds_node), true);

  EXPECT_EQ(output_ds_node.image_formats.size(), 3u);

  // FR graph validation.
  input_node = fr;
  ASSERT_EQ(fr.child_nodes.size(), 1u);
  auto passthrough_fr_node = fr.child_nodes[0];
  auto output_fr_node = passthrough_fr_node.child_nodes[0];
  gdc_ds_node = passthrough_fr_node.child_nodes[1];
  ASSERT_EQ(gdc_ds_node.child_nodes.size(), 1u);
  output_ds_node = gdc_ds_node.child_nodes[0];

  EXPECT_EQ(NodeType::kInputStream, input_node.type);
  EXPECT_EQ(NodeType::kPassthrough, passthrough_fr_node.type);
  EXPECT_EQ(NodeType::kGdc, gdc_ds_node.type);
