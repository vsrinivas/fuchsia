...
class TaskTest : public zxtest::Test {
 protected:
  void SetUpBufferCollections(uint32_t buffer_collection_count) {
    ...
    zx_status_t status =
        CreateFakeBufferCollection(&input_buffer_collection_, bti_handle_,
                                   kWidth, kHeight, buffer_collection_count);
    ASSERT_OK(status);

    status =
        CreateFakeBufferCollection(&output_buffer_collection_, bti_handle_,
                                   kWidth, kHeight, buffer_collection_count);
    ASSERT_OK(status);

    status =
        zx_vmo_create_contiguous(bti_handle_, kConfigSize, 0, &config_vmo_);
    ASSERT_OK(status);
  }
...
TEST_F(TaskTest, BasicCreationTest) {
  ...
  zx_status_t status = gdc::Task::Create(
      &input_buffer_collection_, &output_buffer_collection_,
      zx::vmo(config_vmo_), &callback_, zx::bti(bti_handle_), &task);
  EXPECT_OK(status);
}
