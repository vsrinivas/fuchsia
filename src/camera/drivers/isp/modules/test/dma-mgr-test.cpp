...
zx_status_t CreateFakeBufferCollection(fuchsia_sysmem_BufferCollectionInfo* buffer_collection,
...
protected:
    ...
    void SetUp() override {
        ...
        ASSERT_OK(DmaManager::Create(*zx::unowned_bti(bti_handle_),
                                     ddk::MmioView(local_mmio_buffer, 0),
                                     DmaManager::Stream::FullResolution,
                                     &full_resolution_dma_));
        ASSERT_OK(DmaManager::Create(*zx::unowned_bti(bti_handle_),
                                     ddk::MmioView(local_mmio_buffer, 0),
                                     DmaManager::Stream::Downscaled,
                                     &downscaled_dma_));
        zx_status_t status =
            CreateFakeBufferCollection(&full_resolution_buffer_collection_, bti_handle_,
                                       kFullResWidth, kFullResHeight, kFullResNumberOfBuffers);
        ASSERT_OK(status);
        status = CreateFakeBufferCollection(&downscaled_buffer_collection_, bti_handle_,
    ...
    void ConnectToStreams() {
        ...
        status = downscaled_dma_->Start(
            downscaled_buffer_collection_, fit::bind_member(this, &DmaMgrTest::DownScaledCallback));
        EXPECT_OK(status);
    }
...
// Make sure a new address is written to the dma frame every time we call OnNewFrame:
...
TEST_F(DmaMgrTest, DieOnInvalidFrameWritten) {
    ...
    ASSERT_DEATH(([this]() { full_resolution_dma_->OnPrimaryFrameWritten();
               full_resolution_dma_->OnSecondaryFrameWritten(); }));
    ConnectToStreams();
    ...
    ASSERT_DEATH(([this]() { full_resolution_dma_->OnPrimaryFrameWritten();
               full_resolution_dma_->OnSecondaryFrameWritten(); }));
    full_resolution_dma_->OnNewFrame();
    ASSERT_NO_DEATH(([this]() { full_resolution_dma_->OnPrimaryFrameWritten();
               full_resolution_dma_->OnSecondaryFrameWritten(); }));
}
...
TEST_F(DmaMgrTest, MultipleStartCalls) {
    ...
    // DmaMgr cannot tell the difference between vmos, so we can just pass in the same ones.
    ConnectToStreams();

    // Now we should die because we don't have any frames that we are writing:
    ASSERT_DEATH(([this]() { downscaled_dma_->OnPrimaryFrameWritten();
