...
class ExecutionContext {
 public:
  ...
  std::shared_ptr<ObjectSchema> GetObjectSchema(const NodeId& node_id) const {
  ...
  void AddObjectSchema(std::shared_ptr<ObjectSchema> object_schema);
 ...
 private:
  ...
  // Object schemas for the context. They are used to hold unnamed classes which are used for object
  // literals.
...
class Interpreter {
 public:
  ...
  size_t object_schema_count() const { return object_schema_count_; }
  void increment_object_schema_count() { ++object_schema_count_; }
  void decrement_object_schema_count() { --object_schema_count_; }
 ...
 private:
  ...
  // The current allocated schema count.
  size_t object_schema_count_ = 0;
};
