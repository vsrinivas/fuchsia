...
//! Implementation of the daemon diagnostics streaming service.
...
#[ffx_service(bridge::TargetCollectionMarker)]
#[derive(Default)]
pub struct Diagnostics {}

#[async_trait(?Send)]
impl FidlService for Diagnostics {
    type Service = bridge::DiagnosticsMarker;
        ...
        match req {
            bridge::DiagnosticsRequest::StreamDiagnostics {
                ...
                if stream_mode == bridge::StreamMode::SnapshotAll {
                    ...
                    // TODO(fxb/90340): Add a services utility library for feed-forward things like this.
                    let tc_proxy = self.open_target_collection_proxy(cx).await?;
                    // cx.open_service_proxy::<bridge::TargetCollectionMarker>().await?;
...
mod test {
    ...
    use services::testing::*;

    #[fuchsia_async::run_singlethreaded(test)]
    async fn test_no_stream_mode() {
        let daemon = FakeDaemonBuilder::new().register_fidl_service::<Diagnostics>().build();
    ...
    async fn test_no_targets() {
        let daemon = FakeDaemonBuilder::new()
            .register_fidl_service::<Diagnostics>()
            .register_instanced_service_closure::<bridge::TargetCollectionMarker, _>(|_cx, req| {
                match req {
