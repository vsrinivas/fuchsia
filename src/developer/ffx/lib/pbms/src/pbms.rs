...
where
    W: Write + Sync,
{
    let backend = match (metadata_repo_uri.scheme(), blob_repo_uri.scheme()) {
        (GS_SCHEME, GS_SCHEME) => {
            ...
            Box::new(GcsRepository::new(client, metadata_repo_uri.clone(), blob_repo_uri.clone())?)
                as Box<dyn RepositoryBackend + Send + Sync + 'static>
        }
        ("http" | "https", "http" | "https") => {
            let client = new_https_client();
            Box::new(HttpRepository::new(client, metadata_repo_uri.clone(), blob_repo_uri.clone()))
                as Box<dyn RepositoryBackend + Send + Sync + 'static>
        }
        ("file", "file") => {
    ...
    let trusted_targets =
        pkg::resolve::resolve_repository_metadata(&repo, &metadata_dir).await.with_context(
            || format!("downloading repository {} {}", metadata_repo_uri, blob_repo_uri),
        )?;
