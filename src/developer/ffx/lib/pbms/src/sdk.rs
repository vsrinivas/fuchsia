...
pub(crate) async fn fetch_from_sdk(version: &str) -> Result<()> {
    ...
    loop {
        ...
        let local_path = local_metadata_path().await.context("local metadata path")?;
        match client.fetch("fuchsia-sdk", &gcs_path, &local_path).await {
            Ok(()) => break,
...
pub(crate) async fn local_entries() -> Result<Entries> {
    let mut container = File::open(&local_metadata_path().await.context("local metadata path")?)
        .map(BufReader::new)?;
...
mod tests {
    ...
    #[fuchsia_async::run_singlethreaded(test)]
    async fn test_local_metadata_path() {
        ...
        ffx_config::set(("pbms.data.path", ConfigLevel::User), path_str.into())
            .await
            .expect("set path");
        let path = local_metadata_path().await.expect("get path");
