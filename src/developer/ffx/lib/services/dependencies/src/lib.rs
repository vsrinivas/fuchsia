...
//! See //src/developer/ffx/build/templates/services_macro.md for more details.
    ...
    fn visit(&mut self, protocol: &'static str) -> Result<(), VerifyError> {
        let (build_target, deps) = if let Some(node) = self.nodes.get_mut(protocol) {
            ...
            return Err(VerifyError::InvalidDependencyError(format!(
                "'{}' is not a dependency to a service.",
                protocol
            )));
        };
...
depends on '{}' which is not a service endpoint.
Please amend the ffx_service declaration in '{}'.
Services are only permitted to declare dependencies on other known services.
Verify '{}' is not included in the services suite under //src/developer/ffx/daemon/services
...
pub struct Node {
    ...
    /// Adjacency list of service library FIDL protocols depended upon.
