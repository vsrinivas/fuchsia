...
/// Generates the necessary glue code to use your service within the daemon.
///
/// When using this, if you depend on other services within the daemon, make
...
pub fn ffx_service(attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let mut q: proc_macro2::TokenStream = quote! {
        ...
        pub const SERVICE_DEPS: &'static [&'static str]
        ...
        q.extend(quote! {
            impl #struct_name {
                #(
                    ...
                    {
                        cx.open_service_proxy::<#deps>().await
                    }
                )*
            }
        });
