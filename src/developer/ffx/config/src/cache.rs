...
fn init_impl(context: &EnvironmentContext) -> Result<()> {
...
mod test {
    ...
    fn test_expiration_check_does_not_panic() -> Result<()> {
        ...
        let item = CacheItem {
            created: later,
            config: Arc::new(RwLock::new(Config::from_env(&Environment::new_empty(
