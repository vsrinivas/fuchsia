...
impl Environment {
    ...
    pub(crate) fn new_empty(context: EnvironmentContext) -> Result<Self> {
        let _lock = Self::lock_env(&context.env_path()?)?;

        let files = EnvironmentFiles::default();
        Ok(Self { context, files, _lock })
    }
...
mod test {
    ...
    async fn build_config_manual_configure() {
        ...
        assert!(!env_path.is_file(), "Environment file shouldn't exist yet");
        let mut env =
            Environment::new_empty(context.clone()).expect("Creating new empty environment file");
