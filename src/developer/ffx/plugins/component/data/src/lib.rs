...
use ffx_component_data_args::Provider;
use {
...
pub async fn data(remote_proxy: RemoteControlProxy, args: DataCommand) -> Result<()> {
    ...
    match remote_proxy.connect(selector, server).await.context("awaiting connect call")? {
        Ok(_) => {
            ...
            data_cmd(storage_admin, args.subcommand).await
        }
        Err(e) => {
...
async fn data_cmd(storage_admin: StorageAdminProxy, subcommand: SubcommandEnum) -> Result<()> {
    match subcommand {
