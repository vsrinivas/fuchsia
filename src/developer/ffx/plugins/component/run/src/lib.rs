...
pub async fn run(rcs_proxy: rc::RemoteControlProxy, cmd: RunComponentCommand) -> Result<()> {
    ...
    run_impl(lifecycle_controller, cmd.url, cmd.name, &mut std::io::stdout()).await
}
...
async fn run_impl<W: std::io::Write>(
    ...
    match create_result {
        Err(fcomponent::Error::InstanceAlreadyExists) => {
            ffx_bail!("Component instance already exists. Use `ffx component destroy` or the `--name` flag to resolve this conflict.")
        }
        Err(e) => {
