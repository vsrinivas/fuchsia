...
impl Images {
    ...
    pub fn find_path(&self, name: &str, image_type: &str) -> Result<String> {
        self.0
            .iter()
            .find(|x| x.name == name && x.image_type == image_type)
            .map(|i| i.path.clone())
            .ok_or(anyhow!("cannot find matching image artifact"))
    }
}
...
mod test {
    ...
    async fn test_image_parse() -> Result<()> {
        ...
        assert_eq!(images.find_path("storage-full", "blk")?, "obj/build/images/fvm.blk");
        assert_eq!(images.find_path("qemu-kernel", "kernel")?, "multiboot.bin");
        assert_eq!(images.find_path("zircon-a", "zbi")?, "fuchsia.zbi");
        Ok(())
    }
}
