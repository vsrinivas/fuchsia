...
use ffx_emulator_common::{
    ...
    instances::{clean_up_instance_dir, get_all_instances, get_instance_dir},
};
...
pub async fn stop(cmd: StopCommand, proxy: TargetCollectionProxy) -> Result<()> {
    ...
    if cmd.all {
        names = match get_all_instances(&ffx_config).await {
    ...
    for mut some_name in names {
        ...
        if !cmd.persist {
            if let Ok(path) = get_instance_dir(&ffx_config, &name, false).await {
