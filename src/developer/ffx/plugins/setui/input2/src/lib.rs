...
use ffx_setui_input2_args::Input;
...
pub async fn run_command(input_proxy: InputProxy, input: Input) -> Result<()> {
    handle_mixed_result("Input2", command(input_proxy, InputState::from(input)).await).await
}
...
async fn command(proxy: InputProxy, mut input_state: InputState) -> WatchOrSetResult {
    if input_state == InputState::EMPTY {
        ...
        Ok(Either::Set(if let Err(err) = proxy.set_states(&mut states).await? {
            format!("{:?}", err)
        } else {
...
mod test {
    ...
    async fn test_run_command() {
        let proxy = setup_fake_input_proxy(move |req| match req {
            InputRequest::Set { .. } => {
                panic!("Unexpected call to watch set");
            }
            InputRequest::Watch { .. } => {
            ...
            InputRequest::SetStates { responder, .. } => {
    ...
    #[fuchsia_async::run_singlethreaded(test)]
    async fn validate_input2_set_output(mut expected_input: Input) -> Result<()> {
        let proxy = setup_fake_input_proxy(move |req| match req {
            InputRequest::Set { .. } => {
                panic!("Unexpected call to watch set");
            }
    ...
    #[fuchsia_async::run_singlethreaded(test)]
    async fn validate_input2_watch_output() -> Result<()> {
        let proxy = setup_fake_input_proxy(move |req| match req {
            InputRequest::Set { .. } => {
