...
// Tests different types of casting when there are derived classes.
TEST(Cast, Derived) {
  // Set up two structs with an inherited relationship. The object layout is
  //   - Base #1 class data: 2 bytes int16.
  //   - Base class data: 4 bytes int32
  //   - Derived class data: 4 bytes int32
  // The two bytes of nothing wouldn't be generated by a compiler for this
  // structure but allow us to test the offset computation. In practice this
  // would happen for multiple inheritance if there was another 2-byte struct
  // there.
  constexpr uint64_t kBase1Offset = 0;
  constexpr uint64_t kBase2Offset = 2;
  auto int16_type = MakeInt16Type();
  auto int32_type = MakeInt32Type();
  auto base1_type = MakeCollectionType(DwarfTag::kStructureType, "Base1",
                                       {{"b", int16_type}});
  auto base2_type = MakeCollectionType(DwarfTag::kStructureType, "Base2",
                                       {{"b", int32_type}});
  auto derived_type = MakeCollectionTypeWithOffset(
  ...
  // Class holds two integers, base followed by derived data.
  ...
  std::vector<uint8_t> expected_base1_storage = {kB1Value, 0};
  std::vector<uint8_t> expected_base2_storage = {kB2Value, 0, 0, 0};

  // ExprValue holding the derived class.
  constexpr uint64_t kDerivedAddr = 0x3000;
  constexpr uint64_t kBase1Addr = kDerivedAddr + kBase1Offset;
  constexpr uint64_t kBase2Addr = kDerivedAddr + kBase2Offset;
  ExprValue derived_value(derived_type, derived_storage,
                          ExprValueSource(kDerivedAddr));
  ExprValue base1_value(base2_type, expected_base1_storage,
                        ExprValueSource(kBase1Addr));
  ExprValue base2_value(base2_type, expected_base2_storage,
                        ExprValueSource(kBase2Addr));

  // Should be able to implicit cast derived to base 1 object.
  ExprValue result;
  Err err = CastExprValue(CastType::kImplicit, derived_value, base1_type,
                          &result, ExprValueSource());
  EXPECT_FALSE(err.has_error());
  EXPECT_EQ(base1_type.get(), result.type());
  EXPECT_EQ(base1_value, result);
  // Check the source explicitly since == doesn't check sources.
  EXPECT_EQ(base1_value.source(), result.source());

  // Same for base 2.
  err = CastExprValue(CastType::kImplicit, derived_value, base2_type, &result,
                      ExprValueSource());
  EXPECT_FALSE(err.has_error());
  EXPECT_EQ(base2_type.get(), result.type());
  EXPECT_EQ(base2_value, result);
  // Check the source explicitly since == doesn't check sources.
  EXPECT_EQ(base2_value.source(), result.source());

  // Should not be able to implicit cast from base to derived.
  result = ExprValue();
  err = CastExprValue(CastType::kImplicit, base2_value, derived_type, &result,
                      ExprValueSource());
  EXPECT_TRUE(err.has_error());

  // Now test pointer casting.
  auto base_ptr_type = fxl::MakeRefCounted<ModifiedType>(
      DwarfTag::kPointerType, LazySymbol(base2_type));
  auto derived_ptr_type = fxl::MakeRefCounted<ModifiedType>(
      DwarfTag::kPointerType, LazySymbol(derived_type));

  // Should be able to implicit cast derived ptr to base ptr type.
  // This data matches kDerivedAddr in 64-bit little endian.
  std::vector<uint8_t> derived_ptr_storage = {0, 0x30, 0, 0, 0, 0, 0, 0};
  ExprValue derived_ptr_value(derived_ptr_type, derived_ptr_storage,
                              ExprValueSource());
  err = CastExprValue(CastType::kImplicit, derived_ptr_value, base_ptr_type,
                      &result, ExprValueSource());
  EXPECT_FALSE(err.has_error()) << err.msg();

  // That should have adjusted the pointer value appropriately.
  ExprValue base2_ptr_value(base_ptr_type, {0x02, 0x30, 0, 0, 0, 0, 0, 0},
                            ExprValueSource());
  EXPECT_EQ(base2_ptr_value, result);

  // Should not allow implicit casts from Base to Derived.
  err = CastExprValue(CastType::kImplicit, base2_ptr_value, derived_ptr_type,
                      &result, ExprValueSource());
  EXPECT_TRUE(err.has_error());
