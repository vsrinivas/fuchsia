...
TEST(Cast, Implicit) {
  ...
  ExprValue result;
  Err err = CastExprValue(eval_context.get(), CastType::kImplicit, int32_one,
                          int32_type, &result);
  ...
  // 0xffffffff),
  err = CastExprValue(eval_context.get(), CastType::kImplicit, int32_minus_one,
                      uint32_type, &result);
  EXPECT_FALSE(err.has_error());
  ...
  err = CastExprValue(eval_context.get(), CastType::kImplicit, int32_zero,
                      bool_type, &result);
  EXPECT_FALSE(err.has_error());
  ...
  err = CastExprValue(eval_context.get(), CastType::kImplicit, float_minus_zero,
                      bool_type, &result);
  EXPECT_FALSE(err.has_error());
  EXPECT_EQ(0, result.GetAs<int8_t>());
  ...
  err = CastExprValue(eval_context.get(), CastType::kImplicit, float_one_third,
                      bool_type, &result);
  EXPECT_FALSE(err.has_error());
  EXPECT_EQ(1, result.GetAs<int8_t>());
  ...
  err = CastExprValue(eval_context.get(), CastType::kImplicit, float_minus_two,
                      int32_type, &result);
  EXPECT_FALSE(err.has_error());
  ...
  err = CastExprValue(eval_context.get(), CastType::kImplicit, double_minus_two,
                      float_type, &result);
  EXPECT_FALSE(err.has_error());
  ...
  err = CastExprValue(eval_context.get(), CastType::kImplicit, ptr_value,
                      uint32_type, &result);
  EXPECT_FALSE(err.has_error());
...
TEST(Cast, ImplicitDerived) {
  ...
  ExprValue result;
  Err err = CastExprValue(eval_context.get(), CastType::kImplicit,
                          d.derived_value, d.base1_type, &result);
  EXPECT_FALSE(err.has_error());
  ...
  // Same for base 2.
  err = CastExprValue(eval_context.get(), CastType::kImplicit, d.derived_value,
                      d.base2_type, &result);
  EXPECT_FALSE(err.has_error());
  ...
  result = ExprValue();
  err = CastExprValue(eval_context.get(), CastType::kImplicit, d.base2_value,
                      d.derived_type, &result);
  ...
  err = CastExprValue(eval_context.get(), CastType::kImplicit,
                      d.derived_ptr_value, d.base2_ptr_type, &result);
  EXPECT_FALSE(err.has_error()) << err.msg();
  ...
  err = CastExprValue(eval_context.get(), CastType::kImplicit,
                      d.base2_ptr_value, d.derived_ptr_type, &result);
  EXPECT_TRUE(err.has_error());
  ...
  err = CastExprValue(eval_context.get(), CastType::kImplicit,
                      d.derived_ptr_value, void_ptr_type, &result);
  EXPECT_FALSE(err.has_error()) << err.msg();
  ...
  err = CastExprValue(eval_context.get(), CastType::kImplicit, void_ptr_value,
...
TEST(Cast, Reinterpret) {
  ...
  ExprValue result;
  Err err = CastExprValue(eval_context.get(), CastType::kReinterpret,
                          ptr_to_void, ptr_to_int32_type, &result);
  EXPECT_FALSE(err.has_error());
  ...
  err = CastExprValue(eval_context.get(), CastType::kReinterpret,
                      int32_minus_one, ptr_to_void_type, &result);
  EXPECT_FALSE(err.has_error());
  EXPECT_EQ(0xffffffffu, result.GetAs<uint64_t>());

  // Truncation of a number. This is also disallowed in C++.
  err = CastExprValue(eval_context.get(), CastType::kReinterpret, int64_big_num,
                      int32_type, &result);
  EXPECT_FALSE(err.has_error());
  ...
  err = CastExprValue(eval_context.get(), CastType::kReinterpret, double_pi,
                      ptr_to_void_type, &result);
  EXPECT_TRUE(err.has_error());
...
TEST(Cast, Static) {
  ...
  ExprValue result;
  Err err = CastExprValue(eval_context.get(), CastType::kStatic, d.base2_value,
                          d.derived_type, &result);
  EXPECT_TRUE(err.has_error());

  // Cast a derived class reference to a base class reference.
  // This is "static_cast<Base&>(derived_reference);
  err = CastExprValue(eval_context.get(), CastType::kStatic,
                      d.derived_ref_value, d.base2_ref_type, &result);
  EXPECT_FALSE(err.has_error()) << err.msg();
  EXPECT_EQ(d.base2_ref_value, result);

  // Should be able to go from base->derived with pointers.
  // This is "static_cast<Derived*>(&base);"
  err = CastExprValue(eval_context.get(), CastType::kStatic, d.base2_ptr_value,
                      d.derived_ptr_type, &result);
  EXPECT_FALSE(err.has_error()) << err.msg();
  EXPECT_EQ(d.derived_ptr_value, result);

  // Base->derived conversion for references.
  err = CastExprValue(eval_context.get(), CastType::kStatic, d.base2_ref_value,
  ...
  err = CastExprValue(eval_context.get(), CastType::kStatic,
                      derived_rvalue_value, d.base2_ref_type, &result);
  EXPECT_FALSE(err.has_error());
  EXPECT_EQ(d.base2_ref_value, result);

  // Don't allow reference->pointer or pointer->reference casts.
  err = CastExprValue(eval_context.get(), CastType::kStatic,
                      d.derived_ref_value, d.derived_ptr_type, &result);
  EXPECT_TRUE(err.has_error());
  err = CastExprValue(eval_context.get(), CastType::kStatic,
                      d.derived_ptr_value, d.derived_ref_type, &result);
  EXPECT_TRUE(err.has_error());
}
...
TEST(Cast, C) {
  ...
  // related types.
  ExprValue result;
  Err err = CastExprValue(eval_context.get(), CastType::kC, d.derived_ref_value,
                          d.base2_ref_type, &result);
  EXPECT_FALSE(err.has_error()) << err.msg();
  EXPECT_EQ(d.base2_ref_value, result);
  ...
  ExprValue ptr_value(ptr_to_double_type,
                      {0xf0, 0xde, 0xbc, 0x9a, 0x78, 0x56, 0x34, 0x12});
  err = CastExprValue(eval_context.get(), CastType::kC, ptr_value,
