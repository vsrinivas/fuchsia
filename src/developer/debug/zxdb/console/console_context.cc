...
void ConsoleContext::OutputThreadContext(
    const Thread* thread, debug_ipc::ExceptionType type,
  ...
  if (!hit_breakpoints.empty()) {
  ...
  } else if (type == debug_ipc::ExceptionType::kGeneral) {
    ...
    out.Append(fxl::StringPrintf("on %s exception ", debug_ipc::ExceptionTypeToString(type)));
  }
...
void ConsoleContext::OnThreadStopped(Thread* thread, debug_ipc::ExceptionType type,
