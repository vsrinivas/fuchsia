...
TEST(CommandUtils, FormatLocation) {
  ...
  EXPECT_EQ("<invalid address>",
            FormatLocation(nullptr, Location(), no_addrs_no_params).AsString());

  // Address-only location.
  EXPECT_EQ("0x12345", FormatLocation(nullptr, Location(Location::State::kAddress, 0x12345),
                                      no_addrs_no_params)
                           .AsString());
  ...
  EXPECT_EQ("Func() + 0x34", FormatLocation(nullptr, loc, no_file_line).AsString());
  ...
  EXPECT_EQ("Func() + 0x34", FormatLocation(nullptr, func_loc, no_file_line).AsString());
  ...
  EXPECT_EQ("0x1234, Func() • /path/foo.cc:21",
            FormatLocation(nullptr, loc, always_addr).AsString());
  EXPECT_EQ("Func() • /path/foo.cc:21",
            FormatLocation(nullptr, loc, no_addrs_no_params).AsString());
}
