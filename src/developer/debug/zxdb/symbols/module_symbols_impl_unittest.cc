...
TEST(ModuleSymbols, NonExistantFile) {
  ModuleSymbolsImpl module(TestSymbolModule::GetCheckedInTestFileName() + "_NONEXISTANT", "", "");
  Err err = module.Load();
...
TEST(ModuleSymbols, Basic) {
  ...
  auto locations = module.ResolveInputLocation(symbol_context, InputLocation(addrs[0].address()));
...
TEST(ModuleSymbols, LineDetailsForAddress) {
  ...
  addrs = module.ResolveInputLocation(symbol_context,
                                      InputLocation(FileLine(file_name, kLineToQuery)), options);
  ...
  // for it.
  LineDetails line_details = module.LineDetailsForAddress(symbol_context, addrs[0].address());
  ...
  LineDetails prev_details = module.LineDetailsForAddress(symbol_context, begin_range - 1);
...
TEST(ModuleSymbols, ResolveLineInputLocation) {
  ...
  std::vector<Location> addrs;
  addrs =
      module.ResolveInputLocation(symbol_context, InputLocation(FileLine(file_name, 27)), options);
  ASSERT_LE(1u, addrs.size());
  auto locations = module.ResolveInputLocation(symbol_context, InputLocation(addrs[0].address()));
  ASSERT_EQ(1u, locations.size());
  ...
  addrs =
      module.ResolveInputLocation(symbol_context, InputLocation(FileLine(file_name, 26)), options);
  ASSERT_LE(1u, addrs.size());
  locations = module.ResolveInputLocation(symbol_context, InputLocation(addrs[0].address()));
  ASSERT_EQ(1u, locations.size());
  EXPECT_EQ(27, locations[0].file_line().line());
  EXPECT_EQ(file_name, locations[0].file_line().file());
  ...
  addrs =
      module.ResolveInputLocation(symbol_context, InputLocation(FileLine(file_name, 15)), options);
  ASSERT_EQ(2u, addrs.size());
  locations = module.ResolveInputLocation(symbol_context, InputLocation(addrs[0].address()));
  ASSERT_EQ(1u, locations.size());
  EXPECT_EQ(15, locations[0].file_line().line());
  EXPECT_EQ(file_name, locations[0].file_line().file());
  locations = module.ResolveInputLocation(symbol_context, InputLocation(addrs[1].address()));
  ASSERT_EQ(1u, locations.size());
  EXPECT_EQ(15, locations[0].file_line().line());
  EXPECT_EQ(file_name, locations[0].file_line().file());
  ...
  addrs =
      module.ResolveInputLocation(symbol_context, InputLocation(FileLine(file_name, 17)), options);
...
TEST(ModuleSymbols, ResolveMainFunction) {
  ...
  // ("MyFunction").
  auto my_function_matches =
      module.index_.FindExact(Identifier(IdentifierComponent(TestSymbolModule::kMyFunctionName)));
