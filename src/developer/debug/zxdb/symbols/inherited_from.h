...
class InheritedFrom final : public Symbol {
 public:
  ...
  // This is the DW_AT_data_member_location attribute. In the common case it is a constant that's
  // the offset from the beginning of the derived class to the beginning of the base class.
  //
  // DWARF also allows these to be a location description where the location of the derived class is
  // pushed on the stack, the expression is evaluated, and the result is the location. I have not
  // seen our toolchain generate this type of location so it's not been implemented. If we have a
  // test case, this more complex mode should be supported and tested.
