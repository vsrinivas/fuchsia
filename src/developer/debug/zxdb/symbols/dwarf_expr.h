...
class DwarfExpr {
 public:
  ...
  // The source can be an empty LazySymbol if there is no corresponding source symbol for this
  // expression. This should only be the case for tests. This will mean calls to GetAddrBase() will
  // fail.
  explicit DwarfExpr(std::vector<uint8_t> data, LazySymbol source = LazySymbol())
      : data_(std::move(data)), source_(std::move(source)) {}

  const std::vector<uint8_t>& data() const { return data_; }
  const LazySymbol& source() const { return source_; }
