...
// Makes a frame with the given name for matching against. All frames will have the same address
// which will confuse some other code, but the pretty matchers don't need it.
std::unique_ptr<MockFrame> MakeFrame(std::string func_name, FileLine file_line) {
  ...
  Location location(0x100000, std::move(file_line), 0, SymbolContext::ForRelativeAddresses(),
                    function);

  return std::make_unique<MockFrame>(nullptr, nullptr, location, 0x1000, 0x2000);
}

std::vector<std::unique_ptr<Frame>> GetTestFrames() {
  std::vector<std::unique_ptr<Frame>> frames;
  frames.push_back(MakeFrame("Function1", FileLine("file1.cc", 23)));
  frames.push_back(MakeFrame("Function2", FileLine("file2.cc", 23)));
  frames.push_back(MakeFrame("Function3", FileLine("file3.cc", 23)));
  frames.push_back(MakeFrame("Function4", FileLine("file4.cc", 23)));
  return frames;
}
