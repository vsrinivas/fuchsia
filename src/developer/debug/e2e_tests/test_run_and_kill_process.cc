...
class RunAndKillProcess : public E2eTest {
 public:
  ...
  void Run() {
    console().ProcessInputLine("attach crasher", [this](const Err& e) {
    ...
    console().ProcessInputLine("break blind_write", [this](const Err& e) {
      EXPECT_TRUE(e.ok()) << e.msg();
    ...
    console().ProcessInputLine("run /boot/bin/crasher", [this](const Err& e) {
  ...
  void OnThreadStopped(Thread* thread, const StopInfo& info) override {
    ...
    auto frame_cb = [](const Err& e) { EXPECT_TRUE(e.ok()) << e.msg(); };

    console().ProcessInputLine("frame", frame_cb);

    auto kill_cb = [](const Err& e) { EXPECT_TRUE(e.ok()) << e.msg(); };

    console().ProcessInputLine("kill", kill_cb);
  }
