...
uint64_t AutomationInstructionExecutor::EvalOperand(const debug_ipc::AutomationOperand& operand,
  ...
  switch (operand.kind()) {
    ...
    case debug_ipc::AutomationOperandKind::kRegister:
      return regs.GetRegister(operand.register_index()).value();
    ...
    case debug_ipc::AutomationOperandKind::kRegisterTimesConstant:
      return regs.GetRegister(operand.register_index()).value() * operand.value();
    case debug_ipc::AutomationOperandKind::kIndirectUInt32: {
      uint32_t result = 0;
      size_t result_size;
      handle.ReadMemory(regs.GetRegister(operand.register_index()).value() + operand.offset(),
