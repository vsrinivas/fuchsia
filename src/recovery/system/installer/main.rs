...
enum InstallerMessages {
    ...
    GotBoorloaderType(BootloaderType),
        ...
        match message {
            ...
            InstallerMessages::MenuEnter => {
                ...
                match self.menu_state_machine.get_selected_button_type() {
                    ...
                    MenuButtonType::Disk => {
                        // Disk was selected as intallation target
            ...
            InstallerMessages::GotBoorloaderType(bootloader_type) => {
...
async fn get_installation_paths(app_sender: AppSender, view_key: ViewKey) -> Result<(), Error> {
    ...
    app_sender.clone().queue_message(
        ...
        make_message(InstallerMessages::GotBoorloaderType(bootloader_type)),
    );
...
async fn fuchsia_install(
    ...
    // Execute install
    match do_install(app_sender, view_key, installation_paths).await {
        Ok(_) => {
            println!("IIIIIIIIIIIIIIIII SUCCESS");
        }
        Err(e) => {
            print!("IIIIIIIIIIIIIIIIIII ERROR: {}", e)
        }
    }
}
...
async fn do_install(
    ...
    for part in to_install {
        ...
        app_sender.clone().queue_message(
            MessageTarget::View(view_key),
            make_message(InstallerMessages::ProgressUpdate(String::from(format!("paving: {:?}", part),
            ))),
        );
    ...
    app_sender.clone().queue_message(
        MessageTarget::View(view_key),
        make_message(InstallerMessages::ProgressUpdate(String::from("Setting active configuration for the new system"))),
    ); // debug, gets here
