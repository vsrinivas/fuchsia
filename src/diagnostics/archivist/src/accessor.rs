...
impl ArchiveAccessor {
    ...
    async fn make_accessor_server(
    ...
    ) -> Result<AccessorServer, ServerError> {
        ...
        match params.data_type.ok_or(ServerError::MissingDataType)? {
            DataType::Inspect => {
                ...
                AccessorServer::new(server.stream(), requests, stats)
            }
            DataType::Lifecycle => {
                ...
                AccessorServer::new(events, requests, stats)
            }
            DataType::Logs => {
    ...
    fn run_accessor_server(
        &self,
        requests: BatchIteratorRequestStream,
        control: BatchIteratorControlHandle,
        params: StreamParameters,
    ) -> Task<()> {
        let repo = self.diagnostics_repo.clone();
        let accessor_stats = self.archive_accessor_stats.clone();
        Task::spawn(async move {
            match Self::make_accessor_server(repo, requests, params, accessor_stats).await {
                Ok(server) => {
                    if let Err(e) = server.run().await {
                        e.close(control);
                    }
                }
                Err(e) => e.close(control),
            }
        })
    }
    ...
    pub fn spawn_archive_accessor_server(self, mut stream: ArchiveAccessorRequestStream) {
        ...
        fasync::Task::spawn(async move {
            ...
            })) = stream.try_next().await
            {
                ...
                self.run_accessor_server(requests, control, stream_parameters).detach()
            }
