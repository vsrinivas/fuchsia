...
pub struct ReaderServer {
    repo_data: std::vec::IntoIter<UnpopulatedInspectDataContainer>,
...
impl ReaderServer {
    ...
    /// Convert the server into a stream of inspect data.
    pub fn stream(self) -> impl Stream<Item = Data<Inspect>> + Send + 'static {
        // we make a stream of populated & filtered Vec's here
        futures::stream::unfold(self, |mut server| async move {
            if let Some(unpopulated) = server.repo_data.next() {
                let filtered = server.populate_and_filter(unpopulated).await;
                Some((filtered, server))
            } else {
                None
            }
        })
    ...
    /// Accepts an `unpopulated` inspect data container, populates it from inspect vmos, and
    ...
    async fn populate_and_filter(
        ...
        let populated =
            unpopulated.populate(self.timeout, || self.stats.global_stats().add_timeout()).await;
        self.filter_snapshot(populated)
    }
...
mod tests {
    use {
        ...
        fuchsia_async::{self as fasync, DurationExt, Task},
    ...
    async fn read_snapshot(
        reader_server: impl Stream<Item = Data<Inspect>> + Send + 'static,
        _test_inspector: Arc<Inspector>,
        stats: Arc<DiagnosticsServerStats>,
    ) -> serde_json::Value {
        let reader_server = Box::pin(reader_server);
        let (consumer, batch_iterator_requests) =
            create_proxy_and_stream::<BatchIteratorMarker>().unwrap();
        let _server = Task::spawn(async {
