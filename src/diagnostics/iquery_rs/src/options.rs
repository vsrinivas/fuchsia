...
mod tests {
    ...
    fn parse_opts() {
        let opts =
            "iquery_rs --ls --recursive --sort --format=json --full_paths --dir=DIR PATH1 PATH2"
                .split(" ")
                .map(|s| s.to_string());
    ...
    #[test]
    fn default_opts() {
        let opts = "iquery_rs PATH".split(" ").map(|s| s.to_string());
        let options = Options::read(Box::new(opts.into_iter())).expect("failed to read options");
    ...
    #[test]
    fn parse_report() {
        let opts = "iquery_rs --report PATH".split(" ").map(|s| s.to_string());
        let options = Options::read(Box::new(opts.into_iter())).expect("failed to read options");
        assert_eq!(options.mode, ModeCommand::Cat);
        assert_eq!(options.global.dir, None);
        assert_eq!(options.formatting.format, Format::Text);
