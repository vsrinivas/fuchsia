...
const SERVICE_URL: &str =
    "fuchsia-pkg://fuchsia.com/iquery_rs_golden_tests#meta/iquery_rs_example_component.cmx";
...
impl Golden {
    fn load(filename: &str) -> Result<Self, Error> {
        ...
        if let Some(command_line) = lines.next() {
            if command_line.starts_with("iquery_rs") {
...
impl GoldenTest {
    ...
    fn validate(&self, hub_path: &str) -> Result<(), Error> {
        ...
        for (golden_line, output_line) in self.golden.lines.iter().zip(reader.lines()) {
            ...
            let output = output.replace("iquery_rs", "iquery");
...
fn get_hub_path() -> Result<String, Error> {
    let glob_query = "/hub/r/test/*/c/iquery_rs_example_component.cmx/*/out";
