...
impl FramebufferServer {
    ...
    pub fn new() -> Result<Self, Errno> {
        ...
        let collection = init_scene(&flatland, &allocator);
...
fn init_scene(
    ...
    std::thread::spawn(move || {
        ...
        buffer_allocator
            .set_name(100, "Starnix ViewProvider")
            .expect("Failed to set buffer allocator name");

        let sysmem_buffer_collection_token = executor
            .run_singlethreaded(buffer_allocator.duplicate_token())
            .expect("Error duplicating token");
    ...
    let sysmem_buffer_collection_token = collection_receiver.recv().expect("");
    ...
    let allocation =
        allocation_receiver.recv().expect("Failed to create bufffer collection").expect("");
