...
// - - - - - - - - - -
//
VK_TEST_P(DisplayCompositorParameterizedPixelTest, MultipleParentPixelTest) {
  ...
  ASSERT_TRUE(capture_collection_result.is_ok());
  auto capture_collection = std::move(capture_collection_result.value());
  ...
  auto texture_collection = SetupClientTextures(display_compositor.get(), kTextureCollectionId,
                                                fuchsia::sysmem::PixelFormatType::BGRA32, 60, 40,
                                                /*num_vmos*/ 1, &texture_collection_info);

  // Write to the texture. Make it half white and half blue. BGRA format
  const uint8_t kBlueBgraValues[] = {255U, 0U, 0U, 255U};
  const uint8_t kWhiteBgraValues[] = {255U, 255U, 255U, 255U};

  MapHostPointer(texture_collection_info, /*vmo_index*/ 0,
                 [&kBlueBgraValues, &kWhiteBgraValues](uint8_t* vmo_host, uint32_t num_bytes) {
  ...
  MapHostPointer(render_target_info, /*vmo_index*/ 0, [&](uint8_t* vmo_host, uint32_t num_bytes) {
    ...
    // There should be a total of 15 white pixels (3 for the normal square and 12 for the
    // magnified square) and 5 blue pixels (1 for the normal square and 4 for the magnified square).
