...
TEST(TransformGraphTest, GlobalTopologyUnknownGraph) {
  TransformHandle unknown_handle = {1, 1};

  auto output =
      TransformGraph::ComputeGlobalTopologyVector({}, {}, kLinkInstanceId, unknown_handle);
  EXPECT_TRUE(output.topology_vector.empty());
  EXPECT_TRUE(output.live_handles.empty());
}

TEST(TransformGraphTest, GlobalTopologyLinkExpansion) {
