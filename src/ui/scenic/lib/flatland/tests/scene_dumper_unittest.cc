...
size_t ExpectNodeLineNumberAndGetIndex(flatland::TransformHandle& node, int node_line_number,
...
size_t ExpectNameLineNumberAndGetIndex(const std::string& name, int name_line_number,
                                       const std::vector<std::string>& line_dump) {
...
void ExpectTopologyNodeHasLessDepthLevel(flatland::TransformHandle node_a, int node_a_line_number,
                                         flatland::TransformHandle node_b, int node_b_line_number,
                                         const std::vector<std::string>& line_dump) {
...
void ExpectTopologyNodeHasSameDepthLevel(flatland::TransformHandle node_a, int node_a_line_number,
                                         flatland::TransformHandle node_b, int node_b_line_number,
                                         const std::vector<std::string>& line_dump) {
  const auto node_a_index = ExpectNodeLineNumberAndGetIndex(node_a, node_a_line_number, line_dump);
  const auto node_b_index = ExpectNodeLineNumberAndGetIndex(node_b, node_b_line_number, line_dump);
...
void ExpectNodeName(flatland::TransformHandle node, int node_line_number, const std::string& name,
                    const std::vector<std::string>& line_dump) {
  const auto node_index = ExpectNodeLineNumberAndGetIndex(node, node_line_number, line_dump);
  // Name appears one line above node.
  const auto name_index = ExpectNameLineNumberAndGetIndex(name, node_line_number - 1, line_dump);
  EXPECT_EQ(node_index, node_index);
}
...
int FindImageDumpLineNumber(const std::vector<std::string>& line_dump, size_t beginning_at = 0) {
  for (size_t i = beginning_at; i < line_dump.size(); i++) {
    if (line_dump[i].find(kImageDumpLineIdentifierToken) != std::string::npos) {
      return i;
    }
  }
  FX_NOTREACHED() << "Image not found.";
  return -1;
}
...
int ExpectImageDump(ImageMetadata image, flatland::TransformHandle node, Rectangle2D rect,
...
TEST(SceneDumperTest, TopologyTreeWithNames) {
  ...
  ExpectTopologyNodeHasLessDepthLevel({1, 0}, 0, {2, 0}, 2, lines);
  // {2, 0} has two children - {3, 0} and {4, 0}.
  ExpectTopologyNodeHasLessDepthLevel({2, 0}, 2, {3, 0}, 4, lines);
  ExpectTopologyNodeHasSameDepthLevel({3, 0}, 4, {4, 0}, 5, lines);
  // {5, 0} is direct child of {1, 0} and sibling of {2, 0}.
  ExpectTopologyNodeHasLessDepthLevel({1, 0}, 0, {5, 0}, 7, lines);
  ExpectTopologyNodeHasSameDepthLevel({2, 0}, 2, {5, 0}, 7, lines);

  ExpectNodeName({2, 0}, 2, names[1], lines);
  ExpectNodeName({3, 0}, 4, names[2], lines);
  ExpectNodeName({5, 0}, 7, names[4], lines);
}
...
TEST(SceneDumperTest, ImageRectangleMetadata) {
  ...
  const auto next_image_dump_line_number = ExpectImageDump(image, node, image_rectangles[0], lines);
  // Second image dump.
  ...
  ExpectImageDump(second_image, second_node, image_rectangles[1], lines,
