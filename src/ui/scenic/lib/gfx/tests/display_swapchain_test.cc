...
class DisplaySwapchainTest : public Fixture {
 public:
  ...
  void OnVsync(DisplaySwapchain* swapchain, zx::time timestamp,
    ...
    swapchain->OnVsync(timestamp, image_ids);
  }
...
VK_TEST_F(DisplaySwapchainTest, RenderStress) {
  ...
  auto layer = fxl::MakeRefCounted<Layer>(session(), 0);
  ...
  RunLoopUntil([&budget] { return budget == kMaxOutstanding - 1; });
  RunLoopUntilIdle();
}
...
VK_TEST_F(DisplaySwapchainTest, MultipleRendersBeforeVsync_PresentInOrder) {
  ...
  display_manager()->EnableVsync([](zx::time, const std::vector<uint64_t>&) {});
