// Copyright 2017 The Fuchsia Authors. All rights reserved.
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.

#ifndef SRC_UI_SCENIC_LIB_GFX_RESOURCES_LIGHTS_DIRECTIONAL_LIGHT_H_
#define SRC_UI_SCENIC_LIB_GFX_RESOURCES_LIGHTS_DIRECTIONAL_LIGHT_H_

#include "src/ui/scenic/lib/gfx/resources/lights/light.h"

namespace scenic_impl {
namespace gfx {

class DirectionalLight final : public Light {
 public:
  static const ResourceTypeInfo kTypeInfo;

  DirectionalLight(Session* session, SessionId session_id, ResourceId id);

  // The direction will be normalized before storing.  Returns false if the
  // length of |direction| is nearly zero.
  bool SetDirection(const glm::vec3& direction, ErrorReporter* reporter);

  // The normalized direction of the light source.
  const glm::vec3& direction() const { return direction_; }

  // |Resource|.
  void Accept(class ResourceVisitor* visitor) override;

 private:
  glm::vec3 direction_ = {0.f, 0.f, -1.f};
};

}  // namespace gfx
}  // namespace scenic_impl

#endif  // SRC_UI_SCENIC_LIB_GFX_RESOURCES_LIGHTS_DIRECTIONAL_LIGHT_H_
