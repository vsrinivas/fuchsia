...
TypedVariableBinding<VT, T>::TypedVariableBinding(
    ...
    : variable_(variable), on_value_changed_callback_(std::move(on_value_changed_callback)) {
...
template <::fuchsia::ui::gfx::Value::Tag VT, typename T>
void TypedVariableBinding<VT, T>::OnVariableValueChanged(TypedVariable<VT, T>* v) {
  on_value_changed_callback_(v->value());
}

template class TypedVariableBinding<::fuchsia::ui::gfx::Value::Tag::kVector3, escher::vec3>;
template class TypedVariableBinding<::fuchsia::ui::gfx::Value::Tag::kQuaternion, escher::quat>;
