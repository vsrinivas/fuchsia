...
bool ScreenshotBufferCollectionImporter::ImportBufferCollection(
  ...
  // Create a VkRenderer token.
  fuchsia::sysmem::BufferCollectionTokenSyncPtr local_token = token.BindSync();
  fuchsia::sysmem::BufferCollectionTokenSyncPtr vk_renderer_token;
  zx_status_t status =
      local_token->Duplicate(std::numeric_limits<uint32_t>::max(), vk_renderer_token.NewRequest());
  if (status != ZX_OK) {
