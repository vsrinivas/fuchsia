...
class SpeakerTest : public gtest::TestLoopFixture {
 public:
  SpeakerTest()
      : context_provider_(),
    ...
    speaker_ = std::make_unique<a11y::Speaker>(&executor_, &tts_engine_ptr_,
...
TEST_F(SpeakerTest, SpeaksANodeWithTimeSpacedUtterances) {
  const zx::duration kDelay = zx::msec(300);
  ...
  auto task = speaker_->SpeakNodePromise(&node, {.interrupt = true});
  executor_.schedule_task(std::move(task));
  RunLoopFor(kDelay - zx::msec(100));
  ASSERT_LT(Now(), start + kDelay);
