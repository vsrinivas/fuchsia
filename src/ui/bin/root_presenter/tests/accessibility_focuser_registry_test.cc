  ...
  void SetUp() override {
    ...
    // services added here.
    {
      ...
      zx_status_t status = services->AddServiceWithLaunchInfo(
          std::move(launch_info), fuchsia::ui::views::accessibility::FocuserRegistry::Name_);
    ...
    }
    {
      fuchsia::sys::LaunchInfo launch_info{
          "fuchsia-pkg://fuchsia.com/root_presenter#meta/root_presenter.cmx"};
      zx_status_t status = services->AddServiceWithLaunchInfo(
          std::move(launch_info), fuchsia::ui::policy::Presenter::Name_);
      EXPECT_EQ(ZX_OK, status);
    }
...
TEST_F(AccessibilityFocuserRegistryTest, AccessibilityFocusRequestFailsWhenScenicIsNotInitialized) {
  ...
  zx::eventpair a;
  FX_CHECK(zx::eventpair::create(0u, &dummy_view_ref.reference, &a) == ZX_OK);
  view_focuser->RequestFocus(std::move(dummy_view_ref), std::move(callback));
