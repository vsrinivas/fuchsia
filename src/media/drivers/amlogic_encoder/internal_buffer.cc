...
fit::result<InternalBuffer, zx_status_t> InternalBuffer::Create(
    ...
    size_t size, bool is_writable, bool is_mapping_needed) {
  return CreateAligned(name, sysmem, bti, size, 0, is_writable, is_mapping_needed);
}

fit::result<InternalBuffer, zx_status_t> InternalBuffer::CreateAligned(
    const char* name, fuchsia::sysmem::AllocatorSyncPtr* sysmem, const zx::unowned_bti& bti,
    size_t size, size_t alignment, bool is_writable, bool is_mapping_needed) {
  ZX_DEBUG_ASSERT(sysmem);
  ZX_DEBUG_ASSERT(*sysmem);
  ZX_DEBUG_ASSERT(*bti);
  ZX_DEBUG_ASSERT(size);
  ZX_DEBUG_ASSERT(size % ZX_PAGE_SIZE == 0);
  InternalBuffer local_result(size, is_writable, is_mapping_needed);
...
zx_status_t InternalBuffer::vmo_duplicate(zx::vmo* dup) {
  return vmo_.duplicate(ZX_RIGHT_SAME_RIGHTS, dup);
}
...
InternalBuffer::InternalBuffer(size_t size, bool is_writable, bool is_mapping_needed)
    : size_(size),
...
zx_status_t InternalBuffer::Init(const char* name, fuchsia::sysmem::AllocatorSyncPtr* sysmem,
  ...
  // amlogic-encoder always requires contiguous; only contiguous is supported by InternalBuffer.
