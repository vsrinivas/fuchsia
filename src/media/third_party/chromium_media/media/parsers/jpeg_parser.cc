...
auto RoundUp(const T& val, const U& multiple)
    -> decltype(fbl::round_up<T, U>(val, multiple)) {
  return fbl::round_up<T, U>(val, multiple);
}

#if 0
// Round up |value| to multiple of |mul|. |value| must be non-negative. |mul|
// must be positive.
...
static bool ParseSOF(const uint8_t* buffer,
  ...
  if (!InRange(frame_header->num_components, 1,
               base::checked_cast<int>(std::size(frame_header->components)))) {
  ...
  // Fuchsia change: Fix implicit conversions
  frame_header->coded_width =
      RoundUp(frame_header->visible_width,
              base::checked_cast<uint16_t>(max_h_factor * 8));
  frame_header->coded_height =
      RoundUp(frame_header->visible_height,
              base::checked_cast<uint16_t>(max_v_factor * 8));
