...
std::unique_ptr<AudioConsumerImpl> AudioConsumerImpl::Create(
  ...
  return std::make_unique<AudioConsumerImpl>(std::move(request), component_context,
                                             std::move(quit_callback));
}

AudioConsumerImpl::AudioConsumerImpl(fidl::InterfaceRequest<fuchsia::media::AudioConsumer> request,
...
void AudioConsumerImpl::CreateStreamSink(
  ...
  // Setup timeline rate in frame units, as that is what later pipeline stages expect.
  pending_simple_stream_sink_ = SimpleStreamSinkImpl::Create(
      stream_type, media::TimelineRate(audio_stream_type.frames_per_second, 1),
...
void AudioConsumerImpl::MaybeSetNewSource() {
  ...
  core_.SetSourceSegment(
      ...
      [simple_stream_sink = std::move(simple_stream_sink), buffers = std::move(buffers)]() mutable {
        int buffer_index = 0;
        while (!buffers.empty()) {
          simple_stream_sink->AddPayloadBuffer(buffer_index++, std::move(*(buffers.end() - 1)));
          buffers.pop_back();
        }
      });
}
...
void AudioConsumerImpl::AddBindingInternal(
    fidl::InterfaceRequest<fuchsia::media::AudioConsumer> request) {
  FX_DCHECK(request);
