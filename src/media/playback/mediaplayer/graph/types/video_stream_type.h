...
#ifndef SRC_MEDIA_PLAYBACK_MEDIAPLAYER_GRAPH_TYPES_VIDEO_STREAM_TYPE_H_
#define SRC_MEDIA_PLAYBACK_MEDIAPLAYER_GRAPH_TYPES_VIDEO_STREAM_TYPE_H_
...
class VideoStreamType : public StreamType {
 public:
  ...
  uint32_t pixel_aspect_ratio_width() const { return pixel_aspect_ratio_width_; }

  uint32_t pixel_aspect_ratio_height() const { return pixel_aspect_ratio_height_; }
...
class VideoStreamTypeSet : public StreamTypeSet {
 public:
  static std::unique_ptr<StreamTypeSet> Create(const std::vector<std::string>& encodings,
    ...
    return std::unique_ptr<StreamTypeSet>(new VideoStreamTypeSet(encodings, width, height));
  }

  VideoStreamTypeSet(const std::vector<std::string>& encodings, Range<uint32_t> width,
...
#endif  // SRC_MEDIA_PLAYBACK_MEDIAPLAYER_GRAPH_TYPES_VIDEO_STREAM_TYPE_H_
