...
BufferSet::BufferSet(const fuchsia::media::StreamBufferSettings& settings,
    ...
    : settings_(fidl::Clone(settings)),
      ...
      buffers_(settings.packet_count_for_server() + settings.packet_count_for_client()) {
...
fuchsia::media::StreamBuffer BufferSet::GetBufferDescriptor(uint32_t buffer_index, bool writeable,
  ...
  buffer_data_vmo.set_vmo_handle(payload_vmo->Duplicate(ZX_RIGHT_READ | ZX_RIGHT_MAP |
...
void BufferSet::CreateBufferForDecoder(uint32_t buffer_index, const PayloadVmos& payload_vmos) {
  std::lock_guard<std::mutex> locker(mutex_);
  FXL_DCHECK(buffer_index < buffers_.size());
  ...
  buffers_[buffer_index].decoder_ref_ = CreateBuffer(buffer_index, payload_vmos.GetVmos());
}
...
fbl::RefPtr<PayloadVmo> BufferSet::BufferVmo(size_t buffer_index,
                                             const PayloadVmos& payload_vmos) const {
  FXL_DCHECK(buffer_index < buffers_.size());
...
fbl::RefPtr<PayloadBuffer> BufferSet::CreateBuffer(
  ...
  uint64_t offset_in_vmo = single_vmo_ ? buffer_index * settings_.per_packet_buffer_bytes() : 0;
  ...
  fbl::RefPtr<PayloadBuffer> payload_buffer = PayloadBuffer::Create(
      settings_.per_packet_buffer_bytes(), payload_vmo->at_offset(offset_in_vmo), payload_vmo,
      offset_in_vmo,
