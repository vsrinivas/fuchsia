...
TEST(MixerStageTest, Advance) {
  ...
  auto clock = clock_realm->CreateClock("clock", Clock::kMonotonicDomain, false);
  ...
  MixerStage mixer_stage("mixer", kDefaultFormat, clock->koid(), 10);
  ...
  mixer_stage.Advance(ctx, Fixed(0));
  ...
  mixer_stage.Advance(ctx, Fixed(10));
  EXPECT_FALSE(fence1.Done());
  EXPECT_FALSE(fence2.Done());
  ...
  mixer_stage.Advance(ctx, Fixed(30));
  ...
  mixer_stage.Advance(ctx, Fixed(40));
  EXPECT_TRUE(fence1.Done());
  EXPECT_FALSE(fence2.Done());
  EXPECT_FALSE(fence3.Done());
  EXPECT_FALSE(source_1->empty());
  ...
  mixer_stage.Advance(ctx, Fixed(100));
  EXPECT_TRUE(fence1.Done());
...
TEST(MixerStageTest, Read) {
  ...
  MixerStage mixer_stage("mixer", kDefaultFormat, clock->koid(), 5);
  mixer_stage.UpdatePresentationTimeToFracFrame(kDefaultPresentationTimeToFracFrame);
  ...
  gain_controls.Add(GainControlId{10}, GainControl(clock->koid()));
  gain_controls.Get(GainControlId{10}).SetGain(ScaleToDb(2.0f));
  gain_controls.Add(GainControlId{20}, GainControl(clock->koid()));
  ...
  for (int64_t i = 1; i <= source_count; ++i) {
    ...
    gain_controls.Add(gain_id, GainControl(clock->koid()));
    ...
    const auto source =
        std::make_shared<SimplePacketQueueProducerStage>(SimplePacketQueueProducerStage::Args{
            "source-" + std::to_string(i), kDefaultFormat, clock->koid()});
    source->UpdatePresentationTimeToFracFrame(kDefaultPresentationTimeToFracFrame);
    mixer_stage.AddSource(source,
                          {.clock_sync = ClockSynchronizer::SelectModeAndCreate(clock, clock),
                           .gain_ids = {gain_id},
                           .sampler = Sampler::Create(kDefaultFormat, kDefaultFormat)});
...

  {
    ...
    const auto packet = mixer_stage.Read(ctx, Fixed(10), 5);
...

  {
    ...
    const auto packet = mixer_stage.Read(ctx, Fixed(15), 5);
    ASSERT_TRUE(packet.has_value());
...

  {
    ...
    const auto packet = mixer_stage.Read(ctx, Fixed(20), 5);
    ASSERT_TRUE(packet.has_value());
...

  {
    ...
    const auto packet = mixer_stage.Read(ctx, Fixed(25), 5);
    ASSERT_TRUE(packet.has_value());
  ...
  EXPECT_FALSE(mixer_stage.Read(ctx, Fixed(30), 5));
}
...
TEST(MixerStageTest, ReadMoreThanMaxFrameCount) {
...

  {
    ...
    const auto packet = mixer_stage.Read(ctx, Fixed(20), 10);
    ASSERT_TRUE(packet.has_value());
    EXPECT_EQ(packet->start(), Fixed(20));
    EXPECT_EQ(packet->length(), 5);
    for (int64_t i = 0; i < packet->length(); ++i) {
...

  {
    ...
    const auto packet = mixer_stage.Read(ctx, Fixed(25), 10);
    ASSERT_TRUE(packet.has_value());
    EXPECT_EQ(packet->start(), Fixed(25));
    EXPECT_EQ(packet->length(), 5);
  ...
  EXPECT_FALSE(mixer_stage.Read(ctx, Fixed(30), 10));
}
...
TEST(MixerStageTest, ReadSilent) {
  ...
  auto& gain_controls = mixer_stage.gain_controls();
  gain_controls.Add(dest_gain_id, GainControl(clock->koid()));
  ...
  EXPECT_FALSE(mixer_stage.Read(ctx, Fixed(10), 5).has_value());
...
TEST(MixerStageTest, SetDestGains) {
  ...
  MixerStage mixer_stage("mixer", kDefaultFormat, clock->koid(), 1);
  mixer_stage.UpdatePresentationTimeToFracFrame(kDefaultPresentationTimeToFracFrame);
  auto& gain_controls = mixer_stage.gain_controls();
  ...
  gain_controls.Add(gain_id_1, GainControl(clock->koid()));
  ...
  float value_1 = 1.0f;
  {
    ...
    const auto packet = mixer_stage.Read(ctx, Fixed(10), 1);
    ASSERT_TRUE(packet.has_value());
    EXPECT_EQ(packet->start(), Fixed(10));
  ...
  gain_controls.Add(gain_id_2, GainControl(clock->koid()));
...

  {
    ...
    const auto packet = mixer_stage.Read(ctx, Fixed(11), 1);
    ASSERT_TRUE(packet.has_value());
  ...
  float value_2 = 2.0f;
  {
    ...
    const auto packet = mixer_stage.Read(ctx, Fixed(12), 1);
    ASSERT_TRUE(packet.has_value());
...

  {
    ...
    const auto packet = mixer_stage.Read(ctx, Fixed(13), 1);
    ASSERT_TRUE(packet.has_value());
...

  {
    ...
    const auto packet = mixer_stage.Read(ctx, Fixed(14), 1);
    ASSERT_TRUE(packet.has_value());
