  ...
  // Now apply the second command.
  {
    ...
    ExpectStartApplied(control, result2,
                       When{
                           ...
                           .reference_time = zx::time(0),
                           .stream_time = zx::msec(1),                      // 128 frames = 1ms
                           .packet_timestamp = zx::msec(1).to_nsecs() / 2,  // 2 ns/tick
                           .frame = Fixed(128),
                       });
  }
...

  {
    ...
    ExpectStartApplied(control, result3,
                       When{
                           ...
                           .stream_time = zx::usec(500),                      // 64 frames = 0.5ms
                           .packet_timestamp = zx::usec(500).to_nsecs() / 2,  // 2 ns/tick
                           .frame = Fixed(64),
                       });
  }
}
...
TEST_F(StartStopControlTest, MultipleStopCommands) {
  ...
  ASSERT_TRUE(result1.has_value());
  ASSERT_TRUE(result1->is_error());
  EXPECT_EQ(result1->error(), StopError::kCanceled);
  ...
  EXPECT_EQ(result3->error(), StopError::kAlreadyStopped);
}
