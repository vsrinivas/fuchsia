...
void TestAdvance(int64_t step_size = 1) {
  ...
  MixerSource mixer_source(
      source,
      {.clock_sync = ClockSynchronizer::SelectModeAndCreate(dest_clock, source_clock),
  ...
  mixer_source.Advance(ctx, kDefaultPresentationTimeToFracFrame, Fixed(0));
  ...
  mixer_source.Advance(ctx, kDefaultPresentationTimeToFracFrame, Fixed(10 * step_size));
  EXPECT_FALSE(fence1.Done());
  EXPECT_FALSE(fence2.Done());
  EXPECT_FALSE(source->empty());

  // Advance exactly when the first packet is fully consumed, which should close its packet fence.
  mixer_source.Advance(ctx, kDefaultPresentationTimeToFracFrame, Fixed(30 * step_size));
  EXPECT_TRUE(fence1.Done());
  EXPECT_FALSE(fence2.Done());
  EXPECT_FALSE(source->empty());

  // Advance before the second packet is fully consumed, which should not close its packet fence.
  mixer_source.Advance(ctx, kDefaultPresentationTimeToFracFrame, Fixed(40 * step_size));
  EXPECT_TRUE(fence1.Done());
  EXPECT_FALSE(fence2.Done());
  EXPECT_FALSE(source->empty());

  // Advance beyond when the second packet is fully consumed, which should close its packet fence.
  mixer_source.Advance(ctx, kDefaultPresentationTimeToFracFrame, Fixed(100 * step_size));
  EXPECT_TRUE(fence1.Done());
...
TEST(MixerSourceTest, PrepareSourceGainForNextMix) {
  ...
  auto clock_realm = SyntheticClockRealm::Create();
  auto clock = clock_realm->CreateClock("clock", Clock::kExternalDomain, true);

  ClockSnapshots clock_snapshots;
  clock_snapshots.AddClock(clock);
  clock_snapshots.Update(zx::time(0));
  mixer_gain_controls.Add(GainControlId{1}, GainControl(clock->koid()));
  mixer_gain_controls.Add(GainControlId{2}, GainControl(clock->koid()));
  mixer_gain_controls.Add(GainControlId{3}, GainControl(clock->koid()));
  mixer_gain_controls.Advance(clock_snapshots, zx::time(0));

  const auto source = std::make_shared<SimplePacketQueueProducerStage>(
      SimplePacketQueueProducerStage::Args{"source", kDefaultFormat, clock->koid()});
  ...
  mixer_gain_controls.Advance(clock_snapshots, zx::time(3));
...
TEST(MixerSourceTest, SetDestGains) {
  auto clock_realm = SyntheticClockRealm::Create();
  const auto clock_sync = ClockSynchronizer::SelectModeAndCreate(
      clock_realm->CreateClock("source_clock", Clock::kExternalDomain, true),
      clock_realm->CreateClock("dest_clock", Clock::kExternalDomain, true));
