...
class PacketQueueProducerStageTest : public ::testing::Test {
 public:
  ...
  const void* PushPacketToCommandQueue(uint32_t packet_id, int64_t start = 0, int64_t length = 1) {
    auto& packet = NewPacket(packet_id, start, length);
...
TEST_F(PacketQueueProducerStageTest, ReadWithCommandQueue) {
  ...
  const void* payload_0 = PushPacketToCommandQueue(0, 0, 20);
  const void* payload_1 = PushPacketToCommandQueue(1, 20, 20);
  PushPacketToCommandQueue(2, 40, 20);
  ...
  PushPacketToCommandQueue(3, 60, 20);
  TestFence clear_fence;
  command_queue().push(ClearCommand{.fence = clear_fence.Take()});
  const void* payload_4 = PushPacketToCommandQueue(4, 80, 20);
  const void* payload_5 = PushPacketToCommandQueue(5, 100, 20);
...
TEST_F(PacketQueueProducerStageTest, AdvanceWithCommandQueue) {
  ...
  // Push some packets onto the command queue.
  PushPacketToCommandQueue(0, 0, 20);
  PushPacketToCommandQueue(1, 20, 20);
  PushPacketToCommandQueue(2, 40, 20);
