...
TEST(MixerNodeTest, Create) {
  ...
  EXPECT_EQ(mixer_node->pipeline_stage()->thread(), detached_thread->pipeline_thread());
  EXPECT_EQ(mixer_node->thread(), detached_thread);
...
TEST(MixerNodeTest, CreateDeleteEdge) {
  ...
  EXPECT_EQ(mixer_node->reference_clock(), DefaultClock());
  EXPECT_EQ(mixer_node->pipeline_stage()->format(), kDestFormat);
  EXPECT_EQ(mixer_node->pipeline_stage()->thread(), ctx.detached_thread->pipeline_thread());
  EXPECT_EQ(mixer_node->thread(), ctx.detached_thread);
  EXPECT_THAT(mixer_node->sources(), ElementsAre());
  EXPECT_EQ(mixer_node->dest(), nullptr);

  // Connect graph node `1` to `mixer_node` with gain control `10`.
  ASSERT_TRUE(
      Node::CreateEdge(graph.ctx(), graph.node(1), mixer_node, {.gain_ids = {GainControlId{10}}})
          .is_ok());
  EXPECT_THAT(mixer_node->sources(), ElementsAre(graph.node(1)));
  EXPECT_EQ(mixer_node->dest(), nullptr);
  EXPECT_EQ(ctx.gain_controls.at(GainControlId{10})->num_mixers(), 1ul);
  EXPECT_EQ(ctx.gain_controls.at(GainControlId{20})->num_mixers(), 0ul);
  EXPECT_EQ(ctx.gain_controls.at(GainControlId{30})->num_mixers(), 0ul);
  ...
  ASSERT_TRUE(Node::CreateEdge(graph.ctx(), graph.node(2), mixer_node,
                               {.gain_ids = {GainControlId{10}, GainControlId{20}}})
                  .is_ok());
  EXPECT_THAT(mixer_node->sources(), ElementsAre(graph.node(1), graph.node(2)));
  EXPECT_EQ(mixer_node->dest(), nullptr);
  EXPECT_EQ(ctx.gain_controls.at(GainControlId{10})->num_mixers(), 1ul);
  EXPECT_EQ(ctx.gain_controls.at(GainControlId{20})->num_mixers(), 1ul);
  EXPECT_EQ(ctx.gain_controls.at(GainControlId{30})->num_mixers(), 0ul);
  ...
  ASSERT_TRUE(
      Node::CreateEdge(graph.ctx(), mixer_node, graph.node(3), {.gain_ids = {GainControlId{30}}})
          .is_ok());
  EXPECT_THAT(mixer_node->sources(), ElementsAre(graph.node(1), graph.node(2)));
  EXPECT_EQ(mixer_node->dest(), graph.node(3));
  EXPECT_EQ(ctx.gain_controls.at(GainControlId{10})->num_mixers(), 1ul);
  EXPECT_EQ(ctx.gain_controls.at(GainControlId{20})->num_mixers(), 1ul);
  EXPECT_EQ(ctx.gain_controls.at(GainControlId{30})->num_mixers(), 1ul);
  ...
  ASSERT_TRUE(Node::DeleteEdge(graph.ctx(), graph.node(1), mixer_node).is_ok());
  ...
  ASSERT_TRUE(Node::DeleteEdge(graph.ctx(), mixer_node, graph.node(3)).is_ok());
  EXPECT_THAT(mixer_node->sources(), ElementsAre(graph.node(2)));
  EXPECT_EQ(mixer_node->dest(), nullptr);
  EXPECT_EQ(ctx.gain_controls.at(GainControlId{10})->num_mixers(), 1ul);
  EXPECT_EQ(ctx.gain_controls.at(GainControlId{20})->num_mixers(), 1ul);
  ...
  ASSERT_TRUE(Node::DeleteEdge(graph.ctx(), graph.node(2), mixer_node).is_ok());
  EXPECT_THAT(mixer_node->sources(), ElementsAre());
  EXPECT_EQ(mixer_node->dest(), nullptr);
  EXPECT_EQ(ctx.gain_controls.at(GainControlId{10})->num_mixers(), 0ul);
  EXPECT_EQ(ctx.gain_controls.at(GainControlId{20})->num_mixers(), 0ul);
  EXPECT_EQ(ctx.gain_controls.at(GainControlId{30})->num_mixers(), 0ul);
}
...
TEST(MixerNodeTest, CreateEdgeCannotAcceptSourceFormat) {
  ...
  auto result = Node::CreateEdge(ctx, graph.node(1), mixer_node, /*options=*/{});
