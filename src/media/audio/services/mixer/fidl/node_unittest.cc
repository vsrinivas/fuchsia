...
TEST(NodeCreateDeleteEdgeTest, MaxDownstreamConsumers) {
  FakeGraph graph({
      //
      //         1
  ...
  auto dest = graph.node(7);

  auto thread1 = graph.thread(1);
  auto thread2 = graph.thread(2);
  auto detached_thread = graph.detached_thread();
  ...
  // Since dest (node 7) is on thread2, a task is queued for thread2 first. That task updates the
  // counts for PipelineStages 4,5,6 but not for any others (yet).
  q->RunForThread(thread2->id());

  EXPECT_EQ(graph.node(1)->fake_pipeline_stage()->max_downstream_consumers(), 0);
  EXPECT_EQ(graph.node(2)->fake_pipeline_stage()->max_downstream_consumers(), 0);
  EXPECT_EQ(graph.node(4)->fake_pipeline_stage()->max_downstream_consumers(), 1);
  EXPECT_EQ(graph.node(5)->fake_pipeline_stage()->max_downstream_consumers(), 1);
  EXPECT_EQ(graph.node(6)->fake_pipeline_stage()->max_downstream_consumers(), 1);

  // After running thread1's task, all PipelineStages should be updated.
  q->RunForThread(thread1->id());

  EXPECT_EQ(graph.node(1)->fake_pipeline_stage()->max_downstream_consumers(), 2);
  EXPECT_EQ(graph.node(2)->fake_pipeline_stage()->max_downstream_consumers(), 1);
  EXPECT_EQ(graph.node(4)->fake_pipeline_stage()->max_downstream_consumers(), 1);
  EXPECT_EQ(graph.node(5)->fake_pipeline_stage()->max_downstream_consumers(), 1);
  EXPECT_EQ(graph.node(6)->fake_pipeline_stage()->max_downstream_consumers(), 1);
  ...
  // Check Node counts.
  EXPECT_EQ(graph.node(1)->max_downstream_consumers(), 1);
  ...
  EXPECT_EQ(graph.node(1)->fake_pipeline_stage()->max_downstream_consumers(), 2);
  EXPECT_EQ(graph.node(2)->fake_pipeline_stage()->max_downstream_consumers(), 1);
  EXPECT_EQ(graph.node(4)->fake_pipeline_stage()->max_downstream_consumers(), 0);
  EXPECT_EQ(graph.node(5)->fake_pipeline_stage()->max_downstream_consumers(), 0);
  EXPECT_EQ(graph.node(6)->fake_pipeline_stage()->max_downstream_consumers(), 0);

  // After running thread1's task, all PipelineStages should be updated.
  q->RunForThread(thread1->id());

  EXPECT_EQ(graph.node(1)->fake_pipeline_stage()->max_downstream_consumers(), 1);
  EXPECT_EQ(graph.node(2)->fake_pipeline_stage()->max_downstream_consumers(), 0);
  EXPECT_EQ(graph.node(4)->fake_pipeline_stage()->max_downstream_consumers(), 0);
  EXPECT_EQ(graph.node(5)->fake_pipeline_stage()->max_downstream_consumers(), 0);
  EXPECT_EQ(graph.node(6)->fake_pipeline_stage()->max_downstream_consumers(), 0);
