...
std::shared_ptr<test_effects_v1_module_ext> OpenTestEffectsExt() {
  ...
  void* sym = dlsym(lib, "test_effects_v1_module_ext_instance");
  if (!sym) {
    return nullptr;
  }
  ...
  return std::shared_ptr<test_effects_v1_module_ext>(
      reinterpret_cast<test_effects_v1_module_ext*>(sym), [lib](auto* ptr) { dlclose(lib); });
}
