...
std::unique_ptr<AudioClock> FakeClockManager::CreateClientAdjustable(zx::clock clock) {
...
std::unique_ptr<AudioClock> FakeClockManager::CreateClientFixed(zx::clock clock) {
  UpdateRefToMonoTransform(clock);
  return std::make_unique<FakeAudioClock>(
      FakeAudioClock::ClientFixed(shared_from_this(), std::move(clock)));
}

std::unique_ptr<AudioClock> FakeClockManager::CreateDeviceAdjustable(zx::clock clock,
                                                                     uint32_t domain) {
  UpdateRefToMonoTransform(clock);
  return std::make_unique<FakeAudioClock>(
...
std::unique_ptr<AudioClock> FakeClockManager::CreateDeviceFixed(zx::clock clock, uint32_t domain) {
  UpdateRefToMonoTransform(clock);
  return std::make_unique<FakeAudioClock>(
      FakeAudioClock::DeviceFixed(shared_from_this(), std::move(clock), domain));
}

std::unique_ptr<AudioClock> FakeClockManager::CreateClientAdjustable(zx::time start_time,
...
std::unique_ptr<AudioClock> FakeClockManager::CreateClientFixed(zx::time start_time,
                                                                int32_t rate_adjust_ppm) {
  auto clock = clock::testing::CreateCustomClock(
                   {.start_val = start_time, .rate_adjust_ppm = rate_adjust_ppm})
                   .take_value();
...
std::unique_ptr<AudioClock> FakeClockManager::CreateDeviceAdjustable(zx::time start_time,
                                                                     int32_t rate_adjust_ppm,
                                                                     uint32_t domain) {
  auto clock = clock::testing::CreateCustomClock(
                   {.start_val = start_time, .rate_adjust_ppm = rate_adjust_ppm})
...
std::unique_ptr<AudioClock> FakeClockManager::CreateDeviceFixed(zx::time start_time,
                                                                int32_t rate_adjust_ppm,
                                                                uint32_t domain) {
  auto clock = clock::testing::CreateCustomClock(
                   {.start_val = start_time, .rate_adjust_ppm = rate_adjust_ppm})
...
void FakeClockManager::UpdateClockRate(zx_koid_t clock_id, int32_t rate_adjust_ppm) {
...
void FakeClockManager::UpdateRefToMonoTransform(const zx::clock& clock) {
  std::lock_guard<std::mutex> lock(mutex_);
...
void FakeClockManager::UpdateRefToMonoTransform(zx_koid_t clock_id, zx::time start_time,
                                                int32_t rate_adjust_ppm) {
  std::lock_guard<std::mutex> lock(mutex_);
