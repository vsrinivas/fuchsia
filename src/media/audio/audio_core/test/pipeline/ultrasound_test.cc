...
class UltrasoundTest : public HermeticAudioCoreTest {
 protected:
  ...
  struct RendererHolder {
    fuchsia::media::AudioRendererPtr renderer;
  ...
  RendererHolder CreateUltrasoundRenderer() {
    ...
    bool created = false;
    ultrasound_factory_->CreateRenderer(holder.renderer.NewRequest(),
    ...
    holder.renderer.set_error_handler(ErrorHandler());
    return holder;
  }
...
TEST_F(UltrasoundTest, RendererDoesNotSupportSetPcmStreamType) {
  ...
  holder.renderer.set_error_handler([&renderer_error](auto status) { renderer_error = {status}; });

  // Call SetPcmStreamType. We use the current stream type here to ensure we're definitely
  // requesting a supported stream type.
  holder.renderer->SetPcmStreamType(holder.stream_type);
...
TEST_F(UltrasoundTest, RendererDoesNotSupportSetUsage) {
  ...
  holder.renderer->SetUsage(fuchsia::media::AudioRenderUsage::MEDIA);
...
TEST_F(UltrasoundTest, RendererDoesNotSupportBindGainControl) {
  ...
  holder.renderer->BindGainControl(gain_control.NewRequest());
...
TEST_F(UltrasoundTest, RendererDoesNotSupportSetReferenceClock) {
  ...
  holder.renderer->SetReferenceClock(std::move(clock_to_set));
