...
void MeasureFreqRespSinad(Mixer* mixer, uint32_t src_buf_size, double* level_db, double* sinad_db) {
  ...
  for (uint32_t idx = 0; idx < num_freqs; ++idx) {
    ...
    OverwriteCosine(source.data(), src_buf_size, FrequencySet::kReferenceFreqs[freq_idx]);
    ...
    MeasureAudioFreq(accum.data(), kFreqTestBufSize, FrequencySet::kReferenceFreqs[freq_idx],
...
void TestDownSampleRatio0(Resampler sampler_type, double* freq_resp_results,
                          double* sinad_results) {
  auto mixer =
      SelectMixer(fuchsia::media::AudioSampleFormat::FLOAT, 1, 192000 - 2, 1, 48000, sampler_type);

  MeasureFreqRespSinad(mixer.get(), round(kFreqTestBufSize * 4) - 2, freq_resp_results,
                       sinad_results);
}
...
void TestDownSampleRatio1(Resampler sampler_type, double* freq_resp_results,
  ...
  MeasureFreqRespSinad(mixer.get(), round(kFreqTestBufSize * 2.0), freq_resp_results,
                       sinad_results);
}
...
void TestUpSampleRatio3(Resampler sampler_type, double* freq_resp_results, double* sinad_results) {
  auto mixer =
      SelectMixer(fuchsia::media::AudioSampleFormat::FLOAT, 1, 12000, 1, 48000, sampler_type);

  MeasureFreqRespSinad(mixer.get(), (kFreqTestBufSize / 4), freq_resp_results, sinad_results);
}
...
void TestNxNEquivalence(Resampler sampler_type, double* freq_resp_results, double* sinad_results) {
  ...
  uint32_t num_source_frames = round(kFreqTestBufSize * source_rate / dest_rate);
  ...
  uint32_t frac_src_frames = num_chans * (num_source_frames + 1) * Mixer::FRAC_ONE;
