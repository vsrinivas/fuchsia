...
// Verify correct FROM and TO locations, and quantity. frac_source_frames &
// source_offset are specified in fractional values (fixed .13 format).
//
...
void TestBasicPosition(Resampler samplerType) {
  ...
  int32_t frac_source_offset = 2 << kPtsFractionalBits;
  ...
  mix_result = mixer->Mix(accum, 4, &dest_offset, source, 5 << kPtsFractionalBits,
                          &frac_source_offset, true);

  EXPECT_TRUE(mix_result);
  EXPECT_EQ(4u, dest_offset);
  EXPECT_EQ(5 << kPtsFractionalBits, frac_source_offset);
  ...
  EXPECT_EQ(4u, dest_offset);
  EXPECT_EQ(2 << kPtsFractionalBits, frac_source_offset);
  ...
  frac_source_offset = 2 << kPtsFractionalBits;
  dest_offset = 0;
  ...
  EXPECT_EQ(3 << kPtsFractionalBits, frac_source_offset);
...
// TODO(mpuryear): Change frac_source_frames parameter to be (integer) source_frames,
...
TEST(Position, Position_Fractional_Point) {
  ...
  int32_t frac_source_offset = 3 << (kPtsFractionalBits - 1);
  uint32_t dest_offset = 1;
  int16_t source[] = {1, 0xC, 0x7B, 0x4D2, 0x3039};
  ...
  EXPECT_EQ(7 << (kPtsFractionalBits - 1), frac_source_offset);
  EXPECT_THAT(accum, Pointwise(FloatEq(), expect));

  //
  // Check: Destination demand exceeds source supply
  // Source (offset 2.49 of 4) has 2. Destination (offset 1 of 4) wants 3.
  frac_source_offset = (5 << (kPtsFractionalBits - 1)) - 1;
  dest_offset = 1;
  // Mix will move source[2,3] to accum[1,2]
  float expect2[] = {-0x00002000, 0x0007B000, 0x004D2000, -0x00929000, -0x05BA0000};
  NormalizeInt28ToPipelineBitwidth(expect2, std::size(expect2));

  mix_result = mixer->Mix(accum, 4, &dest_offset, source, 4 << kPtsFractionalBits,
                          &frac_source_offset, false);

  EXPECT_TRUE(mix_result);
  EXPECT_EQ(3u, dest_offset);
  EXPECT_EQ((9 << (kPtsFractionalBits - 1)) - 1, frac_source_offset);
...
// frac_source_offset should be advanced).
void TestLateSourceOffset(Resampler sampler_type) {
  ...
  if (mixer->pos_filter_width().raw_value() > 0) {
    ...
    const auto initial_frac_source_offset = frac_source_offset;
    ...
    EXPECT_EQ(frac_source_offset, initial_frac_source_offset);
