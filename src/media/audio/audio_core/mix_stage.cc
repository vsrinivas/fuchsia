...

    {
      ...
      if (consumed_source) {
        FX_DCHECK(source_offset + pos_width >= source_buffer.length())
            << "source_offset (" << ffl::String::DecRational << source_offset
            << ") plus pos_width (" << ffl::String::DecRational << pos_width
            << ") should equal/exceed source_buffer.length (" << ffl::String::DecRational
            << source_buffer.length() << ")";
      } else {
        FX_DCHECK(source_offset + pos_width < source_buffer.length())
            << "source_offset (" << ffl::String::DecRational << source_offset
            << ") plus pos_width (" << ffl::String::DecRational << pos_width
            << ") should be less than source_buffer.length (" << ffl::String::DecRational
            << source_buffer.length() << ")";
      }
