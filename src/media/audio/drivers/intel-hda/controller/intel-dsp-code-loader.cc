...
zx_status_t IntelDspCodeLoader::TransferFirmware(const fzl::PinnedVmo& pinned_fw, size_t fw_size) {
  ...
  for (num_region = 0; (num_region < region_count) && (remaining > 0); num_region++) {
    ...
    bdl[num_region].length =
        (remaining > r.size) ? static_cast<uint32_t>(r.size) : static_cast<uint32_t>(remaining);
  ...
  uint32_t ctl_val = HDA_SD_REG_CTRL_STRM_TAG(ADSP_CLDMA_STREAM_TAG) | HDA_SD_REG_CTRL_STRIPE1;
