...
zx_status_t GaussPdmInputStream::Create(zx_device_t* parent) {
  ...
  auto stream = fbl::AdoptRef(new GaussPdmInputStream(parent, std::move(domain)));
...
zx_status_t GaussPdmInputStream::Bind(const char* devname, zx_device_t* parent) {
  ...
  zx_status_t status =
      pdev_get_interrupt(&audio_device_.pdev, 0 /* PDM IRQ */, 0, &audio_device_.pdm_irq);
  ...
  status = thrd_create_with_name(
      &irqthrd_,
      [](void* thiz) -> int { return reinterpret_cast<GaussPdmInputStream*>(thiz)->IrqThread(); },
      this, "pdm_irq_thread");
...
void GaussPdmInputStream::GetChannel(GetChannelCompleter::Sync completer) {
...
zx_status_t GaussPdmInputStream::OnGetStreamFormats(dispatcher::Channel* channel,
  ...
  if (supported_formats_.size() > std::numeric_limits<uint16_t>::max()) {
    zxlogf(ERROR,
           "Too many formats (%zu) to send during "
...
int GaussPdmInputStream::IrqThread() {
  ...
  for (;;) {
    ...
    if (status != ZX_OK) {
      zxlogf(DEBUG1, "audio_pdm_input: interrupt error: %d.", status);
      break;
    }

    a113_toddr_clear_interrupt(&audio_device_, 0x4);

    uint32_t offset = a113_toddr_get_position(&audio_device_) -
    ...
    size_t data_available = offset >= last_notification_offset
...
zx_status_t GaussPdmInputStream::OnGetBuffer(dispatcher::Channel* channel,
  ...
  uint32_t notifications_per_ring = req.notifications_per_ring ? req.notifications_per_ring : 1;

  uint32_t requested_period_size =
      req.min_ring_buffer_frames * frame_size_ / notifications_per_ring;

  uint32_t period_size = fbl::round_up(requested_period_size, static_cast<uint32_t>(fifo_depth_));

  ring_buffer_size_.store(
      fbl::round_up(period_size * notifications_per_ring, static_cast<uint32_t>(PAGE_SIZE)));
  ...
  if (ring_buffer_size_.load() % frame_size_) {
    zxlogf(ERROR, "Frame size (%u) does not divide ring buffer size (%zu)", frame_size_,
  ...
  zxlogf(DEBUG1, "ring_buffer_size=%lu", ring_buffer_size_.load());
  zxlogf(DEBUG1, "req.notifications_per_ring=%u", req.notifications_per_ring);
  ...
  if (resp.result != ZX_OK) {
    zxlogf(ERROR, "Failed to create ring buffer (size %lu)", ring_buffer_size_.load());
    goto finished;
  }
  ...
  a113_toddr_set_buf(&audio_device_, (uint32_t)start_address, (uint32_t)end_address);
  a113_toddr_set_intrpt(&audio_device_, static_cast<uint32_t>(period_size / 8));
