...
int get_midi_message_length(uint8_t status_byte) {
  if (status_byte >= 0xF0) {
    ...
    static_assert(std::size(SYSTEM_BYTE_LENGTHS) >= 16, "SYSTEM_BYTE_LENGTHS too small");
    return SYSTEM_BYTE_LENGTHS[status_byte & 0x0F];
  } else if (status_byte >= 0x80) {
    // Channel voice messages use high nibble for size.
    static_assert(std::size(CHANNEL_BYTE_LENGTHS) >= 0xE - 8, "CHANNEL_BYTE_LENGTHS too small");
