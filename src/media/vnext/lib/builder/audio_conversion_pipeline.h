...
#ifndef SRC_MEDIA_VNEXT_LIB_BUILDER_AUDIO_CONVERSION_PIPELINE_H_
#define SRC_MEDIA_VNEXT_LIB_BUILDER_AUDIO_CONVERSION_PIPELINE_H_
...
// This class creates conversion pipelines that decode, encode or transcode audio streams as needed.
...
// return a unique pointer to a valid |AudioConversionPipeline|. No attempt is made to actually
// create and connect converters until the |ConnectInputStream| method is called.
//
// |AudioConversionPipeline| instances are not thread-safe.
//
// TODO(dalesat): This could maybe be merged with VideoConversionPipeline using templates.
class AudioConversionPipeline {
 public:
  // Returns a unique pointer to a |AudioConversionPipeline| unless the parameters indicate no need
  ...
  static std::unique_ptr<AudioConversionPipeline> Create(
  ...
  // throughout the lifetime of the returned pipeline.
  // TODO(dalesat): Define a type that combines |output_supported_compression_types| and
  // |output_supports_uncompressed|.
  static std::unique_ptr<AudioConversionPipeline> Create(
      const fuchsia::mediastreams::AudioFormat& format,
      const fuchsia::mediastreams::CompressionPtr& input_compression,
  ...
  // Constructs an |AudioConversionPipeline|. Use a |Create| method instead.
  AudioConversionPipeline(fuchsia::mediastreams::AudioFormat format,
  ...
  ~AudioConversionPipeline() = default;

  // Disallow copy, assign and move.
  AudioConversionPipeline(const AudioConversionPipeline&) = delete;
  AudioConversionPipeline& operator=(const AudioConversionPipeline&) = delete;
  AudioConversionPipeline(AudioConversionPipeline&&) = delete;
  AudioConversionPipeline& operator=(AudioConversionPipeline&&) = delete;
  ...
  fuchsia::mediastreams::AudioFormat output_format() const {
 ...
 private:
  ...
  fuchsia::audio::DecoderPtr decoder_;
};

}  // namespace fmlib

#endif  // SRC_MEDIA_VNEXT_LIB_BUILDER_AUDIO_CONVERSION_PIPELINE_H_
