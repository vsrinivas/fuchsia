...
// static
std::unique_ptr<VideoConversionPipeline> VideoConversionPipeline::Create(
...
std::unique_ptr<VideoConversionPipeline> VideoConversionPipeline::Create(
  ...
  if (!input_compression) {
    ...
    if (output_supports_uncompressed) {
      ...
      // Encode uncompressed.
      return std::make_unique<VideoConversionPipeline>(
...
VideoConversionPipeline::ConnectInputStream(
...
[[nodiscard]] fpromise::promise<void, fuchsia::media2::ConnectionError>
VideoConversionPipeline::ConnectOutputStream(zx::eventpair buffer_collection_token,
  ...
  decoder_->ConnectOutputStream(
      ...
      [completer = std::move(bridge.completer)](
          const fuchsia::video::Decoder_ConnectOutputStream_Result& result) mutable {
        if (result.is_err()) {
...
[[nodiscard]] fpromise::promise<void, fuchsia::media2::ConnectionError>
VideoConversionPipeline::BuildForDecode(
    zx::eventpair buffer_collection_token,
    fidl::InterfaceRequest<fuchsia::media2::StreamSink> request) {
  auto decoder_creator = service_provider_.ConnectToService<fuchsia::video::DecoderCreator>();
  ...
  decoder_->ConnectInputStream(
      ...
      [this, completer = std::move(bridge.completer)](
          const fuchsia::video::Decoder_ConnectInputStream_Result& result) mutable {
        if (result.is_err()) {
          completer.complete_error(result.err());
          return;
        }
...
[[nodiscard]] fpromise::promise<void, fuchsia::media2::ConnectionError>
VideoConversionPipeline::BuildForEncode(
    zx::eventpair buffer_collection_token,
    fidl::InterfaceRequest<fuchsia::media2::StreamSink> request) {
...
[[nodiscard]] fpromise::promise<void, fuchsia::media2::ConnectionError>
VideoConversionPipeline::BuildForTranscode(
    zx::eventpair buffer_collection_token,
    fidl::InterfaceRequest<fuchsia::media2::StreamSink> request) {
  return BuildForDecode(std::move(buffer_collection_token), std::move(request))
