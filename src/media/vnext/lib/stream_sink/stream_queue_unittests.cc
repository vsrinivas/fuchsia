...
// Tests basic synchronous fifo behavior.
TEST(StreamQueueTest, SyncFifo) {
  ...
  for (size_t i = 0; i < kElements; ++i) {
    EXPECT_FALSE(under_test.empty());
    EXPECT_EQ(kElements - i, under_test.size());
    EXPECT_TRUE(under_test.front().is_packet());
    EXPECT_EQ(i, under_test.front().packet());
    under_test.pop();
  }
...
// Tests the |clear| method synchronous behavior.
TEST(StreamQueueTest, SyncClear) {
  StreamQueue<size_t, float> under_test;
  static const size_t kElements = 10;
  ...
  // Expect just |kCleared| on the queue.
  EXPECT_EQ(1u, under_test.size());
  EXPECT_TRUE(under_test.front().is_clear_request());
  EXPECT_EQ(0.0f, under_test.front().clear_request());
  under_test.pop();
...
// Tests the |end| method synchronous behavior.
TEST(StreamQueueTest, SyncEnd) {
  StreamQueue<size_t, float> under_test;
  static const size_t kElements = 10;
  ...
  for (size_t i = 0; i < kElements; ++i) {
    EXPECT_FALSE(under_test.empty());
    EXPECT_EQ(kElements - i + 1, under_test.size());
    EXPECT_TRUE(under_test.front().is_packet());
    EXPECT_EQ(i, under_test.front().packet());
    under_test.pop();
  }

  // Expect just |kEnded| on the queue.
  EXPECT_EQ(1u, under_test.size());
  EXPECT_TRUE(under_test.front().is_ended());
  under_test.pop();
}

// Tests the |drain| method synchronous behavior.
TEST(StreamQueueTest, SyncDrain) {
  StreamQueue<size_t, float> under_test;
  static const size_t kElements = 10;
  ...
  EXPECT_FALSE(under_test.is_drained());
  ...
  // Expect the queue to be drained.
  EXPECT_TRUE(under_test.empty());
  EXPECT_TRUE(under_test.is_drained());
}
