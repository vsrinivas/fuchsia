...
constexpr size_t kDefaultChunkSize = 256ull * 1024ull;
...
constexpr float kConservativeFactor = 0.8f;

constexpr size_t kByteRateMaxSamples = 8;

}  // namespace

// static
std::shared_ptr<ReaderCache> ReaderCache::Create(async::Executor& executor,
                                                 std::shared_ptr<Reader> upstream_reader) {
  return std::make_shared<ReaderCache>(executor, std::move(upstream_reader));
}

ReaderCache::ReaderCache(async::Executor& executor, std::shared_ptr<Reader> upstream_reader)
    : executor_(executor),
      upstream_reader_(std::move(upstream_reader)),
      dispatcher_(async_get_default_dispatcher()),
      demux_byte_rate_(kByteRateMaxSamples),
      upstream_reader_byte_rate_(kByteRateMaxSamples) {
  upstream_reader_->Describe(
      // TODO(dalesat): Maybe use a struct here to get rid of the lint error.
      // NOLINTNEXTLINE(bugprone-easily-swappable-parameters)
      [this, upstream_reader](zx_status_t status, size_t size, bool can_seek) {
        upstream_size_ = size;
        upstream_can_seek_ = can_seek;
        last_status_ = status;
        describe_is_complete_.Complete();
      });
}

void ReaderCache::Describe(DescribeCallback callback) {
  executor_.schedule_task(
      describe_is_complete_.When().and_then([this, callback = std::move(callback)]() mutable {
...
void ReaderCache::ReadAt(size_t position, uint8_t* buffer, size_t bytes_to_read,
  ...
  FX_CHECK(buffer);
  FX_CHECK(bytes_to_read > 0);

  executor_.schedule_task(describe_is_complete_.When().and_then(
...
void ReaderCache::SetCacheOptions(size_t capacity, size_t max_backtrack) {
  FX_CHECK(!load_in_progress_) << "SetCacheOptions cannot be called while a load is"
                                  " in progress.";
...
void ReaderCache::ServeReadAtRequest(ReaderCache::ReadAtRequest request) {
  FX_CHECK(buffer_);
  FX_CHECK(request.buffer);
  FX_CHECK(request.callback);
  FX_CHECK(request.position < upstream_size_);
...
void ReaderCache::StartLoadForPosition(size_t position,
  ...
  FX_CHECK(buffer_);
  FX_CHECK(!load_in_progress_);
  load_in_progress_ = true;

  auto load_range = CalculateLoadRange(position);
  FX_CHECK(load_range) << "The media is fully cached for the read, but a load was requested.";
