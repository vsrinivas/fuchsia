...
#include "src/media/vnext/lib/reader/file_reader.h"
...
void FileReader::ReadAt(size_t position, uint8_t* buffer, size_t bytes_to_read,
  ...
  off_t seek_result = lseek(fd_.get(), static_cast<off_t>(position), SEEK_SET);
  if (seek_result < 0) {
  ...
  ssize_t result = fxl::ReadFileDescriptor(fd_.get(), reinterpret_cast<char*>(buffer),
