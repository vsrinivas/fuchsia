...
std::optional<float> ByteRateEstimator::Estimate() {
  ...
  int64_t n = static_cast<int64_t>(std::min(samples_.size(), max_sample_count_));
  for (int64_t i = 0; i < static_cast<int64_t>(samples_.size()); ++i) {
...
void ByteRateEstimator::AddSample(const ByteRateSample& sample) {
  FX_CHECK(sample.stop_time > sample.start_time);
  zx::duration duration = sample.stop_time - sample.start_time;
  zx::duration time_per_byte = duration / static_cast<int64_t>(sample.bytes_processed);
