...
void main() {
  ...
  test('tests that harvester and dockyard can talk', () async {
    ...
    try {
      ...
      // Detect when the Server is listening.
      final serverListening = Completer();
      dockyardHost.stdout
          .transform(utf8.decoder)
          .transform(LineSplitter())
          .listen((String string) {
        if (string.contains('Server listening')) {
          serverListening.complete(true);
        }
        if (string.contains('memory:device_total_bytes')) {
          receiveDeviceTotalBytes.complete(true);
        }
      });
      await serverListening.future;
