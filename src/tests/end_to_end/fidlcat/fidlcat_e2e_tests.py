...
class Fidlcat:
    ...
    _args = []
    _env = {'FUCHSIA_ANALYTICS_DISABLED': '1'}
    _ffx_bridge = None

    def __init__(self, *args, redirect_stderr=False):
        """
        redirect_stderr: whether to redirect stderr to stdout. Otherwise, stderr will be discarded.
        """
        ...
        self.process = subprocess.Popen(
            ...
            stdout=subprocess.PIPE,
            stderr=stderr)
        # Contains both stdout and stderr, if redirect_stderr.
        self.stdout = ''

    def wait(self):
        """Wait for the process to terminate, assert the returncode, fill the stdout."""
    ...
    def read_until(self, pattern: str):
        """
        Read the stdout until EOF or a line contains pattern.
...
class FidlcatE2eTests(unittest.TestCase):
    ...
    def test_run_echo(self):
        ...
        self.assertEqual(fidlcat.wait(), 0)
    ...
    def test_stay_alive(self):
        fidlcat = Fidlcat(
            '--remote-name=echo_client', '--stay-alive', redirect_stderr=True)
