...
class Ffx {
  static const _ffxTimeout = Duration(seconds: 10);
  static const _maxFfxTries = 5;
  ...
  // TODO(fxbug.dev/98759): retry `ffx log` if the command times out.
  static Future<bool> expectLog(String tag, String expect) async {
    final success = await retry(
      () => _expectLog(tag, expect).timeout(_ffxTimeout),
      retryIf: (e) => e is TimeoutException,
      maxAttempts: _maxFfxTries,
    );
    return success;
  }

  static Future<bool> _expectLog(String tag, String expect) async {
    var process =
        await Ffx.start(['log', '--severity', 'TRACE', '--filter', tag]);
    var success = false;
    var found = Completer();
...
void main() {
  ...
  Future<bool> runComponentExpect(
      String url, String name, String expect) async {
