...
func (ex testCase) toTmplParams(index int) map[string]interface{} {
	return map[string]interface{}{
		"index":        index,
		"comment":      ex.comment,
		"escapedValue": escapeStr(ex.value),
		"lenValue":     len(ex.value),
		"bytesValue":   []byte(ex.value),
		"padding":      fidlAlign(len(ex.value)) - len(ex.value),
	}
}

func printValidCase(params map[string]interface{}) {
...
func printInvalidCase(params map[string]interface{}) {
	var buf bytes.Buffer
...
`)
	var validIndex, invalidIndex int
	for _, ex := range cases {
		switch ex.kind {
		case validCase:
			if !utf8.ValidString(ex.value) {
				panic(fmt.Sprintf("supposedly valid example seems invalid: %+v", ex))
			}
			validIndex++
			printValidCase(ex.toTmplParams(validIndex))
		case invalidCase:
			if utf8.ValidString(ex.value) {
				panic(fmt.Sprintf("supposedly invalid example seems valid: %+v", ex))
			}
			invalidIndex++
			printInvalidCase(ex.toTmplParams(invalidIndex))
		}
	}
}
