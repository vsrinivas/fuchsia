...
SystemDataUpdaterImpl::SystemDataUpdaterImpl(
    encoder::SystemData* system_data, const std::string& cache_file_name_prefix)
...
std::string SystemDataUpdaterImpl::Restore(const std::string& suffix) {
  ...
  FX_LOGS(INFO) << "Restored `" << str << "` from `" << cache_file_name_prefix_
                << suffix << "`";
  return str;
}

void SystemDataUpdaterImpl::Persist(const std::string& suffix,
                                    const std::string& value) {
  FX_LOGS(INFO) << "Writing `" << value << "` to `" << cache_file_name_prefix_
                << suffix << "`";
...
void SystemDataUpdaterImpl::DeleteData(const std::string& suffix) {
  if (std::remove((cache_file_name_prefix_ + suffix).c_str()) == 0) {
    FX_LOGS(INFO) << "Successfully deleted `" << cache_file_name_prefix_
                  << suffix << "`";
  }
}
