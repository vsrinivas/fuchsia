...
#error Unknown architecture.
  ...
  if (cfg.has_virtio_balloon() && cfg.virtio_balloon()) {
    ...
    status = pci_bus_->Connect(balloon->pci_device(), device_loop_->dispatcher(), true);
    if (status != ZX_OK) {
  ...
  for (auto& block_device : *cfg.mutable_block_devices()) {
    ...
    status = pci_bus_->Connect(block->pci_device(), device_loop_->dispatcher(), true);
    if (status != ZX_OK) {
  ...
  if (cfg.has_virtio_console() && cfg.virtio_console()) {
    ...
    status = pci_bus_->Connect(console_->pci_device(), device_loop_->dispatcher(), true);
    if (status != ZX_OK) {
  ...
  if (cfg.has_virtio_gpu() && cfg.virtio_gpu()) {
    ...
    status = pci_bus_->Connect(input_keyboard_->pci_device(), device_loop_->dispatcher(), true);
    if (status != ZX_OK) {
    ...
    status = pci_bus_->Connect(input_pointer_->pci_device(), device_loop_->dispatcher(), true);
    if (status != ZX_OK) {
    ...
    // Setup GPU device.
    status = pci_bus_->Connect(gpu_->pci_device(), device_loop_->dispatcher(), true);
    if (status != ZX_OK) {
  ...
  if (cfg.has_virtio_rng() && cfg.virtio_rng()) {
    ...
    status = pci_bus_->Connect(rng_->pci_device(), device_loop_->dispatcher(), true);
    if (status != ZX_OK) {
  ...
  if (cfg.has_virtio_vsock() && cfg.virtio_vsock()) {
    ...
    status = pci_bus_->Connect(vsock_device->pci_device(), device_loop_->dispatcher(), true);
    if (status != ZX_OK) {
  ...
  if (cfg.has_wayland_device()) {
    ...
    status = pci_bus_->Connect(wl_->pci_device(), device_loop_->dispatcher(), true);
    if (status != ZX_OK) {
  ...
  if (cfg.has_magma_device()) {
    ...
    status = pci_bus_->Connect(magma_->pci_device(), device_loop_->dispatcher(), true);
    if (status != ZX_OK) {
  ...
  if (cfg.has_virtio_sound() && cfg.virtio_sound()) {
    ...
    status = pci_bus_->Connect(sound_->pci_device(), device_loop_->dispatcher(), true);
    if (status != ZX_OK) {
  ...
  for (auto& net_device : *cfg.mutable_net_devices()) {
    ...
    status = pci_bus_->Connect(net->pci_device(), device_loop_->dispatcher(), true);
    if (status != ZX_OK) {
