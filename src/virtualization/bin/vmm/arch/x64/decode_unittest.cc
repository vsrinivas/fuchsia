...
TEST(InstDecodeTest, failure) {
  ...
  ASSERT_EQ(inst_decode(nullptr, 32, 4, &vcpu_state, nullptr), ZX_ERR_OUT_OF_RANGE);
  ...
  ASSERT_EQ(inst_decode(bad_rex, 3, 4, &vcpu_state, nullptr), ZX_ERR_NOT_SUPPORTED);

  uint8_t bad_len[] = {0x00, 0b00'000'000};  // opcode modrm
  ASSERT_EQ(inst_decode(bad_len, 2, 4, &vcpu_state, nullptr), ZX_ERR_NOT_SUPPORTED);

  ASSERT_EQ(inst_decode(bad_len, 2, 3, &vcpu_state, nullptr), ZX_ERR_NOT_SUPPORTED);
}

TEST(InstDecodeTest, mov_89) {
  zx_vcpu_state_t vcpu_state;
  uint8_t bad_len[] = {0x89, 0, 0};  // opcode modrm ?
  ASSERT_EQ(inst_decode(bad_len, 3, 4, &vcpu_state, nullptr), ZX_ERR_OUT_OF_RANGE);
  ...
  ASSERT_EQ(inst_decode(bad_h66, 4, 4, &vcpu_state, nullptr), ZX_ERR_NOT_SUPPORTED);
...
TEST(InstDecodeTest, mov_88) {
  ...
  ASSERT_EQ(inst_decode(mov_ah, 2, 4, &vcpu_state, &inst), ZX_ERR_NOT_SUPPORTED);
  // movb %bh, (%rsi)
  uint8_t mov_bh[] = {0x88, 0b00'111'110};  // opcode modrm
  ASSERT_EQ(inst_decode(mov_bh, 2, 4, &vcpu_state, &inst), ZX_ERR_NOT_SUPPORTED);
  // movb %ch, (%rsi)
  uint8_t mov_ch[] = {0x88, 0b00'101'110};  // opcode modrm
  ASSERT_EQ(inst_decode(mov_ch, 2, 4, &vcpu_state, &inst), ZX_ERR_NOT_SUPPORTED);
  // movb %dh, (%rsi)
  uint8_t mov_dh[] = {0x88, 0b00'110'110};  // opcode modrm
  ASSERT_EQ(inst_decode(mov_dh, 2, 4, &vcpu_state, &inst), ZX_ERR_NOT_SUPPORTED);
...
TEST(InstDecodeTest, movz_0f_b6) {
  ...
  ASSERT_EQ(inst_decode(bad_len, 4, 4, &vcpu_state, nullptr), ZX_ERR_OUT_OF_RANGE);
  uint8_t bad_disp[] = {0x0f, 0xb6, 0b01'000'000};  // opcode opcode modrm
  ASSERT_EQ(inst_decode(bad_disp, 3, 4, &vcpu_state, nullptr), ZX_ERR_OUT_OF_RANGE);
...
TEST(InstDecodeTest, test_f6) {
  ...
  ASSERT_EQ(inst_decode(bad_mod_rm, 3, 4, &vcpu_state, nullptr), ZX_ERR_INVALID_ARGS);
