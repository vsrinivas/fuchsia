...
TEST(InstDecodeTest, failure) {
  ...
  ASSERT_EQ(inst_decode({bad_rex, 1}, 4, &vcpu_state, nullptr), ZX_ERR_NOT_SUPPORTED);
  ASSERT_EQ(inst_decode({bad_rex, 2}, 4, &vcpu_state, nullptr), ZX_ERR_OUT_OF_RANGE);
  ASSERT_EQ(inst_decode(bad_rex, 4, &vcpu_state, nullptr), ZX_ERR_NOT_SUPPORTED);

  uint8_t bad_len[] = {0x00, 0b00'000'000};  // opcode modrm
  ASSERT_EQ(inst_decode(bad_len, 4, &vcpu_state, nullptr), ZX_ERR_NOT_SUPPORTED);

  ASSERT_EQ(inst_decode(bad_len, 3, &vcpu_state, nullptr), ZX_ERR_NOT_SUPPORTED);
}

TEST(InstDecodeTest, mov_89) {
  zx_vcpu_state_t vcpu_state;
  uint8_t bad_len[] = {0x89, 0, 0};  // opcode modrm ?
  ASSERT_EQ(inst_decode(bad_len, 4, &vcpu_state, nullptr), ZX_ERR_OUT_OF_RANGE);
  uint8_t bad_disp[] = {0x89, 0b01'000'000};  // opcode modrm
  ASSERT_EQ(inst_decode(bad_disp, 4, &vcpu_state, nullptr), ZX_ERR_OUT_OF_RANGE);
  uint8_t bad_h66[] = {0x66, 0b0100'1000, 0x89, 0b00'010'000};  // h66 rex opcode modrm
  ASSERT_EQ(inst_decode(bad_h66, 4, &vcpu_state, nullptr), ZX_ERR_NOT_SUPPORTED);

  // mov %ecx, (%rax)
  uint8_t mov[] = {0x89, 0b00'001'000};  // opcode modrm
  Instruction inst;
  ASSERT_EQ(inst_decode(mov, 4, &vcpu_state, &inst), ZX_OK);
  ...
  ASSERT_EQ(inst_decode(mov_16bit, 2, &vcpu_state, &inst), ZX_OK);
  EXPECT_EQ(inst.type, InstructionType::kWrite);
  ...
  ASSERT_EQ(inst_decode(rex_mov, 4, &vcpu_state, &inst), ZX_OK);
  EXPECT_EQ(inst.type, InstructionType::kWrite);
  EXPECT_EQ(inst.access_size, 4u);
  EXPECT_EQ(inst.imm, 0u);
  ...
  ASSERT_EQ(inst_decode(mov_disp_1, 4, &vcpu_state, &inst), ZX_OK);
  EXPECT_EQ(inst.type, InstructionType::kWrite);
  EXPECT_EQ(inst.access_size, 4u);
  EXPECT_EQ(inst.imm, 0u);
  ...
  ASSERT_EQ(inst_decode(mov_disp_4, 4, &vcpu_state, &inst), ZX_OK);
  EXPECT_EQ(inst.type, InstructionType::kWrite);
  EXPECT_EQ(inst.access_size, 4u);
  EXPECT_EQ(inst.imm, 0u);
  EXPECT_EQ(inst.reg, &vcpu_state.rbx);
  ...
  ASSERT_EQ(inst_decode(h66_mov_disp, 4, &vcpu_state, &inst), ZX_OK);
  EXPECT_EQ(inst.type, InstructionType::kWrite);
  EXPECT_EQ(inst.access_size, 2u);
  EXPECT_EQ(inst.imm, 0u);
  ...
  ASSERT_EQ(inst_decode(mov_sib_nobase, 4, &vcpu_state, &inst), ZX_OK);
  EXPECT_EQ(inst.type, InstructionType::kWrite);
  EXPECT_EQ(inst.access_size, 4u);
  EXPECT_EQ(inst.imm, 0u);
...
TEST(InstDecodeTest, mov_88) {
  ...
  ASSERT_EQ(inst_decode(mov_ah, 4, &vcpu_state, &inst), ZX_ERR_NOT_SUPPORTED);
  // movb %bh, (%rsi)
  uint8_t mov_bh[] = {0x88, 0b00'111'110};  // opcode modrm
  ASSERT_EQ(inst_decode(mov_bh, 4, &vcpu_state, &inst), ZX_ERR_NOT_SUPPORTED);
  // movb %ch, (%rsi)
  uint8_t mov_ch[] = {0x88, 0b00'101'110};  // opcode modrm
  ASSERT_EQ(inst_decode(mov_ch, 4, &vcpu_state, &inst), ZX_ERR_NOT_SUPPORTED);
  // movb %dh, (%rsi)
  uint8_t mov_dh[] = {0x88, 0b00'110'110};  // opcode modrm
  ASSERT_EQ(inst_decode(mov_dh, 4, &vcpu_state, &inst), ZX_ERR_NOT_SUPPORTED);
...
TEST(InstDecodeTest, mov_c7) {
  ...
  ASSERT_EQ(inst_decode(bad_len_16bit, 2, &vcpu_state, nullptr), ZX_ERR_OUT_OF_RANGE);
  ...
  ASSERT_EQ(inst_decode(bad_mod_rm, 4, &vcpu_state, nullptr), ZX_ERR_INVALID_ARGS);
...
TEST(InstDecodeTest, movz_0f_b6) {
  ...
  Instruction inst;
  ASSERT_EQ(inst_decode(movz, 4, &vcpu_state, &inst), ZX_OK);
  EXPECT_EQ(inst.type, InstructionType::kRead);
  EXPECT_EQ(inst.access_size, 1u);
  EXPECT_EQ(inst.imm, 0u);
  EXPECT_EQ(inst.reg, &vcpu_state.rcx);
  ...
  ASSERT_EQ(inst_decode(movz_disp_4, 4, &vcpu_state, &inst), ZX_OK);
  EXPECT_EQ(inst.type, InstructionType::kRead);
  EXPECT_EQ(inst.access_size, 1u);
  EXPECT_EQ(inst.imm, 0u);
  EXPECT_EQ(inst.reg, &vcpu_state.rbx);
  ...
  ASSERT_EQ(inst_decode(rex_movz_disp, 4, &vcpu_state, &inst), ZX_OK);
  EXPECT_EQ(inst.type, InstructionType::kRead);
  EXPECT_EQ(inst.access_size, 1u);
  EXPECT_EQ(inst.imm, 0u);
  EXPECT_EQ(inst.reg, &vcpu_state.r12);
  ...
  ASSERT_EQ(inst_decode(mov_to_esi, 4, &vcpu_state, &inst), ZX_OK);
  EXPECT_EQ(inst.type, InstructionType::kRead);
  EXPECT_EQ(inst.access_size, 1u);
  EXPECT_EQ(inst.imm, 0u);
...
TEST(InstDecodeTest, movz_0f_b7) {
  ...
  ASSERT_EQ(inst_decode(has_h66, 4, &vcpu_state, nullptr), ZX_ERR_BAD_STATE);
  ...
  ASSERT_EQ(inst_decode(movz_16bit, 2, &vcpu_state, &inst), ZX_OK);
  EXPECT_EQ(inst.type, InstructionType::kRead);
  EXPECT_EQ(inst.access_size, 2u);
  EXPECT_EQ(inst.imm, 0u);
  EXPECT_EQ(inst.reg, &vcpu_state.rcx);
...
TEST(InstDecodeTest, test_f6) {
  ...
  Instruction inst;
  ASSERT_EQ(inst_decode(test, 4, &vcpu_state, &inst), ZX_OK);
  ...
  ASSERT_EQ(inst_decode(test_disp_4, 4, &vcpu_state, &inst), ZX_OK);
  EXPECT_EQ(inst.type, InstructionType::kTest);
  EXPECT_EQ(inst.access_size, 1u);
  ...
  ASSERT_EQ(inst_decode(test_sib_disp, 4, &vcpu_state, &inst), ZX_OK);
  EXPECT_EQ(inst.type, InstructionType::kTest);
  EXPECT_EQ(inst.access_size, 1u);
  EXPECT_EQ(inst.imm, 0x11u);
  EXPECT_EQ(inst.reg, nullptr);
  ...
  uint8_t test_sib_nobase[] = {0xf6, 0b00'000'100, 0b00'100'101, 0xef, 0xcd,
                               0xab, 0x00,         0x11};  // opcode modrm sib disp4 disp3 disp2
                                                           // disp1 imm
  ASSERT_EQ(inst_decode(test_sib_nobase, 4, &vcpu_state, &inst), ZX_OK);
  EXPECT_EQ(inst.type, InstructionType::kTest);
  EXPECT_EQ(inst.access_size, 1u);
  EXPECT_EQ(inst.imm, 0x11u);
  EXPECT_EQ(inst.reg, nullptr);
...
TEST(InstDecodeTest, or_81) {
  ...
  ASSERT_EQ(inst_decode(register_to_register, 4, &vcpu_state, nullptr), ZX_ERR_NOT_SUPPORTED);

  // orl 0x1, (%rax)
  uint8_t orl[] = {0x81, 0b00'001'000, 0x1, 0, 0, 0};  // opcode modrm imm4 imm3 imm2 imm1
  Instruction inst;
  ASSERT_EQ(inst_decode(orl, 4, &vcpu_state, &inst), ZX_OK);
  ...
  ASSERT_EQ(inst_decode(orw_16bit, 2, &vcpu_state, &inst), ZX_OK);
  EXPECT_EQ(inst.type, InstructionType::kLogicalOr);
  EXPECT_EQ(inst.access_size, 2u);
  EXPECT_EQ(inst.imm, 0x1u);
  EXPECT_EQ(inst.reg, nullptr);
  ...
  uint8_t rex_orq[] = {0b0100'1000, 0x81, 0b00'001'000, 0,
                       0,           0,    0x1};  // rex opcode modrm imm4 imm3 imm2 imm1
  ASSERT_EQ(inst_decode(rex_orq, 4, &vcpu_state, &inst), ZX_OK);
  EXPECT_EQ(inst.type, InstructionType::kLogicalOr);
  EXPECT_EQ(inst.access_size, 8u);
  EXPECT_EQ(inst.imm, 0x1000000u);
  EXPECT_EQ(inst.reg, nullptr);
  ...
  uint8_t orl_disp_1[] = {0x81, 0b01'001'011, 0xff, 0x10, 0, 0,
                          0};  // opcode modrm disp imm4 imm3 imm2 imm1
  ASSERT_EQ(inst_decode(orl_disp_1, 4, &vcpu_state, &inst), ZX_OK);
  EXPECT_EQ(inst.type, InstructionType::kLogicalOr);
  EXPECT_EQ(inst.access_size, 4u);
  EXPECT_EQ(inst.imm, 0x10u);
  EXPECT_EQ(inst.reg, nullptr);
  ...
  uint8_t orl_disp_4[] = {0x81, 0b10'001'011, 0, 0, 0, 0xff, 0, 0,
                          0,    0x1};  // opcode modrm disp4 disp3 disp2 disp1 imm4 imm3 imm2 imm1
  ASSERT_EQ(inst_decode(orl_disp_4, 4, &vcpu_state, &inst), ZX_OK);
  EXPECT_EQ(inst.type, InstructionType::kLogicalOr);
  EXPECT_EQ(inst.access_size, 4u);
  EXPECT_EQ(inst.imm, 0x1000000u);
  EXPECT_EQ(inst.reg, nullptr);
  ...
  uint8_t h66_orw_disp[] = {0x66, 0b0100'0100, 0x81, 0b01'001'000,
                            0xff, 0,           0x1};  // h66 rex opcode modrm disp imm2 imm1
  ASSERT_EQ(inst_decode(h66_orw_disp, 4, &vcpu_state, &inst), ZX_OK);
  EXPECT_EQ(inst.type, InstructionType::kLogicalOr);
  EXPECT_EQ(inst.access_size, 2u);
  EXPECT_EQ(inst.imm, 0x100u);
  EXPECT_EQ(inst.reg, nullptr);
  ...
  uint8_t orl_sib[] = {0x81, 0b00'001'100, 0b01'001'000, 0x10, 0, 0,
                       0};  // opcode modrm sib imm4 imm3 imm2 imm1
  ASSERT_EQ(inst_decode(orl_sib, 4, &vcpu_state, &inst), ZX_OK);
  EXPECT_EQ(inst.type, InstructionType::kLogicalOr);
  EXPECT_EQ(inst.access_size, 4u);
  EXPECT_EQ(inst.imm, 0x10u);
  EXPECT_EQ(inst.reg, nullptr);
  ...
  uint8_t orl_sib_disp[] = {0x81, 0b01'001'100, 0b00'001'000, 0x04, 0x10, 0, 0,
                            0};  // opcode modrm sib disp imm4 imm3 imm2 imm1
  ASSERT_EQ(inst_decode(orl_sib_disp, 4, &vcpu_state, &inst), ZX_OK);
  EXPECT_EQ(inst.type, InstructionType::kLogicalOr);
  EXPECT_EQ(inst.access_size, 4u);
  EXPECT_EQ(inst.imm, 0x10u);
  EXPECT_EQ(inst.reg, nullptr);
  ...
  uint8_t orl_sib_nobase[] = {0x81, 0b00'001'100, 0b00'100'101, 0xEF, 0xCD, 0xAB, 0x00, 0x10, 0, 0,
                              0};  // opcode modrm sib disp4 disp3 disp2 disp1 imm4 imm3 imm2 imm1
  ASSERT_EQ(inst_decode(orl_sib_nobase, 4, &vcpu_state, &inst), ZX_OK);
  EXPECT_EQ(inst.type, InstructionType::kLogicalOr);
  EXPECT_EQ(inst.access_size, 4u);
  EXPECT_EQ(inst.imm, 0x10u);
  EXPECT_EQ(inst.reg, nullptr);
...
TEST(InstDecodeTest, or_80) {
  ...
  ASSERT_EQ(inst_decode(orb, 4, &vcpu_state, &inst), ZX_OK);
  EXPECT_EQ(inst.type, InstructionType::kLogicalOr);
  EXPECT_EQ(inst.access_size, 1u);
  EXPECT_EQ(inst.imm, 0x1u);
  EXPECT_EQ(inst.reg, nullptr);
  ...
  ASSERT_EQ(inst_decode(orb_disp_1, 4, &vcpu_state, &inst), ZX_OK);
  EXPECT_EQ(inst.type, InstructionType::kLogicalOr);
  EXPECT_EQ(inst.access_size, 1u);
  EXPECT_EQ(inst.imm, 0x1u);
  EXPECT_EQ(inst.reg, nullptr);
