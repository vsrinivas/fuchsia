...
TEST_P(FileSystemTest, SetModificationTime) {
  zx_time_t now = 0;
...
TEST_P(FileSystemTest, WriteSetsModificationTime) {
  const std::string file = GetPath("file.txt");
  int fd1 = open(file.c_str(), O_CREAT | O_RDWR, 0644);
  EXPECT_GT(fd1, 0);
...
TEST_P(FileSystemTest, StatReturnsCorrectBlockSize) {
  const std::string file = GetPath("file.txt");
...
TEST_P(FileSystemTest, ParentModificationTimeUpdatedCorrectly) {
  zx_time_t now = 0;
  ASSERT_EQ(zx_clock_get(ZX_CLOCK_UTC, &now), ZX_OK);
  ASSERT_NE(now, 0u) << "zx_clock_get only returns zero on error";
