...
impl File for FxFile {
    ...
    async fn read_at(&self, offset: u64, buffer: &mut [u8]) -> Result<u64, Status> {
        ...
        let bytes_read = self.handle.read(offset, buf.as_mut()).await.map_err(fxfs_error)?;
    ...
    async fn write_at(&self, offset: u64, content: &[u8]) -> Result<u64, Status> {
        ...
        self.handle.write(offset, buf.as_ref()).await.map_err(fxfs_error)?;
        Ok(content.len() as u64)
    }

    async fn append(&self, content: &[u8]) -> Result<(u64, u64), Status> {
        // TODO this needs to be made atomic. We already lock at the Device::write level but we
        // might need to lift a lock higher.
    ...
    async fn truncate(&self, length: u64) -> Result<(), Status> {
        ...
        self.handle.truncate(&mut transaction, length).await.map_err(fxfs_error)?;
...
mod tests {
    ...
    async fn test_empty_file() -> Result<(), Error> {
        ...
        let filesystem = object_store::FxFilesystem::new_empty(device.clone()).await?;
