...
impl MemDataBuffer {
    ...
    async fn read_some(
        ...
        let amount = source.read(offset, read_buf.as_mut()).await?;
        ...
        for page in pages.range(aligned_range) {
            ...
            if page.read_key == read_key {
                // TODO: add test for end of range page-in
...
impl DataBuffer for MemDataBuffer {
    ...
    async fn read(
...

        {
            ...
            for page in pages.range(offset..offset + read_buf.len() as u64) {
                let page = unsafe { page.page_mut() };

                // Handle any gap between the last page we found and this one.
                let len = page.offset() - offset;
                if len > 0 {
                    // Schedule a read for the gap.
                    let (head, tail) = read_buf.split_at_mut(len as usize);
...
impl<'a> ReadKeys<'a> {
    ...
    fn new_read(
        ...
        for offset in range.clone().step_by(PAGE_SIZE as usize) {
            new_pages
