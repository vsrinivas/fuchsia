...
pub async fn bind_zxcrypt_driver(controller: &ControllerProxy) -> Result<()> {
    controller.bind(ZXCRYPT_DRIVER_PATH).await?.map_err(zx::Status::from_raw)?;
    Ok(())
}
...
pub async fn wait_for_zxcrypt_driver(block_device: &Path) -> Result<PathBuf> {
    ...
    recursive_wait_and_open_node(&device, ZXCRYPT_DEVICE_NAME).await?;
...
pub async fn set_up_insecure_zxcrypt(block_device: &Path) -> Result<PathBuf> {
    ...
    bind_zxcrypt_driver(&controller).await?;

    let zxcrypt_path = wait_for_zxcrypt_driver(block_device).await?;
    let zxcrypt =
    ...
    recursive_wait_and_open_node(&zxcrypt_dir, UNSEALED_BLOCK_PATH).await?;
