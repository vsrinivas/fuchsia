...
TEST(CommonTest, PaddedBlobLayoutFormatIsRoundTrippedThroughTheSuperblock) {
  ...
  Superblock info;
  InitializeSuperblock(kBlockCount, kBlobfsDefaultInodeCount, {.blob_layout_format = format},
                       &info);
...
TEST(CommonTest, InodesRoundedUpToFillBlock) {
  Superblock info;
  InitializeSuperblock(kBlockCount, kBlobfsDefaultInodeCount + kBlobfsInodesPerBlock - 1, {},
                       &info);
...
TEST(CommonTest, TooFewInodesFailsCheck) {
  ...
  InitializeSuperblock(kBlockCount, kBlobfsDefaultInodeCount - kBlobfsInodesPerBlock, {}, &info);
