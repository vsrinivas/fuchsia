...
zx::status<> RebootDevice() {
  using llcpp::fuchsia::hardware::power::statecontrol::Admin;
  using llcpp::fuchsia::hardware::power::statecontrol::RebootReason;
  zx::channel local, remote;

  if (zx_status_t status = zx::channel::create(0, &local, &remote); status != ZX_OK) {
    return zx::error(status);
  }
  std::string path = std::string("/svc/") + Admin::Name;
  if (zx_status_t status = fdio_service_connect(path.c_str(), remote.release()); status != ZX_OK) {
  ...
  fidl::ClientEnd<Admin> client_end(std::move(local));
  fidl::Client client(std::move(client_end), loop.dispatcher());
  client->Reboot(RebootReason::FACTORY_DATA_RESET, [](auto) { /*ignored*/ });
  return zx::ok();
}
