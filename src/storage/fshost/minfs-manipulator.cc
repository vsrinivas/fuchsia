...
zx::status<> ShredZxcrypt(const zx::unowned_channel& device) {
  auto topological_path = llcpp::fuchsia::device::Controller::Call::GetTopologicalPath(device);
  if (topological_path.status() != ZX_OK) {
    return zx::error(topological_path.status());
  }
  if (topological_path->result.is_err()) {
    return zx::error(topological_path->result.err());
  }

  std::filesystem::path path(topological_path->result.response().path.get());
  ...
  fdio_cpp::UnownedFdioCaller zxcrypt_caller(zxcrypt_fd.get());
  auto result = llcpp::fuchsia::hardware::block::encrypted::DeviceManager::Call::Shred(
...
zx::status<llcpp::fuchsia::hardware::block::BlockInfo> GetBlockDeviceInfo(
    const zx::unowned_channel& device) {
  auto result = llcpp::fuchsia::hardware::block::Block::Call::GetInfo(device);
  if (result.status() != ZX_OK) {
    return zx::error(result.status());
  }
  if (result->status != ZX_OK) {
...
MaybeResizeMinfsResult MaybeResizeMinfs(zx::channel device, uint64_t partition_size_limit,
  ...
  if (!previously_failed_while_writing.is_error() && previously_failed_while_writing.value()) {
  ...
  if (zx::status<> status = CopyGracefulRebootReason(minfs.value(), copier.value());
      status.is_error()) {
  ...
  uint64_t required_space_estimate = EstimateMinfsRequiredSpace(copier.value());
  ...
  if (zx::status<> status = MountedMinfs::Unmount(std::move(minfs).value()); status.is_error()) {
  ...
  zx::status<MountedMinfs> formatted_minfs = MountedMinfs::Mount(CloneDeviceChannel(device));
  if (formatted_minfs.is_error()) {
    FX_LOGS(ERROR) << "Failed to mount minfs: " << formatted_minfs.status_string();
    // If minfs was corrupt then fsck should fail on next boot and minfs will be reformated provided
    // format-minfs-on-corruption is set. All files are lost.
    return MaybeResizeMinfsResult::kRebootRequired;
  }
  if (zx::status<> status = formatted_minfs->PopulateFilesystem(std::move(copier).value());
      status.is_error()) {
...
zx::status<llcpp::fuchsia::io::FilesystemInfo> MountedMinfs::GetFilesystemInfo() const {
  auto query_result = llcpp::fuchsia::io::DirectoryAdmin::Call::QueryFilesystem(root_.borrow());
...
zx::status<> MountedMinfs::PopulateFilesystem(Copier copier) const {
  ...
  if (zx_status_t status = copier.Write(std::move(root).value()); status != ZX_OK) {
    return zx::error(status);
  }
  zx::status<fbl::unique_fd> root2 = GetRootFd();
  if (root2.is_error()) {
    return root2.take_error();
  }
  if (syncfs(root2->get()) != 0) {
    return zx::error(ZX_ERR_IO);
  }
...
zx::status<Copier> MountedMinfs::ReadFilesystem(
  ...
  return Copier::Read(std::move(root).value(), excluded_paths);
}
...
zx::status<> MountedMinfs::Unmount() {
  ...
  zx::channel root = std::move(root_);
  auto result = llcpp::fuchsia::io::DirectoryAdmin::Call::Unmount(root.borrow());
  if (result.status() != ZX_OK) {
    return zx::error(result.status());
  }
