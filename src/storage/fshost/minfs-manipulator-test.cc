...
zx::status<std::string> CreateZxcryptVolume(const std::string& device_path) {
  ...
  if (zx_status_t status =
          zxcrypt::FdioVolume::CreateWithDeviceKey(std::move(fd), std::move(dev_fd), &volume);
      status != ZX_OK) {
    FX_LOGS(ERROR) << "Could not create test zxcrypt volume on " << device_path;
    return zx::error(status);
  }

  zx::channel zxcrypt_manager_channel;
  if (zx_status_t status =
          volume->OpenManager(zx::sec(2), zxcrypt_manager_channel.reset_and_get_address());
      status != ZX_OK) {
    FX_LOGS(ERROR) << "Failed to open zxcrypt volume manager: " << status;
    return zx::error(status);
  }

  zxcrypt::FdioVolumeManager zxcrypt_manager(std::move(zxcrypt_manager_channel));
  if (zx_status_t status = zxcrypt_manager.UnsealWithDeviceKey(0); status != ZX_OK) {
  ...
  if (zx_status_t status = wait_for_device(zxcrypt_device_path.c_str(), zx::sec(2).get());
      status != ZX_OK) {
...
class MinfsManipulatorTest : public testing::Test {
 public:
  ...
  void SetUp() override {
    zx::status<isolated_devmgr::RamDisk> ram_disk =
        isolated_devmgr::RamDisk::Create(kBlockSize, kBlockCount);
    ASSERT_OK(ram_disk.status_value());

    zx::status<std::string> fvm_partition_path =
        isolated_devmgr::CreateFvmPartition(ram_disk->path(), kFvmSliceSize);
    ASSERT_OK(fvm_partition_path.status_value());

    zx::status<std::string> zxcrypt_device_path = CreateZxcryptVolume(fvm_partition_path.value());
    ...
    ram_disk_ = std::move(ram_disk).value();
    device_ = std::move(device);
    fvm_partition_path_ = std::move(fvm_partition_path).value();
    zxcrypt_device_path_ = std::move(zxcrypt_device_path).value();
  ...
  zx::status<> SetPartitionLimit(uint64_t byte_count) {
    auto guid_result = llcpp::fuchsia::hardware::block::partition::Partition::Call::GetInstanceGuid(
        device_.borrow());
    ...
    auto result = llcpp::fuchsia::hardware::block::volume::VolumeManager::Call::SetPartitionLimit(
        caller.channel(), *guid_result->guid, byte_count);
  ...
  zx::status<> MinfsFsck() {
    ...
    return zx::make_status(
        fsck(zxcrypt_device_path_.c_str(), DISK_FORMAT_MINFS, &options, launch_stdio_sync));
  }
...
TEST_F(MinfsManipulatorTest, MaybeResizeMinfsWithAcceptableSizeDoesNothing) {
  ...
  ASSERT_GT(filled_size.value(), initialize_size.value());
  ...
  EXPECT_EQ(final_size.value(), filled_size.value());
...
TEST_F(MinfsManipulatorTest, MaybeResizeMinfsWithTooManySlicesResizes) {
  ...
  MaybeResizeMinfsResult result =
      MaybeResizeMinfs(device(), initialize_size.value(), kMinfsDefaultInodeCount,
                       kMinfsDataSizeLimit, kNoExcludedPaths, manager());
  ASSERT_EQ(result, MaybeResizeMinfsResult::kMinfsMountable);
  ...
  zx::status<uint64_t> final_size = GetBlockDeviceSize();
  ASSERT_OK(final_size.status_value());
  EXPECT_EQ(final_size.value(), initialize_size.value());
...
TEST_F(MinfsManipulatorTest, MaybeResizeMinfsResizeInProgressIsCorrectlyDetected) {
  ...
  }
  {
    ...
    auto is_resize_in_progress = minfs->IsResizeInProgress();
    ASSERT_OK(is_resize_in_progress.status_value());
    EXPECT_TRUE(is_resize_in_progress.value());
...
TEST_F(MinfsManipulatorTest, MaybeResizeMinfsRebootReasonIsPreserved) {
  ...
  constexpr char kOtherCacheFilePath[] = "cache/cache-file";
  {
    ...
    ASSERT_OK(root.status_value());
    ASSERT_TRUE(
        CreateFileAndParentDirectories(root.value(), kGracefulRebootReasonFilePath, kRebootReason));
...
TEST_F(MinfsManipulatorTest, MaybeResizeMinfsRebootReasonAlreadyCopied) {
  ...
  constexpr char kExcludedFilePath[] = "exclude/file";
  {
    ...
    ASSERT_TRUE(
        CreateFileAndParentDirectories(root.value(), kGracefulRebootReasonFilePath, kRebootReason));
    ASSERT_TRUE(CreateFileAndParentDirectories(root.value(), kExcludedFilePath, "contents"));
  }
