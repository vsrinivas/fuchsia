...
/// Runs odu with the provided arguments. The arguments *should* exclude the binary path which is
/// usually the first argument when running a binary.
pub fn run<T: Clone + Into<OsString>>(
    args: impl std::iter::Iterator<Item = T>,
) -> Result<(), Error> {
...
impl std::iter::Iterator for ArgsWrapper {
    type Item = String;
    fn next(&mut self) -> Option<Self::Item> {
        unsafe {
            if *self.ptr == std::ptr::null() {
                ...
                let result = CStr::from_ptr(*self.ptr).to_string_lossy().into_owned();
...
/// Provides a C/C++ interface to run odu.  The arguments should be a null terminated list of C
/// strings.
#[no_mangle]
pub extern "C" fn run_odu_test(args: *const *const c_char) -> bool {
    if let Err(e) = run(ArgsWrapper { ptr: args }) {
        eprintln!("run_odu_test failed: {:?}", e);
        false
    } else {
        true
    }
