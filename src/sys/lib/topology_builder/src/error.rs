...
#[derive(Debug, Error)]
pub enum Error {
    #[error("error encountered while loading topology")]
    Builder(#[from] BuilderError),

    #[error("error encountered while assembling topology")]
    Topology(#[from] TopologyError),
}

#[derive(Debug, Error)]
pub enum BuilderError {
    #[error("can't add {} to the topology because {} already exists, and added components must be leaf nodes in the generated topology", _0, _1)]
    AddedComponentsMustBeLeafNodes(Moniker, Moniker),

    #[error("can't add {} to the topology as a component with that name already exists", _0)]
...
pub enum TopologyError {
    ...
    #[error("can't add {} to the topology as something with that name already exists", _0)]
    ComponentAlreadyExists(Moniker),
