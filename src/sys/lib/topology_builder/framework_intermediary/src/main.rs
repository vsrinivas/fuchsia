...
async fn main() -> Result<(), Error> {
    syslog::init_with_tags(&["topology_builder_framework_intermediary"])?;
    info!("started");
...
async fn handle_framework_intermediary_stream(
    mut stream: ftopologybuilder::FrameworkIntermediaryRequestStream,
    registry: Arc<resolver::Registry>,
    runner: Arc<runner::MocksRunner>,
) -> Result<(), Error> {
    while let Some(req) = stream.try_next().await? {
        match req {
            ftopologybuilder::FrameworkIntermediaryRequest::RegisterDecl { decl, responder } => {
