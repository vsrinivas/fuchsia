            ...
            let lazy_node_fn = move || {
                ...
                async move {
                    ...
                    if let Some(this_mutex) = weak_clone.upgrade() {
                        ...
                        for (idx, cmp) in this.running_components.iter().enumerate() {
                        ...
                        for (idx, cmp) in this.destroyed_before_start.iter().enumerate() {
                            destroyed_before_start.set(idx, format!("{}", cmp))
                        }
                        root.record(destroyed_before_start);
                        let seen_realms =
                            root.create_string_array("seen_realms", this.seen_realms.len());
                        for (idx, cmp) in this.seen_realms.iter().enumerate() {
