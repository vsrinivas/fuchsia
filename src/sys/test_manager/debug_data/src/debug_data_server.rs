...
use crate::message::{PublisherRequest, PublisherRequestMessage, VmoMessage};
use anyhow::Error;
...
async fn serve_publisher(
    ...
    match request {
        PublisherRequest::Publisher(r) => {
            let request_stream = r.into_stream()?;
            request_stream
...
mod test {
    ...
    fn create_proxy_and_message(
        ...
        (
            ...
            PublisherRequestMessage {
                ...
                request: PublisherRequest::Publisher(request),
            },
        )
    }
    ...
    mod deprecated_debug_data {
        use super::*;

        fn create_proxy_and_message(
            test_url: &str,
        ) -> (fdebug::DebugDataProxy, PublisherRequestMessage) {
            let (proxy, request) = create_proxy::<fdebug::DebugDataMarker>().unwrap();
            (
                proxy,
                PublisherRequestMessage {
                    test_url: test_url.to_string(),
                    request: PublisherRequest::DebugData(request),
                },
            )
        }
