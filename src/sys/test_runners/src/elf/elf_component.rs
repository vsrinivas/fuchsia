                ...
                while let Some(req) = stream.try_next().await? {
                    ...
                    match req {
                        ...
                        LoaderRequest::LoadObject { object_name, responder } => {
                            ...
                            current_response_map.lock().await.insert(object_name, (rv, vmo_clone));
                            responder.send(rv, vmo)?;
                        }
...
fn duplicate_vmo(vmo: &Option<zx::Vmo>) -> Result<Option<zx::Vmo>, anyhow::Error> {
    Ok(match &vmo {
        Some(vmo) => {
            vmo.duplicate_handle(zx::Rights::SAME_RIGHTS).context("Cannot clone vmo")?.into()
        }
        None => None,
    })
}
