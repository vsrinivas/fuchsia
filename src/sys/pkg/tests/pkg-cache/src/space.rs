...
/// static packages.
#[fasync::run_singlethreaded(test)]
async fn gc_noop_system_image_pkgfs() {
    ...
    let proxy = env.pkgfs.root_dir_proxy().expect("to get a handle to pkgfs");
    let client =
        pkgfs::control::Client::open_from_pkgfs_root(&proxy).expect("to get a control client");
    client.gc().await.expect("to perform a gc without error");
...
/// static packages.
#[fasync::run_singlethreaded(test)]
async fn gc_noop_system_image_pkg_cache() {
    let static_package = PackageBuilder::new("static-package")
        .add_resource_at("resource", &[][..])
        .build()
        .await
...
#[fasync::run_singlethreaded(test)]
async fn gc_dynamic_index_protected_pkgfs() {
    let (env, sysimg_pkg) = setup_test_env(None, &[]).await;

    let pkg = PackageBuilder::new("gc_dynamic_index_protected_pkgfs")
    ...
    let pkgprime = PackageBuilder::new("gc_dynamic_index_protected_pkgfs")
        ...
        .await
        .unwrap();
    // pkgfs would let us call do_fetch here, but pkg-cache differs from pkgfs in that the
    // NeededBlobs protocol can be "canceled". This means that if the channel is closed before the
    // protocol is completed, the blobs mentioned in the meta far are no longer protected by the
    // dynamic index. That's WAI, but complicating that interface further isn't worth it.
    //
    ...
    // Fetch the remaining blobs, and run a GC to kick out the old ones.
    let missing_blobs = get_missing_blobs(&needed_blobs).await;
    for mut blob in missing_blobs {
        let buf = contents.remove(&blob.blob_id.into()).unwrap();
...
/// packages' blobs survive until the new package is entirely written.
#[fasync::run_singlethreaded(test)]
async fn gc_dynamic_index_protected_pkg_cache() {
    let (env, sysimg_pkg) = setup_test_env(None, &[]).await;
...
#[fasync::run_singlethreaded(test)]
async fn gc_random_blobs_pkgfs() {
    let static_package = PackageBuilder::new("static-package")
        .add_resource_at("resource", &[][..])
        .build()
        .await
...
/// Test that a blobfs with blobs not belonging to a known package will lose those blobs on GC.
#[fasync::run_singlethreaded(test)]
async fn gc_random_blobs_pkg_cache() {
    let static_package = PackageBuilder::new("static-package")
        .add_resource_at("resource", &[][..])
        .build()
        .await
...
/// Effectively the same as gc_dynamic_index_protected_pkgfs, except that the updated package also
/// existed as a static package as well.
#[fasync::run_singlethreaded(test)]
async fn gc_updated_static_package_pkgfs() {
    let static_package = PackageBuilder::new("gc_updated_static_package_pkgfs")
    ...
    let pkg = PackageBuilder::new("gc_updated_static_package_pkgfs")
        .add_resource_at("bin/x", "bin-x-version-1".as_bytes())
        .add_resource_at("data/unchanging", "unchanging-content".as_bytes())
        .build()
        .await
    ...
    let pkgprime = PackageBuilder::new("gc_updated_static_package_pkgfs")
        .add_resource_at("bin/x", "bin-x-version-2".as_bytes())
        .add_resource_at("data/unchanging", "unchanging-content".as_bytes())
        .build()
        .await
...
/// existed as a static package as well.
#[fasync::run_singlethreaded(test)]
async fn gc_updated_static_package_pkg_cache() {
