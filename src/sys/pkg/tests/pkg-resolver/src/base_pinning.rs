...
/// This module tests pkg_resolver's behavior when resolving base packages.
use {
...
async fn test_base_package_found() {
    ...
    let static_pkg = test_package(pkg_name, "static").await;
    ...
    let system_image_package =
        SystemImageBuilder::new().static_packages(&[&static_pkg]).build().await;
    let pkgfs = pkgfs_with_system_image_and_pkg(&system_image_package, Some(&static_pkg)).await;
    ...
    static_pkg.verify_contents(&package_dir).await.unwrap();
    ...
    assert_eq!(hash.unwrap(), static_pkg.meta_far_merkle_root().clone().into());
...
async fn test_base_package_with_merkle_pin() {
    ...
    assert!(static_pkg.verify_contents(&package_dir).await.is_err());
