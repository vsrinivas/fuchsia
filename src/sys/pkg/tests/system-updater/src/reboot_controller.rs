...
async fn reboot_controller_detach_causes_deferred_reboot_v1() {
...
#[fasync::run_singlethreaded(test)]
async fn reboot_controller_unblock_causes_reboot_v1() {
    let env = TestEnv::builder().build().await;
...
#[fasync::run_singlethreaded(test)]
async fn reboot_controller_dropped_causes_reboot_v1() {
    let env = TestEnv::builder().build().await;
