...
pub async fn publish_app_id_to_feedback(app_set: AppSet) {
    let proxy =
    ...
    publish_app_id_to_feedback_impl(proxy, app_set).await;
}

async fn publish_app_id_to_feedback_impl(proxy: ComponentDataRegisterProxy, app_set: AppSet) {
    let component_data = ComponentData {
        namespace: Some("omaha".to_string()),
        annotations: Some(vec![Annotation {
            key: "app-id".to_string(),
            value: app_set.get_current_app_id().await,
        }]),
        ..ComponentData::EMPTY
    };
    ...
    match proxy.upsert(component_data).await {
        Ok(()) => {}
        Err(e) => {
            error!("Could not set app id in snapshot metadata: {}", e);
        }
    }
}
...
mod tests {
    ...
    async fn test_publish_app_id_to_feedback() {
        let app_set = AppSet::new(vec![App::builder("some-app-id", [1, 2]).build()]);
        ...
        future::join(publish_app_id_to_feedback_impl(proxy, app_set), stream_fut).await;
    }
}
