...
async fn query_commit_status(
    ...
    let event_pair = provider.is_current_system_committed().await.unwrap_or_else(|e| {
        ...
        // system-update-checker is a critical component). Ideally, we'd implement this in the
        // component serving CommitStatusProvider. However, since that component is v2, and v2
        // components do not have watchdog support, for now we'll make a "DIY watchdog" here to
        // ensure the system reboots. We can remove this once there's a v2 equivalent to critical
        // components.
        //
