...
impl Client {
    ...
    pub async fn gc(&self) -> Result<(), GcError> {
        self.proxy
            .unlink("do-not-use-this-garbage", fio::UnlinkOptions::EMPTY)
            .await
...
mod tests {
    use {
        ...
        fuchsia_async as fasync, futures_util::stream::TryStreamExt,
    };

    #[fasync::run_singlethreaded(test)]
    async fn gc() {
        let (proxy, mut stream) = create_proxy_and_stream::<fio::DirectoryMarker>().unwrap();
        fasync::Task::spawn(async move {
            match stream.try_next().await.unwrap().unwrap() {
                fio::DirectoryRequest::Unlink { name, options: _, responder } => {
