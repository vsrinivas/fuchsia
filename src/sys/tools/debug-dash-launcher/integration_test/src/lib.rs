...
pub async fn spawn_dash_with_tools_package() {
    ...
    launcher
        .launch_with_socket(".", stdio_server, Some("fuchsia-pkg://fuchsia.com/foo"))
        .await
        .unwrap()
        .unwrap();
...
pub async fn spawn_dash_self() {
    ...
    let launcher = connect_to_protocol::<fdash::LauncherMarker>().unwrap();
    launcher.launch_with_socket(".", stdio_server, None).await.unwrap().unwrap();
...
pub async fn unknown_tools_package() {
    ...
    let launcher = connect_to_protocol::<fdash::LauncherMarker>().unwrap();
    let err = launcher
        .launch_with_socket(".", stdio_server, Some("fuchsia-pkg://fuchsia.com/bar"))
        .await
        .unwrap()
        .unwrap_err();
...
pub async fn bad_moniker() {
    ...
    let err =
        launcher.launch_with_socket("!@#$%^&*(", stdio_server, None).await.unwrap().unwrap_err();
...
pub async fn instance_not_found() {
    ...
    let err = launcher
        .launch_with_socket("./does_not_exist", stdio_server, None)
        .await
        .unwrap()
        .unwrap_err();
