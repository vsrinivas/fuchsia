...
FakeAsyncCorpusReader::FakeAsyncCorpusReader(ExecutorPtr executor)
    : binding_(this), executor_(executor) {}

void FakeAsyncCorpusReader::Bind(fidl::InterfaceRequest<CorpusReader> request) {
  auto status = binding_.Bind(std::move(request), executor_->dispatcher());
  FX_DCHECK(status == ZX_OK) << zx_status_get_string(status);
}

fidl::InterfaceHandle<CorpusReader> FakeAsyncCorpusReader::NewBinding() {
  fidl::InterfaceHandle<CorpusReader> client;
  Bind(client.NewRequest());
  return client;
}

void FakeAsyncCorpusReader::Next(FidlInput fidl_input, NextCallback callback) {
  if (error_after_-- == 0) {
