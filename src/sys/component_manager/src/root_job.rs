...
impl RootJob {
    pub fn new(capability_path: CapabilityPath, rights: zx::Rights) -> Self {
        Self { capability_path, rights }
    }
...
mod tests {
    ...
    fn create_root_job(rights: zx::Rights) -> Arc<RootJob> {
        Arc::new(RootJob::new(CAPABILITY_PATH.clone(), rights))
    }

    #[fasync::run_singlethreaded(test)]
    async fn has_correct_rights() -> Result<(), Error> {
        let root_job = create_root_job(zx::Rights::TRANSFER);
        ...
        let info = zx::Handle::from(resource).basic_info()?;
    ...
    async fn can_connect() -> Result<(), Error> {
        let root_job = create_root_job(zx::Rights::SAME_RIGHTS);
