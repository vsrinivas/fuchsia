...
async fn main() {
    ...
    expect_dir_listing("/hub/children/child_a", vec!["children", "resolved"]).await;
    resolve_component("./child_b", true).await;
    expect_dir_listing("/hub/children/child_b", vec!["children", "resolved"]).await;
    start_component("./child_b", true).await;
    expect_dir_listing("/hub/children/child_b", vec!["children", "exec", "resolved"]).await;
    start_component("./child_a", true).await;
    expect_dir_listing("/hub/children/child_a", vec!["children", "exec", "resolved"]).await;
