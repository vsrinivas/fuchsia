...
async fn verify_ambient_vmex_denied() -> Result<(), Error> {
    ...
    EventSequence::new().has_subset(vec![EventMatcher::ok().r#type(Started::TYPE).moniker(moniker.clone()), EventMatcher::ok().r#type(Stopped::TYPE).moniker(moniker.clone())], Ordering::Unordered).expect(event_stream).await.unwrap();
