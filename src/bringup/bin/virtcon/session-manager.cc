...
void SessionManager::CreateSession(::zx::channel session, CreateSessionCompleter::Sync& completer) {
  completer.Reply(
      CreateSession(std::move(session), !keep_log_visible_, color_scheme_).status_value());
}

zx::status<vc_t*> SessionManager::CreateSessionForTest(zx::channel session) {
  return CreateSession(std::move(session), !keep_log_visible_, color_scheme_);
}
