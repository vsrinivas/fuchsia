...
TEST(VirtconSessionManager, LifeTimeTest) {
  ...
  SessionManager sessions(loop.dispatcher(), false, &color_schemes[kDefaultColorScheme]);
  zx::channel local, remote;
  ASSERT_EQ(ZX_OK, zx::channel::create(0, &local, &remote));
  ASSERT_EQ(ZX_OK, sessions.CreateSessionForTest(std::move(remote)).status_value());
}
...
TEST(VirtconSessionManager, TestWriting) {
  ...
  {
    auto response = sessions.CreateSessionForTest(std::move(remote));
