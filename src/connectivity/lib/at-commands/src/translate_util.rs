...
pub fn extract_vec_from_args(arguments: &Arguments) -> Result<&Vec<Argument>, DeserializeError> {
    if let Arguments::ArgumentList(arg_vec) = arguments {
        ...
        Err(DeserializeError::UnknownArguments(arguments.clone()))
    }
}

pub fn extract_vec_vec_from_args(
    arguments: &Arguments,
) -> Result<&Vec<Vec<Argument>>, DeserializeError> {
...
) -> Result<&'a PrimitiveArgument, DeserializeError> {
    if let Argument::PrimitiveArgument(arg) = field {
        Ok(arg)
    } else {
        Err(DeserializeError::UnknownArguments(args_for_error_reporting.clone()))
    }
}
