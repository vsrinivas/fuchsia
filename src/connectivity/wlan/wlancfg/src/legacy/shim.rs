...
mod tests {
    ...
    impl IfaceManagerApi for FakeIfaceManager {
        ...
        async fn get_sme_proxy_for_scan(
            &mut self,
        ) -> Result<fidl_fuchsia_wlan_sme::ClientSmeProxy, anyhow::Error> {
            unimplemented!()
        }
    ...
    fn connect_succeeds() {
        ...
        assert_variant!(
            ...
            Poll::Ready(Ok(legacy::Error { code: legacy::ErrCode::Ok, .. }))
        );
    }
    ...
    fn connect_timeout() {
        ...
        assert_variant!(
            exec.run_until_stalled(&mut connect_fut),
            Poll::Ready(Ok(legacy::Error { code: legacy::ErrCode::Internal, .. }))
        );
    }
