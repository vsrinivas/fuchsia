...
template <typename T>
zx_status_t DecodeFidl(const void* data, size_t size, T* out) {
  std::vector<uint8_t> decoded(size);
  memcpy(&decoded[0], data, size);
  const char* err_msg = nullptr;
  fidl::Message msg(fidl::BytePart(decoded.data(), size, size),
                    fidl::HandlePart());
...
struct WlantapCtl {
  ...
  static zx_status_t DdkIoctl(void* ctx, uint32_t op, const void* in_buf,
    ...
    switch (op) {
      case IOCTL_WLANTAP_CREATE_WLANPHY:
        ...
        return self.IoctlCreateWlanphy(in_buf, in_len, out_buf, out_len,
                                       out_actual);
      default:
  ...
  zx_status_t IoctlCreateWlanphy(const void* in_buf, size_t in_len,
                                 void* out_buf, size_t out_len,
                                 size_t* out_actual) {
    if (in_buf == nullptr || in_len < sizeof(wlantap_ioctl_create_wlanphy_t)) {
      zxlogf(
          ERROR,
          "wlantapctl: IOCTL_WLANTAP_CREATE_WLANPHY: invalid input buffer\n");
      return ZX_ERR_INVALID_ARGS;
    }
    ...
    zx_status_t status =
        DecodeFidl(&in.config[0], in_end - &in.config[0], phy_config.get());
    if (status != ZX_OK) {
      zxlogf(ERROR,
             "wlantapctl: IOCTL_WLANTAP_CREATE_WLANPHY: failed to parse input "
             "buffer as FIDL\n");
      return status;
    }
    ...
    status = wlan::CreatePhy(device_, std::move(user_channel),
