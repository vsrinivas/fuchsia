...
// Port keys
//
// When waiting on a port, the key will have both a type and an id. The type is
// used for routing the packet to the correct handler. The id may be used by the
// handler to further route the packet within a subsystem (e.g., Mlme).
...
static inline uint64_t ToPortKeyId(uint64_t key) { return key >> 8; }
...
#endif  // SRC_CONNECTIVITY_WLAN_LIB_MLME_CPP_INCLUDE_WLAN_MLME_WLAN_H_
