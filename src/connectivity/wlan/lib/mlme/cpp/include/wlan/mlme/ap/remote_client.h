...
class DeauthenticatedState : public BaseState {
 public:
  enum class MoveReason {
    ...
    INIT,
    // RemoteClient moved to DeauthenticatedState due to a deauthenticate
    // request, whether from
    // the user or due to the AP deciding that the client is inactive
    EXPLICIT_DEAUTH,
    // RemoteClient moved to DeauthenticatedState due to a failed authentication
    // attempt
    FAILED_AUTH,
    // RemoteClient received an authentication frame while already authenticated
    // or associated,
...
#endif  // SRC_CONNECTIVITY_WLAN_LIB_MLME_CPP_INCLUDE_WLAN_MLME_AP_REMOTE_CLIENT_H_
