...
void DeauthenticatedState::HandleAnyMgmtFrame(MgmtFrame<>&& frame) {
  if (auto auth_frame =
    ...
    if (auth_alg != AuthAlgorithm::kOpenSystem) {
      errorf(
          "[client] [%s] received auth attempt with unsupported algorithm: "
          "%u\n",
...
void AuthenticatingState::OnExit() { client_->CancelTimer(auth_timeout_); }
...
void AuthenticatedState::OnExit() { client_->CancelTimer(auth_timeout_); }
...
zx_status_t AssociatingState::HandleMlmeMsg(const BaseMlmeMsg& msg) {
  if (auto assoc_resp = msg.As<wlan_mlme::AssociateResponse>()) {
    ...
    if (st_code == WLAN_STATUS_CODE_SUCCESS) {
...
void AssociatedState::HandleAssociationRequest(
  ...
  debugbss("[client] [%s] received Assocation Request while being associated\n",
           client_->addr().ToString().c_str());
  // Client believes it is not yet associated. Move it back to authenticated
  // state and then have it process the frame.
  MoveToState<AuthenticatedState>();
...
void AssociatedState::OnEnter() {
  ...
  // TODO(NET-833): Establish BlockAck session conditionally on the client
  // capability and the AP configurations
...
void AssociatedState::UpdatePowerSaveMode(const FrameControl& fc) {
  if (eapol_controlled_port_ == eapol::PortState::kBlocked) {
    return;
  }
...
bool AssociatedState::HasBufferedFrames() const { return bu_queue_.size() > 0; }
...
void RemoteClient::CancelTimer(TimeoutId id) { return bss_->CancelTimeout(id); }
...
void RemoteClient::ReportBuChange(aid_t aid, size_t bu_count) {
  if (listener_ != nullptr) {
    listener_->HandleClientBuChange(addr_, aid, bu_count);
  }
}

void RemoteClient::ReportFailedAuth() {
  if (listener_ != nullptr) {
    listener_->HandleClientFailedAuth(addr_);
  }
}

void RemoteClient::ReportDeauthentication() {
  if (listener_ != nullptr) {
    listener_->HandleClientDeauth(addr_);
  }
}

void RemoteClient::ReportDisassociation(aid_t aid) {
  if (listener_ != nullptr) {
    listener_->HandleClientDisassociation(aid);
  }
}
...
wlan_assoc_ctx_t RemoteClient::BuildAssocContext(uint16_t aid) {
  ...
  // If the client supports either HT or VHT, tell the driver to send out with
  // QoS header (if the driver/firmware supports it).
