...
zx_status_t ClientMlme::Init() {
  ...
  auto rust_device = rust_device_interface_t{
      ...
      .start_passive_scan =
      [](void* device, const wlanmac_passive_scan_args_t* passive_scan_args, uint64_t* out_scan_id) -> zx_status_t {
        return DEVICE(device)->StartPassiveScan(passive_scan_args, out_scan_id);
      },
      .start_active_scan =
      [](void* device, const wlanmac_active_scan_args_t* active_scan_args, uint64_t* out_scan_id) -> zx_status_t {
