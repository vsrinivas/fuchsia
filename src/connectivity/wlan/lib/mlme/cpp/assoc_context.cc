...
std::optional<std::vector<SupportedRate>> BuildAssocReqSuppRates(
    const std::vector<uint8_t>& ap_raw_rates, const std::vector<SupportedRate>& client_rates) {
  std::vector<SupportedRate> ap_rates(ap_raw_rates.size());
  std::transform(ap_raw_rates.cbegin(), ap_raw_rates.cend(), ap_rates.begin(),
                 [](uint8_t r) { return SupportedRate(r); });

  auto rates = IntersectRatesAp(ap_rates, client_rates);

  std::set<SupportedRate> ap_rate_set(ap_rates.cbegin(), ap_rates.cend());
  size_t num_ap_basic_rates =
      std::count_if(ap_rate_set.cbegin(), ap_rate_set.cend(), [](auto& r) { return r.is_basic(); });
  size_t num_basic_rates =
      std::count_if(rates.cbegin(), rates.cend(), [](auto& r) { return r.is_basic(); });

  if (num_basic_rates != num_ap_basic_rates) {
...
wlan_assoc_ctx_t AssocContext::ToDdk() const {
  ...
  if (ht_cap.has_value()) {
    ddk.ht_cap = ht_cap->ToDdk();
  }

  ddk.has_ht_op = ht_op.has_value();
  if (ht_op.has_value()) {
    ddk.ht_op = ht_op->ToDdk();
  }

  ddk.has_vht_cap = vht_cap.has_value();
  if (vht_cap.has_value()) {
    ddk.vht_cap = vht_cap->ToDdk();
  }

  ddk.has_vht_op = vht_op.has_value();
  if (vht_op.has_value()) {
    ddk.vht_op = vht_op->ToDdk();
  }
