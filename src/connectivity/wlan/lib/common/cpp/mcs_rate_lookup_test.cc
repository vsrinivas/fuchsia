...
const auto kValidHtCbw = wlan_common::CBW::CBW20;
const auto kInvalidHtCbw = wlan_common::CBW::CBW160;

// VHT-specific parameters used in tests below.
const auto kValidVhtCbw = wlan_common::CBW::CBW80;
...
TEST(McsRateLookup, HtLookupSucceedsFor20MhzCbw) {
  ...
  ASSERT_EQ(HtDataRateLookup(wlan_common::CBW::CBW20, mcs, wlan_common::GuardInterval::LONG_GI,
                             &out_kbps),
            ZX_OK);
  EXPECT_EQ(out_kbps, expected_long_gi_kbps);

  const uint32_t expected_short_gi_kbps = 288900;
  ASSERT_EQ(HtDataRateLookup(wlan_common::CBW::CBW20, mcs, wlan_common::GuardInterval::SHORT_GI,
                             &out_kbps),
            ZX_OK);
...
TEST(McsRateLookup, HtLookupSucceedsFor40MhzCbw) {
  ...
  ASSERT_EQ(HtDataRateLookup(wlan_common::CBW::CBW40, mcs, wlan_common::GuardInterval::LONG_GI,
                             &out_kbps),
            ZX_OK);
  EXPECT_EQ(out_kbps, expected_long_gi_kbps);

  const uint32_t expected_short_gi_kbps = 90000;
  ASSERT_EQ(HtDataRateLookup(wlan_common::CBW::CBW40, mcs, wlan_common::GuardInterval::SHORT_GI,
                             &out_kbps),
            ZX_OK);
  EXPECT_EQ(out_kbps, expected_short_gi_kbps);
}
...
TEST(McsRateLookup, HtLookupFailsWithSpecialCaseInvalidMcs) {
  ...
  EXPECT_EQ(HtDataRateLookup(wlan_common::CBW::CBW20, 32, kValidGi, &out_kbps),
            ZX_ERR_OUT_OF_RANGE);
}
...
TEST(McsRateLookup, VhtLookupSucceedsFor20MhzCbw) {
  ...
  ASSERT_EQ(VhtDataRateLookup(wlan_common::CBW::CBW20, mcs, wlan_common::GuardInterval::LONG_GI,
                              nss, &out_kbps),
            ZX_OK);
  EXPECT_EQ(out_kbps, expected_long_gi_kbps);

  const uint32_t expected_short_gi_kbps = 7200;
  ASSERT_EQ(VhtDataRateLookup(wlan_common::CBW::CBW20, mcs, wlan_common::GuardInterval::SHORT_GI,
                              nss, &out_kbps),
            ZX_OK);
  EXPECT_EQ(out_kbps, expected_short_gi_kbps);
}
...
TEST(McsRateLookup, VhtLookupSucceedsFor40MhzCbw) {
  ...
  ASSERT_EQ(VhtDataRateLookup(wlan_common::CBW::CBW40, mcs, wlan_common::GuardInterval::LONG_GI,
                              nss, &out_kbps),
            ZX_OK);
  EXPECT_EQ(out_kbps, expected_long_gi_kbps);

  const uint32_t expected_short_gi_kbps = 240000;
  ASSERT_EQ(VhtDataRateLookup(wlan_common::CBW::CBW40, mcs, wlan_common::GuardInterval::SHORT_GI,
                              nss, &out_kbps),
            ZX_OK);
  EXPECT_EQ(out_kbps, expected_short_gi_kbps);
}
...
TEST(McsRateLookup, VhtLookupSucceedsFor80MhzCbw) {
  ...
  ASSERT_EQ(VhtDataRateLookup(wlan_common::CBW::CBW80, mcs, wlan_common::GuardInterval::LONG_GI,
                              nss, &out_kbps),
            ZX_OK);
  EXPECT_EQ(out_kbps, expected_long_gi_kbps);

  const uint32_t expected_short_gi_kbps = 2340000;
  ASSERT_EQ(VhtDataRateLookup(wlan_common::CBW::CBW80, mcs, wlan_common::GuardInterval::SHORT_GI,
                              nss, &out_kbps),
            ZX_OK);
  EXPECT_EQ(out_kbps, expected_short_gi_kbps);
}
...
TEST(McsRateLookup, VhtLookupSucceedsFor160MhzCbw) {
  ...
  ASSERT_EQ(VhtDataRateLookup(wlan_common::CBW::CBW160, mcs, wlan_common::GuardInterval::LONG_GI,
                              nss, &out_kbps),
            ZX_OK);
  EXPECT_EQ(out_kbps, expected_long_gi_kbps);

  const uint32_t expected_short_gi_kbps = 1560000;
  ASSERT_EQ(VhtDataRateLookup(wlan_common::CBW::CBW160, mcs, wlan_common::GuardInterval::SHORT_GI,
                              nss, &out_kbps),
            ZX_OK);
  EXPECT_EQ(out_kbps, expected_short_gi_kbps);
}
...
TEST(McsRateLookup, VhtLookupFailsWithSpecialCaseInvalidMcs) {
  ...
  using invalid_param_t = std::tuple<wlan_common::CBW, uint8_t, uint8_t>;
  const std::array<invalid_param_t, invalid_count> invalidVhtParams = {
      std::make_tuple(wlan_common::CBW::CBW20, 1, 9),
      std::make_tuple(wlan_common::CBW::CBW20, 2, 9),
      std::make_tuple(wlan_common::CBW::CBW20, 4, 9),
      std::make_tuple(wlan_common::CBW::CBW20, 5, 9),
      std::make_tuple(wlan_common::CBW::CBW20, 7, 9),
      std::make_tuple(wlan_common::CBW::CBW20, 8, 9),
      std::make_tuple(wlan_common::CBW::CBW80, 6, 9),
      std::make_tuple(wlan_common::CBW::CBW80, 32, 1),
      std::make_tuple(wlan_common::CBW::CBW160, 3, 9),
  };
