...
dB operator+(const dB& lhs, const dB& rhs) { return dB(lhs.val + rhs.val); }

dB operator-(const dB& lhs, const dB& rhs) { return dB(lhs.val - rhs.val); }

dB operator-(const dB& rhs) { return dB(-rhs.val); }

dBh::dBh(int16_t v) : EnergyType<int16_t, dBh>(v) {}

dBh operator+(const dBh& lhs, const dBh& rhs) { return dBh(lhs.val + rhs.val); }

dBh operator-(const dBh& lhs, const dBh& rhs) { return dBh(lhs.val - rhs.val); }

dBh operator-(const dBh& rhs) { return dBh(-rhs.val); }

dBm operator+(const dBm& lhs, const dB& rhs) { return dBm(lhs.val + rhs.val); }
...
static dBm add_dBm(const dBm& lhs, const dBm& rhs) {
  ...
  // Since dBm is an integral type, it can be quantized to the integer
  // precision.
...
dBm operator+(const dBm& lhs, const dBm& rhs) { return add_dBm(lhs, rhs); }
...
dBh to_dBh(dB u) { return dBh(u.val * 2); }

dBm to_dBm(dBmh u) { return dBm(u.val / 2); }

dBmh to_dBmh(dBm u) { return dBmh(u.val * 2); }
