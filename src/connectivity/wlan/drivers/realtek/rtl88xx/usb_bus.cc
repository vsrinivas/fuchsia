...
zx_status_t UsbBus::Create(usb_protocol_t* usb_protocol,
  ...
  reg::SYS_CFG2 cfg2;
  if ((status = bus_interface->ReadRegister(&cfg2)) != ZX_OK) {
    return status;
  }
  if (cfg2.u3_term_detect()) {
    ...
    if ((status = bus_interface->ReadRegister(&usb_usbstat)) != ZX_OK) {
      return status;
    }
  ...
  if ((status = bus_interface->WriteRegister(rxdma_mode)) != ZX_OK) {
    return status;
  }

  reg::TXDMA_OFFSET_CHK txdma_offset_chk;
  if ((status = bus_interface->ReadRegister(&txdma_offset_chk)) != ZX_OK) {
    return status;
  }
  txdma_offset_chk.set_drop_data_en(1);
  if ((status = bus_interface->WriteRegister(txdma_offset_chk)) != ZX_OK) {
    return status;
  }
...
Bus::BusType UsbBus::GetBusType() const { return Bus::BusType::kUsb; }
...
zx_status_t CreateUsbBus(zx_device_t* bus_device, std::unique_ptr<Bus>* bus) {
  ...
  const usb_interface_descriptor_t* usb_iface_desc = usb_desc_iter_next_interface(&usb_iter, true);
  while (usb_iface_desc != nullptr) {
    if (IsRealtekWlanDevice(*usb_iface_desc)) {
      break;
    }
