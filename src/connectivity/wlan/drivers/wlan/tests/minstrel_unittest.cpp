...
struct MinstrelTest : public ::testing::Test {
  ...
  void AdvanceTimeBy(zx::duration duration) {
    clock.Set(clock.Now() + duration);
  }
  ...
  wlan_assoc_ctx_t assoc_ctx_ht_{
      .rates_cnt = 12,
      .rates = {2, 4, 11, 22, 12 | kBasicRateBit, 18, 24, 36, 48, 72, 96,
                108 | kBasicRateBit},
      .has_ht_cap = true,
      .ht_cap =
          {
...
TEST_F(MinstrelTest, UpdateStats) {
  ...
  EXPECT_TRUE(
      minstrel_.HandleTimeout());  // tx_status are processed at HandleTimeout()
  EXPECT_EQ(ZX_OK, minstrel_.GetStatsToFidl(kTestMacAddr, &peer));
  EXPECT_EQ(
      13, peer.max_tp);  // Everything above 45 has 0 success, thus 0 throughput
  ...
  memcpy(tx_status.tx_status_entry, &entries,
         sizeof(tx_status.tx_status_entry));
...
std::unordered_set<tx_vec_idx_t> GetAllIndices(
    const wlan_minstrel::Peer& peer) {
...
TEST_F(MinstrelTest, AddMissingTxVector) {
  ...
  const uint8_t fewer_rates[10] = {2,  4,  11, 22, 12,
                                   18, 24, 36, 48, 72};  // missing 96 and 108
  std::copy(std::cbegin(fewer_rates), std::cend(fewer_rates),
            assoc_ctx_ht_.rates);
  minstrel_.AddPeer(assoc_ctx_ht_);
  ...
  wlan_tx_status_t tx_status{
      ...
      .tx_status_entry =
          {
              // ERP, CBW20, GI 800 ns,
              {kErpStartIdx + kErpNumTxVector - 1,
               1},  // MCS 7, 108, non-present, fail
              {kErpStartIdx + kErpNumTxVector - 3,
               1},  // MCS 5, 72,  present,     succeed
          },
  };
  kTestMacAddr.CopyTo(tx_status.peer_addr);
...
TEST_F(MinstrelTest, DataFramesEligibleForProbing) {
  ...
  for (tx_vec_idx_t i = 0; i < kProbeInterval * fbl::count_of(want_probe_idx_);
       ++i) {
