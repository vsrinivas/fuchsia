...
bool ath10k_tm_event_wmi(struct ath10k* ar, uint32_t cmd_id, struct sk_buff* skb) {
  ...
  nl_skb =
      cfg80211_testmode_alloc_event_skb(ar->hw->wiphy, 2 * sizeof(uint32_t) + skb->len, GFP_ATOMIC);
  if (!nl_skb) {
...
static int ath10k_tm_cmd_get_version(struct ath10k* ar, struct nlattr* tb[]) {
  ...
  ret = nla_put_u32(skb, ATH10K_TM_ATTR_WMI_OP_VERSION, ar->normal_mode_fw.fw_file.wmi_op_version);
  if (ret) {
    kfree_skb(skb);
    return ret;
  }
