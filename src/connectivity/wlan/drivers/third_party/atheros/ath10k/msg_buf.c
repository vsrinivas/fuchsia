...
zx_status_t ath10k_msg_bufs_init(struct ath10k* ar) {
  ...
  if (!ath10k_msg_types_initialized) {
    for (size_t ndx = 0; ndx < countof(ath10k_msg_types_init_data); ndx++) {
      ...
      type_info->offset =
          ath10k_msg_types_info[parent_type].offset + ath10k_msg_types_info[parent_type].hdr_size;
  ...
  for (unsigned i = 0; i < ATH10K_INITIAL_BUF_COUNT; i++) {
    ath10k_msg_buf_alloc_internal(ar, &msg_buf, ATH10K_MSG_TYPE_BASE, 1, true, __FILE__, __LINE__);
...
zx_status_t ath10k_msg_buf_alloc_internal(struct ath10k* ar, struct ath10k_msg_buf** msg_buf_ptr,
  ...
  if (!list_is_empty(&state->buf_pool) && !force_new) {
    ...
    status = io_buffer_init(&msg_buf->buf, bti_handle, PAGE_SIZE, IO_BUFFER_RW | IO_BUFFER_CONTIG);
    if (status != ZX_OK) {
      goto err_free_buf;
    }
...
static void pop_and_free_n(list_node_t* msg_buf_list, int n) {
  while (n--) {
    struct ath10k_msg_buf* msg_buf =
        list_remove_head_type(msg_buf_list, struct ath10k_msg_buf, listnode);
    if (!msg_buf) {
      break;
    }
    ath10k_msg_buf_free(msg_buf);
...
void ath10k_msg_buf_purge(list_node_t* msg_buf_list) { pop_and_free_n(msg_buf_list, -1); }
