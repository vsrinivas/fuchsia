...
void __ath10k_ce_send_revert(struct ath10k_ce_pipe* pipe) {
  ...
  if (COND_WARN_ONCE(src_ring->write_index == ath10k_ce_src_ring_write_index_get(ar, ctrl_addr))) {
    return;
  }
...
void ath10k_ce_dump_registers(struct ath10k* ar, struct ath10k_fw_crash_data* crash_data) {
  ...
  for (id = 0; id < CE_COUNT; id++) {
    ...
    ath10k_err("[%02d]: 0x%08x %3u %3u %3u %3u\n", id, ce.base_addr, ce.src_wr_idx, ce.src_r_idx,
               ce.dst_wr_idx, ce.dst_r_idx);
  }
