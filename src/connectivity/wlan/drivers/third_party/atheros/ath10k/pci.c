...
static void ath10k_bus_pci_write32(struct ath10k* ar, uint32_t offset, uint32_t value) {
  ...
  if (unlikely(offset + sizeof(value) > ar_pci->mmio.size)) {
    ath10k_warn("refusing to write mmio out of bounds at 0x%08x - 0x%08zx (max 0x%08zx)\n", offset,
                offset + sizeof(value), ar_pci->mmio.size);
    return;
  }
...
static uint32_t ath10k_bus_pci_read32(struct ath10k* ar, uint32_t offset) {
  ...
  if (unlikely(offset + sizeof(val) > ar_pci->mmio.size)) {
    ath10k_warn("refusing to read mmio out of bounds at 0x%08x - 0x%08zx (max 0x%08zx)\n", offset,
                offset + sizeof(val), ar_pci->mmio.size);
    return 0;
  }
...
zx_status_t ath10k_pci_hif_tx_sg(struct ath10k* ar, uint8_t pipe_id,
  ...
  for (i = 0; i < n_items - 1; i++) {
    ...
    err = ath10k_ce_send_nolock(ce_pipe, items[i].transfer_context, items[i].paddr, items[i].len,
                                items[i].transfer_id, CE_SEND_FLAG_GATHER);
    if (err != ZX_OK) {
      goto err;
    }
  }
...
void ath10k_pci_flush(struct ath10k* ar) { ath10k_pci_buffer_cleanup(ar); }
...
static void ath10k_pci_bmi_recv_data(struct ath10k_ce_pipe* ce_state) {
  ...
  if (ath10k_ce_completed_recv_next(ce_state, (void**)&xfer, &nbytes)) {
    return;
  }
...
zx_status_t ath10k_pci_init_config(struct ath10k* ar) {
  ...
  ret = ath10k_pci_diag_read32(
      ar, (pcie_state_targ_addr + offsetof(struct pcie_state, pipe_cfg_addr)), &pipe_cfg_targ_addr);
  if (ret != ZX_OK) {
  ...
  ret = ath10k_pci_diag_read32(
      ar, (pcie_state_targ_addr + offsetof(struct pcie_state, svc_to_pipe_map)), &svc_to_pipe_map);
  if (ret != ZX_OK) {
...
static zx_status_t ath10k_pci_set_key(void* ctx, const wlan_key_config_t* key_config) {
  ...
  ath10k_info(
      ...
      : key_config->protection == WLAN_PROTECTION_RX_TX ? "rx/tx"
                                                        : "unknown",
      ...
      : key_config->key_type == WLAN_KEY_TYPE_PEER   ? "peer"
                                                     : "unknown",
...
wlan_softmac_protocol_ops_t wlan_softmac_ops = {
    ...
    .start_passive_scan = ath10k_pci_start_hw_scan_passive,
    .start_active_scan = ath10k_pci_start_hw_scan_active,
};
...
static zx_status_t ath10k_pci_probe(void* ctx, zx_device_t* dev) {
  ...
  if (pci_get_bti(&pci, 0, &pci_btih) != ZX_OK) {
    return ZX_ERR_NOT_SUPPORTED;
  }
  ...
  ret = ath10k_core_create(&ar, sizeof(*ar_pci), ATH10K_BUS_PCI, hw_rev, &ath10k_pci_hif_ops);
  if (ret != ZX_OK) {
