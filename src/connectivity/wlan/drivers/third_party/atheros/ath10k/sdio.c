...
static int ath10k_sdio_mbox_rx_alloc(struct ath10k* ar, uint32_t lookaheads[], int n_lookaheads) {
  ...
  for (i = 0; i < n_lookaheads; i++) {
    ...
    if (htc_hdr->flags & ATH10K_HTC_FLAG_BUNDLE_MASK) {
      ...
      ret = ath10k_sdio_mbox_alloc_pkt_bundle(ar, &ar_sdio->rx_pkts[i], htc_hdr, full_len, act_len,
                                              &bndl_cnt);
...
static int ath10k_sdio_mbox_proc_cpu_intr(struct ath10k* ar) {
  ...
  mtx_lock(&irq_data->mtx);
  cpu_int_status = irq_data->irq_proc_reg->cpu_int_status & irq_data->irq_en_reg->cpu_int_status_en;
  if (!cpu_int_status) {
...
static void __ath10k_sdio_write_async(struct ath10k* ar, struct ath10k_sdio_bus_request* req) {
  ...
  if (ret)
    ath10k_warn("failed to write skb to 0x%x asynchronously: %d", req->address, ret);
...
static int ath10k_sdio_hif_disable_intrs(struct ath10k* ar) {
  ...
  ret = ath10k_sdio_write(ar, MBOX_INT_STATUS_ENABLE_ADDRESS, &regs->int_status_en, sizeof(*regs));
  if (ret) {
...
static int ath10k_sdio_hif_suspend(struct ath10k* ar) { return -EOPNOTSUPP; }
 ...
 */
static int ath10k_sdio_pm_suspend(struct device* device) { return 0; }

static int ath10k_sdio_pm_resume(struct device* device) { return 0; }
...
static void __exit ath10k_sdio_exit(void) { sdio_unregister_driver(&ath10k_sdio_driver); }
