...
#ifndef SRC_CONNECTIVITY_WLAN_DRIVERS_THIRD_PARTY_ATHEROS_ATH10K_WMI_OPS_H_
#define SRC_CONNECTIVITY_WLAN_DRIVERS_THIRD_PARTY_ATHEROS_ATH10K_WMI_OPS_H_
...
struct wmi_ops {
  ...
  int (*pull_wow_event)(struct ath10k* ar, struct ath10k_msg_buf* buf, struct wmi_wow_ev_arg* arg);
  ...
  zx_status_t (*gen_wow_add_pattern)(struct ath10k* ar, struct ath10k_msg_buf** msg_buf_ptr,
                                     uint32_t vdev_id, uint32_t pattern_id, const uint8_t* pattern,
                                     const uint8_t* mask, int pattern_len, int pattern_offset);
  ...
  zx_status_t (*ext_resource_config)(struct ath10k* ar, struct ath10k_msg_buf** msg_buf_ptr,
                                     enum wmi_host_platform_type type, uint32_t fw_feature_bitmap);
  zx_status_t (*get_vdev_subtype)(struct ath10k* ar, enum wmi_vdev_subtype subtype, int* result);
  zx_status_t (*gen_pdev_bss_chan_info_req)(struct ath10k* ar, struct ath10k_msg_buf** msg_buf_ptr,
...
static inline zx_status_t ath10k_wmi_vdev_sta_uapsd(struct ath10k* ar, uint32_t vdev_id,
  ...
  zx_status_t status = ar->wmi.ops->gen_vdev_sta_uapsd(ar, &buf, vdev_id, peer_addr, args, num_ac);
  if (status != ZX_OK) {
    return status;
  }
...
static inline zx_status_t ath10k_wmi_delba_send(struct ath10k* ar, uint32_t vdev_id,
  ...
  zx_status_t status = ar->wmi.ops->gen_delba_send(ar, &buf, vdev_id, mac, tid, initiator, reason);
  if (status != ZX_OK) {
    return status;
  }
...
static inline zx_status_t ath10k_wmi_get_vdev_subtype(struct ath10k* ar,
                                                      enum wmi_vdev_subtype subtype, int* result) {
...
#endif  // SRC_CONNECTIVITY_WLAN_DRIVERS_THIRD_PARTY_ATHEROS_ATH10K_WMI_OPS_H_
