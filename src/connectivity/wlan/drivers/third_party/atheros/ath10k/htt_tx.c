...
static zx_status_t ath10k_htt_tx_alloc_txq(struct ath10k_htt* htt) {
  ...
  ret = io_buffer_init(&htt->tx_q_state.handle, bti_handle, size, IO_BUFFER_RW | IO_BUFFER_CONTIG);
  if (ret != ZX_OK) {
    return ret;
  }
...
zx_status_t ath10k_htt_mgmt_tx(struct ath10k_htt* htt, struct ath10k_msg_buf* msdu) {
  ...
  struct htt_mgmt_tx_desc* mgmt_tx =
      ath10k_msg_buf_get_header(txdesc, ATH10K_MSG_TYPE_HTT_CMD_MGMT_TX);
...
zx_status_t ath10k_htt_tx(struct ath10k_htt* htt, enum ath10k_hw_txrx_mode txmode,
  ...
  if ((ieee80211_get_frame_type(hdr) == IEEE80211_FRAME_TYPE_MGMT) &&
    ...
    msdu->used += fuchsia_wlan_ieee80211_CCMP_128_MIC_LEN;
  ...
  ath10k_dbg(ar, ATH10K_DBG_HTT,
             ...
             "%hhu tid %hhu freq %hu\n",
             flags0, flags1, msdu->used, msdu_id, frags_paddr, (uint32_t)msdu->paddr, vdev_id, tid,
             freq);
  ath10k_dbg_dump(ar, ATH10K_DBG_HTT_DUMP, NULL, "htt tx msdu: ", ath10k_msg_buf_get_payload(msdu),
                  msdu->used);
