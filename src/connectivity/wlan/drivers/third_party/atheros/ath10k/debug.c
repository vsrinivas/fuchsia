...
void ath10k_debug_print_board_info(struct ath10k* ar) {
  ...
  if (ar->id.bmi_ids_valid)
    SNPRINTF_USED(boardinfo, sizeof(boardinfo), "%d:%d", ar->id.bmi_chip_id, ar->id.bmi_board_id);
  else {
...
void ath10k_debug_print_boot_info(struct ath10k* ar) {
  ath10k_trace("htt-ver %d.%d wmi-op %d htt-op %d cal %s max-sta %d raw %d hwcrypto %d\n",
               ...
               ar->normal_mode_fw.fw_file.wmi_op_version, ar->normal_mode_fw.fw_file.htt_op_version,
               ath10k_cal_mode_str(ar->cal_mode), ar->max_num_stations,
...
static ssize_t ath10k_read_wmi_services(struct file* file, char __user* user_buf, size_t count,
  ...
  for (i = 0; i < WMI_SERVICE_MAX; i++) {
    ...
    if (!name) {
      if (enabled)
        len +=
            SNPRINTF_USED(buf + len, buf_len - len, "%-40s %s (bit %d)\n", "unknown", "enabled", i);

      continue;
    }

    len += SNPRINTF_USED(buf + len, buf_len - len, "%-40s %s\n", name, enabled ? "enabled" : "-");
  }
...
static void ath10k_tpc_stats_print(struct ath10k_tpc_stats* tpc_stats, unsigned int j, char* buf,
  ...
  *len += SNPRINTF_USED(buf + *len, buf_len - *len,
  ...
  for (i = 0; i < tpc_stats->rate_max; i++) {
    *len +=
        SNPRINTF_USED(buf + *len, buf_len - *len, "%8d %s 0x%2x %s\n", i,
                      pream_str[tpc_stats->tpc_table[j].pream_idx[i]],
                      tpc_stats->tpc_table[j].rate_code[i], tpc_stats->tpc_table[j].tpc_value[i]);
  }
...
static ssize_t ath10k_debug_fw_checksums_read(struct file* file, char __user* user_buf,
  ...
  len += SNPRINTF_USED(buf + len, buf_len - len, "board-N.bin\t\t%08x\n",
                       crc32_le(0, ar->normal_mode_fw.board->data, ar->normal_mode_fw.board->size));
...
void ath10k_dbg_dump(struct ath10k* ar, enum ath10k_debug_mask mask, const char* msg,
  ...
  if (ath10k_debug_mask & mask) {
    ...
    for (ptr = buf; (ptr - buf) < len; ptr += 16) {
      linebuflen = 0;
      linebuflen += SNPRINTF_USED(linebuf + linebuflen, sizeof(linebuf) - linebuflen,
