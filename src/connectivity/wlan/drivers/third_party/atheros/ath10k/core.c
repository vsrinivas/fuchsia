...
static zx_status_t ath10k_fetch_fw_file(struct ath10k* ar, const char* dir, const char* file,
  ...
  uintptr_t vaddr;
  ret = zx_vmar_map(zx_vmar_root_self(), ZX_VM_PERM_READ, 0, firmware->vmo, 0, firmware->size,
                    &vaddr);
  if (ret != ZX_OK) {
    goto close_vmo;
  }
...
static zx_status_t ath10k_push_board_ext_data(struct ath10k* ar, const void* data,
  ...
  ret =
      ath10k_bmi_write_memory(ar, board_ext_data_addr, data + board_data_size, board_ext_data_size);
  if (ret != ZX_OK) {
...
static zx_status_t ath10k_core_create_board_name(struct ath10k* ar, char* name, size_t name_len) {
  ...
  if (ar->id.bmi_ids_valid) {
    snprintf(name, name_len, "bus=%s,bmi-chip-id=%d,bmi-board-id=%d", ath10k_bus_str(ar->hif.bus),
...
static zx_status_t ath10k_init_hw_params(struct ath10k* ar) {
  ...
  for (i = 0; i < countof(ath10k_hw_params_list); i++) {
    ...
    if (hw_params->id == ar->target_version && hw_params->dev_id == ar->dev_id) {
      break;
    }
  }
...
static zx_status_t ath10k_core_set_ps_mode(void* ctx, const wlanphy_ps_mode_t* ps_mode) {
  if (ps_mode == NULL) {
    return ZX_ERR_INVALID_ARGS;
  }

  ath10k_err("ath10k_core_set_ps_mode() not implemented\n");
  return ZX_ERR_NOT_SUPPORTED;
}

zx_status_t ath10k_core_get_ps_mode(void* ctx, wlanphy_ps_mode_t* out_ps_mode) {
  if (out_ps_mode == NULL) {
    return ZX_ERR_INVALID_ARGS;
  }

  ath10k_err("ath10k_core_get_ps_mode() not implemented\n");
  return ZX_ERR_NOT_SUPPORTED;
}
...
static wlanphy_impl_protocol_ops_t wlanphy_ops = {
    ...
    .set_ps_mode = ath10k_core_set_ps_mode,
    .get_ps_mode = ath10k_core_get_ps_mode,
};
