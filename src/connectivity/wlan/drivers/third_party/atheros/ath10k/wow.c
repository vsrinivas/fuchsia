...
static int ath10k_vif_wow_set_wakeups(struct ath10k_vif* arvif, struct cfg80211_wowlan* wowlan) {
  ...
  for (i = 0; i < wowlan->n_patterns; i++) {
    ...
    ret = ath10k_wmi_wow_add_pattern(ar, arvif->vdev_id, pattern_id, patterns[i].pattern, bitmask,
                                     patterns[i].pattern_len, patterns[i].pkt_offset);
    if (ret) {
      ath10k_warn("failed to add pattern %i to vdev %i: %d\n", pattern_id, arvif->vdev_id, ret);
      return ret;
    }
