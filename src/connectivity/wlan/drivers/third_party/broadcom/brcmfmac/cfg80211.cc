...
static zx_status_t brcmf_vif_add_validate(struct brcmf_cfg80211_info* cfg, mac_role_t new_type) {
  struct brcmf_cfg80211_vif* pos;
  struct iface_combination_params params = {
      .num_different_channels = 1,
  };
...
static zx_status_t brcmf_ap_add_vif(struct brcmf_cfg80211_info* cfg, const char* name,
  ...
  if (!brcmf_feat_is_enabled(ifp, BRCMF_FEAT_MFG)) {
    ...
    err = brcmf_alloc_vif(cfg, MAC_ROLE_AP, &vif);
    if (err != ZX_OK) {
      if (dev_out) {
        *dev_out = nullptr;
      }
    ...
    vif = ifp->vif;
    vif->ifp = ifp;
    err = brcmf_cfg80211_change_iface_mac_role(cfg, ifp->ndev, MAC_ROLE_AP);
    if (err != ZX_OK) {
...
static bool brcmf_is_existing_macaddr(brcmf_pub* drvr, const uint8_t mac_addr[ETH_ALEN],
  ...
  if (is_ap) {
    ...
    for (const auto& iface : drvr->iflist) {
      if (iface != nullptr && iface->vif->wdev.iftype != MAC_ROLE_CLIENT &&
  ...
  switch (req->role) {
    ...
    case MAC_ROLE_CLIENT: {
      ...
      if (brcmf_feat_is_enabled(ifp, BRCMF_FEAT_MFG)) {
        ...
        // IF type.
        err = brcmf_cfg80211_change_iface_mac_role(drvr->config, ifp->ndev, MAC_ROLE_CLIENT);
        if (err != ZX_OK) {
...
static void brcmf_link_down(struct brcmf_cfg80211_vif* vif, wlan_ieee80211::ReasonCode reason_code,
  ...
  if (brcmf_test_and_clear_bit_in_array(BRCMF_VIF_STATUS_CONNECTED, &vif->sme_state)) {
    ...
    if (vif->wdev.iftype == MAC_ROLE_CLIENT) {
...
zx_status_t brcmf_cfg80211_connect(struct net_device* ndev, const wlan_fullmac_assoc_req_t* req) {
  struct brcmf_if* ifp = ndev_to_if(ndev);
  struct brcmf_cfg80211_info* cfg = ifp->drvr->config;
...
void brcmf_if_join_req(net_device* ndev, const wlan_fullmac_join_req_t* req) {
  ...
  if (!ssid.empty()) {
    ...
    BRCMF_IFDBG(WLANIF, ndev, "  ssid: " FMT_SSID ", bssid: " FMT_MAC ", channel: %u",
                FMT_SSID_VECT(ssid), FMT_MAC_ARGS(sme_bss.bssid), sme_bss.channel.primary);
#endif /* !defined(NDEBUG) */
...
void brcmf_if_auth_req(net_device* ndev, const wlan_fullmac_auth_req_t* req) {
  ...
  wlan_fullmac_impl_ifc_auth_conf(&ndev->if_proto, &response);
}
...
static void brcmf_dump_if_query_info(wlan_fullmac_query_info_t* info) {
  ...
  BRCMF_DBG_UNFILTERED("   role(s): %s%s%s", info->role & MAC_ROLE_CLIENT ? "client " : "",
                       info->role & MAC_ROLE_AP ? "ap " : "",
                       info->role & MAC_ROLE_MESH ? "mesh " : "");
...
void brcmf_if_stats_query_req(net_device* ndev) {
  ...
  struct wireless_dev* wdev = ndev_to_wdev(ndev);
  wlan_fullmac_stats_query_response_t response = {};
  struct brcmf_if* ifp = ndev_to_if(ndev);
  bcme_status_t fw_err;
  ...
  switch (wdev->iftype) {
    case MAC_ROLE_CLIENT: {
      ...
      std::vector<wlan_fullmac_hist_bucket_t> noise_floor_samples, rssi_samples,
          rx_rate_index_samples, snr_samples;
      ...
      *mlme_stats = {};
      ...
      mlme_stats->tag = WLAN_FULLMAC_MLME_STATS_TYPE_CLIENT;
      if (brcmf_test_bit_in_array(BRCMF_VIF_STATUS_CONNECTED, &ifp->vif->sme_state)) {
        ...
        // Skip wlan_fullmac detailed histogram collection if feature is not enabled.
        ...
        if (hist_status != ZX_OK) {
          // If wlan_fullmac detailed histogram collection fails, leave the histogram fields empty.
          break;
        }
      ...
      wlan_fullmac_impl_ifc_stats_query_resp(&ndev->if_proto, &response);
      return;
      break;
    }
    case MAC_ROLE_AP:
...
zx_status_t brcmf_alloc_vif(struct brcmf_cfg80211_info* cfg, uint16_t type,
  ...
  if (type == MAC_ROLE_AP) {
    ...
    list_for_every_entry (&cfg->vif_list, vif_walk, struct brcmf_cfg80211_vif, list) {
      if (vif_walk->wdev.iftype == MAC_ROLE_AP) {
        mbss = true;
        break;
      }
    }
...
zx_status_t brcmf_notify_channel_switch(struct brcmf_if* ifp, const struct brcmf_event_msg* e,
  ...
  if (wdev->iftype == MAC_ROLE_CLIENT) {
...
const char* brcmf_cfg80211_get_iface_str(struct net_device* ndev) {
  if (ndev_to_vif(ndev)->wdev.iftype == MAC_ROLE_CLIENT)
    return "Client";
  else
    return "SoftAP";
}
...
zx_status_t brcmf_cfg80211_attach(struct brcmf_pub* drvr) {
  ...
  err = brcmf_alloc_vif(cfg, MAC_ROLE_CLIENT, &vif);
  if (err != ZX_OK) {
    goto cfg80211_info_out;
  }
