...
static void brcmf_log_client_stats(struct brcmf_cfg80211_info* cfg) {
  ...
  if (ndev->stats.rx_freeze_count == BRCMF_RX_FREEZE_THRESHOLD / BRCMF_CONNECT_LOG_DUR) {
    ...
    if (trigger_deauth) {
      // Disassociate, using an otherwise-unused reason code that will uniquely identify this
      // trigger.
      ...
      brcmf_link_down(ifp->vif, WLANIF_REASON_CODE_MESH_PATH_ERROR_NO_PROXY_INFORMATION,
                      BRCMF_E_DEAUTH_IND);
...
void brcmf_cfg80211_rx(struct brcmf_if* ifp, const void* data, size_t size) {
  ...
  if (eth_type == EAPOL_ETHERNET_TYPE_UINT16) {
    ...
    if (ifp->arp_logger != nullptr && eth_type == ARP_ETHERNET_TYPE_UINT16 && arp_opt_code == 2) {
...
static void brcmf_return_scan_result(struct net_device* ndev, uint16_t channel,
  ...
  BRCMF_DBG(SCAN, "Returning scan result %.*s, channel %d, dbm %d, id %lu", (int)ssid.size(),
            ssid.data(), channel, result.bss.rssi_dbm, result.txn_id);
...
void brcmf_if_start_req(net_device* ndev, const wlanif_start_req_t* req) {
  BRCMF_IFDBG(WLANIF, ndev, "Start AP request from SME. ssid: %.*s, channel: %u, rsne_len: %zu",
...
void brcmf_if_stop_req(net_device* ndev, const wlanif_stop_req_t* req) {
  ...
  BRCMF_IFDBG(WLANIF, ndev, "Stop AP request from SME. ssid: %.*s", req->ssid.len, req->ssid.data);
...
void brcmf_if_data_queue_tx(net_device* ndev, uint32_t options, ethernet_netbuf_t* netbuf,
  ...
  if (ifp->arp_logger != nullptr && eth_type == ARP_ETHERNET_TYPE_UINT16 && arp_opt_code == 1) {
    ...
    if (ifp->arp_logger->AddArpRequestFrame(arp_frame_str)) {
      ifp->drvr->inspect->LogArpRequestFrame(zx_clock_get_monotonic(), frame, netbuf->data_size);
    }
