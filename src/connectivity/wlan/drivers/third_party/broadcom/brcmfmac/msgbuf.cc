...
struct brcmf_msgbuf {
    ...
    std::atomic<unsigned long>* flow_map;
    std::atomic<unsigned long>* txstatus_done_map;
...
static struct brcmf_msgbuf_pktids* brcmf_msgbuf_init_pktids(uint32_t nr_array_entries,
    ...
    array = static_cast<decltype(array)>(calloc(nr_array_entries, sizeof(*array)));
    if (!array) {
        return NULL;
    }

    pktids = static_cast<decltype(pktids)>(calloc(1, sizeof(*pktids)));
    if (!pktids) {
        free(array);
        return NULL;
    }
...
static zx_status_t brcmf_msgbuf_alloc_pktid(struct brcmf_device* dev,
    ...
    *idx = pktids->last_allocated_idx;
    ...
    do {
        ...
        if (array[*idx].allocated.load() == 0) {
            int expected = 0;
            array[*idx].allocated.compare_exchange_strong(expected, 1);
            if (expected == 0) {
                break;
            }
        }
...
static uint32_t brcmf_msgbuf_flowring_create(struct brcmf_msgbuf* msgbuf, int ifidx,
    ...
    create = static_cast<decltype(create)>(calloc(1, sizeof(*create)));
    if (create == NULL) {
        return BRCMF_FLOWRING_INVALID_ID;
    }
...
static void brcmf_msgbuf_txflow(struct brcmf_msgbuf* msgbuf, uint16_t flowid) {
    ...
    while (brcmf_flowring_qlen(flow, flowid)) {
        ...
        commonring->outstanding_tx.fetch_add(1);
...
static zx_status_t brcmf_msgbuf_schedule_txdata(struct brcmf_msgbuf* msgbuf, uint32_t flowid,
    ...
    commonring = msgbuf->flowrings[flowid];
    if ((force) || (commonring->outstanding_tx.load() < BRCMF_MSGBUF_DELAY_TXWORKER_THRS)) {
...
static void brcmf_msgbuf_process_txstatus(struct brcmf_msgbuf* msgbuf, void* buf) {
    ...
    commonring = msgbuf->flowrings[flowid];
    commonring->outstanding_tx.fetch_sub(1);
...
static uint32_t brcmf_msgbuf_rxbuf_data_post(struct brcmf_msgbuf* msgbuf, uint32_t count) {
    ...
    for (i = 0; i < alloced; i++) {
        ...
        ret_ptr = static_cast<char*>(ret_ptr) + brcmf_commonring_len_item(commonring);
    }
...
again:
    buf = static_cast<char*>(brcmf_commonring_get_read_ptr(commonring, &count));
    if (buf == NULL) {
        return;
    }
...
zx_status_t brcmf_proto_msgbuf_rx_trigger(struct brcmf_device* dev) {
    ...
    for_each_set_bit(flowid, msgbuf->txstatus_done_map, msgbuf->max_flowrings) {
        ...
        if ((qlen > BRCMF_MSGBUF_TRICKLE_TXWORKER_THRS) ||
                ((qlen) &&
                 (commonring->outstanding_tx.load() < BRCMF_MSGBUF_TRICKLE_TXWORKER_THRS))) {
            brcmf_msgbuf_schedule_txdata(msgbuf, flowid, true);
        }
    }
...
void brcmf_msgbuf_delete_flowring(struct brcmf_pub* drvr, uint16_t flowid) {
    struct brcmf_msgbuf* msgbuf = (struct brcmf_msgbuf*)drvr->proto->pd;
    struct msgbuf_tx_flowring_delete_req* delete_req;
    struct brcmf_commonring* commonring;
    void* ret_ptr;
    uint8_t ifidx;
    zx_status_t err;
    ...
    delete_req = (struct msgbuf_tx_flowring_delete_req*)ret_ptr;

    ifidx = brcmf_flowring_ifidx_get(msgbuf->flow, flowid);

    delete_req->msg.msgtype = MSGBUF_TYPE_FLOW_RING_DELETE;
    delete_req->msg.ifidx = ifidx;
    delete_req->msg.request_id = 0;

    delete_req->flow_ring_id = flowid + BRCMF_H2D_MSGRING_FLOWRING_IDSTART;
    delete_req->reason = 0;
...
zx_status_t brcmf_proto_msgbuf_attach(struct brcmf_pub* drvr) {
    ...
    msgbuf = static_cast<decltype(msgbuf)>(calloc(1, sizeof(*msgbuf)));
    if (!msgbuf) {
        goto fail;
    }
    ...
    msgbuf->flow_map = static_cast<decltype(msgbuf->flow_map)>(calloc(1, count));
    if (!msgbuf->flow_map) {
        goto fail;
    }

    msgbuf->txstatus_done_map = static_cast<decltype(msgbuf->txstatus_done_map)>(calloc(1, count));
