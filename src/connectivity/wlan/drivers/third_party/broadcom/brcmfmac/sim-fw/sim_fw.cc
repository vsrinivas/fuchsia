...
zx_status_t SimFirmware::BusTxCtl(unsigned char* msg, unsigned int len) {
    ...
    switch(hdr->cmd) {
        ...
        case BRCMF_C_SET_VAR:
             {
                 uint8_t* str_end = static_cast<uint8_t*>(std::memchr(bcdc_data, '\0',
                                                          bcdc_data_len));
                 if (str_end == nullptr) {
                     BRCMF_ERR("iovar name not null-terminated\n");
                     return ZX_ERR_INVALID_ARGS;
                 }
                 status = IovarsSet(reinterpret_cast<char*>(bcdc_data),
                                    static_cast<void*>(str_end + 1),
...
void SimFirmware::BcdcResponse::Clear() {
    len_ = 0;
}
...
bool SimFirmware::BcdcResponse::IsClear() {
    return len_ == 0;
}
