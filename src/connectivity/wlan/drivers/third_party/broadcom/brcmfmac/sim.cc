...
static const struct brcmf_bus_ops brcmf_sim_bus_ops = {
    .preinit = [](struct brcmf_device* dev) {
        return BUS_OP(dev)->BusPreinit();
    },
    .stop = [](struct brcmf_device* dev) {
        return BUS_OP(dev)->BusStop();
    },
    .txdata = [](struct brcmf_device* dev, struct brcmf_netbuf* netbuf) {
        return BUS_OP(dev)->BusTxData(netbuf);
    },
    .txctl = [](struct brcmf_device* dev, unsigned char* msg, uint len) {
        return BUS_OP(dev)->BusTxCtl(msg, len);
    },
    .rxctl = [](struct brcmf_device* dev, unsigned char* msg, uint len, int* rxlen_out) {
        return BUS_OP(dev)->BusRxCtl(msg, len, rxlen_out);
    },
    .gettxq = [](struct brcmf_device* dev) {
        return BUS_OP(dev)->BusGetTxQueue();
    },
    .wowl_config = [](struct brcmf_device* dev, bool enabled) {
        return BUS_OP(dev)->BusWowlConfig(enabled);
    },
    .get_ramsize = [](struct brcmf_device* dev) {
        return BUS_OP(dev)->BusGetRamsize();
    },
    .get_memdump = [](struct brcmf_device* dev, void* data, size_t len) {
        return BUS_OP(dev)->BusGetMemdump(data, len);
    },
    .get_fwname = [](struct brcmf_device* dev, uint chip, uint chiprev, unsigned char* fw_name) {
        return BUS_OP(dev)->BusGetFwName(chip, chiprev, fw_name);
    },
    .get_bootloader_macaddr = [](struct brcmf_device* dev, uint8_t* mac_addr) {
...
zx_status_t brcmf_sim_probe(brcmf_simdev* simdev) {
    ...
    simdev->settings = brcmf_get_module_param(simdev->bus_if.dev, BRCMF_BUS_TYPE_SIM, chip,
                                              chiprev);
