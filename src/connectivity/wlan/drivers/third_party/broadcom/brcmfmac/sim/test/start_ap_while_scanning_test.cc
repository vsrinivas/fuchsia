...
TEST_F(ScanAndApStartTest, ScanApStartInterference) {
  ...
  SCHEDULE_CALL(zx::msec(200), &ScanAndApStartTest::StartAp, this);
...
TEST_F(ScanAndApStartTest, ScanWhileApStart) {
  ...
  SCHEDULE_CALL(zx::msec(10), &ScanAndApStartTest::StartAp, this);
  SCHEDULE_CALL(zx::msec(300), &SimInterface::StartScan, &client_ifc_, kDefaultScanId, false);

  static constexpr zx::duration kTestDuration = zx::sec(50);
  env_->Run(kTestDuration);

  // The first scan should be done because the abort is failed
  auto first_result = client_ifc_.ScanResultCode(kDefaultScanId);
