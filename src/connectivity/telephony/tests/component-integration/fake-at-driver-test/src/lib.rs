...
use {
    ...
    fuchsia_zircon::{self as zx, DurationNum}, qmi as tel_ctrl,
    tel_dev::{component_test::*, at_test_common::*},
};
...
async fn fake_at_driver_test() {
    ...
    let at_device = get_fake_device_in_isolated_devmgr(TEL_PATH).await.expect("getting fake device");
    let ctrl_channel =
    ...
    let at_msg_vec = read_next_msg_from_channel(
        &ctrl_channel,
        zx::Time::after(1.second()),
    ).expect("receiving at msg");
    assert_eq!(&at_msg_vec, &AT_CMD_RESP_NO_CARRIER.to_vec());
    fx_log_info!("received and verified responses");
    unbind_fake_device(&at_device, ONE_SECOND_IN_NANOS).expect("removing fake device");
    fx_log_info!("unbinded device");
    validate_removal_of_fake_device(TEL_PATH, ONE_SECOND_IN_NANOS)
        .await
