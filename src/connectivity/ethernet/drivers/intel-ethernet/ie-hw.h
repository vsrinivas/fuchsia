...
#define IE_TCTL_BST(n) \
  (((n)&0x3ff) << 12)  // Back-Off Slot Time. This value determines
                       // the back-off slot time value in byte time.
#define IE_STATUS_PF_RST_DONE \
  (1u << 21)  // When set, indicates that software reset
              // (CTRL.RST) or device reset (CTRL.DEV_RST)
              // has completed and the software device
              // driver can begin initialization process.
#define IE_INT_RXDW \
  (1u << 7)  // Receiver Descriptor Write Back. Set when
