...
TEST_F(PairingStateTest,
  ...
  EXPECT_EQ(hci::Status(status_code), *status_handler.status());
}
...
TEST_F(PairingStateTest, ResponderSignalsCompletionOfPairing) {
  ...
  EXPECT_EQ(1, status_handler.call_count());
  ASSERT_TRUE(status_handler.status().has_value());
  EXPECT_EQ(hci::Status(expected_status), status_handler.status().value());
  ...
  EXPECT_EQ(hci::Status(expected_status), new_pairing_handler.status().value());
