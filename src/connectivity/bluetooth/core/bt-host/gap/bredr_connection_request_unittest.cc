...
const hci::Status RetryableError(hci_spec::StatusCode::kPageTimeout);
...
TEST_F(BrEdrConnectionRequestLoopTest, ShouldntRetryWithNonRetriableErrorCode) {
  connection_req().RecordHciCreateConnectionAttempt();
  RunLoopFor(zx::sec(1));
  EXPECT_FALSE(connection_req().ShouldRetry(hci::Status(HostError::kCanceled)));
}
