...
void LowEnergyConnection::StartConnectionPausePeripheralTimeout() {
  ...
  conn_pause_peripheral_timeout_.emplace([this]() {
    ...
    // kLEConnectionPausePeripheral after establishing a connection." (Core Spec v5.2, Vol 3,
    // Part C, Sec 9.3.12).
...
void LowEnergyConnection::StartConnectionPauseCentralTimeout() {
  ...
  conn_pause_central_timeout_.emplace([this]() {
    ...
    // Peripheral device has not initiated any other actions within kLEConnectionPauseCentral,
    // then the Central device should update the connection parameters to either the Peripheral
    // Preferred Connection Parameters or self-determined values." (Core Spec v5.2, Vol 3, Part
    // C, Sec 9.3.12).
    //
    // If the GAP service preferred connection parameters characteristic has not been read by
    // now, just use the default parameters.
...
void LowEnergyConnection::OnL2capFixedChannelsOpened(
  ...
  if (peer->le() && peer->le()->bond_data()) {
    ...
    // have the [...] (LTK, EDIV, and Rand) distributed by the slave device in LE legacy [...]
    // to setup an encrypted session" (V5.0 Vol. 3 Part H 2.4.4.2). For Secure Connections
    // peer_ltk and local_ltk will be equal, so this check is unnecessary but correct.
...
void LowEnergyConnection::HandleRequestConnectionParameterUpdateCommandStatus(
  ...
  // Note that this callback is for the Connection Update Complete event, not the Connection
  // Update status event, which is handled by the above code (see v5.2, Vol. 4, Part E 7.7.15
  // / 7.7.65.3).
