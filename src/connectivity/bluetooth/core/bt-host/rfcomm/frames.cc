...
inline bool FrameHasCreditOctet(bool credit_based_flow, FrameType frame_type, bool poll_final) {
  return credit_based_flow && (frame_type == FrameType::kUnnumberedInfoHeaderCheck) &&
         poll_final == true;
}

}  // namespace

Frame::Frame(Role role, CommandResponse command_response, DLCI dlci, uint8_t control,
             bool poll_final)
    : role_(role),
...
std::unique_ptr<Frame> Frame::Parse(bool credit_based_flow, Role role, const ByteBuffer& buffer) {
  ...
  if (IsMultiplexerStarted(role)) {
    // See table 1 in 5.2.1.2, which describes exactly how the C/R bit is
    // interpreted.
    command_response = (role == Role::kInitiator && cr_bit) || (role == Role::kResponder && !cr_bit)
    ...
    switch (frame_type) {
      case FrameType::kSetAsynchronousBalancedMode:
        command_response = cr_bit ? CommandResponse::kCommand : CommandResponse::kResponse;
        break;
      case FrameType::kDisconnectedMode:
      case FrameType::kUnnumberedAcknowledgement:
        command_response = cr_bit ? CommandResponse::kResponse : CommandResponse::kCommand;
        break;
      default:
        // TODO(armansito): Add a unit test for this case.
        bt_log(ERROR, "rfcomm", "malformed frame type: %u", static_cast<unsigned int>(frame_type));
        return nullptr;
    }
  }
  ...
  uint8_t credits = 0;
  bool has_credit_octet = FrameHasCreditOctet(credit_based_flow, frame_type, poll_final);
  if (has_credit_octet) {
  ...
  // octet.
  size_t header_size = 2 + (two_octet_length ? 2 : 1) + (has_credit_octet ? 1 : 0);
  ...
  size_t num_octets = frame_type == FrameType::kUnnumberedInfoHeaderCheck ? 2 : 3;
  ...
  if (frame_type == FrameType::kUnnumberedInfoHeaderCheck) {
    ...
    if (dlci == kMuxControlDLCI) {
      std::unique_ptr<MuxCommand> mux_command = MuxCommand::Parse(buffer.view(header_size, length));
      if (!mux_command) {
        bt_log(WARN, "rfcomm", "unable to parse mux command");
        return nullptr;
      }
      auto mux_command_frame =
          std::make_unique<MuxCommandFrame>(role, credit_based_flow, std::move(mux_command));
      ...
      auto user_data_frame =
          std::make_unique<UserDataFrame>(role, credit_based_flow, dlci, std::move(information));
      user_data_frame->set_credits(credits);
      return user_data_frame;
    } else {
      bt_log(WARN, "rfcomm", "Parsed DLCI %u is not a valid multiplexer or user data channel",
             dlci);
      return nullptr;
    }
  } else {
    return std::make_unique<Frame>(role, command_response, dlci, uint8_t(frame_type), poll_final);
  }
}
...
void Frame::Write(MutableBufferView buffer) const {
  ...
  size_t num_octets = (FrameType)control_ == FrameType::kUnnumberedInfoHeaderCheck ? 2 : 3;
...
void Frame::WriteHeader(MutableBufferView buffer) const {
  ...
  if (role_ == Role::kInitiator || role_ == Role::kResponder) {
    // GSM Table 1.
    command_response_bit =
        (role_ == Role::kInitiator && command_response_ == CommandResponse::kCommand) ||
        (role_ == Role::kResponder && command_response_ == CommandResponse::kResponse);
    ...
    ZX_PANIC("unexpected role while writing frame: %u", static_cast<unsigned int>(role_));
    return;
  }
...
size_t Frame::header_size() const {
  ...
  return sizeof(uint8_t) + sizeof(uint8_t) + sizeof(uint8_t) * (TwoOctetLength(length()) ? 2 : 1);
}
...
SetAsynchronousBalancedModeCommand::SetAsynchronousBalancedModeCommand(Role role, DLCI dlci)
    : Frame(role,
            // SABM frames are always commands.
            CommandResponse::kCommand, dlci, uint8_t(FrameType::kSetAsynchronousBalancedMode),
            // Poll bit is always 1. See GSM 5.4.4.1.
            true) {}

UnnumberedAcknowledgementResponse::UnnumberedAcknowledgementResponse(Role role, DLCI dlci)
    : Frame(role,
            // UA frames are always responses.
            CommandResponse::kResponse, dlci, uint8_t(FrameType::kUnnumberedAcknowledgement),
...
DisconnectedModeResponse::DisconnectedModeResponse(Role role, DLCI dlci)
    : Frame(role,
            ...
            CommandResponse::kResponse, dlci, uint8_t(FrameType::kDisconnectedMode),
...
UnnumberedInfoHeaderCheckFrame::UnnumberedInfoHeaderCheckFrame(Role role, bool credit_based_flow,
                                                               DLCI dlci)
    : Frame(role,
            // UIH frames are always commands.
            CommandResponse::kCommand, dlci, uint8_t(FrameType::kUnnumberedInfoHeaderCheck),
...
void UnnumberedInfoHeaderCheckFrame::WriteHeader(MutableBufferView buffer) const {
  ZX_DEBUG_ASSERT(buffer.size() >= header_size());
...
size_t UserDataFrame::written_size() const {
  return UnnumberedInfoHeaderCheckFrame::header_size() + length()  // Information
         + sizeof(uint8_t);                                        // FCS
}

ByteBufferPtr UserDataFrame::TakeInformation() { return std::move(information_); }
...
std::unique_ptr<MuxCommand> MuxCommandFrame::TakeMuxCommand() { return std::move(mux_command_); }
