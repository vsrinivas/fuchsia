...
use {
    anyhow::Error, bt_test_harness::inspect_v2::InspectHarness,
...
async fn test_gap_hierarchy_published(harness: InspectHarness) -> Result<(), Error> {
    harness.write_state().moniker_to_track = vec!["bt-init".to_string(), "bt-gap".to_string()];
    let min_num_hierarchies: usize = 1;
    let state = harness.expect_n_hierarchies(min_num_hierarchies).await?;
