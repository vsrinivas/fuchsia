...
#define BT_LIB_U_IS_UNICODE_CHAR(c) \
  ((uint32_t)(c) < 0xd800 ||        \
   ((uint32_t)(c) > 0xdfff && (uint32_t)(c) <= 0x10ffff && !BT_LIB_U_IS_UNICODE_NONCHAR(c)))
 ...
 * @internal
 */
#define BT_LIB_U8_COUNT_TRAIL_BYTES(leadByte) (bt_lib_icu::utf8_countTrailBytes[(uint8_t)leadByte])
  ...
  {                                                                                                \
    ...
    if (((uint8_t)(c)) >= 0x80) {                                                                  \
      if (BT_LIB_U8_IS_LEAD(c)) {                                                                  \
        (c) = bt_lib_icu::utf8_nextCharSafeBody((const uint8_t*)s, &(i), (size_t)(length), c, -1); \
      } else {                                                                                     \
        (c) = BT_LIB_U_SENTINEL;                                                                   \
      }                                                                                            \
    }                                                                                              \
...
#define BT_LIB_U16_GET_SUPPLEMENTARY(lead, trail) \
  (((bt_lib_icu::UChar32)(lead) << 10UL) + (bt_lib_icu::UChar32)(trail)-BT_LIB_U16_SURROGATE_OFFSET)
 ...
 * @stable ICU 2.4
 */
#define BT_LIB_U16_LEAD(supplementary) (bt_lib_icu::UChar)(((supplementary) >> 10) + 0xd7c0)
 ...
 * @stable ICU 2.4
 */
#define BT_LIB_U16_TRAIL(supplementary) (bt_lib_icu::UChar)(((supplementary)&0x3ff) | 0xdc00)
