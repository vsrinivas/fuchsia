...
class BtHciMediatek : public DeviceType, public ddk::EmptyProtocol<ZX_PROTOCOL_BT_TRANSPORT> {
 ...
 private:
  ...
  zx::channel cmd_channel_ __TA_GUARDED(thread_mutex_);
  zx::channel acl_channel_ __TA_GUARDED(thread_mutex_);
  zx::channel snoop_channel_ __TA_GUARDED(thread_mutex_);
  thrd_t thread_;
  bool thread_running_ __TA_GUARDED(thread_mutex_) = false;
