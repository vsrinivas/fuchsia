...
use fidl_fuchsia_bluetooth_fastpair::ProviderHandleProxy;
...
pub struct FastPairConnectionManager {
    ...
    handle: Option<ProviderHandleProxy>,
}
...
impl FastPairConnectionManager {
    ...
    pub fn reset(&mut self) {
        if let Some(_upstream) = self.handle.take() {
            debug!("Reset upstream Fast Pair connection");
        }
    }

    #[cfg(test)]
    pub fn new_with_upstream(handle: ProviderHandleProxy) -> Self {
        Self { handle: Some(handle) }
    }

    pub fn is_enabled(&self) -> bool {
        self.handle.as_ref().map_or(false, |h| !h.is_closed())
    }

    pub fn set(&mut self, handle: ProviderHandleProxy) -> Result<(), Error> {
        if self.is_enabled() {
    ...
    pub fn notify_pairing_complete(&self, id: PeerId) {
        if let Some(handle) = &self.handle {
            ...
            // Should we care?
            if let Err(e) = handle.on_pairing_complete(&mut id.into()).check() {
                warn!("Couldn't notify upstream client of pairing complete: {:?}", e);
            }
        }
    }

    /// If set, returns a Future that resolves when the `ProviderHandle` channel is closed.
    /// Otherwise, returns a Future that never resolves.
    pub fn on_upstream_client_closed(&self) -> impl Future<Output = ()> + 'static {
        match &self.handle {
            Some(handle) => handle.on_closed().extend_lifetime().map(|_| ()).left_future(),
...
pub(crate) mod tests {
    ...
    pub(crate) struct MockUpstreamClient {
        connection: ProviderHandleRequestStream,
    }

    impl MockUpstreamClient {
        pub fn new() -> (Self, ProviderHandleProxy) {
            let (c, s) = create_proxy_and_stream::<ProviderHandleMarker>().unwrap();
        ...
        pub async fn make_enable_request(
            ...
            let (handle_client, handle_server) =
                create_request_stream::<ProviderHandleMarker>().unwrap();
            let mock_client = Self { connection: handle_server };
            let fut = c.enable(handle_client);
            let (handle, responder) =
                s.select_next_some().await.expect("fidl request").into_enable().unwrap();

            (
                fut,
                ServiceRequest::EnableFastPair { handle: handle.into_proxy().unwrap(), responder },
                mock_client,
            )
        }
    }
