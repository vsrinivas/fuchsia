...
// TODO(fxbug.dev/96222): Remove this when this is used in the key-based pairing procedure impl.
#[allow(unused)]
...
/// Generates an Anti-Spoofing AES Key from the remote Public key and the local Private key.
///
/// `local_secret_key` is the 32-byte Private Key owned by the local device.
...
///
/// Returns the calculated anti-spoofing Account Key on success, or Error otherwise.
///
/// Note: While the `elliptic_curve` crate recommends "[w]henever possible, [use] the high-level
/// ECDH ephemeral API provided by EphemeralSecret" instead of this `diffie_hellman` function, it
/// makes an exception for "implementing a protocol which requires a static scalar value as part of
/// an ECDH exchange". While the remote public key from the Seeker will vary, the `local_secret_key`
/// will always be the FastPair Anti-Spoofing Private key assigned at model registration.
// TODO(fxbug.dev/96222): Remove this when this is used in the key-based pairing procedure impl.
#[allow(unused)]
pub fn calculate_anti_spoofing_key(
...
mod tests {
    ...
    fn ecdh_key_exchange() {
        let remote_public_key = public_key_from_bytes(vec![
        ...
        let local_private_key = private_key_from_bytes(vec![
            0xD7, 0x5E, 0x54, 0xC7, 0x7D, 0x76, 0x24, 0x89, 0xE5, 0x7C, 0xFA, 0x92, 0x37, 0x43,
            0xF1, 0x67, 0x77, 0xA4, 0x28, 0x3D, 0x99, 0x80, 0x0B, 0xAC, 0x55, 0x58, 0x48, 0x38,
            0x93, 0xE5, 0xB0, 0x6D,
        ])
    ...
    fn aes_key_generation_from_public_private_keys() {
        ...
        let calculated_aes_key =
            calculate_anti_spoofing_key(&local_private_key, &remote_public_key)
                .expect("valid calculation");

        let expected_aes_key = AccountKey::new([
            0xB0, 0x7F, 0x1F, 0x17, 0xC2, 0x36, 0xCB, 0xD3, 0x35, 0x23, 0xC5, 0x15, 0xF3, 0x50,
            0xAE, 0x57,
        ]);
        assert_eq!(calculated_aes_key, expected_aes_key);
    }
