...
use crate::bindings::{
    socket::{IntoErrno, IpSockAddrExt, SockAddr as _, ZXSIO_SIGNAL_CONNECTED},
    LockableContext,
};
...
use netstack3_core::{
    ...
    BufferIpSocketHandler, Ctx, IpExt, SyncCtx, TransportIpContext,
};
                    ...
                    fposix_socket::StreamSocketRequest::Accept { want_addr, responder } => {
                        let mut result = (|| match self.id {
                            SocketId::Listener(listener) => {
                                ...
                                worker
                                    .local
                                    .signal_peer(zx::Signals::NONE, zx::Signals::USER_3)
