...
mod tests {
    ...
    fn test_send_neighbor_solicitation_on_cache_miss() {
        ...
        for packet_num in 0..usize::from(MAX_MULTICAST_SOLICIT) {
            assert_eq!(ctx.sync_ctx.dispatcher.frames_sent().len(), packet_num + 1);
    ...
    fn test_address_resolution() {
        ...
        .unwrap();
        assert_empty(net.context("local").sync_ctx.dispatcher.frames_sent());
        assert_empty(net.context("remote").sync_ctx.dispatcher.frames_sent());
        ...
        // local.
        assert_eq!(net.context("local").sync_ctx.dispatcher.frames_sent().len(), 1);
        // A timer should've been started.
        assert_eq!(net.context("local").ctx.timer_ctx().timers().len(), 1);
        ...
        assert_eq!(net.context("remote").sync_ctx.dispatcher.frames_sent().len(), 1);
        ...
        assert_empty(net.context("local").ctx.timer_ctx().timers());
    ...
    fn test_dad_duplicate_address_detected_solicitation() {
        ...
        assert_eq!(
            crate::add_ethernet_device(net.context("local"), mac, Ipv6::MINIMUM_LINK_MTU.into(),),
            device_id
        );
        crate::device::update_ipv6_configuration(net.context("local"), device_id, update);
        assert_eq!(
            crate::add_ethernet_device(net.context("remote"), mac, Ipv6::MINIMUM_LINK_MTU.into(),),
            device_id
        );
    ...
    fn test_dad_three_transmits_with_conflicts() {
        ...
        assert_eq!(net.context("local").sync_ctx.dispatcher.frames_sent().len(), 3);
        assert_eq!(net.context("remote").sync_ctx.dispatcher.frames_sent().len(), 1);
        ...
        assert_empty(net.context("local").ctx.timer_ctx().timers());
        assert_empty(net.context("remote").ctx.timer_ctx().timers());
    ...
    fn test_dad_multiple_ips_simultaneously() {
        ...
        assert_empty(ctx.sync_ctx.dispatcher.frames_sent());
        ...
        assert!(get_ipv6_device_state(&ctx, dev_id)
            ...
            .state
            .is_tentative());
        assert_eq!(ctx.sync_ctx.dispatcher.frames_sent().len(), 3);
        ...
        assert!(get_ipv6_device_state(&ctx, dev_id)
            ...
            .state
            .is_tentative());
        assert_eq!(ctx.sync_ctx.dispatcher.frames_sent().len(), 6);
    ...
    fn test_dad_cancel_when_ip_removed() {
        ...
        assert!(get_ipv6_device_state(&ctx, dev_id)
            ...
            .state
            .is_tentative());
        assert_eq!(ctx.sync_ctx.dispatcher.frames_sent().len(), 5);
    ...
    fn test_sending_ipv6_packet_after_hop_limit_change() {
        ...
        fn inner_test(ctx: &mut crate::testutil::DummyCtx, hop_limit: u8, frame_offset: usize) {
            ...
            .unwrap();
            let (buf, _, _, _) =
                parse_ethernet_frame(&ctx.sync_ctx.dispatcher.frames_sent()[frame_offset].1[..])
                    .unwrap();
    ...
    fn test_host_send_router_solicitations() {
        ...
        let (src_mac, _, src_ip, _, _, message, code) =
            parse_icmp_packet_in_ip_packet_in_ethernet_frame::<Ipv6, _, RouterSolicitation, _>(
                &ctx.sync_ctx.dispatcher.frames_sent()[1].1,
                |_| {},
            )
            .unwrap();
        validate_params(src_mac, src_ip, message, code);
        ...
        let (src_mac, _, src_ip, _, _, message, code) =
            parse_icmp_packet_in_ip_packet_in_ethernet_frame::<Ipv6, _, RouterSolicitation, _>(
                &ctx.sync_ctx.dispatcher.frames_sent()[2].1,
                |p| {
        ...
        for f in ctx.sync_ctx.dispatcher.frames_sent() {
            let (src_mac, _, src_ip, _, _, message, code) =
                parse_icmp_packet_in_ip_packet_in_ethernet_frame::<Ipv6, _, RouterSolicitation, _>(
                    &f.1,
                    |_| {},
    ...
    fn test_host_generate_temporary_slaac_address(
        ...
        let temporary_slaac_addresses =
            iter_global_ipv6_addrs(&ctx, &mut (), device.try_into().unwrap())
