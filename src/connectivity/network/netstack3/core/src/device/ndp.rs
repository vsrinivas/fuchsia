...
impl<D: LinkDevice, C, SC: NdpContext<D, C>> NdpHandler<D, C> for SC
where
...
pub(crate) trait NdpContext<D: LinkDevice, C>:
    Sized
    + DeviceIdContext<D>
    + RngContext
    + CounterContext
    ...
    + TimerContext<NdpTimerId<D, <Self as DeviceIdContext<D>>::DeviceId>>
{
    ...
    // L3.
    fn get_retrans_timer(&self, ctx: &mut C, device_id: Self::DeviceId) -> NonZeroDuration;
    ...
    fn is_router_device(&self, ctx: &mut C, device_id: Self::DeviceId) -> bool {
        self.get_ip_device_state(ctx, device_id).routing_enabled
    }
}

fn deinitialize<D: LinkDevice, C, SC: NdpContext<D, C>>(
    sync_ctx: &mut SC,
    _ctx: &mut C,
...
fn handle_timer<D: LinkDevice, C, SC: NdpContext<D, C>>(
    sync_ctx: &mut SC,
    ctx: &mut C,
            ...
            }) = ndp_state.neighbors.get_neighbor_state_mut(&neighbor_addr)
            {
                if *transmit_counter < MAX_MULTICAST_SOLICIT {
                    ...
                    *transmit_counter += 1;
                    ...
                    let retrans_timer = sync_ctx.get_retrans_timer(ctx, id.device_id);
...
fn lookup<D: LinkDevice, C, SC: NdpContext<D, C>>(
    sync_ctx: &mut SC,
    ctx: &mut C,
    device_id: SC::DeviceId,
    lookup_addr: UnicastAddr<Ipv6Addr>,
    ...
    D::Address: for<'a> From<&'a MulticastAddr<Ipv6Addr>>,
{
    ...
    match result {
        ...
        None => {
            ...
            // advertisements back.
            let retrans_timer = sync_ctx.get_retrans_timer(ctx, device_id);
            let _: Option<SC::Instant> = sync_ctx.schedule_timer(
                retrans_timer.get(),
...
#[cfg(test)]
fn insert_neighbor<D: LinkDevice, C, SC: NdpContext<D, C>>(
    sync_ctx: &mut SC,
    _ctx: &mut C,
    device_id: SC::DeviceId,
    net: UnicastAddr<Ipv6Addr>,
...
fn send_neighbor_solicitation<D: LinkDevice, C, SC: NdpContext<D, C>>(
    sync_ctx: &mut SC,
    ctx: &mut C,
    device_id: SC::DeviceId,
    lookup_addr: UnicastAddr<Ipv6Addr>,
    ...
    if let Some(src_ip) = sync_ctx.get_non_tentative_global_or_link_local_addr(ctx, device_id) {
        assert!(src_ip.is_valid_unicast());
        let src_ll = sync_ctx.get_link_layer_addr(ctx, device_id);
...
fn send_neighbor_advertisement<D: LinkDevice, C, SC: NdpContext<D, C>>(
    sync_ctx: &mut SC,
    ctx: &mut C,
    device_id: SC::DeviceId,
    solicited: bool,
    ...
    let is_router_device = sync_ctx.is_router_device(ctx, device_id);
    let _ = send_ndp_packet::<_, _, _, &[u8], _>(
        sync_ctx,
        ctx,
        device_id,
...
// errors don't happen?
pub(super) fn send_ndp_packet<D: LinkDevice, C, SC: NdpContext<D, C>, B: ByteSlice, M>(
    sync_ctx: &mut SC,
    ctx: &mut C,
    device_id: SC::DeviceId,
    src_ip: Ipv6Addr,
...
pub(crate) fn receive_ndp_packet<D: LinkDevice, C, SC: NdpContext<D, C>, B>(
    sync_ctx: &mut SC,
    ctx: &mut C,
    device_id: SC::DeviceId,
    src_ip: Ipv6SourceAddr,
    ...
    B: ByteSlice,
{
    ...
    match packet {
        NdpPacket::RouterSolicitation(p) => {
            ...
            if !sync_ctx.is_router_device(ctx, device_id) {
        ...
        NdpPacket::RouterAdvertisement(p) => {
            ...
            if let Some(hop_limit) = ra.current_hop_limit() {
                ...
                sync_ctx.get_ip_device_state_mut(ctx, device_id).default_hop_limit = hop_limit;
            }
        ...
        NdpPacket::NeighborAdvertisement(p) => {
            ...
            if neighbor_state.is_incomplete() {
                ...
                if let Some(address) = target_ll {
                    ...
                    let _: Option<SC::Instant> = sync_ctx.cancel_timer(
...
mod tests {
    ...
    fn test_send_neighbor_solicitation_on_cache_miss() {
        ...
        crate::device::testutil::enable_device(&mut sync_ctx, dev_id);
        // Now we have to manually assign the IP addresses, see
        // `EthernetLinkDevice::get_ipv6_addr`
        add_ip_addr_subnet(&mut sync_ctx, dev_id, AddrSubnet::new(local_ip().get(), 128).unwrap())
            .unwrap();
    ...
    fn test_address_resolution() {
        ...
        .unwrap();
        assert_empty(net.sync_ctx("local").dispatcher.frames_sent());
        assert_empty(net.sync_ctx("remote").dispatcher.frames_sent());
        ...
        // local.
        assert_eq!(net.sync_ctx("local").dispatcher.frames_sent().len(), 1);
        // A timer should've been started.
        assert_eq!(net.sync_ctx("local").ctx.timer_ctx().timers().len(), 1);
        ...
        assert_eq!(net.sync_ctx("remote").dispatcher.frames_sent().len(), 1);
        ...
        assert_empty(net.sync_ctx("local").ctx.timer_ctx().timers());
    ...
    fn test_deinitialize_cancels_timers() {
        ...
        assert_eq!(
            lookup::<EthernetLinkDevice, _, _>(&mut sync_ctx, &mut (), device_id, remote_ip()),
            None
        );
        ...
        deinitialize(&mut sync_ctx, &mut (), other_id);
        sync_ctx.ctx.timer_ctx().assert_timers_installed([(timer_id, ..)]);

        // Deinitializing the correct ID should cancel the timer.
        deinitialize(&mut sync_ctx, &mut (), dev_id.try_into().expect("expected ethernet ID"));
    ...
    fn test_dad_duplicate_address_detected_solicitation() {
        ...
        assert_eq!(
            crate::add_ethernet_device(net.sync_ctx("local"), mac, Ipv6::MINIMUM_LINK_MTU.into(),),
            device_id
        );
        crate::device::update_ipv6_configuration(net.sync_ctx("local"), device_id, update);
        assert_eq!(
            crate::add_ethernet_device(net.sync_ctx("remote"), mac, Ipv6::MINIMUM_LINK_MTU.into(),),
            device_id
        );
        crate::device::update_ipv6_configuration(net.sync_ctx("remote"), device_id, update);
        assert_eq!(net.sync_ctx("local").dispatcher.frames_sent().len(), 1);
        assert_eq!(net.sync_ctx("remote").dispatcher.frames_sent().len(), 1);
    ...
    fn test_dad_duplicate_address_detected_advertisement() {
        ...
        add_ip_addr_subnet(net.sync_ctx("local"), device_id, addr).unwrap();
        ...
        add_ip_addr_subnet(net.sync_ctx("remote"), device_id, addr).unwrap();
    ...
    fn test_dad_set_ipv6_address_when_ongoing() {
        ...
        let dev_id =
            crate::add_ethernet_device(&mut sync_ctx, local_mac(), Ipv6::MINIMUM_LINK_MTU.into());
        crate::device::update_ipv6_configuration(&mut sync_ctx, dev_id, |config| {
            config.ip_config.ip_enabled = true;
            config.dad_transmits = NonZeroU8::new(1);
        });
        let addr = local_ip();
        add_ip_addr_subnet(&mut sync_ctx, dev_id, AddrSubnet::new(addr.get(), 128).unwrap())
            .unwrap();
        assert_eq!(
    ...
    fn test_dad_three_transmits_no_conflicts() {
        let Ctx { mut sync_ctx } = Ctx::default();
        let dev_id =
            crate::add_ethernet_device(&mut sync_ctx, local_mac(), Ipv6::MINIMUM_LINK_MTU.into());
        crate::device::testutil::enable_device(&mut sync_ctx, dev_id);
    ...
    fn test_dad_three_transmits_with_conflicts() {
        ...
        assert_eq!(net.sync_ctx("local").dispatcher.frames_sent().len(), 3);
        assert_eq!(net.sync_ctx("remote").dispatcher.frames_sent().len(), 1);
        ...
        assert_empty(net.sync_ctx("local").ctx.timer_ctx().timers());
        assert_empty(net.sync_ctx("remote").ctx.timer_ctx().timers());
    ...
    fn test_dad_multiple_ips_simultaneously() {
        ...
        crate::device::update_ipv6_configuration(&mut sync_ctx, dev_id, |ipv6_config| {
            ipv6_config.ip_config.ip_enabled = true;
            ipv6_config.dad_transmits = NonZeroU8::new(3);
        ...
        // Add another IP
        add_ip_addr_subnet(&mut sync_ctx, dev_id, AddrSubnet::new(remote_ip().get(), 128).unwrap())
            .unwrap();
        assert!(get_ipv6_device_state(&sync_ctx, dev_id)
            .find_addr(&local_ip())
            .unwrap()
    ...
    fn test_dad_cancel_when_ip_removed() {
        ...
        assert!(get_ipv6_device_state(&sync_ctx, dev_id)
            ...
            .state
            .is_tentative());
        assert_eq!(sync_ctx.dispatcher.frames_sent().len(), 5);

        // Remove local ip
        del_ip_addr(&mut sync_ctx, dev_id, &local_ip().into_specified()).unwrap();
    ...
    fn test_sending_ipv6_packet_after_hop_limit_change() {
        ...
        fn inner_test(
            ...
            .unwrap();
            let (buf, _, _, _) =
                parse_ethernet_frame(&sync_ctx.dispatcher.frames_sent()[frame_offset].1[..])
                    .unwrap();
    ...
    fn test_receiving_router_advertisement_mtu_option() {
        ...
        let Ctx { mut sync_ctx } = DummyEventDispatcherBuilder::default().build();
        let hw_mtu = 5000;
        let device = crate::add_ethernet_device(&mut sync_ctx, local_mac(), hw_mtu);
        let src_mac = Mac::new([10, 11, 12, 13, 14, 15]);
        let src_ip = src_mac.to_ipv6_link_local().addr();
    ...
    fn test_host_send_router_solicitations() {
        ...
        let Ctx { mut sync_ctx } = Ctx::<DummyEventDispatcher, _>::default();
        ...
        crate::device::update_ipv6_configuration(&mut sync_ctx, device_id, |config| {
            config.ip_config.ip_enabled = true;
        ...
        assert!(sync_ctx.now().duration_since(time) < MAX_RTR_SOLICITATION_DELAY);
        assert_eq!(sync_ctx.dispatcher.frames_sent().len(), 1);
        ...
        let (src_mac, _, src_ip, _, _, message, code) =
            parse_icmp_packet_in_ip_packet_in_ethernet_frame::<Ipv6, _, RouterSolicitation, _>(
                &sync_ctx.dispatcher.frames_sent()[1].1,
                |_| {},
            )
            .unwrap();
        validate_params(src_mac, src_ip, message, code);
        ...
        let (src_mac, _, src_ip, _, _, message, code) =
            parse_icmp_packet_in_ip_packet_in_ethernet_frame::<Ipv6, _, RouterSolicitation, _>(
                &sync_ctx.dispatcher.frames_sent()[2].1,
                |p| {
        ...
        for f in sync_ctx.dispatcher.frames_sent() {
            let (src_mac, _, src_ip, _, _, message, code) =
                parse_icmp_packet_in_ip_packet_in_ethernet_frame::<Ipv6, _, RouterSolicitation, _>(
                    &f.1,
                    |_| {},
    ...
    fn test_router_solicitation_on_routing_enabled_changes() {
        ...
        assert_empty(sync_ctx.dispatcher.frames_sent());
        assert_empty(sync_ctx.ctx.timer_ctx().timers());
        ...
        crate::device::update_ipv6_configuration(&mut sync_ctx, device, |config| {
            config.ip_config.ip_enabled = true;
        ...
        set_ipv6_routing_enabled(&mut sync_ctx, &mut (), device, true)
        ...
        // solicitation timer.
        assert_eq!(sync_ctx.dispatcher.frames_sent().len(), 1);
        assert_empty(sync_ctx.ctx.timer_ctx().timers().iter().filter(|x| x.1 == timer_id));

        // Unsetting routing should succeed.
        set_ipv6_routing_enabled(&mut sync_ctx, &mut (), device, false)
            .expect("error setting routing enabled");
        assert!(!is_ipv6_routing_enabled(&sync_ctx, device));
        assert_eq!(sync_ctx.dispatcher.frames_sent().len(), 1);
        sync_ctx.ctx.timer_ctx().assert_timers_installed([(timer_id, ..)]);
    ...
    fn test_set_ndp_config_dup_addr_detect_transmits() {
        ...
        crate::device::update_ipv6_configuration(&mut sync_ctx, device, |ipv6_config| {
            ipv6_config.ip_config.ip_enabled = true;
        ...
        assert_eq!(sync_ctx.dispatcher.frames_sent().len(), 1);
        assert_eq!(sync_ctx.ctx.timer_ctx().timers().len(), 1);
        ...
        add_ip_addr_subnet(&mut sync_ctx, device, AddrSubnet::new(new_ip.get(), 128).unwrap())
            .unwrap();
        assert_eq!(
            NdpContext::<EthernetLinkDevice, ()>::get_ip_device_state(
                &sync_ctx,
    ...
    fn iter_global_ipv6_addrs<'a, D: LinkDevice, C, SC: NdpContext<D, C>>(
        sync_ctx: &'a SC,
        ctx: &mut C,
        device_id: SC::DeviceId,
    ) -> impl Iterator<Item = &'a Ipv6AddressEntry<SC::Instant>> {
        sync_ctx.get_ip_device_state(ctx, device_id).iter_addrs().filter(|entry| {
    ...
    fn test_router_stateless_address_autoconfiguration() {
        ...
        assert_empty(iter_global_ipv6_addrs(&sync_ctx, &mut (), device.try_into().unwrap()));
    ...
    fn initialize_with_temporary_addresses_enabled(
        ...
        let device =
            crate::add_ethernet_device(sync_ctx, config.local_mac, Ipv6::MINIMUM_LINK_MTU.into());
        crate::device::testutil::enable_device(sync_ctx, device);
        ...
        crate::ip::device::update_ipv6_configuration(sync_ctx, &mut (), device, |ipv6_config| {
    ...
    fn test_host_stateless_address_autoconfiguration_multiple_prefixes() {
        let (Ctx { mut sync_ctx }, device, _): (_, _, SlaacConfiguration) =
        ...
        let (prefix_1_static, prefix_1_temporary) = {
            let slaac_configs =
                iter_global_ipv6_addrs(&sync_ctx, &mut (), device.try_into().unwrap())
    ...
    fn test_host_generate_temporary_slaac_address(
        ...
        let interface_identifier = generate_opaque_interface_identifier(
            ...
            OpaqueIidNonce::Random(sync_ctx.ctx.rng().clone().next_u64()),
        ...
        assert!(valid_until <= sync_ctx.now().checked_add(max_valid_lifetime).unwrap());
    ...
    fn test_host_temporary_slaac_and_manual_addresses_conflict() {
        ...
        let conflicted_addr = {
            let (Ctx { mut sync_ctx }, device, _config) =
                initialize_with_temporary_addresses_enabled();
        ...
        let (Ctx { mut sync_ctx }, device, _config) = initialize_with_temporary_addresses_enabled();
        let device_id = device.try_into().unwrap();
        crate::device::add_ip_addr_subnet(&mut sync_ctx, device, conflicted_addr.to_witness())
            .expect("adding address failed");

        // Sanity check: `conflicted_addr` is already assigned on the device.
        assert_matches!(
            iter_global_ipv6_addrs(&sync_ctx, &mut (), device_id)
    ...
    fn test_host_slaac_invalid_prefix_information() {
        ...
        let device_id = device.try_into().unwrap();
        assert_empty(iter_global_ipv6_addrs(&sync_ctx, &mut (), device_id));
    ...
    fn test_host_slaac_address_deprecate_while_tentative() {
        ...
        assert_eq!(
            iter_global_ipv6_addrs(&sync_ctx, &mut (), device_id).collect::<Vec<_>>(),
    ...
    fn get_matching_slaac_address_entries<F: FnMut(&&Ipv6AddressEntry<DummyInstant>) -> bool>(
        ...
        iter_global_ipv6_addrs(sync_ctx, &mut (), device.try_into().unwrap()).filter(filter)
    }
    ...
    fn test_host_static_slaac_valid_lifetime_updates() {
        ...
        fn inner_test(
            ...
            assert_slaac_lifetimes_enforced(sync_ctx, device, entry, valid_until, preferred_until);
        }
    ...
    fn test_host_temporary_slaac_regenerates_address_on_dad_failure() {
        ...
        assert_matches!(second_addr_entry.config, AddrConfig::Slaac(SlaacConfig::Temporary(
            ...
            assert_eq!(creation_time, sync_ctx.now());
    ...
    fn test_host_temporary_slaac_deprecate_before_regen() {
        ...
        for address in &addresses {
            assert_matches!(
                sync_ctx.scheduled_instant(SlaacTimerId::new_deprecate_slaac_address(
                    device,
                    *address,
                )),
                Some(deprecate_at) => {
                    let preferred_for = deprecate_at - sync_ctx.now();
    ...
    fn test_host_temporary_slaac_config_update_skips_regen() {
        ...
        // immediately.
        let prefix_preferred_for = preferred_until - sync_ctx.now();
    ...
    fn test_host_temporary_slaac_lifetime_updates_respect_max() {
        ...
        let src_ip = src_mac.to_ipv6_link_local().addr().get();
        let subnet = subnet_v6!("0102:0304:0506:0708::/64");
        let (Ctx { mut sync_ctx }, device, config) = initialize_with_temporary_addresses_enabled();
        let now = sync_ctx.now();
        let start = now;
    ...
    fn test_remove_stable_slaac_address() {
        ...
        del_ip_addr(&mut sync_ctx, device, &expected_addr.into_specified()).unwrap();
        assert_empty(iter_global_ipv6_addrs(&sync_ctx, &mut (), device_id));
        sync_ctx.ctx.timer_ctx().assert_no_timers_installed();
    }
    ...
    fn test_remove_temporary_slaac_address() {
        ...
        let (Ctx { mut sync_ctx }, device, expected_addr) =
            test_host_generate_temporary_slaac_address(INFINITE_LIFETIME, INFINITE_LIFETIME);

        // Deleting the address should cancel its SLAAC timers.
        del_ip_addr(&mut sync_ctx, device, &expected_addr.into_specified()).unwrap();
        assert_empty(
            iter_global_ipv6_addrs(&sync_ctx, &mut (), device.try_into().unwrap()).filter(|e| {
                match e.config {
                    AddrConfig::Slaac(SlaacConfig::Temporary(_)) => true,
                    AddrConfig::Slaac(SlaacConfig::Static { valid_until: _ }) => false,
                    AddrConfig::Manual => false,
