...
mod types {
    ...
    ///  option data itself. While kind-byte-only options are supported, all
    ///  such kinds are handled by the utilities in `wire::records::options`, so
    ///  this type only supports options with variable-length data.
    ///
