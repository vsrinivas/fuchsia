...
mod tests {
    ...
    use crate::{
        context::testutil::{
            DummyCtx, DummyInstant, DummyNetwork, DummyNetworkLinks, InstantAndData,
    ...
    impl<I: TcpTestIpExt> TcpBufferContext for TcpCtx<I> {
        type ReceiveBuffer = RingBuffer;
        type SendBuffer = RingBuffer;
    }

    impl<I: TcpTestIpExt> TransportIpContext<I, ()> for TcpCtx<I>
    where
        TcpCtx<I>: IpSocketHandler<I, ()>,
    {
    ...
    fn handle_frame<I: TcpTestIpExt>(
        sync_ctx: &mut TcpCtx<I>,
        meta: SendIpPacketMeta<I, DummyDeviceId, SpecifiedAddr<I::Addr>>,
        buffer: Buf<Vec<u8>>,
    ) where
        TcpCtx<I>: BufferIpSocketHandler<I, (), Buf<Vec<u8>>>
    ...
    {
        set_logger_for_test();
        let mut net = new_test_net();

        let server = create_socket(net.context(REMOTE), &mut ());
        let client = create_socket(net.context(LOCAL), &mut ());
        let server =
            bind(net.context(REMOTE), &mut (), server, *I::DUMMY_CONFIG.remote_ip, Some(PORT_1))
                .expect("failed to bind the servekr socket");
        let client =
            bind(net.context(LOCAL), &mut (), client, *I::DUMMY_CONFIG.local_ip, Some(PORT_1))
                .expect("failed to bind the client socket");
        let backlog = NonZeroUsize::new(1).unwrap();
        let server = listen(net.context(REMOTE), &mut (), server, backlog);
        let client = connect(
            net.context(LOCAL),
            &mut (),
            client,
        ...
        assert_matches!(
            net.context(REMOTE).get_mut().sockets.get_listener_by_id_mut(server),
        ...
        assert_matches!(accept(net.context(REMOTE), &mut (), server), Err(AcceptError::WouldBlock));

        // Step the test network until the handshake is done.
        net.run_until_idle(handle_frame, panic_if_any_timer);
        let (accepted, addr) =
            accept(net.context(REMOTE), &mut (), server).expect("failed to accept");
    ...
    {
        set_logger_for_test();
        let mut ctx = TcpCtx::with_state(TcpState::new(
            I::DUMMY_CONFIG.local_ip,
            I::DUMMY_CONFIG.local_ip,
            I::DUMMY_CONFIG.subnet.prefix(),
        ));
        let s1 = create_socket(&mut ctx, &mut ());
        let s2 = create_socket(&mut ctx, &mut ());

        let _b1 = bind(&mut ctx, &mut (), s1, *I::DUMMY_CONFIG.local_ip, Some(PORT_1))
            .expect("first bind should succeed");
        assert_matches!(
            bind(&mut ctx, &mut (), s2, *I::DUMMY_CONFIG.local_ip, Some(PORT_1)),
            Err(BindError::Conflict(_))
        );
        let _b2 = bind(&mut ctx, &mut (), s2, *I::DUMMY_CONFIG.local_ip, Some(PORT_2))
            ...
            + IpDeviceIdContext<I, DeviceId = DummyDeviceId>,
    {
        ...
        let _b1 = bind(&mut ctx, &mut (), s1, *I::DUMMY_CONFIG.local_ip, Some(PORT_1))
            .expect("first bind should succeed");
        assert_matches!(
            bind(&mut ctx, &mut (), s2, I::UNSPECIFIED_ADDRESS, Some(PORT_1)),
            Err(BindError::Conflict(_))
        );
        let _b2 = bind(&mut ctx, &mut (), s2, I::UNSPECIFIED_ADDRESS, Some(PORT_2))
            .expect("able to rebind to a free address");
    }
