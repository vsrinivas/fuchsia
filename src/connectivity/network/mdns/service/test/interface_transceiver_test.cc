...
class MdnsInterfaceTransceiverTest : public MdnsInterfaceTransceiver {
 public:
  MdnsInterfaceTransceiverTest(inet::IpAddress address, const std::string& name,
                               uint32_t index)
    ...
    for (size_t i = 0; i < send_to_size_; ++i) {
      ...
      std::cout << "0x" << std::hex << std::setw(2) << std::setfill('0')
                << static_cast<uint16_t>(
                       reinterpret_cast<const uint8_t*>(send_to_buffer_)[i])
                << std::dec << ", ";
    }
...
TEST(InterfaceTransceiverTest, SendSimpleMessage) {
  ...
  inet::SocketAddress to_address(inet::IpAddress(4, 3, 2, 1),
                                 inet::IpPort::From_uint16_t(4321));

  MdnsInterfaceTransceiverTest under_test(nic_address, nic_name, nic_index);

  auto ptr_resource =
      std::make_shared<DnsResource>("_test_name._whatever.", DnsType::kPtr);
  ptr_resource->time_to_live_ = 234;
  ptr_resource->ptr_.pointer_domain_name_ =
      DnsName("_test_ptr_name._whatever.");
  ...
  std::vector<uint8_t> expected_message = {
      ...
      0x01, 0x0a, 0x5f, 0x74, 0x65, 0x73, 0x74, 0x5f, 0x6e, 0x61, 0x6d,
      0x65, 0x09, 0x5f, 0x77, 0x68, 0x61, 0x74, 0x65, 0x76, 0x65, 0x72,
      0x00, 0x00, 0x0c, 0x00, 0x01, 0x00, 0x00, 0x00, 0xea, 0x00, 0x11,
      0x0e, 0x5f, 0x74, 0x65, 0x73, 0x74, 0x5f, 0x70, 0x74, 0x72, 0x5f,
      0x6e, 0x61, 0x6d, 0x65, 0xc0, 0x17};

  EXPECT_EQ(expected_message.size(), under_test.send_to_size_);
  EXPECT_EQ(0, memcmp(expected_message.data(), under_test.send_to_buffer_,
                      under_test.send_to_size_));
...
TEST(InterfaceTransceiverTest, SendLeadingA) {
  ...
  auto a_resource =
      std::make_shared<DnsResource>("_test_a_name._whatever.", DnsType::kA);
  ...
  std::vector<uint8_t> expected_message = {
      ...
      0x0a, 0x5f, 0x74, 0x65, 0x73, 0x74, 0x5f, 0x6e, 0x61, 0x6d, 0x65, 0x09,
      0x5f, 0x77, 0x68, 0x61, 0x74, 0x65, 0x76, 0x65, 0x72, 0x00, 0x00, 0x0c,
      0x00, 0x01, 0x00, 0x00, 0x00, 0xea, 0x00, 0x11, 0x0e, 0x5f, 0x74, 0x65,
      0x73, 0x74, 0x5f, 0x70, 0x74, 0x72, 0x5f, 0x6e, 0x61, 0x6d, 0x65, 0xc0,
      0x17, 0x0c, 0x5f, 0x74, 0x65, 0x73, 0x74, 0x5f, 0x61, 0x5f, 0x6e, 0x61,
      0x6d, 0x65, 0xc0, 0x17, 0x00, 0x01, 0x80, 0x01, 0x00, 0x00, 0x00, 0x78,
...
TEST(InterfaceTransceiverTest, SendLeadingAAndAAAA) {
  ...
  auto aaaa_resource =
      std::make_shared<DnsResource>("_test_a_name._whatever.", DnsType::kAaaa);
...
TEST(InterfaceTransceiverTest, SendLeadingAWithAlternate) {
  ...
  std::vector<uint8_t> expected_message = {
      ...
      0x17, 0x0c, 0x5f, 0x74, 0x65, 0x73, 0x74, 0x5f, 0x61, 0x5f, 0x6e, 0x61,
      0x6d, 0x65, 0xc0, 0x17, 0x00, 0x01, 0x80, 0x01, 0x00, 0x00, 0x00, 0x78,
      0x00, 0x04, 0x01, 0x02, 0x03, 0x04, 0xc0, 0x3d, 0x00, 0x1c, 0x80, 0x01,
      0x00, 0x00, 0x00, 0x78, 0x00, 0x10, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00,
