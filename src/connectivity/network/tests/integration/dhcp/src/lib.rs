...
use dhcp::protocol::{FromFidlExt as _, IntoFidlExt as _};
...
struct DhcpTestConfig<'a> {
    ...
    // Prefix length of the server subnet.
    prefix_len: u8,

    // IPv4 address at start of the pool range.
    pool_range_start: fidl_fuchsia_net::Ipv4Address,

    // IPv4 address at end of the pool range.
    pool_range_stop: fidl_fuchsia_net::Ipv4Address,
...
impl DhcpTestConfig<'_> {
    fn address_pool(self) -> Result<fidl_fuchsia_net_dhcp::Parameter> {
        let Self { server_addr, prefix_len, pool_range_start, pool_range_stop, .. } = self;
        let subnet_mask = dhcp::configuration::SubnetMask::try_from(prefix_len)?;

        Ok(fidl_fuchsia_net_dhcp::Parameter::AddressPool(fidl_fuchsia_net_dhcp::AddressPool {
            network_id: Some(
                subnet_mask.apply_to(&std::net::Ipv4Addr::from_fidl(server_addr)).into_fidl(),
            ),
            broadcast: Some(
                subnet_mask.broadcast_of(&std::net::Ipv4Addr::from_fidl(server_addr)).into_fidl(),
            ),
            mask: Some(subnet_mask.into_fidl()),
    ...
    fn expected_acquired(self) -> fidl_fuchsia_net::Subnet {
        let Self { prefix_len, pool_range_start, .. } = self;
        fidl_fuchsia_net::Subnet {
            addr: fidl_fuchsia_net::IpAddress::Ipv4(pool_range_start),
            prefix_len: prefix_len,
        }
    }

    fn server_subnet(self) -> fidl_fuchsia_net::Subnet {
        let Self { server_addr, prefix_len, .. } = self;
        fidl_fuchsia_net::Subnet {
    ...
    fn dhcp_parameters(self) -> Result<Vec<fidl_fuchsia_net_dhcp::Parameter>> {
        let Self { server_addr, netemul_device_name, .. } = self;
        let address_pool = self.address_pool()?;
        Ok(vec![
...
const DEFAULT_CONFIG: DhcpTestConfig<'static> = DhcpTestConfig {
    server_addr: fidl_ip_v4!("192.168.0.1"),
    prefix_len: 25,
    pool_range_start: fidl_ip_v4!("192.168.0.2"),
    pool_range_stop: fidl_ip_v4!("192.168.0.5"),
    netemul_device_name: "eth2",
};

const ALT_CONFIG: DhcpTestConfig<'static> = DhcpTestConfig {
    server_addr: fidl_ip_v4!("192.168.1.1"),
    prefix_len: 24,
    pool_range_start: fidl_ip_v4!("192.168.1.2"),
    pool_range_stop: fidl_ip_v4!("192.168.1.5"),
...
async fn acquire_dhcp_with_dhcpd_bound_device<E: netemul::Endpoint>(name: &str) -> Result {
    let mut parameters = DEFAULT_CONFIG.dhcp_parameters()?;
    test_dhcp::<E>(
        name,
        &mut [DhcpTestNetwork {
...
async fn acquire_dhcp_then_renew_with_dhcpd_bound_device<E: netemul::Endpoint>(
    ...
    let mut dhcp_parameters = {
        // Use shortest lease length (4s) that provides distinct T1 and T2
        // values, i.e. 2s and 3s respectively.
        let lease = fidl_fuchsia_net_dhcp::Parameter::Lease(fidl_fuchsia_net_dhcp::LeaseLength {
            default: Some(4),
            max: Some(4),
            ..fidl_fuchsia_net_dhcp::LeaseLength::EMPTY
        });
        let mut dhcp_parameters = DEFAULT_CONFIG.dhcp_parameters()?;
        let () = dhcp_parameters.push(lease);
        dhcp_parameters
    };
    test_dhcp::<E>(
        name,
...
async fn acquire_dhcp_with_dhcpd_bound_device_dup_addr<E: netemul::Endpoint>(name: &str) -> Result {
    let expected_acquired = DEFAULT_CONFIG.expected_acquired();
...
async fn acquire_dhcp_with_multiple_network<E: netemul::Endpoint>(name: &str) -> Result {
    let mut default_parameters = DEFAULT_CONFIG.dhcp_parameters()?;
    let mut alt_parameters = ALT_CONFIG.dhcp_parameters()?;
    test_dhcp::<E>(
        name,
        &mut [
            DhcpTestNetwork {
        ...
        &mut [&mut default_parameters, &mut alt_parameters],
        1,
        false,
    )
    .await
...
impl PersistenceMode {
    ...
    fn dhcpd_params_after_restart(
        ...
        let params = test_dhcpd_params()?;
        Ok(match self {
            Self::Persistent => params.into_iter().map(|p| (param_name(&p), p)).collect(),
            Self::Ephemeral => vec![
...
fn test_dhcpd_params() -> Result<Vec<fidl_fuchsia_net_dhcp::Parameter>> {
    let address_pool = DEFAULT_CONFIG.address_pool()?;
    Ok(vec![
        fidl_fuchsia_net_dhcp::Parameter::IpAddrs(vec![DEFAULT_CONFIG.server_addr]),
        address_pool,
        fidl_fuchsia_net_dhcp::Parameter::Lease(fidl_fuchsia_net_dhcp::LeaseLength {
            default: Some(60),
            max: Some(60),
...
async fn acquire_dhcp_server_after_restart<E: netemul::Endpoint>(
    ...
    let _server_ep = server_env
        .join_network::<E, _>(
            &network,
            "server-ep",
            &netemul::InterfaceConfig::StaticIp(DEFAULT_CONFIG.server_subnet()),
        )
        .await
    ...
    // persistent storage.
    {
        ...
        let mut parameters = DEFAULT_CONFIG.dhcp_parameters()?;
        let () = set_server_parameters(&dhcp_server, &mut parameters).await?;
        let () = dhcp_server
            .start_serving()
            .await
            .context("failed to call dhcp/Server.StartServing")?
        ...
        let () = client_acquires_addr(
            &client_env,
            &[client_ep],
            DEFAULT_CONFIG.expected_acquired(),
            1,
            false,
        )
        .await
...
async fn test_dhcp_server_persistence_mode<E: netemul::Endpoint>(
    ...
    {
        let mut params = test_dhcpd_params()?;
        let (mut dhcpd, dhcp_server) = setup_component_proxy(mode, &server_env)?;
    ...
    // server.
    {
        ...
        let mut params = mode.dhcpd_params_after_restart()?;
