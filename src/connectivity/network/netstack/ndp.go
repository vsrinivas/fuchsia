			...
			case *stack.DADSucceeded:
				...
				if nicInfo, ok := n.ns.stack.NICInfo()[event.nicID]; ok {
					...
					n.ns.onAddressAddLocked(event.nicID, tcpip.AddressWithPrefix{
						Address: event.addr,
						// NDP does not offer us the prefix length on its API.
						// At ToT this is solved more elegantly, but we know
						// that for all supported interfaces for F7 the prefix
						// length generated by NDP is 64. Hard-code this to get
						// around b/238942648.
						PrefixLen: 64,
					}, false /* strict */)
					ifs.mu.Unlock()
