...
func (pi *adminAddressStateProviderImpl) OnChanged(lifetimes stack.AddressLifetimes, state stack.AddressAssignmentState) {
	...
	fidlState, err := fidlconv.ToAddressAssignmentState(state)
	if err != nil {
		panic(fmt.Sprintf("failed to convert address assignment state on %s changed: %s", pi.protocolAddr.AddressWithPrefix, err))
	}

	pi.mu.Lock()
	defer pi.mu.Unlock()

	pi.mu.state = fidlState
	if pi.mu.lastObserved != pi.mu.state {
		syslog.DebugTf(addressStateProviderName, "address %+v state changed from %s to %s", pi.protocolAddr.AddressWithPrefix, pi.mu.lastObserved, pi.mu.state)
		select {
...
func (pi *adminAddressStateProviderImpl) OnRemoved(reason stack.AddressRemovalReason) {
	...
	fidlReason, err := fidlconv.ToAddressRemovalReason(reason)
	if err != nil {
		panic(fmt.Sprintf("failed to convert address removal state on %s removed: %s", pi.protocolAddr.AddressWithPrefix, err))
	}
	...
	if err := pi.mu.eventProxy.OnAddressRemoved(fidlReason); err != nil {
		var zxError *zx.Error
		if !errors.As(err, &zxError) || (zxError.Status != zx.ErrPeerClosed && zxError.Status != zx.ErrBadHandle) {
			_ = syslog.ErrorTf(addressStateProviderName, "failed to send OnAddressRemoved(%s) for %s: %s", reason, pi.protocolAddr.AddressWithPrefix.Address, err)
		}
	}
	pi.cancelServe()
}
...
func (ci *adminControlImpl) AddAddress(_ fidl.Context, subnet net.Subnet, parameters admin.AddressParameters, request admin.AddressStateProviderWithCtxInterfaceRequest) error {
	...
	go func() {
		...
		if !detached && !removed {
			_ = syslog.DebugTf(addressStateProviderName, "removing address %s from NIC %d due to protocol closure", addr, ci.nicid)
