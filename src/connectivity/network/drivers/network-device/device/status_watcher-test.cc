...
class StatusWatcherTest : public zxtest::Test {
 public:
  ...
  void TearDown() override {
    for (auto& w : created_watchers_) {
      w->Unbind();
    }
  }

 protected:
  std::shared_ptr<StatusWatcher> MakeWatcher(
      zx::channel channel, uint32_t buffers,
      fit::callback<void(StatusWatcher*)> on_closed = nullptr) {
    auto watcher = std::make_shared<StatusWatcher>(buffers);
    zx_status_t status =
        watcher->Bind(loop_.dispatcher(), std::move(channel), std::move(on_closed));
    if (status != ZX_OK) {
      ADD_FATAL_FAILURE("Failed to bind watcher: %s", zx_status_get_string(status));
    }
    created_watchers_.push_back(watcher);
    return watcher;
  }

  async::Loop loop_;
  std::vector<std::shared_ptr<StatusWatcher>> created_watchers_;
};
...
TEST_F(StatusWatcherTest, HangsForStatus) {
  ...
  std::shared_ptr<StatusWatcher> watcher;
