...
class FidlStatus {
 public:
  explicit FidlStatus(const status_t& status)
      : mtu_(status.mtu), flags_(status.flags), builder_(netdev::Status::Build()) {
    builder_.set_flags(&flags_);
    builder_.set_mtu(&mtu_);
  }

  netdev::Status view() { return builder_.view(); }

 private:
  uint32_t mtu_;
  netdev::StatusFlags flags_;
  netdev::Status::Builder builder_;
};
