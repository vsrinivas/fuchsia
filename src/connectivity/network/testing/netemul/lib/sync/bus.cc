...
class WaitForClientsWatch : public CallbackWatch<Bus::WaitForClientsCallback> {
 public:
  ...
  WaitForClientsWatch(std::vector<std::string> clients, Bus::WaitForClientsCallback callback)
  ...
  void OnTimeout() override {
    ...
    FireCallback(false, fidl::VectorPtr<std::string>(missing_callback_(clients_)));
  }
 ...
 public:
  ...
  bool EventMatches(const Bus::FEvent& cmp) const {
    if (event_.has_code() && (!cmp.has_code() || cmp.code() != event_.code())) {
      return false;
    } else if (event_.has_message() && (!cmp.has_message() || cmp.message() != event_.message())) {
      return false;
    } else if (event_.has_arguments() &&
               (!cmp.has_arguments() || cmp.arguments().size() != event_.arguments().size() ||
...
class BusBinding : public fuchsia::netemul::sync::Bus {
 public:
  ...
  BusBinding(std::string name, ::netemul::Bus* parent, async_dispatcher_t* dispatcher,
             fidl::InterfaceRequest<FBus> request)
      : client_name_(std::move(name)),
        parent_(parent),
        binding_(this, std::move(request), dispatcher) {}

  void Publish(FEvent data) override { parent_->Publish(std::move(data), client_name_); }

  void EnsurePublish(FEvent data, FBus::EnsurePublishCallback callback) override {
  ...
  void WaitForEvent(fuchsia::netemul::sync::Event data, int64_t timeout,
    ...
    auto watcher = std::make_unique<WaitForEventWatch>(std::move(data), std::move(callback));
...
void Bus::Subscribe(const std::string& clientName, fidl::InterfaceRequest<Bus::FBus> request) {
  // reject if clientName is already in clients
  if (clients_.find(clientName) != clients_.end()) {
    return;
  }

  auto binding = std::make_unique<BusBinding>(clientName, this, dispatcher_, std::move(request));
  ...
  clients_.insert(std::pair<std::string, ClientBinding>(clientName, std::move(binding)));
...
const std::unordered_map<std::string, Bus::ClientBinding>& Bus::clients() { return clients_; }
...
void Bus::WaitForClients(std::vector<std::string> clients, int64_t timeout,
  ...
  auto client_watch =
      std::make_unique<WaitForClientsWatch>(std::move(clients), std::move(callback));
  if (CheckClientWatch(client_watch.get())) {
    return;
  } else {
    if (timeout > 0) {
      client_watch->SetMissingClientsCallback([this](const std::vector<std::string>& wait) {
