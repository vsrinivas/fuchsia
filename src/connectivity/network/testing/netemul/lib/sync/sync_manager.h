...
class SyncManager : public fuchsia::netemul::sync::SyncManager {
 public:
  ...
  explicit SyncManager(async_dispatcher_t* dispatcher) : dispatcher_(dispatcher) {}

  void BusSubscribe(std::string busName, std::string clientName,
                    ::fidl::InterfaceRequest<Bus::FBus> bus) override;
  void WaitForBarrierThreshold(::std::string barrierName, uint32_t threshold, int64_t timeout,
 ...
 private:
  ...
  std::unordered_map<std::string, std::unique_ptr<CounterBarrier>> counter_barriers_;
