...
bool LaunchApp::ParseFromJSON(const rapidjson::Value& value, json::JSONParser* parser) {
  if (value.IsString()) {
    ...
    if (value.GetStringLength() != 0) {
      ...
      if (!pkgUrl.Parse(url)) {
        parser->ReportError("launch options url is not a valid fuchsia package url");
        return false;
      }
    }
    url_ = url;
    ...
    for (auto i = value.MemberBegin(); i != value.MemberEnd(); i++) {
      if (i->name == kUrl) {
        ...
        for (auto a = arg_arr.Begin(); a != arg_arr.End(); a++) {
          if (!a->IsString()) {
            parser->ReportError("launch options arguments element must be string");
            return false;
          }
        ...
        parser->ReportError(
            fxl::StringPrintf("Unrecognized launch args member \"%s\"", i->name.GetString()));
        return false;
      }
    }
  } else {
...
const std::vector<std::string>& LaunchApp::arguments() const { return arguments_; }
