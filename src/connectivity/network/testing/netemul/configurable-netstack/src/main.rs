...
async fn configure_interface(
    ...
    if without_autogenerated_addresses.unwrap_or_default() {
        ...
        let mut addr = match addresses.as_mut_slice() {
            [fnet_interfaces_ext::Address { value, valid_until: _ }] => Ok(value),
            addrs => {
        ...
        let _removed: bool = control
            ...
            .await
            .map_err(NetstackError::InterfaceControl)?
            .map_err(|error| NetstackError::RemoveAddress { error, addr: *addr })?;
