...
#include "src/connectivity/overnet/deprecated/lib/endpoint/integration_tests/tests.h"
...
void NodeDescriptionPropagationTest(Env* env) {
  TestService service(env->endpoint1(), "#ff00ff",
  ...
  env->endpoint2()->ForEachNodeDescription(
      [env, &found](NodeId id, const fuchsia::overnet::protocol::PeerDescription& m) {
...
TestTimes OneMessageSrcToDest(Env* env, Slice body, Optional<TimeDelta> allowed_time) {
  ...
  TestService service(
      ...
      [got_pull_cb, body](RouterEndpoint::NewStream new_stream) {
        ...
        auto stream = MakeClosedPtr<RouterEndpoint::Stream>(std::move(new_stream));
        ...
        op->PullAll(StatusOrCallback<Optional<std::vector<Slice>>>(
            ALLOCATED_CALLBACK, [got_pull_cb, body, stream{std::move(stream)},
                                 op](const StatusOr<Optional<std::vector<Slice>>>& status) mutable {
              OVERNET_TRACE(INFO) << "ep2: pull_all status=" << status.AsStatus();
  ...
  auto new_stream = env->endpoint1()->InitiateStream(
      ...
      fuchsia::overnet::protocol::ReliabilityAndOrdering::ReliableOrdered, kService);
  ZX_ASSERT(new_stream.is_ok());
  auto stream = MakeClosedPtr<RouterEndpoint::Stream>(std::move(*new_stream.get()));
  RouterEndpoint::SendOp(stream.get(), body.length()).Push(body, Callback<void>::Ignored());

  auto start_flush = env->timer()->Now();

  env->FlushTodo([got_pull_cb]() { return *got_pull_cb; },
                 allowed_time.has_value() ? start_flush + *allowed_time
...
TestTimes RequestResponse(Env* env, Slice request_body, Slice response_body,
  ...
  TestService service(
      ...
      [request_body, response_body](RouterEndpoint::NewStream new_stream) {
        ...
        OVERNET_TRACE(INFO) << "ep2: op=" << op;
        op->PullAll(StatusOrCallback<Optional<std::vector<Slice>>>(
            ALLOCATED_CALLBACK, [request_body, response_body, stream{std::move(stream)},
                                 op](const StatusOr<Optional<std::vector<Slice>>>& status) mutable {
              OVERNET_TRACE(INFO) << "ep2: pull_all status=" << status.AsStatus();
              ZX_ASSERT(status.is_ok());
              ZX_ASSERT(status->has_value());
  ...
  op->PullAll(StatusOrCallback<Optional<std::vector<Slice>>>(
      ALLOCATED_CALLBACK, [response_body, stream{std::move(stream)}, op, got_response](
