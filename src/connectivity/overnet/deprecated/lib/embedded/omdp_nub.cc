...
#include "src/connectivity/overnet/deprecated/lib/embedded/omdp_nub.h"

#include <net/if.h>

#include "garnet/public/lib/fostr/fidl/fuchsia/overnet/omdp/formatting.h"
#include "src/connectivity/overnet/deprecated/lib/omdp/omdp.h"
#include "src/connectivity/overnet/deprecated/lib/vocabulary/ip_addr.h"
...
Status OmdpNub::Start() {
  return incoming_.Create(AF_INET6, SOCK_DGRAM, 0)
      ...
      .Then([&] {
        return incoming_.SetOpt(IPPROTO_IPV6, IPV6_JOIN_GROUP,
...
void OmdpNub::Broadcast(Slice data) {
  ...
  for (size_t i = 0; interfaces[i].if_index != 0 || interfaces[i].if_name != nullptr; i++) {
    if (auto status = socket.SetOpt(IPPROTO_IPV6, IPV6_MULTICAST_IF, interfaces[i].if_index)
                          .Then([&] { return socket.SendTo(data, 0, kMulticastGroupAddr); })
                          .WithLazyContext([&] {
...
void OmdpNub::WaitForInbound() {
  reactor_->OnRead(incoming_.get(), [this](const Status& status) { InboundReady(status); });
}
