...
class MockStreamLink {
 public:
  ...
  LinkPtr<StreamLink> MakeLink(Router* router, NodeId peer, std::unique_ptr<StreamFramer> framer) {
    ...
    class PacketLinkImpl final : public StreamLink {
     public:
      PacketLinkImpl(MockStreamLink* mock, Router* router, NodeId peer,
          ...
          : StreamLink(router, peer, std::move(framer), next_label++), mock_(mock) {}
      void Emit(Slice packet, Callback<Status> done) {
        auto cb = std::make_shared<Callback<Status>>(std::move(done));
        mock_->Emit(std::move(packet), [cb](const Status& status) { (*cb)(status); });
      }

     private:
      MockStreamLink* mock_;
    };
    return overnet::MakeLink<PacketLinkImpl>(this, router, peer, std::move(framer));
  }
};
...
TEST_P(StreamLinkTest, SendOne) {
  ...
  router.RegisterLink(mock_link.MakeLink(&router, NodeId(2), GetParam().make_framer()));
  ...
  EXPECT_CALL(mock_link, Emit(_, _)).WillOnce(DoAll(SaveArg<0>(&emitted), SaveArg<1>(&done_emit)));
...
TEST_P(StreamLinkTest, RecvOne) {
  ...
  auto link_unique = mock_link.MakeLink(&router, NodeId(1), GetParam().make_framer());
...
INSTANTIATE_TEST_SUITE_P(
    ...
    ::testing::Values(
        TestArg{[] { return std::make_unique<ReliableFramer>(); }, Slice::FromContainer({7, 8, 9}),
                Slice::FromContainer({0x06, 0x00, 6, 0, 1, 1, 7, 8, 9})},
        TestArg{[] { return std::make_unique<UnreliableFramer>(); },
                Slice::FromContainer({7, 8, 9}),
                Slice::FromContainer({0x0a, 6, 6, 0, 1, 1, 7, 8, 9, 0xb8, 0x80, 0x2a, 0xcf})}));
