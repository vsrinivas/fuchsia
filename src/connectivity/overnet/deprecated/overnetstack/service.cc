...
#include "src/connectivity/overnet/deprecated/overnetstack/service.h"
...
overnet::Status Service::Start() {
  app_->component_context()->outgoing()->AddPublicService(bindings_.GetHandler(this));
  return overnet::Status::Ok();
}

void Service::ListPeers(uint64_t last_seen_version, ListPeersCallback callback) {
  using Peer = fuchsia::overnet::Peer;
  app_->endpoint()->OnNodeDescriptionTableChange(
      last_seen_version,
      overnet::StatusCallback(overnet::ALLOCATED_CALLBACK, [this, callback = std::move(callback)](
...
void Service::RegisterService(std::string service_name,
  ...
  class ServiceProvider final : public OvernetApp::ServiceProvider {
   public:
    explicit ServiceProvider(
        OvernetApp* app, std::string fully_qualified_name,
        fuchsia::overnet::protocol::ReliabilityAndOrdering reliability_and_ordering,
        fuchsia::overnet::ServiceProviderPtr provider)
        : OvernetApp::ServiceProvider(app, fully_qualified_name, reliability_and_ordering),
  ...
  app_->InstantiateServiceProvider<ServiceProvider>(
      ...
      fuchsia::overnet::protocol::ReliabilityAndOrdering::ReliableOrdered, provider.Bind());
}

void Service::ConnectToService(fuchsia::overnet::protocol::NodeId node, std::string service_name,
                               zx::channel channel) {
  ...
  if (app_->endpoint()->node_id() == node_id) {
    ...
    auto ns = app_->endpoint()->InitiateStream(
        node_id, fuchsia::overnet::protocol::ReliabilityAndOrdering::ReliableOrdered,
