...
mod test {
    ...
    use futures::prelude::*;
    use overnet_core::{FutureExt as _, Task, TimeoutError};
    ...
    #[test]
    fn one_bad_channel_doesnt_take_everything_down() {
        logger::init().unwrap();
        run(async move {
    ...
    #[test]
    fn one_bad_link_doesnt_take_the_rest_down() {
        logger::init().unwrap();
        run(async move {
            ...
            let mut buf = [0u8; 10];
            loop {
                match s1b.read(&mut buf).timeout_after(std::time::Duration::from_secs(1)).await {
                    Ok(Ok(0)) => panic!("Should not see s1b closed"),
                    Ok(Ok(_)) => (),
                    Ok(Err(e)) => panic!("Should not see an error on s1b: {:?}", e),
