...
class FuzzingSimulator final : public Simulator {
 public:
  FuzzingSimulator(InputStream* input) : input_(input) {}

  void MakeLinks(RouterEndpoint* ep1, RouterEndpoint* ep2, uint64_t id1,
                 uint64_t id2) const override {
    auto link1 =
        MakeLink<InProcessLink<FuzzingPacketLink>>(ep1, ep2, id1, input_);
    auto link2 =
        MakeLink<InProcessLink<FuzzingPacketLink>>(ep2, ep1, id2, input_);
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
  ...
  const NamedSimulator kSimulator{"fuzzing_simulator",
                                  std::make_unique<FuzzingSimulator>(&input)};
