...
mod test {
    ...
    async fn drop_conn() {
        ...
        let a = fasync::Task::spawn(async move {
            multi_stream(a_reader, a_writer, true, a_streams_out, a_streams_in).await.unwrap()
        });
        let b = fasync::Task::spawn(async move {
            multi_stream(b_reader, b_writer, false, b_streams_out, b_streams_in).await.unwrap()
        });
        ...
        assert!(matches!(
            ab_reader_b.read::<_, ()>(1, |_| unreachable!()).await,
            Err(Error::ConnectionClosed)
        ));

        assert!(matches!(ab_writer_a.write(1, |_| unreachable!()), Err(Error::ConnectionClosed)));

        assert!(matches!(ab_writer_b.write(1, |_| unreachable!()), Err(Error::ConnectionClosed)));
    }
