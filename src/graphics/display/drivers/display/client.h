...
class DisplayControllerBindingState {
 ...
 public:
  ...
  // Sets this object into the event sender state. This may be used to retain
  // the channel after the server is unbound.
  void SetEventSender(zx::channel&& channel) {
    binding_state_ = Protocol::EventSender(std::move(channel));
  }
  ...
  void Unbind() {
    if (auto ref = std::get_if<fidl::ServerBindingRef<Protocol>>(&binding_state_)) {
      // The unbind handler in |Client| will eventually set the state of the
      // |binding_state_| variant to |Protocol::EventSender|.
      ref->Unbind();
    }
  }
