...
int Osd::RdmaThread() {
  ...
  while (1) {
    ...
    for (int i = 0; i < kMaxRdmaChannels; i++) {
      if (RdmaStatusReg::Get().ReadFrom(&(*vpu_mmio_)).done() & RDMA_STATUS_DONE(i)) {
    ...
    // every vsync. If will get disabled if FlipOnVsync does not use AFBC.
...
int Osd::GetNextAvailableRdmaChannel() {
  ...
  for (int i = 0; i < kMaxRdmaChannels; i++) {
    if (!rdma_chnl_container_[i].active ||
      ...
      // clear interrupts
      RdmaCtrlReg::Get().ReadFrom(&(*vpu_mmio_)).set_clear_done(RDMA_CTRL_INT_DONE(i + 1));
      return i;
    }
  }
  return -1;
