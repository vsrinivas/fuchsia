...
int Interrupts::IrqLoop() {
  for (;;) {
    ...
    auto interrupt_ctrl = tgl_registers::MasterInterruptControl::Get().ReadFrom(mmio_space_);
    ...
    if (interrupt_ctrl.sde_int_pending()) {
      ...
      auto hp_ctrl1 = tgl_registers::HotplugCtrl ::Get(tgl_registers::DDI_A).ReadFrom(mmio_space_);
      auto hp_ctrl2 = tgl_registers::HotplugCtrl ::Get(tgl_registers::DDI_E).ReadFrom(mmio_space_);
...
void Interrupts::EnableHotplugInterrupts() {
  auto sfuse_strap = tgl_registers::SouthFuseStrap::Get().ReadFrom(mmio_space_);
  ZX_DEBUG_ASSERT(ddis_.data());
  for (const auto ddi : ddis_) {
    bool enabled = (ddi == tgl_registers::DDI_A) || (ddi == tgl_registers::DDI_E) ||
                   (ddi == tgl_registers::DDI_B && sfuse_strap.port_b_present()) ||
                   (ddi == tgl_registers::DDI_C && sfuse_strap.port_c_present()) ||
                   (ddi == tgl_registers::DDI_D && sfuse_strap.port_d_present());

    auto hp_ctrl = tgl_registers::HotplugCtrl::Get(ddi).ReadFrom(mmio_space_);
...
zx_status_t Interrupts::Init(PipeVsyncCallback pipe_vsync_callback,
  ...
  auto interrupt_ctrl = tgl_registers::MasterInterruptControl::Get().ReadFrom(mmio_space);
  interrupt_ctrl.set_enable_mask(0);
...
void Interrupts::FinishInit() {
  auto ctrl = tgl_registers::MasterInterruptControl::Get().ReadFrom(mmio_space_);
