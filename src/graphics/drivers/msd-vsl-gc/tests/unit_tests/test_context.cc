...
class BatchData {
 public:
  static BatchData Create(std::shared_ptr<MsdVslContext> context, uint32_t num_resources,
 ...
 private:
  ...
  std::unique_ptr<MappedBatch> CreateBatch(std::shared_ptr<MsdVslContext> context) {
    ...
    return MsdVslContext::CreateBatch(context, &command_buffer, resources.data(),
...
class TestMsdVslContext : public ::testing::Test {
 public:
  void SetUp() {
    ...
    connection_ = std::make_shared<MsdVslConnection>(&mock_connection_owner_, address_space_,
                                                     0 /* client_id */);
...
TEST_F(TestMsdVslContext, SubmitBatchesNoResources) {
