...
#ifndef SRC_GRAPHICS_LIB_COMPUTE_TESTS_COMMON_SPINEL_VK_SPINEL_VK_DEVICE_CONFIG_UTILS_H_
#define SRC_GRAPHICS_LIB_COMPUTE_TESTS_COMMON_SPINEL_VK_SPINEL_VK_DEVICE_CONFIG_UTILS_H_
...
// This is filled by spinel_vk_device_config_callback.
// |spinel_target| and |hotsort_target| can later be passed to
// spn_vk_context_create().
typedef struct
{
  // The following are read by spinel_vk_device_config_callback().
  ...
  // The following are filled by spinel_vk_device_config_callback().
  const struct spn_vk_target *     spinel_target;
  const struct hotsort_vk_target * hotsort_target;

  // The following is used internally as storage for the chain of physical device
  // features extension struct. The buffer must be pointer-aligned, and its size
  // is chosen liberally (and could be increased if Spinel requires too many
  // extension structs in the future).
  uintptr_t ext_chain_storage[128];

} spinel_vk_device_configuration_t;
...
#endif  // SRC_GRAPHICS_LIB_COMPUTE_TESTS_COMMON_SPINEL_VK_SPINEL_VK_DEVICE_CONFIG_UTILS_H_
