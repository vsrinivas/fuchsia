  ...
  switch (ops->type)
    {
      ...
      case HSG_OP_TYPE_ELSE:
        ...
        case HSG_OP_TYPE_TARGET_BEGIN: {
          ...
          target->state->source =
            fopen((config->type.dwords == 1) ? "hs_cuda_u32.cu" : "hs_cuda_u64.cu", "wb");
      ...
      case HSG_OP_TYPE_TARGET_END:
        ...
        case HSG_OP_TYPE_TRANSPOSE_KERNEL_PREAMBLE: {
          fprintf(target->state->source, "HS_SLAB_GLOBAL_PREAMBLE();\n");
        }
        break;
        ...
        case HSG_OP_TYPE_BS_KERNEL_PREAMBLE: {
          ...
          if (ops->a == 0)
            ...
            {
              fprintf(target->state->source, "HS_OFFSET_SLAB_GLOBAL_PREAMBLE();\n");
            }
        }
        break;
        ...
        case HSG_OP_TYPE_FM_KERNEL_PREAMBLE: {
          if (ops->a == ops->b)  // equal left and right spans
            ...
            {
              fprintf(target->state->source, "HS_OFFSET_FM_PREAMBLE(%u);\n", ops->a);
            }
        }
        break;
      ...
      case HSG_OP_TYPE_BX_REG_GLOBAL_STORE:
        fprintf(target->state->source, "HS_SLAB_GLOBAL_STORE(%u,r%u);\n", ops->n - 1, ops->n);
        break;
      ...
      case HSG_OP_TYPE_BX_MERGE_H_PRED:
        fprintf(target->state->source, "if (HS_WARP_ID_X() < %u)\n", ops->a);
        break;
