              ...
              void * remap)
{
  ...
  for (uint32_t cc = 1; cc <= cols_log2; cc++)
    {
      uint32_t const mask = BITS_TO_MASK_MACRO(cc);
...
static void
hsg_debug_remap(uint32_t const row_from, uint32_t const row_to, uint32_t * const r)
{
...
static void
hsg_debug_print(uint32_t const rows, uint32_t const * const m, uint32_t const * const r)
{
...
main(int argc, char * argv[])
{
  ...
  hsg_transpose(cols_log2, rows, hsg_debug_blend, b, hsg_debug_remap, r);
