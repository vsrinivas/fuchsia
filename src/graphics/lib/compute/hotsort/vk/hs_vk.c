...
#if defined(HS_VK_SHADER_INFO_AMD_STATISTICS) || defined(HS_VK_SHADER_INFO_AMD_DISASSEMBLY)
             ...
             struct hs_vk_target       const * const target)
{
  ...
  // allocate hs_vk
  //
  struct hs_vk * const hs = malloc(sizeof(*hs) + sizeof(VkPipeline*) * count_all);

  // in-place sort?
  hs->is_in_place =
    (locations->in.set     == locations->out.set) &&
    (locations->in.binding == locations->out.binding);
  ...
  VkShaderModuleCreateInfo smci = {
    ...
    .flags    = 0,
    .codeSize = 0,
    .pCode    = (uint32_t const *)target->modules // FIXME -- unfortunate typecast -- fix during generation
  };

  //
  // BS     shaders have layout: (vout,vin)
  // non-BS shaders have layout: (vout)
  //
  for (uint32_t ii=0; ii<count_all; ii++)
    {
      ...
      //
      // FIXME -- patch descriptor set locations
      //
  ...
  //
#ifdef HS_VK_SHADER_INFO_AMD_STATISTICS
  vk_shader_info_amd_statistics(device,hs->pipelines.all,NULL,hs->pipelines.count);
#endif
#ifdef HS_VK_SHADER_INFO_AMD_DISASSEMBLY
          ...
          uint32_t           * const padded_out)
{
  ...
  uint32_t const slabs_ru_rem_ru = MIN_MACRO(uint32_t,
                                             pow2_ru_u32(slabs_ru_rem),
      ...
      if (block_slabs_rem > 0)
        {
          ...
          uint32_t const block_slabs_hi         = MAX_MACRO(uint32_t,
                                                            block_slabs_rem_ru,
                                                            blocks_lo << (1 - hs->config.merge.fm.scale_min));

          uint32_t const block_slabs_padded_out = MIN_MACRO(uint32_t,
           ...
           bool                            const linearize)
{
  ...
  struct hs_vk_push const push =
    {
      .kv_offset_in  = (uint32_t)(offsets->in  / kv_size),
  ...
  vkCmdPushConstants(cb,
                     hs->pl,
                     HS_VK_PUSH_CONSTANT_RANGE_STAGE_FLAGS,
                     HS_VK_PUSH_CONSTANT_RANGE_OFFSET,
                     HS_VK_PUSH_CONSTANT_RANGE_SIZE,
                     &push);
  ...
  //
  uint32_t const padded_pre_sort   = hs->is_in_place ? padded_out : padded_in;
  bool     const is_pre_sort_reqd  = padded_pre_sort > count;
  bool     const is_pre_merge_reqd = !hs->is_in_place && (padded_out > padded_in);
      ...
      while (true)
        {
          ...
          uint32_t clean_slabs_log2 = hs_cmd_fm(cb,
                                                hs,
                                                bx_ru,
                                                &down_slabs,
                                                up_scale_log2);
