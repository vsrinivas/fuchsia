...
static struct spn_target_image const *
  ...
  for (uint32_t ii = 0; ii < (1 << 26); ii++)
    {
      ...
      spn_raster_fill(rb, &path, &tw, &spn_transform_identity, &cw, &spn_clip_default, 1);
...
main(int argc, char const * argv[])
{
  ...
  //
  struct spn_device_vk device_vk = {.ac = NULL, .qfi = 0};
  ...
  vk(EnumeratePhysicalDevices(instance, &pdc, NULL));

  VkPhysicalDevice * pds = malloc(pdc * sizeof(*pds));

  vk(EnumeratePhysicalDevices(instance, &pdc, pds));
  ...
  uint32_t const device_enabled_extension_count = ARRAY_LENGTH_MACRO(device_enabled_extensions) -
                                                  1 -
