...
static
void
spn_path_move_to_1(spn_path_builder_t path_builder,
                   float x0, float y0)
{
...
static
void
spn_path_move_to_2(spn_path_builder_t path_builder,
                   float x0, float y0,
                   float x1, float y1)
{
  path_builder->curr[0].x = x0;
...
spn_result
spn_path_move_to(spn_path_builder_t path_builder,
                 float x0, float y0)
{
...
#define SPN_PB_CN_COORDS_APPEND(_pb,_p,_n,_c)   \
  *_pb->cn.coords._p[_n]++ = _c
...
spn_result
spn_path_line_to(spn_path_builder_t path_builder,
                 float x1, float y1)
{
...
spn_result
spn_path_quad_to(spn_path_builder_t path_builder,
                 float x1, float y1,
                 float x2, float y2)
{
...
spn_result
spn_path_quad_smooth_to(spn_path_builder_t path_builder,
                        float x2, float y2)
{
...
spn_result
spn_path_cubic_to(spn_path_builder_t path_builder,
                  float x1, float y1,
                  float x2, float y2,
                  float x3, float y3)
{
...
spn_result
spn_path_cubic_smooth_to(spn_path_builder_t path_builder,
                         float x2, float y2,
                         float x3, float y3)
{
  float const x1 = path_builder->curr[0].x * 2.0f - path_builder->curr[1].x;
...
spn_result
spn_path_rat_quad_to(spn_path_builder_t path_builder,
                     float x1, float y1,
                     float x2, float y2,
                     float w0)
{
...
spn_result
spn_path_ellipse(spn_path_builder_t path_builder,
                 float cx, float cy,
                 float rx, float ry)
{
  //
...
#define SPN_KAPPA_FLOAT 0.55228474983079339840f // moar digits!
  ...
  err = spn_path_cubic_to(path_builder,
                          cx + kx, cy + ry,
                          cx + rx, cy + ky,
                          cx + rx, cy);
