...
#define SPN_TARGET_BUFFER_NAME(_ds_id, _name) struct spn_target_buf_##_ds_id##_##_name
...
#define SPN_TARGET_VK_PUSH(_p_id, _p_pc) SPN_TARGET_PUSH_NAME(_p_id){_p_pc};
...
#define SPN_TARGET_DS_TYPE_DECLARE(_ds_id)                                                         \
  SPN_TARGET_DS_TYPE(_ds_id)                                                                       \
  {                                                                                                \
    uint32_t idx;                                                                                  \
  }
...
#define SPN_TARGET_DS_ACQUIRE_PROTO(_ds_id)                                                        \
  void SPN_TARGET_DS_ACQUIRE_FUNC(_ds_id)(struct spn_target * const          target,               \
                                          struct spn_device * const          device,               \
                                          SPN_TARGET_DS_TYPE(_ds_id) * const ds)

#define SPN_TARGET_DS_UPDATE_PROTO(_ds_id)                                                         \
  void SPN_TARGET_DS_UPDATE_FUNC(_ds_id)(struct spn_target * const    target,                      \
                                         struct spn_device_vk * const vk,                          \
                                         SPN_TARGET_DS_TYPE(_ds_id) const ds)

#define SPN_TARGET_DS_RELEASE_PROTO(_ds_id)                                                        \
  void SPN_TARGET_DS_RELEASE_FUNC(_ds_id)(struct spn_target * const target,                        \
                                          SPN_TARGET_DS_TYPE(_ds_id) const ds)
...
#define SPN_TARGET_DS_GET_PROTO_STORAGE_BUFFER(_ds_id, _d_id)                                      \
  VkDescriptorBufferInfo * SPN_TARGET_DS_GET_FUNC(_ds_id, _d_id)(struct spn_target * const target, \
                                                                 SPN_TARGET_DS_TYPE(_ds_id)        \
                                                                   const ds)

#define SPN_TARGET_DS_GET_PROTO_STORAGE_IMAGE(_ds_id, _d_id)                                       \
  VkDescriptorImageInfo * SPN_TARGET_DS_GET_FUNC(_ds_id, _d_id)(struct spn_target * const target,  \
                                                                SPN_TARGET_DS_TYPE(_ds_id)         \
                                                                  const ds);
...
#define SPN_TARGET_DS_BIND_PROTO(_p_id, _ds_id)                                                    \
  void SPN_TARGET_DS_BIND_FUNC(_p_id, _ds_id)(struct spn_target * const target,                    \
                                              VkCommandBuffer           cb,                        \
                                              SPN_TARGET_DS_TYPE(_ds_id) const ds)

#undef SPN_TARGET_VK_DS
#define SPN_TARGET_VK_DS(_p_id, _ds_idx, _ds_id) SPN_TARGET_DS_BIND_PROTO(_p_id, _ds_id);
...
#define SPN_TARGET_P_PUSH_PROTO(_p_id)                                                             \
  ...
  void SPN_TARGET_P_PUSH_FUNC(_p_id)(struct spn_target * const                 target,             \
                                     VkCommandBuffer                           cb,                 \
                                     SPN_TARGET_PUSH_NAME(_p_id) const * const push)

#undef SPN_TARGET_P_EXPAND_X
#define SPN_TARGET_P_EXPAND_X(_p_idx, _p_id, _p_descs) SPN_TARGET_P_PUSH_PROTO(_p_id);
...
#define SPN_TARGET_P_BIND_PROTO(_p_id)                                                             \
  void SPN_TARGET_P_BIND_FUNC(_p_id)(struct spn_target * const target, VkCommandBuffer cb)

#undef SPN_TARGET_P_EXPAND_X
#define SPN_TARGET_P_EXPAND_X(_p_idx, _p_id, _p_descs) SPN_TARGET_P_BIND_PROTO(_p_id);
