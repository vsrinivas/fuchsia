                             ...
                             uint32_t const            handle_count)
{
  ...
  block_pool->bp_blocks.dbi =
    spn_target_ds_get_block_pool_bp_blocks(target, block_pool->ds_block_pool);
  block_pool->bp_host_map.dbi =
    spn_target_ds_get_block_pool_bp_host_map(target, block_pool->ds_block_pool);

  // allocate buffers
  size_t const bp_ids_size =
    SPN_TARGET_BUFFER_OFFSETOF(block_pool, bp_ids, bp_ids) + id_count * sizeof(spn_block_id_t);
  ...
  size_t const   bp_blocks_size =
    SPN_TARGET_BUFFER_OFFSETOF(block_pool, bp_blocks, bp_blocks) + bp_dwords * sizeof(uint32_t);
  ...
  size_t const bp_host_map_size = SPN_TARGET_BUFFER_OFFSETOF(block_pool, bp_host_map, bp_host_map) +
  ...
  spn_target_ds_update_block_pool(target, device->vk, block_pool->ds_block_pool);
  ...
  struct spn_target_push_block_pool_init const push = {.bp_size = block_pool->bp_size};
