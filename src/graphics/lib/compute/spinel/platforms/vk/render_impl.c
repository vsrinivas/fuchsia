...
spn_result
spn_ri_buffer(struct spn_device         * const device,
              spn_render_submit_t const * const submit)
{
  ...
  *spn_target_ds_get_surface_surface(target,ds_surface) = ext->surface;
  ...
  //
  struct spn_target_push_render const push =
    {
      .tile_clip     = {
        submit->tile_clip[0],
        submit->tile_clip[1],
  ...
  if (waitSemaphoreCount == 0)
  ...
  else
    {
      ...
      memcpy(pWaitSemaphores   + waitSemaphoreCount,ext->si->pWaitSemaphores,  count * sizeof(*pWaitSemaphores));
      memcpy(pWaitDstStageMask + waitSemaphoreCount,ext->si->pWaitDstStageMask,count * sizeof(*pWaitDstStageMask));
