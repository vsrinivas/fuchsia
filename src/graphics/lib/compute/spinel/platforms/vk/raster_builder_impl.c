...
static
void
spn_rbi_path_append(struct spn_raster_builder_impl * const impl,
...
static
struct spn_rbi_dispatch *
spn_rbi_dispatch_idx(struct spn_raster_builder_impl * const impl,
                     uint32_t                         const idx)
{
...
spn_rbi_complete_p_2(void * pfn_payload)
{
  ...
  spn_allocator_device_temp_free(&device->allocator.device.temp.local,
                                 p_2->temp.rp.ttrks);
...
spn_rbi_flush(struct spn_raster_builder_impl * const impl)
{
  ...
  *spn_target_ds_get_rasterize_fill_quads(target,p_1.ds.r) = impl->vk.rings.tc;
  ...
  VkDescriptorBufferInfo * const dbi_fill_scan = spn_target_ds_get_rasterize_fill_scan(target,p_1.ds.r);
  ...
  VkDescriptorBufferInfo * const dbi_ttrks = spn_target_ds_get_rasterize_post_ttrks(target,p_1.p_2.ds.rp);
...
#define SPN_TARGET_P_BIND_RASTERIZE_NAME(_p)    \
...
#define SPN_PATH_BUILDER_PRIM_TYPE_EXPAND_X(_p,_i,_n)                                \
  ...
  VkBufferCopy const bc = {
    .srcOffset = p_1.p_2.temp.rp.offset + SPN_TARGET_BUFFER_OFFSETOF(rasterize_post,ttrks,ttrks_count),
        ...
        if (!spn_weakref_get_index(cw,epoch,&cf.clip))
          {
            ...
            memcpy(impl->mapped.tc.extent + c_idx,
                   clips + ii,
                   sizeof(*impl->mapped.tc.extent));
                             ...
                             spn_raster_builder_t * const raster_builder)
{
  ...
  *raster_builder                = rb;
  ...
  uint32_t const tc_ring_size    = config->raster_builder.size.ring * 3;     // 1 transform + 1 clip = 3 quads
