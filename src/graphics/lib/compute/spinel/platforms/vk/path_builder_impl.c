...
struct spn_path_builder_impl
{
  ...
  //
  // buffers used by paths_alloc and paths_copy shaders
  //
  struct {
    struct {
...
#define SPN_PATH_BUILDER_PRIM_TYPE_EXPAND_X(_p,_i,_n)                   \
  static                                                                \
  spn_result                                                            \
  SPN_PBI_PFN_LOST_NAME(_p)(struct spn_path_builder_impl * const impl)  \
  {                                                                     \
    return SPN_ERROR_PATH_BUILDER_LOST;                                 \
  }
...
#undef  SPN_PATH_BUILDER_PRIM_TYPE_EXPAND_X
#define SPN_PATH_BUILDER_PRIM_TYPE_EXPAND_X(_p,_i,_n)   \
  pb->_p      = SPN_PBI_PFN_LOST_NAME(_p);              \
...
static
void
spn_pbi_path_append(struct spn_path_builder_impl * const impl,
                    spn_path_t                     const path)
{
...
static
struct spn_pbi_dispatch *
spn_pbi_dispatch_idx(struct spn_path_builder_impl * const impl,
                     uint32_t                       const idx)
{
...
spn_pbi_complete(void * pfn_payload)
{
  ...
  // release cb
  spn_device_cb_pool_release(device,payload->cb);
  ...
  //
  uint32_t const            dispatch_idx = payload->idx;
...
spn_pbi_flush(struct spn_path_builder_impl * const impl)
{
  ...
  // acquire the PATHS_COPY descriptor set
  ...
  *spn_target_ds_get_paths_copy_pc_ring(target,ds_pc)  = impl->vk.ring.dbi;
  ...
  // update the PATHS_COPY descriptor set
  spn_target_ds_update_paths_copy(target,device->vk,ds_pc);

  // bind the PATHS_COPY descriptor set
  ...
  //
  struct spn_target_push_paths_copy const push =
    {
...
static
void
spn_pb_cn_coords_finalize(float  *       coords[],
                          uint32_t       coords_len,
                          uint32_t const rem)
{
  do {
...
static
spn_result
spn_pbi_acquire_node_segs_block(struct spn_path_builder_impl * const impl,
                                uint32_t                     * const idx)
{
  struct spn_ring * const ring = &impl->mapped.ring;
...
static
void
spn_pbi_node_append_prim(struct spn_path_builder_impl * const impl,
                         uint32_t                       const tag)
{
...
#define SPN_PATH_BUILDER_PRIM_TYPE_EXPAND_X(_p,_i,_n)                           \
  static                                                                        \
  spn_result                                                                       \
  SPN_PBI_PFN_NAME(_p)(struct spn_path_builder_impl * const impl)               \
  {                                                                             \
    return spn_pbi_prim_acquire_subblocks(impl,                                 \
                                          _i,                                   \
                                          impl->path_builder->cn.coords._p,     \
                                          _n);                                  \
  }
...
{
  impl->wip.header.prims = (struct spn_uvec4)
    SPN_PATH_PRIMS_INIT(impl->wip.prims.line,
                        impl->wip.prims.quad,
...
#define SPN_PATH_BUILDER_PRIM_TYPE_EXPAND_X(_p,_i,_n)   \
  ...
  // reset bounds
  impl->wip.header.bounds    = (struct spn_vec4){ +FLT_MIN,
                                                  +FLT_MIN,
                                                  -FLT_MIN,
                                                  -FLT_MIN };
  // reset prim counters
