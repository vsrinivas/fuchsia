...
pub(crate) use segments::PathSegments;
...
impl Path {
    ...
    /// ```
    pub fn new() -> Self {
        Self { commands: vec![], transform: None }
    }
    ...
    pub fn rat_quad(
        ...
        self.commands.push(PathCommand::RatQuad([
            (p0.0 * p0.1, p0.1),
            (p1.0 * p1.1, p1.1),
            (p2.0 * p2.1, p2.1),
        ]));
    ...
    pub fn rat_cubic(
        ...
        self.commands.push(PathCommand::RatCubic([
            (p0.0 * p0.1, p0.1),
            (p1.0 * p1.1, p1.1),
            (p2.0 * p2.1, p2.1),
            (p3.0 * p3.1, p3.1),
    ...
    fn circle_rational_bezier() {
        ...
        let path = Path {
            commands: vec![PathCommand::RatQuad([
                ...
                (Point::new(RADIUS * weight, RADIUS * weight), weight),
        ...
        for segment in dbg!(&segments[1..]) {
            ...
            assert!(len_squared(segment.p0) - RADIUS * RADIUS < 1e-4);
            assert!(len_squared(segment.p1) - RADIUS * RADIUS < 1e-4);
        }
    }
