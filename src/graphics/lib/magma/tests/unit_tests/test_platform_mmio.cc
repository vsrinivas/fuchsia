...
static void test_mock_mmio(magma::PlatformMmio* mmio) {
  ...
  // Verify we can write to and read from the mmio space.
  {
    ...
    mmio->Write32Flipped(0, expected);
    uint32_t val = mmio->Read32(0);
    EXPECT_EQ(val, expected);

    mmio->Write32Flipped(mmio->size() - sizeof(uint32_t), expected);
...

  {
    ...
    mmio->Write64Flipped(0, expected);
    uint64_t val = mmio->Read64(0);
    EXPECT_EQ(val, expected);

    mmio->Write64Flipped(mmio->size() - sizeof(uint64_t), expected);
