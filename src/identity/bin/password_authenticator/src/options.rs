...
impl Options {
    ...
    pub fn validate(&self) -> Result<(), Error> {
        ...
        if !(self.allow_scrypt || self.allow_pinweaver) {
...
pub mod test {
    ...
    fn test_validate() {
        ...
        assert!(Options { allow_scrypt: false, allow_pinweaver: false }.validate().is_err());
        // Allowing a single type is valid
        assert!(Options { allow_scrypt: true, allow_pinweaver: false }.validate().is_ok());
        assert!(Options { allow_scrypt: false, allow_pinweaver: true }.validate().is_ok());
        // Mixing and matching is fine
        assert!(Options { allow_scrypt: true, allow_pinweaver: true }.validate().is_ok());
    }
}
