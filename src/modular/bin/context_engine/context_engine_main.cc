...
class ContextEngineApp {
 public:
  ContextEngineApp(sys::ComponentContext* const context) {
    auto component_context = context->svc()->Connect<fuchsia::modular::ComponentContext>();
    component_context->GetEntityResolver(entity_resolver_.NewRequest());
    context_engine_impl_ = std::make_unique<ContextEngineImpl>(entity_resolver_.get());

    context->outgoing()->AddPublicService<fuchsia::modular::ContextEngine>(
        [this](fidl::InterfaceRequest<fuchsia::modular::ContextEngine> request) {
  ...
  fxl::WeakPtr<ContextDebugImpl> debug() { return context_engine_impl_->debug(); }
...
int main(int argc, const char** argv) {
  ...
  modular::AppDriver<modular::ContextEngineApp> driver(
      context->outgoing(), std::move(context_engine_app), [&loop] { loop.Quit(); });
