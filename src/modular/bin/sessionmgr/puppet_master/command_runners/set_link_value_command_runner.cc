...
void SetLinkValueCommandRunner::Execute(fidl::StringPtr story_id, StoryStorage* const story_storage,
                                        fuchsia::modular::StoryCommand command,
                                        fit::function<void(fuchsia::modular::ExecuteResult)> done) {
  FXL_CHECK(command.is_set_link_value());

  AddSetLinkValueOperation(
      &operation_queue_, story_storage, std::move(command.set_link_value().path),
      [new_value = std::move(command.set_link_value().value)](fidl::StringPtr* value) {
