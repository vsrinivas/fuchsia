...
fidl::InterfaceRequest<fuchsia::modular::MessageQueue> MessageQueueClient::NewRequest() {
  if (reader_.is_bound()) {
    reader_.Unbind();
    receiver_ = nullptr;
  }
...
void MessageQueueClient::GetToken(fit::function<void(fidl::StringPtr)> callback) {
...
void MessageQueueClient::OnReceive(fuchsia::mem::Buffer message, fit::function<void()> ack) {
