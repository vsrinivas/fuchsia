...
class TestContainer : public OperationContainer {
 public:
  ...
  fxl::WeakPtr<OperationContainer> GetWeakPtr() override { return weak_ptr_factory_.GetWeakPtr(); }
...
class TestOperation : public Operation<Args...> {
 public:
  ...
  TestOperation(fit::function<void()> task, ResultCall done)
      : Operation<Args...>("Test Operation", std::move(done)), task_(std::move(task)) {}
...
TEST_F(OperationTest, Lifecycle) {
  ...
  bool op_ran = false;
  bool op_done = false;
  auto op = std::make_unique<TestOperation<>>([&op_ran] { op_ran = true; },
                                              [&op_done, &container] {
                                                op_done = true;
...
TEST_F(OperationTest, ResultsAreReceived) {
  ...
  auto op = std::make_unique<TestOperation<int>>([] {}, [](int result) { EXPECT_EQ(42, result); });
 ...
 public:
  ...
  void SayDone(
    ...
    async::PostTask(async_get_default_dispatcher(),
                    [this, flow, result, call_before_flow_dies = std::move(call_before_flow_dies)] {
                      result_ = result;
...
TEST_F(OperationTest, Lifecycle_FlowToken) {
  ...
  int result{0};
  auto op = std::make_unique<TestFlowTokenOperation>([&result, &done_called](int r) {
    done_called = true;
    result = r;
  });
...
TEST_F(OperationTest, Lifecycle_FlowToken_OperationGoesAway) {
  ...
  bool done_called = false;
  auto op =
      std::make_unique<TestFlowTokenOperation>([&done_called](int result) { done_called = true; });
  ...
  RunLoopUntilIdle();
  if (weak_op) {
    static_cast<TestFlowTokenOperation*>(weak_op.get())->SayDone(42, [&container, weak_op]() {
...
TEST_F(OperationTest, OperationQueue) {
  ...
  bool op1_ran = false;
  bool op1_done = false;
  auto op1 = std::make_unique<TestOperation<>>([&op1_ran]() { op1_ran = true; },
...
TEST_F(OperationTest, OperationCollection) {
  ...
  bool op2_ran = false;
  bool op2_done = false;
  auto op2 = std::make_unique<TestOperation<>>([&op2_ran]() { op2_ran = true; },
...
class TestQueueNotNullPtr : public Operation<> {
 public:
  TestQueueNotNullPtr(ResultCall done) : Operation("TestQueueNotNullPtr", std::move(done)) {}

 private:
  void Run() override {
    // When |flow| goes out of scope, it will call Done() for us.
    FlowToken flow{this};
    operation_queue_.Add(std::make_unique<TestOperationNotNullPtr>(std::move(flow)));
  }
...
class TestCollectionNotNullPtr : public Operation<> {
 ...
 private:
  void Run() override {
    // When |flow| goes out of scope, it will call Done() for us.
    FlowToken flow{this};
    operation_collection_.Add(std::make_unique<TestOperationNotNullPtr>(std::move(flow)));
  }
...
TEST_F(OperationTest, TestQueueNotNullPtr) {
  ...
  bool done_called{false};
  auto op = std::make_unique<TestQueueNotNullPtr>([&done_called] { done_called = true; });
...
TEST_F(OperationTest, TestCollectionNotNullPtr) {
  ...
  bool done_called{false};
  auto op = std::make_unique<TestCollectionNotNullPtr>([&done_called] { done_called = true; });
