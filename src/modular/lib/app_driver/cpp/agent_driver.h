...
class AgentDriver : LifecycleImpl::Delegate, AgentImpl::Delegate, AgentHost {
 public:
  AgentDriver(sys::ComponentContext* const context, fit::function<void()> on_terminated)
      : context_(context),
        lifecycle_impl_(context->outgoing(), this),
        agent_impl_(std::make_unique<AgentImpl>(context->outgoing(),
                                                static_cast<AgentImpl::Delegate*>(this))),
        on_terminated_(std::move(on_terminated)),
        agent_context_(context_->svc()->Connect<fuchsia::modular::AgentContext>()),
