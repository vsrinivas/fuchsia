...
std::unique_ptr<StoryStorage> TestWithSessionStorage::GetStoryStorage(SessionStorage* const storage,
                                                                      std::string story_id) {
...
void TestWithSessionStorage::WriteModuleData(StoryStorage* const story_storage,
...
std::string TestWithSessionStorage::GetLinkValue(StoryStorage* const story_storage,
                                                 const std::string& link_name) {
...
std::string TestWithSessionStorage::GetLinkValue(StoryStorage* const story_storage,
  ...
  bool done{};
  std::string value;
  story_storage->GetLinkValue(path)->Then([&](StoryStorage::Status status, fidl::StringPtr v) {
    value = *v;
    done = true;
  });
  RunLoopUntil([&] { return done; });
