...
func Test128Data_128Kek(t *testing.T) {
	kek := mustDecodeString("000102030405060708090A0B0C0D0E0F")
	data := mustDecodeString("00112233445566778899AABBCCDDEEFF")

	ciphertext, err := Wrap(kek, data)
	if err != nil {
		t.Fatal("Wrapping failed with error. ", err)
	}
	expected := mustDecodeString("1FA68B0A8112B447AEF34BD8FB5A7B829D3E862371D2CFE5")
...
func Test128Data_192Kek(t *testing.T) {
	kek := mustDecodeString("000102030405060708090A0B0C0D0E0F1011121314151617")
	data := mustDecodeString("00112233445566778899AABBCCDDEEFF")
	...
	expected := mustDecodeString("96778B25AE6CA435F92B5B97C050AED2468AB8A17AD84E5D")
	if bytes.Compare(ciphertext, expected) != 0 {
		t.Fatal("Wrong ciphertext.")
	}
...
func Test128Data_256Kek(t *testing.T) {
	kek := mustDecodeString("000102030405060708090A0B0C0D0E0F101112131415161718191A1B1C1D1E1F")
	data := mustDecodeString("00112233445566778899AABBCCDDEEFF")
	...
	expected := mustDecodeString("64E8C3F9CE0F5BA263E9777905818A2A93C8191E7D6E8AE7")
	if bytes.Compare(ciphertext, expected) != 0 {
		t.Fatal("Wrong ciphertext.")
	}
...
func Test192Data_192Kek(t *testing.T) {
	kek := mustDecodeString("000102030405060708090A0B0C0D0E0F1011121314151617")
	data := mustDecodeString("00112233445566778899AABBCCDDEEFF0001020304050607")

	ciphertext, err := Wrap(kek, data)
	if err != nil {
		t.Fatal("Wrapping failed with error. ", err)
	}
	expected := mustDecodeString("031D33264E15D33268F24EC260743EDCE1C6C7DDEE725A936BA814915C6762D2")
	if bytes.Compare(ciphertext, expected) != 0 {
		t.Fatal("Wrong ciphertext.")
	}
...
func Test192Data_256Kek(t *testing.T) {
	...
	expected := mustDecodeString("A8F9BC1612C68B3FF6E6F4FBE30E71E4769C8B80A32CB8958CD5D17D6B254DA1")
	if bytes.Compare(ciphertext, expected) != 0 {
		t.Fatal("Wrong ciphertext.")
	}
...
func Test256Data_256Kek(t *testing.T) {
	kek := mustDecodeString("000102030405060708090A0B0C0D0E0F101112131415161718191A1B1C1D1E1F")
	data := mustDecodeString("00112233445566778899AABBCCDDEEFF000102030405060708090A0B0C0D0E0F")

	ciphertext, err := Wrap(kek, data)
	if err != nil {
		t.Fatal("Wrapping failed with error. ", err)
	}
	expected := mustDecodeString("28C9F404C4B810F4CBCCB35CFB87F8263F5786E2D80ED326CBC7F0E71A99F43BFB988B9B7A02DD21")
	if bytes.Compare(ciphertext, expected) != 0 {
		t.Fatal("Wrong ciphertext.")
	}
...
func TestInvalidKeyLength(t *testing.T) {
	kek := mustDecodeString("0102030405060708090A0B0C0D0E0F") // 240-bit
	data := mustDecodeString("00112233445566778899AABBCCDDEEFF000102030405060708090A0B0C0D0E0F")
...
func TestInvalidDataLength(t *testing.T) {
	kek := mustDecodeString("000102030405060708090A0B0C0D0E0F")
	data := mustDecodeString("012345678912345601234567891234560123456789123456012345678912345610")

	_, err := Wrap(kek, data)
	if err != ErrInvalidTextLength {
...
func TestTooShortDataLength(t *testing.T) {
	kek := mustDecodeString("000102030405060708090A0B0C0D0E0F")
	data := mustDecodeString("0011223344556677")
	...
	ciphertext := mustDecodeString("1FA68B0A8112B447AEF34BD8FB5A7B82")
