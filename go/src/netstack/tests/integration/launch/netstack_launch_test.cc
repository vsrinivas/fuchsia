...
TEST_F(NetstackLaunchTest, DISABLED_Launch) {
    ...
    auto netstack_launch = CreateLaunchInfo("fuchsia-pkg://fuchsia.com/netstack_integration#meta/netstack.cmx");
    env->AddServiceWithLaunchInfo(std::move(netstack_launch), Netstack::Name_);

    NetstackPtr netstack;
    env->ConnectToService(netstack.NewRequest());
    bool interfaces_gotten = false;
    netstack->GetInterfaces([&](::fidl::VectorPtr<NetInterface>){
        interfaces_gotten = true;
    });

    ASSERT_TRUE(
        RunLoopWithTimeoutOrUntil([&]{ return interfaces_gotten; }, zx::sec(10)));
}
