...
bool test_vector(void*) {
    ...
    for (size_t len = 0; len <= sizeof(kInput); len++) {
        ...
        REQUIRE_TRUE(ac.check(), "buffer allocation failure");
        CRYPTO_chacha_20(buf.get(), kInput, len, kKey, kNonce, kCounter);
        EXPECT_BYTES_EQ(kOutput, buf.get(), len, "cipher stream mismatch");
...
UNITTEST_START_TESTCASE(chacha20_tests)
UNITTEST("TestVector", test_vector)
UNITTEST_END_TESTCASE(chacha20_tests, "chacha20", "Test ChaCha20 implementation",
                      nullptr, nullptr);
