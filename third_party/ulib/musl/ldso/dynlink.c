...
__NO_SAFESTACK static void loader_svc_config(const char* config) {
    zx_status_t status = loader_svc_rpc(LDMSG_ORDINAL_CONFIG,
                                        config, strlen(config),
                                        ZX_HANDLE_INVALID, NULL);
    if (status != ZX_OK)
        debugmsg("LDMSG_ORDINAL_CONFIG(%s): %s\n",
...
__NO_SAFESTACK static zx_status_t get_library_vmo(const char* name,
    ...
    return loader_svc_rpc(LDMSG_ORDINAL_LOAD_OBJECT, name, strlen(name),
    ...
    if ((status = _zx_channel_call(loader_svc, 0, ZX_TIME_INFINITE,
        ...
        if (status != ZX_ERR_CALL_FAILED)
            ...
            status = read_status;
               ...
               (rsp.header.ordinal != LDMSG_ORDINAL_CLONE)) {
        status = ZX_ERR_INVALID_ARGS;
...
__NO_SAFESTACK static void log_write(const void* buf, size_t len) {
    ...
    if (logger != ZX_HANDLE_INVALID)
        ...
        status = loader_svc_rpc(LDMSG_ORDINAL_DEBUG_PRINT, buf, len,
                                ZX_HANDLE_INVALID, NULL);
    else {
...
void __sanitizer_publish_data(const char* sink_name, zx_handle_t vmo) {
    pthread_rwlock_rdlock(&lock);
    zx_status_t status = loader_svc_rpc(LDMSG_ORDINAL_DEBUG_PUBLISH_DATA_SINK,
...
zx_status_t __sanitizer_get_configuration(const char* name,
                                          zx_handle_t *out_vmo) {
    pthread_rwlock_rdlock(&lock);
    zx_status_t status = loader_svc_rpc(LDMSG_ORDINAL_DEBUG_LOAD_CONFIG,
                                        name, strlen(name),
