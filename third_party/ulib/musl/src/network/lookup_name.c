...
static int name_from_hosts(struct address buf[static MAXADDRS], char canon[static 256],
...
int __res_mkquery(int, const char*, int, int, const unsigned char*, int, const unsigned char*,
                  unsigned char*, int);
int __res_msend_rc(int, const unsigned char* const*, const int*, unsigned char* const*, int*, int,
...
static int dns_parse_callback(void* c, int rr, const void* data, int len, const void* packet) {
    ...
    case RR_AAAA:
        ...
        break;
    case RR_CNAME:
        if (__dn_expand(packet, (const unsigned char*)packet + 512, data, tmp, sizeof tmp) > 0 &&
...
static int name_from_dns(struct address buf[static MAXADDRS], char canon[static 256],
                         const char* name, int family, const struct resolvconf* conf) {
    ...
    if (family != AF_INET6) {
        qlens[nq] = __res_mkquery(0, name, 1, RR_A, 0, 0, 0, qbuf[nq], sizeof *qbuf);
        nq++;
    }
    if (family != AF_INET) {
        qlens[nq] = __res_mkquery(0, name, 1, RR_AAAA, 0, 0, 0, qbuf[nq], sizeof *qbuf);
        nq++;
    }
...
static int name_from_dns_search(struct address buf[static MAXADDRS], char canon[static 256],
                                const char* name, int family) {
    char search[256];
