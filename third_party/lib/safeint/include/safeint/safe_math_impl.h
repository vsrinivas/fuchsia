...
struct UnsignedIntegerForSize {
  typedef typename fbl::enable_if<
      fbl::numeric_limits<Integer>::is_integer,
...
struct TwiceWiderInteger {
  typedef typename fbl::enable_if<
      ...
      typename IntegerForSizeAndSign<
          sizeof(Integer) * 2,
          fbl::numeric_limits<Integer>::is_signed>::type>::type type;
};

template <typename Integer>
struct PositionOfSignBit {
  static const typename fbl::enable_if<fbl::numeric_limits<Integer>::is_integer,
                                       size_t>::type value =
      8 * sizeof(Integer) - 1;
};
...
template <typename Numeric,
          bool IsInteger = fbl::numeric_limits<Numeric>::is_integer>
...
template <typename T>
typename fbl::enable_if<fbl::numeric_limits<T>::is_integer, T>::type
CheckedAdd(T x, T y, RangeConstraint* validity) {
  ...
  // that of y.
  if (fbl::numeric_limits<T>::is_signed) {
...
template <typename T>
typename fbl::enable_if<fbl::numeric_limits<T>::is_integer &&
...
template <typename T>
typename fbl::enable_if<fbl::numeric_limits<T>::is_integer &&
                            fbl::numeric_limits<T>::is_signed &&
                            (sizeof(T) * 2 > sizeof(uintmax_t)),
                        T>::type
CheckedMul(T x, T y, RangeConstraint* validity) {
  ...
  if (!x || !y) {
    ...
    if (y > 0)
      *validity =
          x <= fbl::numeric_limits<T>::max() / y ? RANGE_VALID : RANGE_OVERFLOW;
    else
      *validity = y >= fbl::numeric_limits<T>::min() / x ? RANGE_VALID
                                                         : RANGE_UNDERFLOW;

  } else {
    if (y > 0)
      *validity = x >= fbl::numeric_limits<T>::min() / y ? RANGE_VALID
                                                         : RANGE_UNDERFLOW;
    else
      *validity =
          y >= fbl::numeric_limits<T>::max() / x ? RANGE_VALID : RANGE_OVERFLOW;
  }
...
typename fbl::enable_if<fbl::numeric_limits<T>::is_integer &&
                        ...
                        T>::type
CheckedMul(T x, T y, RangeConstraint* validity) {
  *validity = (y == 0 || x <= fbl::numeric_limits<T>::max() / y)
                  ? RANGE_VALID
                  : RANGE_OVERFLOW;
  return x * y;
}
...
T CheckedDiv(T x,
             ...
             typename fbl::enable_if<fbl::numeric_limits<T>::is_integer,
                                     int>::type = 0) {
  if (fbl::numeric_limits<T>::is_signed && x == fbl::numeric_limits<T>::min() &&
      y == static_cast<T>(-1)) {
    *validity = RANGE_OVERFLOW;
    return fbl::numeric_limits<T>::min();
  }
...
CheckedNeg(T value, RangeConstraint* validity) {
  *validity =
      value != fbl::numeric_limits<T>::min() ? RANGE_VALID : RANGE_OVERFLOW;
...
CheckedUnsignedAbs(T value) {
  ...
  return value == fbl::numeric_limits<T>::min()
             ? static_cast<UnsignedT>(static_cast<UnsignedT>(fbl::numeric_limits<T>::max()) + 1)
...
struct GetNumericRepresentation {
  static const NumericRepresentation value =
      fbl::numeric_limits<NumericType>::is_integer
          ? NUMERIC_INTEGER
          : NUMERIC_UNKNOWN;
};
