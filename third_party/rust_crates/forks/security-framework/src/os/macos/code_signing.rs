...
impl GuestAttributes {
    ...
    pub fn new() -> Self {
        Self { inner: CFMutableDictionary::new() }
    }
...
impl SecCode {
    ...
    pub fn path(&self, flags: Flags) -> Result<CFURL> {
        ...
        unsafe {
            cvt(SecCodeCopyPath(self.as_CFTypeRef() as _, flags.bits(), url.as_mut_ptr()))?;
...
impl SecStaticCode {
    ...
    pub fn path(&self, flags: Flags) -> Result<CFURL> {
        ...
        // The docs say we can pass a SecCodeRef instead of a SecStaticCodeRef.
        unsafe {
            cvt(SecCodeCopyPath(self.as_concrete_TypeRef(), flags.bits(), url.as_mut_ptr()))?;
