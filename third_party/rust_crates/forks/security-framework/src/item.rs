    ...
    pub fn search(&self) -> Result<Vec<SearchResult>> {
        unsafe {
            ...
            if let Some(limit) = self.limit {
                params.push((CFString::wrap_under_get_rule(kSecMatchLimit), limit.to_value()));
            }
    ...
    fn fmt(&self, fmt: &mut fmt::Formatter<'_>) -> fmt::Result {
        match *self {
            Self::Ref(ref reference) => {
                fmt.debug_struct("SearchResult::Ref").field("reference", reference).finish()
            }
            Self::Data(ref buf) => {
                fmt.debug_struct("SearchResult::Data").field("data", buf).finish()
            }
            Self::Dict(_) => {
...
mod test {
    ...
    fn limit_two() {
        let results =
            ItemSearchOptions::new().class(ItemClass::certificate()).limit(2).search().unwrap();
