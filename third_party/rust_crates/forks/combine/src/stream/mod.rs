...
pub trait RangeStreamOnce: StreamOnce + ResetStream {
        ...
        F: FnMut(Self::Token) -> bool,
    {
        ...
        if committed {
            ...
            PeekErr(Tracked::from(StreamErrorFor::<Self>::unexpected_static_message("")))
        }
    }
    ...
    Input: ?Sized + Stream,
{
    ...
    let x = input.uncons().err().map_or(false, |err| err.is_unexpected_end_of_input());
    ...
    match input.uncons_while(predicate) {
        ...
        Ok(x) => {
            if input.is_partial() && input_at_eof(input) {
                ...
                // retrieved
                CommitErr(Input::Error::from_error(input.position(), StreamError::end_of_input()))
            } else if x.len() == 0 {
                PeekOk(x)
            } else {
                CommitOk(x)
        ...
        match $decoder {
            ref mut decoder => match $read {
                ref mut read => 'outer: loop {
                    ...
                    match decoder.__before_parse_tokio_02(std::pin::Pin::new(&mut *read)).await {
                        Ok(x) => x,
                        Err(error) => {
                            break 'outer Err($crate::stream::decoder::Error::Io {
                                error,
        ...
        match $decoder {
            ref mut decoder => match $read {
                ref mut read => 'outer: loop {
                    ...
                    match decoder.__before_parse_tokio_03(std::pin::Pin::new(&mut *read)).await {
                        Ok(x) => x,
                        Err(error) => {
                            break 'outer Err($crate::stream::decoder::Error::Io {
                                error,
        ...
        match $decoder {
            ref mut decoder => match $read {
                ref mut read => 'outer: loop {
                    ...
                    match decoder.__before_parse_tokio(std::pin::Pin::new(&mut *read)).await {
                        Ok(x) => x,
                        Err(error) => {
                            break 'outer Err($crate::stream::decoder::Error::Io {
                                error,
