    ...
    Input::Token: Clone,
{
    ...
    pub fn new(iter: Input, lookahead: usize) -> Stream<Input> {
        Stream { offset: 0, iter, buffer_offset: 0, buffer: VecDeque::with_capacity(lookahead) }
    }
}
    ...
    Input: StreamOnce + Positioned,
{
    ...
    fn position(&self) -> Self::Position {
        if self.offset >= self.buffer_offset {
            ...
            self.buffer.front().expect("At least 1 element in the buffer").1.clone()
        } else {
            self.buffer[self.buffer.len() - (self.buffer_offset - self.offset)].1.clone()
        }
    }
}
    ...
    Input::Token: Clone,
{
    ...
    fn uncons(&mut self) -> Result<Input::Token, StreamErrorFor<Self>> {
        if self.offset >= self.buffer_offset {
            ...
            let value =
                self.buffer[self.buffer.len() - (self.buffer_offset - self.offset)].0.clone();
            self.offset += 1;
            Ok(value)
        }
    }
