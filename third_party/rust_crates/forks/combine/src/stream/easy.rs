    ...
    Position: Ord + Clone,
{
    ...
    fn is_unexpected_end_of_input(&self) -> bool {
        self.errors.iter().any(StreamError::is_unexpected_end_of_input)
    }
...
impl<T, R, P> Errors<T, R, P> {
        ...
        F: FnMut(T) -> U,
    {
        Errors::from_errors(
            self.position,
            self.errors.into_iter().map(|error| error.map_token(&mut f)).collect(),
        )
    }
        ...
        F: FnMut(R) -> S,
    {
        Errors::from_errors(
            self.position,
            self.errors.into_iter().map(|error| error.map_range(&mut f)).collect(),
        )
    }
}
    ...
    S::Range: PartialEq,
{
    ...
    fn reset(&mut self, checkpoint: Self::Checkpoint) -> Result<(), Self::Error> {
        self.0.reset(checkpoint).map_err(crate::error::ParseError::into_other)
    }
}
