...
impl<E> From<E> for Tracked<E> {
    fn from(error: E) -> Self {
        Tracked { error, offset: ErrorOffset(1) }
    }
}
...
impl<T, E> ParseResult<T, E> {
    ...
    pub fn as_ref(&self) -> ParseResult<&T, &E> {
        match *self {
            ...
            PeekErr(ref e) => PeekErr(Tracked { error: &e.error, offset: e.offset }),
        }
    }
        ...
        F: FnOnce(E) -> E2,
    {
        match self {
            ...
            PeekErr(e) => PeekErr(Tracked { error: f(e.error), offset: e.offset }),
        }
    }
