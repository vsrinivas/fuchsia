    ...
    P: FnMut(Input::Token) -> bool,
{
    ...
    fn parse_lazy(&mut self, input: &mut Input) -> ParseResult<Self::Output, Input::Error> {
        satisfy_impl(input, |c| if (self.predicate)(c.clone()) { Some(c) } else { None })
    }
}
...
where
    ...
    P: FnMut(Input::Token) -> bool,
{
    Satisfy { predicate, _marker: PhantomData }
}
...
where
    ...
    P: FnMut(Input::Token) -> Option<R>,
{
    SatisfyMap { predicate, _marker: PhantomData }
}
        ...
        for c in self.tokens.clone() {
            match crate::stream::uncons(input) {
                ...
                PeekErr(mut error) => {
                    ...
                    return if committed { CommitErr(error.error) } else { PeekErr(error) };
                }
                CommitErr(mut error) => {
...
where
    ...
    Input: Stream,
{
    Tokens { cmp, expected, tokens, _marker: PhantomData }
}
...
{
    OneOf { tokens, _marker: PhantomData }
}
...
where
    ...
    Input::Token: PartialEq<T::Item>,
{
    NoneOf { tokens, _marker: PhantomData }
}
