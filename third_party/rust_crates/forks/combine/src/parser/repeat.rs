    ...
    F: Extend<P::Output> + Default,
{
    ...
    CountMinMax { parser, min, max, _marker: PhantomData }
}
    ...
    S: BorrowMut<P::PartialState>,
{
    pub fn new(parser: P, mode: M, input: &'a mut Input, partial_state: S) -> Self {
        Iter { parser, input, committed: false, state: State::Ok, partial_state, mode }
    }
    ...
    M: ParseMode,
{
    ...
    fn next(&mut self) -> Option<P::Output> {
        ...
        match self.parser.parse_mode(self.mode, self.input, self.partial_state.borrow_mut()) {
            PeekOk(v) => {
                self.mode.set_first();
                Some(v)
            }
...
where
    ...
    S: Parser<Input>,
{
    SepBy { parser, separator, _marker: PhantomData }
}
...
where
    ...
    S: Parser<Input>,
{
    SepBy1 { parser, separator, _marker: PhantomData }
}
...
where
    ...
    S: Parser<Input>,
{
    SepEndBy { parser, separator, _marker: PhantomData }
}
    ...
    S: Parser<Input>,
{
    type Output = F;
    type PartialState =
        (Option<Commit<()>>, F, <With<S, Optional<P>> as Parser<Input>>::PartialState);
...
where
    ...
    S: Parser<Input>,
{
    SepEndBy1 { parser, separator, _marker: PhantomData }
}
    ...
    Op::Output: FnOnce(P::Output, P::Output) -> P::Output,
{
        ...
        M: ParseMode,
    {
        ...
        loop {
            ...
            match (&mut self.1, &mut self.0).parse_mode(mode, input, child_state).into() {
                Ok(((op, r), rest)) => {
                    l = op(l, r);
...
where
    ...
    P: Parser<Input>,
{
    TakeUntil { end, _marker: PhantomData }
}
...
where
    ...
    E: Parser<Input>,
{
    RepeatUntil { parser, end, _marker: PhantomData }
}
                ...
                PeekErr(_) => {
                    ...
                    match uncons(input) {
                        ...
                        _ => {
                            ...
                            return if committed.is_peek() { PeekOk(()) } else { CommitOk(()) };
                        }
                    }
                }
                CommitErr(err) => return CommitErr(err),
...
where
    ...
    Q: Parser<Input>,
{
    Escaped { parser, escape, escape_parser }
}
        ...
        let mut consume = |item: J::Item| {
            ...
            match parser.parse_mode(mode, input, next) {
                ...
                PeekErr(err) => {
                    ...
                    Err((item, if *committed { CommitErr(err.error) } else { PeekErr(err) }))
                }
...
where
    ...
    F: Extend<Q::Output> + Default,
{
    Iterate { parser, iterable, _marker: PhantomData }
}
