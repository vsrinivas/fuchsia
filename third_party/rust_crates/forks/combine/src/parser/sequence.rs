...
impl<T, U: Default> Default for SequenceState<T, U> {
    fn default() -> Self {
        SequenceState { value: None, state: U::default() }
    }
}
    ...
    N: Parser<Input>,
{
        ...
        M: ParseMode,
    {
        ...
        let result = n_parser_cache.as_mut().unwrap().1.parse_committed_mode(mode, input, n_state);
        match result {
            PeekOk(x) => {
    ...
    N: Parser<Input>,
{
        ...
        M: ParseMode,
    {
        ...
        let result = n_parser_cache.as_mut().unwrap().2.parse_committed_mode(mode, input, n_state);
        match result {
            PeekOk(x) => {
