...
mod std_tests {
    ...
    {
        let (s, input) =
            many1::<String, _, _>(digit()).expected("integer").parse_stream(input).into_result()?;
        let mut n = 0;
        for c in s.chars() {
    ...
    fn field() {
        ...
        let c_decl =
            (word(), spaces(), char(':'), spaces(), word()).map(|t| (t.0, t.4)).parse("x: int");
...
";
        ...
        assert_eq!(result.map(|(x, c)| (x, c.map(|_| parsed_state))), Ok((123i64, state)));
    }
