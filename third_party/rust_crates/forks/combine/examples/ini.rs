...
where
    ...
    Input::Error: ParseError<Input::Token, Input::Range, Input::Position>,
{
    (between(token('['), token(']'), many(satisfy(|c| c != ']'))), whitespace(), properties())
...
"#;
    let mut expected = Ini { global: HashMap::new(), sections: HashMap::new() };
    ...
    R: Read,
{
    let mut text = String::new();
    read.read_to_string(&mut text).map_err(Error::Io)?;
    ini().parse(position::Stream::new(&*text)).map_err(Error::Parse)?;
    Ok(())
}
