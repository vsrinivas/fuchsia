...
mod tests_std {
    ...
    fn message_tests() {
        ...
        let mut committed1 = char('h').with(char('o')).message("expected message").map(|x| x);
        let mut committed2 = char('h').with(char('o')).map(|x| x).message("expected message");
        ...
        assert_eq!(empty0.easy_parse(position::Stream::new(input)), empty_expected);
        assert_eq!(empty1.easy_parse(position::Stream::new(input)), empty_expected);
        assert_eq!(empty2.easy_parse(position::Stream::new(input)), empty_expected);

        assert_eq!(committed0.easy_parse(position::Stream::new(input)), committed_expected);
        assert_eq!(committed1.easy_parse(position::Stream::new(input)), committed_expected);
        assert_eq!(committed2.easy_parse(position::Stream::new(input)), committed_expected);
    }
    ...
    fn expected_tests() {
        ...
        let mut committed1 = char('h').with(char('o')).expected("expected message").map(|x| x);
        let mut committed2 = char('h').with(char('o')).map(|x| x).expected("expected message");

        assert!(ok.easy_parse(position::Stream::new(input)).is_ok());

        let empty_expected = Err(Errors {
            position: SourcePosition { line: 1, column: 1 },
            errors: vec![Error::Unexpected('h'.into()), Error::Expected("expected message".into())],
        });
    ...
    fn try_tests() {
        ...
        assert_eq!(
            ...
            Err(Errors {
                position: SourcePosition { line: 1, column: 1 },
                errors: vec![Error::Unexpected('h'.into()), Error::Unexpected("test".into()),],
            })
        );
        assert_eq!(
            ...
            Err(Errors {
                position: SourcePosition { line: 1, column: 2 },
                errors: vec![Error::Unexpected('i'.into()), Error::Unexpected("test".into()),],
            })
        );
    }
    ...
    fn sequence_in_choice_array_parser_empty_err() {
        let mut parser =
            choice([(optional(char('a')), char('1')), (optional(char('b')), char('2'))]);
    ...
    fn count_min_max_empty_error() {
        assert_eq!(count_min_max(1, 1, char('a')).or(value(vec![])).parse("b"), Ok((vec![], "b")));
    }
    ...
    fn parser_macro_must_impl_parse_mode_issue_168() {
        assert_eq!(skip_until(attempt((char('a'), char('b')))).parse("aaab"), Ok(((), "ab")));
    }
    ...
    macro_rules! sequence_many_test {
        ($many:expr, $seq:expr) => {
            ...
            assert_eq!(parser.easy_parse("ab").map_err(|e| e.errors), expected_error,);
        };
    }
    ...
    macro_rules! sequence_sep_by_test {
        ($many:expr, $seq:expr) => {
            ...
            assert_eq!(parser.easy_parse("a,ab").map_err(|e| e.errors), expected_error,);
        };
    }
    ...
    fn choice_compose_issue_175() {
        let ident = |s| attempt(string(s));
        let mut parser = many::<Vec<_>, _, _>(
            position()
                .and(choice((ident("aa").skip(string(";")), choice((ident("bb"), ident("cc")))))),
        )
        .skip(string("."));
    ...
    fn test() {
        ...
        assert_eq!(
            ...
            Err(vec![Error::Unexpected('1'.into()), Error::Expected("letter".into()),]),
        );
    }
    ...
    fn test_nested_count_overflow() {
        ...
        let put = (bytes(b"PUT"), key()).map(|(_, key)| key).and(value_messages);
    ...
    fn not_followed_by_empty_error_issue_220() {
        ...
        assert_eq!(parser.easy_parse("let").map_err(|err| err.errors), Err(vec![]),);
    }
}
