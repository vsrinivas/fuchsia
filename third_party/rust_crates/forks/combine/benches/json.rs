    ...
    Input::Error: ParseError<Input::Token, Input::Range, Input::Position>,
{
    ...
    between(lex(char('{')), lex(char('}')), fields).map(Value::Object).expected("object")
}
...
fn bench_buffered_json(bencher: &mut Bencher<'_>) {
    ...
    bencher.iter(|| {
        ...
        let mut parser = json_value();
        match parser
            .easy_parse(position::Stream::with_positioner(buffer, SourcePosition::default()))
        {
            Ok((Value::Array(v), _)) => {
                black_box(v);
            }
...
fn bench(c: &mut Criterion) {
    ...
    c.bench_function("json_core_error_no_position", bench_json_core_error_no_position);
