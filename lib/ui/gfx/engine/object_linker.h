...
class ObjectLinker : public ObjectLinkerBase {
 public:
  ...
  uint64_t RegisterExport(E* object, zx::eventpair token,
  ...
  // The return value is a unique identifier that can be used to identify this
  // object to the ObjectLinker later.
  uint64_t RegisterImport(I* object, zx::eventpair token,
                           ErrorReporter* error_reporter);
...
template <typename I, typename E>
uint64_t ObjectLinker<I, E>::RegisterExport(E* object, zx::eventpair token,
                                            ErrorReporter* error_reporter) {
  ...
  return RegisterEntry(std::move(pending_export_entry),
                       std::move(export_entry), error_reporter, false);
}

// Template functions must be defined in the header.
template <typename I, typename E>
uint64_t ObjectLinker<I, E>::RegisterImport(I* object, zx::eventpair token,
                                            ErrorReporter* error_reporter) {
  ...
  return RegisterEntry(std::move(pending_import_entry),
                       std::move(import_entry), error_reporter, true);
}

// Template functions must be defined in the header.
template <typename I, typename E>
void ObjectLinker<I, E>::UnregisterExport(uint64_t export_handle) {
  UnregisterEntry(export_handle, false);
}

// Template functions must be defined in the header.
template <typename I, typename E>
void ObjectLinker<I, E>::UnregisterImport(uint64_t import_handle) {
