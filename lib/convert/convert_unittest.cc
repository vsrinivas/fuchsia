...
f1dl::VectorPtr<uint8_t> CreateArray(const char* data, size_t size) {
  f1dl::VectorPtr<uint8_t> result;
...
TEST(Convert, ToSlice) {
  ...
  f1dl::VectorPtr<uint8_t> array = CreateArray(str.data(), str.size());
  slice = ToSlice(str);
  EXPECT_EQ(str, std::string(slice.data(), slice.size()));
}
...
TEST(Convert, ToArray) {
  std::string str = "Hello";
  f1dl::VectorPtr<uint8_t> array = ToArray(str);
  EXPECT_EQ(str,
