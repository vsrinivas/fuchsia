...
class Owner : public ftl::RefCountedThreadSafe<Owner<TypeInfoT>> {
 public:
  typedef Ownable<TypeInfoT> OwnableType;
 ...
 protected:
  ...
  void BecomeOwnerOf(OwnableType* ownable) {
  ...
  void RelinquishOwnershipOf(OwnableType* ownable) {
 ...
 private:
  ...
  virtual void OnReceiveOwnable(std::unique_ptr<OwnableType> unreffed) = 0;
