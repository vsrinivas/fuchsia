...
wlan_ht_caps_t ToDdk(const HtCapabilities& src) {
    wlan_ht_caps_t dst{};
    dst.ht_capability_info = src.ht_cap_info.val();
    dst.ampdu_params = src.ampdu_params.val();
    dst.mcs_set.rx_mcs_head = src.mcs_set.rx_mcs_head.val();
    dst.mcs_set.rx_mcs_tail = src.mcs_set.rx_mcs_tail.val();
    dst.mcs_set.tx_mcs = src.mcs_set.tx_mcs.val();
    dst.ht_ext_capabilities = src.ht_ext_cap.val();
    dst.tx_beamforming_capabilities = src.txbf_cap.val();
    dst.asel_capabilities = src.asel_cap.val();
    return dst;
}
...
HtCapabilities FromDdk(const wlan_ht_caps_t& src) {
    ...
    dst.ht_cap_info.set_val(src.ht_capability_info);
    dst.ampdu_params.set_val(src.ampdu_params);
    dst.mcs_set.rx_mcs_head.set_val(src.mcs_set.rx_mcs_head);
    dst.mcs_set.rx_mcs_tail.set_val(src.mcs_set.rx_mcs_tail);
    dst.mcs_set.tx_mcs.set_val(src.mcs_set.tx_mcs);
    dst.ht_ext_cap.set_val(src.ht_ext_capabilities);
    dst.txbf_cap.set_val(src.tx_beamforming_capabilities);
    dst.asel_cap.set_val(src.asel_capabilities);
    return dst;
}

wlan_ht_op_t ToDdk(const HtOperation& src) {
    wlan_ht_op_t dst{};
    dst.primary_chan = src.primary_chan;
    dst.head = src.head.val();
    dst.tail = src.tail.val();
    dst.basic_mcs_set.rx_mcs_head = src.basic_mcs_set.rx_mcs_head.val();
    dst.basic_mcs_set.rx_mcs_tail = src.basic_mcs_set.rx_mcs_tail.val();
    dst.basic_mcs_set.tx_mcs = src.basic_mcs_set.tx_mcs.val();
    return dst;
}
...
HtOperation FromDdk(const wlan_ht_op_t& src) {
    ...
    dst.primary_chan = src.primary_chan;
    dst.basic_mcs_set.rx_mcs_head.set_val(src.basic_mcs_set.rx_mcs_head);
    dst.basic_mcs_set.rx_mcs_tail.set_val(src.basic_mcs_set.rx_mcs_tail);
    dst.basic_mcs_set.tx_mcs.set_val(src.basic_mcs_set.tx_mcs);
    return dst;
}

wlan_vht_caps_t ToDdk(const VhtCapabilities& src) {
    wlan_vht_caps_t dst{};
    dst.vht_capability_info = src.vht_cap_info.val();
    dst.supported_vht_mcs_and_nss_set = src.vht_mcs_nss.val();
    return dst;
}
...
VhtCapabilities FromDdk(const wlan_vht_caps_t& src) {
    ...
    dst.vht_cap_info.set_val(src.vht_capability_info);
    dst.vht_mcs_nss.set_val(src.supported_vht_mcs_and_nss_set);
    return dst;
}
...
wlan_vht_op_t ToDdk(const VhtOperation& src) {
    ...
    dst.vht_cbw = src.vht_cbw;
    dst.center_freq_seg0 = src.center_freq_seg0;
    dst.center_freq_seg1 = src.center_freq_seg1;
    dst.basic_mcs = src.basic_mcs.val();
...
VhtOperation FromDdk(const wlan_vht_op_t& src) {
    ...
    dst.center_freq_seg0 = src.center_freq_seg0;
    dst.center_freq_seg1 = src.center_freq_seg1;
    dst.basic_mcs.set_val(src.basic_mcs);
    return dst;
}
