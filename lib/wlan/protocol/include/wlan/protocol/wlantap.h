...
typedef struct wlantap_ioctl_create_wlanphy {
    ...
    zx_handle_t channel;
    // A user-supplied channel handle for sending events back to the client.
    // This is a workaround: ideally, we would use one channel, but events are not
    // yet supported by FIDL.
    zx_handle_t event_channel;
...
#define IOCTL_WLANTAP_CREATE_WLANPHY IOCTL(IOCTL_KIND_SET_TWO_HANDLES, IOCTL_FAMILY_WLANTAP, 0)
