...
if [[ $build == *"aarch64" ]]; then
	...
	sysroot=$fuchsia_root/out/build-zircon/build-user-arm64/sysroot
else
	shared_path=x64
	system_processor=x86_64
	sysroot=$fuchsia_root/out/build-zircon/build-user-x86-64/sysroot
fi
