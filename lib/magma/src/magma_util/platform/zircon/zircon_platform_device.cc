...
ZirconPlatformDevice::CpuMapPciMmio(unsigned int pci_bar, PlatformMmio::CachePolicy cache_policy)
{
    ...
    uint64_t size;
    zx_handle_t handle;
    zx_status_t status = pci().ops->map_resource(pci().ctx, pci_bar, cache_policy, &addr, &size, &handle);
    if (status != ZX_OK)
...
bool ZirconPlatformDevice::ReadPciConfig16(uint64_t addr, uint16_t* value)
{
...
std::unique_ptr<PlatformInterrupt> ZirconPlatformDevice::RegisterInterrupt()
{
    uint32_t max_irqs;
    zx_status_t status = pci().ops->query_irq_mode_caps(pci().ctx, ZX_PCIE_IRQ_MODE_LEGACY, &max_irqs);
    if (status != ZX_OK)
...
ZirconPlatformDevice::~ZirconPlatformDevice()
{
    // Clean up the pci config mapping that was made in ::Create().
    zx_status_t status = zx_vmar_unmap(zx_vmar_root_self(), reinterpret_cast<uintptr_t>(cfg_),
                                       cfg_size_);
    if (status != ZX_OK)
...
std::unique_ptr<PlatformDevice> PlatformDevice::Create(void* device_handle)
{
    ...
    return std::unique_ptr<PlatformDevice>(
        new ZirconPlatformDevice(zx_device, pci, cfg, cfg_size, cfg_handle));
}
