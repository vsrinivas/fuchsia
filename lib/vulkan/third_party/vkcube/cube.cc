...
#define GET_DEVICE_PROC_ADDR(dev, entrypoint)                                                      \
    {                                                                                              \
        if (!g_gdpa)                                                                               \
            g_gdpa =                                                                               \
                (PFN_vkGetDeviceProcAddr)vkGetInstanceProcAddr(demo->inst, "vkGetDeviceProcAddr"); \
                                  ...
                                  VkPipelineStageFlags dest_stages)
{
    ...
    vkCmdPipelineBarrier(demo->cmd, src_stages, dest_stages, 0, 0, NULL, 0, NULL, 1,
                         pmemory_barrier);
}
...
static void demo_draw_build_cmd(struct demo* demo, VkCommandBuffer cmd_buf)
{
    ...
    const VkClearValue clear_values[2] = {
            [0] = {.color.float32 = {0.2f, 0.2f, 0.2f, 0.2f}}, [1] = {.depthStencil = {1.0f, 0}},
    };
...
void demo_prepare_buffers(struct demo* demo)
{
    ...
    err = demo->fpGetPhysicalDeviceSurfaceCapabilitiesKHR(demo->gpu, demo->surface,
                                                          &surfCapabilities);
    assert(!err);

    uint32_t presentModeCount;
    err = demo->fpGetPhysicalDeviceSurfacePresentModesKHR(demo->gpu, demo->surface,
...
static void demo_prepare_textures(struct demo* demo)
{
    ...
    for (i = 0; i < DEMO_TEXTURE_COUNT; i++) {
        ...
        if ((props.linearTilingFeatures & VK_FORMAT_FEATURE_SAMPLED_IMAGE_BIT) &&
            ...
            demo_set_image_layout(demo, demo->textures[i].image, VK_IMAGE_ASPECT_COLOR_BIT,
                                  VK_IMAGE_LAYOUT_PREINITIALIZED,
                                  VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_ACCESS_HOST_WRITE_BIT,
                                  VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT,
                                  VK_PIPELINE_STAGE_TRANSFER_BIT);
...
static void demo_init_vk(struct demo* demo)
{
    ...
    if (demo->validate) {
        ...
        demo->DebugReportMessage = (PFN_vkDebugReportMessageEXT)vkGetInstanceProcAddr(
        ...
        dbgCreateInfo.flags = VK_DEBUG_REPORT_ERROR_BIT_EXT | VK_DEBUG_REPORT_WARNING_BIT_EXT;
        err =
            demo->CreateDebugReportCallback(demo->inst, &dbgCreateInfo, NULL, &demo->msg_callback);
        switch (err) {
            case VK_SUCCESS:
                break;
    ...
    demo->queue_props = (VkQueueFamilyProperties*)malloc(demo->queue_family_count *
    ...
    demo->fuchsia_state->view_provider_service = std::make_unique<mozart::ViewProviderService>(
        application_context_.get(), [demo](mozart::ViewContext view_context) {
            auto resize_callback = [demo](
                                       ...
                                       f1dl::InterfaceHandle<ui::gfx::ImagePipe> interface_handle) {
                demo->width = width;
                demo->height = height;
