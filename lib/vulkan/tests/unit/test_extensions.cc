...
// Note: the loader returns results based on the layer's manifest file, not the implementation
// of the vkEnumerateInstanceExtensionProperties and vkEnumerateDeviceExtensionProperties apis
// inside the layer.
...
TEST(Swapchain, InstanceExtensions)
{
    ...
    prop_count = 0;
    EXPECT_EQ(VK_SUCCESS, vkEnumerateInstanceExtensionProperties(nullptr, &prop_count, nullptr));
...
TEST(Swapchain, DeviceExtensions)
{
    ...
    VkDeviceCreateInfo device_create_info = {.sType = VK_STRUCTURE_TYPE_DEVICE_CREATE_INFO,
                                             ...
                                             .enabledExtensionCount =
                                                 ARRAY_SIZE(expected_device_extensions),
                                             .ppEnabledExtensionNames = expected_device_extensions,
                                             .pEnabledFeatures = nullptr};
    VkDevice device;
    EXPECT_EQ(VK_SUCCESS,
              vkCreateDevice(physical_devices[0], &device_create_info, nullptr, &device));
}
