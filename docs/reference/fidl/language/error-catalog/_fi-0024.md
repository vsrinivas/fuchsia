...
```fidl
{% includecode gerrit_repo="fuchsia/fuchsia" gerrit_path="tools/fidl/fidlc/tests/fidl/bad/doc_comment_on_parameter_list.test.fidl" exclude_regexp="\/\/ (Copyright 20|Use of|found in).*" %}
```
...
```fidl
{% includecode gerrit_repo="fuchsia/fuchsia" gerrit_path="tools/fidl/fidlc/tests/fidl/good/simple_one_way_method.test.fidl" exclude_regexp="\/\/ (Copyright 20|Use of|found in).*" %}
```
