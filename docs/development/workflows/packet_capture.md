...
`netdump` is part of the [universe dependency list](https://fuchsia.dev/fuchsia-src/development/build/boards_and_products#universe) of the core product. If a package server is available, there is no extra step to prepare the image. Just running `netdump` will fetch the binary.
