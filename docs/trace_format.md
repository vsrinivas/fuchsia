...
To make traces more compact, frequently references strings, such as event
...
The theoretical maximum length of a string is about 32 KB but in practice
this will be further reduced by the space required to store the rest of the
...
- `[0 .. 3]`: record type (7)
- `[4 .. 15]`: record size (inclusive of this word) as a multiple of 8 bytes
- `[16 .. 23]`: kernel object type (one of the MX_OBJ_TYPE_* constants from <magenta/syscalls-types.h>)
- `[24 .. 39]`: name (string ref)
- `[40 .. 43]`: number of arguments
- `[44 .. 63]`: reserved (must be zero)
