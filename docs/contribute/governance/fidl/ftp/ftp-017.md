...
a very current concern faced by developpers, and a current deficiency of FIDL.
The combination of [channel limits][channel_limits], the ease of defining
messages which can surpass those limits depending on runtime behavior, along
...
need to be able to rely on bindings to implement protocols which may exchange
large messages.

This FTP has many desirable ingredients, and will serve as a reference in the
...
Despite the above, this FTP is rejected for the following reasons:
...
* The introduction of the `box<T>` notation helps identify a place in a message
  ...
  thinking is that providing an annotation at the method level is preferrable,
...
using syntactic sugar as is done for [errors][ftp014], along with bindinings
support, is the current top contender to move forward on this topic.

# [FTP](../README.md)-017: Box&lt;Knox&gt;
...
limit is 64 Kbytes including FIDL headers.
...
A **box** is a container for a possibly large data object which may need to be
...
_Editor's note:_ Rejected [FTP-005: Method Impossible][ftp005] which proposed
banning all methods which could go beyond protocol limits. This is too
constraining due to the limitation of static analysis which prevent capturing
runtime behavior working to properly paginate messages, or manually 'box' them.
...
##### Footnote2
_Editor's note:_ Since this FTP was authored in 2018, the distinction between
value types and resources is formally part of the FIDL language, see
[FTP-057][ftp057].

<!-- xrefs -->
[ftp005]: /docs/contribute/governance/fidl/ftp/ftp-005.md
[ftp014]: /docs/contribute/governance/fidl/ftp/ftp-014.md
