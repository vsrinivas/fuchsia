...
1. Enact a policy preferring relative paths or source-absolute paths over
   system-absolute paths in such areas as build definitions, source code
   generators, and wherever else this may apply.
...
Nodes in a filesystem such as files, directories, and links are typically
identified by paths. All paths are relative to a node, though paths that are
relative to the root of a system are commonly referred to as system-absolute
paths (or absolute paths for short). Paths may be relative to a transient
property, for instance relative to the user's current working directory.

Paths may also be relative to another node, which acts as their root. For
instance RFCs can be found under a Fuchsia source checkout under the path
`docs/contribute/governance/rfcs/`, which itself is relative to the root
directory of the Fuchsia source checkout. Such a path has the benefit that it is
portable - different people can checkout the Fuchsia source tree in different
locations on the same or different filesystem, but still speak in the same terms
when referring to files inside the Fuchsia source tree. This path is also
hermetic to the Fuchsia checkout - it does not encode information that is
external to the Fuchsia source tree.

A common way to refer to the same path is by prefixing it with `//`. This is
also known as a project-absolute path or source-absolute path, because the path
that follows the prefix is relative to the root of the project checkout or of
the source tree.

On my personal computer the same path as above might be resolved for instance to
the absolute path `/home/shayba/fuchsia/docs/contribute/governance/rfcs/`. This
path is only meaningful on my personal computer and is not meaningful to other
people or computers. This path also reveals information that is external to the
Fuchsia source tree and is irrelevant - my username on my personal computer.

### Paths in the build system

Paths are commonly used in the Fuchsia build system, or in any build system, to
refer to files. Files may be sources that are part of the checkout, or perhaps
generated artifacts (outputs of build actions). The [Fuchsia build
system][fuchsia-build] uses [GN][gn] to define its build graph. There is an
[established best practice][gn-best-rebase-path] to prefer expressing paths as
being relative to another root directory, such as the [root build
directory][gn-root-build-dir] or the source root (expressed as `"//"`).

It's possible to generate absolute paths in GN. In fact it's the default
behavior of the built-in [`rebase_path()`][gn-rebase-path] function. Separate
from this RFC is a [proposal][gn-dev-thread] to change this.
...
may be required when specifying the details of the invocation to distribute as well
as in the contents of any uploaded artifacts as they refer to each other. This
is particularly true for distributed build systems that rely on caching, where
the invocation details are used as part of the cache key. Even when absolute
paths are allowed, their use can defeat caching mechanisms, since two clients
could check out the same source code but then send requests to the server that
differ in absolute paths.
...
that are absolute and normalized (i.e. `.` and `..` parts are resolved, links
are followed, etc') can be checked for equivalence by string identity. Since any
path can be made absolute and normalized, and since such a transformation is
idempotent, this offers a simple equivalence check for paths in a local
environment. However there is nothing restricting users from performing this
transformation on absolute paths if they find it more convenient, but since this
transformation from relative to absolute and/or normalized form is destructive
it cannot be performed in the other direction. Therefore it is more inclusive of
all use cases to prefer the relative form.
