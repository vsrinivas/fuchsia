...
void main() {
  setupLogger(
      name: 'inspect_dart_codelab', globalTags: ['part_3', 'integration_test']);

  test('start with fizzbuzz', () async {
    final env = await IntegrationTest.create(3);
  ...
  test('start without fizzbuzz', () async {
    final env = await IntegrationTest.create(3, includeFizzBuzz: false);
    final reverser = env.connectToReverser();
    final result = await reverser.reverse("hello");
    expect(result, equals('olleh'));
