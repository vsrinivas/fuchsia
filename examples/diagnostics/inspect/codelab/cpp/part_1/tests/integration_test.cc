...
class IntegrationTestPart1 : public codelab::testing::IntegrationTest {};

TEST_F(IntegrationTestPart1, StartWithFizzBuzz) {
  auto ptr = ConnectToReverser({.include_fizzbuzz = true});

  bool error = false;
  ptr.set_error_handler([&](zx_status_t unused) { error = true; });

  // CODELAB: Uncomment after Reverse no longer hangs.
  /*
  bool done = false;
  std::string result;
...
TEST_F(IntegrationTestPart1, StartWithoutFizzBuzz) {
