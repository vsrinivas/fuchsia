...
class EffectsProcessorTest : public EffectsLoaderTest {
 protected:
  audio::EffectsProcessor* effects_processor_;

  void SetUp() override {
    EffectsLoaderTest::SetUp();
    effects_processor_ = new ::media::audio::EffectsProcessor(&effects_loader_, 48000);
  }
  void TearDown() override {
    delete effects_processor_;
    EffectsLoaderTest::TearDown();
  }
};
...
TEST_F(EffectsLoaderTest, GetNumEffects) {
  ...
  EXPECT_EQ(effects_loader_.GetNumFx(&num_effects), ZX_OK);
  EXPECT_TRUE(num_effects == DfxBase::kNumTestEffects);

  // Verify null out_param
  EXPECT_NE(effects_loader_.GetNumFx(nullptr), ZX_OK);
}

// Tests the get_info ABI, and that the test DFXs behave as expected.
TEST_F(EffectsLoaderTest, GetInfo) {
  fuchsia_audio_dfx_description dfx_desc;

  EXPECT_EQ(effects_loader_.GetFxInfo(Effect::Delay, &dfx_desc), ZX_OK);
  ...
  EXPECT_EQ(effects_loader_.GetFxInfo(Effect::Swap, &dfx_desc), ZX_OK);
  ...
  EXPECT_EQ(effects_loader_.GetFxInfo(Effect::Rechannel, &dfx_desc), ZX_OK);
  ...
  EXPECT_NE(effects_loader_.GetFxInfo(Effect::Count, &dfx_desc), ZX_OK);
  // Verify null struct*
  EXPECT_NE(effects_loader_.GetFxInfo(Effect::Rechannel, nullptr), ZX_OK);
}

// Tests the get_control_info ABI, and that the test DFXs behave as expected.
TEST_F(EffectsLoaderTest, GetControlInfo) {
  fuchsia_audio_dfx_control_description dfx_control_desc;

  EXPECT_EQ(effects_loader_.GetFxControlInfo(Effect::Delay, 0, &dfx_control_desc), ZX_OK);
  ...
  EXPECT_NE(
      effects_loader_.GetFxControlInfo(Effect::Delay, DfxDelay::kNumControls, &dfx_control_desc),
      ZX_OK);
  // Verify null struct*
  EXPECT_NE(effects_loader_.GetFxControlInfo(Effect::Delay, 0, nullptr), ZX_OK);

  // Verify effects with no controls
  EXPECT_NE(effects_loader_.GetFxControlInfo(Effect::Rechannel, 0, &dfx_control_desc), ZX_OK);
  EXPECT_NE(effects_loader_.GetFxControlInfo(Effect::Swap, 0, &dfx_control_desc), ZX_OK);
  EXPECT_NE(effects_loader_.GetFxControlInfo(Effect::Count, 0, &dfx_control_desc), ZX_OK);
}

// Tests the create ABI.
TEST_F(EffectsLoaderTest, Create) {
  ...
  dfx_token = effects_loader_.CreateFx(Effect::Swap, frame_rate, kTestChans, kTestChans);
  EXPECT_NE(dfx_token, FUCHSIA_AUDIO_DFX_INVALID_TOKEN);

  dfx_token = effects_loader_.CreateFx(Effect::Rechannel, frame_rate, DfxRechannel::kNumChannelsIn,
                                       DfxRechannel::kNumChannelsOut);
  EXPECT_NE(dfx_token, FUCHSIA_AUDIO_DFX_INVALID_TOKEN);

  // Verify num_channels mismatch (is not equal, should be)
  EXPECT_EQ(effects_loader_.CreateFx(Effect::Delay, frame_rate, kTestChans, kTestChans - 1),
            FUCHSIA_AUDIO_DFX_INVALID_TOKEN);

  // Verify num_channels mismatch (is equal, should not be)
  EXPECT_EQ(effects_loader_.CreateFx(Effect::Rechannel, frame_rate, kTestChans, kTestChans),
            FUCHSIA_AUDIO_DFX_INVALID_TOKEN);

  // Verify effect out of range
  EXPECT_EQ(effects_loader_.CreateFx(Effect::Count, frame_rate, kTestChans, kTestChans),
            FUCHSIA_AUDIO_DFX_INVALID_TOKEN);

  // Verify channels out of range
  EXPECT_EQ(effects_loader_.CreateFx(Effect::Delay, frame_rate, FUCHSIA_AUDIO_DFX_CHANNELS_MAX + 1,
                                     FUCHSIA_AUDIO_DFX_CHANNELS_MAX + 1),
            FUCHSIA_AUDIO_DFX_INVALID_TOKEN);

  EXPECT_EQ(
      effects_loader_.CreateFx(Effect::Delay, frame_rate, FUCHSIA_AUDIO_DFX_CHANNELS_SAME_AS_IN,
                               FUCHSIA_AUDIO_DFX_CHANNELS_SAME_AS_IN),
      FUCHSIA_AUDIO_DFX_INVALID_TOKEN);
}

// Tests the delete ABI.
TEST_F(EffectsLoaderTest, Delete) {
  ...
  EXPECT_NE(effects_loader_.DeleteFx(FUCHSIA_AUDIO_DFX_INVALID_TOKEN), ZX_OK);
}
