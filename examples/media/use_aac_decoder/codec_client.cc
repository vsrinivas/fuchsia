...
CodecClient::CodecClient(async::Loop* loop)
    : loop_(loop), async_(loop_->async()), codec_events_binding_(this) {
  ...
  codec_events_binding_.set_error_handler(
      [] { Exit("codec_events_binding_ failed - exiting\n"); });
  ...
  temp_codec_request_ = codec_.NewRequest(loop_->async());
  // We treat CodecEvents as much as possible like a hidden part of creating the
  // CodecPtr.  If NewBinding() has !is_valid(), we rely on the Codec server to
  // close the main Codec channel async.
  codec_->SetEventSink(codec_events_binding_.NewBinding(loop_->async()));
}
...
void CodecClient::Stop() {
  // TODO(dustingreen): We can probably avoid the need to NOP the error handlers
  // once we move to FIDL events instead of using two channels.  Even now, this
  // may not be strictly necessary since the caller has done loop.Quit() and
  // loop.JoinThreads() already.
  // codec_.set_error_handler([] {});
  // codec_events_binding_.set_error_handler([] {});
  if (codec_.is_bound()) {
    codec_.Unbind();
  }
  if (codec_events_binding_.is_bound()) {
    codec_events_binding_.Unbind();
  }
}
