...
pub struct AuthProviderSupplier {
    /// A map from auth_provider_type to a `AuthProviderClient` used to launch the associated
    /// component.
    auth_provider_clients: Arc<HashMap<String, AuthProviderClient>>,
}
...
impl token_manager::AuthProviderSupplier for AuthProviderSupplier {
    ...
    fn get<'a>(
        ...
        let auth_provider_client: &AuthProviderClient =
            match self.auth_provider_clients.get(auth_provider_type) {
                Some(apc) => apc,
                None => {
        ...
        FutureObj::new(Box::new(auth_provider_client.get()))
    }
}
