...
TEST_F(StepThreadControllerTest, SharedLibThunks) {
  FileLine src_line("/path/src.cc", 1);
  FileLine dest_line("/path/dest.cc", 2);

  const uint64_t kAddrSrc = kModuleAddress + 0x100;  // Line 1
  const uint64_t kAddrThunk = kModuleAddress + 0x10000;  // No symbols.
  ...
  // things.
  const uint64_t kAddrDest = kModuleAddress + 0x200;
  ...
  // function so we push an entry on the stack. This should be automatically
  // resumed.
  exception.frames.emplace(exception.frames.begin());
  exception.frames[0].ip = kAddrThunk;
  exception.frames[0].sp = 0x5008;
  exception.frames[0].bp = 0x5000;
  InjectException(exception);
  EXPECT_EQ(2, resume_count());
