...
class Session : public AgentConnection::Sink {
 public:
  ...
  // Changes the connection type. This will cancel all pending transactions
  // if there are any. Can be used to clear out the connection as well.
  void SetAgentConnection(std::unique_ptr<AgentConnection> connection);

  // May be null.
  AgentConnection* agent_connection() const { return agent_connection_.get(); }
...
uint32_t Session::Send(
  ...
  agent_connection_->Send(std::move(serialized));
