...
MediaResult AudioPlugDetector::Start(AudioOutputManager* manager) {
...

  {
    ...
    for (const auto& target : WATCH_TARGETS) {
      ...
      mx::channel watcher;
      ssize_t res;
      res = ioctl_device_watch_dir(dirfd.get(), watcher.get_address());
      if (res < 0) {
...
mx_status_t AudioPlugDetector::ProcessChannel(
  ...
  // Read the message from the watched channel.  It should contain the name of a
  // newly added device.  If we fail to read the channel, propagate the error up
  // to the dispatcher framework.  It will automatically take care of closing
  // the channel for us.
