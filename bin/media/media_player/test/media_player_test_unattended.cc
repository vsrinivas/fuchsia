...
MediaPlayerTestUnattended::MediaPlayerTestUnattended(
        ...
        if (status.end_of_stream) {
          FXL_LOG(INFO) << "MediaPlayerTest "
                        << (fake_audio_out_.expected() ? "SUCCEEDED"
                                                       : "FAILED");
          quit_callback_(fake_audio_out_.expected() ? 0 : 1);
        }
      };

  fake_audio_out_.SetPtsUnits(48000, 1);

  fake_audio_out_.ExpectPackets({{0, 4096, 0x20c39d1e31991800},
  ...
  fuchsia::media::AudioOutPtr fake_audio_out_ptr;
  fake_audio_out_.Bind(fake_audio_out_ptr.NewRequest());

  media_player_->SetAudioOut(std::move(fake_audio_out_ptr));
