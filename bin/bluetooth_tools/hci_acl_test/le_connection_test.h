...
// This is a LE connection tester that works directly against the HCI transport
// classes. This tester performs the following:
//
...
//   - Listen to ACL packets and respond the ATT protocol requests without any
//     L2CAP state management.
class LEConnectionTest final {
 public:
  LEConnectionTest();
  bool Run(fxl::UniqueFD hci_dev_fd,
           const bluetooth::common::DeviceAddress& dst_addr,
           bool cancel_right_away = false);

 private:
  // Initializes the data channel and sends a LE connection request to
  // |dst_addr_|. Exits the run loop if an error occurs.
  ...
  // Called after the connection has been successfully established. Sends 3
  // times the maximum number of LE packets that can be stored in the
  // controller's buffers. Sends ATT protocol Handle-Value notification PDUs.
  ...
  void LogErrorStatusAndQuit(const std::string& msg,
