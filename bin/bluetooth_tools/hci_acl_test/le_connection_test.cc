...
LEConnectionTest::LEConnectionTest()
    : le_conn_complete_handler_id_(0u), disconn_handler_id_(0u) {}

bool LEConnectionTest::Run(fxl::UniqueFD hci_dev_fd,
                           const common::DeviceAddress& dst_addr,
  ...
  // We can pass this by reference to the callbacks below since the MessageLoop
  // is run within this scope and hence these variables will remain in scope.
  ...
  auto read_buf_size_cb = [&](hci::CommandChannel::TransactionId id,
    ...
    bredr_buffer_info = hci::DataBufferInfo(
        le16toh(return_params->hc_acl_data_packet_length),
        le16toh(return_params->hc_total_num_acl_data_packets));
  };
  auto le_read_buf_size_cb = [&, this](hci::CommandChannel::TransactionId id,
                                       const hci::EventPacket& reply) {
    auto return_params =
        reply.return_params<hci::LEReadBufferSizeReturnParams>();
    if (return_params->status != hci::Status::kSuccess) {
      LogErrorStatusAndQuit("LE Read Buffer Size failed",
                            return_params->status);
      return;
    }

    hci::DataBufferInfo le_buffer_info(
        le16toh(return_params->hc_le_acl_data_packet_length),
...
void LEConnectionTest::InitializeDataChannelAndCreateConnection(
  ...
  hci::Connection::LowEnergyParameters conn_params(
      hci::defaults::kLEConnectionIntervalMin,
      hci::defaults::kLEConnectionIntervalMax, 0x0000, 0x0000,
  ...
  auto params = cmd->mutable_view()
                    ->mutable_payload<hci::LECreateConnectionCommandParams>();
  ...
  params->peer_address_type =
      (dst_addr_.type() == common::DeviceAddress::Type::kLEPublic)
  ...
  // Since this is a background task, we use HCI_Command_Status as the
  // completion callback.
  auto le_conn_status_cb = [this](hci::CommandChannel::TransactionId id,
    ...
    if (payload.status != hci::Status::kSuccess) {
      LogErrorStatusAndQuit("LE Create Connection Status (failed)",
                            payload.status);
    }
  };
  ...
  auto le_conn_complete_cb = [this](const hci::EventPacket& event) {
    ...
    auto params =
        event.le_event_params<hci::LEConnectionCompleteSubeventParams>();
    ...
    auto disconn_cb = [this](const hci::EventPacket& event) {
      ...
      FXL_LOG(INFO) << fxl::StringPrintf(
          "Disconnected - handle: 0x%02x, reason: 0x%02x",
    ...
    disconn_handler_id_ = hci_->command_channel()->AddEventHandler(
        hci::kDisconnectionCompleteEventCode, disconn_cb,
  ...
  le_conn_complete_handler_id_ = hci_->command_channel()->AddLEMetaEventHandler(
      hci::kLEConnectionCompleteSubeventCode, le_conn_complete_cb,
      message_loop_.task_runner());
  ...
  hci_->command_channel()->SendCommand(
      std::move(cmd), message_loop_.task_runner(), le_conn_status_cb, nullptr,
  ...
  if (cancel_right_away) {
    ...
    hci_->command_channel()->SendCommand(
        std::move(cancel), message_loop_.task_runner(), cancel_complete_cb,
...
void LEConnectionTest::SendNotifications(
  ...
  for (unsigned int i = 0;
       i < hci_->acl_data_channel()->GetLEBufferInfo().max_num_packets() * 3;
       ++i) {
    auto packet = hci::ACLDataPacket::New(
        connection_handle, hci::ACLPacketBoundaryFlag::kFirstNonFlushable,
...
void LEConnectionTest::ACLDataRxCallback(
    std::unique_ptr<hci::ACLDataPacket> packet) {
  FXL_LOG(INFO) << "Received ACL packet on handle: "
                << fxl::StringPrintf("0x%04x", packet->connection_handle());

  // Since this is an LE connection using a LE-U logical link the payload should
  // contain a L2CAP packet. Look at the channel ID, if this is a ATT protocol
  // request then send back an error response, otherwise just sit back and let
  ...
  uint16_t l2cap_channel_id =
      le16toh(*reinterpret_cast<const uint16_t*>(payload.data() + 2));
  if (l2cap_channel_id != 4)
    return;
  ...
  auto response = hci::ACLDataPacket::New(
...
void LEConnectionTest::LogErrorStatusAndQuit(const std::string& msg,
                                             hci::Status status) {
...
hci::CommandChannel::CommandStatusCallback LEConnectionTest::GetStatusCallback(
  ...
  return std::bind(&LEConnectionTest::StatusCallback, this, command_name, _1,
                   _2);
}

void LEConnectionTest::StatusCallback(
    const std::string& command_name,
