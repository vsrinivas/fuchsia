...
bool HandleReadVersion(const CommandData* cmd_data, const fxl::CommandLine& cmd_line,
  ...
  std::cout << "  Sent HCI Vendor (Intel) Read Version (id=" << id << ")" << std::endl;
...
bool HandleReadBootParams(const CommandData* cmd_data, const fxl::CommandLine& cmd_line,
  ...
  auto id = SendCommand(cmd_data, std::move(packet), cb, complete_cb);
  std::cout << "  Sent HCI Vendor (Intel) Read Boot Params (id=" << id << ")" << std::endl;
...
bool HandleReset(const CommandData* cmd_data, const fxl::CommandLine& cmd_line,
  ...
  auto cb = [](hci::CommandChannel::TransactionId id, const hci::EventPacket& event) {};

  auto packet = hci::CommandPacket::New(kReset, sizeof(IntelResetCommandParams));
  ...
  // Once the reset command is sent, the hardware will shut down and we won't be able to get a
  // response back. Just exit the tool.
