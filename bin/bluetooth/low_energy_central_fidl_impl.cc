...
void LowEnergyCentralFidlImpl::ConnectPeripheral(const ::fidl::String& identifier,
  ...
  if (iter != connections_.end()) {
    if (iter->second) {
      callback(fidl_helpers::NewErrorStatus(::btfidl::ErrorCode::ALREADY,
  ...
  auto conn_cb = [ self, callback, id = identifier.get() ](auto status, auto conn_ref) {
    if (!self) return;
    ...
    if (iter == self->connections_.end()) {
      ...
      auto error =
          fidl_helpers::NewErrorStatus(::btfidl::ErrorCode::FAILED, "Connect request canceled");
    ...
    if (status != ::bluetooth::hci::Status::kSuccess) {
      FXL_DCHECK(!conn_ref);
      auto msg = fxl::StringPrintf("Failed to connect to device (id: %s)", id.c_str());
      FXL_LOG(ERROR) << msg;

      // TODO(armansito): Report PROTOCOL_ERROR only if |status| correspond to an actual HCI error
      // reported from the controller. LE conn mgr currently uses HCI error codes for internal
      // errors which needs to change.
      auto error = fidl_helpers::NewErrorStatus(::btfidl::ErrorCode::PROTOCOL_ERROR, msg);
    ...
    if (!iter->second) {
      ...
      // This can happen if a connect is requested after a previous request was canceled (e.g. if
      // ConnectPeripheral, DisconnectPeripheral, ConnectPeripheral are called in quick succession).
      // In this case we don't claim |conn_ref| since we already have a reference for this
      // peripheral.
      FXL_VLOG(2) << "Dropping extra connection ref due to previously canceled connection attempt";
    }

    callback(::btfidl::Status::New());
  };

  if (!adapter_manager_->GetActiveAdapter()->le_connection_manager()->Connect(identifier.get(),
                                                                              conn_cb)) {
    auto msg =
        fxl::StringPrintf("Cannot connect to unknown device id: %s", identifier.get().c_str());
    FXL_LOG(ERROR) << msg;
...
void LowEnergyCentralFidlImpl::DisconnectPeripheral(const ::fidl::String& identifier,
                                                    const DisconnectPeripheralCallback& callback) {
  auto iter = connections_.find(identifier.get());
  if (iter == connections_.end()) {
    auto msg =
        fxl::StringPrintf("Client not connected to device (id: %s)", identifier.get().c_str());
    FXL_LOG(ERROR) << msg;
    callback(fidl_helpers::NewErrorStatus(::btfidl::ErrorCode::NOT_FOUND, msg));
    return;
  }
...
void LowEnergyCentralFidlImpl::NotifyScanStateChanged(bool scanning) {
  if (delegate_) delegate_->OnScanStateChanged(scanning);
}

void LowEnergyCentralFidlImpl::NotifyPeripheralDisconnected(const std::string& identifier) {
  if (delegate_) delegate_->OnPeripheralDisconnected(identifier);
}
