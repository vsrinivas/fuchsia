...
mod tests {
    ...
    fn async_test<F, Fut>(keys: &[&str], f: F)
        where F: FnOnce(DeviceSettingsManagerServer) -> Fut,
              Fut: Future<Item = (), Error = fidl::CloseChannel>
    {
        let (mut exec, device_settings, _t) =
            setup(keys).expect("Setup should not have failed");
    ...
    fn setup(
        keys: &[&str],
    ) -> Result<(async::Executor, DeviceSettingsManagerServer, TempDir), ()> {
    ...
    fn test_int() {
        async_test(&["TestKey"], |mut device_settings| {
            device_settings.set_integer("TestKey".to_string(), 18)
    ...
    fn test_multiple_keys() {
        async_test(&["TestKey1", "TestKey2"], |mut device_settings| {
            device_settings.set_integer("TestKey1".to_string(), 18)
            .and_then(move |response| {
                assert!(response, "set_integer failed");
                ...
                assert!(response, "set_string failed");
                device_settings.get_integer("TestKey1".to_string())
                    .map(move |response| (response, device_settings))
            }).and_then(|(response, mut device_settings)| {
                assert_eq!(response, (18, Status::Ok));
