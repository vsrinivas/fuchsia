...
TEST_F(ViewManagerTest, CreateAChildView) {
  ...
  FXL_DCHECK(zx::eventpair::create(0u, &parent_view_owner_token,
                                   &parent_view_token) == ZX_OK);
  ...
  FXL_DCHECK(zx::eventpair::create(0u, &child_view_owner_token,
                                   &child_view_token) == ZX_OK);
  ...
  FXL_DCHECK(zx::eventpair::create(0u, &new_child_view_owner_token,
...
TEST_F(ViewManagerTest, SetChildProperties) {
  ...
  // Add the view to the parent
  parent_view_container->AddChild2(child_key, std::move(child_view_owner_token));
