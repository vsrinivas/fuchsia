...
void Renderer::Visitor::VisitNode(Node* r) {
  if (r->clip_to_self()) {
    ...
    for (auto& part : r->parts()) {
      if (part->IsKindOf<ShapeNode>()) {
        part->As<ShapeNode>()->Accept(&part_visitor);
      } else {
        // TODO(MZ-167): accept ShapeNode parts.  This might already work (i.e.
        // it might be as simple as saying "part->Accept(&part_visitor)"), but
    ...
    for (auto& child : r->children()) {
      child->Accept(&child_and_import_visitor);
    }
    for (auto& import : r->imports()) {
      import->delegate()->Accept(&child_and_import_visitor);
    }

    // Create a new "clip object" from the display-lists generated by the two
    // visitors above.
    display_list_.push_back(
        escher::Object(part_visitor.TakeDisplayList(),
                       child_and_import_visitor.TakeDisplayList()));
  } else {
