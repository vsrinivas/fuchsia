...
void ReleaseThunk(void* data) {
  auto release_callback = static_cast<ftl::Closure*>(data);
  (*release_callback)();
...
sk_sp<SkImage> CreateImageFromTexture(const GaneshContext::Scope& scope,
                                      ...
                                      const ftl::Closure& release_callback) {
  FTL_DCHECK(texture_id);
  FTL_DCHECK(width);
  FTL_DCHECK(height);
  ...
  return SkImage::MakeFromTexture(scope.gr_context().get(), desc,
                                  ...
                                  new ftl::Closure(release_callback));
}
