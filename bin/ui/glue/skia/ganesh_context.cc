...
GaneshContext::GaneshContext(const ftl::RefPtr<GLContext>& gl_context)
...
GaneshContext::Scope::Scope(const ftl::RefPtr<GaneshContext>& ganesh_context)
    : ganesh_context_(ganesh_context), gl_scope_(ganesh_context->gl_context_) {
  FTL_DCHECK(!ganesh_context_->scope_entered_);
  FTL_DCHECK(ganesh_context_->gr_context_);
  FTL_DCHECK(!ganesh_context_->is_lost());
