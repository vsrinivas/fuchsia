...
constexpr auto kMeshVertexPositionType = ui::gfx::ValueType::kVector2;
constexpr auto kMeshVertexNormalType = ui::gfx::ValueType::kNone;
constexpr auto kMeshVertexTexCoodType = ui::gfx::ValueType::kVector2;
constexpr auto kMeshIndexFormat = ui::gfx::MeshIndexFormat::kUint32;
...
void MeshBuffer::Prepare(Frame* frame,
  ...
  vk::DeviceSize vertex_size =
      (from_scratch ? 0 : vertex_buffer_->size()) +
  ...
  vk::DeviceSize index_size =
      (from_scratch ? 0 : index_buffer_->size()) +
...
std::pair<escher::BufferPtr, escher::BufferPtr> MeshBuffer::Preserve(
  ...
  if (vertex_buffer_->capacity() < total_vertex_size) {
    ReplaceBuffer(
        frame, vertex_buffer_, total_vertex_size, /* keep_content= */ true);
  }
  ...
  if (index_buffer_->capacity() < total_index_size) {
    ReplaceBuffer(
        frame, index_buffer_, total_index_size, /* keep_content= */ true);
  }
...
void MeshBuffer::ProvideBuffersToScenicMesh(scenic_lib::Mesh* scenic_mesh) {
  ...
  scenic_mesh->BindBuffers(
      ...
      kMeshIndexFormat, /* index_offset= */ 0, index_count_,
