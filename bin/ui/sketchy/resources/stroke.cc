...
void Stroke::TessellateAndMerge(Frame* frame, MeshBuffer* mesh_buffer) {
  ...
  if (is_path_updated_) {
    if (!delta_path_.empty()) {
      control_points_buffer_.AppendData(command, buffer_factory,
                                        delta_path_.control_points_data(),
                                        delta_path_.control_points_size());
      re_params_buffer_.AppendData(command, buffer_factory,
                                   delta_path_.re_params_data(),
                                   delta_path_.re_params_size());
      division_counts_buffer_.AppendData(command, buffer_factory,
                                         delta_path_.division_counts_data(),
      ...
      control_points_buffer_.SetData(command, buffer_factory,
                                     path_.control_points_data(),
                                     path_.control_points_size());
      re_params_buffer_.SetData(command, buffer_factory, path_.re_params_data(),
                                path_.re_params_size());
      division_counts_buffer_.SetData(command, buffer_factory,
                                      path_.division_counts_data(),
  ...
  auto pair = mesh_buffer->Preserve(frame, path_.vertex_count(),
                                    path_.index_count(), path_.bbox());
  ...
  tessellator_->Dispatch(stroke_info_buffer_, control_points_buffer_.get(),
