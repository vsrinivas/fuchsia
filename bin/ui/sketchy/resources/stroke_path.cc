...
StrokePath::StrokePath(sketchy::StrokePathPtr path) {
  ...
  for (const auto& seg : path->segments) {
    ExtendWithCurve(sketchy::CubicBezier2f{{
        {seg->pt0->x, seg->pt0->y},
        {seg->pt1->x, seg->pt1->y},
        {seg->pt2->x, seg->pt2->y},
        {seg->pt3->x, seg->pt3->y}}});
...
void StrokePath::ExtendWithCurve(sketchy::CubicBezier2f curve) {
...
void StrokePath::ExtendWithPath(const StrokePath* path) {
  control_points_.insert(
      control_points_.end(), path->control_points_.begin(),
      path->control_points_.end());
  re_params_.insert(
      re_params_.end(), path->re_params_.begin(), path->re_params_.end());
  segment_lengths_.insert(
      segment_lengths_.end(), path->segment_lengths_.begin(),
