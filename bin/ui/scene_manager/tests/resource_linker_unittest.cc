...
TEST_F(ResourceLinkerTest, AllowsImport) {
  ...
  linker->SetOnImportResolvedCallback(
      [ exported_ptr = exported.get(), &did_resolve ](
          Import*, Resource * resource, ImportResolutionResult cause)
          ->void {
            did_resolve = true;
  ...
  ImportPtr import =
      fxl::MakeRefCounted<Import>(session_.get(), 2, scenic::ImportSpec::NODE);
...
TEST_F(ResourceLinkerTest, CannotImportWithDeadSourceAndDestinationHandles) {
  ...
  ImportPtr import =
      fxl::MakeRefCounted<Import>(session_.get(), 1, scenic::ImportSpec::NODE);
  ASSERT_FALSE(
      linker->ImportResource(import.get(),
                             scenic::ImportSpec::NODE,      // import spec
...
TEST_F(ResourceLinkerTest, ImportsBeforeExportsAreServiced) {
  ...
  linker->SetOnImportResolvedCallback(
      [ exported_ptr = exported.get(), &did_resolve ](
          Import * import, Resource * resource, ImportResolutionResult cause)
          ->void {
            did_resolve = true;
            ASSERT_TRUE(resource);
            ASSERT_EQ(exported_ptr, resource);
...
TEST_F(ResourceLinkerTest, UnresolvedImportIsRemovedIfDestroyed) {
  ...
  linker->SetOnImportResolvedCallback(
      [ exported_ptr = exported.get(), &resolution_count ](
          Import * import, Resource * resource, ImportResolutionResult cause)
          ->void {
            ASSERT_EQ(ImportResolutionResult::kImportDestroyedBeforeBind,
                      cause);
            resolution_count++;
          });
