...
void ReleaseData(void* pixels, void* context) {
  static_cast<mozart::client::HostData*>(context)->Release();
}
}  // namespace
...
HostSkSurfacePool::HostSkSurfacePool(mozart::client::Session* session,
                                     uint32_t num_images)
...
sk_sp<SkSurface> HostSkSurfacePool::GetSkSurface(uint32_t index) {
  ...
  const mozart::client::HostImage* image = image_pool_.GetImage(index);
  if (!image)
    return nullptr;
