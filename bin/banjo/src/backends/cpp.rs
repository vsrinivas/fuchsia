...
use {
    ...
    crate::backends::Backend, failure::{format_err, Error},
    heck::SnakeCase,
    std::io,
    std::iter,
...
fn name_buffer(ty: &str) -> &'static str {
    if ty == "void" { "buffer" } else { "list" }
}
fn name_size(ty: &str) -> &'static str {
    if ty == "void" { "size" } else { "count" }
}
    ...
    m.in_params.iter().map(|(name, ty)| {
        match ty {
            ...
            ast::Ty::Vector {..} => {
                ...
                Ok(format!("const {ty}* {name}_{buffer}, size_t {name}_{size}",
                           buffer = name_buffer(&ty),
                           size = name_size(&ty),
                           ty = ty,
                           name = to_c_name(name)))
            },
            _ => Ok(format!("{} {}", ty_to_cpp_str(ast, wrappers, ty).unwrap(), to_c_name(name)))
        }
    }).collect()
}

fn get_out_params(m: &ast::Method, name: &str, wrappers: bool, ast: &BanjoAst) -> Result<(Vec<String>, String), Error> {
    if m.attributes.has_attribute("Async") {
        return Ok((vec![format!("{protocol_name}_{method_name}_callback callback",
...
fn get_in_args(m: &ast::Method, wrappers: bool, ast: &BanjoAst) -> Result<Vec<String>, Error> {
    Ok(m.in_params.iter().map(|(name, ty)| {
        match ty {
            ast::Ty::Vector {..} => {
                let ty = ty_to_cpp_str(ast, wrappers, ty).unwrap();
                format!("{name}_{buffer}, {name}_{size}",
                        buffer = name_buffer(&ty),
                        size = name_size(&ty),
                        name = to_c_name(name))
            }
    ...
    Ok((m.out_params.iter().skip(skip_amt).map(|(name, ty)| {
        match ty {
            ...
            ast::Ty::Vector {..} => {
                ...
                if ty.is_nullable() {
                    format!("out_{name}_{buffer}, {name}_{size}",
                            buffer = name_buffer(&ty_name),
                            size = name_size(&ty_name),
                            name = to_c_name(name))
                } else {
...
fn filter_interface<'a>(decl: &'a ast::Decl) -> Option<(&'a String, &'a Vec<ast::Method>, &'a ast::Attrs)> {
...
/// Checks whether a decl is an interface, and if it is an interface, checks that it is a "ddk-protocol".
fn filter_protocol<'a>(decl: &'a ast::Decl) -> Option<(&'a String, &'a Vec<ast::Method>, &'a ast::Attrs)> {
    if let ast::Decl::Interface {ref name, ref methods, ref attributes} = *decl  {
        if let Some(layout) = attributes.get_attribute("Layout") {
...
impl<'a, W: io::Write> CppInternalBackend<'a, W> {
    ...
    fn codegen_decls(&self, name: &str, methods: &Vec<ast::Method>, ast: &BanjoAst) -> Result<String, Error> {
        methods.iter().map(|m| {
            let (out_params, return_param) = get_out_params(&m, name, true, ast)?;
            let in_params = get_in_params(&m, true, ast)?;

            let params = in_params.into_iter()
                                  .chain(out_params)
                                  .collect::<Vec<_>>()
                                  .join(", ");

            Ok(format!(include_str!("templates/cpp/internal_decl.h"),
    ...
    fn codegen_static_asserts(&self, name: &str, methods: &Vec<ast::Method>, ast: &BanjoAst) -> Result<String, Error> {
        methods.iter().map(|m| {
            let (out_params, return_param) = get_out_params(&m, name, true, ast)?;
            let in_params = get_in_params(&m, true, ast)?;
            ...
            Ok(format!(include_str!("templates/cpp/internal_static_assert.h"),
                       return_param = return_param,
                       params = params,
                       protocol_name = to_cpp_name(name),
                       protocol_name_snake = to_c_name(name),
    ...
    fn codegen_protocol(&self, ast: &BanjoAst) -> Result<String, Error> {
        ast.namespaces.iter().filter(|n| *n.0 != "zx").flat_map(|n| {
            n.1.iter().filter_map(filter_protocol).map(|(name, methods, _)| {
                Ok(format!(include_str!("templates/cpp/internal_protocol.h"),
                           protocol_name = to_cpp_name(name),
...
impl<'a, W: io::Write> Backend<'a, W> for CppInternalBackend<'a, W> {
    fn codegen(&mut self, ast: BanjoAst) -> Result<(), Error> {
        ...
        self.w
            .write_fmt(format_args!(include_str!("templates/cpp/internal.h"),
...
impl<'a, W: io::Write> CppBackend<'a, W> {
    ...
    fn codegen_proxy_defs(&self, name: &str, methods: &Vec<ast::Method>, ast: &BanjoAst) -> Result<String, Error> {
        methods.iter().map(|m| {
            ...
            accum.push_str(format!("    {return_param} {function_name}({params}) {{\n",
                                 return_param = return_param,
                                 params = params,
                                 function_name = to_cpp_name(m.name.as_str())).as_str());
            ...
            let args = iter::once("ctx_".to_string()).chain(in_args)
                                                     .chain(out_args)
                                                     .collect::<Vec<_>>()
                                                     .join(", ");

            let return_statement = if skip { "return " } else { "" };
            accum.push_str(format!("        {return_statement}ops_->{function_name_snake}({args});\n",
                                 return_statement = return_statement,
                                 args = args,
                                 function_name_snake = to_c_name(m.name.as_str())).as_str());
            accum.push_str("    }\n");
            Ok(accum)
        }).collect::<Result<Vec<String>, Error>>()
          .map(|fns| fns.join("\n"))
    ...
    fn codegen_interfaces(&self, namespaces: &Vec<ast::Decl>, ast: &BanjoAst) -> Result<String, Error> {
        namespaces.iter().filter_map(filter_interface).map(|(name, methods, attributes)| {
            Ok(format!(include_str!("templates/cpp/interface.h"),
                                    protocol_name = to_cpp_name(name),
    ...
    fn codegen_protocols(&self, namespaces: &Vec<ast::Decl>, ast: &BanjoAst) -> Result<String, Error> {
        namespaces.iter().filter_map(filter_protocol).map(|(name, methods, attributes)| {
            Ok(format!(include_str!("templates/cpp/protocol.h"),
                                    protocol_name = to_cpp_name(name),
                                    protocol_name_snake = to_c_name(name).as_str(),
                                    protocol_docs = get_doc_comment(attributes, 0),
    ...
    fn codegen_includes(&self, ast: &BanjoAst) -> Result<String, Error> {
        let mut includes = vec![
            ...
            ast.namespaces.iter().filter(|n| n.0 != "zx").map(|n| n.0.replace('.', "/"))
        ).map(|n| {
            format!("#include <{}.h>", n)
        }).chain(
            // Include handle headers for zx_handle_t wrapper types used in interfaces.
            ast.namespaces.iter().filter(|n| n.0 != "zx").flat_map(|n| {
                n.1.iter().filter_map(|decl| {
                    ...
                    methods.iter().flat_map(|method| {
                        method.in_params.iter().filter_map(|(_, ty)| {
                            match ty {
                                ast::Ty::Handle{..} => Some(ty),
                                _ => None,
                            }
                        }).chain(
                            method.out_params.iter().filter_map(|(_, ty)| {
                                match ty {
                                    ast::Ty::Handle{..} => Some(ty),
                                    _ => None,
                                }
            ...
            }).collect::<HashSet<String>>().into_iter().map(|ty| {
                format!("#include <lib/zx/{}.h>", &ty[4..])
        })).collect::<Vec<_>>();
        includes.sort();
        Ok(includes.join("\n"))
    }

    fn codegen_examples(&self, namespaces: &Vec<ast::Decl>, ast: &BanjoAst) -> Result<String, Error> {
        namespaces.iter().filter_map(filter_protocol).map(|(name, methods, _)| {
            let example_decls = methods.iter().map(|m| {
                ...
                Ok(format!("//     {return_param} {protocol_name}{function_name}({params});",
                           return_param = return_param,
                           protocol_name = to_cpp_name(name),
                           params = params,
            ...
            Ok(format!(include_str!("templates/cpp/example.h"),
                       protocol_name = to_cpp_name(name),
                       protocol_name_snake = to_c_name(name),
...
impl<'a, W: io::Write> Backend<'a, W> for CppBackend<'a, W> {
    fn codegen(&mut self, ast: BanjoAst) -> Result<(), Error> {
        ...
        self.w
            .write_fmt(format_args!(include_str!("templates/cpp/header.h"),
