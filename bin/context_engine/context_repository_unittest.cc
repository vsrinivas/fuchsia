...
TEST_F(ContextRepositoryTest, WildcardQuery) {
  ...
  // Set some new values, and show that we still get updates.
  repository_.Set("topic3", "3");
  ASSERT_TRUE(listener.last_update);
  EXPECT_EQ("1", listener.last_update->values["topic1"]);
  EXPECT_EQ("2", listener.last_update->values["topic2"]);
  EXPECT_EQ("3", listener.last_update->values["topic3"]);
}
...
TEST_F(ContextRepositoryTest, Filter_ForEach) {
  TestListener listener;
  // for_each should return only elements that match another
  ...
  query->filters["array"]->set_for_each(std::move(partial_eq_filter));
  auto id = repository_.AddSubscription(std::move(query), &listener);
  EXPECT_FALSE(listener.last_update);
  listener.reset();
  repository_.RemoveSubscription(id);
