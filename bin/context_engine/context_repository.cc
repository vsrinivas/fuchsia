...
// |filter_array| filter implementation.
bool Filter_FilterArray(const ContextFilterPtr& filter,
                        const rapidjson::Value& value,
                        rapidjson::Document* output_value) {
  if (!value.IsArray())
    return false;
  ...
  for (auto& v : value.GetArray()) {
    ...
    if (EvaluateFilter(filter->get_filter_array(), v, &new_element)) {
...
bool EvaluateFilter(const ContextFilterPtr& filter,
  ...
  } else if (filter->is_filter_array()) {
    func = Filter_FilterArray;
  } else {
...
void ContextRepository::SetInternal(const std::string& topic,
  ...
  for (auto id : subscribed_ids) {
    const auto& subscription = subscriptions_[id];
    ContextUpdatePtr update;
