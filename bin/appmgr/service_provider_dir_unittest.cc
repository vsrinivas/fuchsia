...
TEST_F(ServiceProviderTest, BackingDir) {
  ...
  ServiceProviderDirImpl service_provider;
  auto backing_service_provider = fbl::AdoptRef(new ServiceProviderDirImpl);
  auto backing_dir = OpenAsDirectory(backing_service_provider);
  service_provider.set_backing_dir(std::move(backing_dir));

  AddService(&service_provider, "fake", 1);
  AddService(backing_service_provider.get(), "fake", 2, service2);
  // add same name service to backing dir
  AddService(backing_service_provider.get(), "fake", 1, service3);

  // should call child service
  TestService(&service_provider, "fake_service1", 1);

  // check that backing provider is able to access its service
  TestService(backing_service_provider.get(), "fake_service1", 3);

  // check that we can get backing_dir service from child
  fbl::RefPtr<fs::Service> echo;
  GetService(&service_provider, "fake_service2", &echo);
  ASSERT_TRUE(echo);
...
TEST_F(ServiceProviderTest, ParentAndBackingDirTogether) {
  ServiceProviderDirImpl service_provider;
  auto parent_service_provider = fbl::AdoptRef(new ServiceProviderDirImpl());
  zx::channel b1, b2;
