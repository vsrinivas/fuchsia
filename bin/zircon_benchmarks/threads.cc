...
bool ThreadCreateAndJoinTest() {
  ...
  FXL_CHECK(zx_thread_create(zx_process_self(), tname, sizeof(tname), 0,
                             &thread) == ZX_OK);
  ...
  FXL_CHECK(zx_vmar_map_old(vmar, 0, stack_vmo, 0, stack_size, perm,
                            &stack_base) == ZX_OK);
  ...
  FXL_CHECK(zx_object_wait_one(thread, ZX_THREAD_TERMINATED, ZX_TIME_INFINITE,
                               &observed) == ZX_OK);
