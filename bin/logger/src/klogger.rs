    ...
    F: 'static + Send + Fn(LogMessage, usize) + Sync,
{
    ...
    let f = loop_fn((callback, h), |(callback, handle)| {
        ...
        async::OnSignals::new(&handle, Signals::LOG_READABLE).and_then(|_| {
            ...
            let status = unsafe {
                zx::sys::zx_log_read(handle.raw_handle(), ZX_LOG_RECORD_MAX, buf.as_mut_ptr(), 0)
            };
            ...
            callback(l, size);
            Ok(Loop::Continue((callback, handle)))
        })
    });

    async::spawn(f.recover(|e| {
        eprintln!("not able to apply listener to kernel logs, failed: {:?}", e)
    }));
