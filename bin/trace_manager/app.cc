...
TraceManagerApp::TraceManagerApp(const Config& config)
  ...
  context_->outgoing_services()->AddService<TraceController>([this](
      fidl::InterfaceRequest<TraceController> request) {
    trace_controller_bindings_.AddBinding(&trace_manager_, std::move(request));
  });
}
