...
// Sets a boolean to true when called. For checking whether an asynchronous call
// results in the expected callback.
...
// FakeTaskRunner::Run().
//
// Based on test::TestCoroutineHandler, but using FakeTaskRunner instead of
// MessageLoop.
class TestCoroutineHandler : public coroutine::CoroutineHandler {
 public:
  ...
  void Continue(bool interrupt) override {
    // If interrupting, no need to delay the call as the test will not call
    // FakeTaskRunner::Run() itself.
    if (interrupt) {
      delegate_->Continue(interrupt);
      return;
    }
    task_runner_->QuitNow();
...
class PageStorageTest : public ::testing::Test {
 public:
  ...
  void SetUp() override {
    ...
    bool called;
    Status status;
    storage_->Init(callback::Capture(SetWhenCalled(&called), &status));
    RunTasks();
    ASSERT_TRUE(called);
  ...
  void RunTasks() { task_runner_->Run(); }

  // Runs a function in a coroutine, using FakeTaskRunner.
  void RunInCoroutine(
    ...
    coroutine_service_.StartCoroutine(
        [&](coroutine::CoroutineHandler* handler) {
          test_handler =
              std::make_unique<TestCoroutineHandler>(handler, task_runner_);
 ...
 protected:
  ...
  std::unique_ptr<const Commit> GetCommit(const CommitId& id) {
    ...
    storage_->GetCommit(
        id, callback::Capture(SetWhenCalled(&called), &status, &commit));
    RunTasks();
    EXPECT_TRUE(called);
    EXPECT_EQ(Status::OK, status);
    return commit;
  ...
  ::testing::AssertionResult DeleteFromJournal(Journal* journal,
    ...
    bool called;
    Status status;
    journal->Delete(key, callback::Capture(SetWhenCalled(&called), &status));
    RunTasks();
  ...
  std::vector<std::unique_ptr<const Commit>> GetUnsyncedCommits() {
    ...
    storage_->GetUnsyncedCommits(
        callback::Capture(SetWhenCalled(&called), &status, &commits));
    RunTasks();
    EXPECT_TRUE(called);
    EXPECT_EQ(Status::OK, status);
    return commits;
...
TEST_F(PageStorageTest, AddGetLocalCommits) {
  ...
  storage_->GetCommit(
      RandomCommitId(),
      callback::Capture(SetWhenCalled(&called), &status, &lookup_commit));
  RunTasks();
  ASSERT_TRUE(called);
...
TEST_F(PageStorageTest, AddCommitFromLocalDoNotMarkUnsynedAlreadySyncedCommit) {
  ...
  Status status;
  storage_->AddCommitFromLocal(
      commit->Clone(), {}, callback::Capture(SetWhenCalled(&called), &status));
  RunTasks();
  ASSERT_TRUE(called);
  EXPECT_EQ(Status::OK, status);
  ...
  storage_->MarkCommitSynced(
      id, callback::Capture(SetWhenCalled(&called), &status));
  RunTasks();
  ASSERT_TRUE(called);
  EXPECT_EQ(Status::OK, status);
...
TEST_F(PageStorageTest, CreateJournalHugeNode) {
  ...
  storage_->GetUnsyncedPieces(
      callback::Capture(SetWhenCalled(&called), &status, &object_identifiers));
  RunTasks();
  ASSERT_TRUE(called);
...
TEST_F(PageStorageTest, JournalCommitFailsAfterFailedOperation) {
  ...
  std::unique_ptr<PageStorageImpl> test_storage =
      PageStorageImplAccessorForTest::CreateStorage(
          task_runner_, &coroutine_service_, std::make_unique<FakePageDbImpl>(),
          RandomString(10));
...
TEST_F(PageStorageTest, UnsyncedPieces) {
  ...
  for (size_t i = 0; i < size; ++i) {
    ...
    EXPECT_TRUE(PutInJournal(journal.get(), fxl::StringPrintf("key%lu", i),
                             data_array[i].object_identifier.object_digest,
                             KeyPriority::LAZY));
  ...
  storage_->GetUnsyncedPieces(
      callback::Capture(SetWhenCalled(&called), &status, &objects));
  RunTasks();
  ASSERT_TRUE(called);
  EXPECT_EQ(Status::OK, status);
...
TEST_F(PageStorageTest, UntrackedObjectsComplex) {
  ...
  EXPECT_TRUE(PutInJournal(journal.get(), "key1",
                           data_array[1].object_identifier.object_digest,
                           KeyPriority::LAZY));
  EXPECT_TRUE(PutInJournal(journal.get(), "key2",
                           data_array[2].object_identifier.object_digest,
                           KeyPriority::LAZY));
  EXPECT_TRUE(PutInJournal(journal.get(), "key1",
                           data_array[2].object_identifier.object_digest,
                           KeyPriority::LAZY));
...
TEST_F(PageStorageTest, SyncMetadata) {
  ...
  for (auto key_and_value : keys_and_values) {
    ...
    storage_->GetSyncMetadata(key, callback::Capture(SetWhenCalled(&called),
    ...
    storage_->SetSyncMetadata(
        key, value, callback::Capture(SetWhenCalled(&called), &status));
    RunTasks();
    ASSERT_TRUE(called);
    EXPECT_EQ(Status::OK, status);
...
TEST_F(PageStorageTest, GetEntryFromCommit) {
  ...
  storage_->GetEntryFromCommit(
      *commit, "key not found",
      callback::Capture(SetWhenCalled(&called), &status, &entry));
  RunTasks();
  ASSERT_TRUE(called);
...
TEST_F(PageStorageTest, MarkRemoteCommitSyncedRace) {
  ...
  DelayingFakeSyncDelegate sync(
      callback::Capture(SetWhenCalled(&called), &sync_delegate_call));
  storage_->SetSyncDelegate(&sync);
...
TEST_F(PageStorageTest, GetUnsyncedCommits) {
  ...
  storage_->StartCommit(
      ...
      callback::Capture(SetWhenCalled(&called), &status, &journal_a));
  RunTasks();
  ASSERT_TRUE(called);
  EXPECT_EQ(Status::OK, status);
  ...
  storage_->StartCommit(
      root_id, JournalType::EXPLICIT,
      callback::Capture(SetWhenCalled(&called), &status, &journal_b));
  RunTasks();
  ASSERT_TRUE(called);
  EXPECT_EQ(Status::OK, status);
  ...
  storage_->StartMergeCommit(
      ...
      callback::Capture(SetWhenCalled(&called), &status, &journal_merge));
  RunTasks();
  ASSERT_TRUE(called);
  EXPECT_EQ(storage::Status::OK, status);
  ...
  storage_->StartCommit(
      root_id, JournalType::EXPLICIT,
      callback::Capture(SetWhenCalled(&called), &status, &journal_c));
  RunTasks();
  ASSERT_TRUE(called);
  EXPECT_EQ(Status::OK, status);
