...
Status PageDbEmptyImpl::CreateMergeJournal(const CommitId& base,
...
Status PageDbEmptyImpl::AddCommitStorageBytes(const CommitId& commit_id,
...
Status PageDbEmptyImpl::GetImplicitJournal(const JournalId& journal_id,
                                           std::unique_ptr<Journal>* journal) {
  return Status::NOT_IMPLEMENTED;
}
...
Status PageDbEmptyImpl::RemoveJournalEntry(const JournalId& journal_id,
...
Status PageDbEmptyImpl::GetUnsyncedPieces(std::vector<ObjectId>* object_ids) {
  return Status::NOT_IMPLEMENTED;
}
