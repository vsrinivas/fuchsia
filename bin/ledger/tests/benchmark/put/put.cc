...
void PutBenchmark::Run() {
  ...
  InitializeKeys(
      fxl::MakeCopyable([this, ledger = std::move(ledger)](
                            std::vector<f1dl::Array<uint8_t>> keys) mutable {
...
void PutBenchmark::InitializeKeys(
  ...
  std::vector<f1dl::Array<uint8_t>> keys_cloned;
...
void PutBenchmark::BindWatcher(std::vector<f1dl::Array<uint8_t>> keys) {
...
void PutBenchmark::RunSingle(int i, std::vector<f1dl::Array<uint8_t>> keys) {
  if (i == entry_count_) {
...
void PutBenchmark::PutEntry(f1dl::Array<uint8_t> key,
