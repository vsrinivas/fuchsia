...
void UpdateEntryBenchmark::Run() {
  ...
  f1dl::Array<uint8_t> key = generator_.MakeKey(0, key_size_);
...
void UpdateEntryBenchmark::RunSingle(int i, f1dl::Array<uint8_t> key) {
  if (i == entry_count_) {
    if (transaction_size_ > 0) {
      CommitAndShutDown();
    } else {
...
void UpdateEntryBenchmark::CommitAndRunNext(int i, f1dl::Array<uint8_t> key) {
