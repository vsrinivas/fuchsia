...
void PageDataGenerator::PutEntry(ledger::PagePtr* page,
  ...
  if (ref_strategy == ReferenceStrategy::INLINE) {
    ...
    (*page)->PutWithPriority(
        ...
        priority, [callback = std::move(callback)](ledger::Status status) {
  ...
  (*page)->CreateReferenceFromVmo(
      std::move(vmo).ToTransport(), fxl::MakeCopyable([
        ...
        (*page)->PutReference(
            std::move(key), std::move(*reference),
            priority, [callback = std::move(callback)](ledger::Status status) {
...
void PageDataGenerator::PutInTransaction(
  ...
  (*page)->StartTransaction(fxl::MakeCopyable([
    this, page, partial_keys = std::move(partial_keys), keys = std::move(keys),
    current_key_index, transaction_size, value_size, ref_strategy, priority,
    ...
    PutMultipleEntries(
        ...
        fxl::MakeCopyable([
          ...
          (*page)->Commit(fxl::MakeCopyable([
            this, page, keys = std::move(keys), current_key_index, value_size,
            ref_strategy, transaction_size, priority,
            callback = std::move(callback)
          ](ledger::Status status) mutable {
            ...
            PutInTransaction(
                page, std::move(keys), current_key_index + transaction_size,
                value_size, transaction_size, ref_strategy, priority, callback);
          }));
        }));
  }));
}
