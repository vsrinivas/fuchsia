...
void PageDataGenerator::PutEntry(ledger::PagePtr* page,
  ...
  if (ref_strategy == ReferenceStrategy::OFF ||
      (ref_strategy == ReferenceStrategy::AUTO &&
       value.size() < kMaxInlineDataSize)) {
...
void PageDataGenerator::PutInTransaction(
  ...
  std::vector<fidl::Array<uint8_t>> partial_keys;
...
void PageDataGenerator::PutMultipleEntries(
  ...
  for (size_t i = 0; i < keys.size(); i++) {
    fidl::Array<uint8_t> value = generator_.MakeValue(value_size);
