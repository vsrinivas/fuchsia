...
TEST_F(MergeResolverTest, Empty) {
  ...
  RunLoopUntilIdle();
  ASSERT_TRUE(called);
  EXPECT_TRUE(RunLoopUntilWithTimeout([&] { return resolver.IsEmpty(); }));
...
TEST_F(MergeResolverTest, WaitOnMergeOfMerges) {
  ...
  // inserting tasks into the message loop.
...
TEST_F(MergeResolverTest, NoConflictCallback_ConflictsResolved) {
  ...
  EXPECT_TRUE(RunLoopUntilWithTimeout([&] { return callback_calls >= 2; }));
  EXPECT_TRUE(resolver.IsEmpty());
  ...
  EXPECT_FALSE(RunLoopUntilWithTimeout([&] { return callback_calls > 0; },
...
TEST_F(MergeResolverTest, NoConflictCallback_NoConflicts) {
  ...
  EXPECT_TRUE(RunLoopUntilWithTimeout([&] { return callback_calls >= 1; }));
  EXPECT_TRUE(resolver.IsEmpty());
...
TEST_F(MergeResolverTest, HasUnfinishedMerges) {
  ...
  resolver.set_on_empty(MakeQuitTask());
  EXPECT_TRUE(
      RunLoopUntilWithTimeout([&] { return !resolver.HasUnfinishedMerges(); }));
