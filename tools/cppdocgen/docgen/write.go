...
type WriteSettings struct {
	...
	// Number of path elements to strip from the beginning of a header path to make it relevant
	// to the user. This should also take into account "../.." to get from the build directory
	// to the source root. See GetUserIncludePath().
	StripPathEltCount int
...
func (s WriteSettings) GetUserIncludePath(path string) string {
	return stripPathLeftElements(path, s.StripPathEltCount)
}

func (s WriteSettings) fileSourceLink(file string) string {
	// TODO stripping the build directory needs parameterizing. Maybe should have a setting
	// for "build-to-source-root" and "library include path" to replace
	// WriteSettings.StripPathEltCount.
	repoPath := stripPathLeftElements(file, 2)

	// TODO this URL needs parameterizing. Should probably include the Git hash when this
	// is autogenerated since we should know exactly what it is.
	return "https://cs.opensource.google/fuchsia/fuchsia/+/main:" + repoPath
}
