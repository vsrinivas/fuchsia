...
static inline void cookie_maker_bake(const cookie_maker_t* proto, uint64_t token, zx_time_t time, cookie_maker_bake_callback callback, void* cookie) {
...
typedef struct baker_protocol_ops {
    void (*register)(void* ctx, const cookie_maker_t* intf);
    void (*de_register)(void* ctx);
} baker_protocol_ops_t;
...
static inline void baker_register(const baker_protocol_t* proto, void* intf_ctx, cookie_maker_ops_t* intf_ops) {
