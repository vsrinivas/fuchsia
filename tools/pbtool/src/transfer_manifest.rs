...
impl GenerateTransferManifest {
    ...
    pub fn generate(self) -> Result<()> {
        ...
        let mut product_bundle_transfer = TransferEntry {
            artifact_type: ArtifactType::ProductBundle,
            local: rebased_product_bundle_path.clone(),
            remote: "product_bundle".into(),
            entries: vec![],
        };
        product_bundle_transfer.entries.push(ArtifactEntry { name: "product_bundle.json".into() });
        for partition in &product_bundle.partitions.bootstrap_partitions {
            product_bundle_transfer.entries.push(ArtifactEntry {
        ...
        let transfer_manifest =
            TransferManifest::V1(TransferManifestV1 { entries: vec![product_bundle_transfer] });
