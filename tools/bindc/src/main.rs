...
fn handle_command(command: Command) -> Result<(), Error> {
    match command {
        ...
        Command::GenerateCpp { input, lint, output } => {
            generate_cpp::handle_generate_cpp(input, lint, output)
        }
    }
}
...
mod tests {
    ...
    fn get_test_generated_cpp(input: &str) -> Vec<String> {
        generate_cpp::generate_cpp_header(input, false)
            .unwrap()
            .split("\n")
            .map(|s| s.to_string())
            .filter(|x| !x.is_empty())
    ...
    fn duplicate_key_value() {
        ...
        assert!(generate_cpp::generate_cpp_header(test_str, false).is_err());
    }
