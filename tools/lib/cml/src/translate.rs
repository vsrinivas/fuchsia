...
use {
    crate::{
        ...
        DebugRegistration, Document, Environment, EnvironmentExtends, EnvironmentRef, EventMode,
        EventModesClause, EventSubscriptionsClause, Expose, ExposeFromRef, ExposeToRef, FromClause,
        Offer, OneOrMany, Path, PathClause, Program, ResolverRegistration, RightsClause,
...
pub fn compile(document: &Document) -> Result<fsys::ComponentDecl, Error> {
...
fn translate_program(program: &Program) -> Result<fsys::ProgramDecl, Error> {
    Ok(fsys::ProgramDecl {
                        ...
                        .flat_map(|subscription| {
                            ...
                            subscription.event.iter().map(move |event| fsys::EventSubscription {
                                ...
                                mode: Some(match mode {
                                    Some(EventMode::Sync) => fsys::EventMode::Sync,
...
fn translate_children(children_in: &Vec<Child>) -> Result<Vec<fsys::ChildDecl>, Error> {
        ...
        .map(|env| {
            Ok(fsys::EnvironmentDecl {
                ...
                extends: match env.extends {
                    Some(EnvironmentExtends::Realm) => Some(fsys::EnvironmentExtends::Realm),
                    Some(EnvironmentExtends::None) => Some(fsys::EnvironmentExtends::None),
...
fn extract_use_source(
    ...
    match in_obj.from.as_ref() {
        Some(UseFromRef::Parent) => Ok(fsys::Ref::Parent(fsys::ParentRef {})),
        Some(UseFromRef::Framework) => Ok(fsys::Ref::Framework(fsys::FrameworkRef {})),
        Some(UseFromRef::Debug) => Ok(fsys::Ref::Debug(fsys::DebugRef {})),
        Some(UseFromRef::Self_) => Ok(fsys::Ref::Self_(fsys::SelfRef {})),
...
fn extract_use_event_source(in_obj: &Use) -> Result<fsys::Ref, Error> {
    match in_obj.from.as_ref() {
        Some(UseFromRef::Parent) => Ok(fsys::Ref::Parent(fsys::ParentRef {})),
        Some(UseFromRef::Framework) => Ok(fsys::Ref::Framework(fsys::FrameworkRef {})),
        Some(UseFromRef::Named(name)) => {
...
fn expose_source_from_ref(
    ...
    match reference {
        ...
        ExposeFromRef::Framework => Ok(fsys::Ref::Framework(fsys::FrameworkRef {})),
        ExposeFromRef::Self_ => Ok(fsys::Ref::Self_(fsys::SelfRef {})),
    }
}
...
fn extract_offer_sources_and_targets(
    ...
    all_children: &HashSet<&Name>,
    all_collections: &HashSet<&Name>,
) -> Result<Vec<(fsys::Ref, Name, fsys::Ref, Name)>, Error> {
    let mut out = vec![];
...
fn extract_expose_target(in_obj: &Expose) -> Result<fsys::Ref, Error> {
    match &in_obj.to {
        Some(ExposeToRef::Parent) => Ok(fsys::Ref::Parent(fsys::ParentRef {})),
        Some(ExposeToRef::Framework) => Ok(fsys::Ref::Framework(fsys::FrameworkRef {})),
        ...
        (
            ...
            $(
                ...
                fn $test_name() {
                    ...
                    println!("Input: {}", input);
                    let input = serde_json::from_str(&input).expect("deserialization failed");
