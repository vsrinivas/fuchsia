...
type entry struct {
	...
	// Note: Although //build/testing/golden_test.gni requires a flat directory
	// of golden files, this program allows Golden to contain path separators.
...
func (m *manifest) regen() error {
	...
	return filepath.WalkDir(m.RegenGoldensDir, func(path string, d fs.DirEntry, err error) error {
		if err != nil {
			return err
		}
		if d.Type().IsRegular() && filepath.Ext(d.Name()) == ".golden" {
			rel, err := filepath.Rel(m.RegenGoldensDir, path)
			if err != nil {
				panic(err)
			}
			if _, ok := allGoldensSet[rel]; !ok {
