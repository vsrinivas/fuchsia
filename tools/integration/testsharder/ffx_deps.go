...
// AddFFXDeps selects and adds the files needed to run `ffx emu` and
// `ffx target flash` to the shard's list of dependencies.
func AddFFXDeps(s *Shard, buildDir string, flash bool) error {
	if len(s.Tests) == 0 {
