...
ACLDataTxPacket::ACLDataTxPacket(ConnectionHandle connection_handle,
    ...
    : common::MutablePacket<ACLDataHeader>(buffer, data_length),
...
void ACLDataTxPacket::EncodeHeader() {
  ...
  GetMutableHeader()->handle_and_flags = htole16(handle_and_flags);
...
ConnectionHandle ACLDataRxPacket::GetConnectionHandle() const {
  ...
  return le16toh(GetHeader().handle_and_flags) & 0x0FFF;
}
...
ACLPacketBoundaryFlag ACLDataRxPacket::GetPacketBoundaryFlag() const {
  ...
  return static_cast<ACLPacketBoundaryFlag>((le16toh(GetHeader().handle_and_flags) >> 12) & 0x0003);
}
...
ACLBroadcastFlag ACLDataRxPacket::GetBroadcastFlag() const {
  ...
  return static_cast<ACLBroadcastFlag>(le16toh(GetHeader().handle_and_flags) >> 14);
}
