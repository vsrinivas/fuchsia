...
The manifest must include at least one line for the manifest itself:
```
meta/package=path/to/generated/manifest.txt
```

The rest of the process relies on a tool called `pm` which is available under
`//tools`.
We are also going to use `$PACKAGE_DIR` to denote a staging dir where the
