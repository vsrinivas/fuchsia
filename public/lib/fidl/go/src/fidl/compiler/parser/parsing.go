...
// parseInterfaceDecl() takes the parameter attributes *core.Attributes.
//
...
func (p *Parser) parseAttributes() (attributes *core.Attributes) {
	if !p.OK() {
		return
	}
	...
	attributes = core.NewAttributes(p.lastConsumed)
	...
	for nextToken.Kind != lexer.RBracket {
		...
		if p.peekNextToken("Expecting to find an attribute value.").Kind == lexer.Name {
			...
			value = core.MakeStringLiteralValue(text, &valueToken)
		} else {
		...
		attributes.List = append(attributes.List, core.NewMojomAttribute(key, &keyToken, value))
...
func (p *Parser) parseModuleDecl() (moduleNamespace *core.ModuleNamespace) {
	moduleNamespace = core.NewModuleNamespace("", nil)
	if !p.OK() {
		return
	}
	if p.checkEOF() {
...
func (p *Parser) parseImportStatements() (imports []*core.ImportedFile) {
	imports = make([]*core.ImportedFile, 0)
	if !p.OK() {
		return
	}
	...
	for nextToken.Kind == lexer.Import {
		...
		imports = append(imports, core.NewImportedFile(fileName, &fileNameToken))
...
// INTRFC_DECL  -> interface name lbrace INTRFC_BODY rbrace semi
func (p *Parser) parseInterfaceDecl(attributes *core.Attributes) (mojomInterface *core.MojomInterface) {
	if !p.OK() {
		return
	}
	...
	mojomInterface = core.NewMojomInterface(p.DeclData(simpleName, nameToken, attributes))
...
// ATTR_INTRFC_ELEMENT  -> [ATTRIBUTES] INTRFC_ELEMENT
// INTRFC_ELEMENT       -> METHOD_DECL | ENUM_DECL | CONSTANT_DECL
func (p *Parser) parseInterfaceBody(mojomInterface *core.MojomInterface) bool {
	if !p.OK() {
		return p.OK()
	}
...
// METHOD_DECL -> name [ordinal] lparen [PARAM_LIST] rparen [response lparen [PARAM_LIST] rparen]semi
func (p *Parser) parseMethodDecl(attributes *core.Attributes) *core.MojomMethod {
	if !p.OK() {
		return nil
	}
	...
	var responseParams *core.MojomStruct = nil
	...
	mojomMethod := core.NewMojomMethod(declData, params, responseParams)
	return mojomMethod
}
...
func (p *Parser) parseParamList(methodName string, isRequest bool) (paramStruct *core.MojomStruct) {
	if !p.OK() {
		return nil
	}
	...
	if isRequest {
		paramStruct = core.NewSyntheticRequestStruct(methodName, lexer.Token{}, p.mojomFile)
	} else {
		paramStruct = core.NewSyntheticResponseStruct(methodName, lexer.Token{}, p.mojomFile)
	}
	...
	for nextToken.Kind != lexer.RParen {
		...
		if duplicateNameError := paramStruct.AddField(core.NewStructField(
...
// STRUCT_DECL   -> struct name lbrace STRUCT_BODY rbrace semi
func (p *Parser) parseStructDecl(attributes *core.Attributes) (mojomStruct *core.MojomStruct) {
	if !p.OK() {
		return
	}
	...
	nameToken := p.lastConsumed
	mojomStruct = core.NewMojomStruct(p.DeclData(simpleName, nameToken, attributes))
...
// ATTR_STRUCT_ELEMENT  -> [ATTRIBUTES] STRUCT_ELEMENT
// STRUCT_ELEMENT       -> STRUCT_FIELD | ENUM_DECL | CONSTANT_DECL
func (p *Parser) parseStructBody(mojomStruct *core.MojomStruct) bool {
	if !p.OK() {
		return p.OK()
	}
...
// STRUCT_FIELD         -> TYPE name [ordinal] [equals DEFAULT_VALUE] semi
// DEFAULT_VALUE        -> COMPATIBLE_VALUE_REF | default
func (p *Parser) parseStructField(attributes *core.Attributes) *core.StructField {
	if !p.OK() {
		return nil
	}
	...
	var defaultValue core.ValueRef
	...
	if p.tryMatch(lexer.Equals) {
		...
		if defaultValueToken.Kind == lexer.Default {
			...
			defaultValue = core.MakeDefaultLiteral(&defaultToken)
			defaultKeywordUsed = true
		} else {
			assigneeSpec := core.AssigneeSpec{
				fieldName,
				fieldType,
			}
	...
	field := core.NewStructField(declData, fieldType, defaultValue)
...
// UNION_DECL    -> union name lbrace UNION_BODY rbrace semi
func (p *Parser) parseUnionDecl(attributes *core.Attributes) (union *core.MojomUnion) {
	if !p.OK() {
		return
	}
	...
	nameToken := p.lastConsumed
	union = core.NewMojomUnion(p.DeclData(simpleName, nameToken, attributes))
...
// UNION_BODY         -> {UNION_FIELD_DECL}
// UNION_FIELD_DECL   -> [ATTRIBUTES] TYPE name [ordinal] semi
func (p *Parser) parseUnionBody(union *core.MojomUnion) bool {
	if !p.OK() {
		return p.OK()
	}
...
// ENUM_DECL -> enum name lbrace ENUM_BODY rbrace semi
func (p *Parser) parseEnumDecl(attributes *core.Attributes) (enum *core.MojomEnum) {
	if !p.OK() {
		return
	}
	...
	nameToken := p.lastConsumed
	enum = core.NewMojomEnum(p.DeclData(simpleName, nameToken, attributes))
...
// ENUM_BODY     -> [ ENUN_VALUE {, ENUM_VALUE} [,] ]
// ENUM_VALUE    -> [ATTRIBUTES] name [equals ENUM_VAL_INITIALIZER]
func (p *Parser) parseEnumBody(mojomEnum *core.MojomEnum) bool {
	if !p.OK() {
		return p.OK()
	}
...
// ENUM_VAL_INITIALIZER -> INT32_VAL | APPROPRIATE_ENUM_VALUE_REF
func (p *Parser) parseEnumValueInitializer(mojoEnum *core.MojomEnum, valueName string) core.ValueRef {
	if !p.OK() {
		return nil
	}
	...
	enumType := core.NewResolvedUserTypeRef(mojoEnum.FullyQualifiedName(), mojoEnum)

	valueToken := p.peekNextToken("Parsing an enum value initializer type.")
	valueRef := p.parseValue(core.AssigneeSpec{valueName, enumType})
	if valueRef == nil {
		return nil
	}
...
func (p *Parser) parseConstDecl(attributes *core.Attributes) (constant *core.UserDefinedConstant) {
	if !p.OK() {
		return
	}
	...
	constant = core.NewUserDefinedConstant(p.DeclData(name, nameToken, attributes), declaredType, value)
	p.matchSemicolon()
...
// VALUE_REF -> USER_VALUE_REF | LITERAL_VALUE | BUILT_IN_FLOAT_CONST
func (p *Parser) parseValue(assigneeSpec core.AssigneeSpec) core.ValueRef {
	if !p.OK() {
		return nil
	}
...
//LITERAL_VALUE  -> BOOL_LITERAL | string_literal | NUMBER_LITERAL
// BOOL_LITERAL  -> true | false
func (p *Parser) parseLiteral() core.LiteralValue {
	if !p.OK() {
		return core.LiteralValue{}
	}
	...
	case lexer.StringLiteral:
		...
		valueToken := p.lastConsumed
		return core.MakeStringLiteralValue(literal, &valueToken)
	case lexer.True:
		p.consumeNextToken()
		valueToken := p.lastConsumed
		return core.MakeBoolLiteralValue(true, &valueToken)
	case lexer.False:
		p.consumeNextToken()
		valueToken := p.lastConsumed
		return core.MakeBoolLiteralValue(false, &valueToken)
...
// NUMBER_LITERAL       -> [plus | minus] POS_NUM_LITERAL
// POS_NUM_LITERAL      -> POS_INT_LITERAL | POS_FLOAT_LITERAL
func (p *Parser) parseNumberLiteral() core.LiteralValue {
	if !p.OK() {
		return core.LiteralValue{}
	}
	...
	case lexer.IntConstDec, lexer.IntConstHex:
		...
		case 8:
			if signed {
				return core.MakeInt8LiteralValue(int8(value), &valueToken)
			} else {
				return core.MakeUint8LiteralValue(uint8(absoluteValue), &valueToken)
			}
		case 16:
			if signed {
				return core.MakeInt16LiteralValue(int16(value), &valueToken)
			} else {
				return core.MakeUint16LiteralValue(uint16(absoluteValue), &valueToken)
			}
		case 32:
			if signed {
				return core.MakeInt32LiteralValue(int32(value), &valueToken)
			} else {
				return core.MakeUint32LiteralValue(uint32(absoluteValue), &valueToken)
			}
		case 64:
			if signed {
				return core.MakeInt64LiteralValue(value, &valueToken)
			} else {
				return core.MakeUint64LiteralValue(absoluteValue, &valueToken)
			}
		default:
	...
	case lexer.FloatConst:
		...
		valueToken := p.lastConsumed
		return core.MakeDoubleLiteralValue(value, &valueToken)
...
func (p *Parser) tryParseBuiltInType() core.TypeRef {
	...
	builtInType := core.BuiltInType(typeName)
	if builtInType == nil {
		return nil
	}
	p.consumeNextToken()
	...
	if typeName == "handle" {
		if p.tryMatch(lexer.LAngle) {
			...
			if p.match(lexer.RAngle) {
				...
				if builtInType = core.BuiltInType(typeName); builtInType == nil {
...
// ARRAY_TYPE  -> array langle TYPE [comma int_const_dec] rangle [qstn]
func (p *Parser) tryParseArrayType() core.TypeRef {
	if !p.OK() {
		return nil
	}
...
func (p *Parser) parseTypeReference() core.TypeRef {
	if !p.OK() {
		return nil
	}
	...
	typeReference := core.NewUserTypeRef(identifier, nullable,
...
func (p *Parser) parseUserValueRef(assigneeSpec core.AssigneeSpec) *core.UserValueRef {
	if !p.OK() {
		return nil
	}
	...
	valueReference := core.NewUserValueRef(assigneeSpec, identifier,
...
func (p *Parser) DeclData(name string, nameToken lexer.Token, attributes *core.Attributes) core.DeclarationData {
	return core.DeclData(name, p.mojomFile, nameToken, attributes)
}

func (p *Parser) DeclDataWithOrdinal(name string, nameToken lexer.Token, attributes *core.Attributes, declaredOrdinal int64) core.DeclarationData {
	return core.DeclDataWithOrdinal(name, p.mojomFile, nameToken, attributes, declaredOrdinal)
}

////////////////// Scope Stack /////////////////////
///
func (p *Parser) pushScope(scope *core.Scope) {
