...
inline void TrimURL(const char* spec, size_t* begin, size_t* len, bool trim_path_end = true) {
  ...
  while (*begin < *len && ShouldTrimFromURL(spec[*begin])) (*begin)++;
  ...
  if (trim_path_end) {
    ...
    // input.
    while (*len > *begin && ShouldTrimFromURL(spec[*len - 1])) (*len)--;
  }
}
...
inline size_t CountConsecutiveSlashes(const char* str, size_t begin_offset, size_t str_len) {
  size_t count = 0;
  while (begin_offset + count < str_len && IsURLSlash(str[begin_offset + count])) ++count;
  return count;
}
