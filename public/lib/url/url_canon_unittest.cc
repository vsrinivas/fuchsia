...
TEST(URLCanonTest, UTF) {
  ...
  for (const auto& utf_case : utf_cases) {
    ...
    for (size_t ch = 0; ch < input_len; ch++) {
      success &= AppendUTF8EscapedChar(utf_case.input8, &ch, input_len,
                                       &output);
    }
    output.Complete();
...
TEST(URLCanonTest, Scheme) {
  ...
  for (const auto& scheme_case : scheme_cases) {
    ...
    bool success = CanonicalizeScheme(scheme_case.input, in_comp, &output1,
                                      &out_comp);
    output1.Complete();
...
TEST(URLCanonTest, Host) {
  IPAddressCase host_cases[] = {
    ...
    {"\xef\xbc\x85\xef\xbc\x94\xef\xbc\x91.com", "a.com", Component(0, 5), CanonHostInfo::NEUTRAL, -1, ""},
    {"%ef%bc%85%ef%bc%94%ef%bc%91.com", "a.com", Component(0, 5), CanonHostInfo::NEUTRAL, -1, ""},
      // ...%00 in fullwidth should fail (also as escaped UTF-8 input)
    {"\xef\xbc\x85\xef\xbc\x90\xef\xbc\x90.com", "%00.com", Component(0, 7), CanonHostInfo::BROKEN, -1, ""},
    {"%ef%bc%85%ef%bc%90%ef%bc%90.com", "%00.com", Component(0, 7), CanonHostInfo::BROKEN, -1, ""},
      // Basic IDN support, UTF-8 and UTF-16 input should be converted to IDN
    {"\xe4\xbd\xa0\xe5\xa5\xbd\xe4\xbd\xa0\xe5\xa5\xbd", "xn--6qqa088eba", Component(0, 14), CanonHostInfo::NEUTRAL, -1, ""},
    ...
    {"a\xe2\x80\x8c" "b\xe2\x80\x8d" "c", "abc",
    ...
    {"\xf0\x91\x80\x93.com", "xn--n00d.com",
    ...
    {"%3g%78%63%30%2e%30%32%35%30%2E.01", "%253gxc0.0250..01", Component(0, 17), CanonHostInfo::BROKEN, -1, ""},
      // Something that isn't exactly an IP should get treated as a host and
      // spaces escaped.
    {"192.168.0.1 hello", "192.168.0.1%20hello", Component(0, 19), CanonHostInfo::NEUTRAL, -1, ""},
      // Fullwidth and escaped UTF-8 fullwidth should still be treated as IP.
      // These are "0Xc0.0250.01" in fullwidth.
    {"\xef\xbc\x90%Ef%bc\xb8%ef%Bd%83\xef\xbc\x90%EF%BC%8E\xef\xbc\x90\xef\xbc\x92\xef\xbc\x95\xef\xbc\x90\xef\xbc%8E\xef\xbc\x90\xef\xbc\x91", "192.168.0.1", Component(0, 11), CanonHostInfo::IPV4, 3, "C0A80001"},
    ...
    {"[google.com]", "[google.com]", Component(0, 12), CanonHostInfo::BROKEN, -1, ""},
      ...
      // (incorrect).
    {"\xd1\x82(", "xn--%28-7ed", Component(0, 11),
      CanonHostInfo::NEUTRAL, -1, ""},
    ...
    {"12345678912345.12345678912345.de", "12345678912345.12345678912345.de", Component(0, 32), CanonHostInfo::NEUTRAL, -1, ""},
  ...
  for (const auto& host_case : host_cases) {
    if (host_case.input8) {
      ...
      bool success = CanonicalizeHost(host_case.input8, in_comp, &output,
                                      &out_comp);
      output.Complete();

      EXPECT_EQ(host_case.expected_family != CanonHostInfo::BROKEN,
                success) << "for input: " << host_case.input8;
  ...
  for (const auto& host_case : host_cases) {
    if (host_case.input8) {
      ...
      CanonicalizeHostVerbose(host_case.input8, in_comp, &output,
                              &host_info);
      output.Complete();

      EXPECT_EQ(host_case.expected_family, host_info.family);
      EXPECT_EQ(std::string(host_case.expected), out_str);
      EXPECT_EQ(host_case.expected_component.begin,
...
TEST(URLCanonTest, IPv4) {
  IPAddressCase host_cases[] = {
    ...
    {"192.168.0.1 hello", "", Component(), CanonHostInfo::NEUTRAL, -1, ""},
      // Very large numbers.
    {"0000000000000300.0x00000000000000fF.00000000000000001", "192.255.0.1", Component(0, 11), CanonHostInfo::IPV4, 3, "C0FF0001"},
    {"0000000000000300.0xffffffffFFFFFFFF.3022415481470977", "", Component(0, 11), CanonHostInfo::BROKEN, -1, ""},
...
TEST(URLCanonTest, IPv6) {
  IPAddressCase cases[] = {
    ...
    {"[::]", "[::]", Component(0,4), CanonHostInfo::IPV6, -1, "00000000000000000000000000000000"},
    {"[::1]", "[::1]", Component(0,5), CanonHostInfo::IPV6, -1, "00000000000000000000000000000001"},
    {"[1::]", "[1::]", Component(0,5), CanonHostInfo::IPV6, -1, "00010000000000000000000000000000"},

    // Leading zeros should be stripped.
    {"[000:01:02:003:004:5:6:007]", "[0:1:2:3:4:5:6:7]", Component(0,17), CanonHostInfo::IPV6, -1, "00000001000200030004000500060007"},

    // Upper case letters should be lowercased.
    {"[A:b:c:DE:fF:0:1:aC]", "[a:b:c:de:ff:0:1:ac]", Component(0,20), CanonHostInfo::IPV6, -1, "000A000B000C00DE00FF0000000100AC"},

    // The same address can be written with different contractions, but should
    // get canonicalized to the same thing.
    {"[1:0:0:2::3:0]", "[1::2:0:0:3:0]", Component(0,14), CanonHostInfo::IPV6, -1, "00010000000000020000000000030000"},
    {"[1::2:0:0:3:0]", "[1::2:0:0:3:0]", Component(0,14), CanonHostInfo::IPV6, -1, "00010000000000020000000000030000"},

    // Addresses with embedded IPv4.
    {"[::192.168.0.1]", "[::c0a8:1]", Component(0,10), CanonHostInfo::IPV6, -1, "000000000000000000000000C0A80001"},
    {"[::ffff:192.168.0.1]", "[::ffff:c0a8:1]", Component(0,15), CanonHostInfo::IPV6, -1, "00000000000000000000FFFFC0A80001"},
    {"[::eeee:192.168.0.1]", "[::eeee:c0a8:1]", Component(0, 15), CanonHostInfo::IPV6, -1, "00000000000000000000EEEEC0A80001"},
    {"[2001::192.168.0.1]", "[2001::c0a8:1]", Component(0, 14), CanonHostInfo::IPV6, -1, "200100000000000000000000C0A80001"},
    {"[1:2:192.168.0.1:5:6]", "", Component(), CanonHostInfo::BROKEN, -1, ""},

    // IPv4 with last component missing.
    {"[::ffff:192.1.2]", "[::ffff:c001:2]", Component(0,15), CanonHostInfo::IPV6, -1, "00000000000000000000FFFFC0010002"},

    // IPv4 using hex.
    // TODO(eroman): Should this format be disallowed?
    {"[::ffff:0xC0.0Xa8.0x0.0x1]", "[::ffff:c0a8:1]", Component(0,15), CanonHostInfo::IPV6, -1, "00000000000000000000FFFFC0A80001"},

    // There may be zeros surrounding the "::" contraction.
    {"[0:0::0:0:8]", "[::8]", Component(0,5), CanonHostInfo::IPV6, -1, "00000000000000000000000000000008"},

    {"[2001:db8::1]", "[2001:db8::1]", Component(0,13), CanonHostInfo::IPV6, -1, "20010DB8000000000000000000000001"},
    ...
    {"[1:2:3:4:5:6::8]", "[1:2:3:4:5:6:0:8]", Component(0,17), CanonHostInfo::IPV6, -1, "00010002000300040005000600000008"},
  ...
  for (const auto& test_case : cases) {
    ...
    EXPECT_EQ(std::string(test_case.expected_address_hex),
              BytesToHexString(host_info.address, host_info.AddressLength())) << " host " << test_case.input8;
    if (host_info.family == CanonHostInfo::IPV6) {
      EXPECT_STREQ(test_case.expected, out_str1.c_str());
      EXPECT_EQ(test_case.expected_component.begin,
                host_info.out_host.begin);
...
TEST(URLCanonTest, UserInfo) {
  ...
  for (const auto& user_info_case : user_info_cases) {
    ...
    bool success = CanonicalizeUserInfo(user_info_case.input,
                                        parsed.username,
                                        user_info_case.input,
                                        parsed.password,
                                        &output1,
                                        &out_user,
                                        &out_pass);
...
TEST(URLCanonTest, Port) {
  ...
  for (const auto& port_case : port_cases) {
    ...
    std::string out_str;
    StdStringCanonOutput output1(&out_str);
    bool success = CanonicalizePort(port_case.input,
                                    in_comp,
                                    port_case.default_port,
                                    &output1,
                                    &out_comp);
    output1.Complete();
...
TEST(URLCanonTest, Path) {
  DualComponentCase path_cases[] = {
    ...
    // ----- encoding tests -----
      // Basic conversions
    {"/\xe4\xbd\xa0\xe5\xa5\xbd\xe4\xbd\xa0\xe5\xa5\xbd", "/%E4%BD%A0%E5%A5%BD%E4%BD%A0%E5%A5%BD", Component(0, 37), true},
...
TEST(URLCanonTest, Query) {
  ...
  for (const auto& query_case : query_cases) {
    ...
    if (query_case.input8) {
      ...
      StdStringCanonOutput output(&out_str);
      CanonicalizeQuery(query_case.input8, in_comp, NULL, &output,
                        &out_comp);
      output.Complete();
...
TEST(URLCanonTest, CanonicalizeStandardURL) {
  ...
  for (const auto& url_case : url_cases) {
    ...
    bool success = CanonicalizeStandardURL(
        url_case.input, url_len, parsed, NULL, &output, &out_parsed);
    output.Complete();
...
TEST(URLCanonTest, CanonicalizeMailtoURL) {
  struct URLCase {
    ...
    {"mailto:addr1\0addr2?foo", 22, "mailto:addr1%00addr2?foo", true, Component(7, 13), Component(21, 3)},
