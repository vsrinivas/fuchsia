...
GURL GURL::GetWithEmptyPath() const {
  ...
  // the empty URL.
  if (!is_valid_ || !IsStandard()) return GURL();
  ...
  GURL other(*this);
  if (parsed_.path.is_invalid_or_empty()) return other;
...
int GURL::IntPort() const {
  if (parsed_.port.is_nonempty()) return url::ParsePort(spec_.data(), parsed_.port);
...
std::string GURL::PathForRequest() const {
  ...
  if (parsed_.query.is_valid()) path_len = parsed_.query.end() - parsed_.path.begin;
...
bool GURL::HostIsIPAddress() const {
  if (!is_valid_ || spec_.empty()) return false;
