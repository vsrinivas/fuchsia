...
bool FidlBlockingCopyToString(mx::datapipe_consumer source,
                              std::string* result) {
  FTL_CHECK(result);
  result->clear();
...
bool FidlBlockingCopyFromString(ftl::StringView source,
...
mx::datapipe_consumer FidlWriteStringToConsumerHandle(ftl::StringView source) {
  ...
  mx::datapipe_producer::create(1, size, 0u, &producer_handle,
                                &consumer_handle);
  FidlBlockingCopyFromString(source, std::move(producer_handle));
