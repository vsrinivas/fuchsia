...
template <typename T>
bool operator==(const f1dl::VectorPtr<T>& array_a, const f1dl::VectorPtr<T>& array_b) {
...
void ServiceSubscriber::HandleInstanceUpdates(
  ...
  subscription_->GetInstances(
      version,
      [this](uint64_t version, f1dl::VectorPtr<MdnsServiceInstancePtr> instances) {
        HandleInstanceUpdates(version, std::move(instances));
      });
}

void ServiceSubscriber::IssueCallbacks(
    const f1dl::VectorPtr<MdnsServiceInstancePtr>& instances) {
