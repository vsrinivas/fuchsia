...
static inline unsigned char bootbyte_read(void) {
    return rtc_read(RTC_BOOT_BYTE);
}

static inline void bootbyte_clear(void) {
    rtc_write(RTC_BOOT_BYTE, 0xffu);
}

static inline void bootbyte_set_normal(void) {
    rtc_write(RTC_BOOT_BYTE, RTC_BOOT_NORMAL);
}

static inline void bootbyte_set_recovery(void) {
    rtc_write(RTC_BOOT_BYTE, RTC_BOOT_RECOVERY);
}
