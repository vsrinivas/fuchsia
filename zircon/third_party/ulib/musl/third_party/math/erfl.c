...
static long double erfc1(long double x) {
  ...
  s = fabsl(x) - 1;
  P = pa[0] +
      s * (pa[1] + s * (pa[2] + s * (pa[3] + s * (pa[4] + s * (pa[5] + s * (pa[6] + s * pa[7]))))));
...
static long double erfc2(uint32_t ix, long double x) {
  ...
  if (ix < 0x4000b6db) { /* 1.25 <= |x| < 2.857 ~ 1/.35 */
    R = ra[0] +
        s * (ra[1] +
             s * (ra[2] +
                  s * (ra[3] + s * (ra[4] + s * (ra[5] + s * (ra[6] + s * (ra[7] + s * ra[8])))))));
    S = sa[0] +
        s * (sa[1] +
             s * (sa[2] +
                  s * (sa[3] +
                       s * (sa[4] + s * (sa[5] + s * (sa[6] + s * (sa[7] + s * (sa[8] + s))))))));
  } else if (ix < 0x4001d555) { /* 2.857 <= |x| < 6.6666259765625 */
    R = rb[0] +
        s * (rb[1] +
             s * (rb[2] + s * (rb[3] + s * (rb[4] + s * (rb[5] + s * (rb[6] + s * rb[7]))))));
    S = sb[0] +
...
long double erfl(long double x) {
  ...
  if (ix >= 0x7fff0000) /* erf(nan)=nan, erf(+-inf)=+-1 */
    return 1 - 2 * sign + 1 / x;
...
long double erfcl(long double x) {
  ...
  if (ix >= 0x7fff0000) /* erfc(nan) = nan, erfc(+-inf) = 0,2 */
    return 2 * sign + 1 / x;
  if (ix < 0x3ffed800) { /* |x| < 0.84375 */
