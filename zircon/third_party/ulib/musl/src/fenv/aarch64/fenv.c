...
static inline void set_fpcr(uint32_t value) { __asm__("msr fpcr, %0" ::"r"((uint64_t)value)); }
...
static inline void set_fpsr(uint32_t value) { __asm__("msr fpsr, %0" ::"r"((uint64_t)value)); }

int fegetround(void) { return get_fpcr() & ROUND_MASK; }
...
int fetestexcept(int mask) { return get_fpsr() & mask & FE_ALL_EXCEPT; }
