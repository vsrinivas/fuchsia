...
iconv_t iconv_open(const char* to, const char* from) {
  ...
  if ((t = find_charmap(to)) == -1 || (f = find_charmap(from)) == -1 || (charmaps[t] >= 0320)) {
    errno = EINVAL;
    return (iconv_t)-1;
  }
...
size_t iconv(iconv_t cd0, char** restrict in, size_t* restrict inb, char** restrict out,
        ...
        case BIG5:
          ...
          if (c - 0xa1 >= 0xfa - 0xa1) {
            ...
            if (c / 256 == 0xdc) {
              ...
              x += iconv((iconv_t)(uintptr_t)to, &iconv_data, &(size_t){4}, out, outb);
              continue;
            }
            if (!c)
              goto ilseq;
