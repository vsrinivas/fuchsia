...
void arch_thread_initialize(Thread* t, vaddr_t entry_point) {
  ...
  vaddr_t buf = ROUNDUP(((vaddr_t)t->arch_.extended_register_buffer), 64);
  __UNUSED size_t overhead = buf - (vaddr_t)t->arch_.extended_register_buffer;
  DEBUG_ASSERT(sizeof(t->arch_.extended_register_buffer) - overhead >=
               x86_extended_register_size());
  t->arch_.extended_register_state = (vaddr_t*)buf;
  x86_extended_register_init_state(t->arch_.extended_register_state);

  // set the stack pointer
  t->arch_.sp = (vaddr_t)frame;
#if __has_feature(safe_stack)
  t->arch_.unsafe_sp = ROUNDDOWN(t->stack_.unsafe_base + t->stack_.size, 16);
#endif
  ...
  t->arch_.track_debug_state = false;
  for (size_t i = 0; i < 4; i++) {
    t->arch_.debug_state.dr[i] = 0;
  }
  t->arch_.debug_state.dr6 = X86_DR6_MASK;
  t->arch_.debug_state.dr7 = X86_DR7_MASK;
}
...
void* arch_thread_get_blocked_fp(Thread* t) {
  ...
  struct x86_64_context_switch_frame* frame = (struct x86_64_context_switch_frame*)t->arch_.sp;
...
__NO_SAFESTACK static void x86_context_switch_spec_mitigations(Thread* oldthread,
  ...
  if (x86_cpu_should_ras_fill_on_ctxt_switch() &&
      (oldthread->aspace_ || x86_cpu_vulnerable_to_rsb_underflow())) {
    x86_ras_fill();
  }
  ...
  if (x86_cpu_should_ibpb_on_ctxt_switch() &&
      (((oldthread->aspace_ && newthread->aspace_) && (oldthread->aspace_ != newthread->aspace_)) ||
       ((!oldthread->aspace_ && newthread->aspace_) &&
        (percpu->last_user_aspace != newthread->aspace_)))) {
    MsrAccess msr;
    x86_cpu_ibpb(&msr);
  }
  if (oldthread->aspace_ && !newthread->aspace_) {
    percpu->last_user_aspace = oldthread->aspace_;
  }
}
...
__attribute__((target("fsgsbase"))) static void x86_segment_selector_save_state(Thread* thread) {
  ...
  if (likely(g_x86_feature_fsgsbase)) {
    thread->arch_.fs_base = _readfsbase_u64();
    // Remember, the user and kernel gs_base values have been swapped -- the user value is currently
    // in KERNEL_GS_BASE.
    __asm__ __volatile__("swapgs\n");
    thread->arch_.gs_base = _readgsbase_u64();
    __asm__ __volatile__("swapgs\n");
  } else {
    thread->arch_.fs_base = read_msr(X86_MSR_IA32_FS_BASE);
    thread->arch_.gs_base = read_msr(X86_MSR_IA32_KERNEL_GS_BASE);
  }
}
...
__attribute__((target("fsgsbase"))) static void x86_segment_selector_restore_state(
  ...
  if (likely(g_x86_feature_fsgsbase)) {
    _writefsbase_u64(thread->arch_.fs_base);
    ...
    __asm__ __volatile__("swapgs\n");
    _writegsbase_u64(thread->arch_.gs_base);
    __asm__ __volatile__("swapgs\n");
  } else {
    write_msr(X86_MSR_IA32_FS_BASE, thread->arch_.fs_base);
    write_msr(X86_MSR_IA32_KERNEL_GS_BASE, thread->arch_.gs_base);
  }
}
...
__NO_SAFESTACK __attribute__((target("fsgsbase"))) static void x86_segment_selector_context_switch(
  ...
  if (likely(g_x86_feature_fsgsbase)) {
    oldthread->arch_.fs_base = _readfsbase_u64();
  } else {
    oldthread->arch_.fs_base = read_msr(X86_MSR_IA32_FS_BASE);
  }
  ...
  if (likely(g_x86_feature_fsgsbase)) {
    ...
    write_msr(X86_MSR_IA32_KERNEL_GS_BASE, newthread->arch_.gs_base);
  }
}
...
static void x86_debug_context_switch(Thread* old_thread, Thread* new_thread) {
  ...
  if (unlikely(new_thread->arch_.track_debug_state)) {
    ...
    x86_write_hw_debug_regs(&new_thread->arch_.debug_state);
    return;
  }

  // If the old thread had debug state running and the new one doesn't use it, disable the
  // debug capabilities.
  if (unlikely(old_thread->arch_.track_debug_state)) {
...
static void x86_debug_restore_state(Thread* thread) {
  ...
  if (unlikely(thread->arch_.track_debug_state)) {
    x86_write_hw_debug_regs(&thread->arch_.debug_state);
  } else {
...
__NO_SAFESTACK __attribute__((target("fsgsbase"))) void arch_context_switch(Thread* oldthread,
  ...
  if (likely(!oldthread->user_state_saved_)) {
    ...
    x86_extended_register_restore_state(newthread->arch_.extended_register_state);
    x86_debug_restore_state(newthread);
    x86_segment_selector_restore_state(newthread);
  }

#if __has_feature(safe_stack)
  oldthread->arch_.unsafe_sp = x86_read_gs_offset64(ZX_TLS_UNSAFE_SP_OFFSET);
  x86_write_gs_offset64(ZX_TLS_UNSAFE_SP_OFFSET, newthread->arch_.unsafe_sp);
#endif

  x86_context_switch_spec_mitigations(oldthread, newthread);

  x86_64_context_switch(&oldthread->arch_.sp, newthread->arch_.sp);
}
...
void arch_set_suspended_general_regs(struct Thread* thread, GeneralRegsSource source,
  ...
  thread->arch_.general_regs_source = source;
  thread->arch_.suspended_general_regs.gregs = gregs;
}

void arch_reset_suspended_general_regs(struct Thread* thread) {
  thread->arch_.general_regs_source = GeneralRegsSource::None;
  thread->arch_.suspended_general_regs.gregs = nullptr;
}
