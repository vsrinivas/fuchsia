...
void arch_thread_initialize(thread_t* t, vaddr_t entry_point) {
  ...
  DEBUG_ASSERT(sizeof(t->arch.extended_register_buffer) - overhead >= x86_extended_register_size());
...
__NO_SAFESTACK
__attribute__((target("fsgsbase"))) static void x86_segment_selector_save_state(thread_t* thread) {
...
static void x86_debug_context_switch(thread_t* old_thread, thread_t* new_thread) {
...
static void x86_debug_restore_state(thread_t* thread) {
