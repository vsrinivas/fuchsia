...
class Riscv64ArchVmAspace final : public ArchVmAspaceInterface {
 public:
  Riscv64ArchVmAspace(vaddr_t base, size_t size, Riscv64AspaceType type, page_alloc_fn_t test_paf = nullptr);
  Riscv64ArchVmAspace(vaddr_t base, size_t size, uint mmu_flags, page_alloc_fn_t test_paf = nullptr);
...
private:
  ...
  ssize_t MapPageTable(vaddr_t vaddr_in, vaddr_t vaddr_rel, paddr_t paddr_in, size_t size_in,
                       pte_t attrs, uint level, volatile pte_t* page_table, ConsistencyManager& cm) TA_REQ(lock_);
  ...
  void HarvestAccessedPageTable(vaddr_t vaddr_in, vaddr_t vaddr_rel_in, size_t size_in,
                                ...
                                bool* unmapped_out) TA_REQ(lock_);
  ...
  ssize_t MapPages(vaddr_t vaddr, paddr_t paddr, size_t size, pte_t attrs, ConsistencyManager& cm) TA_REQ(lock_);
