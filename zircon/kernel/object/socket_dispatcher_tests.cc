...
bool TestCreateWriteReadClose() {
  ...
  auto* const write_buffer = reinterpret_cast<unsigned char*>(write->out());
  for (uint i = 0; i < kSize; i++) {
    write_buffer[i] = static_cast<unsigned char>(i);
  }
  size_t written = 0;
  auto write_status =
      dispatcher0.dispatcher()->Write(make_user_in_ptr(write->in()), kSize, &written);
  ...
  for (uint i = 0; i < kSize; i++) {
    ...
    auto read_status = dispatcher1.dispatcher()->Read(
        SocketDispatcher::ReadType::kConsume, make_user_out_ptr(read->out()), 1, &bytes_read);
    ...
    read_buffer[i] = *reinterpret_cast<const unsigned char*>(read->in());
  }
  ...
  EXPECT_EQ(dispatcher0.dispatcher()->Write(make_user_in_ptr(write->in()), 1, &written), ZX_OK);
  EXPECT_EQ(written, 1u);
  EXPECT_EQ(dispatcher0.dispatcher()->Shutdown(ZX_SOCKET_SHUTDOWN_WRITE), ZX_OK);
  EXPECT_EQ(dispatcher0.dispatcher()->Write(make_user_in_ptr(write->in()), 1, &written),
