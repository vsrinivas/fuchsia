...
static zx_status_t object_unbind_exception_port(zx_handle_t obj_handle, bool debugger) {
  ...
  if (process) {
    return process->ResetExceptionPort(debugger) ? ZX_OK : ZX_ERR_BAD_STATE;  // No port was bound.
  }
...
static zx_status_t task_bind_exception_port(zx_handle_t obj_handle, zx_handle_t eport_handle,
                                            uint64_t key, bool debugger) {
  // TODO(ZX-968): check rights once appropriate right is determined
  auto up = ProcessDispatcher::GetCurrent();
  ...
  if (thread) {
    if (debugger)
      return ZX_ERR_INVALID_ARGS;
    status = ExceptionPort::Create(ExceptionPort::Type::THREAD, ktl::move(port), key, &eport);
    if (status != ZX_OK)
      return status;
...
zx_status_t sys_task_bind_exception_port(zx_handle_t handle, zx_handle_t port, uint64_t key,
                                         uint32_t options) {
  LTRACE_ENTRY;
