...
zx::vmo LoaderService::TryLoadObject(const char* name, size_t len,
                                     bool use_prefix) {
    ...
    return zx::vmo(
        bootfs_open(log_->get(), "shared library", fs_, root_, file));
}
...
bool LoaderService::HandleRequest(const zx::channel& channel) {
    ...
    uint32_t size;
    uint32_t hcount;
    zx_status_t status = channel.read(
        0, &req, reqhandle.reset_and_get_address(), sizeof(req), 1,
        &size, &hcount);
...
error_reply:
    ...
    if (vmo) {
        ...
        status = channel.write(
            0, &rsp, static_cast<uint32_t>(ldmsg_rsp_get_size(&rsp)),
            handles, 1);
    } else {
        status = channel.write(0, &rsp, static_cast<uint32_t>(ldmsg_rsp_get_size(&rsp)),
                               nullptr, 0);
    }
    check(log_->get(), status,
...
void LoaderService::Serve(zx::channel channel) {
    ...
    do {
        ...
        zx_status_t status = channel.wait_one(
