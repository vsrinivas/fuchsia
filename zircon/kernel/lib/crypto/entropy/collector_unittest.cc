...
class MockCollector : public Collector {
 public:
  MockCollector(size_t entropy_per_1000_bytes) : Collector("mock", entropy_per_1000_bytes) {}
...
bool entropy_estimate_test() {
  ...
  EXPECT_EQ(0u, ec_1.BytesNeeded(0), "bad entropy estimate (entropy per 1000 bytes = 1, bits = 0");
  ...
  EXPECT_EQ(0u, ec_2.BytesNeeded(0), "bad entropy estimate (entropy per 1000 bytes = 2, bits = 0");
