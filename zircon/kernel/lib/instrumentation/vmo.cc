...
zx_status_t InstrumentationData::Create() {
    ...
    return VmObjectPaged::CreateFromWiredPages(k.start, k.vmo_end - k.start,
                                               false, &vmo_);
}
...
zx_status_t InstrumentationData::GetVmo(Handle** handle) {
    ...
    zx_status_t status =
        VmObjectDispatcher::Create(vmo_, &dispatcher, &rights);
    if (status == ZX_OK) {
        *handle = Handle::Make(
                      ktl::move(dispatcher), rights & ~ZX_RIGHT_WRITE)
                      .release();
    }
    return status;
}
