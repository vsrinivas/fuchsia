...
static int oom_loop(void* arg) {
  ...
  while (true) {
    ...
    uint64_t sleep_duration_ns = 0;
    {
      ...
      if (lowmem) {
        shortfall_bytes = oom_simulate_lowmem ? 512 * 1024 : oom_redline_bytes - free_bytes;
      }
      oom_simulate_lowmem = false;

      printing = lowmem || (oom_printing && free_bytes != last_free_bytes);
    ...
    if (printing) {
      ...
      printf("OOM: %s free (%c%s) / %s total\n", free_buf, delta_sign, delta_buf, total_buf);
    }
