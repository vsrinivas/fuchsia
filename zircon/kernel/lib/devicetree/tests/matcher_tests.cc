...
struct SingleNodeMatcher {
  devicetree::MatcherResult operator()(const devicetree::NodePath& path,
...
TEST_F(MatchTest, SingleMatcherNoAlias) {
  size_t seen = 0;
  SingleNodeMatcher matcher{.path_to_match = "/A/C/D", .cb = [&](auto name, auto props) {
                              seen++;
                              EXPECT_EQ(name, "D");
                            }};
...
TEST_F(MatchTest, MultipleMatchersNoAlias) {
  ...
  SingleNodeMatcher matcher_2{.path_to_match = "/E/F/G/H", .cb = [&](auto name, auto props) {
                                h_seen++;
                                EXPECT_EQ(name, "H");
                              }};
