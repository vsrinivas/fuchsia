...
//
// An owned wait queue maintains an unmanaged pointer to a thread_t in order to
...
// destructor for OwnedWaitQueue and thread_t.  This enforcement is considered
...
class OwnedWaitQueue : public WaitQueue, public fbl::DoublyLinkedListable<OwnedWaitQueue*> {
 public:
  ...
  class Hook {
   public:
    ...
    using Callback = Action (*)(thread_t* thrd, void* ctx);

    Hook() : cbk_(nullptr) {}
    Hook(Callback cbk, void* ctx) : cbk_(cbk), ctx_(ctx) {}

    Action operator()(thread_t* thrd) const TA_REQ(thread_lock) {
  ...
  static void DisownAllQueues(thread_t* t) TA_REQ(thread_lock);

  // const accessor for the owner member.
  thread_t* owner() const TA_REQ(thread_lock) { return owner_; }
  ...
  bool AssignOwner(thread_t* new_owner) TA_REQ(thread_lock) __WARN_UNUSED_RESULT {
  ...
  // permitted to become the new owner of the wait_queue.  Any existing owner
  // will be replaced with no owner in this situation.
  zx_status_t BlockAndAssignOwner(const Deadline& deadline, thread_t* new_owner,
  ...
  bool WakeAndRequeue(uint32_t wake_count, OwnedWaitQueue* requeue_target, uint32_t requeue_count,
                      thread_t* requeue_owner, Hook on_thread_wake_hook = {},
 ...
 private:
  ...
  // consideration.
  //
  // Callable should be a lambda which takes a thread_t* for consideration and
  ...
  void ForeachThread(const Callable& visit_thread) TA_REQ(thread_lock) {
    auto consider_queue = [&visit_thread](thread_t* queue_head) TA_REQ(thread_lock) -> bool {
      ...
      while (true) {
        next = list_peek_head_type(&queue_head->queue_node, thread_t, queue_node);
        ...
        if (!list_in_list(&next->wait_queue_heads_node)) {
          break;
        }
      ...
      do {
        thread_t* t = next;
        next = list_next_type(&queue_head->queue_node, &t->queue_node, thread_t, queue_node);
    ...
    thread_t* last_queue_head = nullptr;
    thread_t* queue_head;

    list_for_every_entry (&this->heads, queue_head, thread_t, wait_queue_heads_node) {
  ...
  //
  // Returns true if a local reschedule is required, or false otherwise.
  static bool QueuePressureChanged(thread_t* t, int old_prio, int new_prio,
  ...
  //
  // Returns true if a local reschedule is required, or false otherwise.
  bool UpdateBookkeeping(thread_t* new_owner, int old_prio,
                         cpu_mask_t* out_accum_cpu_mask = nullptr)
      TA_REQ(thread_lock) __WARN_UNUSED_RESULT;
  ...
  //
  // Returns true if a local reschedule is required, or false otherwise.
  bool WakeThreadsInternal(uint32_t wake_count, thread_t** out_new_owner, Hook on_thread_wake_hook)
      TA_REQ(thread_lock) __WARN_UNUSED_RESULT;

  thread_t* owner_ TA_GUARDED(thread_lock) = nullptr;
};
