...
//
class OwnedWaitQueue : public WaitQueue,
                       public fbl::DoublyLinkedListable<OwnedWaitQueue*> {
public:
    ...
    void AssertNotOwned() const TA_NO_THREAD_SAFETY_ANALYSIS {
    ...
    // permitted to become the new owner of the wait_queue.  Any existing owner
    // will be replaced with no owner in this situation.
    zx_status_t BlockAndAssignOwner(const Deadline& deadline,
                                    thread_t* new_owner,
    ...
    //
    // Returns true if a local reschedule is required, or false otherwise.
    bool WakeAndRequeue(uint32_t wake_count,
...
private:
    ...
    void ForeachThread(const Callable& visit_thread) TA_REQ(thread_lock) {
        auto consider_queue = [&visit_thread](thread_t* queue_head) TA_REQ(thread_lock) -> bool {
            ...
            do {
                thread_t* t = next;
                next = list_next_type(&queue_head->queue_node,
                                      &t->queue_node,
                                      thread_t, queue_node);
    ...
    //
    // Returns true if a local reschedule is required, or false otherwise.
    static bool QueuePressureChanged(thread_t* t,
                                     int old_prio,
                                     int new_prio,
    ...
    //
    // Returns true if a local reschedule is required, or false otherwise.
    bool UpdateBookkeeping(thread_t* new_owner,
                           int old_prio,
    ...
    //
    // Returns true if a local reschedule is required, or false otherwise.
    bool WakeThreadsInternal(uint32_t wake_count,
