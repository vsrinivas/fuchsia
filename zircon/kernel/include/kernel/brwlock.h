...
template <BrwLockEnablePi PI> struct BrwLockWaitQueueType;

template <> struct BrwLockWaitQueueType<BrwLockEnablePi::Yes> { using Type = OwnedWaitQueue; };

template <> struct BrwLockWaitQueueType<BrwLockEnablePi::No> { using Type = WaitQueue; };

template <BrwLockEnablePi PI> struct BrwLockState;

template <> struct alignas(16) BrwLockState<BrwLockEnablePi::Yes> {
...
template <> struct BrwLockState<BrwLockEnablePi::No> {
...
template <BrwLockEnablePi PI> class TA_CAP("mutex") BrwLock {
public:
    BrwLock() {}
    ~BrwLock();
    ...
    void CommonWriteAcquire(uint64_t expected_state_bits, F contended)
        ...
        if constexpr (PI == BrwLockEnablePi::Yes) {
            ...
            unsigned __int128 desired =
                ...
                static_cast<unsigned __int128>(reinterpret_cast<uintptr_t>(ct)) << 64;
