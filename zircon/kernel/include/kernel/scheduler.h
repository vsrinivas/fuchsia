...
// Thread::scheduler_state, which is not present when the new scheduler is
// disabled.
...
class Scheduler {
 ...
 private:
  ...
  // Updates the thread's weight and updates state-dependent bookkeeping.
  static void UpdateWeightCommon(Thread*, int original_priority, SchedWeight weight,
                                 cpu_mask_t* cpus_to_reschedule_mask, PropagatePI propagate)
      TA_REQ(thread_lock);
  ...
  Thread* DequeueThread() TA_REQ(thread_lock);
  ...
  void NextThreadTimeslice(Thread* thread) TA_REQ(thread_lock);

  // Updates the thread virtual timeline based on the current runqueue state.
  void UpdateThreadTimeline(Thread* thread, Placement placement) TA_REQ(thread_lock);
  ...
  using RunQueue = fbl::WAVLTree<SchedTime, Thread*, TaskTraits, TaskTraits>;
