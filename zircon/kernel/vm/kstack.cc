...
static zx_status_t allocate_vmar(bool unsafe, fbl::RefPtr<VmMapping>* out_kstack_mapping,
  ...
  status = vmar->CreateSubVmar(
      ...
      unsafe ? "unsafe_kstack_vmar" : "kstack_vmar", &kstack_vmar);
  if (status != ZX_OK) {
    return status;
  }
  ...
  status = kstack_vmar->CreateVmMapping(padding_size, DEFAULT_STACK_SIZE, 0, VMAR_FLAG_SPECIFIC,
                                        ktl::move(stack_vmo), 0,
                                        ARCH_MMU_FLAG_PERM_READ | ARCH_MMU_FLAG_PERM_WRITE,
                                        unsafe ? "unsafe_kstack" : "kstack", &kstack_mapping);
  if (status != ZX_OK) {
    return status;
  }
