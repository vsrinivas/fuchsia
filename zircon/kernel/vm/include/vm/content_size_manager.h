...
class ContentSizeManager {
 ...
 private:
  ...
  // Commits and then dequeues an `Operation`. This may block until the operation is at the front
  // of a queue.
  void CommitAndDequeueOperation(Operation* op) TA_EXCL(lock_) {
    Guard<Mutex> guard{&lock_};
    return CommitAndDequeueOperationLocked(op);
  }
