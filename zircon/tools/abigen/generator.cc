...
bool Generator::header(ofstream& os) {
  ...
  os << "// Copyright " << ltime->tm_year + 1900 << " " << kAuthors << ". All rights reserved.\n";
  os << "// This is a GENERATED file, see //zircon/tools/abigen.\n";
...
bool VDsoAsmGenerator::syscall(ofstream& os, const Syscall& sc) {
  if (!sc.is_vdso()) {
    ...
    os << syscall_macro_ << " " << name_prefix_ << sc.name << " " << sc.index << " "
       << sc.num_kernel_args() << " " << (is_public ? 1 : 0) << "\n";
  }
  return os.good();
}
...
bool TraceInfoGenerator::syscall(ofstream& os, const Syscall& sc) {
  ...
  os << "{" << sc.index << ", " << sc.num_kernel_args() << ", " << '"' << sc.name << "\"},\n";
...
void write_syscall_invocation(ofstream& os, const Syscall& sc, const string& return_var,
  ...
  // Writes all arguments.
  sc.for_each_kernel_arg([&](const TypeSpec& arg) { os << arg.name << ", "; });
