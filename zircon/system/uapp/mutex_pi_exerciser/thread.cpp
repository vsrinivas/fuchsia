...
zx_status_t Thread::EnsureProfile(uint32_t prio_level) {
    ...
    res = fuchsia_scheduler_ProfileProviderGetProfile(scheduler_service_.get(),
                                                      prio_level,
                                                      name,
                                                      name_len,
                                                      &get_profile_res,
...
zx_status_t Thread::Start(Thunk thunk) {
    ...
    if (state == State::INIT) {
        ...
        auto trampoline = [](void* ctx) -> int {
            return reinterpret_cast<Thread*>(ctx)->EntryPoint();
        };
...
zx_status_t Thread::WaitForState(State target_state) {
    ...
    if (res != ZX_OK) {
        fprintf(stderr, "timed out waiting for \"%s\" to achieve state (%u)\n",
                name_, static_cast<uint32_t>(target_state));
        return res;
    }
