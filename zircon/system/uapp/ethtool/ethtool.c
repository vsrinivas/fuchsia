...
int main(int argc, const char** argv) {
  ...
  if (options.setting_promisc) {
    status =
        fuchsia_hardware_ethernet_DeviceSetPromiscuousMode(svc, options.promisc_on, &call_status);
    if (status != ZX_OK || call_status != ZX_OK) {
  ...
  if (options.filter_macs) {
    ...
    for (int i = 0; i < options.n_filter_macs; i++) {
      ...
      printf("Sending addr %d %d %d %d %d %d\n", addr.octets[0], addr.octets[1], addr.octets[2],
             addr.octets[3], addr.octets[4], addr.octets[5]);
      status = fuchsia_hardware_ethernet_DeviceConfigMulticastAddMac(svc, &addr, &call_status);
      if (status != ZX_OK || call_status != ZX_OK) {
