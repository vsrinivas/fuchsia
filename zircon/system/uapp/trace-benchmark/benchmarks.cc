...
#define RUN_TEST(test_name, pretty_test_name, macro_name, enabled_prefix, category, expression)                 \
    do {                                                                                                        \
        const char* full_test_name =                                                                            \
            #macro_name " macro with " pretty_test_name ": " #category;                                         \
        struct Test {                                                                                           \
            static void Run()                                                                                   \
                __attribute__((__section__(MAKE_TEST_SYMBOL_STRING(test_, macro_name, test_name, category)))) { \
                expression;                                                                                     \
            }                                                                                                   \
        };                                                                                                      \
        ...
        size_t test_size =                                                                                      \
            (MAKE_TEST_SYMBOL_NAME(__stop_test_, macro_name, test_name, category) -                             \
...
#define RUN_NILADIC_DURATION_TEST(test_name, pretty_test_name, DURATION_MACRO, enabled_prefix, category) \
...
#define DURATION_TEST(DURATION_MACRO, enabled_prefix, category)          \
    RUN_NILADIC_DURATION_TEST(zero_args, "0 arguments",                  \
                              DURATION_MACRO, enabled_prefix, category); \
                                                                         \
    RUN_DURATION_TEST(one_int32_arg, "1 int32 argument",                 \
                      DURATION_MACRO, enabled_prefix, category,          \
                      "k1", 1);                                          \
                                                                         \
    RUN_DURATION_TEST(one_double_arg, "1 double argument",               \
                      DURATION_MACRO, enabled_prefix, category,          \
                      "k1", 1.);                                         \
                                                                         \
    RUN_DURATION_TEST(one_string_arg, "1 string argument",               \
                      DURATION_MACRO, enabled_prefix, category,          \
                      "k1", "string1");                                  \
                                                                         \
    RUN_DURATION_TEST(four_int32_args, "4 int32 arguments",              \
                      DURATION_MACRO, enabled_prefix, category,          \
                      "k1", 1, "k2", 2, "k3", 3, "k4", 4);               \
                                                                         \
    RUN_DURATION_TEST(four_double_args, "4 double arguments",            \
                      DURATION_MACRO, enabled_prefix, category,          \
                      "k1", 1., "k2", 2., "k3", 3., "k4", 4.);           \
                                                                         \
    RUN_DURATION_TEST(four_string_args, "4 string arguments",            \
                      DURATION_MACRO, enabled_prefix, category,          \
                      "k1", "string1", "k2", "string2",                  \
                      "k3", "string3", "k4", "string4");                 \
                                                                         \
    RUN_DURATION_TEST(eight_int32_args, "8 int32 arguments",             \
                      DURATION_MACRO, enabled_prefix, category,          \
                      "k1", 1, "k2", 2, "k3", 3, "k4", 4,                \
                      "k5", 5, "k6", 6, "k7", 7, "k8", 8);               \
                                                                         \
    RUN_DURATION_TEST(eight_double_args, "8 double arguments",           \
                      DURATION_MACRO, enabled_prefix, category,          \
                      "k1", 1., "k2", 2., "k3", 3., "k4", 4.,            \
                      "k5", 4., "k6", 5., "k7", 7., "k8", 8.);           \
                                                                         \
    RUN_DURATION_TEST(eight_string_args, "8 string arguments",           \
                      DURATION_MACRO, enabled_prefix, category,          \
                      "k1", "string1", "k2", "string2",                  \
                      "k3", "string3", "k4", "string4",                  \
