...
public:
    ...
    void TokenizeRealKeywordsTest() {
        ...
        std::vector<TokenPtr> tokens = tokenize(input);
        std::vector<TokenPtr> expected_tokens = {L_PARENS, ETHER, SRC, R_PARENS, AND,
                                                 L_PARENS, IP6, OR, IP, R_PARENS, AND, L_PARENS,
    ...
    void TokenizeKeywordsLiteralsStringTest() {
        ...
        std::vector<TokenPtr> expected_tokens = {L_PARENS, ETHER, SRC, xxx, R_PARENS, AND,
                                                 L_PARENS, IP6, OR, IP, R_PARENS, AND, L_PARENS,
                                                 TCP, DST, PORT, twentythree, R_PARENS};
        CheckTokenVectorString(expected_tokens, tokens);
    }
    ...
    void VisitorTest() {
        ...
        auto token_visitor = FunctionalTokenVisitor(
            [&literal_str](TokenPtr t) { literal_str = t->get_term(); },
            [&token_fail](PortTokenPtr /*t*/) { token_fail = true; });

        auto port_token_visitor = FunctionalTokenVisitor(
            [&port_token_fail](TokenPtr /*t*/) { port_token_fail = true; },
    ...
    static inline std::string CheckPort(const TokenPtr& token,
                                        bool* fail, uint16_t* beg_port, uint16_t* end_port) {
        *fail = false;
        *beg_port = 0;
        *end_port = 0;
        FunctionalTokenVisitor visitor = PortTokenVisitor(fail, beg_port, end_port);
    ...
    void PortTest() {
        std::vector<TokenPtr> tokens = {named_port("MYPORT", 1, 1), port("MYPORT"), port("42"),
                                        port("25-35"), port("YOURPORT"), port("42,51"), port("-42"),
                                        port("1--42"), port("100-50"), port("55-66000"),
                                        port("1-ftpxfer"), port("ftpxfer-ftpctl")};
        std::vector<std::string> terms = {"MYPORT", "MYPORT", "42", "25-35", "YOURPORT", "42,51",
                                          "-42", "1--42", "100-50", "55-66000", "1-ftpxfer",
                                          "ftpxfer-ftpctl"};
        PortRanges ranges = {std::optional(PortRange(1, 1)), std::optional(PortRange(1, 1)),
                             std::optional(PortRange(42, 42)), std::optional(PortRange(25, 35)),
    ...
    void PortTokenizationTest() {
        ...
        PortRanges ranges = {std::optional(PortRange(6, 6)), std::optional(PortRange(15, 15)),
                             std::optional(PortRange(10, 20)), std::optional(PortRange(1, 1)),
                             std::nullopt, std::nullopt, std::nullopt, std::nullopt,
                             std::optional(PortRange(37, 37)), std::optional(PortRange(2, 2))};
    ...
    void RealNamedPortsTokenizationTest() {
        ...
        std::vector<TokenPtr> tokens = mult_ports(',', input);
        std::vector<std::string> terms = {"75", "21", "10-20", "ssh", "http", "dbglog", "ftpxfer",
                                          "ftpctl", "badname", "", "SSH"};
        PortRanges ranges = {std::optional(PortRange(75, 75)), std::optional(PortRange(21, 21)),
                             std::optional(PortRange(10, 20)), std::optional(PortRange(22, 22)),
                             std::optional(PortRange(80, 80)),
                             std::optional(PortRange(DEBUGLOG_PORT, DEBUGLOG_PORT)),
                             std::optional(PortRange(20, 20)), std::optional(PortRange(21, 21)),
                             std::nullopt, std::nullopt, std::nullopt};
