...
class MockFilterBuilder : public FilterBuilder<std::monostate> {
 public:
  ...
  std::monostate ethertype(uint16_t type) override { return ethertype_mock.Call(type); }

  mock_function::MockFunction<std::monostate, std::array<uint8_t, ETH_ALEN>, TokenPtr> mac_mock;
  ...
  std::monostate ip_version(uint8_t version) override { return ip_version_mock.Call(version); }
  ...
  mock_function::MockFunction<std::monostate, std::array<uint8_t, IP6_ADDR_LEN>, TokenPtr>
      ipv6_address_mock;
  std::monostate ipv6_address(std::array<uint8_t, IP6_ADDR_LEN> address,
                              TokenPtr addr_type) override {
    return ipv6_address_mock.Call(address, addr_type);
  }

  mock_function::MockFunction<std::monostate, std::vector<std::pair<uint16_t, uint16_t>>, TokenPtr>
      ports_mock;
  std::monostate ports(std::vector<std::pair<uint16_t, uint16_t>> ranges,
                       TokenPtr port_type) override {
    return ports_mock.Call(ranges, port_type);
  }

  mock_function::MockFunction<std::monostate, std::monostate> negation_mock;
  std::monostate negation(std::monostate filter) override { return negation_mock.Call(filter); }
 ...
 public:
  void highlight_error_test() {
    ...
    expect_string2 << ANSI_HIGHLIGHT_ERROR << tkz_.AND->get_term() << ANSI_RESET << " "
                   << tkz_.DNS->get_term() << " " << tkz_.DHCP->get_term();
    ...
    expect_string3 << tkz_.AND->get_term() << " " << ANSI_HIGHLIGHT_ERROR << tkz_.DNS->get_term()
                   << ANSI_RESET << " " << tkz_.DHCP->get_term();
    ...
    ++env;
    env.error_loc = env.cur();
    expect_string4 << tkz_.AND->get_term() << " " << tkz_.DNS->get_term() << " "
                   << ANSI_HIGHLIGHT_ERROR << tkz_.DHCP->get_term() << ANSI_RESET;
    EXPECT_STR_EQ(expect_string4.str().c_str(), highlight_error("spec", &env).c_str());
  }

  explicit TestParser(const Tokenizer& tokenizer) : Parser(tokenizer) {}
};

TEST(NetdumpParserTests, HighlightErrorTest) { TestParser(Tokenizer{}).highlight_error_test(); }
