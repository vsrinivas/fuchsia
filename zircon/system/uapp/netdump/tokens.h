...
protected:
    ...
    explicit TokenBase(std::string term, uint64_t tag)
        : term_(std::move(term)), tag_(tag) {}
...
protected:
    ...
    TokenPtr named_port(const std::string& name, const std::string& synonym,
                        uint16_t begin, uint16_t end, uint64_t tag = 0);
};
