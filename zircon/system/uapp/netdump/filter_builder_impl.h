...
public:
    explicit FilterTreeBuilder(const Tokenizer& tokenizer)
        : FilterBuilder<FilterPtr>(tokenizer) {}

    inline FilterPtr frame_length(uint16_t length, TokenPtr comparator) override {
        return FilterPtr(new FrameLengthFilter(htons(length),
                                               comparator->get_tag<LengthComparator>()));
    }
    ...
    inline FilterPtr ip_version(uint8_t version) override {
        return FilterPtr(new IpFilter(version));
    }

    inline FilterPtr ip_pkt_length(uint8_t version, uint16_t length, TokenPtr comparator) override {
        return FilterPtr(new IpFilter(version, htons(length),
                                      comparator->get_tag<LengthComparator>()));
    }
    ...
    inline FilterPtr ipv6_address(std::array<uint8_t, IP6_ADDR_LEN> address, TokenPtr addr_type)
        override {
        std::reverse(address.begin(), address.end()); // Constructor expects network byte order.
        return FilterPtr(new IpFilter(address, addr_type->get_tag<AddressFieldType>()));
    }

    FilterPtr ports(std::vector<PortRange> ranges, TokenPtr port_type)
        override {
