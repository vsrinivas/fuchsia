...
TokenPtr Tokenizer::keyword(const std::string& term, uint64_t tag /*= 0*/) {
    ...
    if (iter != dictionary_.end()) {
        ...
        ZX_DEBUG_ASSERT_MSG(tag == old_tag,
                            "Redefinition of tag value for '%s', old: %lu, new: %lu",
...
TokenPtr Tokenizer::port(const std::string& port_string) const {
    ...
    if (divider == std::string::npos) {
        ...
        ZX_DEBUG_ASSERT_MSG(port_string.length() == divider + end_port_str_len + 1,
                            "Unexpected port string lengths: %zu and %lu", divider,
                            end_port_str_len);
        if (!port_num(input, divider, &begin_port) ||
            !port_num(end_port_start, end_port_str_len, &end_port) ||
            end_port < begin_port) {
...
TokenPtr Tokenizer::named_port(const std::string& name, const std::string& synonym,
