...
namespace {
namespace virtualbustest = ::llcpp::fuchsia::hardware::usb::virtualbustest;
...
void USBVirtualBus::InitUsbVirtualBus(std::optional<virtualbustest::BusTest::SyncClient>* test) {
  ...
  ASSERT_NO_FATAL_FAILURES(SetupPeripheralDevice(std::move(device_desc), std::move(function_descs)));
