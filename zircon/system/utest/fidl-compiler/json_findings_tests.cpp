...
public:
    JsonFindingsTest(std::string filename, std::string source)
...
protocol TestProtocol {
  -> OnWard();
    ...
    test.AddFinding({.check_id = "check-2",
...
protocol TestProtocol {
  -> OnWard();
    ...
    test.AddFinding({.check_id = "check-3",
                     .message = "Finding message 3",
                     .violation_string = "library"});

    test.AddFinding({.check_id = "check-4",
                     .message = "Finding message 4",
                     .violation_string = "fidl.a"})
        ->SetSuggestion("Suggestion description");

    test.AddFinding({.check_id = "check-5",
