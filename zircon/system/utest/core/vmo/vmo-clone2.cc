...
TEST_F(VmoClone2TestCase, Offset) {
    ...
    zx::vmo clone;
    ASSERT_OK(vmo.create_child(ZX_VMO_CHILD_COPY_ON_WRITE,
...
TEST_F(VmoClone2TestCase, OffsetTest2) {
    ...
    zx::vmo clone1;
    ASSERT_OK(offset_clone.create_child(ZX_VMO_CHILD_COPY_ON_WRITE,
...
TEST_F(VmoClone2TestCase, OffsetProgressiveWrite) {
    ...
    zx::vmo clone2;
    ASSERT_OK(clone.create_child(ZX_VMO_CHILD_COPY_ON_WRITE,
...
TEST(VmoClone2TestCase, DisjointCloneProgressive) {
    ...
    ASSERT_OK(main_clone.create_child(ZX_VMO_CHILD_COPY_ON_WRITE,
...
TEST(VmoClone2TestCase, ManyCloneOffset) {
    ...
    ASSERT_OK(clone1.create_child(ZX_VMO_CHILD_COPY_ON_WRITE, ZX_PAGE_SIZE, ZX_PAGE_SIZE,
                                  &clone2));
...
TEST_F(VmoClone2TestCase, ManyCloneMappingOffset) {
    ...
    ASSERT_OK(vmos[0].create_child(ZX_VMO_CHILD_COPY_ON_WRITE, ZX_PAGE_SIZE, ZX_PAGE_SIZE,
                                   vmos + 2));
...
struct ProgressiveCloneDiscardTests : public KmemStatsFixture {
    static void ProgressiveCloneDiscardTest(bool close) {
        ...
        for (unsigned i = 1; i < kNumClones; i++) {
            ASSERT_OK(vmos[0].create_child(ZX_VMO_CHILD_COPY_ON_WRITE | ZX_VMO_CHILD_RESIZABLE,
...
TEST(VmoClone2TestCase, NoPager) {
    ...
    ASSERT_EQ(vmo.create_child(ZX_VMO_CHILD_COPY_ON_WRITE, 0, ZX_PAGE_SIZE, &clone),
                               ZX_ERR_NOT_SUPPORTED);
    ASSERT_EQ(uni_clone.create_child(ZX_VMO_CHILD_COPY_ON_WRITE,
