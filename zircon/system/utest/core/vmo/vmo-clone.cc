...
TEST(VmoCloneTestCase, Test1) {
  ...
  EXPECT_OK(zx_vmo_create_child(clone_vmo[1], ZX_VMO_CHILD_COPY_ON_WRITE, 0, size, &clone_vmo[2]),
            "vm_clone");
...
TEST(VmoCloneTestCase, Test2) {
  ...
  EXPECT_OK(zx_vmo_read(clone_vmo[0], &val, offset, sizeof(val)),
            "reading back original from clone");
...
TEST(VmoCloneTestCase, Test3) {
  ...
  // clone it
  clone_vmo[0] = ZX_HANDLE_INVALID;
  EXPECT_OK(zx_vmo_create_child(vmo, ZX_VMO_CHILD_COPY_ON_WRITE | ZX_VMO_CHILD_RESIZABLE, 0, size,
                                &clone_vmo[0]),
            "vm_clone");
  EXPECT_NE(ZX_HANDLE_INVALID, clone_vmo[0], "vm_clone_handle");
  ...
  EXPECT_EQ(ZX_ERR_NOT_SUPPORTED,
            zx_vmar_map(zx_vmar_root_self(),
                        ZX_VM_PERM_READ | ZX_VM_PERM_WRITE | ZX_VM_REQUIRE_NON_RESIZABLE, 0,
                        clone_vmo[0], 0, size, &clone_ptr),
            "map");

  // Regular resizable mapping works.
  EXPECT_EQ(ZX_OK,
            zx_vmar_map(zx_vmar_root_self(), ZX_VM_PERM_READ | ZX_VM_PERM_WRITE, 0, clone_vmo[0], 0,
                        size, &clone_ptr),
            "map");
...
TEST(VmoCloneTestCase, Test4) {
  ...
  EXPECT_EQ(ZX_ERR_INVALID_ARGS,
            zx_vmo_create_child(vmo, ZX_VMO_CHILD_COPY_ON_WRITE, 1, size, &clone_vmo[0]),
            "vm_clone");

  // create a clone that extends beyond the parent by one page
  clone_vmo[0] = ZX_HANDLE_INVALID;
  EXPECT_OK(zx_vmo_create_child(vmo, ZX_VMO_CHILD_COPY_ON_WRITE | ZX_VMO_CHILD_RESIZABLE, PAGE_SIZE,
                                size, &clone_vmo[0]),
            "vm_clone");
  ...
  size_t val = 99;
  EXPECT_OK(zx_vmo_write(vmo, &val, size, sizeof(val)), "writing to original after extending");
...
TEST(VmoCloneTestCase, ResizeCloneHazard) {
  ...
  uintptr_t ptr_rw;
  EXPECT_OK(zx_vmar_map(zx_vmar_root_self(), ZX_VM_PERM_READ | ZX_VM_PERM_WRITE, 0, clone_vmo, 0,
                        size, &ptr_rw),
            "map");
...
TEST(VmoCloneTestCase, ShrinkGrowParent) {
  ...
  for (auto &config : configs) {
    ...
    zx_handle_t clone_vmo;
    EXPECT_EQ(ZX_OK, zx_vmo_create_child(vmo, ZX_VMO_CHILD_COPY_ON_WRITE, config.clone_offset,
                                         config.clone_size, &clone_vmo));

    uintptr_t ptr_rw;
    EXPECT_OK(zx_vmar_map(zx_vmar_root_self(), ZX_VM_PERM_READ | ZX_VM_PERM_WRITE, 0, clone_vmo, 0,
                          config.clone_size, &ptr_rw));
