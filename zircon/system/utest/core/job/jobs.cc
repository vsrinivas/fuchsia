...
static bool basic_test() {
  ...
  ASSERT_EQ(zx_object_get_info(job_child, ZX_INFO_JOB, &job_info, sizeof(job_info), NULL, NULL),
            ZX_OK);
...
static bool create_test() {
  ...
  ASSERT_EQ(
      zx_process_create(job_parent, process_name, sizeof(process_name), 0u, &process1, &vmar1),
      ZX_OK, "");

  zx_handle_t process2, vmar2;
  ASSERT_EQ(zx_process_create(job_child, process_name, sizeof(process_name), 0u, &process2, &vmar2),
            ZX_OK, "");
...
static bool policy_timer_slack_invalid_options_test() {
  ...
  ASSERT_EQ(job_child.set_policy(options, ZX_JOB_POL_TIMER_SLACK, &policy, 1), ZX_ERR_INVALID_ARGS,
            "");

  // Valid.
  options = ZX_JOB_POL_RELATIVE;
  ASSERT_EQ(job_child.set_policy(options, ZX_JOB_POL_TIMER_SLACK, &policy, 1), ZX_OK, "");
...
static bool kill_test() {
  ...
  ASSERT_EQ(zx_object_wait_one(process, ZX_TASK_TERMINATED, ZX_TIME_INFINITE, &signals), ZX_OK, "");
  ASSERT_EQ(signals, ZX_TASK_TERMINATED, "");

  ASSERT_EQ(zx_object_wait_one(job_child, ZX_TASK_TERMINATED, ZX_TIME_INFINITE, &signals), ZX_OK,
            "");
  ...
  ASSERT_EQ(
      zx_object_get_info(job_grandchild, ZX_INFO_JOB, &job_info, sizeof(job_info), NULL, NULL),
      ZX_OK);
  EXPECT_TRUE(job_info.exited);
  EXPECT_EQ(job_info.return_code, ZX_TASK_RETCODE_SYSCALL_KILL, "");

  zx_info_process_t proc_info;
  ASSERT_EQ(zx_object_get_info(process, ZX_INFO_PROCESS, &proc_info, sizeof(proc_info), NULL, NULL),
            ZX_OK);
...
static bool kill_job_removes_from_tree() {
  ...
  ASSERT_EQ(zx_task_kill(job_child), ZX_OK, "");
  ASSERT_EQ(zx_object_wait_one(job_child, ZX_TASK_TERMINATED, ZX_TIME_INFINITE, nullptr), ZX_OK,
            "");
...
static bool set_job_oom_kill_bit() {
  ...
  size_t oom = 1u;
  ASSERT_EQ(zx_object_set_property(zx_job_default(), ZX_PROP_JOB_KILL_ON_OOM, &oom, sizeof(size_t)),
            ZX_OK, "");
...
static bool wait_test() {
  ...
  zx_signals_t signals;
  ASSERT_EQ(zx_object_wait_one(job_child, ZX_JOB_NO_JOBS, ZX_TIME_INFINITE, &signals), ZX_OK, "");
  ASSERT_EQ(signals, ZX_JOB_NO_JOBS, "");

  zx_nanosleep(zx_deadline_after(ZX_MSEC(5)));
  ASSERT_EQ(zx_task_kill(process), ZX_OK, "");

  ASSERT_EQ(zx_object_wait_one(job_child, ZX_JOB_NO_PROCESSES, ZX_TIME_INFINITE, &signals), ZX_OK,
            "");
...
static bool info_task_stats_fails() {
  ...
  ASSERT_NE(
      zx_object_get_info(zx_job_default(), ZX_INFO_TASK_STATS, &info, sizeof(info), NULL, NULL),
      ZX_OK, "Just added job support to info_task_status?");
...
static bool max_height_smoke() {
  ...
  zx_handle_t vmar;
  ASSERT_EQ(zx_process_create(parent_job, "test", sizeof("test"), 0u, &child_proc, &vmar), ZX_OK,
            "");
