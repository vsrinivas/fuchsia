...
bool clone_resize_clone_hazard() {
    ...
    EXPECT_EQ(ZX_OK, vmo->vmo().create_child(
        ZX_VMO_CHILD_PRIVATE_PAGER_COPY| ZX_VMO_CHILD_RESIZABLE, 0, kSize, &clone_vmo));
...
bool clone_resize_parent_ok() {
    ...
    zx::vmo clone_vmo;
    ASSERT_EQ(ZX_OK, vmo->vmo().create_child(ZX_VMO_CHILD_PRIVATE_PAGER_COPY,
                                             0, kSize, &clone_vmo));
...
bool clone_shrink_grow_parent() {
    ...
    for (auto& config : configs) {
        ...
        uintptr_t ptr_ro;
        EXPECT_EQ(ZX_OK, zx::vmar::root_self()->map(0, clone_vmo, 0, config.clone_size,
                                                    ZX_VM_PERM_READ, &ptr_ro));
