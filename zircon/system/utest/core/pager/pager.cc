...
// is removed from the VMO *and* added to the pmm free pool. Regression test for fxbug.dev/73865.
TEST(Pager, EvictPages) {
  // Please do not use get_root_resource() in new code. See fxbug.dev/31358.
  if (!&get_root_resource) {
  ...
  // Create a pager vmo.
  zx::vmo vmo;
  // Use a large enough size to observe a noticeable increase in free pages on eviction.
  constexpr uint64_t kNumPages = 4096;  // 16MB
  ASSERT_EQ(pager.create_vmo(0, port, 0, kNumPages * zx_system_get_page_size(), &vmo), ZX_OK);

  // Create an auxiliary vmo to transfer pages out of.
  zx::vmo aux;
  ASSERT_OK(zx::vmo::create(kNumPages * zx_system_get_page_size(), 0, &aux));
  // Commit non-zero pages in the aux vmo.
  const uint64_t data = 67;
  for (uint64_t i = 0; i < kNumPages; i++) {
