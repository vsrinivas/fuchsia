...
void CheckWrongHandleTypeFails(zx_object_info_topic_t topic, uint32_t entry_count,
    ...
    ASSERT_NOT_OK(
        handle.get_info(topic, entries, sizeof(EntryType) * entry_count, &actual, &avail));
}
...
void CheckMissingRightsFail(zx_object_info_topic_t topic, uint32_t entry_count,
    ...
    EXPECT_EQ(unpriviledged_handle.get_info(topic, entries, sizeof(EntryType) * entry_count,
                                            &actual, &avail),
              ZX_ERR_ACCESS_DENIED);
}
