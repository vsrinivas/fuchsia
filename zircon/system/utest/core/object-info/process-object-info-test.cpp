...
zx_status_t GetKoid(const zx::object_base& object, zx_koid_t* koid) {
    ...
    zx_status_t status = zx_object_get_info(object.get(), ZX_INFO_HANDLE_BASIC, &info, sizeof(info),
                                            nullptr, nullptr);
    if (status == ZX_OK) {
        *koid = info.koid;
    }
...
TEST_F(ProcessGetInfoTest, InfoProcessMapsSmokeTest) {
    ...
    for (size_t i = 2; i < actual; i++) {
        ...
        if (entry.type == ZX_INFO_MAPS_TYPE_VMAR && entry.base == test_info.vmar_base &&
            ...
            if (entry.depth <= vmar_depth) {
                ...
                EXPECT_LE(entry.base + entry.size, test_info.vmar_base + test_info.vmar_size, "%s",
                          msg);
    ...
    for (size_t i = 0; i < actual2; i++) {
        ...
        snprintf(msg, sizeof(msg),
                 "[%2zd] %*stype:%u/%u base:0x%" PRIx64 "/0x%" PRIx64 " size:%" PRIu64 "/%" PRIu64,
                 i, (int)e1.depth * 2, "", e1.type, e2.type, e1.base, e2.base, e1.size, e2.size);
        //        LTRACEF("%s\n", msg);
...
TEST_F(ProcessGetInfoTest, InfoProcessMapsOnSelfFails) {
    ASSERT_NO_FATAL_FAILURES(
        (CheckSelfInfoFails<zx_info_maps_t>(ZX_INFO_PROCESS_MAPS, 1, process_provider())));
}
...
TEST_F(ProcessGetInfoTest, InfoProcessMapsNullActualAndAvailSucceeds) {
    ASSERT_NO_FATAL_FAILURES((CheckNullActualAndAvailSuceeds<zx_info_maps_t>(
        ZX_INFO_PROCESS_MAPS, 1, GetHandleProvider())));
}
...
TEST_F(ProcessGetInfoTest, InfoProcessMapsPartiallyUnmappedBufferIsInvalidArgs) {
    ASSERT_NO_FATAL_FAILURES((CheckPartiallyUnmappedBufferIsInvalidArgs<zx_info_maps_t>(
        ZX_INFO_PROCESS_MAPS, GetHandleProvider())));
}
...
TEST_F(ProcessGetInfoTest, InfoProcessVmosSmokeTest) {
    ...
    size_t actual;
    size_t available;
    ASSERT_OK(process.get_info(ZX_INFO_PROCESS_VMOS, vmos.get(),
                               entry_count * sizeof(zx_info_vmo_t), &actual, &available));
    ...
    for (size_t i = 0; i < actual; i++) {
        ...
        char msg[128];
        snprintf(msg, sizeof(msg),
                 "[%2zd] koid:%" PRIu64 " name:'%s' size:%" PRIu64 " flags:0x%" PRIx32, i,
                 entry.koid, entry.name, entry.size_bytes, entry.flags);
        //      LTRACEF("%s\n", msg);
...
TEST_F(ProcessGetInfoTest, InfoProcessVmosOnSelfFails) {
    ASSERT_NO_FATAL_FAILURES(
        (CheckSelfInfoFails<zx_info_vmo_t>(ZX_INFO_PROCESS_VMOS, 1, process_provider())));
}
...
TEST_F(ProcessGetInfoTest, InfoProcessVmosNullActualAndAvailSucceeds) {
    ASSERT_NO_FATAL_FAILURES((CheckNullActualAndAvailSuceeds<zx_info_vmo_t>(ZX_INFO_PROCESS_VMOS, 1,
                                                                            GetHandleProvider())));
}
...
TEST_F(ProcessGetInfoTest, InfoProcessVmosPartiallyUnmappedBufferIsInvalidArgs) {
    ASSERT_NO_FATAL_FAILURES((CheckPartiallyUnmappedBufferIsInvalidArgs<zx_info_vmo_t>(
        ZX_INFO_PROCESS_VMOS, GetHandleProvider())));
}
...
TEST_F(ProcessGetInfoTest, InfoProcessNullActualAndAvailSucceeds) {
    ASSERT_NO_FATAL_FAILURES((CheckNullActualAndAvailSuceeds<zx_info_process_t>(
        ZX_INFO_PROCESS, 1, GetHandleProvider())));
}
...
TEST_F(ProcessGetInfoTest, InfoProcessThreadsNullActualAndAvailSucceeds) {
    ASSERT_NO_FATAL_FAILURES((CheckNullActualAndAvailSuceeds<zx_koid_t>(ZX_INFO_PROCESS_THREADS, 1,
                                                                        GetHandleProvider())));
}
