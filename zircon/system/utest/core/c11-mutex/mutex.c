...
static void xlog(const char* str) {
  ...
  unittest_printf("[%08" PRIu64 ".%08" PRIu64 "]: %s", now / 1000000000, now % 1000000000, str);
}
...
static bool test_static_initializer(void) {
  ...
  EXPECT_BYTES_EQ((const uint8_t*)&static_mutex, (const uint8_t*)&auto_mutex, sizeof(mtx_t),
...
static int test_timeout_helper(void* ctx) TA_NO_THREAD_SAFETY_ANALYSIS {
  ...
  ASSERT_EQ(zx_object_wait_one(args->done_event, ZX_EVENT_SIGNALED, ZX_TIME_INFINITE, NULL), ZX_OK,
            "failed to wait");
...
static bool test_timeout_elapsed(void) {
  ...
  ASSERT_EQ(zx_object_wait_one(args.start_event, ZX_EVENT_SIGNALED, ZX_TIME_INFINITE, NULL), ZX_OK,
            "failed to wait");
