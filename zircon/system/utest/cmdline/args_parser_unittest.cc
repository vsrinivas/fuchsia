...
bool general() {
    ...
    parser.AddGeneralSwitch(
        "on-off-switch", 'o', "[no]on-off-switch help",
        [&on_off_switch]() { on_off_switch = true; },
        [&on_off_switch]() { on_off_switch = false; });

    std::string string_option_val;
    parser.AddGeneralSwitch("str", 's', "string help",
                            [&string_option_val](const std::string& s)
                                -> Status {
                                string_option_val = s;
                                return Status::Ok();
                            });
...
string help
    ...
    const char* long_no_equals[6] = {"program", "--str", "foo2", "--bool",
                                     "--on-off-switch",
                                     "param"};
    ...
    bool_set = false;
    string_option_val.clear();
    const char* short_with_space[6] = {"program", "-s", "foo3", "-b",
                                       "-o",
                                       "param"};
    ...
    EXPECT_STRING_EQ(
        "-s expects an argument but none was given.\n\nstring help",
    ...
    EXPECT_TRUE(status.has_error());
    EXPECT_STRING_EQ(
        "--str expects an argument but none was given.\n\nstring help",
        status.error_message());
    ...
    EXPECT_TRUE(status.has_error());
    EXPECT_STRING_EQ(
        "--notvalid is not a valid option. Try --help",
        status.error_message());
...
bool opt_struct() {
    ...
    parser.AddSwitch(
        "validated-format", 0, "validated-format help",
        &MyOptions::validated_format,
    ...
    parser.AddSwitch("on_by_default", 0, "on_by_default help",
                     &MyOptions::on_by_default);
    ...
    parser.AddSwitch("not-optional-str", 0, "not-optional-str help",
                     &MyOptions::not_optional_str);
    parser.AddSwitch("optional-bool", 0, "optional-bool help",
                     &MyOptions::optional_bool);

    const char* bool_and_optional_str[4] = {
        "program", "--present", "--ps=foo", "bar"};
    ...
    const char* off_sizet_string_optionalbool_validate[7] = {
        "program",
        "--noon_by_default",
        "--size_t=50", "--not-optional-str=hasvalue",
        "--optional-bool", "--validated-format=json",
        "bar"};
    ...
    const char* optionalbool_to_false[3] = {
        "program",
        "--nooptional-bool",
        "bar"};
    ...
    const char* invalid_format[3] = {
        "program",
        "--validated-format=xml",
        "bar"};
