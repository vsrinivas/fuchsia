...
TEST(MyXUnion, CodingTableWhenNullable) {
  ...
  ASSERT_EQ(&fidl_internal_kInt32Table, my_xunion_table.fields[0].type);
  ASSERT_EQ(&fidl_internal_kBoolTable, my_xunion_table.fields[1].type);
...
TEST(MyStrictXUnion, CodingTableWhenNullable) {
  ...
  // The ordering in the coding table is |bar| followed by |foo|, due to sorting.
  ASSERT_EQ(&fidl_internal_kInt32Table, my_strict_xunion_table.fields[0].type);
  ASSERT_EQ(&fidl_internal_kBoolTable, my_strict_xunion_table.fields[1].type);
...
TEST(AltTypes, CodingTable) {
  ...
  ASSERT_EQ(&v1_fidl_test_example_codingtables_MyUnionContainerTable.coded_struct,
            old_struct.alt_type);
  const FidlCodedStruct& v1_struct = *(old_struct.alt_type);
  ...
  const FidlCodedUnion& v1_union = *old_union.alt_type;
  ...
  const FidlCodedArray& v1_array = *old_array.alt_type;
  ...
  const FidlCodedArray& v1_optional_array = *old_optional_array.alt_type;
  ASSERT_EQ(kFidlTypeUnionPointer, v1_optional_array.element->type_tag);
  ...
  const FidlCodedVector& v1_vector = *old_vector.alt_type;
