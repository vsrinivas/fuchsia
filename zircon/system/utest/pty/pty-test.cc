...
static zx_status_t open_client(const fbl::unique_fd& fd, uint32_t client_id, int* out_fd) {
  ...
  auto result = fpty::Device::Call::OpenClient(io.borrow_as<fpty::Device>(), client_id,
...
TEST(PtyTests, pty_test) {
  ...
  auto result1 = fpty::Device::Call::ReadEvents(pc_io.borrow_as<fpty::Device>());
  ...
  auto result2 = fpty::Device::Call::ReadEvents(pc_io.borrow_as<fpty::Device>());
  ASSERT_OK(result2.status());
  ASSERT_OK(result2->status);
  ASSERT_EQ(result2->events, fpty::wire::EVENT_INTERRUPT);

  // should vanish once we read it
  auto result3 = fpty::Device::Call::ReadEvents(pc_io.borrow_as<fpty::Device>());
  ...
  auto result4 = fpty::Device::Call::ReadEvents(pc_io.borrow_as<fpty::Device>());
  ASSERT_OK(result4.status());
  ASSERT_OK(result4->status);
  ASSERT_EQ(result4->events, fpty::wire::EVENT_INTERRUPT);

  auto ws_result1 = fpty::Device::Call::GetWindowSize(pc_io.borrow_as<fpty::Device>());
  ...
  ws.width = 80;
  ws.height = 25;
  auto result5 = fpty::Device::Call::SetWindowSize(pc_io.borrow_as<fpty::Device>(), ws);
  ASSERT_OK(result5.status());
  ASSERT_OK(result5->status);
  auto ws_result2 = fpty::Device::Call::GetWindowSize(pc_io.borrow_as<fpty::Device>());
  ...
  auto result6 = fpty::Device::Call::ClrSetFeature(pc_io.borrow_as<fpty::Device>(), 0,
                                                   fpty::wire::FEATURE_RAW);
  ...
  auto result7 = fpty::Device::Call::ReadEvents(pc_io.borrow_as<fpty::Device>());
  ...
  uint32_t n = 2;
  auto result8 = fpty::Device::Call::MakeActive(pc_io.borrow_as<fpty::Device>(), n);
  ...
  auto result9 = fpty::Device::Call::MakeActive(pc1_io.borrow_as<fpty::Device>(), n);
  ASSERT_EQ(result9.status(), ZX_OK);
  ASSERT_STATUS(result9->status, ZX_ERR_ACCESS_DENIED);

  // but controlling client can
  n = 1;
  auto result10 = fpty::Device::Call::MakeActive(pc_io.borrow_as<fpty::Device>(), n);
  ...
  auto result11 = fpty::Device::Call::ReadEvents(pc_io.borrow_as<fpty::Device>());
...
TEST(PtyTests, not_a_pty_test) {
  ...
  auto result = fpty::Device::Call::GetWindowSize(io.borrow_as<fpty::Device>());
