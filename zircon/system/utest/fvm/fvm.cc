...
// operations.
TEST_F(FvmTest, TestCloseDuringAccess) {
  constexpr uint64_t kBlockSize = 512;
  constexpr uint64_t kBlockCount = 1 << 16;
  constexpr uint64_t kSliceSize = 64 * kBlockSize;
  CreateFVM(kBlockSize, kBlockCount, kSliceSize);
...
TEST_F(FvmTest, TestDestroyDuringAccess) {
  constexpr uint64_t kBlockSize = 512;
  constexpr uint64_t kBlockCount = 1 << 16;
  constexpr uint64_t kSliceSize = 64 * kBlockSize;
  CreateFVM(kBlockSize, kBlockCount, kSliceSize);
...
struct fvm_test_state_t {
  ...
  size_t slice_size;
  size_t slices_total;
  fvm_thread_state_t thread_states[kThreadCount];
...
int random_access_thread(void* arg) {
  auto ta = static_cast<thrd_args_t*>(arg);
  ...
  CheckWriteColor(self->vp_fd.get(), 0, st->slice_size, color);
  CheckReadColor(self->vp_fd.get(), 0, st->slice_size, color);
  ...
  for (size_t i = 0; i < num_ops; ++i) {
    switch (rand_r(&seed) % 5) {
      case 0: {
        ...
        CheckNoAccessBlock(self->vp_fd.get(), byte_off / st->block_size, byte_len / st->block_size);

        fzl::UnownedFdioCaller partition_caller(self->vp_fd.get());
        zx::unowned_channel partition_channel(partition_caller.borrow_channel());
        EXPECT_OK(fuchsia_hardware_block_volume_VolumeExtend(partition_channel->get(), offset,
                                                             length, &status));
        EXPECT_OK(status);
        self->extents[extent_index].len += extension_length;

        CheckWriteColor(self->vp_fd.get(), byte_off, byte_len, color);
        CheckReadColor(self->vp_fd.get(), byte_off, byte_len, color);
        break;
      }
      case 1: {
      ...
      case 2: {
        ...
        fzl::UnownedFdioCaller partition_caller(self->vp_fd.get());
        zx::unowned_channel partition_channel(partition_caller.borrow_channel());
        EXPECT_OK(fuchsia_hardware_block_volume_VolumeShrink(partition_channel->get(), offset,
                                                             length, &status));
        EXPECT_OK(status);
...
TEST_F(FvmTest, TestRandomOpMultithreaded) {
  const uint64_t kBlockSize = 512;
  const uint64_t kBlockCount = 1 << 20;
  const uint64_t kBlocksPerSlice = 32;
  const uint64_t kSliceSize = kBlocksPerSlice * kBlockSize;
  CreateFVM(kBlockSize, kBlockCount, kSliceSize);

  const size_t kDiskSize = kBlockSize * kBlockCount;
  const size_t kSlicesCount = fvm::UsableSlicesCount(kDiskSize, kSliceSize);

  ASSERT_GT(kSlicesCount, kThreadCount * 2, "Not enough slices to distribute between threads");
  ...
  s.slice_size = kSliceSize;
  {
    ...
    s.slices_left = kSlicesCount - kThreadCount;
  ...
  for (size_t i = 0; i < kThreadCount; i++) {
    ...
    s.thread_states[i].vp_fd.reset(
        fvm_allocate_partition_with_devfs(devfs_root().get(), fd.get(), &request));
    ASSERT_TRUE(s.thread_states[i].vp_fd);
  }

  thrd_args_t ta[kThreadCount];
  ...
  for (size_t i = 0; i < kThreadCount; i++) {
    ...
    CheckWriteReadBlock(s.thread_states[i].vp_fd.get(), 0, kBlocksPerSlice);
    EXPECT_EQ(thrd_create(&s.thread_states[i].thr, random_access_thread, &ta[i]), thrd_success);
  }

  partition_entry_t entries[kThreadCount];
  ...
  for (size_t i = 0; i < kThreadCount; i++) {
    request.guid[0] = static_cast<uint8_t>(i);
    fbl::unique_fd vp_fd(
        open_partition_with_devfs(devfs_root().get(), request.guid, request.type, 0, nullptr));
    ASSERT_TRUE(vp_fd);
