...
int StartFVMTest(uint64_t blk_size, uint64_t initial_blk_count, uint64_t max_blk_count,
  ...
  status = fuchsia_device_ControllerBind(fvm_channel.get(), FVM_DRIVER_LIB, STRLEN(FVM_DRIVER_LIB),
                                         &call_status);
  if (status == ZX_OK) {
    status = call_status;
  }
...
fbl::unique_fd FVMRebind(fbl::unique_fd fvm_fd, char* disk_path, const partition_entry_t* entries,
  ...
  if (use_real_disk) {
    ...
    zx_status_t call_status;
    zx_status_t status = fuchsia_device_ControllerBind(disk_caller.borrow_channel(), FVM_DRIVER_LIB,
                                                       STRLEN(FVM_DRIVER_LIB), &call_status);
    if (status == ZX_OK) {
      status = call_status;
    }
      ...
      for (size_t dev_off = dev_off_start; dev_off <= dev_off_end; dev_off += bsz) {
        ...
        for (size_t len = len_start; len <= len_end; len += bsz) {
          ...
          for (size_t vmo_off = 0; vmo_off < 3 * bsz; vmo_off += bsz) {
            ...
            while (sub_off < len) {
              ASSERT_TRUE(vc->CheckRead(vb.get(), vmo_off + sub_off, dev_off + sub_off, sub_len));
              sub_off += sub_len;
...
bool TestPersistenceSimple() {
  ...
  ASSERT_TRUE(
      CheckRead(vp_fd.get(), block_info.block_size * last_block, block_info.block_size, &buf[0]));
  // ... And we can cross slices
  ASSERT_TRUE(CheckRead(vp_fd.get(), block_info.block_size * last_block, block_info.block_size * 2,
                        &buf[0]));
...
bool TestAbortDriverLoadSmallDevice() {
  ...
  ASSERT_EQ(fdio_get_service_handle(ramdisk_fd.get(), fvm_channel.reset_and_get_address()), ZX_OK);
  ASSERT_EQ(fuchsia_device_ControllerBind(fvm_channel.get(), FVM_DRIVER_LIB, STRLEN(FVM_DRIVER_LIB),
                                          &call_status),
            ZX_OK);
    ...
    if (!strcmp(argv[i], "-d")) {
      if (strnlen(argv[i + 1], PATH_MAX) > 0) {
        ...
        zx_status_t status = fuchsia_device_ControllerGetTopologicalPath(
            fdio_unsafe_borrow_channel(io), &call_status, test_disk_path, PATH_MAX - 1, &path_len);
