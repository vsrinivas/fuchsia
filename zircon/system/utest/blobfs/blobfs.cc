...
static bool VerifyCompromised(int fd, const char* data, size_t size_data) {
  ...
  ASSERT_EQ(fs_test_utils::StreamAll(read, fd, &buf[0], size_data), -1, "Expected reading to fail");
  return true;
}
...
static bool uint8_to_hex_str(const uint8_t* data, char* hex_str) {
  for (size_t i = 0; i < 32; i++) {
    ASSERT_EQ(sprintf(hex_str + (i * 2), "%02x", data[i]), 2, "Error converting name to string");
  }
  hex_str[64] = 0;
  return true;
}
...
static bool TestReaddir(BlobfsTest* blobfsTest) {
  ...
  fbl::AllocChecker ac;
  fbl::Array<fbl::unique_ptr<fs_test_utils::BlobInfo>> info(
      new (&ac) fbl::unique_ptr<fs_test_utils::BlobInfo>[kMaxEntries](), kMaxEntries);
  ...
  for (size_t i = 0; i < kMaxEntries; i++) {
    ...
    ASSERT_TRUE(fd, "Failed to-reopen blob");
    ASSERT_TRUE(fs_test_utils::VerifyContents(fd.get(), info[i]->data.get(), info[i]->size_data));
    ASSERT_EQ(close(fd.release()), 0);
  }
  ...
  while ((de = readdir(dir)) != nullptr) {
    for (size_t i = 0; i < kMaxEntries; i++) {
      if ((info[i]->size_data != 0) && strcmp(strrchr(info[i]->path, '/') + 1, de->d_name) == 0) {
...
static bool TestDiskTooSmall(BlobfsTest* blobfsTest) {
  ...
  if (blobfsTest->GetType() == FsTestType::kFvm) {
    ...
    ASSERT_EQ(minimum_size - blobfs_size, fvm::MetadataSize(minimum_size, kTestFvmSliceSize) * 2);
  } else {
  ...
  ASSERT_NE(mkfs(device_path, DISK_FORMAT_BLOBFS, launch_stdio_sync, &default_mkfs_options), ZX_OK);
...
bool WriteAfterUnlink(BlobfsTest* blobfsTest) {
  ...
  ASSERT_EQ(unlink(info->path), 0);
  ASSERT_EQ(
      fs_test_utils::StreamAll(write, fd.get(), info->data.get() + size / 2, size - (size / 2)), 0,
      "Failed to write Data");
  ASSERT_EQ(close(fd.release()), 0);
...
static bool ReadTooLarge(BlobfsTest* blobfsTest) {
  ...
  for (size_t i = 0; i < 16; i++) {
    ...
    for (ssize_t j = 1; j < static_cast<ssize_t>(info->size_data); j *= 2) {
      ...
      ASSERT_EQ(memcmp(buf.get(), &info->data[info->size_data - j], j), 0,
...
static bool WaitForRead(BlobfsTest* blobfsTest) {
...

  {
    ...
    auto join_thread = fbl::MakeAutoCall([&]() { success = thrd_join(waiter_thread, &result); });
...
static bool TestHugeBlobRandom(BlobfsTest* blobfsTest) {
  ...
  ASSERT_TRUE(fs_test_utils::GenerateRandomBlob(MOUNT_PATH, 2 * blobfs::WriteBufferSize(), &info));
...
static bool CorruptAtMount(BlobfsTest* blobfsTest) {
  ...
  char device_path[PATH_MAX];
  ASSERT_TRUE(blobfsTest->GetDevicePath(device_path, PATH_MAX));
  ASSERT_EQ(mkfs(device_path, DISK_FORMAT_BLOBFS, launch_stdio_sync, &default_mkfs_options), ZX_OK);
  ...
  zx_status_t status;
  ASSERT_EQ(
      fuchsia_hardware_block_volume_VolumeShrink(caller.borrow_channel(), offset, length, &status),
      ZX_OK);
  ASSERT_EQ(status, ZX_OK);
  ...
  ASSERT_EQ(fuchsia_hardware_block_volume_VolumeQuerySlices(caller.borrow_channel(), start_slices,
  ...
  options.enable_journal = gEnableJournal;
  caller.reset();
  ASSERT_NE(mount(fd.release(), MOUNT_PATH, DISK_FORMAT_BLOBFS, &options, launch_stdio_async),
            ZX_OK);
  ...
  length = 2;
  ASSERT_EQ(
      fuchsia_hardware_block_volume_VolumeExtend(caller.borrow_channel(), offset, length, &status),
      ZX_OK);
  ASSERT_EQ(status, ZX_OK);
  ...
  caller.reset();
  ASSERT_EQ(mount(fd.release(), MOUNT_PATH, DISK_FORMAT_BLOBFS, &options, launch_stdio_async),
            ZX_OK);
...
static bool TestExtendFailure(void) {
  ...
  ASSERT_TRUE(fs_test_utils::GenerateRandomBlob(
      MOUNT_PATH, original_usage.total_bytes - blobfs::kBlobfsBlockSize, &old_info));
  ...
  ASSERT_TRUE(fs_test_utils::GenerateRandomBlob(MOUNT_PATH, blobfs::kBlobfsBlockSize, &new_info));
...
static bool TestLargeBlob() {
  ...
  uint64_t ramdisk_blocks = (blobfs_blocks * blobfs::kBlobfsBlockSize) / blobfsTest.GetBlockSize();
