...
bool validate_single_present_handle_check_trailing_padding() {
    ...
    for (size_t i = 0; i < 4; i++) {
        ...
        const char* error = nullptr;
        auto status = fidl_validate(&nonnullable_handle_message_type, &message, kBufferSize,
                                    kNumHandles, &error);
...
bool validate_nested_nonnullable_structs_check_padding() {
    ...
    for (const auto padding_offset : padding_offsets) {
        ...
        const char* error = nullptr;
        auto status = fidl_validate(&nested_structs_message_type, &message, kBufferSize,
                                    kNumHandles, &error);
