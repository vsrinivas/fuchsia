...
static void fidl_socket_message_handler(async_dispatcher_t* dispatcher, async_wait_t* wait,
  ...
  if (signal->observed & ZX_SOCKET_CONTROL_READABLE) {
    ...
    for (uint64_t i = 0; i < signal->count; i++) {
      ...
      size_t actual = 0u;
      status = fidl_socket_read_control(wait->object, msg.bytes, sizeof(bytes), &actual);
...
zx_status_t fidl_bind_socket(async_dispatcher_t* dispatcher, zx_handle_t socket,
  ...
  fidl_socket_binding_t* binding =
      static_cast<fidl_socket_binding_t*>(calloc(1, sizeof(fidl_socket_binding_t)));
...
static bool basic_test(void) {
  ...
  ASSERT_EQ(ZX_OK, fidl_bind_socket(
                       dispatcher, server.release(),
  ...
  size_t actual = 0u;
  ASSERT_EQ(ZX_OK, fidl_test_fakesocket_ControlGetPeerAddr(client.get(), 5, buffer, sizeof(buffer),
                                                           &actual));
  ASSERT_EQ(3u, actual);
  buffer[3] = '\0';
