...
acpi_lid::AcpiObjectEvalFunc CreateFakeAcpiEvalFn(AcpiState* state) {
    return [state](ACPI_HANDLE handle, ACPI_STRING pathname,
                   ACPI_OBJECT_LIST* external_params, ACPI_BUFFER* return_buffer,
...
TEST(LidTest, Init) {
    {
        ...
        EXPECT_OK(AcpiLidDevice::Create(fake_ddk::kFakeParent, nullptr, &device,
                                        CreateFakeAcpiEvalFn(&state),
                                        CreateFakeAcpiInstallNotifyHandlerFn(&state),
                                        CreateFakeAcpiRemoveNotifyHandlerFn(&state)));
