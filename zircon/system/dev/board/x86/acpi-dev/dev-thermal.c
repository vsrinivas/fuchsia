...
static zx_status_t acpi_thermal_get_info(acpi_thermal_device_t* dev,
    ...
    info->state = 0;
    if (info->active_trip[0] && (decikelvin_to_celsius(temp) > info->active_trip[0])) {
...
static zx_status_t fidl_GetTemperatureCelsius(void* ctx, fidl_txn_t* txn) {
    ...
    return fuchsia_hardware_thermal_DeviceGetTemperatureCelsius_reply(
        txn, ZX_OK, decikelvin_to_celsius(v));
}
...
static zx_status_t fidl_SetTripCelsius(void* ctx, uint32_t id, float temp, fidl_txn_t* txn) {
    ...
    if (acpi_status != AE_OK) {
        ...
        return fuchsia_hardware_thermal_DeviceSetTripCelsius_reply(txn,
                                                                   acpi_to_zx_status(acpi_status));
    }
    mtx_lock(&dev->lock);
