...
ACPI_STATUS AcpiFakeEvaluateObject(ACPI_HANDLE handle, char* key,
                                ACPI_OBJECT_LIST* args, ACPI_BUFFER* buffer) {
...
TEST(TestCase, TestSignalOnBatteryChargeLevel) {
    verify_battery_change_signal(49, 0);
}

TEST(TestCase, TestSignalOnBatteryDischargeLevel) {
    verify_battery_change_signal(51, 0);
}
