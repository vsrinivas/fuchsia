...
#ifndef ZIRCON_SYSTEM_DEV_TEE_OPTEE_SYSMEM_TEE_SERVER_H_
#define ZIRCON_SYSTEM_DEV_TEE_OPTEE_SYSMEM_TEE_SERVER_H_
...
// TODO(36201): (dustingreen) Move SysmemTeeServer-like code to aml-securemem.

// This is used with fidl::Bind() to dispatch fuchsia::sysmem::Tee requests.
class SysmemTeeServer : public llcpp::fuchsia::sysmem::Tee::Interface {
 public:
  using TeeServerDone = fit::callback<void(bool is_success)>;

  SysmemTeeServer(thrd_t ddk_dispatcher_thread, zx::channel loopback_tee_client);
  ~SysmemTeeServer() override;

  zx_status_t BindAsync(zx::channel sysmem_tee_server, thrd_t* sysmem_tee_server_thread,
                        TeeServerDone tee_server_done);
  void StopAsync();

  // llcpp::fuchsia::sysmem::Tee::Interface impl
  void GetPhysicalSecureHeaps(
      llcpp::fuchsia::sysmem::Tee::Interface::GetPhysicalSecureHeapsCompleter::Sync _completer)
      override;
  void SetPhysicalSecureHeaps(
      llcpp::fuchsia::sysmem::PhysicalSecureHeaps heaps,
      llcpp::fuchsia::sysmem::Tee::Interface::SetPhysicalSecureHeapsCompleter::Sync _completer)
      override;
  void Epitaph(zx_status_t epitaph,
               llcpp::fuchsia::sysmem::Tee::Interface::EpitaphCompleter::Sync completer) override;
 ...
 private:
  ...
  zx::channel loopback_tee_client_channel_;
  TEEC_Context context_ = {};
  async::Loop loop_;
  thrd_t loop_thread_ = {};
  bool was_thread_started_ = {};
  bool is_loop_done_ = {};
  TeeServerDone tee_server_done_;
...
#endif  // ZIRCON_SYSTEM_DEV_TEE_OPTEE_SYSMEM_TEE_SERVER_H_
