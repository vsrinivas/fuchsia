...
SysmemTeeServer::SysmemTeeServer(thrd_t ddk_dispatcher_thread,
                                 zx::channel loopback_tee_client_channel)
    : ddk_dispatcher_thread_(ddk_dispatcher_thread),
      loopback_tee_client_channel_(std::move(loopback_tee_client_channel)),
      loop_(&kAsyncLoopConfigNoAttachToThread) {
  ZX_DEBUG_ASSERT(loopback_tee_client_channel_);
}

SysmemTeeServer::~SysmemTeeServer() {
...
zx_status_t SysmemTeeServer::BindAsync(zx::channel sysmem_tee_server,
                                       thrd_t* sysmem_tee_server_thread,
                                       TeeServerDone tee_server_done) {
  ZX_DEBUG_ASSERT(sysmem_tee_server);
  ZX_DEBUG_ASSERT(sysmem_tee_server_thread);
  ZX_DEBUG_ASSERT(tee_server_done);
  ZX_DEBUG_ASSERT(thrd_current() == ddk_dispatcher_thread_);
  zx_status_t status = loop_.StartThread("sysmem_tee_server_loop", &loop_thread_);
  if (status != ZX_OK) {
  ...
  *sysmem_tee_server_thread = loop_thread_;
  tee_server_done_ = std::move(tee_server_done);
  PostToLoop([this, sysmem_tee_server = std::move(sysmem_tee_server)]() mutable {
    ZX_DEBUG_ASSERT(thrd_current() == loop_thread_);
    zx_status_t status =
        fidl::Bind<SysmemTeeServer>(loop_.dispatcher(), std::move(sysmem_tee_server), this,
                                    [this](SysmemTeeServer* sysmem_tee_server) {
                                      // This can get called from the ddk_dispatcher_thread_ if
                                      // we're doing loop_.Shutdown() to unbind an llcpp server (so
                                      // far the only way to unbind an llcpp server).
                                      //
                                      // If tee_server_done_ still set by this point, !is_success.
...
void SysmemTeeServer::StopAsync() {
...
void SysmemTeeServer::GetPhysicalSecureHeaps(
    llcpp::fuchsia::sysmem::Tee::Interface::GetPhysicalSecureHeapsCompleter::Sync completer) {
  ZX_DEBUG_ASSERT(thrd_current() == loop_thread_);
  llcpp::fuchsia::sysmem::Tee_GetPhysicalSecureHeaps_Result result;
...
void SysmemTeeServer::SetPhysicalSecureHeaps(
    llcpp::fuchsia::sysmem::PhysicalSecureHeaps heaps,
    llcpp::fuchsia::sysmem::Tee::Interface::SetPhysicalSecureHeapsCompleter::Sync completer) {
  ZX_DEBUG_ASSERT(thrd_current() == loop_thread_);
  llcpp::fuchsia::sysmem::Tee_SetPhysicalSecureHeaps_Result result;
  auto complete = fit::defer([&completer, &result] {
    ZX_DEBUG_ASSERT(!result.has_invalid_tag());
    completer.Reply(std::move(result));
  });
  ...
  result.set_response(llcpp::fuchsia::sysmem::Tee_SetPhysicalSecureHeaps_Response{});
}

void SysmemTeeServer::Epitaph(
    zx_status_t epitaph, llcpp::fuchsia::sysmem::Tee::Interface::EpitaphCompleter::Sync completer) {
  ZX_DEBUG_ASSERT(thrd_current() == loop_thread_);
  // Since Epitaph() is a one-way message, we don't need the completer.
  bool is_success = (epitaph == ZX_OK);
  LOG(INFO, "calling EnsureLoopDone() - is_success: %d", is_success);
  // This will Quit() the loop; we don't want to recieve any more messages.
  EnsureLoopDone(is_success);
}

void SysmemTeeServer::PostToLoop(fit::closure to_run) {
  // For now this is only expected to be called from ddk_dispatcher_thread_.
  ZX_DEBUG_ASSERT(thrd_current() == ddk_dispatcher_thread_);
  closure_queue_.Enqueue(std::move(to_run));
}

zx_status_t SysmemTeeServer::TrySetupSecmemClientSession() {
  ZX_DEBUG_ASSERT(thrd_current() == loop_thread_);
  ...
  if (!is_setup_secmem_client_session_attempted_) {
    ...
    ZX_DEBUG_ASSERT(loopback_tee_client_channel_);
    context_.imp.tee_channel = loopback_tee_client_channel_.release();
...
void SysmemTeeServer::EnsureLoopDone(bool is_success) {
  if (is_loop_done_) {
    return;
  }
  ...
  if (tee_server_done_) {
    tee_server_done_(is_success);
  }
}

zx_status_t SysmemTeeServer::GetPhysicalSecureHeapsInternal(
    llcpp::fuchsia::sysmem::PhysicalSecureHeaps* heaps) {
  ZX_DEBUG_ASSERT(thrd_current() == loop_thread_);
...
zx_status_t SysmemTeeServer::SetPhysicalSecureHeapsInternal(
    llcpp::fuchsia::sysmem::PhysicalSecureHeaps heaps) {
  ZX_DEBUG_ASSERT(thrd_current() == loop_thread_);
...
zx_status_t SysmemTeeServer::SetupVdec(uint64_t* physical_address, size_t* size_bytes) {
  ZX_DEBUG_ASSERT(thrd_current() == loop_thread_);
  ...
  if (tee_status != TEEC_SUCCESS) {
    LOG(ERROR, "secmem_client_session_.AllocateSecureMemory() failed - tee_status: %" PRIu32, tee_status);
...
zx_status_t SysmemTeeServer::ProtectMemoryRange(uint64_t physical_address, size_t size_bytes) {
  ZX_DEBUG_ASSERT(thrd_current() == loop_thread_);
  ZX_DEBUG_ASSERT(is_setup_secmem_client_session_attempted_);
  ZX_DEBUG_ASSERT(setup_secmem_client_session_status_ == ZX_OK);
  ...
  TEEC_Result tee_status = secmem_client_session_->ProtectMemoryRange(start, length);
  if (tee_status != TEEC_SUCCESS) {
