...
class DivisorLatchLowerRegister
    : public hwreg::RegisterBase<DivisorLatchLowerRegister, uint8_t> {
public:
    DEF_FIELD(7, 0, data);
...
class DivisorLatchUpperRegister
    : public hwreg::RegisterBase<DivisorLatchUpperRegister, uint8_t> {
public:
    DEF_FIELD(7, 0, data);
...
Uart16550::Uart16550(zx_device_t* parent)
...
size_t Uart16550::FifoDepth() const {
    return uart_fifo_len_;
}
...
zx_status_t Uart16550::Init() {
    ...
    status = io_port.get_info(ZX_INFO_RESOURCE, &resource_info,
                              sizeof(resource_info), nullptr, nullptr);
    if (status != ZX_OK) {
        zxlogf(ERROR, "%s: io_port.get_info failed\n", __func__);
        return status;
    }

    const uint16_t port_base = static_cast<uint16_t>(resource_info.base);
    const uint32_t port_size = static_cast<uint32_t>(resource_info.size);
    ...
    {
        std::lock_guard<std::mutex> lock(device_mutex_);
        auto port_read = [](uint16_t port) -> uint8_t {
            return inp(port);
        };

        auto port_write = [](uint8_t data, uint16_t port) {
            outp(port, data);
        };
...
zx_status_t Uart16550::Init(zx::interrupt interrupt,
...
zx::unowned_interrupt Uart16550::InterruptHandle() {
    return zx::unowned_interrupt(interrupt_);
}
...
zx_status_t Uart16550::SerialImplConfig(uint32_t baud_rate, uint32_t flags) {
    ...
    if ((flags & SERIAL_FLOW_CTRL_MASK) != SERIAL_FLOW_CTRL_NONE &&
    ...
    lcr.set_divisor_latch_access(true)
        .WriteTo(&port_io_);
...
void Uart16550::DdkUnbind() {
    DdkRemove();
}

bool Uart16550::SupportsAutomaticFlowControl() const {
    return uart_fifo_len_ == kFifoDepth16750;
}
...
void Uart16550::InitFifosLocked() {
    ResetFifosLocked();
    const auto iir = InterruptIdentRegister::Get()
                         .ReadFrom(&port_io_);
