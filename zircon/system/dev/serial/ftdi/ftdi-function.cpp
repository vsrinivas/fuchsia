...
public:
    FakeFtdiFunction(zx_device_t* parent)
...
void FakeFtdiFunction::UsbFunctionInterfaceGetDescriptors(void* ctx, void* out_descriptors_buffer,
                                                          ...
                                                          size_t* out_descriptors_actual) {
    FakeFtdiFunction* func = static_cast<FakeFtdiFunction*>(ctx);
    memcpy(out_descriptors_buffer, &func->descriptor_, std::min(descriptors_size,
           func->descriptor_size_));
...
zx_status_t FakeFtdiFunction::Bind() {
    ...
    status = usb::Request<>::Alloc(&data_out_req_, BULK_MAX_PACKET, bulk_out_addr_,
                                   parent_req_size_);
    if (status != ZX_OK) {
        return status;
    }
    status = usb::Request<>::Alloc(&data_in_req_, BULK_MAX_PACKET, bulk_in_addr_,
                                   parent_req_size_);
    if (status != ZX_OK) {
        return status;
    }
    ...
    thrd_create(&thread_,
                [](void* ctx) { return static_cast<FakeFtdiFunction*>(ctx)->Thread(); },
                this);
...
void FakeFtdiFunction::DdkRelease() {
    delete this;
}
