...
const std::map<uint64_t, Platform> kPlatformTranslation = {
    {MakeVidPidKey(PDEV_VID_AMLOGIC, PDEV_PID_AMLOGIC_S912),
     kPlatform_Amlogic_S912},
    {MakeVidPidKey(PDEV_VID_AMLOGIC, PDEV_PID_AMLOGIC_S905D2),
     kPlatform_Amlogic_S905D2},
    {MakeVidPidKey(PDEV_VID_AMLOGIC, PDEV_PID_AMLOGIC_T931),
     kPlatform_Amlogic_T931},
};
...
Platform FindPlatform(uint32_t pdev_device_info_vid,
                      uint32_t pdev_device_info_pid) {
    auto iter = kPlatformTranslation.find(
        MakeVidPidKey(pdev_device_info_vid, pdev_device_info_pid));
...
// |a| to check
// |r| required bits
bool HasAllRequiredBits(uint32_t a, uint32_t r) {
    return (r & a) == r;
}
...
bool HasAllRequiredUsageBits(const fuchsia_sysmem_BufferUsage& a,
    ...
    return HasAllRequiredBits(a.cpu, r.cpu) &&
           HasAllRequiredBits(a.vulkan, r.vulkan) &&
           HasAllRequiredBits(a.display, r.display) &&
           HasAllRequiredBits(a.video, r.video);
}
...
uint32_t SharedUsageBitsCount(const fuchsia_sysmem_BufferUsage& a,
    ...
    return SharedBitsCount(a.cpu, b.cpu) + SharedBitsCount(a.vulkan, b.vulkan) +
           SharedBitsCount(a.display, b.display) +
           SharedBitsCount(a.video, b.video);
}
...
bool IsBetterMatch(
    ...
    ZX_DEBUG_ASSERT(
        HasAllRequiredUsageBits(usage, a->required_buffer_usage_bits));
    // We intentionally allow b to be nullptr.
    if (b == nullptr) {
        return true;
    }
    ZX_DEBUG_ASSERT(
        HasAllRequiredUsageBits(usage, b->required_buffer_usage_bits));
    uint32_t a_shared_bits =
        SharedUsageBitsCount(usage, a->required_buffer_usage_bits);
    uint32_t b_shared_bits =
        SharedUsageBitsCount(usage, b->required_buffer_usage_bits);
    return a_shared_bits > b_shared_bits;
}

double
GetCostInternal(const fuchsia_sysmem_BufferCollectionConstraints* constraints,
    ...
    while (platform_costs) {
        for (const UsagePixelFormatCostEntry& cost : platform_costs->costs) {
            if (IsBetterMatch(constraints, image_format_constraints_index,
                              &cost, best_match)) {
                best_match = &cost;
            }
        }
...
double GetCost(uint32_t pdev_device_info_vid, uint32_t pdev_device_info_pid,
    ...
    Platform platform =
        FindPlatform(pdev_device_info_vid, pdev_device_info_pid);
    if (platform == kPlatform_None) {
        return kDefaultCost;
    }
    return GetCostInternal(constraints, image_format_constraints_index,
                           platform);
}
...
int32_t UsagePixelFormatCost::Compare(
    ...
    double cost_a = GetCost(pdev_device_info_vid, pdev_device_info_pid,
                            constraints, image_format_constraints_index_a);
    double cost_b = GetCost(pdev_device_info_vid, pdev_device_info_pid,
                            constraints, image_format_constraints_index_b);
