...
TEST(DeviceControllerIntegrationTest, AddDevicePowerCaps_Success) {
  IsolatedDevmgr devmgr;
  IsolatedDevmgr::Args args;
  args.load_drivers.push_back("/boot/driver/ddk-power-test.so");
  args.load_drivers.push_back("/boot/driver/ddk-power-test-child.so");
...
TEST(DeviceControllerIntegrationTest, GetDevicePowerCaps_Success) {
  ...
  zx_status_t status = IsolatedDevmgr::Create(&args, &devmgr);
  ASSERT_OK(status);
  fbl::unique_fd parent_fd, child_fd, child2_fd;
  ASSERT_OK(devmgr_integration_test::RecursiveWaitForFile(
      devmgr.devfs_root(), "sys/platform/11:0b:0/power-test", &parent_fd));
  ASSERT_GT(parent_fd.get(), 0);
  ...
  auto power_states = ::fidl::VectorView<DevicePowerStateInfo>(3,
                       reinterpret_cast<DevicePowerStateInfo *>(states));
  auto response = TestDevice::Call::AddDeviceWithPowerArgs(zx::unowned_channel(child_device_handle),
                                                           power_states);
  ASSERT_OK(response.status());
  ...
  ASSERT_OK(devmgr_integration_test::RecursiveWaitForFile(devmgr.devfs_root(),
                              "sys/platform/11:0b:0/power-test/power-test-child-2", &child2_fd));
  ...
  auto response2 = Controller::Call::GetDevicePowerCaps(zx::unowned_channel(child2_device_handle));
  ...
  ASSERT_TRUE(out_dpstates[static_cast<uint8_t>
              (DevicePowerState::DEVICE_POWER_STATE_D0)].is_supported);
  ASSERT_TRUE(out_dpstates[static_cast<uint8_t>
              (DevicePowerState::DEVICE_POWER_STATE_D1)].is_supported);
  ASSERT_EQ(out_dpstates[static_cast<uint8_t>
            (DevicePowerState::DEVICE_POWER_STATE_D1)].restore_latency, 100);
  ASSERT_TRUE(out_dpstates[static_cast<uint8_t>
              (DevicePowerState::DEVICE_POWER_STATE_D3COLD)].is_supported);
  ASSERT_EQ(out_dpstates[static_cast<uint8_t>
            (DevicePowerState::DEVICE_POWER_STATE_D3COLD)].restore_latency, 1000);
}
