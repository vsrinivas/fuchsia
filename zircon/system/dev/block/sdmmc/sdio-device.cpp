...
zx_status_t SdioDevice::Create(zx_device_t* parent, const SdmmcDevice& sdmmc,
                               fbl::RefPtr<SdioDevice>* out_dev) {
    fbl::AllocChecker ac;
    auto dev = fbl::MakeRefCountedChecked<SdioDevice>(&ac, parent, sdmmc);
    if (!ac.check()) {
...
zx_status_t SdioDevice::ProbeSdio() {
...
zx_status_t SdioDevice::AddDevice() {
    zx_device_prop_t props[] = {
        {BIND_SDIO_VID, 0, funcs_[0].hw_info.manufacturer_id},
        {BIND_SDIO_PID, 0, funcs_[0].hw_info.product_id},
    };

    zx_status_t st = DdkAdd("sdmmc-sdio", 0, props, countof(props));
    if (st != ZX_OK) {
...
void SdioDevice::DdkRelease() {
    dead_ = true;
    __UNUSED bool dummy = Release();
}

zx_status_t SdioDevice::SdioGetDevHwInfo(sdio_hw_info_t* out_hw_info) {
...
zx_status_t SdioDevice::SdioEnableFn(uint8_t fn_idx) {
    uint8_t ioex_reg = 0;
    ...
    if ((st = sdmmc_.SdioIoRwDirect(false, 0, SDIO_CIA_CCCR_IOEx_EN_FUNC_ADDR, 0, &ioex_reg)) !=
        ZX_OK) {
    ...
    st = sdmmc_.SdioIoRwDirect(true, 0, SDIO_CIA_CCCR_IOEx_EN_FUNC_ADDR, ioex_reg, nullptr);
    if (st != ZX_OK) {
        zxlogf(ERROR, "sdio_enable_function: Error enabling func:%d status:%d\n", fn_idx, st);
        return st;
    }
...
zx_status_t SdioDevice::SdioDisableFn(uint8_t fn_idx) {
    uint8_t ioex_reg = 0;
    zx_status_t st = ZX_OK;
...
zx_status_t SdioDevice::SdioEnableFnIntr(uint8_t fn_idx) {
    zx_status_t st = ZX_OK;
    ...
    uint8_t intr_byte;
    st = sdmmc_.SdioIoRwDirect(false, 0, SDIO_CIA_CCCR_IEN_INTR_EN_ADDR, 0, &intr_byte);
    if (st != ZX_OK) {
    ...
    st = sdmmc_.SdioIoRwDirect(true, 0, SDIO_CIA_CCCR_IEN_INTR_EN_ADDR, intr_byte, nullptr);
    if (st != ZX_OK) {
        zxlogf(ERROR, "sdio_enable_interrupt: Failed to enable interrupt for fn: %d status: %d\n",
               fn_idx, st);
        return st;
...
zx_status_t SdioDevice::SdioDisableFnIntr(uint8_t fn_idx) {
    zx_status_t st = ZX_OK;
...
zx_status_t SdioDevice::SdioUpdateBlockSize(uint8_t fn_idx, uint16_t blk_sz, bool deflt) {
    zx_status_t st = ZX_OK;
...
zx_status_t SdioDevice::SdioGetBlockSize(uint8_t fn_idx, uint16_t* out_cur_blk_size) {
...
zx_status_t SdioDevice::SdioDoRwTxn(uint8_t fn_idx, sdio_rw_txn_t* txn) {
    if (!SdioFnIdxValid(fn_idx)) {
        return ZX_ERR_INVALID_ARGS;
    }
    ...
    while (rem_blocks > 0) {
        ...
        if (mbs) {
            ...
            max_host_blocks = static_cast<uint32_t>(
                use_dma ? (sdmmc_.host_info().max_transfer_size / func_blk_size)
                        : (sdmmc_.host_info().max_transfer_size_non_dma / func_blk_size));
...
zx_status_t SdioDevice::SdioDoRwByte(bool write, uint8_t fn_idx, uint32_t addr, uint8_t write_byte,
...
zx_status_t SdioDevice::SdioGetInBandIntr(zx::interrupt* out_irq) {
    return sdmmc_.host().GetInBandInterrupt(out_irq);
}

zx_status_t SdioDevice::SdioReset() {
    zx_status_t st = ZX_OK;
    uint8_t abort_byte;

    st = sdmmc_.SdioIoRwDirect(false, 0, SDIO_CIA_CCCR_ASx_ABORT_SEL_CR_ADDR, 0, &abort_byte);
    if (st != ZX_OK) {
        abort_byte = SDIO_CIA_CCCR_ASx_ABORT_SOFT_RESET;
    } else {
        abort_byte |= SDIO_CIA_CCCR_ASx_ABORT_SOFT_RESET;
    }
    return sdmmc_.SdioIoRwDirect(true, 0, SDIO_CIA_CCCR_ASx_ABORT_SEL_CR_ADDR, abort_byte, nullptr);
}

zx_status_t SdioDevice::ProcessCccr() {
    uint8_t cccr_vsn, sdio_vsn, vsn_info, bus_speed, card_caps, uhs_caps, drv_strength;

    // version info
    zx_status_t status =
        sdmmc_.SdioIoRwDirect(false, 0, SDIO_CIA_CCCR_CCCR_SDIO_VER_ADDR, 0, &vsn_info);
    if (status != ZX_OK) {
    ...
    // card capabilities
    status = sdmmc_.SdioIoRwDirect(false, 0, SDIO_CIA_CCCR_CARD_CAPS_ADDR, 0, &card_caps);
    if (status != ZX_OK) {
    ...
    // speed
    status = sdmmc_.SdioIoRwDirect(false, 0, SDIO_CIA_CCCR_BUS_SPEED_SEL_ADDR, 0, &bus_speed);
    if (status != ZX_OK) {
    ...
    // Is UHS supported?
    status = sdmmc_.SdioIoRwDirect(false, 0, SDIO_CIA_CCCR_UHS_SUPPORT_ADDR, 0, &uhs_caps);
    if (status != ZX_OK) {
        zxlogf(ERROR, "sdio_process_cccr: Error reading SPEED reg: %d\n", status);
        return status;
    }
    ...
    // drv_strength
    status = sdmmc_.SdioIoRwDirect(false, 0, SDIO_CIA_CCCR_DRV_STRENGTH_ADDR, 0, &drv_strength);
    if (status != ZX_OK) {
        zxlogf(ERROR, "sdio_process_cccr: Error reading SPEED reg: %d\n", status);
        return status;
    }
...
zx_status_t SdioDevice::ProcessCis(uint8_t fn_idx) {
    zx_status_t st = ZX_OK;
    ...
    while (true) {
        ...
        st = sdmmc_.SdioIoRwDirect(false, 0, cis_ptr + SDIO_CIS_TPL_FRMT_TCODE_OFF, 0, &tuple_code);
        if (st != ZX_OK) {
        ...
        st = sdmmc_.SdioIoRwDirect(false, 0, cis_ptr + SDIO_CIS_TPL_FRMT_TLINK_OFF, 0, &tuple_link);
        if (st != ZX_OK) {
        ...
        for (size_t i = 0; i < tuple_link; i++, cis_ptr++) {
            st = sdmmc_.SdioIoRwDirect(false, 0, cis_ptr, 0, &cur_tup.tuple_body[i]);
            if (st != ZX_OK) {
...
zx_status_t SdioDevice::ParseFnTuple(uint8_t fn_idx, const SdioFuncTuple& tup) {
    zx_status_t st = ZX_OK;
...
zx_status_t SdioDevice::ParseFuncExtTuple(uint8_t fn_idx, const SdioFuncTuple& tup) {
    SdioFunction* func = &funcs_[fn_idx];
    if (fn_idx == 0) {
...
zx_status_t SdioDevice::ParseMfidTuple(uint8_t fn_idx, const SdioFuncTuple& tup) {
...
zx_status_t SdioDevice::ProcessFbr(uint8_t fn_idx) {
    zx_status_t st = ZX_OK;
    uint8_t fbr, fn_intf_code;

    SdioFunction* func = &funcs_[fn_idx];
    if ((st = sdmmc_.SdioIoRwDirect(false, 0,
...
zx_status_t SdioDevice::InitFunc(uint8_t fn_idx) {
    zx_status_t st = ZX_OK;
...
zx_status_t SdioDevice::SwitchFreq(uint32_t new_freq) {
    zx_status_t st;
...
zx_status_t SdioDevice::TrySwitchHs() {
    ...
    st = sdmmc_.SdioIoRwDirect(false, 0, SDIO_CIA_CCCR_BUS_SPEED_SEL_ADDR, 0, &speed);
    if (st != ZX_OK) {
    ...
    st = sdmmc_.SdioIoRwDirect(true, 0, SDIO_CIA_CCCR_BUS_SPEED_SEL_ADDR, speed, nullptr);
    if (st != ZX_OK) {
...
zx_status_t SdioDevice::TrySwitchUhs() {
...
zx_status_t SdioDevice::Enable4BitBus() {
    zx_status_t st = ZX_OK;
    ...
    if ((st = sdmmc_.SdioIoRwDirect(false, 0, SDIO_CIA_CCCR_BUS_INTF_CTRL_ADDR, 0,
                                    &bus_ctrl_reg)) != ZX_OK) {
    ...
    UpdateBitsU8(&bus_ctrl_reg, SDIO_CIA_CCCR_INTF_CTRL_BW_MASK, SDIO_CIA_CCCR_INTF_CTRL_BW_LOC,
                 SDIO_BW_4BIT);
    if ((st = sdmmc_.SdioIoRwDirect(true, 0, SDIO_CIA_CCCR_BUS_INTF_CTRL_ADDR, bus_ctrl_reg,
                                    nullptr)) != ZX_OK) {
...
zx_status_t SdioDevice::SwitchBusWidth(uint32_t bw) {
    zx_status_t st = ZX_OK;
...
zx_status_t SdioDevice::ReadData16(uint8_t fn_idx, uint32_t addr, uint16_t* word) {
...
zx_status_t SdioDevice::WriteData16(uint8_t fn_idx, uint32_t addr, uint16_t word) {
