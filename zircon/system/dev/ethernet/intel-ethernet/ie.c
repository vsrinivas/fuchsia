...
void eth_dump_regs(ethdev_t* eth) {
  printf("STAT %08x CTRL %08x EXT %08x IMS %08x\n", readl(IE_STATUS), readl(IE_CTRL),
         readl(IE_CTRL_EXT), readl(IE_IMS));
  printf("RCTL %08x RDLN %08x RDH %08x RDT %08x\n", readl(IE_RCTL), readl(IE_RDLEN), readl(IE_RDH),
         readl(IE_RDT));
  printf("RXDC %08x RDTR %08x RBH %08x RBL %08x\n", readl(IE_RXDCTL), readl(IE_RDTR),
         readl(IE_RDBAH), readl(IE_RDBAL));
  printf("TCTL %08x TDLN %08x TDH %08x TDT %08x\n", readl(IE_TCTL), readl(IE_TDLEN), readl(IE_TDH),
         readl(IE_TDT));
  printf("TXDC %08x TIDV %08x TBH %08x TBL %08x\n", readl(IE_TXDCTL), readl(IE_TIDV),
         readl(IE_TDBAH), readl(IE_TDBAL));
}
...
bool eth_status_online(ethdev_t* eth) { return readl(IE_STATUS) & IE_STATUS_LU; }
...
static zx_status_t phy_read(ethdev_t* eth, uint8_t phyadd, uint8_t regadd, uint16_t* result) {
  uint32_t mdic = IE_MDIC_PUT_PHYADD(phyadd) | IE_MDIC_PUT_REGADD(regadd) | IE_MDIC_OP_READ;
  writel(mdic, IE_MDIC);
...
static zx_status_t phy_write(ethdev_t* eth, uint8_t phyadd, uint8_t regadd, uint16_t value) {
  uint32_t mdic = IE_MDIC_PUT_DATA(value) | IE_MDIC_PUT_PHYADD(phyadd) |
                  IE_MDIC_PUT_REGADD(regadd) | IE_MDIC_OP_WRITE;
  writel(mdic, IE_MDIC);
...
void eth_init_hw(ethdev_t* eth) {
  ...
  writel(IE_RCTL_BSIZE2048 | IE_RCTL_DPF | IE_RCTL_SECRC | IE_RCTL_BAM | IE_RCTL_MPE | IE_RCTL_EN,
         IE_RCTL);
