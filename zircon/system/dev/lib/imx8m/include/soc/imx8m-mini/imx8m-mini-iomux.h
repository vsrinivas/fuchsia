 ...
 *   GPIO PAD-ctl options
 * [6:4]        :   DSE: Drive Strength
 * [8:7]        :   FSEL: Slew Rate Field
 * [9]          :   ODE: Open Drain Enable Field
 * [10]         :   PUE: Control IO port PS Field
 * [11]         :   HYS: Hysteresis Enable Field
 * [12]         :   PE: Pull Resistors Enable Field
 * [15:13]      :   RSVD: Future Use
 *   Daisy-ctl options
...
#define FSEL_START            7
#define FSEL_COUNT            2
#define SET_FSEL_VAL(x)   \
                        ((x & ((1 << FSEL_COUNT) - 1)) << FSEL_START)
#define GET_FSEL_VAL(x)   \
                        ((x >> FSEL_START) & ((1 << FSEL_COUNT) - 1))
...
#define PE_COUNT              1
#define SET_PE_VAL(x)     \
                        ((x & ((1 << PE_COUNT) - 1)) << PE_START)
#define GET_PE_VAL(x)     \
                        ((x >> PE_START) & ((1 << PE_COUNT) - 1))
...
#define MAKE_PIN_CFG(mux_mode, sion, dse, fsel, ode, pue, hys, pe, daisy, \
...
#define MAKE_PIN_CFG_UART(mux_mode, mux_ctl_off, pad_ctl_off, sel_inp_off) \
            MAKE_PIN_CFG(mux_mode, 0, DSR_X1, SRE_SLOW, 0, 0, 0, 0, 0, \
                mux_ctl_off, pad_ctl_off, sel_inp_off)

#define MAKE_PIN_CFG_DEFAULT(mux_mode, mux_ctl_off) \
            MAKE_PIN_CFG(mux_mode, 0, 0, 0, 0, 0, 0, 0, 0, \
                mux_ctl_off, 0x000ULL, 0x000ULL)

/* IMX8M-Mini IOMUX Register Offsets */
...
#define IOMUX_CFG_FSEL_START            3
#define IOMUX_CFG_FSEL_COUNT            2
#define IOMUX_CFG_FSEL_VAL(x)    \
                        ((x & ((1 << IOMUX_CFG_FSEL_COUNT) - 1)) << IOMUX_CFG_FSEL_START)
...
#define IOMUX_CFG_PE_START              8
#define IOMUX_CFG_PE_COUNT              1
#define IOMUX_CFG_PE_VAL(x)    \
                        ((x & ((1 << IOMUX_CFG_PE_COUNT) - 1)) << IOMUX_CFG_PE_START)
...
/* Available UART Drive Strengths (lsb of this field not used) */
#define DSR_X1                  (0x000)
#define DSR_X2                  (0x100)
#define DSR_X4                  (0x010)
#define DSR_X8                  (0x110)
...
/* Slew-rates */
#define FSEL_SLOW                (0x00)
#define FSEL_FAST                (0x01)
