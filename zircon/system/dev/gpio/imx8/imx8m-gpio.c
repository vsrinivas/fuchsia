...
static zx_status_t imx8m_gpio_set_alt_function(void* ctx, const uint32_t pin, const uint64_t fn) {
...
static gpio_impl_protocol_ops_t gpio_ops = {
    ...
    .set_alt_function = imx8m_gpio_set_alt_function,
    .read = imx8_gpio_read,
...
static void imx8m_gpio_release(void* ctx) {
    unsigned i;
    imx8_gpio_t* gpio = ctx;
...
static zx_protocol_device_t gpio_device_proto = {
    ...
    .release = imx8m_gpio_release,
};

static zx_status_t imx8m_gpio_bind(void* ctx, zx_device_t* parent) {
    zx_status_t status;
    unsigned i;
...
fail:
    imx8m_gpio_release(gpio);
    return status;
}

static zx_driver_ops_t imx8m_gpio_driver_ops = {
    .version = DRIVER_OPS_VERSION,
    .bind = imx8m_gpio_bind,
};

ZIRCON_DRIVER_BEGIN(imx8m_gpio, imx8m_gpio_driver_ops, "zircon", "0.1", 4)
