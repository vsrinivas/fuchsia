...
static zx_status_t console_write(void* ctx, const void* buf, size_t count, zx_off_t off,
                                 size_t* actual) {
  const void* ptr = buf;
...
static zx_status_t console_OpenClient(void* ctx, uint32_t id, zx_handle_t handle, fidl_txn_t* txn) {
...
static fuchsia_hardware_pty_Device_ops_t fidl_ops = {.OpenClient = console_OpenClient,
...
static zx_protocol_device_t console_device_proto = {.version = DEVICE_OPS_VERSION,
                                                    .read = console_read,
                                                    .write = console_write,
...
BI_MATCH_IF(EQ, BIND_PROTOCOL, ZX_PROTOCOL_MISC_PARENT), ZIRCON_DRIVER_END(console)
