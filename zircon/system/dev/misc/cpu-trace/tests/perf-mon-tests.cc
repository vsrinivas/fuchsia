...
class Perfmon : public zxtest::Test {
 public:
  void SetUp() override {
    ...
    device_.reset(new perfmon::PerfmonDevice(nullptr, std::move(bti), props,
                                             mtrace_control_always_ok));
  }
...
#error "unsupported arch"
  ...
  config.events[0].flags |= fuchsia_perfmon_cpu_EventConfigFlags_COLLECT_OS;
