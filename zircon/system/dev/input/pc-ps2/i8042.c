...
static const uint8_t kbd_hid_report_desc[] = {
    ...
    0x03,  //   Report Size (3)
    0x91,
    0x01,  //   Output (Const,Array,Abs,No Wrap,Linear,Preferred State,No Null
...
static const hid_boot_kbd_report_t report_err_rollover = {.modifier = 1,
                                                          .usage = {1, 1, 1, 1, 1, 1}};
...
static void i8042_identify(int (*cmd)(uint8_t* param, int command)) {
  uint8_t resp[3];
  if (cmd(resp, I8042_CMD_SCAN_DIS) < 0)
    return;
  resp[0] = 0;
...
static zx_status_t i8042_set_idle(void* ctx, uint8_t rpt_type, uint8_t duration) { return ZX_OK; }

static zx_status_t i8042_get_protocol(void* ctx, uint8_t* protocol) { return ZX_ERR_NOT_SUPPORTED; }

static zx_status_t i8042_set_protocol(void* ctx, uint8_t protocol) { return ZX_OK; }
...
static zx_status_t i8042_dev_init(i8042_device_t* dev, const char* name, zx_device_t* parent) {
  ...
  // Please do not use get_root_resource() in new code. See ZX-1467.
  zx_status_t status =
      zx_interrupt_create(get_root_resource(), interrupt, ZX_INTERRUPT_REMAP_IRQ, &dev->irq);
  if (status != ZX_OK) {
    return status;
  }
