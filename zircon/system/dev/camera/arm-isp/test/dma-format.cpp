...
void TestPixelType(DmaFormat::PixelType pixel_type) {
    ...
    // Because of line and page alignment, the size should be >= width * height * bytesperpixel
    ...
    // Line offset should be different, because flipped images start in different locations
...
TEST(DmaFormat, CreateWithoutSysmemInvalid) {
    ...
    ASSERT_DEATH(([]() { DmaFormat(kTestWidth, kTestHeight,
                                   DmaFormat::PixelType::NV12, false); }));
    ASSERT_DEATH(([]() { DmaFormat(kTestWidth, kTestHeight,
                                   DmaFormat::PixelType::YV12, false); }));
    ASSERT_DEATH(([]() { DmaFormat(kTestWidth, kTestHeight,
                                   DmaFormat::PixelType::INVALID, false); }));
}
