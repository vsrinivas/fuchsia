...
DmaFormat::DmaFormat(uint32_t width, uint32_t height, PixelType pixel_format, bool flip_vertical)
    : width_(width), height_(height), flip_vertical_(flip_vertical),
      // Use the "base_mode" bits as the pixel format, and plane_select as secondary_plane_select_
    ...
    // Disallow the NV12 and YV12 types in the constructor; they are only used internally.
    ZX_ASSERT(pixel_format != PixelType::NV12);
    ZX_ASSERT(pixel_format != PixelType::YV12);
}

uint8_t DmaFormat::GetPlaneSelect() const {
    return 0;
}
...
uint8_t DmaFormat::GetBaseMode() const {
    return base_mode_;
}
