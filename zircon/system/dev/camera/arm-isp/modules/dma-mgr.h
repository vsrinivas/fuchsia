...
class DmaManager {
public:
    enum class Stream : bool { FullResolution = false,
                               Downscaled };
    ...
    // with a different buffer collection.  However, doing so will remove all knowledge of
    // the locked status of frames of the previous BufferCollection.
    // This has the following effects:
    //  - Frames that are currently being written will be dropped.  Calls to On*FrameWritten()
    ...
    //  - ReleaseFrame calls with currently used indices (relating to the old BufferCollection)
    //    will return errors.
    zx_status_t Start(
...
private:
    ...
    void WriteFormat();
    // Releases the write lock on the frame and calls the frame_available_callback.
    void OnFrameWritten();
};
