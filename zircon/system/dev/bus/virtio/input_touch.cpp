...
zx_status_t HidTouch::GetDescriptor(uint8_t desc_type, void** data, size_t* len) {
...
void HidTouch::ReceiveEvent(virtio_input_event_t* event) {
    if (event->type == VIRTIO_INPUT_EV_ABS) {
        ...
        if (event->code == VIRTIO_INPUT_EV_MT_TRACKING_ID) {
            ...
            report_.fingers[mt_slot_].x = static_cast<uint16_t>(event->value * PARADISE_X_MAX / x_info_.max);
        } else if (event->code == VIRTIO_INPUT_EV_MT_POSITION_Y) {
            report_.fingers[mt_slot_].y = static_cast<uint16_t>(event->value * PARADISE_Y_MAX / y_info_.max);
        }
    }
}
