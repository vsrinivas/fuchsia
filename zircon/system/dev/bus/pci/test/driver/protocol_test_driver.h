...
public:
    void OnTestStart(const zxtest::TestCase&, const zxtest::TestInfo&) final {
        report_.test_count++;
    }
...
class ProtocolTestDriver : public ProtocolTestDriverType,
                           public TestObserver {
public:
...
private:
    ProtocolTestDriver(zx_device_t* parent)
        : ProtocolTestDriverType(parent), pci_(parent) {}
...
protected:
    PciProtocolTests()
        : drv_(ProtocolTestDriver::GetInstance()) {}
