...
zx_status_t CompositeDevice::Create(const pbus_dev_t* pdev, zx_device_t* parent, PlatformBus* bus,
  ...
  fbl::unique_ptr<platform_bus::CompositeDevice> dev(
      new (&ac) platform_bus::CompositeDevice(parent, bus, pdev));
  if (!ac.check()) {
...
zx_status_t CompositeDevice::Init(const pbus_dev_t* pdev) { return resources_.Init(pdev); }
...
zx_status_t CompositeDevice::PDevGetInterrupt(uint32_t index, uint32_t flags,
...
void CompositeDevice::DdkRelease() { delete this; }
