...
public:
    Msm8x53PowerTest(ddk_mock::MockMmioRegRegion& core_mmio_registers,
        ...
        : Msm8x53Power(nullptr,
                       ddk::MmioBuffer(core_mmio_registers.GetMmioBuffer()),
    ...
    zx_status_t InvokePmicArbInit() {
        return PmicArbInit();
    }

    uint32_t GetApid(uint32_t ppid) {
        return ppid_to_apid_[ppid];
    }

    void SetApid(uint32_t ppid, uint32_t apid) {
        ppid_to_apid_[ppid] = apid;
    }
};
...
TEST(PowerTest, PmicArbInit) {
    ...
    Msm8x53PowerTest power(mock_core_regs, mock_dummy_regs, mock_dummy_regs,
                           mock_dummy_regs, mock_dummy_regs);
    ...
    mock_core_regs[PMIC_ARB_CORE_CHANNEL_INFO_OFFSET(0)].ExpectRead(PmicArbCoreChannelInfo()
                                                          .set_slave_id(0xe)
                                                          .set_periph_id(0x8).reg_value());
    mock_core_regs[PMIC_ARB_CORE_CHANNEL_INFO_OFFSET(1)].ExpectRead(PmicArbCoreChannelInfo()
                                     .set_slave_id(0x0)
                                     .set_periph_id(0x77).reg_value());
    mock_core_regs[PMIC_ARB_CORE_CHANNEL_INFO_OFFSET(14)].ExpectRead(PmicArbCoreChannelInfo()
                                     .set_slave_id(0x2)
                                     .set_periph_id(0x1b).reg_value());
...
TEST(PowerTest, PmicWritePmicCtrlReg) {
    ...
    ddk_mock::MockMmioReg dummy_reg_array[kDummyRegCount];
    ddk_mock::MockMmioRegRegion mock_dummy_regs(dummy_reg_array, sizeof(uint32_t),
                                                kDummyRegCount);

    Msm8x53PowerTest power(mock_dummy_regs, mock_chnls_regs, mock_obsvr_regs,
                           mock_dummy_regs, mock_dummy_regs);

    uint32_t apid = 0;
    //Disabling the interrupt
    uint32_t cmd_cfg_offset = PMIC_ARB_CHANNEL_CMD_CONFIG_OFFSET(apid);
    mock_chnls_regs[cmd_cfg_offset].ExpectRead(0xFFFFFFFF)
                                   .ExpectWrite(0x0);
    ...
    mock_chnls_regs[PMIC_ARB_CHANNEL_CMD_WDATA0_OFFSET(apid)]
                                    ...
                                    .ExpectWrite(PmicArbCoreChannelCmdWData()
                                                 .set_data(write_value)
                                                 .reg_value());

    // Write the CMD
    uint32_t cmd_offset = PMIC_ARB_CHANNEL_CMD_OFFSET(apid);
    mock_chnls_regs[cmd_offset].ExpectRead(0x00000000)
    ...
    mock_chnls_regs[PMIC_ARB_CHANNEL_CMD_STATUS_OFFSET(apid)]
                            .ExpectRead(PmicArbCoreChannelCmdStatus()
...
TEST(PowerTest, PmicReadPmicCtrlReg) {
    ...
    Msm8x53PowerTest power(mock_dummy_regs, mock_dummy_regs, mock_obsvr_regs,
                           mock_dummy_regs, mock_dummy_regs);

    uint32_t apid = 0;
    //Disabling the interrupt
    uint32_t cmd_cfg_offset = PMIC_ARB_CHANNEL_CMD_CONFIG_OFFSET(apid);
    mock_obsvr_regs[cmd_cfg_offset].ExpectRead(0xFFFFFFFF)
                                   .ExpectWrite(0x0);
    //Set the ppid to apid mapping
    uint32_t slave_id = 0x3;
    uint32_t periph_id = 0xc0;
    ...
    // Write the Read CMD
    uint32_t cmd_offset = PMIC_ARB_CHANNEL_CMD_OFFSET(apid);
    mock_obsvr_regs[cmd_offset].ExpectRead(0x00000000)
                               .ExpectWrite(PmicArbCoreChannelCmdInfo()
                                            .set_byte_cnt(0)
                                            .set_reg_offset_addr(reg_offset)
                                            .set_periph_id(periph_id)
    ...
    mock_obsvr_regs[PMIC_ARB_CHANNEL_CMD_STATUS_OFFSET(apid)]
                            .ExpectRead(PmicArbCoreChannelCmdStatus()
                                        .set_status(PmicArbCoreChannelCmdStatus::kPmicArbCmdDone)
                                        .reg_value());
    // Read first 4 bytes to RDATA0
    mock_obsvr_regs[PMIC_ARB_CHANNEL_CMD_RDATA0_OFFSET(apid)]
                                    .ExpectRead(PmicArbCoreChannelCmdRData()
                                                 .set_data(expected_value)
                                                 .reg_value());
