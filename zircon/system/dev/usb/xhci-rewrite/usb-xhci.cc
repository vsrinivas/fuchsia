...
zx_status_t UsbXhci::Init() {
  ...
  zx_status_t status =
      device_get_profile(zxdev_, /*HIGH_PRIORITY*/ 31, "zircon/system/dev/usb/xhci/usb-xhci",
...
zx_status_t UsbXhci::Create(void* ctx, zx_device_t* parent) {
  ...
  if (dev->composite_.is_valid()) {
    ...
    dev->composite_.GetComponents(components, COUNT, &actual);
    ...
    if (actual > 1) {
      ddk::UsbPhyProtocolClient phy_proto(components[PHY]);
      dev->phy_ = std::move(phy_proto);
    }
    dev->pdev_ = components[PDEV];
