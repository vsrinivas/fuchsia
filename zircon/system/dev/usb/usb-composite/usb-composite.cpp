...
static inline const usb_descriptor_header_t* NextDescriptor(const void* header) {
    ...
    return reinterpret_cast<const usb_descriptor_header_t*>(
                            reinterpret_cast<const uint8_t*>(desc) + desc->bLength);
}
...
zx_status_t UsbComposite::AddInterfaces() {
    ...
    while (header < end) {
        if (header->bDescriptorType == USB_DT_INTERFACE_ASSOCIATION) {
            ...
            size_t length = reinterpret_cast<uintptr_t>(next)
                          - reinterpret_cast<uintptr_t>(assoc_desc);
            ...
            size_t length = reinterpret_cast<uintptr_t>(next)
                          - reinterpret_cast<uintptr_t>(intf_desc);
    ...
    *out_desc_actual = 0;
    ...
    while (header < end) {
        if (header->bDescriptorType == USB_DT_INTERFACE) {
            ...
            if (test_intf->bAlternateSetting == 0 &&
                test_intf->bInterfaceNumber > last_interface_id) {
                result = test_intf;
                break;
            }
        }
