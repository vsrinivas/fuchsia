...
zx_status_t UsbBus::UsbBusInterfaceReinitializeDevice(uint32_t device_id) {
    ...
    auto status = device->GetDescriptor(USB_DT_DEVICE, 0, 0, &updated_desc, sizeof(updated_desc),
                                        &actual);
    ...
    if (status == ZX_OK) {
        ...
        bool descriptors_changed = memcmp(&old_desc, &updated_desc,
        ...
        if (descriptors_changed) {
            zxlogf(INFO, "device updated from VID 0x%x PID 0x%x to VID 0x%x PID 0x%x\n",
                   old_desc.idVendor, old_desc.idProduct, updated_desc.idVendor,
                   updated_desc.idProduct);
