...
class AmlVoltageRegulator {
 public:
  ...
  // For testing
  zx_status_t Init(ddk::MmioBuffer pwm_AO_D_mmio, const pwm_protocol_t* pwm_AO_D,
 ...
 private:
  ...
  template <typename T>
  zx_status_t SetClusterVoltage(int* current_voltage_index, T* pwm, uint32_t microvolt);
  zx_status_t SetBigClusterVoltage(uint32_t microvolt) {
    if (pid_ == PDEV_PID_AMLOGIC_S905D2) {
      // Astro
      return SetClusterVoltage<AmlPwm>(&current_big_cluster_voltage_index_, &pwm_AO_D_astro_,
                                       microvolt);
    }
    if (pid_ == PDEV_PID_AMLOGIC_T931) {
      // Sherlock
      return SetClusterVoltage<ddk::PwmProtocolClient>(&current_big_cluster_voltage_index_, &pwm_A_,
                                                       microvolt);
    }
  ...
  zx_status_t SetLittleClusterVoltage(uint32_t microvolt) {
    return SetClusterVoltage<ddk::PwmProtocolClient>(&current_little_cluster_voltage_index_,
                                                     &pwm_AO_D_, microvolt);
  }

  AmlPwm pwm_AO_D_astro_;  // TODO (rdzhuang): migrate astro to PWM driver as well
