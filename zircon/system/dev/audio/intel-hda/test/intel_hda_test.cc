...
bool IntelHdaDevicePresent() {
    ...
    return FileExists(kIntelHdaDefaultDevice) &&
           FileExists(kIntelHdaInputCodecPath) &&
...
std::string_view
StringResponseAsStringView(audio_stream_cmd_get_string_resp_t* response) {
    if (response->strlen > sizeof(response->str)) {
        return "";
    }
    return std::string_view(reinterpret_cast<const char*>(response->str),
                            response->strlen);
}
