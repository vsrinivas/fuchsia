...
zx_status_t TdmOutputStream::Create(zx_device_t* parent) {
  ...
  auto stream = fbl::AdoptRef(new TdmOutputStream(parent, std::move(domain)));
...
void TdmOutputStream::ReleaseRingBufferLocked() { io_buffer_release(&ring_buffer_); }
...
void TdmOutputStream::DdkUnbindNew(ddk::UnbindTxn txn) {
...
zx_status_t TdmOutputStream::OnGetStreamFormatsLocked(
    dispatcher::Channel* channel, const audio_proto::StreamGetFmtsReq& req) const {
  ZX_DEBUG_ASSERT(channel != nullptr);
...
zx_status_t TdmOutputStream::OnGetFifoDepthLocked(
    dispatcher::Channel* channel, const audio_proto::RingBufGetFifoDepthReq& req) const {
...
extern "C" void gauss_tdm_release(void*) { dispatcher::ThreadPool::ShutdownAll(); }
