...
FakeNandInterface::FakeNandInterface(uint32_t page_size, uint32_t oob_size, uint32_t block_size,
                                     uint32_t size)
    : read_actual_(page_size),
      fail_read_(false),
      fail_write_(false),
      fail_erase_(false),
      fail_is_bad_block_(false),
...
uint32_t FakeNandInterface::PageSize() { return page_size_; }
uint32_t FakeNandInterface::BlockSize() { return block_size_; }
uint32_t FakeNandInterface::OobSize() { return oob_size_; }
uint32_t FakeNandInterface::Size() { return size_; }
