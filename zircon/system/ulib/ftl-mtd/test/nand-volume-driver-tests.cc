...
class NandVolumeDriverTest : public zxtest::Test {
 protected:
  void SetUpDriver(uint32_t block_offset, uint32_t group_size, uint32_t oob_size) {
    ...
    read_page_buffer_ = std::make_unique<uint8_t[]>(group_size_ * kPageSize * page_multiplier_);
    read_oob_buffer_ = std::make_unique<uint8_t[]>(group_size_ * oob_size_ * page_multiplier_);

    write_page_buffer_ = std::make_unique<uint8_t[]>(group_size_ * kPageSize * page_multiplier_);
  ...
  uint32_t PageBufferSize() { return group_size_ * kPageSize * page_multiplier_; }

  uint32_t OobBufferSize() { return group_size_ * oob_size_ * page_multiplier_; }
...
TEST_F(NandVolumeDriverTest, WriteAllSucceeds) {
  ...
  for (uint32_t page = 0; page < num_pages; page += group_size) {
    ASSERT_EQ(ftl::kNdmOk,
              nand_volume_driver_->NandWrite(page, group_size, write_page_buffer_.get(),
...
TEST_F(NandVolumeDriverTest, BadWriteReportsError) {
  ...
  ASSERT_EQ(ftl::kNdmFatalError, nand_volume_driver_->NandWrite(kSize, 1, write_page_buffer_.get(),
                                                                write_oob_buffer_.get()));
...
TEST_F(NandVolumeDriverTest, ReadAllSucceeds) {
  ...
  for (uint32_t page = 0; page < num_pages; page += group_size) {
    ...
    ASSERT_EQ(ftl::kNdmOk, nand_volume_driver_->NandRead(page, group_size, read_page_buffer_.get(),
...
TEST_F(NandVolumeDriverTest, BadReadReportsFatalError) {
  ...
  ASSERT_EQ(ftl::kNdmFatalError, nand_volume_driver_->NandRead(kSize, 1, read_page_buffer_.get(),
                                                               read_oob_buffer_.get()));
