...
const zx_port_packet_t dummy_exception{
    ...
    .type = 0,
    .status = ZX_OK,
    .exception = {.pid = dummy_pid, .tid = dummy_tid, .reserved0 = 0u, .reserved1 = 0u},
};
...
class Harness {
 public:
  ...
  void Handler(async_dispatcher_t* dispatcher, async::ExceptionBase* exception, zx_status_t status,
...
class LambdaHarness : public Harness {
 public:
  LambdaHarness(zx_handle_t task = ZX_HANDLE_INVALID, uint32_t options = 0)
...
class MethodHarness : public Harness {
 public:
  MethodHarness(zx_handle_t task = ZX_HANDLE_INVALID, uint32_t options = 0)
...
bool exception_run_handler_test() {
...

  {
    ...
    harness.Reset();
    dispatcher.last_exception->handler(&dispatcher, dispatcher.last_exception, ZX_OK,
                                       &dummy_exception);
...
bool unsupported_bind_test() {
  ...
  EXPECT_EQ(ZX_ERR_NOT_SUPPORTED, async_bind_exception_port(&dispatcher, &exception), "valid args");
...
bool unsupported_unbind_test() {
  ...
  async::DispatcherStub dispatcher;
  async_exception_t exception{};
  EXPECT_EQ(ZX_ERR_NOT_SUPPORTED, async_unbind_exception_port(&dispatcher, &exception),
            "valid args");
...
bool unsupported_resume_test() {
  ...
  EXPECT_EQ(ZX_ERR_NOT_SUPPORTED,
            async_resume_from_exception(&dispatcher, &exception, ZX_HANDLE_INVALID, 0u),
            "valid args");
