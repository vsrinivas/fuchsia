...
ExceptionBase::ExceptionBase(zx_handle_t task, uint32_t options, async_exception_handler_t* handler)
...
Exception::Exception(zx_handle_t task, uint32_t options, Handler handler)
    : ExceptionBase(task, options, &Exception::CallHandler), handler_(std::move(handler)) {}

Exception::~Exception() = default;

void Exception::CallHandler(async_dispatcher_t* dispatcher, async_exception_t* exception,
