...
class TestEnvironment : public TestEnvironmentSpecialized<TestEnvTraits> {
 public:
  ...
  void NodeState() {
    // Test copy/move consturctors and assignment operators when node is not in
    // a container.
    {
      // Default construction
      typename ObjType::ContainerStateType ns;
      ASSERT_TRUE(ns.IsValid());
      ASSERT_FALSE(ns.InContainer());

      // Copy construction
      {
        typename ObjType::ContainerStateType ns2(ns);
        ASSERT_TRUE(ns.IsValid());
        ASSERT_FALSE(ns.InContainer());
        ASSERT_TRUE(ns2.IsValid());
        ASSERT_FALSE(ns2.InContainer());
      }

      // Move construction
      {
        typename ObjType::ContainerStateType ns2(std::move(ns));
        ASSERT_TRUE(ns.IsValid());
        ASSERT_FALSE(ns.InContainer());
        ASSERT_TRUE(ns2.IsValid());
        ASSERT_FALSE(ns2.InContainer());
      ...
      // Copy assignment
      {
        typename ObjType::ContainerStateType ns2;
        ns2 = ns;
        ASSERT_TRUE(ns.IsValid());
        ASSERT_FALSE(ns.InContainer());
        ASSERT_TRUE(ns2.IsValid());
    ...
    // Test copy/move constructors and assignment operators when node is in a
    // container.
    {
      ASSERT_NO_FAILURES(Populate(container()));
      auto& ns_in_container = NodeTraits::node_state(*container().begin());
      ASSERT_TRUE(ns_in_container.IsValid());
      ASSERT_TRUE(ns_in_container.InContainer());

      // Copy construction, in container
      {
        auto lambda = [&]() {
          typename ObjType::ContainerStateType ns2(ns_in_container);
          ASSERT_TRUE(ns_in_container.IsValid());
          ASSERT_TRUE(ns_in_container.InContainer());
          ASSERT_TRUE(ns2.IsValid());
          ASSERT_FALSE(ns2.InContainer());
        ...
        if (ZX_DEBUG_ASSERT_IMPLEMENTED) {
          ...
          ASSERT_NO_FAILURES(lambda());
        }
      }

      // Move construction, in container
      {
        auto lambda = [&]() {
          typename ObjType::ContainerStateType ns2(std::move(ns_in_container));
          ASSERT_TRUE(ns_in_container.IsValid());
          ASSERT_TRUE(ns_in_container.InContainer());
          ASSERT_TRUE(ns2.IsValid());
          ASSERT_FALSE(ns2.InContainer());
      ...
      // Copy assignment, lhs in container
      {
        auto lambda = [&]() {
          typename ObjType::ContainerStateType ns_rhs;
          ns_in_container = ns_rhs;
          ASSERT_TRUE(ns_in_container.IsValid());
          ASSERT_TRUE(ns_in_container.InContainer());
          ASSERT_TRUE(ns_rhs.IsValid());
          ASSERT_FALSE(ns_rhs.InContainer());
        };
        if (ZX_DEBUG_ASSERT_IMPLEMENTED) {
          ASSERT_DEATH(lambda);
        } else {
      ...
      // Copy assignment, rhs in container
      {
        auto lambda = [&]() {
          typename ObjType::ContainerStateType ns_lhs;
          ns_lhs = ns_in_container;
          ASSERT_TRUE(ns_lhs.IsValid());
          ASSERT_FALSE(ns_lhs.InContainer());
          ASSERT_TRUE(ns_in_container.IsValid());
          ASSERT_TRUE(ns_in_container.InContainer());
        };
        if (ZX_DEBUG_ASSERT_IMPLEMENTED) {
      ...
      // Move assignment, lhs in container
      {
        auto lambda = [&]() {
          typename ObjType::ContainerStateType ns_rhs;
          ns_in_container = std::move(ns_rhs);
          ASSERT_TRUE(ns_in_container.IsValid());
          ASSERT_TRUE(ns_in_container.InContainer());
          ASSERT_TRUE(ns_rhs.IsValid());
          ASSERT_FALSE(ns_rhs.InContainer());
      ...
      // Move assignment, rhs in container
      {
        auto lambda = [&]() {
          typename ObjType::ContainerStateType ns_lhs;
          ns_lhs = std::move(ns_in_container);
          ASSERT_TRUE(ns_lhs.IsValid());
          ASSERT_FALSE(ns_lhs.InContainer());
          ASSERT_TRUE(ns_in_container.IsValid());
          ASSERT_TRUE(ns_in_container.InContainer());
