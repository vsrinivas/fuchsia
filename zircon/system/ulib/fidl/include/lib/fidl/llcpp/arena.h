...
// Class common to all the Arena classes. This one is independent of the initial buffer
// size.
// All the implementation is done here. The only specialization available is Arena which
// defines the initial buffer size.
// The arena owns all the data which are allocated. That means that the allocated data can be
// used by pure views.
// The allocated data are freed when the arena is freed.
// The methods of the arena are not directly called by the user. Instead ObjectView, StringView
// and VectorView use them.
// The allocation is done within the initial buffer. When the initial buffer is full (or, at least,
// the next allocation doesn't fit in the remaining space), the arena allocates extra buffers
...
class AnyArena {
 private:
  ...
  // Deallocate anything allocated by the arena. Any data previously allocated must not be
  // accessed anymore.
  void Clean();

  // Deallocate anything allocated by the arena. After this call, the arena is in the
  ...
  // during the arena destruction if |destructor_function| is not null.
...
// Class which supports arena allocation of data for the views (ObjectView, StringView, VectorView).
