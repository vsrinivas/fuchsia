...
zx_status_t fidl_socket_write_control(zx_handle_t socket, const void* buffer, size_t capacity) {
  for (;;) {
    zx_status_t status = zx_socket_write(socket, ZX_SOCKET_CONTROL, buffer, capacity, nullptr);
...
zx_status_t fidl_socket_read_control(zx_handle_t socket, void* buffer, size_t capacity,
                                     size_t* out_actual) {
  for (;;) {
    zx_status_t status = zx_socket_read(socket, ZX_SOCKET_CONTROL, buffer, capacity, out_actual);
    if (status != ZX_ERR_SHOULD_WAIT) {
      return status;
    }
