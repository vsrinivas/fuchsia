...
public:
    ...
    zx_status_t create_vmo(uint32_t options, const port& port, uint64_t key,
                           uint64_t size, vmo* result) const {
        return zx_pager_create_vmo(get(), options, port.get(), key, size,
                                   result->reset_and_get_address());
    }

    zx_status_t detach_vmo(const vmo& vmo) const {
        return zx_pager_detach_vmo(get(), vmo.get());
    }

    zx_status_t supply_pages(const vmo& pager_vmo,
                             uint64_t offset, uint64_t length,
                             const vmo& aux_vmo, uint64_t aux_offset) const {
        return zx_pager_supply_pages(get(), pager_vmo.get(), offset, length,
                                     aux_vmo.get(), aux_offset);
    }
};
