...
constexpr int8_t GetLevels(size_t id_count) {
    return log64_int8_ceil(id_count);
}
...
void IdAllocator::MarkBusy(size_t id) {
    MarkBusyInternal(id, 0);
}
...
void IdAllocator::MarkFree(size_t id) {
    MarkFreeInternal(id, 0);
}
...
zx_status_t IdAllocator::Create(size_t id_count,
