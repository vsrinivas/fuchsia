...
static zx_status_t read_file_into_vmo(fdio_t* io, zx_handle_t* out_vmo) {
  ...
  while (size > 0) {
    if (size < MIN_WINDOW) {
      ...
      uintptr_t start = 0;
      status = zx_vmar_map(current_vmar_handle, ZX_VM_PERM_READ | ZX_VM_PERM_WRITE, 0, *out_vmo,
                           offset, window, &start);
      if (status != ZX_OK) {
        zx_handle_close(*out_vmo);
        return status;
...
static zx_status_t copy_file_vmo(fdio_t* io, zx_handle_t* out_vmo) {
  ...
  if ((status = read_file_into_vmo(io, &vmo)) == ZX_OK) {
    status = zx_handle_replace(
        vmo, ZX_RIGHTS_BASIC | ZX_RIGHTS_PROPERTY | ZX_RIGHT_READ | ZX_RIGHT_MAP, out_vmo);
  }
  return status;
}
...
zx_status_t fdio_get_vmo_exact(int fd, zx_handle_t* out_vmo) {
  ...
  zx_status_t status = fdio_get_ops(io)->get_vmo(
      io, fuchsia::io::VMO_FLAG_READ | fuchsia::io::VMO_FLAG_EXACT, out_vmo);
  fdio_release(io);
  return status;
}
