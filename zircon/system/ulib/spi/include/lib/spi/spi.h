...
static inline zx_status_t spi_transmit(zx_handle_t channel, const void* data, size_t length) {
    zx_status_t status;
    zx_status_t call_status = fuchsia_hardware_spi_DeviceTransmit(channel, (const uint8_t *)data,
                                                                  length, &status);
...
static inline zx_status_t spi_exchange(zx_handle_t channel, const void* txdata, void* rxdata,
    ...
    zx_status_t status;
    size_t actual;
    zx_status_t call_status = fuchsia_hardware_spi_DeviceExchange(channel, (const uint8_t *)txdata,
                                                                  length, &status,
                                                                  (uint8_t *)rxdata,
                                                                  length, &actual);
    if (call_status != ZX_OK) {
