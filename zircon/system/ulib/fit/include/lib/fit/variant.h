...
template <size_t Index, typename T, typename Ti,
          bool IsBool = std::is_same<bool, std::remove_cv_t<Ti>>::value,
          typename = void>
...
public:
    ...
    constexpr variant() noexcept(nothrow_default_constructible)
        : storage_{default_init_v} {}
    ...
    template <typename T,
              ...
              requires_conditions<
                  occurs_once<Ti>,
                  std::is_constructible<Ti, T>> = true>
    ...
    template <typename T, typename... Args,
              requires_conditions<
                  occurs_once<T>,
                  std::is_constructible<T, Args...>> = true>
    ...
    template <typename T, typename U, typename... Args,
              requires_conditions<
                  occurs_once<T>,
                  std::is_constructible<T, std::initializer_list<T>&, Args...>> = true>
    ...
    template <size_t Index, typename... Args,
              requires_conditions<
                  std::is_constructible<alternative_t<Index>, Args...>> = true>
    ...
    template <size_t Index, typename U, typename... Args,
              requires_conditions<
                  std::is_constructible<alternative_t<Index>,
    ...
    constexpr assignment_requires_conditions<
        occurs_once<selected_t<T>>,
        std::is_constructible<selected_t<T&&>, T>,
        std::is_assignable<selected_t<T&&>&, T>,
        disjunction<
            std::is_nothrow_constructible<selected_t<T&&>, T>,
    ...
    constexpr assignment_requires_conditions<
        ...
        std::is_assignable<selected_t<T&&>&, T>,
        conjunction<
            negation<std::is_nothrow_constructible<selected_t<T&&>, T>>,
    ...
    template <typename T, typename... Args>
    constexpr emplace_constructible_by_type<T, Args&&...>
    emplace(Args&&... args) {
        storage_.reset();
    ...
    template <typename T, typename U, typename... Args>
    constexpr emplace_constructible_by_type<T, std::initializer_list<U>&, Args&&...>
    ...
    template <size_t Index, typename... Args>
    constexpr emplace_constructible_by_index<Index, Args&&...>
    emplace(Args&&... args) {
        storage_.reset();
    ...
    template <size_t Index, typename U, typename... Args>
    constexpr emplace_constructible_by_index<Index, std::initializer_list<U>&, Args&&...>
    emplace(std::initializer_list<U> init_list, Args&&... args) {
        storage_.reset();
    ...
    friend constexpr bool operator==(const variant& lhs, const variant& rhs) {
        bool result = false;
        const bool has_value = lhs.storage_.visit(
            [&result, &lhs, &rhs](auto, auto active_index_tag, const auto*) {
                if (lhs.index() != rhs.index()) {
                    result = false;
                } else {
                    result = lhs.storage_.get(active_index_tag) ==
                             rhs.storage_.get(active_index_tag);
                }
            });
    ...
    friend constexpr bool operator<(const variant& lhs, const variant& rhs) {
        bool result = true;
        const bool has_value = rhs.storage_.visit(
            [&result, &lhs, &rhs](auto, auto active_index_tag, const auto*) {
