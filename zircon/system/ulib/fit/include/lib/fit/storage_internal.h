...
using make_storage_class = storage_class<
    is_trivially_destructible_v<Ts...> ? destructor_class::trivial
                                       : destructor_class::non_trivial,
    is_trivially_copyable_v<Ts...> ? copy_class::trivial
                                   : copy_class::non_trivial,
    is_trivially_movable_v<Ts...> ? move_class::trivial
                                  : move_class::non_trivial>;
...
union storage_base<destructor_class::non_trivial, type_index<empty_type, empty_index>> {
    ...
    storage_base(type_tag<empty_type>, Args&&...)
        : empty{} {}
    template <typename... Args>
    storage_base(index_tag<empty_index>, Args&&...)
        : empty{} {}
    ...
    empty_type& get(type_tag<empty_type>) {
        return empty;
    }
    const empty_type& get(type_tag<empty_type>) const {
        return empty;
    }
    empty_type& get(index_tag<empty_index>) {
        return empty;
    }
    const empty_type& get(index_tag<empty_index>) const {
        return empty;
    }

    size_t index(type_tag<empty_type>) const {
        return empty_index;
    }
...
union storage_base<destructor_class::trivial, type_index<empty_type, empty_index>> {
    ...
    template <typename... Args>
    constexpr storage_base(type_tag<empty_type>, Args&&...)
        : empty{} {}
    template <typename... Args>
    constexpr storage_base(index_tag<empty_index>, Args&&...)
        : empty{} {}
    ...
    constexpr empty_type& get(type_tag<empty_type>) {
        return empty;
    }
    constexpr const empty_type& get(type_tag<empty_type>) const {
        return empty;
    }
    constexpr empty_type& get(index_tag<empty_index>) {
        return empty;
    }
    constexpr const empty_type& get(index_tag<empty_index>) const {
        return empty;
    }

    constexpr size_t index(type_tag<empty_type>) const {
        return empty_index;
    }
...
union storage_base<destructor_class::non_trivial, type_index<T, Index>, type_index<Ts, Is>...> {
    ...
    template <typename... Args>
    storage_base(type_tag<T>, Args&&... args)
        : value(std::forward<Args>(args)...) {}
    template <typename... Args>
    storage_base(index_tag<Index>, Args&&... args)
        : value(std::forward<Args>(args)...) {}

    template <typename U, typename... Args>
    storage_base(type_tag<U>, Args&&... args)
    ...
    T& get(index_tag<Index>) {
        return value;
    }
    const T& get(index_tag<Index>) const {
        return value;
    }
...
union storage_base<destructor_class::trivial, type_index<T, Index>, type_index<Ts, Is>...> {
    ...
    template <typename... Args>
    constexpr storage_base(type_tag<T>, Args&&... args)
        : value(std::forward<Args>(args)...) {}
    template <typename... Args>
    constexpr storage_base(index_tag<Index>, Args&&... args)
        : value(std::forward<Args>(args)...) {}
    ...
    constexpr T& get(index_tag<Index>) {
        return value;
    }
    constexpr const T& get(index_tag<Index>) const {
        return value;
    }
    template <size_t OtherIndex>
    ...
    constexpr size_t index(type_tag<T>) const {
        return Index;
    }
    template <typename U>
...
private:
    using base_type = storage_base<DestructorClass, type_index<Ts, Is>...,
                                   type_index<empty_type, empty_index>>;
...
public:
    ...
    constexpr indexed_storage(trivial_init_t)
        : indexed_storage{} {}

    constexpr indexed_storage(default_init_t)
        : index_{0}, base_{index_tag<0>{}} {}
    ...
    constexpr void swap(indexed_storage& other) {
        if (index() == other.index()) {
            ...
            auto do_swap = [](indexed_storage& a, indexed_storage& b) {
                return a.base_.visit(
                    a.index_,
                    [&a, &b](auto type_tag_v, auto index_tag_v, auto* element) {
...
struct storage<storage_class<destructor_class::trivial,
                             copy_class::trivial,
                             move_class::trivial>,
               type_index<Ts, Is>...>
