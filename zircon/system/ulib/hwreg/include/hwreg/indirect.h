...
// IndirectIo is a replacement for RegisterIo that sequences access to a common PCI device construct
...
class IndirectIo {
 public:
  explicit IndirectIo(RegisterIo io) : io_(io) {}
  ...
  void Write(IntType value, uint32_t offset) {
    ...
    io_.Write<IndexType>(static_cast<IndexType>(offset), index_offset);
    io_.Write(value, data_offset);
  }
  ...
  IntType Read(uint32_t offset) {
    std::lock_guard<std::mutex> l(lock_);
    io_.Write<IndexType>(static_cast<IndexType>(offset), index_offset);
    return io_.Read<IntType>(data_offset);
  }
