...
JournalEntry::JournalEntry(JournalBase* journal, EntryStatus status, size_t header_index,
  ...
  for (size_t i = 0; i < work_->Transaction().Operations().size(); i++) {
    ...
    for (size_t j = operation.op.dev_offset; j < operation.op.dev_offset + operation.op.length;
         j++) {
...
void JournalEntry::SetChecksum(uint32_t checksum) { commit_block_.checksum = checksum; }
