...
UserPager::ReadRange UserPager::AlignReadRange(UserPagerInfo* info, uint64_t offset,
                                               uint64_t length) {
...
UserPager::ReadRange UserPager::ExtendReadRange(UserPagerInfo* info, uint64_t offset,
  ...
  return AlignReadRange(info, read_ahead_offset, read_ahead_length);
}
...
zx_status_t UserPager::TransferCompressedPagesToVmo(uint64_t requested_offset,
  ...
  const auto [ offset, length ] = AlignReadRange(info, requested_offset, requested_length);
  ...
  // Move the pages from the decompresion buffer to the destination VMO.
  status = pager_.supply_pages(
      vmo, mapping.decompressed_offset,
