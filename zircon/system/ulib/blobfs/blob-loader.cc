...
zx_status_t BlobLoader::LoadBlobPaged(uint32_t node_index,
  ...
  userpager_info.compression_algorithm = AlgorithmFromInodeHeaderFlags(inode->header.flags);
...
zx_status_t BlobLoader::LoadAndDecompressData(uint32_t node_index, const Inode& inode,
                                              const fzl::OwnedVmoMapper& vmo) const {
  std::optional<CompressionAlgorithm> algorithm = AlgorithmFromInodeHeaderFlags(inode.header.flags);
  if (!algorithm) {
