...
constexpr uint64_t MinimumFilesystemBlocks() {
    ...
    const uint64_t kBlockMapBlocks = fbl::round_up(kDataBlocks, kBlobfsBlockBits)
            / kBlobfsBlockBits;
...
TEST(FormatFilesystemTest, FormatFVMSmallestDevice) {
    ...
    // Smallest possible device.
    {
        auto device = std::make_unique<FakeFVMBlockDevice>(kBlockCount, kBlockSize, kSliceSize,
                                                           kSliceCount);
        ASSERT_OK(FormatFilesystem(device.get()));
        ASSERT_OK(CheckMountability(std::move(device)));
    }
    ...
    {
        auto device = std::make_unique<FakeFVMBlockDevice>(kBlockCount, kBlockSize,
                                                           kSliceSize, kSliceCount - 1);
        ASSERT_EQ(ZX_ERR_NO_SPACE, FormatFilesystem(device.get()));
    }
}
...
TEST(FormatFilesystemTest, FormatDeviceAutoConvertReadonly) {
    ...
    ASSERT_EQ(ZX_ERR_ACCESS_DENIED,
              blobfs->Open(root, &unused_outvnode, "foo", &unused_outpath,
