...
class FakeJournal : public JournalBase {
 public:
  ...
  fbl::unique_ptr<WritebackWork> CreateDefaultWork() { return CreateWork(); }
  ...
  fbl::unique_ptr<WritebackWork> DequeueWork() { return work_queue_.pop(); }

 private:
  using WorkQueue = fs::Queue<fbl::unique_ptr<WritebackWork>>;

  size_t GetCapacity() const final { return capacity_; }

  bool IsReadOnly() const final { return readonly_; }
...
TEST(JournalTest, JournalEntryLifetime) {
  ...
  fbl::unique_ptr<JournalEntry> entry(
      new JournalEntry(&journal, EntryStatus::kInit, 0, 0, journal.CreateBufferedWork(1)));
  ...
  entry.reset(new JournalEntry(&journal, EntryStatus::kInit, 0, 0, journal.CreateBufferedWork(1)));
...
TEST(JournalTest, JournalProcessorResetsWork) {
  ...
  // Create and process another 'work' entry.
  entry.reset(new JournalEntry(&journal, EntryStatus::kInit, 2, 3, journal.CreateBufferedWork(1)));
  fbl::unique_ptr<WritebackWork> second_work = journal.CreateDefaultWork();
  second_work->SetSyncCallback(entry->CreateSyncCallback());
  processor.ProcessWorkEntry(std::move(entry));
...
TEST(JournalTest, DestroyJournalWithoutTeardown) {
  ...
  transaction_manager.SetTransactionCallback([](const block_fifo_request_t& request,
    ...
    if (request.dev_offset == 0 && request.opcode == BLOCKIO_READ) {
      ...
      return vmo.write(&journal_info, request.vmo_offset * kBlobfsBlockSize, sizeof(JournalInfo));
    }
    return ZX_OK;
  });
