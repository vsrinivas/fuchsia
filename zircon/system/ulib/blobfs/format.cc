...
zx_status_t TryFormattingFVM(BlockDevice* device, Superblock* superblock) {
    ...
    superblock->inode_count = static_cast<uint32_t>(superblock->ino_slices * superblock->slice_size
                                                    / kBlobfsInodeSize);

    superblock->data_block_count = static_cast<uint32_t>(superblock->dat_slices *
                                                         superblock->slice_size /
                                                         kBlobfsBlockSize);
    superblock->journal_block_count = static_cast<uint32_t>(superblock->journal_slices *
                                                            superblock->slice_size /
                                                            kBlobfsBlockSize);
    return ZX_OK;
}
...
zx_status_t WriteFilesystemToDisk(BlockDevice* device,
    ...
    for (uint64_t n = 0; n < journal_blocks; n++) {
        uint64_t offset = kBlobfsBlockSize * (superblock_blocks + blockmap_blocks + nodemap_blocks +
                                              n);
        uint64_t length = kBlobfsBlockSize;
        status = vmo.write(block, offset, length);
        if (status != ZX_OK) {
