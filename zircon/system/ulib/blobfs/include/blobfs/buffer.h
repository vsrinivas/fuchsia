...
class Buffer {
 public:
  ...
  void Load(fs::ReadTxn* txn, size_t disk_start) { txn->Enqueue(vmoid_, 0, disk_start, capacity_); }
  ...
  void FreeAllSpace() { FreeSpace(length_); }
  ...
  size_t ReserveIndex() { return (start_ + length_++) % capacity_; }
