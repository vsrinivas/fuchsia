...
zx_status_t Journal::Create(TransactionManager* transaction_manager, uint64_t journal_blocks,
  ...
  fbl::unique_ptr<Journal> journal(
      new Journal(transaction_manager, std::move(info), std::move(buffer), start_block));
...
zx_status_t Journal::Replay() {
  ...
  if (total_entries > 0) {
    printf(
        "Found and replayed %zu total blobfs journal entries starting from index %zu, "
...
zx_status_t Journal::InitWriteback() {
  ...
  if (thrd_create_with_name(&thread_, JournalThread, this, "blobfs-journal") != thrd_success) {
...
fbl::unique_ptr<JournalEntry> Journal::CreateEntry(uint64_t header_index, uint64_t commit_index,
  ...
  return std::make_unique<JournalEntry>(this, status, header_index, commit_index, std::move(work));
}
...
void Journal::PrepareDelete(JournalEntry* entry, WritebackWork* work) {
  ...
  entries_->AddTransaction(header_index, start_block_ + 1 + header_index, 1, &work->Transaction());
  entries_->AddTransaction(commit_index, start_block_ + 1 + commit_index, 1, &work->Transaction());
}
