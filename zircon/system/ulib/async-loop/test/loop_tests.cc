...
class TestException : public async_exception_t {
 public:
  ...
  zx_status_t Bind() { return async_bind_exception_port(dispatcher_, this); }

  zx_status_t Unbind() { return async_unbind_exception_port(dispatcher_, this); }
 ...
 private:
  static void CallHandler(async_dispatcher_t* dispatcher, async_exception_t* receiver,
...
class TestProcessException : public TestException {
 public:
  TestProcessException(async_dispatcher_t* dispatcher, zx_handle_t task, uint32_t options,
      ...
      : TestException(dispatcher, task, options), test_thread_tid_(test_thread_tid) {}
 ...
 private:
  ...
  void ResumeTryNext(zx_koid_t tid) {
    ...
    zx_handle_t thread;
    auto status = zx_object_get_child(zx_process_self(), tid, ZX_RIGHT_SAME_RIGHTS, &thread);
    switch (status) {
      case ZX_OK:
...
bool exception_test() {
  ...
  EXPECT_EQ(get_thread_exception_port_type(crashing_thread), ZX_EXCEPTION_PORT_TYPE_THREAD);
  ...
  EXPECT_EQ(get_thread_exception_port_type(crashing_thread), ZX_EXCEPTION_PORT_TYPE_PROCESS);
  EXPECT_EQ(1u, thread_exception.run_count);
...
bool threads_exceptions_run_concurrently_test() {
  ...
  ThreadAssertException receiver(loop.dispatcher(), zx_process_self(), 0, &measure);
