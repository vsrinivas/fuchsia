...
TEST_F(PinnedVmoTester, FailPinRangeTest) {
  ...
  EXPECT_EQ(pinned_vmo_.PinRange(zx_system_get_page_size() + 1, zx_system_get_page_size(), vmo_,
                                 *zx::unowned_bti(bti_), options),
            ZX_ERR_INVALID_ARGS);
  // length not page aligned:
  EXPECT_EQ(
      pinned_vmo_.PinRange(0, zx_system_get_page_size() + 1, vmo_, *zx::unowned_bti(bti_), options),
      ZX_ERR_INVALID_ARGS);
  // zero length:
