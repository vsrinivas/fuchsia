...
zx_status_t fvm_validate_header(const void* metadata, const void* backup, size_t metadata_size,
    ...
    bool primary_valid =
        primary_metadata_size <= metadata_size && fvm_check_hash(metadata, primary_metadata_size);
    bool backup_valid =
        backup_metadata_size <= metadata_size && fvm_check_hash(backup, backup_metadata_size);
