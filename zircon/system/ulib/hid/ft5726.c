...
static const uint8_t ft5726_touch_report_desc[] = {
...
bool is_ft5726_touch_report_desc(const uint8_t* data, size_t len) {
  ...
  if (len != sizeof(ft5726_touch_report_desc))
    return false;

  return (memcmp(data, ft5726_touch_report_desc, len) == 0);
}
...
size_t get_ft5726_report_desc(const uint8_t** buf) {
  *buf = ft5726_touch_report_desc;
  return sizeof(ft5726_touch_report_desc);
}
