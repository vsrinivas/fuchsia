...
class FakeLogger : public Logger {
 public:
  ...
  bool Log(const RemoteMetricInfo& metric_info, const HistogramBucket* bucket, size_t num_buckets) {
    if (!should_fail_) {
  ...
  bool Log(const RemoteMetricInfo& metric_info, int64_t count) {
    if (!should_fail_) {
  ...
  const fbl::Vector<HistogramBucket> GetHistogram(const RemoteMetricInfo& info) const {
  ...
  RemoteCounter::Type GetCounter(const RemoteMetricInfo& info) const {
