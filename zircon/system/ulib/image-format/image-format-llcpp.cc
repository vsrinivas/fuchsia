...
fuchsia_sysmem_ImageFormatConstraints GetCConstraints(
  ...
  static_assert(
      std::is_trivially_copyable<llcpp::fuchsia::sysmem::wire::ImageFormatConstraints>::value,
...
llcpp::fuchsia::sysmem::wire::PixelFormat GetCppPixelFormat(const fuchsia_sysmem_PixelFormat& c) {
  sysmem::wire::PixelFormat cpp;
  static_assert(sizeof(cpp) == sizeof(c), "LLCPP and C pixel formats don't match");
  static_assert(std::is_trivially_copyable<llcpp::fuchsia::sysmem::wire::PixelFormat>::value,
                "Not trivially copyable");
...
fuchsia_sysmem_PixelFormat GetCPixelFormat(const llcpp::fuchsia::sysmem::wire::PixelFormat& cpp) {
...
llcpp::fuchsia::sysmem::wire::ImageFormat_2 GetCppImageFormat(
    const fuchsia_sysmem_ImageFormat_2& c) {
  sysmem::wire::ImageFormat_2 cpp;
  static_assert(sizeof(cpp) == sizeof(c), "LLCPP and C image formats don't match");
  static_assert(std::is_trivially_copyable<llcpp::fuchsia::sysmem::wire::ImageFormat_2>::value,
                "Not trivially copyable");
  // Hacky copy that should work for now. We need the static_cast because sysmem::wire::PixelFormat
  // is a non-trivial class and otherwise GCC complains.
  // TODO(fxbug.dev/37078): Switch away from C bindings everywhere.
...
fuchsia_sysmem_ImageFormat_2 GetCImageFormat(
    const llcpp::fuchsia::sysmem::wire::ImageFormat_2& cpp) {
