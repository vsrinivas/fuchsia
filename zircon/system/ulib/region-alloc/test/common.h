...
static inline bool region_contains_region(const ralloc_region_t* contained_by,
  ...
  return ((contained->base >= contained_by->base) && (contained_end >= contained_by->base) &&
          (contained->base <= contained_by_end) && (contained_end <= contained_by_end));
}
...
void check_region_match(const ralloc_region_t* test, const ralloc_region_t* expected) {
