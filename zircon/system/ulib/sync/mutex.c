...
static zx_status_t lock_slow_path(sync_mutex_t* mutex,
                                  zx_time_t deadline,
    ...
    for (;;) {
        ...
        // who we think is holding the mutex (and therefor "owns" the futex) as
        // we do so.
        ...
        if ((contested_state == old_state) ||
            ...
            zx_status_t status = _zx_futex_wait(&mutex->futex,
                                                contested_state,
                                                libsync_mutex_make_owner_from_state(old_state),
                                                deadline);
