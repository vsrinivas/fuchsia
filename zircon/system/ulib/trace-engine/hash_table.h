...
private:
    ...
    static constexpr PtrType sentinel() {
        return reinterpret_cast<PtrType>(kContainerSentinelBit);
    }
...
template <typename _KeyType,
          typename _NodeType,
          size_t _NumBuckets = kDefaultNumBuckets>
class HashTable {
public:
    ...
    PtrType lookup(const KeyType& key) {
        return FindInBucket(GetBucket(key), key);
    }

private:
    static HashType GetHash(const KeyType& key) {
        return NodeType::GetHash(key);
    }

    BucketType& GetBucket(const KeyType& key) {
        return buckets_[GetHash(key) % kNumBuckets];
    }
