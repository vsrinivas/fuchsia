...
#ifndef INTEL_HDA_UTILS_CODEC_COMMANDS_H_
#define INTEL_HDA_UTILS_CODEC_COMMANDS_H_
...
struct CodecCommand {
  ...
  CodecCommand(uint8_t codec_id, uint16_t nid, CodecVerb verb) { Pack(codec_id, nid, verb); }

  void Pack(uint8_t codec_id, uint16_t nid, CodecVerb verb) {
    data = verb.val | (static_cast<uint32_t>(nid) << 20) | (static_cast<uint32_t>(codec_id) << 28);
  }
  ...
  static bool SanityCheck(uint8_t codec_id, uint16_t nid, CodecVerb verb) {
    ...
    //
    return ((codec_id < HDA_MAX_CODECS) && (nid < HDA_MAX_NID) && (nid || verb.val) &&
            verb.SanityCheck());
  }
...
static inline constexpr CodecVerb SET_AMPLIFIER_GAIN_MUTE(bool mute, uint8_t gain_steps,
                                                          ...
                                                          uint8_t ndx = 0, bool set_left = true,
                                                          bool set_right = true) {
  return LP_SET_VERB<0x03>(static_cast<uint16_t>(
      (set_output ? (1u << 15) : 0) | (set_input ? (1u << 14) : 0) | (set_left ? (1u << 13) : 0) |
      (set_right ? (1u << 12) : 0) | (mute ? (1u << 7) : 0) |
      (static_cast<uint16_t>(ndx & 0xF) << 8) | (gain_steps & 0x7F)));
}

static inline constexpr CodecVerb SET_INPUT_AMPLIFIER_GAIN_MUTE(bool mute, uint8_t gain_steps,
                                                                uint8_t ndx = 0,
                                                                bool set_left = true,
                                                                bool set_right = true) {
...
static inline constexpr CodecVerb SET_OUTPUT_AMPLIFIER_GAIN_MUTE(bool mute, uint8_t gain_steps,
                                                                 uint8_t ndx = 0,
                                                                 bool set_left = true,
                                                                 bool set_right = true) {
...
// Section 7.3.3.12
static inline constexpr CodecVerb SET_ANALOG_PIN_WIDGET_CTRL(bool enable_out, bool enable_in,
                                                             bool enable_hp_amp,
                                                             VRefEn vref = VRefEn::HiZ) {
  return SP_SET_VERB<0x07>(
      static_cast<uint8_t>((enable_out ? (1u << 6) : 0) | (enable_in ? (1u << 5) : 0) |
                           (enable_hp_amp ? (1u << 7) : 0) | (static_cast<uint8_t>(vref) & 0x7)));
}

static inline constexpr CodecVerb SET_DIGITAL_PIN_WIDGET_CTRL(bool enable_out, bool enable_in,
                                                              EPT ept = EPT::Native) {
...
#endif  // INTEL_HDA_UTILS_CODEC_COMMANDS_H_
