...
#ifndef INTEL_HDA_UTILS_CODEC_CAPS_H_
#define INTEL_HDA_UTILS_CODEC_CAPS_H_
...
struct AudioWidgetCaps {
  ...
  uint8_t ch_count() const {
    return static_cast<uint8_t>((((raw_data_ >> 12) & 0xE) | (raw_data_ & 0x1)) + 1);
  }
...
struct SampleCaps {
  ...
  SampleCaps(uint32_t size_rate, uint32_t formats)
      : pcm_size_rate_(size_rate), pcm_formats_(formats) {}
...
struct AmpCaps {
  ...
  float max_gain_db() const {
    return min_gain_db() + (step_size_db() * static_cast<float>((num_steps() - 1)));
  }
};
...
#endif  // INTEL_HDA_UTILS_CODEC_CAPS_H_
