...
static void print_help(const char* prog_name, FILE* f) {
  ...
  fprintf(f,
          ...
          TEST_ENV_NAME, WATCHDOG_ENV_NAME, DEFAULT_BASE_TIMEOUT_SECONDS);
  fprintf(f,
          ...
          "        Large       - x %d\n"
          "        Performance - x %d\n",
          TEST_TIMEOUT_FACTOR_SMALL, TEST_TIMEOUT_FACTOR_MEDIUM, TEST_TIMEOUT_FACTOR_LARGE,
          TEST_TIMEOUT_FACTOR_PERFORMANCE);
}
...
bool unittest_run_all_tests(int argc, char** argv) {
  ...
  if (watchdog_timeout_str != nullptr) {
    ...
    if (*watchdog_timeout_str == '\0' || *end != '\0' || timeout < 0 || timeout > INT_MAX) {
  ...
  auto result =
      unittest_run_all_tests_etc(argv[0], test_type, case_matcher, test_matcher, list_tests_only);
