...
static inline const char* unittest_get_message(const char* arg) { return arg; }
#ifdef __cplusplus
 ...
 */
void unittest_printf_critical(const char* format, ...) __attribute__((format(printf, 1, 2)));
...
#define UNITTEST_FAIL_TRACEF(str, x...)                                           \
  do {                                                                            \
    unittest_printf_critical(UNITTEST_FAIL_TRACEF_FORMAT str, __FILE__, __LINE__, \
...
#define RUN_NAMED_TEST_TYPE(name, test, test_type)                                      \
  if (!test_name_matching || strcmp(test_name_matching, name) == 0) {                   \
    if (list_only) {                                                                    \
      ...
      unittest_run_named_test(name, test, test_type, &current_test_info, &all_success); \
    }                                                                                   \
  }
...
#define END_HELPER return _ut_helper_test_info.all_ok
  ...
  do {                                                                                       \
    ...
    if (!(_lhs_val op _rhs_val)) {                                                           \
      UNITTEST_FAIL_TRACEF(                                                                  \
          ...
          unittest_get_message(__VA_ARGS__), lhs_str, #op, rhs_str, (long long int)_lhs_val, \
          (unsigned long long)_lhs_val, #op, (long long int)_rhs_val,                        \
...
#define UT_TRUE(actual, ret, ...)                                                            \
  do {                                                                                       \
    ...
    UNITTEST_TRACEF(2, "%s\n", #actual);                                                     \
    if (!(actual)) {                                                                         \
      UNITTEST_FAIL_TRACEF("%s: %s is false\n", unittest_get_message(__VA_ARGS__), #actual); \
      current_test_info->all_ok = false;                                                     \
      ret;                                                                                   \
    }                                                                                        \
  } while (0)
...
#define UT_STR_EQ(str1, str2, ret, ...)                                                          \
  do {                                                                                           \
    ...
    if (!unittest_expect_str_eq((str1), (str2), #str1, #str2, unittest_get_message(__VA_ARGS__), \
                                __FILE__, __LINE__, __PRETTY_FUNCTION__)) {                      \
      current_test_info->all_ok = false;                                                         \
      ret;                                                                                       \
    }                                                                                            \
...
#define UT_STR_NE(str1, str2, ret, ...)                                                          \
  do {                                                                                           \
    ...
    if (!unittest_expect_str_ne((str1), (str2), #str1, #str2, unittest_get_message(__VA_ARGS__), \
                                __FILE__, __LINE__, __PRETTY_FUNCTION__)) {                      \
      current_test_info->all_ok = false;                                                         \
      ret;                                                                                       \
    }                                                                                            \
...
#define UT_STR_STR(str1, str2, ret, ...)                                                          \
  do {                                                                                            \
     ...
     * storage for the C string will get deallocated before the                                   \
     * string is used.  Instead we must use a helper function. */                                 \
    if (!unittest_expect_str_str((str1), (str2), #str1, #str2, unittest_get_message(__VA_ARGS__), \
                                 __FILE__, __LINE__, __PRETTY_FUNCTION__)) {                      \
      current_test_info->all_ok = false;                                                          \
      ret;                                                                                        \
    }                                                                                             \
...
#define EXPECT_BYTES_EQ(expected, actual, length, msg) \
  UT_BYTES_EQ(expected, actual, length, msg, DONOT_RET)
#define EXPECT_BYTES_NE(bytes1, bytes2, length, msg) \
  UT_BYTES_NE(bytes1, bytes2, length, msg, DONOT_RET)
...
#define ASSERT_BYTES_EQ(expected, actual, length, msg) \
  UT_BYTES_EQ(expected, actual, length, msg, RET_FALSE)
#define ASSERT_BYTES_NE(bytes1, bytes2, length, msg) \
  UT_BYTES_NE(bytes1, bytes2, length, msg, RET_FALSE)
 ...
 * }
 */
#define ASSERT_DEATH(fn, arg, msg) \
  ASSERT_EQ((death_test_result_t)DEATH_TEST_RESULT_DIED, unittest_run_death_fn(fn, arg), msg)
#define ASSERT_NO_DEATH(fn, arg, msg) \
  ASSERT_EQ((death_test_result_t)DEATH_TEST_RESULT_LIVED, unittest_run_death_fn(fn, arg), msg)
...
void unittest_run_named_test(const char* name, bool (*test)(void), test_type_t test_type,
