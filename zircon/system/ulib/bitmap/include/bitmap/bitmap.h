...
class Bitmap {
 public:
  ...
  virtual zx_status_t Find(bool is_set, size_t bitoff, size_t bitmax, size_t run_len,
                           size_t* out) const = 0;

  // Returns true in the bit at bitoff is set.
  virtual bool GetOne(size_t bitoff) const { return Get(bitoff, bitoff + 1, nullptr); }
  ...
  virtual bool Get(size_t bitoff, size_t bitmax, size_t* first_unset = nullptr) const = 0;

  // Sets the bit at bitoff.  Only fails on allocation error.
  virtual zx_status_t SetOne(size_t bitoff) { return Set(bitoff, bitoff + 1); }
  ...
  virtual zx_status_t ClearOne(size_t bitoff) { return Clear(bitoff, bitoff + 1); }
