...
class UseLdsvc {
 public:
  ...
  zx_status_t operator()(std::string_view name, zx::vmo* vmo) {
    int32_t rv;
    zx_status_t status = fuchsia_ldsvc_LoaderLoadObject(ldsvc_->get(), name.data(), name.size(),
...
constexpr Decompressor kDecompressors[] = {
    {"hermetic/decompress-lz4f.so", &cache_lz4f, HermeticDecompressorEngineService::kLz4fMagic},
    {"hermetic/decompress-zstd.so", &cache_zstd, HermeticDecompressorEngineService::kZstdMagic},
};

}  // namespace

zx_status_t HermeticDecompressorEngineService::GetEngine(Magic magic, zx::unowned_vmo* vmo) {
