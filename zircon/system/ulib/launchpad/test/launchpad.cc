...
static bool run_one_argument_size_test(size_t size) {
  ...
  launchpad_t* lp;
  ASSERT_EQ(launchpad_create(ZX_HANDLE_INVALID, "argument size test", &lp), ZX_OK, "");
  ...
  EXPECT_EQ(zx_object_wait_one(proc, ZX_PROCESS_TERMINATED, ZX_TIME_INFINITE, NULL), ZX_OK, "");
  zx_info_process_t info;
  EXPECT_EQ(zx_object_get_info(proc, ZX_INFO_PROCESS, &info, sizeof(info), NULL, NULL), ZX_OK, "");
...
static bool argument_size_test(void) {
  ...
  for (size_t size = 0; size < 2 * PAGE_SIZE; size += 1024) {
    if (!run_one_argument_size_test(size)) {
      ok = false;
      unittest_printf_critical("    argument size test at %-29zu [FAILED]\n", size);
    }
  }
  return ok;
}

static bool run_with_args_env_handles(unsigned int num_args, unsigned int num_env,
  ...
  for (unsigned int i = 0; i < num_handles; ++i) {
    ...
    ASSERT_EQ(status, ZX_OK, "");
    ASSERT_EQ(launchpad_add_handle(lp, vmo_dup.release(), PA_HND(PA_USER0, i)), ZX_OK,
  ...
  ASSERT_EQ(zx_object_wait_one(proc.get(), ZX_PROCESS_TERMINATED, ZX_TIME_INFINITE, NULL), ZX_OK,
            "");
  zx_info_process_t info;
  ASSERT_EQ(zx_object_get_info(proc.get(), ZX_INFO_PROCESS, &info, sizeof(info), NULL, NULL), ZX_OK,
            "");
