...
class SystemInstance {
 public:
  ...
  SystemInstance() : fshost_ldsvc(zx::channel()) {}
  ...
  void do_autorun(const char* name, const char* cmd) {
    if (cmd != nullptr) {
      ...
      devmgr::devmgr_launch_with_loader(svc_job, name, zx::vmo(), std::move(ldsvc),
  ...
  int FuchsiaStarter(devmgr::Coordinator* coordinator) {
    ...
    do {
      ...
      if (!appmgr_started && stat(argv_appmgr[0], &s) == 0) {
        ...
        devmgr::devmgr_launch_with_loader(fuchsia_job, "appmgr", zx::vmo(),
  ...
  int ServiceStarter(devmgr::Coordinator* coordinator) {
    ...
    devmgr::devmgr_launch(svc_job, "miscsvc", args, nullptr, -1, handles, types,
    ...
    if (!(coordinator->boot_args().GetBool("netsvc.disable", false) ||
      ...
      zx::process proc;
      zx_status_t status = devmgr::devmgr_launch(svc_job, "netsvc", args, nullptr, -1,
...

    {
      ...
      devmgr::devmgr_launch(svc_job, "device-name-provider", args, nullptr, -1, handles,
    ...
    if (!coordinator->boot_args().GetBool("virtcon.disable", false)) {
      ...
      devmgr::devmgr_launch(svc_job, "virtual-console", args, env.data(), -1, handles,
  ...
  int ConsoleStarter(const devmgr::BootArgs* arg) {
    ...
    for (;;) {
      ...
      zx::process proc;
      status = devmgr::devmgr_launch_with_loader(svc_job, "sh:console", zx::vmo(),
  ...
  zx_status_t CreateFuchsiaJob(const zx::job& root_job) {
    ...
    status = fuchsia_job.set_policy(ZX_JOB_POL_RELATIVE, ZX_JOB_POL_TIMER_SLACK,
  ...
  zx_status_t StartSvchost(const zx::job& root_job, bool require_system,
    ...
    zx::channel appmgr_svc;
    {
      ...
      status =
          fdio_service_connect_at(appmgr_client.get(), "svc", appmgr_svc_req.release());
      if (status != ZX_OK) {
        return status;
      }
    }
    ...
    zx::channel miscsvc_svc;
    {
      ...
      status =
          fdio_service_connect_at(miscsvc_client.get(), "svc", miscsvc_svc_req.release());
      if (status != ZX_OK) {
        return status;
      }
    }
  ...
  void fshost_start(devmgr::Coordinator* coordinator, const devmgr::DevmgrArgs& devmgr_args,
    ...
    devmgr::devmgr_launch(svc_job, "fshost", args.data(), env.data(), -1, handles, types, n,
...
int main(int argc, char** argv) {
  ...
  if (devmgr_args.start_svchost) {
    status = system_instance.StartSvchost(root_job, require_system, &coordinator, std::move(fshost_client));
    if (status != ZX_OK) {
  ...
  int ret = thrd_create_with_name(&t, SystemInstance::pwrbtn_monitor_starter,
                                  &system_instance, "pwrbtn-monitor-starter");
  if (ret != thrd_success) {
  ...
  starter_args->coordinator = &coordinator;
  ret = thrd_create_with_name(&t, SystemInstance::service_starter,
                              starter_args.release(), "service-starter");
  if (ret != thrd_success) {
