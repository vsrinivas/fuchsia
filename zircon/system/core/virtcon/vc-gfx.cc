...
void vc_gfx_draw_char(vc_t* vc, vc_char_t ch, unsigned x, unsigned y, bool invert) {
  ...
  gfx_putchar(vc_gfx, vc->font, vc_char_get_char(ch), x * vc->charw, y * vc->charh,
...
zx_status_t vc_init_gfx(gfx_surface* test) {
  ...
  // init the status bar
  vc_tb_gfx = gfx_create_surface(NULL, test->width, font->height, test->stride, test->format, 0);
  if (!vc_tb_gfx) {
    return ZX_ERR_NO_MEMORY;
  }

  // init the main surface
  vc_gfx = gfx_create_surface(NULL, test->width, test->height, test->stride, test->format, 0);
  if (!vc_gfx) {
...
void vc_gfx_invalidate_status() { gfx_copylines(vc_test_gfx, vc_tb_gfx, 0, 0, vc_tb_gfx->height); }
...
void vc_gfx_invalidate(vc_t* vc, unsigned x, unsigned y, unsigned w, unsigned h) {
  ...
  if ((x == 0) && (w == vc->columns)) {
    ...
    gfx_blend(vc_test_gfx, vc_gfx, x * vc->charw, y * vc->charh, w * vc->charw, h * vc->charh,
              x * vc->charw, desty);
  }
}
...
zx_status_t vc_init_gfx(zx_handle_t fb_vmo, int32_t width, int32_t height, zx_pixel_format_t format,
  ...
  if ((r = zx_vmo_set_cache_policy(fb_vmo, ZX_CACHE_POLICY_CACHED)) == ZX_ERR_BAD_STATE) {
    if ((r = zx_vmar_map(zx_vmar_root_self(), ZX_VM_PERM_READ | ZX_VM_PERM_WRITE, 0, fb_vmo, 0,
                         vc_gfx_size, &vc_hw_gfx_mem)) < 0) {
      goto fail;
    }

    if ((vc_hw_gfx = gfx_create_surface((void*)vc_hw_gfx_mem, width, height, stride, format, 0)) ==
        NULL) {
      r = ZX_ERR_INTERNAL;
      goto fail;
    }
  ...
  uintptr_t ptr;
  vc_gfx_vmo = fb_vmo;
  if ((r = zx_vmar_map(zx_vmar_root_self(), ZX_VM_PERM_READ | ZX_VM_PERM_WRITE, 0, vc_gfx_vmo, 0,
                       vc_gfx_size, &vc_gfx_mem)) < 0) {
    goto fail;
  }

  r = ZX_ERR_NO_MEMORY;
  // init the status bar
  if ((vc_tb_gfx = gfx_create_surface((void*)vc_gfx_mem, width, font->height, stride, format, 0)) ==
      NULL) {
    goto fail;
  }
  ...
  if ((vc_gfx = gfx_create_surface((void*)ptr, width, height - font->height, stride, format, 0)) ==
      NULL) {
    goto fail;
  }
