...
#ifdef DEPRECATE_COMPAT_SYSCALLS
#define __MX_SYSCALL static inline __attribute__((deprecated))
#else
#define __MX_SYSCALL static inline
#endif

__MX_SYSCALL mx_status_t mx_message_pipe_create(mx_handle_t* out_handles, uint32_t flags) {
    return mx_msgpipe_create(out_handles, flags);
}
__MX_SYSCALL mx_status_t mx_message_read(mx_handle_t handle, void* bytes, uint32_t* num_bytes,
...
__MX_SYSCALL mx_status_t mx_message_write(mx_handle_t handle, const void* bytes, uint32_t num_bytes,
...
__MX_SYSCALL mx_handle_t mx_interrupt_event_create(mx_handle_t handle, uint32_t vector, uint32_t flags) {
...
__MX_SYSCALL mx_status_t mx_process_vm_unmap(mx_handle_t proc_handle, uintptr_t address, mx_size_t len) {
    return mx_process_unmap_vm(proc_handle, address, len);
}
__MX_SYSCALL mx_status_t mx_process_vm_protect(mx_handle_t proc_handle, uintptr_t address, mx_size_t len, uint32_t prot) {
    return mx_process_protect_vm(proc_handle, address, len, prot);
}

__MX_SYSCALL mx_handle_t mx_vm_object_create(uint64_t size) {
    return mx_vmo_create(size);
}
__MX_SYSCALL mx_ssize_t mx_vm_object_read(mx_handle_t handle, void *data, uint64_t offset, mx_size_t len) {
    return mx_vmo_read(handle, data, offset, len);
}
__MX_SYSCALL mx_ssize_t mx_vm_object_write(mx_handle_t handle, const void *data, uint64_t offset, mx_size_t len) {
    return mx_vmo_write(handle, data, offset, len);
}
__MX_SYSCALL mx_status_t mx_vm_object_get_size(mx_handle_t handle, uint64_t* size) {
    return mx_vmo_get_size(handle, size);
}
__MX_SYSCALL mx_status_t mx_vm_object_set_size(mx_handle_t handle, uint64_t size) {
    return mx_vmo_set_size(handle, size);
}

__MX_SYSCALL mx_handle_t mx_io_port_create(uint32_t options) {
    return mx_port_create(options);
}
__MX_SYSCALL mx_status_t mx_io_port_queue(mx_handle_t handle, const void* packet, mx_size_t size) {
    return mx_port_queue(handle, packet, size);
}
__MX_SYSCALL mx_status_t mx_io_port_wait(mx_handle_t handle, void* packet, mx_size_t size) {
    return mx_port_wait(handle, packet, size);
}
__MX_SYSCALL mx_status_t mx_io_port_bind(mx_handle_t handle, uint64_t key, mx_handle_t source, mx_signals_t signals) {
    return mx_port_bind(handle, key, source, signals);
}


__MX_SYSCALL mx_handle_t mx_data_pipe_create(uint32_t options, mx_size_t element_size, mx_size_t capacity, mx_handle_t* handle) {
    return mx_datapipe_create(options, element_size, capacity, handle);
}
__MX_SYSCALL mx_ssize_t mx_data_pipe_write(mx_handle_t handle, uint32_t flags, mx_size_t requested, const void* buffer) {
    return mx_datapipe_write(handle, flags, requested, buffer);
}
__MX_SYSCALL mx_ssize_t mx_data_pipe_read(mx_handle_t handle, uint32_t flags, mx_size_t requested, void* buffer) {
...
__MX_SYSCALL mx_handle_t mx_wait_set_create(void) {
    return mx_waitset_create();
}
__MX_SYSCALL mx_status_t mx_wait_set_add(mx_handle_t waitset_handle, mx_handle_t handle, mx_signals_t signals, uint64_t cookie) {
    return mx_waitset_add(waitset_handle, handle, signals, cookie);
}
__MX_SYSCALL mx_status_t mx_wait_set_remove(mx_handle_t waitset_handle, uint64_t cookie) {
