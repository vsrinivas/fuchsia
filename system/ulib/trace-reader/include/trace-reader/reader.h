...
public:
    ...
    using RecordConsumer = mxtl::Function<void(Record)>;

    // Callback invoked when decoding errors are detected in the trace.
    using ErrorHandler = mxtl::Function<void(mxtl::String)>;
...
private:
    ...
    void RegisterProvider(ProviderId id, mxtl::String name);
    void RegisterString(trace_string_index_t index, mxtl::String string);
    ...
    void ReportError(mxtl::String error) const;
    ...
    struct StringTableEntry : public mxtl::SinglyLinkedListable<
                                  mxtl::unique_ptr<StringTableEntry>> {
        StringTableEntry(trace_string_index_t index,
                         mxtl::String string)
            : index(index), string(mxtl::move(string)) {}

        trace_string_index_t const index;
        mxtl::String const string;
    ...
    struct ThreadTableEntry : public mxtl::SinglyLinkedListable<
                                  mxtl::unique_ptr<ThreadTableEntry>> {
    ...
    struct ProviderInfo : public mxtl::SinglyLinkedListable<mxtl::unique_ptr<ProviderInfo>> {
        ProviderId id;
        mxtl::String name;
        ...
        // individually.
        mxtl::HashTable<trace_string_index_t, mxtl::unique_ptr<StringTableEntry>> string_table;
        mxtl::HashTable<trace_thread_index_t, mxtl::unique_ptr<ThreadTableEntry>> thread_table;
    ...
    mxtl::HashTable<ProviderId, mxtl::unique_ptr<ProviderInfo>> providers_;
...
public:
    ...
    bool ReadString(size_t length, mxtl::StringPiece* out_string);
