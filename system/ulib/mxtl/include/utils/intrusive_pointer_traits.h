...
struct ContainerPtrTraits<::mxtl::unique_ptr<T, Deleter>> {
    using ValueType       = T;
    using RefType         = T&;
    using ConstRefType    = const T&;
    using PtrType         = ::mxtl::unique_ptr<T, Deleter>;
    using ConstPtrType    = ::mxtl::unique_ptr<const T, Deleter>;
    using RawPtrType      = T*;
    using ConstRawPtrType = const T*;
...
struct ContainerPtrTraits<::mxtl::RefPtr<T>> {
    ...
    static constexpr PtrType MakeSentinel(void* sentinel) {
        return ::mxtl::internal::MakeRefPtrNoAdopt(reinterpret_cast<RawPtrType>(
