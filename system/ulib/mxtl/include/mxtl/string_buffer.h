...
//
// mxtl::StringBuffer is designed to resemble std::string except that it
...
public:
    ...
    // The result is truncated if the appended content does not fit completely.
    StringBuffer& Append(const mxtl::StringPiece& piece) {
    ...
    // The result is truncated if the appended content does not fit completely.
    StringBuffer& Append(const mxtl::String& other) {
    ...
    mxtl::String ToString() const {
        return mxtl::String(data(), length());
    }

    // Gets the buffer's contents as a string piece.
    mxtl::StringPiece ToStringPiece() const {
        return mxtl::StringPiece(data(), length());
    }
