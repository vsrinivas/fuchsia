...
//
// mxtl::DoublyLinkedList<> is a templated intrusive container class which
// allows users to manage doubly linked lists of objects.
//
// mxtl::DoublyLinkedList<> follows the same patterns as
// mxtl::SinglyLinkedList<> and implements a superset of the functionality
...
public:
    ...
    void push_front(PtrType&& ptr) { internal_insert(PtrTraits::GetRaw(head_), mxtl::move(ptr)); }

    // push_back : Push an element onto the end of the list.
    void push_back(const PtrType& ptr) { push_back(PtrType(ptr)); }
    void push_back(PtrType&& ptr) { internal_insert(sentinel(), mxtl::move(ptr)); }
    ...
    void insert(const iterator& iter, PtrType&& ptr) {
        internal_insert(iter.node_, mxtl::move(ptr));
    }
    ...
    void insert_after(const iterator& iter, PtrType&& ptr) {
        ...
        internal_insert(PtrTraits::GetRaw(ns.next_), mxtl::move(ptr));
    }
...
private:
    ...
    void internal_insert(RawPtrType before, PtrType&& ptr) {
        ...
        // Update the next pointers.
        ptr_ns.next_ = mxtl::move(tgt_next);
        tgt_next     = mxtl::move(ptr);
    }
