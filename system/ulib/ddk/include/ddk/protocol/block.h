...
typedef struct block_protocol_ops {
    ...
    void (*read)(void* ctx, uint32_t flags, zx_handle_t vmo, uint64_t length, uint64_t vmo_offset,
                 uint64_t dev_offset, void* cookie);
    void (*write)(void* ctx, uint32_t flags, zx_handle_t vmo, uint64_t length, uint64_t vmo_offset,
                  uint64_t dev_offset, void* cookie);
...
static inline void block_read(block_protocol_t* block, uint32_t flags, zx_handle_t vmo,
                              uint64_t length, uint64_t vmo_offset, uint64_t dev_offset,
                              void* cookie) {
    block->ops->read(block->ctx, flags, vmo, length, vmo_offset, dev_offset, cookie);
}


// Write from the VMO to the block device
static inline void block_write(block_protocol_t* block, uint32_t flags, zx_handle_t vmo,
                               uint64_t length, uint64_t vmo_offset, uint64_t dev_offset,
                               void* cookie) {
    block->ops->write(block->ctx, flags, vmo, length, vmo_offset, dev_offset, cookie);
}
