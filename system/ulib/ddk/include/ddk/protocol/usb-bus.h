...
static inline mx_status_t usb_bus_add_device(usb_bus_protocol_t* bus, uint32_t device_id,
                                             uint32_t hub_id, usb_speed_t speed) {
    return bus->ops->add_device(bus->ctx, device_id, hub_id, speed);
}
static inline void usb_bus_remove_device(usb_bus_protocol_t* bus, uint32_t device_id) {
