...
zx_status_t HMAC::Verify(digest::Algorithm digest, const Bytes& key, const void* in, size_t in_len,
    ...
    if (hmac_len != tmp_len) {
        ...
        if ((flags & ALLOW_TRUNCATION) == 0 || hmac_len < tmp_len / 2 || hmac_len < 16) {
            xprintf("%s: digest to verify is too short: %zu\n", __PRETTY_FUNCTION__, hmac_len);
    ...
    if (tmp != hmac) {
        xprintf("%s: HMAC verification failed\n", __PRETTY_FUNCTION__);
