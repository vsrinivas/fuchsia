...
void sync_mtx_lock(sync_mtx_t* mutex) __TA_ACQUIRE(mutex);

// Locks the mutex and mark the mutex as having a waiter.
//
// Similar to |sync_mtx_lock| but markes the mutex as having a waiter. Intended
// to be used by the condition variable implementation.
void sync_mtx_lock_with_waiter(sync_mtx_t* mutex) __TA_ACQUIRE(mutex);
...
void sync_mtx_unlock(sync_mtx_t* mutex) __TA_RELEASE(mutex);
