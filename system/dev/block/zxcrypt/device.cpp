...
Device::Device(zx_device_t* parent)
    : DeviceType(parent), state_(0), info_(nullptr), head_(nullptr), tail_(nullptr), hint_(0) {}
...
void Device::BlockQueue(block_op_t* block) {
    ...
    memset(extra, 0, sizeof(extra_op_t));
...
void Device::EnqueueWrite(block_op_t* block) {
    ...
    extra_op_t* prev = nullptr;
    for (extra = head_; extra; extra = extra->next) {
    ...
    // Split off portion of queue that's ready
    extra_op_t* pending = head_;
    head_ = extra;
    if (!head_) {
        tail_ = nullptr;
    }
    ...
    lock.release();
    while (pending) {
        SendToWorker(ExtraToBlock(pending, info_->op_size));
