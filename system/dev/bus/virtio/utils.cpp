...
zx_status_t map_contiguous_memory(size_t size, uintptr_t* _va, zx_paddr_t* _pa) {
    ...
    if (r) {
        zxlogf(ERROR, "zx_vmo_create_contiguous failed %d\n", r);
        return r;
    }
    ...
    if (r) {
        zxlogf(ERROR, "zx_process_map_vm failed %d size: %zu\n", r, size);
        return r;
    }
...
// TODO: add when LOCK operation implemented in kernel
    ...
    if (r) {
        zxlogf(ERROR, "zx_vmo_op_range LOCK failed %d\n", r);
        return r;
    }
#endif
    ...
    if (r) {
        zxlogf(ERROR, "zx_vmo_op_range LOOKUP failed %d\n", r);
        return r;
    }
