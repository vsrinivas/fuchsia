...
bool good_const_test_enum() {
    BEGIN_TEST;

    TestLibrary library(R"FIDL(
library example;

enum foo : int32 { A = 1; B = 2; };
const foo c = 2;
)FIDL");
    ASSERT_TRUE(library.Compile());
...
bool bad_const_test_enum_with_string() {
    BEGIN_TEST;
...
const foo c = "nope";
    ...
    auto errors = library.errors();
    ASSERT_EQ(errors.size(), 1);
    ASSERT_STR_STR(errors[0].c_str(), "cannot convert \"nope\" (type string:4) to type example/foo");
...
RUN_TEST(good_const_test_enum);
RUN_TEST(bad_const_test_enum_with_string);
