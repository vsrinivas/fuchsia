...
bool test_persist_simple(void) {
    ...
    for (ssize_t i = countof(paths) - 1; i >= 0; i--) {
        if (is_directory(paths[i])) {
...
bool test_persist_with_data(void) {
    ...
    mxtl::unique_ptr<uint8_t[]> buffers[countof(files)];
...
RUN_FOR_ALL_FILESYSTEMS(persistence_tests,
    ...
    RUN_TEST_MEDIUM((test_persist_with_data<8192 + 1>))
    RUN_TEST_MEDIUM((test_persist_with_data<8192 * 128>))
    RUN_TEST_MEDIUM((test_rename_loop<false, 2, 2>));
    RUN_TEST_MEDIUM((test_rename_loop<false, 2, 100>));
    RUN_TEST_MEDIUM((test_rename_loop<false, 15, 100>));
    RUN_TEST_MEDIUM((test_rename_loop<false, 25, 500>));
    ...
    RUN_TEST_MEDIUM((test_rename_loop<true, 25, 500>));
)
