...
static bool basic_test_op(int mode) {
    BEGIN_TEST;
    mx::timer timer;
...
static bool restart_test_op(int mode) {
    BEGIN_TEST;
    mx::timer timer;
    ASSERT_EQ(mx::timer::create(0, MX_CLOCK_MONOTONIC, &timer), MX_OK);
...
static bool edge_cases() {
    ...
    mx::timer timer;
    ASSERT_EQ(mx::timer::create(0, MX_CLOCK_MONOTONIC, &timer), MX_OK);
    ASSERT_EQ(timer.start(0u, 0u, 0u), MX_OK);
    ASSERT_EQ(timer.start(0U, MX_TIMER_MIN_PERIOD, 0u), MX_OK);
...
// the signal.
static bool signals_asserted_immediately_op(int mode) {
    BEGIN_TEST;
    mx::timer timer;
    ASSERT_EQ(mx::timer::create(0, MX_CLOCK_MONOTONIC, &timer), MX_OK);
...
static bool basic_test_set() {
    return basic_test_op(USE_SET);
}

static bool restart_test_set() {
    return restart_test_op(USE_SET);
}

static bool signals_asserted_immediately_set() {
    return signals_asserted_immediately_op(USE_SET);
}

static bool basic_test_start() {
    return basic_test_op(USE_START);
}

static bool restart_test_start() {
    return restart_test_op(USE_START);
}

static bool signals_asserted_immediately_start() {
    return signals_asserted_immediately_op(USE_START);
}


BEGIN_TEST_CASE(timers_test)
RUN_TEST(invalid_calls)
RUN_TEST(basic_test_set)
RUN_TEST(basic_test_start)
RUN_TEST(restart_test_set)
RUN_TEST(restart_test_start)
RUN_TEST(edge_cases)
RUN_TEST(periodic)
RUN_TEST(restart_race)
RUN_TEST(signals_asserted_immediately_set)
RUN_TEST(signals_asserted_immediately_start)
