...
struct UnmanagedTestTraits {
    ...
    class ObjType : public TestBase,
                    public mxtl::SlabAllocated<AllocTraits>,
                    public mxtl::DoublyLinkedListable<PtrType> {
    public:
        ObjType()                                     : TestBase() { }
...
struct UniquePtrTestTraits {
    class ObjType;
    using PtrType       = mxtl::unique_ptr<ObjType>;
...
struct RefPtrTestTraits {
    class ObjType;
    using PtrType       = mxtl::RefPtr<ObjType>;
    using AllocTraits   = mxtl::SlabAllocatorTraits<PtrType, 1024, LockType>;
    using AllocatorType = mxtl::SlabAllocator<AllocTraits>;
    using RefList       = mxtl::DoublyLinkedList<PtrType>;

    class ObjType : public TestBase,
                    public mxtl::RefCounted<ObjType>,
                    public mxtl::SlabAllocated<AllocTraits>,
                    public mxtl::DoublyLinkedListable<PtrType> {
    public:
        ObjType()                                     : TestBase() { }
...
bool do_slab_test(typename Traits::AllocatorType& allocator, size_t test_allocs) {
    ...
    for (size_t i = 0; i < test_allocs; ++i) {
        ...
        case 2: ptr = allocator.New(mxtl::move(i)); break;
        case 3: ptr = allocator.New(i, mxtl::move(i)); break;
        }

        if (i < MAX_ALLOCS) {
            ASSERT_NONNULL(ptr, "Allocation failed when it should not have!");
            ref_list.push_front(mxtl::move(ptr));
        } else {
...
struct StaticUnmanagedTestTraits {
    class ObjType;
    using PtrType       = ObjType*;
    using AllocTraits   = mxtl::StaticSlabAllocatorTraits<PtrType, 1024, LockType>;
    using AllocatorType = mxtl::SlabAllocator<AllocTraits>;
    using RefList       = mxtl::DoublyLinkedList<PtrType>;
...
bool do_static_slab_test(size_t test_allocs) {
    ...
    for (size_t i = 0; i < test_allocs; ++i) {
        ...
        case 2: ptr = AllocatorType::New(mxtl::move(i)); break;
        case 3: ptr = AllocatorType::New(i, mxtl::move(i)); break;
        }
...
using MutexLock = ::mxtl::Mutex;
using NullLock  = ::mxtl::NullLock;
