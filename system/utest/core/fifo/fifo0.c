...
static bool basic_test(void) {
    BEGIN_TEST;
    mx_handle_t fifo;
    ASSERT_EQ(mx_fifo0_create(0, &fifo), ERR_INVALID_ARGS, "Error during fifo create");
    ASSERT_EQ(mx_fifo0_create(3, &fifo), ERR_INVALID_ARGS, "Error during fifo create");
    ASSERT_EQ(mx_fifo0_create(4, &fifo), 0, "Error during fifo create");

    mx_fifo_state_t state;
    reset_state(&state);
    ASSERT_EQ(mx_fifo0_op(fifo, MX_FIFO_OP_READ_STATE, 0, &state), 0, "Error getting fifo state");
    ...
    reset_state(&state);
    ASSERT_EQ(mx_fifo0_op(fifo, MX_FIFO_OP_ADVANCE_HEAD, 1, &state), 0, "Error advancing head");
    ...
    reset_state(&state);
    ASSERT_EQ(mx_fifo0_op(fifo, MX_FIFO_OP_ADVANCE_HEAD, 3, &state), 0, "Error advancing head");
...
static bool advance_too_many_test(void) {
    ...
    ASSERT_EQ(mx_fifo0_op(fifo, MX_FIFO_OP_ADVANCE_HEAD, 5, &state), ERR_OUT_OF_RANGE,
            "Error advancing head");
    ASSERT_EQ(state.head, 0u, "Error advancing head");
    ASSERT_EQ(state.tail, 0u, "Error advancing head");

    reset_state(&state);
    ASSERT_EQ(mx_fifo0_op(fifo, MX_FIFO_OP_ADVANCE_TAIL, 1, &state), ERR_OUT_OF_RANGE,
    ...
    reset_state(&state);
    ASSERT_EQ(mx_fifo0_op(fifo, MX_FIFO_OP_ADVANCE_HEAD, 2, &state), 0, "Error advancing head");
    ASSERT_EQ(state.head, 2u, "Error advancing head");
    ASSERT_EQ(state.tail, 0u, "Error advancing head");

    reset_state(&state);
    ASSERT_EQ(mx_fifo0_op(fifo, MX_FIFO_OP_ADVANCE_TAIL, 3, &state), ERR_OUT_OF_RANGE,
            "Error advancing tail");
...
static bool restrict_rights_test(void) {
    ...
    ASSERT_EQ(mx_fifo0_op(consumer, MX_FIFO_OP_ADVANCE_HEAD, 1u, NULL), ERR_ACCESS_DENIED,
            "Error advancing head (should have been denied)");

    // move head so fifo is not empty
    ASSERT_EQ(mx_fifo0_op(producer, MX_FIFO_OP_ADVANCE_HEAD, 1u, NULL), 0, "Error advancing head");

    // producer can't move tail
    ASSERT_EQ(mx_fifo0_op(producer, MX_FIFO_OP_ADVANCE_TAIL, 1u, NULL), ERR_ACCESS_DENIED,
...
static int thread_consumer(void* arg) {
    ...
    ASSERT_EQ(mx_fifo0_op(fifo, MX_FIFO_OP_ADVANCE_TAIL, 1u, NULL), 0, "Error advancing tail");
...
static bool exception_test(void) {
    ...
    reset_state(&state);
    ASSERT_EQ(mx_fifo0_op(fifo, MX_FIFO_OP_PRODUCER_EXCEPTION, 1u, &state), 0,
    ...
    reset_state(&state);
    ASSERT_EQ(mx_fifo0_op(fifo, MX_FIFO_OP_PRODUCER_EXCEPTION, 0u, &state), 0,
    ...
    reset_state(&state);
    ASSERT_EQ(mx_fifo0_op(fifo, MX_FIFO_OP_CONSUMER_EXCEPTION, 1u, &state), 0,
    ...
    reset_state(&state);
    ASSERT_EQ(mx_fifo0_op(fifo, MX_FIFO_OP_CONSUMER_EXCEPTION, 0u, &state), 0,
