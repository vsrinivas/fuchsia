...
static bool test_resource_actions(void) {
    ...
    size_t count;
    ASSERT_EQ(mx_object_get_info(rrh, MX_INFO_RESOURCE_RECORDS, &rec, sizeof(rec), &count, 0), MX_OK, "");
    ...
    ASSERT_EQ(check_signals(rrh, MX_RESOURCE_WRITABLE), MX_OK, "");
    ...
    ASSERT_EQ(status, MX_OK, "cannot create child resource");

    // verify that we're notified about the creation
    ASSERT_EQ(mx_port_wait(ph, 0, &pkt, 0), MX_OK, "");
    ASSERT_EQ(mx_port_wait(ph, 0, &pkt, 0), MX_ERR_TIMED_OUT, "");
    ...
    for (int n = 0; n < 5; n++) {
        ...
        ASSERT_EQ(mx_resource_create(rh, &r0, 1, &ch[n]), MX_OK, "");
    }

    // enumerate children
    ASSERT_EQ(mx_object_get_info(rh, MX_INFO_RESOURCE_CHILDREN, rec, sizeof(rec), &count, 0), MX_OK, "");
    ASSERT_EQ(count, 5u, "");
    ...
    for (int n = 0; n < 5; n++) {
        ...
        ASSERT_EQ(mx_object_get_info(ch[n], MX_INFO_HANDLE_BASIC, &info, sizeof(info), NULL, NULL), MX_OK, "");
        ASSERT_EQ(info.koid, rec[n].self.koid, "");
        mx_handle_close(ch[n]);

        mx_handle_t h;
        ASSERT_EQ(mx_object_get_child(rh, info.koid, MX_RIGHT_SAME_RIGHTS, &h), MX_OK, "");

        mx_info_handle_basic_t info2;
        ASSERT_EQ(mx_object_get_info(h, MX_INFO_HANDLE_BASIC, &info2, sizeof(info2), NULL, NULL), MX_OK, "");
    ...
    mx_handle_t h;
    ASSERT_EQ(mx_object_get_child(rh, rec[2].self.koid, MX_RIGHT_SAME_RIGHTS, &h), MX_OK, "");

    // verify that our signals are in the expected state
    ASSERT_EQ(check_signals(h, MX_RESOURCE_WRITABLE), MX_OK, "");

    ASSERT_EQ(mx_resource_destroy(h), MX_OK, "");

    // verify that our signals are in the expected state
    ASSERT_EQ(check_signals(h, MX_RESOURCE_WRITABLE | MX_RESOURCE_DESTROYED), MX_OK, "");
    ...
    ASSERT_EQ(mx_handle_close(h), MX_OK, "");

    // check that bogus children are disallowed
    r0.self.subtype = 0xabcd;
    status = mx_resource_create(rrh, &r0, 1, &rh);
    ASSERT_EQ(status, MX_ERR_INVALID_ARGS, "creation of bogus resource succeeded");


    ASSERT_EQ(mx_object_get_info(rrh, MX_INFO_RESOURCE_CHILDREN, rec, sizeof(rec), &count, 0), MX_OK, "");
    END_TEST;
}
...
static bool test_resource_connect(void) {
    ...
    ASSERT_EQ(mx_resource_accept(rrh, &h), MX_ERR_SHOULD_WAIT, "");

    // should initially be writable (connect queue empty);
    ASSERT_EQ(check_signals(rrh, MX_RESOURCE_WRITABLE), MX_OK, "");

    mx_handle_t c0, c1;
    ASSERT_EQ(mx_channel_create(0, &c0, &c1), MX_OK, "");
    // verify that non-channel handle is rejected
    ASSERT_EQ(mx_resource_connect(rrh, rrh), MX_ERR_WRONG_TYPE, "");
    // verify that a channel handle is accepted
    ASSERT_EQ(mx_resource_connect(rrh, c0), MX_OK, "");
    ASSERT_EQ(check_signals(rrh, MX_RESOURCE_READABLE), MX_OK, "");
    // verify that the accepted handle is gone and a bad handle is rejected
    ASSERT_EQ(mx_resource_connect(rrh, c0), MX_ERR_BAD_HANDLE, "");
    // the connect queue is only one deep, so this must fail
    ASSERT_EQ(mx_resource_connect(rrh, c1), MX_ERR_SHOULD_WAIT, "");

    mx_handle_t c;
    // accept succeeds when there's a handle in the queue
    ASSERT_EQ(mx_resource_accept(rrh, &c), MX_OK, "");
