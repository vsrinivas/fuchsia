...
class PagedVmo : public fbl::DoublyLinkedListable<fbl::unique_ptr<PagedVmo>> {
public:
    ~PagedVmo() = default;
...
private:
    PagedVmo(zx::vmo vmo, uint64_t size, uint64_t* base, uint64_t base_addr, uint64_t base_val)
...
public:
    ...
    bool CreateVmo(uint64_t size, PagedVmo** vmo_out);
    // Destroyes the paged vmo.
    void ReleaseVmo(PagedVmo* vmo);
    // Unmaps the paged vmo.
    bool UnmapVmo(PagedVmo* vmo);
    // Replaces the paged vmo's mapping with new content.
    bool ReplaceVmo(PagedVmo* vmo, zx::vmo* old_vmo);
    ...
    bool SupplyPages(PagedVmo* vmo, uint64_t page_offset, uint64_t page_count,
    ...
    // wait until |deadline|.
    bool WaitForPageRead(PagedVmo* vmo, uint64_t page_offset,
                         uint64_t page_count, zx_time_t deadline);

    // Gets the first page read request. Blocks until |deadline|.
    bool GetPageReadRequest(PagedVmo* vmo, zx_time_t deadline,
...
private:
    ...
    fbl::DoublyLinkedList<fbl::unique_ptr<PagedVmo>> vmos_;
