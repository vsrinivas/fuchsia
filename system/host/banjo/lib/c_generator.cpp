...
void EmitFileComment(std::ostream* file) {
    *file << "// WARNING: This file is machine generated by banjoc.\n\n";
}
...
void EmitServerDispatchDecl(std::ostream* file, StringView interface_name) {
    *file << "zx_status_t " << interface_name
          << "_dispatch(void* ctx, banjo_txn_t* txn, banjo_msg_t* msg, const "
          << interface_name << "_ops_t* ops)";
}

void EmitServerTryDispatchDecl(std::ostream* file, StringView interface_name) {
    *file << "zx_status_t " << interface_name
          << "_try_dispatch(void* ctx, banjo_txn_t* txn, banjo_msg_t* msg, const "
          << interface_name << "_ops_t* ops)";
}

void EmitServerReplyDecl(std::ostream* file, StringView method_name,
                         const std::vector<CGenerator::Member>& response) {
    *file << "zx_status_t " << method_name << "_reply(banjo_txn_t* _txn";
...
void EmitMeasureInParams(std::ostream* file,
    ...
    for (const auto& member : params) {
        if (member.kind == flat::Type::Kind::kVector)
            *file << " + BANJO_ALIGN(sizeof(*" << member.name << "_data) * " << member.name << "_count)";
        else if (member.kind == flat::Type::Kind::kString)
            *file << " + BANJO_ALIGN(" << member.name << "_size)";
...
void EmitMeasureOutParams(std::ostream* file,
    ...
    for (const auto& member : params) {
        if (member.kind == flat::Type::Kind::kVector)
            *file << " + BANJO_ALIGN(sizeof(*" << member.name << "_buffer) * " << member.name << "_capacity)";
        else if (member.kind == flat::Type::Kind::kString)
            *file << " + BANJO_ALIGN(" << member.name << "_capacity)";
        else if (IsStoredOutOfLine(member))
...
void CGenerator::GeneratePrologues() {
    ...
    EmitIncludeHeader(&file_, "<zircon/banjo.h>");
...
void CGenerator::GenerateTaggedUnionDeclaration(StringView name,
    ...
    file_ << "struct " << name << " {\n";
    file_ << kIndent << "BANJO_ALIGNDECL\n";
    file_ << kIndent << "banjo_union_tag_t tag;\n";
    file_ << kIndent << "union {\n";
    for (const auto& member : members) {
...
void CGenerator::ProduceInterfaceExternDeclaration(const NamedInterface& named_interface) {
    for (const auto& method_info : named_interface.methods) {
        if (method_info.request)
            file_ << "extern const banjo_type_t " << method_info.request->coded_name << ";\n";
        if (method_info.response)
            file_ << "extern const banjo_type_t " << method_info.response->coded_name
                  << ";\n";
    }
}
...
void CGenerator::ProduceInterfaceClientImplementation(const NamedInterface& named_interface) {
    for (const auto& method_info : named_interface.methods) {
        ...
        file_ << ";\n";
        file_ << kIndent << "BANJO_ALIGNDECL char _wr_bytes[_wr_num_bytes];\n";
        ...
        if (encode) {
            ...
            file_ << kIndent << "zx_status_t _status = banjo_encode(&" << method_info.request->coded_name
            ...
            file_ << kIndent << "// OPTIMIZED AWAY banjo_encode() of POD-only request\n";
        }
        if (!method_info.response) {
            if (encode) {
            ...
            file_ << ";\n";
            file_ << kIndent << "BANJO_ALIGNDECL char _rd_bytes[_rd_num_bytes];\n";
            if (!encode) {
                file_ << kIndent << "zx_handle_t _handles[ZX_CHANNEL_MAX_MSG_HANDLES];\n";
            }
            if (!response.empty())
            ...
            if (count > 0u) {
                ...
                for (const auto& member : response) {
                    if (member.kind == flat::Type::Kind::kVector) {
                        ...
                        if (i++ > 0u)
                            file_ << " || ";
                        file_ << "((uintptr_t)_response->" << member.name << " == BANJO_ALLOC_PRESENT && out_" << member.name << " == NULL)";
                    }
                }
                if (count > 1u)
                    file_ << ")";
            ...
            if ((count == 0) && (hcount == 0)) {
                file_ << kIndent << "// OPTIMIZED AWAY banjo_decode() of POD-only response\n";
                file_ << kIndent << "if (_actual_num_handles > 0) {\n";
                file_ << kIndent << kIndent << "zx_handle_close_many(_handles, _actual_num_handles);\n";
                file_ << kIndent << kIndent << "return ZX_ERR_INTERNAL;\n"; // WHAT ERROR?
                file_ << kIndent << "}\n";
            } else {
                // TODO(BANJO-162): Validate the response ordinal. C++ bindings also need to do that.
                file_ << kIndent << "_status = banjo_decode(&" << method_info.response->coded_name
...
void CGenerator::ProduceInterfaceServerImplementation(const NamedInterface& named_interface) {
    EmitServerTryDispatchDecl(&file_, named_interface.c_name);
    file_ << " {\n";
    file_ << kIndent << "if (msg->num_bytes < sizeof(banjo_message_header_t)) {\n";
    file_ << kIndent << kIndent << "zx_handle_close_many(msg->handles, msg->num_handles);\n";
    file_ << kIndent << kIndent << "return ZX_ERR_INVALID_ARGS;\n";
    file_ << kIndent << "}\n";
    file_ << kIndent << "banjo_message_header_t* hdr = (banjo_message_header_t*)msg->bytes;\n";
    ...
    for (const auto& method_info : named_interface.methods) {
        ...
        file_ << kIndent << kIndent << "status = banjo_decode_msg(&" << method_info.request->coded_name << ", msg, NULL);\n";
    ...
    for (const auto& method_info : named_interface.methods) {
        ...
        file_ << kIndent << "banjo_msg_t _msg = {\n";
        ...
        file_ << kIndent << "};\n";
        if (encode) {
            file_ << kIndent << "zx_status_t _status = banjo_encode_msg(&"
                  << method_info.response->coded_name << ", &_msg, &_msg.num_handles, NULL);\n";
            file_ << kIndent << "if (_status != ZX_OK)\n";
            file_ << kIndent << kIndent << "return _status;\n";
        } else {
            file_ << kIndent << "// OPTIMIZED AWAY banjo_encode() of POD-only reply\n";
        }
...
std::ostringstream CGenerator::ProduceHeader() {
    ...
    for (const auto* decl : library_->declaration_order_) {
        ...
        case flat::Decl::Kind::kTable:
        case flat::Decl::Kind::kUnion:
            // Only messages have extern banjo_type_t declarations.
            break;
        case flat::Decl::Kind::kInterface: {
            auto iter = named_interfaces.find(decl);
            if (iter != named_interfaces.end()) {
...
std::ostringstream CGenerator::ProduceClient() {
    EmitFileComment(&file_);
    EmitIncludeHeader(&file_, "<lib/banjo/coding.h>");
