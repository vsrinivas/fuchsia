...
static zx_status_t crashanalyzer_run(const char* name, int argc, const char* const* argv) {
    launchpad_t *lp;
    ...
    if (status != ZX_OK) {
        ...
        if (signals & ZX_TASK_TERMINATED) {
            ...
            if (status == ZX_OK && info.exited) {
                ...
                // This shouldn't happen, but we don't want to kill crashanalyzer
...
void try_dump_pt_data() {
    ...
    zx_status_t status = crashanalyzer_run("ipt-dump",
                                         static_cast<int>(fbl::count_of(argv_pt_dump)),
                                         argv_pt_dump);
    if (status == ZX_OK) {
        printf("PT output written to " PT_PATH_FORMAT ".*\n",
               pt_path_prefix, seq_num);
    } else {
        fprintf(stderr, "crashanalyzer: error dumping IPT data: %s\n",
