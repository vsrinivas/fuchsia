...
A component almost always implements the **ServiceProvider** interface, which
...
A **Connection** is the request sent to a FIDL component to return its
**ServiceProvider** interface to another component. Requests for subsequent
interfaces are _not_ connections. A connection refers to the construction of the
initial channel between two components.
...
_first_ channel with another component. Strictly speaking, the connection is
complete when both components have bound their ServiceProvider to that first
