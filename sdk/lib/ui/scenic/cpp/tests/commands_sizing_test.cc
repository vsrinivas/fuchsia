...
TEST(CommandsSizingTest, SendPointerInputCmd) {
  ...
  EXPECT_EQ(size.num_bytes, 88 /* 16 + 16 + 56 */);
...
TEST(CommandsSizingTest, CreateResourceCmdWithMemoryArgs) {
  ...
  auto size = Measure(cmd);
  EXPECT_EQ(size.num_bytes, 80 /* 16 + 16 + 8 + 16 + 24 */);
...
TEST(CommandsSizingTest, ViewsCmd) {
  ...
  ::fuchsia::ui::views::Command views_cmd;  // 16 bytes
  views_cmd.set_empty(6);

  ::fuchsia::ui::scenic::Command cmd;  // 16 + 16 bytes
  cmd.set_views(std::move(views_cmd));

  auto size = Measure(cmd);
  EXPECT_EQ(size.num_bytes, 32 /* 16 + 16 */);
  EXPECT_EQ(size.num_handles, 0);
}
...
TEST(CommandsSizingTest, CreateResourceCmdWithViewArgs3) {
  ...
  ::fuchsia::ui::gfx::ResourceArgs resource_args;  // 16 + 32 bytes
  resource_args.set_view3(std::move(view_args3));

  ::fuchsia::ui::gfx::CreateResourceCmd create_resource_cmd;  // 8 + 16 + 32 bytes
  create_resource_cmd.resource = std::move(resource_args);

  ::fuchsia::ui::gfx::Command gfx_cmd;  // 16 + 8 + 16 + 32 bytes
  gfx_cmd.set_create_resource(std::move(create_resource_cmd));

  ::fuchsia::ui::scenic::Command cmd;  // 16 + 16 + 8 + 16 + 32 bytes
  cmd.set_gfx(std::move(gfx_cmd));

  auto size = Measure(cmd);
  EXPECT_EQ(size.num_bytes, 88 /* 16 + 16 + 8 + 16 + 32 */);
...
TEST(CommandsSizingTest, CreateResourceCmdWithViewArgs3WithDebugName) {
  ...
  ::fuchsia::ui::gfx::ResourceArgs resource_args;  // 16 + 32 + 16 bytes
  resource_args.set_view3(std::move(view_args3));

  ::fuchsia::ui::gfx::CreateResourceCmd create_resource_cmd;  // 8 + 16 + 32 + 16 bytes
  create_resource_cmd.resource = std::move(resource_args);

  ::fuchsia::ui::gfx::Command gfx_cmd;  // 16 + 8 + 16 + 32 + 16 bytes
  gfx_cmd.set_create_resource(std::move(create_resource_cmd));

  ::fuchsia::ui::scenic::Command cmd;  // 16 + 16 + 8 + 16 + 32 + 16 bytes
  cmd.set_gfx(std::move(gfx_cmd));

  auto size = Measure(cmd);
  EXPECT_EQ(size.num_bytes, 104 /* 16 + 16 + 8 + 16 + 32 + 16 */);
  EXPECT_EQ(size.num_handles, 3);
}
...
TEST(CommandsSizingTest, CreateResourceCmdWithSetStereoCameraProjectionCmd) {
  ...
  ::fuchsia::ui::gfx::Command gfx_cmd;  // 16 + 144
  gfx_cmd.set_set_stereo_camera_projection(std::move(stereo_camera_projection_cmd));

  ::fuchsia::ui::scenic::Command cmd;  // 16 + 16 + 144
  cmd.set_gfx(std::move(gfx_cmd));

  auto size = Measure(cmd);
  EXPECT_EQ(size.num_bytes, 176);
  EXPECT_EQ(size.num_handles, 0);
}
