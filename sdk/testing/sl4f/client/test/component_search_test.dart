...
void main(List<String> args) {
  ...
  test('call Search with Component not running', () {
    ...
    expect(
        ComponentSearch(sl4f).search('fake_name'), completion(equals(false)));
  });
  ...
  test('call Search with Component running', () {
    ...
    expect(ComponentSearch(sl4f).search('running_comp.cmx'),
  ...
  test('call List to list running component', () {
    ...
    void handler(HttpRequest req) async {
      expect(req.contentLength, greaterThan(0));
      final body = jsonDecode(await utf8.decoder.bind(req).join());
      expect(body['method'], 'component_search_facade.List');
    ...
    fakeServer.listen(handler);
    expect(ComponentSearch(sl4f).list(), completion(equals(expectedList)));
  });
}
