...
class Performance {
  ...
  ///
  /// TODO(PT-216): Avoid explicitly passing the [converterPath].
  /// TODO(39301): Replace all uses of [processTrace] with this, and then
  ///              switch the name of this function to just [processTrace].
  ///
  /// Returns the benchmark result [File] generated by the processor.
  Future<File> processTrace2(MetricsSpecSet metricsSpecSet, File trace,
      {String converterPath}) async {
    ...
    for (final metricsSpec in metricsSpecSet.metricsSpecs) {
      ...
      // TODO: In the future we could refactor the metrics registry into
      // something more dynamic, if there's a need for it.
      final testCaseResultss =
          processWithDefaultMetricsRegistry(model, metricsSpec);
