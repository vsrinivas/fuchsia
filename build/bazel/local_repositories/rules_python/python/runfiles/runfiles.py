...
class _Runfiles(object):
    ...
    def Rlocation(self, path):
        ...
        if path[0] == "\\":
            raise ValueError(
                'path is absolute without a drive letter: "%s"' % path)
...
class _ManifestBased(object):
    ...
    def _GetRunfilesDir(self):
        # type: () -> str
        if self._path.endswith("/MANIFEST") or self._path.endswith(
                "\\MANIFEST"):
...
def _PathsFrom(
        argv0, runfiles_mf, runfiles_dir, is_runfiles_manifest,
