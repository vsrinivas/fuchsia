...
def _select_golden_requirements_file(
        requirements_txt, requirements_linux, requirements_darwin,
...
if __name__ == "__main__":
    ...
    if "TEST_TMPDIR" in os.environ:
        ...
        requirements_out = os.path.join(
            ...
            os.path.basename(requirements_txt) + ".out")
    ...
    update_command = os.getenv(
        "CUSTOM_COMPILE_COMMAND") or "bazel run %s" % (update_target_label,)
        ...
        try:
            ...
            if e.code == 2:
                ...
                if golden != out:
                    ...
                    print(
                        "".join(difflib.unified_diff(golden, out)),
                        file=sys.stderr)
                    print(
                        "Lock file out of date. Run '" + update_command +
                        "' to update.",
                        file=sys.stderr,
                    )
                    sys.exit(1)
