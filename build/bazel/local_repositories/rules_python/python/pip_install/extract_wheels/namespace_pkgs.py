...
def implicit_namespace_packages(
        directory: str,
        ignored_dirnames: Optional[List[str]] = None) -> Set[Path]:
    ...
    ignored_dirname_paths: List[Path] = [
        Path(p) for p in ignored_dirnames or ()
    ]
    # Traverse bottom-up because a directory can be a namespace pkg because its child contains module files.
    for dirpath, dirnames, filenames in map(lambda t: (Path(t[0]), *t[1:]),
        ...
        ) and  # The root of the directory should never be an implicit namespace
