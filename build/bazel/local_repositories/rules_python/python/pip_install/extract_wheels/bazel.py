...
def generate_entry_point_contents(
        module: str,
        attribute: str,
        shebang: str = "#!/usr/bin/env python3") -> str:
    ...
    return textwrap.dedent(
        ...
        if __name__ == "__main__":
            sys.exit({attribute}())
        """.format(shebang=shebang, module=module, attribute=attribute))
...
def generate_entry_point_rule(name: str, script: str, pkg: str) -> str:
    ...
    return textwrap.dedent(
        ...
        """.format(name=name, src=str(script).replace("\\", "/"), pkg=pkg))
...
def generate_requirements_file_contents(
        repo_name: str, targets: Iterable[str]) -> str:
...
def extract_wheel(
    ...
    if incremental:
        sanitised_dependencies = [
            sanitised_repo_library_label(d, repo_prefix=repo_prefix)
            for d in whl_deps
        ]
        sanitised_wheel_file_dependencies = [
            sanitised_repo_file_label(d, repo_prefix=repo_prefix)
            for d in whl_deps
        ]
    else:
        sanitised_dependencies = [
    ...
    library_name = (
        PY_LIBRARY_LABEL if incremental else sanitise_name(
            whl.name, repo_prefix))
    ...
    with open(os.path.join(directory, "BUILD.bazel"), "w") as build_file:
        ...
        contents = generate_build_file_contents(
            name=PY_LIBRARY_LABEL if incremental else sanitise_name(
                whl.name, repo_prefix),
