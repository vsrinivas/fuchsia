...
def parse_install_requirements(
    ...
    unpinned_reqs = []
    for parsed_line, (_, line) in zip(parser.parse(requirements_lock,
                                                   constraint=False),
                                      preprocess(content)):
...
def repo_names_and_requirements(
        ...
        repo_prefix: str) -> List[Tuple[str, str]]:
    return [
        (
...
dependencies from a fully resolved requirements lock file.")
    ...
    parser.add_argument(
        ...
        help=
        "Label used to declare the requirements.lock, included in comments in the file.",
    )
    parser.add_argument(
        "--python_interpreter",
        help=
        "The python interpreter that will be used to download and unpack the wheels.",
    )
    parser.add_argument(
...
If set, it will take precedence over python_interpreter.",
    ...
    annotations = args.annotations.collect(
        req_names) if args.annotations else {}
