    ...
    def test_generated_requirements_bzl(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            requirements_lock.write_bytes(
                bytes(
                    comments_and_flags + requirement_string, encoding="utf-8"))
            ...
            self.assertIn(
                "'extra_pip_args': {}".format(repr(all_flags)), contents,
                contents)
            self.assertIn(
            ...
            self.assertIn(
                "'python_interpreter': '/custom/python3'", contents, contents)
            self.assertIn(
        ...
        for requirement_args in ("", "--index-url https://index.python.com"):
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                self.assertIsInstance(
                    install_req_and_lines[0][0], InstallRequirement)
                self.assertIsInstance(
                    install_req_and_lines[1][0], InstallRequirement)
