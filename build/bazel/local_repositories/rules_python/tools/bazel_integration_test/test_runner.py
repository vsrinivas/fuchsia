...
def main(conf_file):
    ...
    with tempfile.TemporaryDirectory(
            dir=os.environ["TEST_TMPDIR"]) as tmp_homedir:
        ...
        with tempfile.TemporaryDirectory(
                dir=os.environ["TEST_TMPDIR"]) as tmpdir:
            workdir = os.path.join(tmpdir, "wksp")
            print(
                "copying workspace under test %s to %s" %
            ...
            for command in config["bazelCommands"]:
                ...
                if platform.system() == "Windows":
                    ...
                    bazel_process = Popen(
                        [bazelBinary, "shutdown"], cwd=workdir)
                    bazel_process.wait()
                    error |= bazel_process.returncode != 0
