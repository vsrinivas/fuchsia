...
class WheelMaker(object):
    ...
    def __init__(
        ...
        self._distinfo_dir = (
            ...
            escape_filename_segment(self._version) + ".dist-info/")
        self._zipfile = None
    ...
    def add_recordfile(self):
        ...
        for filename, digest, size in entries:
            if sys.version_info[0] > 2 and isinstance(filename, str):
                filename = filename.lstrip("/").encode(
...
def resolve_version_stamp(
        version: str, volatile_status_stamp: Path,
        stable_status_stamp: Path) -> str:
...
def parse_args() -> argparse.Namespace:
    ...
    contents_group.add_argument(
        ...
        action="append",
        help=
        "A file that has all the input files defined as a list to avoid the long command",
    )
