...
def main():
    ...
    parser.add_argument('--max-blob-contents-size',
                        default='0',
                        help='Maximum size for blob contents')
    parser.add_argument('--max-blob-image-size',
                        default='0',
                        help='Maximum size for blob image')
    parser.add_argument('--max-data-contents-size',
                        default='0',
                        help='Maximum size for data contents')
    parser.add_argument('--max-data-image-size',
                        default='0',
                        help='Maximum size for data contents')
    parser.add_argument('--max-fuchsia-zbi-size',
                        default='0',
                        help='Maximum size for fuchsia.zbi')
    parser.add_argument('--max-zedboot-zbi-size',
                        default='0',
    ...
    if args.fvm_blk:
        data_points.append(
            CmdSize('fvm/contents_size', [args.fvm_tool, args.fvm_blk, 'size'],
    ...
    if args.data_blk:
        ...
        data_points.extend([
            CmdSize('data/contents_size',
                    minfs_tool_prefix + ['used-data-size'],
                    args.max_data_contents_size),
            CmdSize('data/image_size', minfs_tool_prefix + ['used-size'],
                    args.max_data_image_size)
        ])
    if args.fuchsia_zbi:
        data_points.append(
            PathSize('fuchsia.zbi', args.fuchsia_zbi,
                     args.max_fuchsia_zbi_size))
    if args.zedboot_zbi:
        data_points.append(
            PathSize('zedboot.zbi', args.zedboot_zbi,
                     args.max_zedboot_zbi_size))
