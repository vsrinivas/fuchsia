...
def collect_binaries(manifest, input_binaries, aux_binaries, examined):
    ...
    def add_binary(binary, context=None, auxiliary=False):
        # TODO(bwb): Temporary workaround for ZN/GN migration. Remove when all drivers are in //src
        if "libdriver.so" in binary.entry.source and "default.zircon" not in binary.entry.source:
            return
