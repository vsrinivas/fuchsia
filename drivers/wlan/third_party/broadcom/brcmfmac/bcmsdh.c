...
int brcmf_sdiod_recv_chain(struct brcmf_sdio_dev* sdiodev, struct sk_buff_head* pktq, uint totlen) {
    ...
    if (pktq->qlen == 1) {
        err = brcmf_sdiod_skbuff_read(sdiodev, sdiodev->func2, addr, pktq->next);
...
int brcmf_sdiod_ramrw(struct brcmf_sdio_dev* sdiodev, bool write, u32 address, u8* data,
    ...
    while (size) {
        ...
        if (write) {
            ...
            err = brcmf_sdiod_skbuff_write(sdiodev, sdiodev->func1, sdaddr, pkt);
        } else {
            err = brcmf_sdiod_skbuff_read(sdiodev, sdiodev->func1, sdaddr, pkt);
        }
...
static int brcmf_ops_sdio_probe(struct sdio_func* func, const struct sdio_device_id* id) {
    int err;
