...
static int brcmf_msgbuf_alloc_pktid(struct device* dev, struct brcmf_msgbuf_pktids* pktids,
                                    struct sk_buff* skb, u16 data_offset, dma_addr_t* physaddr,
                                    u32* idx) {
    struct brcmf_msgbuf_pktid* array;
    u32 count;
...
static int brcmf_msgbuf_tx_ioctl(struct brcmf_pub* drvr, int ifidx, uint cmd, void* buf, uint len) {
...
static int brcmf_msgbuf_query_dcmd(struct brcmf_pub* drvr, int ifidx, uint cmd, void* buf, uint len,
                                   int* fwerr) {
    struct brcmf_msgbuf* msgbuf = (struct brcmf_msgbuf*)drvr->proto->pd;
...
static int brcmf_msgbuf_set_dcmd(struct brcmf_pub* drvr, int ifidx, uint cmd, void* buf, uint len,
                                 int* fwerr) {
    return brcmf_msgbuf_query_dcmd(drvr, ifidx, cmd, buf, len, fwerr);
}

static int brcmf_msgbuf_hdrpull(struct brcmf_pub* drvr, bool do_fws, struct sk_buff* skb,
                                struct brcmf_if** ifp) {
    return -ENODEV;
}
...
static void brcmf_msgbuf_remove_flowring(struct brcmf_msgbuf* msgbuf, u16 flowid) {
    u32 dma_sz;
    void* dma_buf;
...
static int brcmf_msgbuf_schedule_txdata(struct brcmf_msgbuf* msgbuf, u32 flowid, bool force) {
    struct brcmf_commonring* commonring;
...
static int brcmf_msgbuf_tx_queue_data(struct brcmf_pub* drvr, int ifidx, struct sk_buff* skb) {
    struct brcmf_msgbuf* msgbuf = (struct brcmf_msgbuf*)drvr->proto->pd;
    struct brcmf_flowring* flow = msgbuf->flow;
    struct ethhdr* eh = (struct ethhdr*)(skb->data);
    u32 flowid;
...
static void brcmf_msgbuf_delete_peer(struct brcmf_pub* drvr, int ifidx, u8 peer[ETH_ALEN]) {
    struct brcmf_msgbuf* msgbuf = (struct brcmf_msgbuf*)drvr->proto->pd;

    brcmf_flowring_delete_peer(msgbuf->flow, ifidx, peer);
}

static void brcmf_msgbuf_add_tdls_peer(struct brcmf_pub* drvr, int ifidx, u8 peer[ETH_ALEN]) {
    struct brcmf_msgbuf* msgbuf = (struct brcmf_msgbuf*)drvr->proto->pd;
...
static void brcmf_msgbuf_process_ioctl_complete(struct brcmf_msgbuf* msgbuf, void* buf) {
    ...
    msgbuf->ioctl_resp_status = (s16)le16_to_cpu(ioctl_resp->compl_hdr.status);
...
static void brcmf_msgbuf_update_rxbufpost_count(struct brcmf_msgbuf* msgbuf, u16 rxcnt) {
...
static u32 brcmf_msgbuf_rxbuf_ctrl_post(struct brcmf_msgbuf* msgbuf, bool event_buf, u32 count) {
    struct brcmf_commonring* commonring;
    void* ret_ptr;
    struct sk_buff* skb;
    u16 alloced;
...
int brcmf_proto_msgbuf_rx_trigger(struct device* dev) {
    ...
    for_each_set_bit(flowid, msgbuf->txstatus_done_map, msgbuf->max_flowrings) {
        ...
        if ((qlen > BRCMF_MSGBUF_TRICKLE_TXWORKER_THRS) ||
                ((qlen) &&
                 (atomic_read(&commonring->outstanding_tx) < BRCMF_MSGBUF_TRICKLE_TXWORKER_THRS))) {
            brcmf_msgbuf_schedule_txdata(msgbuf, flowid, true);
        }
    }
