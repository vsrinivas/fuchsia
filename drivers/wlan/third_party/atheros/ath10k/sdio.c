...
static inline int ath10k_sdio_calc_txrx_padded_len(struct ath10k_sdio* ar_sdio, size_t len) {
...
static inline int ath10k_sdio_mbox_alloc_rx_pkt(struct ath10k_sdio_rx_data* pkt, size_t act_len,
                                                size_t full_len, bool part_of_bundle,
                                                bool last_in_bundle) {
    pkt->skb = dev_alloc_skb(full_len);
    if (!pkt->skb) { return -ENOMEM; }
...
static inline bool is_trailer_only_msg(struct ath10k_sdio_rx_data* pkt) {
    ...
    if (len == htc_hdr->trailer_len) { trailer_only = true; }
...
static inline void ath10k_sdio_set_cmd52_arg(uint32_t* arg, uint8_t write, uint8_t raw,
    ...
    *arg = FIELD_PREP((1 << 31), write) | FIELD_PREP((1 << 27), raw) | FIELD_PREP((1 << 26), 1) |
           FIELD_PREP(BITMASK(9, 25), address) | FIELD_PREP((1 << 8), 1) |
           FIELD_PREP(BITMASK(0, 7), val);
}

static int ath10k_sdio_func0_cmd52_wr_byte(struct mmc_card* card, unsigned int address,
                                           unsigned char byte) {
...
static int ath10k_sdio_func0_cmd52_rd_byte(struct mmc_card* card, unsigned int address,
                                           unsigned char* byte) {
    struct mmc_command io_cmd;
    int ret;
...
static int ath10k_sdio_config(struct ath10k* ar) {
    ...
    byte = 0;
    ret = ath10k_sdio_func0_cmd52_rd_byte(func->card, SDIO_CCCR_DRIVE_STRENGTH, &byte);

    byte &= ~ATH10K_SDIO_DRIVE_DTSX_MASK;
    byte |= FIELD_PREP(ATH10K_SDIO_DRIVE_DTSX_MASK, ATH10K_SDIO_DRIVE_DTSX_TYPE_D);

    ret = ath10k_sdio_func0_cmd52_wr_byte(func->card, SDIO_CCCR_DRIVE_STRENGTH, byte);

    byte = 0;
    ret = ath10k_sdio_func0_cmd52_rd_byte(func->card, CCCR_SDIO_DRIVER_STRENGTH_ENABLE_ADDR, &byte);

    byte |= (CCCR_SDIO_DRIVER_STRENGTH_ENABLE_A | CCCR_SDIO_DRIVER_STRENGTH_ENABLE_C |
             CCCR_SDIO_DRIVER_STRENGTH_ENABLE_D);

    ret = ath10k_sdio_func0_cmd52_wr_byte(func->card, CCCR_SDIO_DRIVER_STRENGTH_ENABLE_ADDR, byte);
    if (ret) {
    ...
    byte = 0;
    ret = ath10k_sdio_func0_cmd52_rd_byte(func->card, CCCR_SDIO_IRQ_MODE_REG_SDIO3, &byte);

    byte |= SDIO_IRQ_MODE_ASYNC_4BIT_IRQ_SDIO3;

    ret = ath10k_sdio_func0_cmd52_wr_byte(func->card, CCCR_SDIO_IRQ_MODE_REG_SDIO3, byte);
    if (ret) {
    ...
    byte = 0;
    ret = ath10k_sdio_func0_cmd52_rd_byte(func->card, CCCR_SDIO_ASYNC_INT_DELAY_ADDRESS, &byte);

    byte &= ~CCCR_SDIO_ASYNC_INT_DELAY_MASK;
    byte |= FIELD_PREP(CCCR_SDIO_ASYNC_INT_DELAY_MASK, asyncintdelay);

    ret = ath10k_sdio_func0_cmd52_wr_byte(func->card, CCCR_SDIO_ASYNC_INT_DELAY_ADDRESS, byte);
...
static int ath10k_sdio_write32(struct ath10k* ar, uint32_t addr, uint32_t val) {
    struct ath10k_sdio* ar_sdio = ath10k_sdio_priv(ar);
    struct sdio_func* func = ar_sdio->func;
    int ret;
...
static int ath10k_sdio_read32(struct ath10k* ar, uint32_t addr, uint32_t* val) {
    struct ath10k_sdio* ar_sdio = ath10k_sdio_priv(ar);
    struct sdio_func* func = ar_sdio->func;
    int ret;
...
static int ath10k_sdio_read(struct ath10k* ar, uint32_t addr, void* buf, size_t len) {
    struct ath10k_sdio* ar_sdio = ath10k_sdio_priv(ar);
    struct sdio_func* func = ar_sdio->func;
    int ret;
...
static int ath10k_sdio_write(struct ath10k* ar, uint32_t addr, const void* buf, size_t len) {
    struct ath10k_sdio* ar_sdio = ath10k_sdio_priv(ar);
    struct sdio_func* func = ar_sdio->func;
    int ret;
...
static int ath10k_sdio_readsb(struct ath10k* ar, uint32_t addr, void* buf, size_t len) {
    struct ath10k_sdio* ar_sdio = ath10k_sdio_priv(ar);
    struct sdio_func* func = ar_sdio->func;
    int ret;
...
static int ath10k_sdio_mbox_rx_process_packet(struct ath10k* ar, struct ath10k_sdio_rx_data* pkt,
                                              uint32_t* lookaheads, int* n_lookaheads) {
    ...
    if (trailer_present) {
        trailer = skb->data + sizeof(*htc_hdr) + payload_len - htc_hdr->trailer_len;

        eid = pipe_id_to_eid(htc_hdr->eid);

        ret = ath10k_htc_process_trailer(htc, trailer, htc_hdr->trailer_len, eid, lookaheads,
                                         n_lookaheads);
        if (ret) { return ret; }

        if (is_trailer_only_msg(pkt)) { pkt->trailer_only = true; }
...
static int ath10k_sdio_mbox_rx_process_packets(struct ath10k* ar, uint32_t lookaheads[],
                                               int* n_lookahead) {
    struct ath10k_sdio* ar_sdio = ath10k_sdio_priv(ar);
    struct ath10k_htc* htc = &ar->htc;
...
static int ath10k_sdio_mbox_alloc_pkt_bundle(struct ath10k* ar, struct ath10k_sdio_rx_data* rx_pkts,
                                             struct ath10k_htc_hdr* htc_hdr, size_t full_len,
                                             size_t act_len, size_t* bndl_cnt) {
    int ret, i;
    ...
    *bndl_cnt = FIELD_GET(ATH10K_HTC_FLAG_BUNDLE_MASK, htc_hdr->flags);
    ...
    for (i = 0; i < *bndl_cnt; i++) {
        ret = ath10k_sdio_mbox_alloc_rx_pkt(&rx_pkts[i], act_len, full_len, true, false);
        if (ret) { return ret; }
    }
...
static int ath10k_sdio_mbox_rx_alloc(struct ath10k* ar, uint32_t lookaheads[], int n_lookaheads) {
    struct ath10k_sdio* ar_sdio = ath10k_sdio_priv(ar);
    ...
    for (i = 0; i < n_lookaheads; i++) {
        ...
        if (htc_hdr->len > ATH10K_HTC_MBOX_MAX_PAYLOAD_LENGTH) {
        ...
        if (full_len > ATH10K_SDIO_MAX_BUFFER_SIZE) {
            ath10k_warn("rx buffer requested with invalid htc_hdr length (%d, 0x%x): %d\n",
                        htc_hdr->eid, htc_hdr->flags, htc_hdr->len);
            ret = -EINVAL;
            goto err;
        }
        ...
        if (htc_hdr->flags & ATH10K_HTC_FLAG_BUNDLE_MASK) {
            ...
            ret = ath10k_sdio_mbox_alloc_pkt_bundle(ar, &ar_sdio->rx_pkts[i], htc_hdr, full_len,
                                                    act_len, &bndl_cnt);
         ...
         */
        ret = ath10k_sdio_mbox_alloc_rx_pkt(&ar_sdio->rx_pkts[i], act_len, full_len, last_in_bundle,
                                            last_in_bundle);
    }
...
err:
    for (i = 0; i < ATH10K_SDIO_MAX_RX_MSGS; i++) {
        if (!ar_sdio->rx_pkts[i].alloc_len) { break; }
        ath10k_sdio_mbox_free_rx_pkt(&ar_sdio->rx_pkts[i]);
    }
...
static int ath10k_sdio_mbox_rx_packet(struct ath10k* ar, struct ath10k_sdio_rx_data* pkt) {
    ...
    ret = ath10k_sdio_readsb(ar, ar_sdio->mbox_info.htc_addr, skb->data, pkt->alloc_len);
    pkt->status = ret;
    if (!ret) { skb_put(skb, pkt->act_len); }
...
static int ath10k_sdio_mbox_rxmsg_pending_handler(struct ath10k* ar, uint32_t msg_lookahead,
                                                  bool* done) {
    struct ath10k_sdio* ar_sdio = ath10k_sdio_priv(ar);
    uint32_t lookaheads[ATH10K_SDIO_MAX_RX_MSGS];
    int n_lookaheads = 1;
    ...
    *done = true;
    ...
    while (time_before(jiffies, timeout)) {
         ...
         * n_lookaheads.
         */
        ret = ath10k_sdio_mbox_rx_alloc(ar, lookaheads, n_lookaheads);
        if (ret) { break; }
         ...
         */
        n_lookaheads = 0;
        ret = ath10k_sdio_mbox_rx_process_packets(ar, lookaheads, &n_lookaheads);
    ...
    if (ret && (ret != -ECANCELED)) ath10k_warn("failed to get pending recv messages: %d\n", ret);
...
static int ath10k_sdio_mbox_proc_dbg_intr(struct ath10k* ar) {
    ...
    if (ret) { ath10k_warn("failed to clear debug interrupt: %d\n", ret); }
...
static int ath10k_sdio_mbox_proc_counter_intr(struct ath10k* ar) {
    ...
    counter_int_status =
        irq_data->irq_proc_reg->counter_int_status & irq_data->irq_en_reg->cntr_int_status_en;
...
static int ath10k_sdio_mbox_proc_err_intr(struct ath10k* ar) {
    ...
    ath10k_dbg(ar, ATH10K_DBG_SDIO, "sdio error_int_status 0x%x\n", error_int_status);

    if (FIELD_GET(MBOX_ERROR_INT_STATUS_WAKEUP_MASK, error_int_status)) {
        ath10k_dbg(ar, ATH10K_DBG_SDIO, "sdio interrupt error wakeup\n");
    }

    if (FIELD_GET(MBOX_ERROR_INT_STATUS_RX_UNDERFLOW_MASK, error_int_status)) {
        ath10k_warn("rx underflow interrupt error\n");
    }

    if (FIELD_GET(MBOX_ERROR_INT_STATUS_TX_OVERFLOW_MASK, error_int_status)) {
    ...
    ret = ath10k_sdio_writesb32(ar, MBOX_ERROR_INT_STATUS_ADDRESS, error_int_status);
    if (ret) {
...
static int ath10k_sdio_mbox_proc_cpu_intr(struct ath10k* ar) {
    ...
    mtx_lock(&irq_data->mtx);
    cpu_int_status =
        irq_data->irq_proc_reg->cpu_int_status & irq_data->irq_en_reg->cpu_int_status_en;
    if (!cpu_int_status) {
     ...
     * Set W1C value to clear the interrupt, this hits the register first.
     */
    ret = ath10k_sdio_writesb32(ar, MBOX_CPU_INT_STATUS_ADDRESS, cpu_int_status);
    if (ret) {
...
static int ath10k_sdio_mbox_read_int_status(struct ath10k* ar, uint8_t* host_int_status,
                                            uint32_t* lookahead) {
    struct ath10k_sdio* ar_sdio = ath10k_sdio_priv(ar);
    struct ath10k_sdio_irq_data* irq_data = &ar_sdio->irq_data;
    ...
    *host_int_status = 0;
     ...
     */
    ret = ath10k_sdio_read(ar, MBOX_HOST_INT_STATUS_ADDRESS, irq_proc_reg, sizeof(*irq_proc_reg));
    if (ret) { goto out; }

    /* Update only those registers that are enabled */
    *host_int_status = irq_proc_reg->host_int_status & irq_en_reg->int_status_en;
    ...
    *host_int_status &= ~htc_mbox;
    if (irq_proc_reg->rx_lookahead_valid & htc_mbox) {
        *lookahead = irq_proc_reg->rx_lookahead[ATH10K_HTC_MAILBOX];
        if (!*lookahead) { ath10k_warn("sdio mbox lookahead is zero\n"); }
    }
...
static int ath10k_sdio_mbox_proc_pending_irqs(struct ath10k* ar, bool* done) {
    ...
    if (lookahead) {
        ath10k_dbg(ar, ATH10K_DBG_SDIO, "sdio pending mailbox msg lookahead 0x%08x\n", lookahead);
...
out:
    ...
    ath10k_dbg(ar, ATH10K_DBG_SDIO, "sdio pending irqs done %d status %d", *done, ret);
...
static void ath10k_sdio_set_mbox_info(struct ath10k* ar) {
    ...
    case QCA_MANUFACTURER_ID_AR6005_BASE:
        if (dev_id_chiprev < 4)
             ...
             * to 56K
             */
            mbox_info->ext_info[0].htc_ext_sz = ATH10K_HIF_MBOX0_EXT_WIDTH_ROME_2_0;
        break;
    ...
    mbox_info->ext_info[1].htc_ext_addr = mbox_info->ext_info[0].htc_ext_addr +
...
static int ath10k_sdio_bmi_get_rx_lookahead(struct ath10k* ar) {
    ...
    while ((time_before(jiffies, timeout)) && !rx_word) {
        ret = ath10k_sdio_read32(ar, MBOX_HOST_INT_STATUS_ADDRESS, &rx_word);
        if (ret) {
...
static int ath10k_sdio_bmi_exchange_msg(struct ath10k* ar, void* req, uint32_t req_len, void* resp,
                                        uint32_t* resp_len) {
    struct ath10k_sdio* ar_sdio = ath10k_sdio_priv(ar);
    uint32_t addr;
    int ret;
...
static struct ath10k_sdio_bus_request* ath10k_sdio_alloc_busreq(struct ath10k* ar) {
    ...
    bus_req = list_first_entry(&ar_sdio->bus_req_freeq, struct ath10k_sdio_bus_request, list);
...
static void __ath10k_sdio_write_async(struct ath10k* ar, struct ath10k_sdio_bus_request* req) {
    ...
    if (ret) ath10k_warn("failed to write skb to 0x%x asynchronously: %d", req->address, ret);
...
static void ath10k_sdio_write_async_work(struct work_struct* work) {
    struct ath10k_sdio* ar_sdio = container_of(work, struct ath10k_sdio, wr_async_work);
...
static int ath10k_sdio_prep_async_req(struct ath10k* ar, uint32_t addr, struct sk_buff* skb,
                                      sync_completion_t* comp, bool htc_msg,
                                      enum ath10k_htc_ep_id eid) {
    struct ath10k_sdio* ar_sdio = ath10k_sdio_priv(ar);
    struct ath10k_sdio_bus_request* bus_req;
...
static int ath10k_sdio_hif_disable_intrs(struct ath10k* ar) {
    ...
    ret =
        ath10k_sdio_write(ar, MBOX_INT_STATUS_ENABLE_ADDRESS, &regs->int_status_en, sizeof(*regs));
    if (ret) { ath10k_warn("unable to disable sdio interrupts: %d\n", ret); }
...
static void ath10k_sdio_hif_power_down(struct ath10k* ar) {
    ...
    if (ret) { ath10k_warn("unable to disable sdio function: %d\n", ret); }
...
static int ath10k_sdio_hif_tx_sg(struct ath10k* ar, uint8_t pipe_id,
    ...
    for (i = 0; i < n_items; i++) {
        ...
        padded_len = ath10k_sdio_calc_txrx_padded_len(ar_sdio, skb->len);
        skb_trim(skb, padded_len);

        /* Write TX data to the end of the mbox address space */
        address = ar_sdio->mbox_addr[eid] + ar_sdio->mbox_size[eid] - skb->len;
        ret = ath10k_sdio_prep_async_req(ar, address, skb, NULL, true, eid);
        if (ret) { return ret; }
    }
...
static int ath10k_sdio_hif_enable_intrs(struct ath10k* ar) {
     ...
     */
    regs->int_status_en |= FIELD_PREP(MBOX_INT_STATUS_ENABLE_MBOX_DATA_MASK, 1);

    /* Set up the CPU Interrupt status Register */
    regs->cpu_int_status_en = 0;

    /* Set up the Error Interrupt status Register */
    regs->err_int_status_en = FIELD_PREP(MBOX_ERROR_STATUS_ENABLE_RX_UNDERFLOW_MASK, 1) |
    ...
    regs->cntr_int_status_en =
        FIELD_PREP(MBOX_COUNTER_INT_STATUS_ENABLE_BIT_MASK, ATH10K_SDIO_TARGET_DEBUG_INTR_MASK);

    ret =
        ath10k_sdio_write(ar, MBOX_INT_STATUS_ENABLE_ADDRESS, &regs->int_status_en, sizeof(*regs));
    if (ret) ath10k_warn("failed to update mbox interrupt status register : %d\n", ret);
...
static int ath10k_sdio_hif_set_mbox_sleep(struct ath10k* ar, bool enable_sleep) {
    ...
    if (ret) {
        ath10k_warn("failed to write to FIFO_TIMEOUT_AND_CHIP_CONTROL: %d", ret);
        return ret;
    }
...
static int ath10k_sdio_hif_diag_read(struct ath10k* ar, uint32_t address, void* buf,
                                     size_t buf_len) {
    int ret;
...
static int ath10k_sdio_hif_diag_read32(struct ath10k* ar, uint32_t address, uint32_t* value) {
    uint32_t* val;
    int ret;
...
static int ath10k_sdio_hif_diag_write_mem(struct ath10k* ar, uint32_t address, const void* data,
                                          int nbytes) {
    int ret;
...
static int ath10k_sdio_hif_start(struct ath10k* ar) {
    ...
    if (ret) { ath10k_warn("failed to enable sdio interrupts: %d\n", ret); }
    ...
    if (val & HI_ACS_FLAGS_SDIO_SWAP_MAILBOX_FW_ACK) {
        ath10k_dbg(ar, ATH10K_DBG_SDIO, "sdio mailbox swap service enabled\n");
...
static void ath10k_sdio_irq_disable(struct ath10k* ar) {
    ...
    ret = ath10k_sdio_prep_async_req(ar, MBOX_INT_STATUS_ENABLE_ADDRESS, skb, &irqs_disabled_comp,
                                     false, 0);
    if (ret) { goto out; }
    ...
    if (ret) { ath10k_warn("failed to release sdio interrupt: %d\n", ret); }
...
static int ath10k_sdio_hif_resume(struct ath10k* ar) {
    ...
    case ATH10K_STATE_OFF:
        ath10k_dbg(ar, ATH10K_DBG_SDIO, "sdio resume configuring sdio\n");
...
static int ath10k_sdio_hif_map_service_to_pipe(struct ath10k* ar, uint16_t service_id,
                                               uint8_t* ul_pipe, uint8_t* dl_pipe) {
    struct ath10k_sdio* ar_sdio = ath10k_sdio_priv(ar);
    struct ath10k_htc* htc = &ar->htc;
    uint32_t htt_addr, wmi_addr, htt_mbox_size, wmi_mbox_size;
    enum ath10k_htc_ep_id eid;
    bool ep_found = false;
    int i;
...
static void ath10k_sdio_hif_get_default_pipe(struct ath10k* ar, uint8_t* ul_pipe,
                                             uint8_t* dl_pipe) {
 ...
 * hif op wrapper.
 */
static void ath10k_sdio_hif_send_complete_check(struct ath10k* ar, uint8_t pipe, int force) {}
...
static SIMPLE_DEV_PM_OPS(ath10k_sdio_pm_ops, ath10k_sdio_pm_suspend, ath10k_sdio_pm_resume);
...
static int ath10k_sdio_probe(struct sdio_func* func, const struct sdio_device_id* id) {
    struct ath10k_sdio* ar_sdio;
    struct ath10k* ar;
    enum ath10k_hw_rev hw_rev;
    uint32_t chip_id, dev_id_base;
    int ret, i;
    ...
    ar = ath10k_core_create(sizeof(*ar_sdio), &func->dev, ATH10K_BUS_SDIO, hw_rev,
                            &ath10k_sdio_hif_ops);
    if (!ar) {
    ...
    ar_sdio->irq_data.irq_proc_reg = kzalloc(sizeof(struct ath10k_sdio_irq_proc_regs), GFP_KERNEL);
    if (!ar_sdio->irq_data.irq_proc_reg) {
        ret = -ENOMEM;
        goto err_core_destroy;
    }

    ar_sdio->irq_data.irq_en_reg = kzalloc(sizeof(struct ath10k_sdio_irq_enable_regs), GFP_KERNEL);
    ...
    default:
        ret = -ENODEV;
        ath10k_err("unsupported device id %u (0x%x)\n", dev_id_base, id->device);
        goto err_free_bmi_buf;
    }
    ...
    if (ret) {
        ath10k_err("failed to config sdio: %d\n", ret);
        goto err_free_wq;
    }
...
static void ath10k_sdio_remove(struct sdio_func* func) {
    ...
    ath10k_dbg(ar, ATH10K_DBG_BOOT, "sdio removed func %d vendor 0x%x device 0x%x\n", func->num,
...
static const struct sdio_device_id ath10k_sdio_devices[] = {
    {SDIO_DEVICE(QCA_MANUFACTURER_CODE, (QCA_SDIO_ID_AR6005_BASE | 0xA))},
    {SDIO_DEVICE(QCA_MANUFACTURER_CODE, (QCA_SDIO_ID_QCA9377_BASE | 0x1))},
    {},
};
...
static int __init ath10k_sdio_init(void) {
    ...
    if (ret) { pr_err("sdio driver registration failed: %d\n", ret); }
