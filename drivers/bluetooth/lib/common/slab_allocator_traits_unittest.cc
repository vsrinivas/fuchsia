...
// The static slab allocator needs to be declared in the global namespace and cannot refer to a
// traits type that's declared in an anonymous namespace. So we use the test namespace here.
namespace test {
...
TEST(SlabAllocatedBufferTest, Basic) {
  ...
  // Free one of the allocated buffers and request a new one. This should return the memory back to
  // the allocator and the next allocation should succeed.
  buffer1 = nullptr;
...
DECLARE_STATIC_SLAB_ALLOCATOR_STORAGE(::bluetooth::common::test::TestTraits, 1, true);
