...
// A LowEnergyConnector abstracts over the HCI commands and events involved in initiating a
// direct link-layer connection with a peer device. This class also provides a way for a delegate to
// be notified when a connection is initiated by a remote.
//
// This class vends Connection objects for LE link layer connections.
//
// Instances of this class are expected to each exist as a singleton on a per-transport basis as
// multiple instances cannot accurately reflect the state of the controller while allowing
...
class LowEnergyConnector {
 public:
  ...
  //
  //   - |task_runner|: The task runner that will be used to run all asynchronous operations. This
  //     must be bound to the thread on which the LowEnergyConnector is created.
  //
  //   - |delegate|: The delegate that will be notified when a new logical link is established. This
  //     can be due to either an explicit request via CreateConnection() (local host initiated),
  ...
  //
  // |own_address_type| indicates which local Bluetooth address will be used during the request.
  //
  // If |use_whitelist| is true, then the controller white list is used to determine which
  // advertiser to connect to. Otherwise, the controller will connect to |peer_address|.
  //
  // |result_callback| is called asynchronously to notify the result of the operation. On success,
  // the delegate will be notified with a new connection object.
  //
  // |timeout_ms| specifies a time period after which the request will time out. When a request to
  // create connection times out, |result_callback| will be called with its |result| parameter set
  // to Result::kFailed and |hci_status| set to Status::kCommandTimeout.
  enum class Result {
    kSuccess,
    kCanceled,
    kFailed,
  ...
  bool CreateConnection(LEOwnAddressType own_address_type, bool use_whitelist,
                        ...
                        const ResultCallback& result_callback, int64_t timeout_ms);
 ...
 private:
  struct PendingRequest {
    ...
    PendingRequest(const common::DeviceAddress& peer_address, uint16_t interval_min,
