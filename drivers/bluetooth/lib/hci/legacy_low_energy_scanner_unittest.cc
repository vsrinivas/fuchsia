...
const common::DeviceAddress kAddress2(common::DeviceAddress::Type::kLERandom, "00:00:00:00:00:03");
const common::DeviceAddress kAddress3(common::DeviceAddress::Type::kLERandom, "00:00:00:00:00:04");
const common::DeviceAddress kAddress4(common::DeviceAddress::Type::kLERandom, "00:00:00:00:00:05");
const common::DeviceAddress kAddress5(common::DeviceAddress::Type::kLERandom, "00:00:00:00:00:06");
...
class LegacyLowEnergyScannerTest : public TestingBase, public LowEnergyScanner::Delegate {
 ...
 protected:
  ...
  void SetUp() override {
    ...
    scanner_ =
        std::make_unique<LegacyLowEnergyScanner>(this, transport(), message_loop()->task_runner());
  ...
  using DeviceFoundCallback =
      std::function<void(const LowEnergyScanResult&, const common::ByteBuffer&)>;
  void set_device_found_callback(const DeviceFoundCallback& cb) { device_found_cb_ = cb; }

  // LowEnergyScanner::Observer overrides:
  void OnDeviceFound(const LowEnergyScanResult& result, const common::ByteBuffer& data) override {
    if (device_found_cb_) device_found_cb_(result, data);
  }

  // Adds 6 fake devices using kAddress[0-5] above.
  void AddFakeDevices() {
    // We use malformed data for testing purposes, as we don't care about integrity here.
    ...
    // Generates ADV_SCAN_IND, scan response is reported over multiple HCI events.
    fake_device = std::make_unique<FakeDevice>(kAddress1, false, true);
    fake_device->SetAdvertisingData(adv_data);
    fake_device->SetScanResponse(false, scan_rsp);
    test_device()->AddLEDevice(std::move(fake_device));

    // Generates ADV_IND, empty scan response is reported over multiple HCI events.
    fake_device = std::make_unique<FakeDevice>(kAddress2, true, true);
    fake_device->SetAdvertisingData(adv_data);
    fake_device->SetScanResponse(false, empty_data);
    test_device()->AddLEDevice(std::move(fake_device));

    // Generates ADV_IND, empty adv data and non-empty scan response is reported over multiple HCI
    // events.
    fake_device = std::make_unique<FakeDevice>(kAddress3, true, true);
    fake_device->SetScanResponse(false, scan_rsp);
    test_device()->AddLEDevice(std::move(fake_device));

    // Generates ADV_IND, a scan response is never sent even though ADV_IND is scannable.
...
TEST_F(LegacyLowEnergyScannerTest, StartScanHCIErrors) {
  ...
  test_device()->SetDefaultResponseStatus(hci::kLESetScanParameters, hci::Status::kHardwareFailure);
  EXPECT_EQ(0, test_device()->le_scan_state().scan_interval);

  EXPECT_TRUE(scanner()->StartScan(false, hci::defaults::kLEScanInterval,
                                   hci::defaults::kLEScanWindow, false,
  ...
  EXPECT_FALSE(scanner()->StartScan(false, hci::defaults::kLEScanInterval,
                                    hci::defaults::kLEScanWindow, false,
                                    hci::LEScanFilterPolicy::kNoWhiteList, kScanPeriodMs, cb));
  ...
  test_device()->SetDefaultResponseStatus(hci::kLESetScanEnable, hci::Status::kHardwareFailure);
  ...
  EXPECT_EQ(LowEnergyScanner::Status::kFailed, status);
  EXPECT_EQ(hci::defaults::kLEScanInterval, test_device()->le_scan_state().scan_interval);
  EXPECT_EQ(hci::defaults::kLEScanWindow, test_device()->le_scan_state().scan_window);
  EXPECT_EQ(hci::LEScanFilterPolicy::kNoWhiteList, test_device()->le_scan_state().filter_policy);
  EXPECT_FALSE(test_device()->le_scan_state().enabled);
  EXPECT_EQ(LowEnergyScanner::State::kIdle, scanner()->state());
  EXPECT_FALSE(scanner()->IsScanning());
}
...
TEST_F(LegacyLowEnergyScannerTest, StartScan) {
  ...
  EXPECT_TRUE(scanner()->StartScan(true /* active */, hci::defaults::kLEScanInterval,
  ...
  EXPECT_FALSE(scanner()->StartScan(true /* active */, hci::defaults::kLEScanInterval,
                                    hci::defaults::kLEScanWindow, true /* filter_duplicates */,
                                    hci::LEScanFilterPolicy::kNoWhiteList, kScanPeriodMs, cb));
...
TEST_F(LegacyLowEnergyScannerTest, StopScan) {
  ...
  // Pass a long scan period value. This should not matter as we will terminate the scan directly.
  EXPECT_TRUE(scanner()->StartScan(true /* active */, hci::defaults::kLEScanInterval,
                                   hci::defaults::kLEScanWindow, true /* filter_duplicates */,
  ...
  // StopScan() should terminate the scan session and the status should be kStopped.
...
TEST_F(LegacyLowEnergyScannerTest, StopScanWhileInitiating) {
  ...
  // Call StopScan(). This should cancel the HCI command sequence set up by StartScan() so that the
  // it never completes. The HCI_LE_Set_Scan_Parameters command *may* get sent but the scan should
  // never get enabled.
  EXPECT_TRUE(scanner()->StopScan());
  RunMessageLoop();
...
TEST_F(LegacyLowEnergyScannerTest, ActiveScanResults) {
  ...
  auto cb = [&status, this](LowEnergyScanner::Status in_status) {
    status = in_status;
    if (status == LowEnergyScanner::Status::kComplete) message_loop()->QuitNow();
  };
  ...
  // Verify the 6 results against the fake devices that were set up by AddFakeDevices(). Since the
  // scan period ended naturally, LowEnergyScanner should generate a device found event for all
  // pending reports even if a scan response was not received for a scannable device (see Fake
  // Device 4, i.e. kAddress4):
  // Result 0
...
TEST_F(LegacyLowEnergyScannerTest, StopDuringActiveScan) {
  ...
  set_device_found_callback([&results, this](const auto& result, const auto& data) {
    ...
    if (result.address == kAddress5) message_loop()->QuitNow();
  });

  // Perform an active scan indefinitely. This means that the scan period will never complete by
  // itself.
  EXPECT_TRUE(scanner()->StartScan(
      true, hci::defaults::kLEScanInterval, hci::defaults::kLEScanWindow, true,
      hci::LEScanFilterPolicy::kNoWhiteList, LowEnergyScanner::kPeriodInfinite, cb));
  EXPECT_EQ(LowEnergyScanner::State::kInitiating, scanner()->state());
  RunMessageLoop();
  EXPECT_EQ(LowEnergyScanner::State::kScanning, scanner()->state());

  // Run the message loop until we've seen an event for the last device that we added. Fake Device 4
  // (i.e. kAddress4) is scannable but it never sends a scan response so we expect that remain in
  // the scanner's pending reports list.
  RunMessageLoop();
  EXPECT_EQ(5u, results.size());
  EXPECT_EQ(results.find(kAddress4), results.end());

  // Stop the scan. Since we are terminating the scan period early, LowEnergyScanner should not send
  // a report for the pending device.
  EXPECT_TRUE(scanner()->StopScan());
  RunMessageLoop();
  EXPECT_EQ(LowEnergyScanner::State::kIdle, scanner()->state());
...
TEST_F(LegacyLowEnergyScannerTest, PassiveScanResults) {
  ...
  // Verify the 6 results against the fake devices that were set up by AddFakeDevices(). All Scan
  // Response PDUs should have been ignored.
  // Result 0
  auto iter = results.find(kAddress0);
  EXPECT_NE(iter, results.end());
