...
struct LowEnergyScanResult {
  LowEnergyScanResult();
  LowEnergyScanResult(const common::DeviceAddress& address, bool connectable, int8_t rssi);

  // The device address of the remote device.
  // TODO(armansito): Report resolved address if address is resolvable and we can resolve it.
  common::DeviceAddress address;

  // True if this device accepts connections. This is the case if this device sent a connectable
  // advertising PDU.
  bool connectable;

  // The received signal strength of the advertisement packet corresponding to this device.
  int8_t rssi;
};

// LowEnergyScanner manages Low Energy device scan procedures that are used during general and
// limited device discovery and connection establishment procedures. This is an abstract class that
// provides a common interface over 5.0 Extended Advertising and Legacy Advertising features.
//
 ...
 public:
  ...
  class Delegate {
   public:
    ...
    // Called when a device is found. |data| contains the advertising data, as well as any scan
    // response data that was received during an active scan.
    virtual void OnDeviceFound(const LowEnergyScanResult& result, const common::ByteBuffer& data);
  ...
  // Initiates a device scan. This is an asynchronous operation that abides by the following rules:
  //
  //   - This method synchronously returns false if the procedure could not be started, e.g. because
  //     discovery is already in progress, or it is in the process of being stopped, or the
  //     controller does not support discovery, etc.
  //
  ...
  // an ongoing scan will terminate at the end of the scan period if a finite value for |period_ms|
  // was provided.
  //
  // If an active scan is being performed, then scannable advertising reports will NOT generate an
  // OnDeviceFound event until a scan response is received from the corresponding broadcaster. If
  // a scan response from a scannable device is never received during a scan period, then an
  // OnDeviceFound event (exluding scan response data) will be generated for that device at the end
  ...
  virtual bool StartScan(bool active, uint16_t scan_interval, uint16_t scan_window,
                         ...
                         int64_t period_ms, const StatusCallback& callback) = 0;

  // Stops a previously started device scan. Returns false if a scan is not in progress. Otherwise,
  // cancels any in progress scan procedure and returns true.
