...
//   using PacketTraits = ...;
//
// The following defines a SlabAllocator that returns instances of Packet<MyHeader>. Each packet is
// configured to be backed by a 256 octet buffer and each slab can allocate 128 packets:
//
// my_packet.h:
//   #include "packet.h"
...
//   using MyPacketTraits = hci::slab_allocators::PacketTraits<MyHeader, 256, 128>;
//   ...
//
// my_packet.cc:
//   DECLARE_STATIC_SLAB_ALLOCATOR_STORAGE(MyPacketTraits, my_max_num_slabs, true);
//   ...
//
// foo.cc:
//   #include "my_packet.h"
...
// Slab sizes for control (command/event) and ACL data packets used by the slab allocators. These
// are used by the CommandPacket, EventPacket, and ACLDataPacket classes.

// TODO(armansito): The slab sizes below are arbitrary; fine tune them based on usage.
...
template <typename HeaderType, size_t BufferSize, size_t NumBuffers>
using PacketTraits =
    common::SlabAllocatorTraits<internal::SlabPacket<HeaderType, BufferSize, NumBuffers>,
