...
// our new interface could support scatter-gather for the header and the payload. Then, the bt-hci
// driver could read the payload fragment directly out of |data| and we would only construct the
// headers, removing the extra copy.
//
...
PDU Fragmenter::BuildBasicFrame(ChannelId channel_id, const common::ByteBuffer& data,
  ...
  for (size_t i = 0; i < num_fragments; i++) {
    ...
    // Special-case the first fragment separately as it contains the basic header.
    if (i == 0) {
