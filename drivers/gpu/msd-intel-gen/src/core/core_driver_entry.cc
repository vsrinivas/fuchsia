...
static zx_intel_gpu_core_protocol_ops_t sysdrv_gpu_core_protocol_ops = {
...
static display_protocol_ops_t sysdrv_display_proto_ops = {
...
static zx_status_t sysdrv_gpu_core_ioctl(void* ctx, uint32_t op, const void* in_buf, size_t in_len,
                                         void* out_buf, size_t out_len, size_t* out_actual)
{
    DASSERT(false);
    return ZX_ERR_NOT_SUPPORTED;
...
static zx_protocol_device_t sysdrv_display_device_proto = {
    ...
    .ioctl = sysdrv_display_ioctl,
    .release = sysdrv_display_release,
};

static void sysdrv_gpu_core_release(void* ctx)
{
    // TODO(ZX-1170) - when testable:
...
static zx_protocol_device_t sysdrv_gpu_core_device_proto = {
    .version = DEVICE_OPS_VERSION,
    .ioctl = sysdrv_gpu_core_ioctl,
    .release = sysdrv_gpu_core_release,
};
...
static zx_status_t sysdrv_bind(void* ctx, zx_device_t* zx_device)
{
    ...
    args.proto_ops = &sysdrv_display_proto_ops;
    ...
    args.ctx = device.get();
    args.ops = &sysdrv_gpu_core_device_proto;
    args.proto_id = ZX_PROTOCOL_INTEL_GPU_CORE;
    args.proto_ops = &sysdrv_gpu_core_protocol_ops;
    ...
    DLOG("initialized magma display driver");
...
static int magma_start(sysdrv_device_t* device)
{
    ...
    device->magma_system_device = device->magma_driver->CreateDevice(device->core_device.get());
    if (!device->magma_system_device)
